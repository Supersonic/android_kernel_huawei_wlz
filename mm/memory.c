...
/*
 * __vm_normal_page -- This function gets the "struct page" associated with a pte.
 *
...
int do_swap_page(struct vm_fault *vmf)
{
	...
	if (!page) {
#ifdef CONFIG_SPECULATIVE_PAGE_FAULT
		if (vma_readahead && !(vmf->flags & FAULT_FLAG_SPECULATIVE))
#else
		if (vma_readahead)
#endif
			page = do_swap_page_readahead(entry,
				GFP_HIGHUSER_MOVABLE | ___GFP_CMA, vmf, &swap_ra);
		else
			page = swapin_readahead(entry,
				GFP_HIGHUSER_MOVABLE | ___GFP_CMA, vma, vmf->address);
		if (!page) {
			/*
...
static int do_anonymous_page(struct vm_fault *vmf)
{
	...
	if (!(vmf->flags & FAULT_FLAG_WRITE) &&
		...
		entry = pte_mkspecial(pfn_pte(my_zero_pfn(vmf->address),		//lint !e436
#ifdef CONFIG_SPECULATIVE_PAGE_FAULT	//lint !e436
						vmf->vma_page_prot));	//lint !e436
#else	//lint !e436
...
static int do_read_fault(struct vm_fault *vmf)
{
	...
	&& likely(!(current->flags & PF_WSCG_MONITOR))) {
#else
	if (vma->vm_ops->map_pages && fault_around_bytes >> PAGE_SHIFT > 1) {
#endif
...
static int do_cow_fault(struct vm_fault *vmf)
{
	...
	vmf->cow_page = alloc_page_vma(GFP_HIGHUSER_MOVABLE | ___GFP_CMA, vma, vmf->address);
	if (!vmf->cow_page)
		return VM_FAULT_OOM;
