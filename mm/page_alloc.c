...
retry:
	...
	if (unlikely(!page) && __rmqueue_fallback(zone, order, migratetype, gfp_flags))
		goto retry;
			...
			gfp_t gfp_flags, int migratetype)
{
	...
	pcp = &this_cpu_ptr(zone->pageset)->pcp;
	list = &pcp->lists[migratetype];
	page = __rmqueue_pcplist(zone,  migratetype, cold, pcp, list, gfp_flags);
	if (page) {
...
#ifdef	CONFIG_HIGHMEM /*lint !e436*/
		", %luK highmem"
#endif  /*lint !e436*/
		"%s%s)\n",
		       ...
		       unsigned migratetype, gfp_t gfp_mask)
{
	...
	pr_info("%s(): alloc migrate range:%lld us\n", __func__, t_ns/1000L);
#else
	if (t_ns/1000L > HISI_CMA_ALLOC_MAX_LATENCY_US)
		pr_err("%s(): migrate cost %lld us\n", __func__, t_ns/1000L);
#endif
	if (ret && ret != -EBUSY)
		goto done;
	ret =0;
	...
	t_ns = ktime_to_ns(ktime_sub(time_stop, time_start));
#ifdef CONFIG_CMA_DEBUG
	pr_info("%s(): drain all cost %lld us\n", __func__, t_ns/1000L);
#else
	if (t_ns/1000L > HISI_CMA_ALLOC_MAX_LATENCY_US)
		pr_err("%s(): drain cost %lld us\n", __func__, t_ns/1000L);
#endif
