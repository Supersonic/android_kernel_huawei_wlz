...
enum wireless_protocol_type {
	...
	WIRELESS_PROTOCOL_QI = WIRELESS_PROTOCOL_BEGIN, /* qi/wpc */
...
struct wireless_protocol_ops {
	...
	int (*send_tx_id)(u8 *, int);
	int (*send_cert_confirm)(bool);
	int (*send_charger_state)(u8);
	int (*send_fod_status)(int);
	int (*send_charger_mode)(u8);
	int (*set_fan_speed_limit)(u8);
	int (*set_rx_max_power)(u8);
...
struct wireless_dev {
	struct device *dev;
	struct wireless_protocol_info info;
...
static inline int wireless_send_tx_id(int prot, u8 *id, int len)
{
	return -1;
}
...
static inline int wireless_send_charger_state(int prot, u8 state)
{
	return -1;
}
...
static inline int wireless_send_charger_mode(int prot, u8 mode)
{
	return -1;
}
...
static inline int wireless_set_rx_max_power(int prot, u8 power)
{
	return -1;
}
...
static inline int wireless_show_tx_capability(int prot,
	struct wireless_protocol_tx_cap *cap)
{
	return -1;
}
