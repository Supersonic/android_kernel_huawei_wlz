...
#define DDR_REQUEST_VALUE_DOWN 3841
/* twice of DDR_REQUEST_VALUE_DOWN */
#define DDR_REQUEST_VALUE_UP 7682
#endif
/*
...
#endif
#define LITTLE_CORE_IO_IS_BUSY                                                 \
	"/sys/devices/system/cpu/cpu0/cpufreq/interactive/io_is_busy"
#define BIG_CORE_IO_IS_BUSY                                                    \
	"/sys/devices/system/cpu/cpu4/cpufreq/interactive/io_is_busy"

#ifdef HISI_DDR_FREQ_REQ
static s32 ddr_qos_update_request(
	s32 ddr_request_value, struct pm_qos_request *ddr_req)
{
	s32 new_value;
	int cur_ddr_band;

	if (BOOT_DEVICE_EMMC == get_bootdevice_type())
		return 0;
	...
	if (new_value != ddr_request_value) {
		...
		pr_debug("%s: block ddr freq request update to %d,curent ddr "
			 "band is %d\n",
...
static s32 ddr_qos_add_request(struct pm_qos_request *ddr_req)
{
	...
	pm_qos_add_request(
		ddr_req, PM_QOS_MEMORY_THROUGHPUT, ddr_request_value);
	pr_err("%s: block ddr freq request add.request_value = %d, curent ddr "
	       "band is %d\n",
		__func__, ddr_request_value, cur_ddr_band);
...
#endif
static long set_io_is_busy(const char *io_is_busy_path)
{
	long ret = 0;
...
#ifdef CONFIG_HISI_CPU_FREQ_GOV_SCHEDUTIL
	sched_set_io_is_busy(1);
#else
	fd_io_is_busy = sys_open(io_is_busy_path, O_WRONLY, 0660);
	if (fd_io_is_busy < 0) {
		/*
...
static long clear_io_is_busy(const char *io_is_busy_path)
{
	long ret = 0;
#ifndef CONFIG_HISI_CPU_FREQ_GOV_SCHEDUTIL
	long fd_io_is_busy;
...
{
	if (!set_io_is_busy(LITTLE_CORE_IO_IS_BUSY))
		pr_debug("%s little core io_is_busy is set,blk cpu freq req "
			 "add\n",
			__func__);
	...
	if (!set_io_is_busy(BIG_CORE_IO_IS_BUSY))
		pr_debug("%s big core io_is_busy is set,blk cpu freq req add\n",
			__func__);
}
...
{
	if (!clear_io_is_busy(LITTLE_CORE_IO_IS_BUSY))
		pr_debug("%s little core io_is_busy is clear,cpu_freq req "
			 "remove\n",
			__func__);
	...
	if (!clear_io_is_busy(BIG_CORE_IO_IS_BUSY))
		pr_debug(
			"%s big core io_is_busy is clear,cpu_freq req remove\n",
			__func__);
}
...
#ifdef HISI_DDR_FREQ_REQ
	.ddr_update_req = ddr_qos_update_request,
	.ddr_add_req = ddr_qos_add_request,
	.ddr_remove_request = ddr_qos_remove_request,
#else
	.ddr_update_req = NULL,
	.ddr_add_req = NULL,
...
void hisi_blk_freq_ctrl_init(void)
{
	...
	if (NULL == freq_ctrl_ptr) {
		...
		pr_err("%s: hisi blk freq ctrl init: "
		       "io_wait_time_threshold=%d, remove_req_time=%dms\n",
			__func__, IO_WAIT_TIME_THRESHOLD, REMOVE_REQ_TIME_MS);
#ifdef HISI_DDR_FREQ_REQ
		pr_err("%s: hisi block ddr frequence ctrl: "
		       "ddr_req_value_down=%d, ddr_req_value_up=%d\n",
			__func__, DDR_REQUEST_VALUE_DOWN, DDR_REQUEST_VALUE_UP);
#endif
		spin_lock_init(&freq_ctrl_ptr->lock);
		mutex_init(&freq_ctrl_ptr->m_lock);
		freq_ctrl_ptr->ddr_req = NULL;
	}
	...
	if (NULL == freq_ctrl_ptr->workqueue) {
		...
		INIT_DELAYED_WORK(
			&freq_ctrl_ptr->del_work, delete_freq_req_work);
	}
	goto out;
free_ddr_req:
	if (freq_ctrl_ptr->ddr_req) {
		kfree(freq_ctrl_ptr->ddr_req);
		freq_ctrl_ptr->ddr_req = NULL;
	}
