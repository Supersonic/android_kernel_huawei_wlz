...
#define BLK_BUSY_IDLE_NB_FLAG_NOT_JOIN_POLL                                    \
	(1 << BLK_BUSY_IDLE_NB_NOT_JOIN_POLL)
...
struct blk_queue_ops {
	...
	int (*mq_iosched_init_fn)(
		struct request_queue *); /* MQ Scheduler Init */
	int (*mq_iosched_exit_fn)(
		struct request_queue *); /* MQ Scheduler Deinit */
	int (*mq_req_alloc_prep_fn)(struct blk_mq_alloc_data *data,
		unsigned long ioflag, bool fs_submit); /*Prepare for MQ reqeust allocation */
	int (*mq_req_init_fn)(struct request_queue *, struct blk_mq_ctx *,
		struct request *); /* Request init in MQ */
	int (*mq_req_complete_fn)(struct request *, struct request_queue *,
		bool); /* Request complete in MQ */
	int (*mq_req_deinit_fn)(struct request_queue *,
		struct request *); /* Request deinit in MQ */
	int (*mq_req_insert_fn)(struct request *req, struct request_queue *,
		bool); /* Request insert to MQ */
	int (*mq_req_requeue_fn)(struct request *,
		struct request_queue *); /* Request requeue in MQ */
	int (*mq_req_timeout_fn)(
		struct request *, bool); /* Request timeout process in MQ */
	void (*mq_ctx_put_fn)(struct blk_mq_ctx *); /* Release the CTX in MQ */
	int (*mq_hctx_get_by_req_fn)(struct request *,
		struct blk_mq_hw_ctx **); /* Get hctx object by request */
	int (*mq_hctx_free_in_ctx_map_fn)(struct request_queue *,
		struct blk_mq_hw_ctx *); /* Free hctx during ctx map */
	unsigned int (*mq_tag_get_fn)(struct blk_mq_alloc_data *data); /* Get tag in MQ */
	int (*mq_tag_put_fn)(struct blk_mq_hw_ctx *, struct blk_mq_ctx *,
		unsigned int, struct request *); /* Release tag in MQ */
	int (*mq_tag_update_depth_fn)(struct request_queue *,
		struct blk_mq_tags *,
		unsigned int); /* Update the tag depth in MQ */
	int (*mq_tag_busy_iter_fn)(struct request_queue *, busy_iter_fn *,
		void *); /* iterate all the busy tag */
	int (*mq_tag_wakeup_all_fn)(
		struct blk_mq_tags *); /* wakeup all threads waiting tag */
	int (*mq_exec_queue_fn)(struct request_queue
			*); /* Execute queue function directly in MQ */
	int (*mq_run_hw_queue_fn)(
		struct request_queue *); /* Run hw queue in MQ */
	int (*mq_run_delay_queue_fn)(
		struct request_queue *); /* Run delay queue in MQ */
	int (*mq_run_requeue_fn)(
		struct request_queue *); /* Run requeue in MQ */
	int (*blk_poll_enable_fn)(
		struct request_queue *, blk_qc_t, bool *); /* Enable poll */
	void (*blk_status_dump_fn)(struct request_queue *, unsigned char *,
		enum blk_dump_scenario); /* Dump ioscheduler status*/
	void (*blk_req_dump_fn)(struct request_queue *, unsigned char *,
		enum blk_dump_scenario); /* Dump req status in ioscheduler */
...
struct blk_tagset_ops {
	struct blk_mq_tags * (*tagset_init_tags_fn)(unsigned int total_tags, unsigned int reserved_tags,
	unsigned int high_prio_tags, int node, int alloc_policy);			  /* MQ tagset init */
	void (*tagset_free_tags_fn)(struct blk_mq_tags *); /* MQ tagset free */
	void (*tagset_all_tag_busy_iter_fn)(struct blk_mq_tags *,
		busy_tag_iter_fn *,
		void *); /* iterate all the busy tag in whole tagset */
...
extern void hisi_blk_queue_bounce(
	struct request_queue *q, struct bio *bio_orig, struct bio *bio_cloned);
...
extern void hisi_blk_bio_merge_done(
	struct request_queue *q, struct request *req, struct request *next);
extern int hisi_blk_account_io_completion(
	struct request *req, unsigned int bytes);
...
extern void hisi_blk_request_execute_nowait(struct request_queue *q,
...
extern void hisi_blk_busy_idle_check_bio(
	struct request_queue *q, struct bio *bio);
extern bool hisi_blk_busy_idle_check_request_bio(
	struct request_queue *q, struct request *rq);
extern void hisi_blk_busy_idle_check_execute_request(
	struct request_queue *q, struct request *rq, rq_end_io_fn *done);
...
extern bool hisi_blk_flush_sync_dispatch(
	struct request_queue *q, struct bio *bio);
...
extern void hisi_blk_dump_queue_status(struct request_queue *q,
	unsigned char *prefix, enum blk_dump_scenario scenario);
extern unsigned long hisi_blk_dump_queue_status2(struct request_queue *q,
	char *out_buf, unsigned long length, bool need_print);
extern unsigned long hisi_blk_dump_lld_status(struct blk_dev_lld *lld,
	char *out_buf, unsigned long length, bool need_print);
...
extern bool hisi_blk_bio_is_cp(struct bio *bio);
#endif
...
extern ssize_t hisi_queue_busy_idle_enable_store(
	struct request_queue *q, const char *page, size_t count);
extern ssize_t hisi_queue_busy_idle_statistic_reset_store(
	struct request_queue *q, const char *page, size_t count);
extern ssize_t hisi_queue_busy_idle_statistic_show(
...
extern ssize_t hisi_queue_io_latency_tst_enable_store(
	struct request_queue *q, const char *page, size_t count);
extern ssize_t hisi_queue_busy_idle_tst_enable_store(
	struct request_queue *q, const char *page, size_t count);
extern ssize_t hisi_queue_busy_idle_multi_nb_tst_enable_store(
	struct request_queue *q, const char *page, size_t count);
extern ssize_t hisi_queue_busy_idle_tst_proc_result_simulate_store(
	struct request_queue *q, const char *page, size_t count);
extern ssize_t hisi_queue_busy_idle_tst_proc_latency_simulate_store(
	struct request_queue *q, const char *page, size_t count);
...
static inline void hisi_blk_dump_fs(struct bio *bio)
{
	if (bio->dump_fs)
		bio->dump_fs();
}
