...
static int __init hi3xxx_asp_codec_init(void)
{
	AUDIO_LOGI("hi3xxx asp codec init");
	return platform_driver_register(&hi3xxx_asp_codec_driver);
}

late_initcall(hi3xxx_asp_codec_init);

static void __exit hi3xxx_asp_codec_exit(void)
{
	platform_driver_unregister(&hi3xxx_asp_codec_driver);
}
module_exit(hi3xxx_asp_codec_exit);
