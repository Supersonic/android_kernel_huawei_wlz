...
#endif
static unsigned int debug_mask = 0x05;
#ifdef DEBUG_HISI_ISP
module_param_named(debug_mask, debug_mask, uint, S_IRUGO | S_IWUSR | S_IWGRP);
#endif
#define RPROC_INFO(fmt, args...) \
	do { \
		if (debug_mask & 0x01) \
			printk("Hisi rprocDrv Info: [%s] " \
				fmt, __func__, ##args); \
	} while (0)
#define RPROC_DEBUG(fmt, args...) \
	do { \
		if (debug_mask & 0x02) \
			printk("Hisi rprocDrv Debug: [%s] " \
				fmt, __func__, ##args); \
	} while (0)
#define RPROC_ERR(fmt, args...) \
	do { \
		if (debug_mask & 0x04) \
			printk("Hisi rprocDrv Dump: [%s] " \
				fmt, __func__, ##args); \
	} while (0)
...
#endif
extern struct iommu_domain *hicam_buf_get_domain_for_dev(void);
...
int hisp_rproc_enable_iommu(struct rproc *rproc, iommu_fault_handler_t handler)
{
	...
	if (rproc_dev->use_smmuv3_flag)
		domain = hicam_buf_get_domain_for_dev();
	else
...
#ifdef ISP_DEBUG_MMU_BYPASS
static struct sg_table *isp_mem_reserve_allocate(
			unsigned long length, unsigned int addr)
{
	...
	int alloc_pages_num          = (length / 0x100000);
	int page_num                 = 0;

	INIT_LIST_HEAD(&pages_list);

	for (page_num = 0; page_num < alloc_pages_num; page_num++) {
		page_info = kzalloc(sizeof(struct rproc_page_info), GFP_KERNEL);
		if (!page_info) {
			pr_err("%s: alloc rproc_page_info fail\n", __func__);
			goto free_pages;
		}
		page_info->order = 8;/* 1M */
		page_info->page = phys_to_page(addr  + (page_num * 0x100000));
		if (!page_info->page) {
...
#ifdef ISP_DEBUG_MMU_BYPASS
	ret = memset_s(vaddr, length, 0, length);
	if (ret != 0)
		pr_err("%s: memset_s vaddr to 0.%d!\n", __func__, ret);
#endif
	return vaddr;
}
...
void virtqueue_sg_init(struct scatterlist *sg,
			void *va, dma_addr_t dma, int size)
{
				...
				dma_addr_t dma, int total_space)
{
	...
	struct rproc_mem_entry *vringmapping[RVDEV_NUM_VRINGS];
...
int hisp_rproc_rsctable_init(void)
{
	...
	rproc_dev->rsctable_vaddr =
		kmemdup(rproc_dev->isp_bin_vaddr + 0x3000, 0x1000, GFP_KERNEL);
	if (rproc_dev->rsctable_vaddr == NULL) {
...
int hisi_firmware_load_func(struct rproc *rproc)
{
	...
	rproc_auto_boot_callback(NULL, rproc);
...
void set_shared_mdc_pa_addr(u64 mdc_pa_addr)
{
	...
	hisp_lock_sharedbuf();
	if (use_sec_isp())
		share_para = (struct rproc_shared_para *)getsec_a7sharedmem_addr();
	else if (use_nonsec_isp())
		share_para = (struct rproc_shared_para *)get_a7sharedmem_va();
...
int hispmdc_map_fw(unsigned long iova, phys_addr_t paddr, size_t size, int prot)
{
	...
	if (!domain)
		return -EINVAL;
	pr_err("[%s] :iova.0x%lx, paddr.0x%lx, size.0x%lx, prot.0x%x, ret.%d\n",
...
int hispmdc_unmap_fw(unsigned long iova, size_t size)
{
	...
	pr_info("[%s] + : addr.0x%llx, size.0x%lx", __func__, iova, size);
		...
		int offset, int avail)
{
	...
	printk("[%s] Firmware_version: magic = %x, module = %s, version = %s\n",
			__func__, rsc->magic, rsc->module, rsc->version);
	printk("[%s] Firmware_version: build_time = %s, reserved = %s.\n",
		__func__, rsc->build_time, rsc->reserved != 0?rsc->reserved:"NULL");
 ...
 */
/*lint -save -e429 */
int rproc_handle_cda(struct rproc *rproc, struct fw_rsc_hdr *hdr,
		int offset, int avail)
{
	struct rproc_mem_entry *cda = NULL;
	struct device *dev = NULL;
	struct fw_rsc_cda *rsc = NULL;
	void *ptr = NULL;
	char name[15];
	int ret = 0;
	...
	rsc = (struct fw_rsc_cda *)((void *)hdr + sizeof(*hdr));
	...
	ret = snprintf_s(name, sizeof(name), sizeof(name) - 1, "cda%d",
				rproc->num_cdas);
	if (ret < 0) {
		dev_err(dev, "snprintf_s cda name failed.%d\n", ret);
		kfree(cda);
		cda = NULL;
		return ret;
	}
...
int rproc_handle_dynamic_memory(struct rproc *rproc,
		...
		int offset, int avail)
{
	struct rproc_mem_entry *dynamic_mem = NULL, *mapping = NULL;
...
#ifdef ISP_DEBUG_MMU_BYPASS
	pr_info("%s: isp_mem_reserve_allocate +.\n", __func__);
	if (!strncmp(rsc->name, "ISP_MEM_DATA", strlen(rsc->name)))
		table = isp_mem_reserve_allocate(rsc->len, DATA_BASE);
	else
		table = isp_mem_reserve_allocate(rsc->len, TEXT_BASE);
	pr_info("%s: isp_mem_reserve_allocate -.\n", __func__);
#else
...
int rproc_handle_reserved_memory(struct rproc *rproc,
		...
		int offset, int avail)
{
	struct rproc_mem_entry *reserved_mem = NULL, *mapping = NULL;
	struct rproc_cache_entry *cache_entry = NULL;
	struct device *dev = NULL;
...
#ifdef ISP_DEBUG_MMU_BYPASS
	a7sharedmem_addr = (u64)ISP_DEBUG_SHARE_ADDR;
#else
			...
			size_t addr, size_t size, unsigned int prot)
{
	...
	if ((rproc_dev->isp_rproc == NULL) ||
	   (rproc_dev->isp_rproc->rproc == NULL)) {
...
static void rproc_set_shared_clk_init(void)
{
	...
	if (ret != 0)
		pr_err("%s:memset_s share_para->clk_value to 0 failed.ret\n",
				__func__, ret);
	hisp_unlock_sharedbuf();
}
...
int use_default_isp(void)
{
	struct rproc_boot_device *dev = &rproc_boot_dev;

	return ((dev->case_type == DEFAULT_CASE) ? 1:0);
}
...
int hisp_set_clk_rate(unsigned int type, unsigned int rate)
{
	...
	case NONSEC_CASE:
		ret = nsec_setclkrate(type, rate);
		if (ret < 0)
			pr_err("[%s] Failed : nsec_setclkrate.%d\n",
				__func__, ret);
		break;
	case SEC_CASE:
		ret = sec_setclkrate(type, rate);
		if (ret < 0)
...
int _stat_poll(void __iomem *addr, unsigned int value,
		void __iomem *stat_addr, unsigned int cond)
{
	unsigned int stat;
	int          timeout;
...
static int dump_a7boot_stone(void)
{
	...
	RPROC_ERR("FIRMWARE::0x%x, OS::0x%x\n",param->bw_stat, param->fw_stat);
...
void dump_media1_regs(void)
{
	...
	pr_info("[%s] +\n", __func__);
	ret = get_media1_subsys_power_state();
	if (ret == 0) {
...
static void dump_ispcpu_stone(void)
{
#ifdef DEBUG_HISI_ISP
	struct rproc_shared_para *param      = NULL;
	struct hisi_ispcpu_stone_s *cpu_info = NULL;
	...
	cpu_info = (struct hisi_ispcpu_stone_s *)((unsigned char *)param + ISPCPU_STONE_OFFSET);
	if(cpu_info->type <= ISPCPU_EXC_TYPE_MAX) {
		RPROC_ERR("uwExcType.%x : %s\n", cpu_info->type, ispcpu_exc_type[cpu_info->type]);
		}
	RPROC_ERR("R0    %08x        R8    %08x\n", cpu_info->uw_r0, cpu_info->uw_r8);
	RPROC_ERR("R1    %08x        R9    %08x\n", cpu_info->uw_r1, cpu_info->uw_r9);
	RPROC_ERR("R2    %08x        R10   %08x\n", cpu_info->uw_r2, cpu_info->uw_r10);
	RPROC_ERR("R3    %08x        R11   %08x\n", cpu_info->uw_r3, cpu_info->uw_r11);
	RPROC_ERR("R4    %08x        R12   %08x\n", cpu_info->uw_r4, cpu_info->uw_r12);
	RPROC_ERR("R5    %08x        SP    %08x\n", cpu_info->uw_r5, cpu_info->uw_sp);
	RPROC_ERR("R6    %08x        LR    %08x\n", cpu_info->uw_r6, cpu_info->uw_lr);
	RPROC_ERR("R7    %08x        PC    %08x\n", cpu_info->uw_r7, cpu_info->uw_pc);
	RPROC_ERR("SPSR  %08x        CPSR  %08x\n", cpu_info->uw_cpsr, cpu_info->uw_cpsr);
	RPROC_ERR("DFSR  %08x        DFAR  %08x\n", cpu_info->dfsr, cpu_info->dfar);
	RPROC_ERR("IFSR  %08x        IFAR  %08x\n", cpu_info->ifsr, cpu_info->ifar);
	RPROC_ERR("SMMU_FAR_H %08x   SMMU_FAR_L  %08x\n", cpu_info->smmu_far_high, cpu_info->smmu_far_low);
	RPROC_ERR("SMMU_FSR   %08x   SMMU_FSYNR  %08x\n", cpu_info->smmu_fsr, cpu_info->smmu_fsynr0);
	hisp_unlock_sharedbuf();
#else
	return;
#endif
...
static int cpu_debug_init(void)
{
	...
	__raw_writel(0xC5ACCE55, (volatile void __iomem *)(cssys_base + DBGLAR));
	__raw_writel(0x00000000, (volatile void __iomem *)(cssys_base + DBGOSLAR));
	__raw_writel(0xC5ACCE55, (volatile void __iomem *)(cssys_base + PMLAR));
	__raw_writel(0x00000007, (volatile void __iomem *)(cssys_base + PMCR));
	__raw_writel(0x00000008, (volatile void __iomem *)(cssys_base + PMXEVTYPER0));
	__raw_writel(0x80000001, (volatile void __iomem *)(cssys_base + PMCNTENSET));
...
static int dis_reset_a7(struct rproc_boot_device *dev)
{
	...
	ret = dis_rststat_poll(addr, value);
	if (ret != 0)
...
static void remap_a7_entry(struct rproc_boot_device *dev)
{
	...
	__raw_writel((unsigned int)(addr >> 4) | ISPA7_VINITHI_HIGH |
			ISPA7_REMAP_ENABLE | ISPA7_DBGPWRDUP,
			(volatile void __iomem *)(dev->reg[CRGPERI] +
			CRG_C90_PERIPHISP_ISPA7_CTRL_BASE));
}
...
static int isp_device_disable(void)
{
	...
	case NONSEC_CASE:
		...
		break;
	default:
		ret = default_isp_device_disable();
		if (ret < 0)
...
#define TIMEOUT_ISPLOG_START (10)
	...
	case NONSEC_CASE:
		...
		break;
	default:
		ret = default_isp_device_enable();
		if (ret < 0)
			RPROC_ERR("Failed : default_isp_enable.%d\n", ret);
		break;
	}
	dev->ispcpu_status = 1;
	RPROC_INFO("-\n");
...
static void isp_mbox_rx_work(void)
{
	...
	while (kfifo_len(&isp_rx_mbox->rpmsg_rx_fifo) >= sizeof(struct rx_buf_data)) {
		...
		irqret = rproc_vq_interrupt(hisi_rproc, mbox_reg.rpmsg_rx_buf[0]);
		if (irqret == IRQ_NONE)
...
#define PART_ISPFW_SIZE 0x00E00000
	...
	if ((hisi_rproc != NULL) && (hisi_rproc->firmware != NULL)) {
		ret = snprintf_s(pathname, sizeof(pathname), sizeof(pathname) - 1,
				"/system/vendor/firmware/%s", hisi_rproc->firmware);
		if (ret < 0) {
			RPROC_ERR("snprintf_s firmware name fail.%d", ret);
			return ret;
		}
	} else {
		ret = snprintf_s(pathname, sizeof(pathname), sizeof(pathname) - 1,
		...
		if (ret < 0) {
			RPROC_ERR("snprintf_s firmware name failed.%d", ret);
			return ret;
		}
	}
...
static int hisp_elf_load_segments(const u8 *data, void *dst, int fw_size)
{
	...
	for (i = 0; i < ehdr->e_phnum; i++, phdr++) {
		...
		if (phdr->p_filesz) {
			...
			if (ret != 0) {
				RPROC_ERR("memcpy_s ptr elf_data fail.%d\n", ret);
				break;
			}
		}
		 ...
		 */
		if (memsz > filesz) {
			ret = memset_s(ptr + filesz, memsz - filesz, 0, memsz - filesz);
			if (ret != 0) {
				RPROC_ERR("memset_s ptr + filesz to 0 fail.%d\n", ret);
				break;
			}
		}
	}
...
static int isp_elf_load(void)
{
	...
	ret = ispupdate_elf_load((char **)&isp_elf_vaddr, &length);
	if (ret < 0) {
	...
	ret = hisp_elf_load_segments(isp_elf_vaddr,
				rproc_dev->isp_bin_vaddr, length);
	if (ret < 0) {
		RPROC_ERR("Failed : hisp_elf_load_segments\n");
		RPROC_ERR("isp_elf_vaddr.0x%pK\n", isp_elf_vaddr);
		RPROC_ERR("isp_bin_vaddr.0x%pK\n", rproc_dev->isp_bin_vaddr);
		RPROC_ERR("length.0x%x\n", length);
	}
	vfree(isp_elf_vaddr);
...
#define MEM_BOOTWARE_SIZE              0xF000
	...
	/* put the segment where the remote processor expects it */
	ret = memcpy_s(ptr, memsz, rproc_dev->isp_bin_vaddr, memsz);
	if (ret != 0) {
		RPROC_ERR("memcpy_s text vaddr to ptr fail.%d\n", ret);
		return ret;
	}
	...
	ret = memcpy_s(ptr, memsz,
			rproc_dev->isp_bin_vaddr + ISP_TEXT_SIZE, memsz);
	if (ret != 0) {
		RPROC_ERR("memcpy_s data vaddr to ptr fail.%d\n", ret);
		return ret;
	}
	...
	ret = memcpy_s(ptr, memsz, rproc_dev->isp_bin_vaddr, memsz);
	if (ret != 0) {
		RPROC_ERR("memcpy_s bw vaddr to ptr fail.%d\n", ret);
		return ret;
	}
...
static unsigned int hisp_get_isplogic(void)
{
	...
	ret = of_property_read_u32(np, "hisi,isplogic", (unsigned int *)(&isplogic));
	if (ret < 0) {
...
#define BOARDID_SIZE (4)
	...
	ret = of_property_read_u32_array(root, "hisi,boardid", boardid, BOARDID_SIZE);
	if (ret < 0) {
...
static struct hisi_rproc_data *hisi_rproc_data_dtget(struct device *pdev)
{
	...
	void *va = NULL;
	unsigned int share_size     = 0x1000;
	u64 a7sharedmem_addr        = 0;
	int ret;
	...
	ret = of_property_read_string(np, "isp-names", &name);
	if (ret < 0) {
	...
	ret = of_property_read_string(np, "firmware-names", &name);
	if (ret < 0) {
	...
	ret = of_property_read_string(np, "bootware-names", &name);
	if (ret < 0) {
	...
	ret = of_property_read_string(np, "mailbox-names", &name);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "isp-ipc-addr", &addr);
	if (ret < 0) {
	...
	data->ipc_addr = addr;
#ifdef ISP_DEBUG_MMU_BYPASS
	data->ipc_addr = ISP_RPMSG_VQ_BASE;
#endif
	...
	if (use_default_isp()) {
		a7sharedmem_addr = get_a7sharedmem_addr();
		va = ioremap_wc(a7sharedmem_addr, share_size);
		if (va == NULL) {
			RPROC_ERR("ioremap failed a7sharedmem_addr, size.0x%x\n", share_size);
			return NULL;
		}
		ret = memset_s(va, share_size, 0, share_size);
		if (ret != 0) {
			pr_err("[%s] Failed : memset_s va to 0.%d\n", __func__, ret);
			return NULL;
		}
		iounmap(va);
	}
	...
	ret = of_property_read_u32(np, "ispa7-default-clk", &ispa7_clk);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "isppd-adb-flag", &isppd_adb_flag);
	if (ret < 0) {
	...
	if (ret < 0) {
		RPROC_ERR("Failed : hisp_qos_dtget ret.%d\n", ret);
		return NULL;
	}
...
void hisi_rproc_data_dtput(struct device *pdev)
{
	hisp_qos_free();
}
...
void coresight_mem_init(struct device *dev)
{
	...
	addr_vir = dma_alloc_coherent(dev, 0x1000, &dma_addr, GFP_KERNEL);
	if (addr_vir == NULL) {
	...
	pr_err("[coresight] [%s] coresight_mem_init  addr_vir:%pK addr_ph:%pK\n",
		__func__, (void *)(uintptr_t)addr_vir, (void *)(uintptr_t)dma_addr);

	g_cs_para.coresight_addr_da = (unsigned long long)(uintptr_t)dma_addr;
	g_cs_para.coresight_addr_vir = (unsigned long long)(uintptr_t)addr_vir;

	pr_err("[coresight] coresight_mem_init  addr_vir:%llx addr_ph:%llx\n",
		g_cs_para.coresight_addr_vir, g_cs_para.coresight_addr_da);
}

void coresight_mem_exit(struct device *dev)
{
	if (g_cs_para.coresight_addr_vir == 0)
		return;

	dma_free_coherent(dev, 0x1000, (void *)(uintptr_t)g_cs_para.coresight_addr_vir,
				g_cs_para.coresight_addr_da);
}
...
static int get_vring_dma_addr_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < HISI_ISP_VRING_NUM; i++) {
		...
		switch (i) {
		case VRING0:
			hisi_isp_vring->paddr = (u64)ISP_RPMSG_VR0_BASE;
			break;
		case VRING1:
			hisi_isp_vring->paddr = (u64)ISP_RPMSG_VR1_BASE;
			break;
		case VIRTQUEUE:
			hisi_isp_vring->paddr = (u64)ISP_RPMSG_VQ_BASE;
			break;
		default:
			break;
		}
		pr_info("%s: i.%d, paddr.0x%llx, size.0x%x\n", __func__,
				hisi_isp_vring->paddr, hisi_isp_vring->size);
		hisi_isp_vring->virt_addr = ioremap_wc(hisi_isp_vring->paddr, hisi_isp_vring->size);
		pr_info("%s: i.%d ioremap_wc end!\n", __func__);
#else
...
#ifdef ISP_DEBUG_MMU_BYPASS
		iounmap(hisi_isp_vring->virt_addr);
#else
...
static int hisp_regaddr_init(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "reg-num", (unsigned int *)(&rproc_dev->reg_num));
	if (ret < 0) {
	...
	for (i = 0; i < HISP_MIN(HISP_MAX_REGTYPE, rproc_dev->reg_num); i++) {/*lint !e574 */
		rproc_dev->r[i] = platform_get_resource(pdev, IORESOURCE_MEM, i);
		...
		rproc_dev->reg[i] = (void __iomem *)ioremap(rproc_dev->r[i]->start,
							resource_size(rproc_dev->r[i]));
...
static int hisi_rproc_probe(struct platform_device *pdev)
{
	...
	rproc_dev->isp_bin_vaddr = NULL;
	rproc_dev->rsctable_vaddr = NULL;
	ret = memset_s(&rproc_dev->hisi_isp_clk, sizeof(struct hisi_isp_clk_dump_s),
			0, sizeof(struct hisi_isp_clk_dump_s));
	if (ret != 0) {
		pr_err("[%s] Failed : memset_s rproc_dev->hisi_isp_clk.%d\n",
				__func__, ret);
		return ret;
	}
	...
	if (ret != 0) {
		...
		goto hisi_isp_rproc_case_set_fail;
	}

	ret = of_property_read_u32(np, "hisi,use_secisp", &rproc_dev->secisp);
	if (ret < 0) {/*lint !e64 */
	...
	ret = of_property_read_u32(np, "useloadbin", &rproc_dev->loadbin);
	if (ret < 0) {/*lint !e64 */
	...
	ret = of_property_read_u32(np, "use-smmuv3-flag", &rproc_dev->use_smmuv3_flag);
	if (ret < 0) {/*lint !e64 */
		RPROC_ERR("Failed: use_smmuv3_flag.0x%x of_property_read_u32.%d\n",
				rproc_dev->use_smmuv3_flag, ret);
		goto hisi_isp_rproc_case_set_fail;
	}
	...
	data = hisi_rproc_data_dtget(dev);
	if (data == NULL) {
	...
	if (client_isp == NULL) {
		...
		if (client_isp == NULL) {
			RPROC_ERR("Failed : dsm_register_client fail\n");
			goto dsm_register_client_fail;
		}
	}
...
ssize_t ispbinupdate_show(struct device *pdev, struct device_attribute *attr, char *buf)
{
	...
	ret = ispupdate_bin_load(rproc_dev->isp_bin_vaddr);
	if (ret < 0) {
