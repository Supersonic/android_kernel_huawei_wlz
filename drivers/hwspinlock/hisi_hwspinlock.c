...
static int hisi_hwspinlock_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(
		np, "hwlock,register-width", (u32 *)&register_width);
	if (ret) {
	...
	ret = of_property_read_u32(
		np, "hwlock,bits-per-single", (u32 *)&lock_bits_g);
	if (ret) {
	...
	addr = devm_kzalloc(
		dev, lock_groups * sizeof(unsigned int), GFP_KERNEL);
	if (!addr) {
		ret = -ENOMEM;
		goto addr_err;
	}

	hwspinlock_info = devm_kzalloc(
		dev, num_lock * sizeof(*hwspinlock_info), GFP_KERNEL);
	if (!hwspinlock_info) {
		ret = -ENOMEM;
	...
	for (i = 0, hwlock = &bank->lock[0]; i < num_lock;
		i++, hwlock++, hwspinlock_info++) {
		hwspinlock_info->id_in_group = i % locks_per_register;
		hwspinlock_info->address =
			hwspinlock_base + addr[i / locks_per_register];
