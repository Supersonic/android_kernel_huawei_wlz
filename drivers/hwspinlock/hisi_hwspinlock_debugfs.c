 ...
 * but trylock timout maybe cause the atomic question because of sys_write
 * is atomic
...
static ssize_t debugfs_write(
	struct file *filp, const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	if ((ubuf == NULL) || (cnt == 0) || (cnt > ARRAY_SIZE(buf))) {
		pr_err("buf is null !\n");
		return (-EINVAL);
	}
	if (cnt > sizeof(buf)) {
