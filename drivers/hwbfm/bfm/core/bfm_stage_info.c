...
static bfm_stages_t *stages_info;
...
void bfm_add_stage_info(bfm_stage_info_t *pstage)
{
	...
	if (cur_stage_index < BMFR_MAX_BOOTUP_STAGE_COUNT) {
		bfm_stage_info_t *current_stage_node = (bfm_stage_info_t *) &
...
int bfm_init_stage_info(void)
{
	...
	if (stages_info == NULL) {
		stages_info = bfmr_malloc(sizeof(bfm_stages_t));
		if (stages_info == NULL) {
			BFMR_PRINT_KEY_INFO("malloc stages_info failed\n");
			mutex_unlock(&record_stage_mutex);
			return -1;
		}
		memset((void *)stages_info, 0, sizeof(bfm_stages_t));
	}
