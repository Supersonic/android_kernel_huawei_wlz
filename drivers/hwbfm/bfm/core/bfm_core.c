...
#define BFM_BASIC_LOG_MAX_LEN \
	(BFM_BOOTFAIL_INFO_LOG_MAX_LEN + BFM_RECOVERY_INFO_LOG_MAX_LEN)
#define BFM_BFMR_TEMP_BUF_LOG_MAX_LEN \
	(BFM_BOOLTOADER1_LOG_MAX_LEN + BFM_BOOLTOADER2_LOG_MAX_LEN + \
...
#define BFM_SELF_HEAL_CHAIN_MAX_LEN 256
...
static long long bfm_get_basic_space(bfmr_bootfail_errno_e bootfail_errno);
static long long bfm_get_extra_space(bfm_process_bootfail_param_t *pparam);
static int bfm_save_extra_bootfail_logs(bfmr_log_dst_t *pdst,
		bfmr_log_src_t *psrc, bfm_process_bootfail_param_t *pparam);
...
static int bfm_save_bootfail_info_txt(bfmr_log_dst_t *pdst,
		bfmr_log_src_t *psrc,
		bfm_process_bootfail_param_t *pparam);
static void bfm_save_recovery_info_txt(bfmr_log_dst_t *pdst,
		bfmr_log_src_t *psrc,
		bfm_process_bootfail_param_t *pparam);
...
/*
 * @function: static int bfmr_capture_and_save_log(bfmr_log_type_e type,
 * bfmr_log_dst_t *dst)
 ...
 */
static int bfmr_capture_and_save_log(bfmr_log_src_t *src,
		bfmr_log_dst_t *dst, bfm_process_bootfail_param_t *pparam);
...
static bool bfm_is_user_sensible_boot_fail(
		bfm_bootfail_log_info_t *pbootfail_log_info);
static void bfm_merge_bootfail_logs(bfm_bootfail_log_info_t *pbootfail_log_info);
static void bfm_delete_user_unsensible_logs(
		bfm_bootfail_log_info_t *pbootfail_log_info);
static int bfm_traverse_log_root_dir(bfm_bootfail_log_info_t *pbootfail_log_info);
static void bfm_user_space_process_read_its_own_file(
		bfm_process_bootfail_param_t *pparam);
...
static bfmr_recovery_method_maptable_t bfmr_rmethod_maptable[] = {
...
struct bfm_log_size_param log_size_param[] = {
	{
		...
		BFM_BOOLTOADER1_LOG_MAX_LEN + BFM_BASIC_LOG_MAX_LEN
	}, {
		(BL2_STAGE << BFM_BOOT_STAGE_SHIFTING),
		BFM_BOOLTOADER1_LOG_MAX_LEN +
			BFM_BOOLTOADER2_LOG_MAX_LEN + BFM_BASIC_LOG_MAX_LEN
	}, {
...
static struct bfm_boot_fail_no_desc bootfail_errno_desc[] = {
...
static bfmr_bootfail_errno_e sensible_bootfail_with_reboot_recovery[] = {
	KERNEL_PRESS10S,
	KERNEL_BOOT_TIMEOUT,
...
static bfmr_selfheal_code_e bfm_get_selfheal_code(
		bfr_recovery_method_e recovery_method,
		bfmr_detail_boot_stage_e boot_stage)
{
	int i = 0;
	int count = ARRAY_SIZE(bfmr_rmethod_maptable);

	if (boot_stage == STAGE_BOOT_SUCCESS)
		boot_stage = ANDROID_FRAMEWORK_STAGE_START;

	for (i = 0; i < count; i++) {
		if (recovery_method == bfmr_rmethod_maptable[i].recovery_method &&
		    ((unsigned int)boot_stage >> BFM_SELF_HEAL_STAGE_OFF_LEN) ==
			((unsigned int)bfmr_rmethod_maptable[i].boot_stage >>
			  BFM_SELF_HEAL_STAGE_OFF_LEN))
			return bfmr_rmethod_maptable[i].selfheal_code;
	}

	return SH_UNKNOW;
}

static void bfm_update_bootfail_logs_to_file(
		bfm_bootfail_log_info_t *bf_log_info,
		char *pdata, long pdata_len, long src_file_len)
{
	...
	log_dir_cnt = bf_log_info->log_dir_count;
	if (log_dir_cnt == 0) {
	...
	snprintf(start_time, sizeof(start_time) - 1, "%u",
		 bf_log_info->real_recovery_info.boot_fail_rtc_time[log_dir_cnt - 1]);
	...
	if (ptemp == NULL) {
		...
		if (buf_len != 0)
			bfmr_save_log(bf_log_info->bootfail_logs[0].log_dir,
...
static void bfm_update_bootfail_logs(bfm_bootfail_log_info_t *pbootfail_log_info)
{
	...
	if (pbootfail_log_info->log_dir_count == 0) {
		BFMR_PRINT_ERR("log_dir_count=0!\n");
		return;
	}
	...
	bfm_update_bootfail_logs_to_file(pbootfail_log_info, pdata, buf_len,
					 src_file_len);
...
{
	int i = 0;
	int count = ARRAY_SIZE(bootfail_errno_desc);

	for (i = 0; i < count; i++) {
		if (bootfail_errno == bootfail_errno_desc[i].bootfail_errno)
			return bootfail_errno_desc[i].desc;
	}
...
static int save_bootfail_info_to_dst(bfmr_log_dst_t *pdst, bfmr_log_src_t *psrc,
		bfm_process_bootfail_param_t *pparam, char *log, size_t log_len)
{
	int ret;
...
static size_t format_bootfail_info(bfm_process_bootfail_param_t *pparam,
		char *out_data, int out_len)
{
	size_t bytes_formatted;
	char record_count_str[BFM_MAX_INT_NUMBER_LEN] = {'\0'};
	int ret = bfmr_wait_for_part_mount_with_timeout("/data",
		...
		bfmr_log_src_t *psrc, bfm_process_bootfail_param_t *pparam)
{
	...
	bytes_formatted = format_bootfail_info(pparam, pdata,
						BFMR_TEMP_BUF_LEN);

	ret = save_bootfail_info_to_dst(pdst, psrc, pparam, pdata,
					bytes_formatted);
...
static int bfm_get_recovery_result(
...
static void save_recovery_info_to_dst(bfmr_log_dst_t *pdst,
		bfmr_log_src_t *psrc,
		bfm_process_bootfail_param_t *pparam,
		char *pdata)
{
...
static void format_recovery_info_for_save(bfm_process_bootfail_param_t *pparam,
		char *pdata)
{
	char record_count_str[BFM_MAX_INT_NUMBER_LEN] = {'\0'};
	bfmr_selfheal_code_e selfheal_code;

	selfheal_code = bfm_get_selfheal_code(pparam->recovery_method,
					      pparam->boot_stage);
	...
	(void)snprintf(pdata, BFMR_TEMP_BUF_LEN - 1,
		       BFM_RCV_FILE_CONTENT_FORMAT,
		       record_count_str,
		       bfr_get_recovery_method_desc(pparam->recovery_method),
		       record_count_str,
		       (bfm_get_recovery_result(pparam->suggested_recovery_method) == BFM_RECOVERY_SUCCESS_INT_VALUE) ?
				BFM_RECOVERY_SUCCESS_STR : BFM_RECOVERY_FAIL_STR,
		       record_count_str, selfheal_code,
		       record_count_str,
		...
		bfm_process_bootfail_param_t *pparam)
{
	...
	format_recovery_info_for_save(pparam, pdata);

	save_recovery_info_to_dst(pdst, psrc, pparam, pdata);
...
static long long bfm_get_basic_space(bfmr_bootfail_errno_e bootfail_errno)
{
	...
	return (long long)(BFM_BOOTFAIL_INFO_LOG_MAX_LEN +
				BFM_RECOVERY_INFO_LOG_MAX_LEN);
}
...
static int prepare_for_save_log(bfmr_log_dst_t *dst, bfmr_log_src_t *src,
		bfm_process_bootfail_param_t *pparam)
{
...
static void bfm_save_bootfail_runtime_log(bfmr_log_dst_t *dst,
		bfmr_log_src_t *src, bfm_process_bootfail_param_t *pparam)
{
...
static int bfm_capture_and_save_bootfail_log(
		bfm_process_bootfail_param_t *pparam)
{
	int ret;
	bfmr_log_dst_t dst;
...
static int bfm_process_upper_layer_boot_fail(void *param)
{
	bfm_process_bootfail_param_t *pparam = param;
...
static bfmr_detail_boot_stage_e bfm_correct_boot_stage(
		bfmr_detail_boot_stage_e boot_stage,
		bfmr_bootfail_errno_e bootfail_errno)
{
	...
	bfmr_bootfail_addl_info_t *paddl_info)
{
	...
	pparam = bfmr_malloc(sizeof(bfm_process_bootfail_param_t));
	if (pparam == NULL) {
		BFMR_PRINT_ERR("bfmr_malloc failed!\n");
		goto __out;
	}
	...
	pparam->is_bootup_successfully = is_bootup_successfully;
	if (paddl_info != NULL) {
	...
	pparam->capture_and_save_bootfail_log =
		bfm_capture_and_save_bootfail_log;
...
int bfmr_set_boot_stage(bfmr_detail_boot_stage_e boot_stage)
{
	...
	if (is_bootup_successfully == false) {
...
static int bfm_update_bootfail_info_for_each_log(
		bfm_bootfail_log_info_t *pbootfail_log_info)
{
	bfm_update_bootfail_logs(pbootfail_log_info);
	return 0;
}

static void format_recovery_info_for_update(char *pdata,
		bfr_real_recovery_info_t *rinfo, int record_count)
{
	int i;
	char record_count_str[BFM_MAX_INT_NUMBER_LEN] = {'\0'};
	char time[BFM_MAX_INT_NUMBER_LEN] = {'\0'};
	int bytes_formatted = 0;
	bfmr_selfheal_code_e selfheal_code = SH_UNKNOW;
	char selfheal_chain_str[BFM_SELF_HEAL_CHAIN_MAX_LEN] = {'\0'};
	int selfheal_chain_len = 0;

	for (i = 0; i < record_count; i++) {
		selfheal_code = bfm_get_selfheal_code(rinfo->recovery_method[i],
						      rinfo->boot_fail_stage[i]);
		selfheal_chain_len += snprintf(selfheal_chain_str +
						selfheal_chain_len,
						BFM_SELF_HEAL_CHAIN_MAX_LEN - selfheal_chain_len - 1,
						"%d, ", selfheal_code);
		...
		bytes_formatted += snprintf(pdata + bytes_formatted,
			...
			(record_count == 1) ?
				("%s" BFM_RCV_FILE_CONTENT_FORMAT) :
				(BFM_LOG_SEPRATOR BFM_RCV_FILE_CONTENT_FORMAT),
			(record_count == 1) ? ("") : (time),
			record_count_str,
			bfr_get_recovery_method_desc(rinfo->recovery_method[i]),
			record_count_str,
			(((record_count == (i + 1)) ||
			  (bfmr_is_boot_success(rinfo->boot_fail_stage[i]))) ?
				BFM_RECOVERY_SUCCESS_STR :
				BFM_RECOVERY_FAIL_STR),
			record_count_str, selfheal_code,
			record_count_str,
			(((record_count == (i + 1)) ||
			  (bfmr_is_boot_success(rinfo->boot_fail_stage[i]))) ?
				(BFM_RECOVERY_SUCCESS_INT_VALUE) :
				(BFM_RECOVERY_FAIL_INT_VALUE)),
			record_count_str,
			(((record_count == (i + 1)) ||
			  (bfmr_is_boot_success(rinfo->boot_fail_stage[i]))) ?
				selfheal_chain_str : ""));
...
static int bfm_update_recovery_info_for_each_log(
		bfm_bootfail_log_info_t *pbootfail_log_info)
{
	char *pdata = NULL;
	char c = '\0';
	...
	format_recovery_info_for_update(pdata, 
			&(pbootfail_log_info->real_recovery_info),
			pbootfail_log_info->real_recovery_info.record_count);

	/* save recovery info */
	if (bfmr_save_log(pbootfail_log_info->bootfail_logs[0].log_dir,
			  BFM_RECOVERY_FILE_NAME,
	...
	bfmr_save_log(pbootfail_log_info->bootfail_logs[0].log_dir,
		      BFM_DONE_FILE_NAME, (void *)&c, sizeof(char), 0);
...
static bool is_user_sensible_rmethod(
		bfm_bootfail_log_info_t *pbootfail_log_info)
{
	int i;
	bfr_real_recovery_info_t *rinfo = &(pbootfail_log_info->real_recovery_info);
...
static bool is_user_sensible_boot_fail_no(
		bfm_bootfail_log_info_t *pbootfail_log_info)
{
	int i;
	int j;
	int count = ARRAY_SIZE(sensible_bootfail_with_reboot_recovery);
	bfr_real_recovery_info_t *rinfo = &(pbootfail_log_info->real_recovery_info);

	for (i = 0; i < count; i++) {
		for (j = 0; j < rinfo->record_count; j++) {
			if ((unsigned int)sensible_bootfail_with_reboot_recovery[i] == rinfo->boot_fail_no[j]) {
...
static bool is_user_sensible_boot_fail_stage(
		bfm_bootfail_log_info_t *pbootfail_log_info)
{
	int i;
	bfr_real_recovery_info_t *rinfo = &(pbootfail_log_info->real_recovery_info);
...
static bool is_user_sensible_boot_fail_time(
		bfm_bootfail_log_info_t *pbootfail_log_info)
{
	int i;
	bfr_real_recovery_info_t *rinfo = &(pbootfail_log_info->real_recovery_info);
...
static bool bfm_is_user_sensible_boot_fail(
		bfm_bootfail_log_info_t *pbootfail_log_info)
{
	if ((pbootfail_log_info->real_recovery_info.record_count == 0) &&
	    (pbootfail_log_info->log_dir_count > 0)) {
		/* Maybe DO_NOTHING log */
		return true;
	}

	/* 2, if bootfail times > 2, it must be a user sensible bootfail */
	if (pbootfail_log_info->real_recovery_info.record_count >
	    BFM_USER_NOT_SENSIBLE_BOOTFAIL_MAX_COUNT)
		return true;

	/* 3, if the recovery method of bootfail is not REBOOT, it must be user sensible */
	if (is_user_sensible_rmethod(pbootfail_log_info))
		return true;

	/* 4, some time even if the recovery method is FRM_REBOOT, it is also user sensible such as: PRESS10S */
	if (is_user_sensible_boot_fail_no(pbootfail_log_info))
		return true;

	/* 5, if the bootfail occurs @ NATIVE/FRAMEWORK stage, it maybe a user sensible bootfail with high probability */
	if (is_user_sensible_boot_fail_stage(pbootfail_log_info))
		return true;

	/* 6, if the bootfail occurs @ NATIVE/FRAMEWORK stage, it maybe a user sensible bootfail with high probability */
	if (is_user_sensible_boot_fail_time(pbootfail_log_info))
		return true;

	return false;
}

static void bfm_merge_one_bootfail_logs(
		bfm_bootfail_log_info_t *pbootfail_log_info, int log_dir_idx,
		int log_name_idx)
{
	char *pdata = NULL;
	char *pstart = NULL;
	...
	memset(src_file_path, 0, BFMR_MAX_PATH);
	snprintf(src_file_path, BFMR_MAX_PATH - 1, "%s/%s",
		 pbootfail_log_info->bootfail_logs[log_dir_idx].log_dir,
	...
	/* read src file */
	snprintf(time, sizeof(time) - 1, "0x%08x",
		 pbootfail_log_info->real_recovery_info.boot_fail_rtc_time[log_dir_idx]);
...
static void bfm_merge_bootfail_logs(bfm_bootfail_log_info_t *pbootfail_log_info)
{
	...
	for (i = 0; i < pbootfail_log_info->log_dir_count; i++) {
		for (j = 0; j < log_count; j++)
			bfm_merge_one_bootfail_logs(pbootfail_log_info, i, j);
...
static int bfm_traverse_log_root_dir(bfm_bootfail_log_info_t *pbootfail_log_info)
{
	...
	log_max_count = ARRAY_SIZE(pbootfail_log_info->bootfail_logs);
	dirp = buf;
	num = sys_getdents64(fd, dirp, BFMR_MAX_PATH);
	while (num > 0) {
		while ((num > 0) && (log_idx < log_max_count)) {
			bfm_stat_t st;
			int ret;
			bfm_single_bootfail_log_info_t *log_info =
				&(pbootfail_log_info->bootfail_logs[log_idx]);
...
static int bfm_update_info_for_each_log(void)
{
	...
	pbootfail_log_info = bfmr_malloc(sizeof(bfm_bootfail_log_info_t));
	if (pbootfail_log_info == NULL) {
		BFMR_PRINT_ERR("bfmr_malloc failed!\n");
		goto __out;
	}
	...
	if (bfr_get_real_recovery_info(&(pbootfail_log_info->real_recovery_info)) != 0) {
	...
	if (!bfm_is_user_sensible_boot_fail(pbootfail_log_info)) {
		BFMR_PRINT_KEY_INFO("%s is a user unsensible bootfail log!\n",
			pbootfail_log_info->bootfail_logs[0].log_dir);
		(void)bfm_delete_user_unsensible_logs(pbootfail_log_info);
		goto __out;
	}
...
static int bfm_notify_boot_success(void *param)
{
	...
	if ((bootup_time >= BFM_BOOTUP_SLOWLY_THRESHOLD_IN_SECOND) &&
	    (!is_bootup_successfully)) {
...
/*
 * @function: int bfmr_capture_and_save_log(bfmr_log_type_e type,
 *    bfmr_log_dst_t *dst)
 * @brief: capture and save log, initialised in core.
 * @param: src [in] log src info.
 * @param: dst [in] infomation about the media of saving log.
...
static long bfmr_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	bfmr_detail_boot_stage_e boot_stage;
	struct bfmr_boot_fail_info *bf_info = NULL;
	...
	case BFMR_GET_BOOT_STAGE:
		bfmr_get_boot_stage(&boot_stage);
		if (copy_to_user((bfmr_detail_boot_stage_e *)args,
	...
	case BFMR_PROCESS_BOOT_FAIL:
		bf_info = bfmr_malloc(sizeof(struct bfmr_boot_fail_info));
		if (bf_info == NULL) {
			BFMR_PRINT_ERR("bfmr_malloc failed!\n");
			ret = -ENOMEM;
			break;
		}
		memset((void *)bf_info, 0,
			sizeof(struct bfmr_boot_fail_info));
		if (copy_from_user(bf_info,
		    ((struct bfmr_boot_fail_info *)args),
		...
		BFMR_PRINT_KEY_INFO("bootfail_errno: 0x%08x, suggested_recovery_method: %d, log_file [%s]'s lenth:%ld\n",
				    (unsigned int)bf_info->boot_fail_no,
				    (int)bf_info->suggested_recovery_method,
				    bf_info->addl_info.log_path,
				    bfmr_get_file_length(bf_info->addl_info.log_path));
		(void)boot_fail_err(bf_info->boot_fail_no,
				    bf_info->suggested_recovery_method,
				    &bf_info->addl_info);
		bfmr_free(bf_info);
		break;
	...
	case BFMR_ENABLE_CTRL: {
		...
		if (copy_from_user(&bfmr_enbale_flag, (int *)args,
		    sizeof(int)) != 0) {
	...
	case BFMR_ACTION_TIMER_CTL:
		pact_data = bfmr_malloc(sizeof(struct action_ioctl_data));
		if (pact_data == NULL) {
			BFMR_PRINT_ERR("bfmr_malloc failed!\n");
			ret = -ENOMEM;
			break;
...
static int bfm_process_ocp_excp(bfmr_hardware_fault_type_e fault_type,
		ocp_excp_info_t *pexcp_info)
{
	...
	paddl_info = bfmr_malloc(sizeof(bfmr_bootfail_addl_info_t));
	if (paddl_info == NULL) {
		BFMR_PRINT_ERR("bfmr_malloc failed!\n");
		goto __out;
	}
...
int bfm_init(void)
{
	...
	memset((void *)&chipsets_init_param, 0,
		sizeof(bfm_chipsets_init_param_t));
	chipsets_init_param.log_saving_param.capture_and_save_bootfail_log =
		bfm_capture_and_save_bootfail_log;
