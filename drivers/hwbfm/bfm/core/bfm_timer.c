 ...
 * bfm_timer.c
 *
 * define external enum/macros/interface for BFM (Boot Fail Monitor)
 *
...
/* timer count unit is msec */
#define BOOT_TIMER_COUNT_STEP 5000
#define BFMR_TIMEOUT_VALUE_FOR_SHORT_TIMER (1000 * 60 * 10)
#define BFMR_TIMEOUT_VALUE_FOR_LONG_TIMER (1000 * 60 * 30)
#define BFMR_MAX_TIMEOUT_VALUE_FOR_ACTION_TIMER (1000 * 60 * 30)
...
static inline struct action_timer *__bfm_get_action_timer(const char *act_name,
						struct list_head *atlist);


static void bfm_check_timer_expired(struct action_timer  *atimer,
		struct bfm_boot_timer_info *boot_timer_info)
{
	if(!atimer || !boot_timer_info)
		return;
	...
	if (atimer->action_timer_timing_count >=
		...
		BFMR_PRINT_SIMPLE_INFO("hwboot_timer: %s expired! boot fail\n",
				atimer->action_name);
		...
		if (bfmr_is_dir_existed(BFMR_VENDOR_HUAWEI_FLAG)) {
			...
			boot_fail_err(KERNEL_BOOT_TIMEOUT,
					NO_SUGGESTION, NULL);
		}
	}
}
...
static void bfm_check_boot_time(struct bfm_boot_timer_info *boot_timer_info)
{
	...
	u32 short_value = boot_timer_info->short_timer_timeout_value;
	u32 long_value = boot_timer_info->long_timer_timeout_value;

	long_timer_timing_count += boot_timer_info->count_step_value;
	if (boot_timer_info->boot_timer_count_enable) {
		unsigned long jif_time =
			jiffies - boot_timer_info->short_timer_last_start_time;
		...
		BFMR_PRINT_SIMPLE_INFO("hwboot_timer:update timer [long]\n");
	}
	...
	list_for_each_entry(atimer, &action_timer_active_list, act_list) {
		BFMR_PRINT_SIMPLE_INFO("hwboot_timer:update timer [%s]!\n",
				atimer->action_name);
		bfm_check_timer_expired(atimer, boot_timer_info);
	}
}
...
{
	struct bfm_boot_timer_info *boot_timer_info =
				(struct bfm_boot_timer_info *)(uintptr_t)data;
...
static __ref int bfm_boot_timer_count_kthread(void *arg)
{
	struct bfm_boot_timer_info *boot_timer_info = arg;
	...
	while (!kthread_should_stop()) {
		while (wait_for_completion_interruptible(
				&boot_timer_info->boot_complete) != 0)
			;
		reinit_completion(&boot_timer_info->boot_complete);

		mutex_lock(&s_boot_timer_mutex);
		delay_time =
			msecs_to_jiffies(boot_timer_info->count_step_value);

		if (boot_timer_info->boot_timer_count_task_should_run) {
			bfm_check_boot_time(boot_timer_info);
			mod_timer(&boot_timer_info->boot_timer,
					jiffies + delay_time);
		} else {
			BFMR_PRINT_SIMPLE_INFO("hwboot_timer: time thread will stop now:[boot_short_timer:%d]!\n",
...
int bfm_suspend_boot_timer(void)
{
	...
	mutex_lock(&s_boot_timer_mutex);
	if (likely(s_boot_timer_info) &&
		s_boot_timer_info->boot_timer_count_enable) {
		unsigned long short_time =
			s_boot_timer_info->short_timer_last_start_time;
...
static void __bfm_free_action_timer_list(struct list_head *atlist)
{
	...
	list_for_each_safe(pos, next, atlist) {
		...
		BFMR_PRINT_KEY_INFO("WARNING:[%s:%d]%s act_timer in list(activetime:%d ms, timeout:%d ms)\n",
					atimer->action_comm,
					atimer->action_pid,
					atimer->action_name,
...
/*
 * function: int bfmr_action_timer_start(char *act_name,
...
int bfm_action_timer_start(char *act_name, unsigned int timeout_value)
{
	...
	if (!atimer)
		atimer = __bfm_get_action_timer(act_name,
	...
	atimer = bfmr_malloc(sizeof(struct action_timer));
	if (!atimer)
		goto __out;
...
static void bfm_set_short_time(void)
{
...
int bfm_action_timer_stop(char *act_name)
{
	...
	if (atimer) {
		...
		if (atimer->action_pid != task_pid_tmp)
			BFMR_PRINT_ERR("W: pid mismatch stop[%s:%d]: start[%s:%d]",
...
int bfm_action_timer_pause(char *act_name)
{
	...
	BFMR_PRINT_KEY_INFO("\"%s\" timer is running, so pause it!\n",
			act_name);
...
int bfm_action_timer_resume(char *act_name)
{
	...
	if (!atimer) {
		BFMR_PRINT_KEY_INFO("\"%s\" timer not in pause quene\n",
				act_name);
		goto __out;
	}
	BFMR_PRINT_KEY_INFO("\"%s\" timer is paused, resume it\n",
			act_name);

	if (atimer->action_pid != task_pid_tmp)
		BFMR_PRINT_ERR("W: pid mismatch resume[%s:%d]: start[%s:%d]!!",
				task_comm_tmp, task_pid_tmp,
...
int bfm_init_boot_timer(void)
{
	int ret = 0;
	unsigned long delay_time = 0;
	struct bfm_boot_timer_info *boot_timer_info = NULL;
	...
	boot_timer_info->boot_time_task =
			kthread_create(bfm_boot_timer_count_kthread,
	...
	boot_timer_info->long_timer_timeout_value =
					BFMR_TIMEOUT_VALUE_FOR_LONG_TIMER;
	boot_timer_info->short_timer_timeout_value =
					BFMR_TIMEOUT_VALUE_FOR_SHORT_TIMER;
	...
	boot_timer_info->boot_timer.data =
				(unsigned long)(uintptr_t)boot_timer_info;
	boot_timer_info->boot_timer.function =
					bfm_wakeup_boot_timer_count_task;
