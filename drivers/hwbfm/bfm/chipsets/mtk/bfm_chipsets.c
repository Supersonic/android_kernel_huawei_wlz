...
extern unsigned long get_last_kmsg_pstore(char *dst, ssize_t *length);
...
unsigned int bfmr_capture_log_from_src_file(char *buf, unsigned int buf_len,
		char *src_log_path);
static unsigned int bfmr_capture_tombstone(char *buf, unsigned int buf_len,
		char *src_log_file_path);
static unsigned int bfmr_capture_vm_crash(char *buf, unsigned int buf_len,
		char *src_log_file_path);
static unsigned int bfmr_capture_vm_watchdog(char *buf, unsigned int buf_len,
		char *src_log_file_path);
static unsigned int bfmr_capture_normal_framework_bootfail_log(char *buf,
...
static unsigned int bfmr_capture_critical_process_crash_log(char *buf,
		unsigned int buf_len, char *src_log_file_path);
static unsigned int bfmr_capture_fixed_framework_bootfail_log(char *buf,
		unsigned int buf_len, char *src_log_file_path);
...
int bfm_get_boot_stage(bfmr_detail_boot_stage_e *pbfmr_bootstage)
{
	...
	*pbfmr_bootstage = get_boot_stage();
	return 0;
}
...
unsigned int bfmr_capture_log_from_system(char *buf, unsigned int buf_len,
		bfmr_log_src_t *src, int timeout)
{
...
static int bfm_check_validity_of_bootfail_log_in_dfx(
		bfm_bfi_member_info_t *pbfi_info, u64 rtc_time, int *log_idx)
{
	char *pdata_buf = NULL;
	...
	seek_result = sys_lseek(fd,
		BFM_BFI_HEADER_SIZE + BFM_BFI_MEMBER_SIZE * (*log_idx), SEEK_SET);
	if (seek_result != (long)(BFM_BFI_HEADER_SIZE + BFM_BFI_MEMBER_SIZE * (*log_idx))) {
	...
	for (i = (*log_idx); i < BFM_BFI_RECORD_TOTAL_COUNT; i++) {
		...
		if ((pbfi_info->rtcValue != 0) && (BFM_BFI_UNSAVE == pbfi_info->is_log_saved)) {
			*log_idx = i + 1;
			BFMR_PRINT_KEY_INFO("RTC time in BFI[%d]: %llx, is_log_saved: %llx\n",
					i, pbfi_info->rtcValue, pbfi_info->is_log_saved);
			ret = 0;
			break;
		}
		BFMR_PRINT_KEY_INFO("RTC time in BFI[%d]: %llx, is_log_saved: %llx\n",
...
static int bfm_update_save_flag_in_bfi(bfm_bfi_member_info_t *pbfi_info)
{
	...
	for (i = 0; i < BFM_BFI_RECORD_TOTAL_COUNT; i++) {
		...
		if ((pbfi_info->rtcValue == pbfi_info_local->rtcValue) &&
			...
			seek_result = sys_lseek(fd,
				(long)(BFM_BFI_HEADER_SIZE + i * BFM_BFI_MEMBER_SIZE), SEEK_SET);
			if (seek_result != (long)(BFM_BFI_HEADER_SIZE + i * BFM_BFI_MEMBER_SIZE))
			{
				BFMR_PRINT_ERR("sys_lseek [%s] failed!, seek_results: %ld ,it must be: %ld\n",
						bfi_part_full_path, seek_result,
						(long)(BFM_BFI_HEADER_SIZE + i * BFM_BFI_MEMBER_SIZE));
				goto __out;
			}
...
static int bfm_save_bootfail_log_to_fs_immediately(
		bfm_process_bootfail_param_t *pfail_param,
		...
		int save_bottom_layer_bootfail_log)
{
	...
	BFMR_PRINT_ENTER();
	if (unlikely((NULL == pfail_param) || (NULL == psave_param) ||
	...
	pbfi_info = (bfm_bfi_member_info_t *)bfmr_malloc(BFM_BFI_MEMBER_SIZE);
	if (NULL == pbfi_info) {
		BFMR_PRINT_ERR("bfmr_malloc failed!\n");
		goto __out;
	}
	...
	while ((bfm_check_validity_of_bootfail_log_in_dfx(pbfi_info, 0, &log_idx) == 0) &&
		(check_times < BFM_BFI_RECORD_TOTAL_COUNT)) {
		if (pbfi_info->bfmStageCode >= NATIVE_STAGE_START)
			pfail_param->save_bottom_layer_bootfail_log = 0;
		else
			pfail_param->save_bottom_layer_bootfail_log =
				save_bottom_layer_bootfail_log;
		pfail_param->bootfail_errno = pbfi_info->bfmErrNo;
		pfail_param->bootfail_time = pbfi_info->rtcValue;
		pfail_param->bootup_time = pbfi_info->bootup_time;
		pfail_param->recovery_method = (bfr_recovery_method_e)pbfi_info->rcvMethod;
		pfail_param->boot_stage = (bfmr_detail_boot_stage_e)pbfi_info->bfmStageCode;
		pfail_param->is_user_sensible = (int)pbfi_info->isUserPerceptiable;
		pfail_param->is_system_rooted = (int)pbfi_info->isSystemRooted;
		pfail_param->dmd_num = pbfi_info->dmdErrNo;
		pfail_param->is_bootup_successfully = pbfi_info->is_bootup_successfully;
		pfail_param->suggested_recovery_method = pbfi_info->suggested_recovery_method;
		pfail_param->reboot_type = pbfi_info->reboot_type;
		strncpy(pfail_param->excepInfo,
				pbfi_info->excepInfo, sizeof(pbfi_info->excepInfo) - 1);

		ret = psave_param->capture_and_save_bootfail_log(pfail_param);
		if (0 != ret) {
		...
		unsigned int buf_len)
{
	...
	if (0 != bfmr_wait_for_part_mount_with_timeout(
			bfm_get_bfmr_log_part_mount_point(),
			BFM_WAIT_FOR_LOG_PART_TIMEOUT)) {
		...
		unsigned int log_len, int append)
{
	...
	fd = sys_open(dst_file_path, O_CREAT | O_RDWR | ((0 == append) ? (0U) : O_APPEND),
		BFMR_FILE_LIMIT);
	if (fd < 0) {
...
static int bfm_save_pwrkey_press_flag_in_bfi(
		bfm_process_bootfail_param_t *pparam)
{
	...
	pbfi_info_local->bfmErrNo = pparam->bootfail_errno;
	pbfi_info_local->bfmStageCode = pparam->boot_stage;
	pbfi_info_local->is_log_saved = BFM_BFI_UNSAVE;
	ret = bfmr_write_emmc_raw_part(bfi_part_full_path,
		BFM_BFI_PWK_PRESS_OFF, (char *)pbfi_info_local, BFM_BFI_MEMBER_SIZE);

__out:
	if (NULL != bfi_part_full_path) {
		bfmr_free(bfi_part_full_path);
		bfi_part_full_path = NULL;
	}
		...
		bfm_process_bootfail_param_t *param)
{
	...
	capture_and_save_bootfail_log_tmp =
		(bfmr_capture_and_save_bootfail_log)(param->capture_and_save_bootfail_log);
	...
	switch (param->bootfail_errno) {
		...
		default:
		{
			if (0 != bfmr_wait_for_part_mount_without_timeout(
					bfm_get_bfmr_log_part_mount_point())) {
...
int bfm_update_platform_logs(bfm_bootfail_log_info_t *pbootfail_log_info)
{
	...
	snprintf(src_file_path, BFMR_MAX_PATH - 1, "%s/%s",
			BFM_QCOM_LOG_ROOT_PATH, BFM_DBG_FILE_NAME);
	...
	pdata = (char *)bfmr_malloc(src_file_len);
	if (NULL == pdata) {
		BFMR_PRINT_ERR("bfmr_malloc failed!\n");
		goto __out;
	}
...
void bfmr_alloc_and_init_raw_log_info(bfm_process_bootfail_param_t *pparam,
		bfmr_log_dst_t *pdst)
{
...
void bfmr_save_and_free_raw_log_info(bfm_process_bootfail_param_t *pparam)
{
	...
	if (NULL != pparam->log_save_context) {
		...
		bfmr_save_log_to_raw_part(BFM_QCOM_RAW_PART_NAME,
								  ...
								  (char *)brit, sizeof(bfm_record_info_t));
...
void bfmr_update_raw_log_info(bfmr_log_src_t *psrc,
		bfmr_log_dst_t *pdst, unsigned int bytes_read)
{
	bfm_record_info_t *brit;
	...
	switch (psrc->log_type) {
		case LOG_TYPE_BOOTLOADER_1: {
			strncpy(brit->log_name[psrc->log_type],
					BFM_QCOM_BL1_BOOTFAIL_LOG_NAME, BFMR_SIZE_128 - 1);
			break;
		}
		case LOG_TYPE_BOOTLOADER_2: {
			strncpy(brit->log_name[psrc->log_type],
					BFM_QCOM_BL2_BOOTFAIL_LOG_NAME, BFMR_SIZE_128 - 1);
			break;
		}
		case LOG_TYPE_TEXT_KMSG: {
			strncpy(brit->log_name[psrc->log_type],
					BFM_QCOM_KERNEL_BOOTFAIL_LOG_NAME, BFMR_SIZE_128 - 1);
			break;
		}
		case LOG_TYPE_RAMOOPS: {
			strncpy(brit->log_name[psrc->log_type],
					BFM_QCOM_RAMOOPS_BOOTFAIL_LOG_NAME, BFMR_SIZE_128 - 1);
			break;
		}
		case LOG_TYPE_BETA_APP_LOGCAT: {
			strncpy(brit->log_name[psrc->log_type],
		...
		case LOG_TYPE_FIXED_FRAMEWORK_BOOTFAIL_LOG: {
			strncpy(brit->log_name[psrc->log_type],
					BFM_FRAMEWORK_BOOTFAIL_LOG_FILE_NAME, BFMR_SIZE_128 - 1);
			break;
		}
		case LOG_TYPE_CRITICAL_PROCESS_CRASH:
		case LOG_TYPE_VM_TOMBSTONES:
		...
		case LOG_TYPE_BFM_RECOVERY_LOG: {
			strncpy(brit->log_name[psrc->log_type],
					qcom_get_file_name(psrc->src_log_file_path), BFMR_SIZE_128 - 1);
			break;
		}
		case LOG_TYPE_BFMR_TEMP_BUF:
		default:
...
int bfm_chipsets_init(bfm_chipsets_init_param_t *param)
{
	...
	memset((void *)&s_bootfail_log_saving_param, 0,
