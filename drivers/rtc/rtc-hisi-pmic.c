...
noinline int atfd_hisi_service_rtc_smc(
	u64 function_id, u64 arg0, u64 arg1, u64 arg2)
{
...
#ifdef CONFIG_HISI_RTC_SECURE_FEATURE
static int hisi_rtc_nve_write(unsigned int index, int *data, unsigned int len)
{
	int ret;
...
void hisi_pmu_rtc_setalarmtime(unsigned long time)
{
	...
	hisi_pmic_reg_write(PMU_RTC_XO_THRESHOLD_HIGH,
		((ldata->pmu_rtc_xo_thrshd_pwroff_val >> 8) & 0xFF));
#endif
...
static int hisi_soc_rtc_read_alarmtime(
...
static int hisi_soc_rtc_alarm_irq_enable(
...
static int hisi_soc_rtc_set_alarmtime(
	struct device *dev, struct rtc_wkalrm *alarm)
{
	struct hisi_rtc_dev *ldata = dev_get_drvdata(dev); /*lint !e578*/
	unsigned long time;
...
static irqreturn_t hisi_soc_rtc_interrupt(int irq, void *dev_id)
{
	...
	rtcmis = readl(
		ldata->soc_rtc_baseaddr + SOC_RTC_MIS); /*read interrupt */
	if (rtcmis) {
	...
	struct device_node *np, struct hisi_rtc_dev *prtc)
{
	...
	ret = of_property_read_u32(
		np, "hisilicon,pmic-rtc-base", &prtc->pmu_rtc_baseaddr);
	if (ret) {
	...
	ret = of_property_read_u32(
		np, "hisilicon,pmic-rtc-icr", &prtc->pmu_rtc_icr);
	if (ret) {
	...
	ret = of_property_read_u32(
		np, "hisilicon,pmic-rtc-ib", &prtc->pmu_rtc_ib);
	if (ret) {
	...
	ret = of_property_read_u32(
		np, "hisilicon,pmic-rtc-xo-thrshd-pwroff-val",
...
unsigned long hisi_rtc_test(int cmd, unsigned long wtime)
{
	...
	case RTC_TEST_ENABLE_PMURTC_ALARM:
		...
		wtime = rtime + wtime;
		hisi_pmu_rtc_write_bulk(
			data->pmu_rtc_baseaddr + PMU_RTC_MR, wtime);
	...
	case RTC_TEST_ATF_READ_WRITE:
		atfd_hisi_service_rtc_smc(
			RTC_REGISTER_FN_ID, PMU_RTC_TEST, 0, 0); /*lint !e747*/
		return 0;
#endif
	default:
		return 0;
