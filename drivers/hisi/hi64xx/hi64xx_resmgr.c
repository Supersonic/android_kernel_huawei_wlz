...
static int _turn_on_pll(struct hi64xx_resmgr_priv *priv, enum hi64xx_pll_type pll_type)
{
...
static int _turn_off_pll(struct hi64xx_resmgr_priv *priv, enum hi64xx_pll_type pll_type)
{
	int ret;

	ret = priv->config.pfn_pll_ctrls[pll_type].turn_off(priv->codec);
...
static int _request_pll_single(struct hi64xx_resmgr_priv *priv, enum hi64xx_pll_type pll_type)
{
...
static int _request_pll_multiple(struct hi64xx_resmgr_priv *priv, enum hi64xx_pll_type pll_type)
{
	struct pll_switch_event event;
...
static int _release_pll_single(struct hi64xx_resmgr_priv *priv, enum hi64xx_pll_type pll_type)
{
	struct pll_switch_event event;
...
static int _release_pll_multiple(struct hi64xx_resmgr_priv *priv, enum hi64xx_pll_type pll_type)
{
	struct pll_switch_event event;
	enum hi64xx_pll_type new_pll;
...
static int _pll_delay_release(struct hi64xx_resmgr_priv* priv, enum hi64xx_pll_type pll_type)
{
	...
	if (priv->pll_wait_rel[pll_type] == REL_IDLE) {
		...
		if (priv->config.pll_sw_mode == MODE_SINGLE)
			(void)_release_pll_single(priv, pll_type);
		else
			(void)_release_pll_multiple(priv, pll_type);
	}
...
static int _pll_delayed_release_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		wait_event_interruptible_locked(priv->pll_rel_wq,
			_is_pll_waiting_release(priv) || kthread_should_stop()); /*lint !e666*/
		...
		while (pending && !kthread_should_stop()) {
			...
			for (i = 0; i < PLL_MAX; i++) {
				...
				if (priv->pll_wait_rel[i] == REL_PENDING) {
					...
					spin_unlock(&priv->pll_rel_wq.lock);
					if (priv->config.pll_sw_mode == MODE_SINGLE)
						_release_pll_single(priv, (enum hi64xx_pll_type)i);
					else
						_release_pll_multiple(priv, (enum hi64xx_pll_type)i);
                                     ...
                                     unsigned int reg_addr)
{
	...
	if (priv->config.pll_sw_mode == MODE_SINGLE)
		ret = _request_pll_single(priv, req_pll);
	else
		ret = _request_pll_multiple(priv, req_pll);
                              ...
                              enum hi64xx_pll_type pll_type)
{
	...
	if (priv->config.pll_sw_mode == MODE_SINGLE)
		return _request_pll_single(priv, pll_type);
	else
		return _request_pll_multiple(priv, pll_type);
}
                              ...
                              enum hi64xx_pll_type pll_type)
{
	...
	if (priv->config.pll_sw_mode == MODE_SINGLE)
		return _release_pll_single(priv, pll_type);
	else
		return _release_pll_multiple(priv, pll_type);
}
...
EXPORT_SYMBOL(hi64xx_resmgr_pm_get_clk);

void hi64xx_resmgr_pm_put_clk(void)
{
	hi_cdcctrl_pm_put();
}
EXPORT_SYMBOL(hi64xx_resmgr_pm_put_clk);
