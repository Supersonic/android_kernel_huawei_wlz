...
struct hispcpe_s {
	...
	atomic_t cpe_cpe_refs;
	atomic_t cpe_slam_refs;
	atomic_t sec_slam_refs;
	atomic_t reorder_refs;
	atomic_t compare_refs;
	struct mutex lock;
...
static irqreturn_t hispcpe_irq_handler(int irq, void *devid)
{
	...
	if (irq == dev->irq[CPE_IRQ_0]) {
		value = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_CMDLST_ACPU_IRQ_REG2_REG));
		...
		if (value & (0x1 << CMP_CHANNEL)) {
			irq_value = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_CMP_IRQ_REG2_REG));
		...
		if (value & (0x1 << RDR_CHANNEL)) {
			irq_value = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_RDR_IRQ_REG2_REG));
		...
		if (value & (0x1 << SLAM_CHANNEL)) {
			irq_value = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_SLAM_IRQ_REG2_REG));
		...
		if (value & (0x1 << CPE_CHANNEL)) {
			irq_value = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_CPE_IRQ_REG4_REG));
...
{
	U_CMDLST_ACPU_IRQ_REG1 cfg_acpu;
...
int sec_slam_powerup(struct hispcpe_s *dev, int share_fd)
{
	...
	if (dev->sec_slamdrv->enable_jpgclk == NULL) {
...
{
	int ret = 0, err = 0;
	unsigned long transition_source_rate      = 600000000;
	unsigned long transition_dest_rate0       = 238000000;
	unsigned long transition_transition_rate0 = 240000000;
	unsigned long transition_dest_rate1       = 415000000;
	unsigned long transition_transition_rate1 = 400000000;
	...
	if ((ret = jpeg_ipp_set_transition_rate(transition_source_rate, transition_dest_rate0,
			transition_transition_rate0)) != true) {
		pr_err("[%s] Failed: jpeg_ipp_set_transition_rate0.%d\n", __func__, ret);
		goto fail_enjgpclk;
	}

	if ((ret = jpeg_ipp_set_transition_rate(transition_source_rate, transition_dest_rate1,
			transition_transition_rate1)) != true) {
...
int sec_slam_powerdn(struct hispcpe_s *dev)
{
	...
	if (dev->sec_slamdrv->disable_jpgclk == NULL) {
...
static int hispcpe_cpe_clk_enable(struct hispcpe_s *dev)
{
	...
	if (atomic_read(&dev->cpe_cpe_refs) > 0) {
		pr_err("[%s] Failed: cpe Opened.0x%x\n", __func__, atomic_read(&dev->cpe_cpe_refs));
		return -EINVAL;
	}
...
static int hispcpe_cpe_clk_disable(struct hispcpe_s *dev)
{
	...
	if (atomic_read(&dev->cpe_cpe_refs) <= 0) {
		pr_err("[%s] Failed: cpe not Opened.0x%x\n", __func__, atomic_read(&dev->cpe_cpe_refs));
		return -EINVAL;
	}
...
static int hispcpe_slam_clk_check(struct hispcpe_s *dev)
{
	...
	if (0 == dev->sec_path) {
		if (atomic_read(&dev->cpe_supply_refs) <= 0) {
			pr_err("[%s] Failed: slam_supply not Opened.0x%x\n", __func__, atomic_read(&dev->cpe_supply_refs));
			return -EINVAL;
		}
...
static int hispcpe_reorder_clk_enable(struct hispcpe_s *dev)
{
	...
	if (atomic_read(&dev->cpe_supply_refs) <= 0) {
		pr_err("[%s] Failed: rdr_supply not Opened.0x%x\n", __func__, atomic_read(&dev->cpe_supply_refs));
		return -EINVAL;
	}

	if (atomic_read(&dev->reorder_refs) > 0) {
		pr_err("[%s] Failed: reorder_refs Opened.0x%x\n", __func__, atomic_read(&dev->reorder_refs));
		return -EINVAL;
	}

	cfg.u32 = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_RDR_CRG_CFG0_REG));
	cfg.bits.rdr_clken  = 1;
	writel(cfg.u32, (volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_RDR_CRG_CFG0_REG));
	atomic_set(&dev->reorder_refs, 1);
	D("-\n");
	return 0;
}
...
static int hispcpe_reorder_clk_disable(struct hispcpe_s *dev)
{
	...
	if (atomic_read(&dev->reorder_refs) <= 0) {
		pr_err("[%s] Failed: rdr not Opened.0x%x\n", __func__, atomic_read(&dev->reorder_refs));
		return -EINVAL;
	}
...
static int hispcpe_compare_clk_enable(struct hispcpe_s *dev)
{
	...
	if (atomic_read(&dev->compare_refs) > 0) {
		pr_err("[%s] Failed: cpe CMP Opened.0x%x\n", __func__, atomic_read(&dev->compare_refs));
		return -EINVAL;
	}

	cfg.u32 = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_CMP_CRG_CFG0_REG));
	cfg.bits.cmp_clken  = 1;
	writel(cfg.u32, (volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_CMP_CRG_CFG0_REG));
	atomic_set(&dev->compare_refs, 1);
	D("-\n");
	return 0;
}
...
static int hispcpe_compare_clk_disable(struct hispcpe_s *dev)
{
	...
	if (atomic_read(&dev->compare_refs) <= 0) {
		pr_err("[%s] Failed: cpe cmp not Opened.0x%x\n", __func__, atomic_read(&dev->compare_refs));
		return -EINVAL;
	}
...
{
	U_RDR_CRG_CFG0 rdr_cfg;
	U_CMP_CRG_CFG0 cmp_cfg;
	D("+\n");
	...
	if (atomic_read(&dev->compare_refs) > 0) {
		pr_err("[%s] Failed: compare_refs Opened.0x%x\n", __func__, atomic_read(&dev->compare_refs));
		return -EINVAL;
	}

	rdr_cfg.u32 = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_RDR_CRG_CFG0_REG));
	rdr_cfg.bits.rdr_clken  = 1;
	writel(rdr_cfg.u32, (volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_RDR_CRG_CFG0_REG));
	cmp_cfg.u32 = readl((volatile void __iomem *)(dev->reg[CPE_TOP] + SUB_CTRL_CMP_CRG_CFG0_REG));
...
static int hispcpe_matcher_clk_disable(struct hispcpe_s *dev)
{
	...
	if (atomic_read(&dev->compare_refs) <= 0) {
		pr_err("[%s] Failed: compare_refs not Opened.0x%x\n", __func__, atomic_read(&dev->compare_refs));
		return -EINVAL;
	}
...
static int hispcpe_wait(struct hispcpe_s *dev, unsigned int wait_mode)
{
	...
	if (wait_mode == 0) {
		while (1) {
			ret = wait_event_interruptible_timeout(dev->cpe_wait, dev->cpe_ready, msecs_to_jiffies(HISP_CPE_TIMEOUT_MS));
		...
		while (1) {
			ret = wait_event_interruptible_timeout(dev->slam_wait, dev->slam_ready, msecs_to_jiffies(HISP_CPE_TIMEOUT_MS));
			if ((ret == -ERESTARTSYS) && (times++ < 200)) {
				msleep(5);
			} else {
				break;
		...
		while (1) {
			ret = wait_event_interruptible_timeout(dev->reorder_wait, dev->reorder_ready, msecs_to_jiffies(HISP_CPE_TIMEOUT_MS));
			if ((ret == -ERESTARTSYS) && (times++ < 200)) {
				msleep(5);
			} else {
				break;
		...
		while (1) {
			ret = wait_event_interruptible_timeout(dev->compare_wait, dev->compare_ready, msecs_to_jiffies(HISP_CPE_TIMEOUT_MS));
			if ((ret == -ERESTARTSYS) && (times++ < 200)) {
				msleep(1);
			} else {
				break;
...
int hispcpe_compare_request(struct hispcpe_s *dev, msg_req_compare_request_t *ctrl)
{
	...
	match_kpt = (unsigned int)(readl((volatile void __iomem *)(dev->reg[COMPARE_REG] + 0x0))) >> 8;
...
static int hispcpe_map_kernel(struct hispcpe_s *dev, unsigned long args, unsigned int *ipp_buff_size)
{
	...
	if ((ret = copy_from_user(&pstFrameBuf, args_mapkernel, sizeof(struct memory_block_s))) != 0) {
	...
	dev->devbuf = dma_buf_get(pstFrameBuf.shared_fd);
	...
	iova = hisi_iommu_map_dmabuf(&dev->pdev->dev, dev->devbuf, pstFrameBuf.prot, &iova_size);
	...
	if (pstFrameBuf.size != iova_size) {
		pr_err("[%s] hisi_iommu_map_range failed:  %d len 0x%lx\n", __func__, pstFrameBuf.size, iova_size);
	...
	dev->daddr = iova;
	*ipp_buff_size = pstFrameBuf.size;
...
static int sec_slam_int(struct hispcpe_s *dev, msg_req_slam_request_t *ctrl_slam)
{
	...
	if (TRUSTED ==  ctrl_slam->req_orb.secure_flag) {
		if ((ret = sec_slam_smmu_int(dev)) != 0) {
			pr_err("[%s] Failed : sec_slam_smmu_int.%d\n", __func__, ret);
			return ret;
		}

		if (dev->sec_slamdrv->shrf2da == NULL) {
			pr_err("[%s] Failed : sec_slamdrv->shrf2da.%pK\n", __func__, dev->sec_slamdrv->shrf2da);
			return -EINVAL;
		}

		ctrl_slam->req_orb.streams[0][0].buf = dev->sec_slamdrv->shrf2da(dev->sec_slamdrv, ctrl_slam->req_orb.streams[0][0].buf,
											   &g_share_fd_size[BI_SHARE_FD]);

		if (ctrl_slam->req_orb.streams[0][0].buf == 0) {
			E("[%s] Failed : hipp_shrf2da.BI_SHARE_FD\n", __func__);
			return -ENOMEM;
		}

		ctrl_slam->req_orb.streams[0][2].buf = dev->sec_slamdrv->shrf2da(dev->sec_slamdrv, ctrl_slam->req_orb.streams[0][2].buf,
											   &g_share_fd_size[STAT_SHARE_FD]);

		if (ctrl_slam->req_orb.streams[0][2].buf == 0) {
			E("[%s] Failed : hipp_shrf2da.STAT_SHARE_FD\n", __func__);
			return -ENOMEM;
		}
	} else if (NONTRUS ==  ctrl_slam->req_orb.secure_flag) {
		if (dev->ippdrv == NULL) {
...
static int sec_slam_deint(struct hispcpe_s *dev, msg_req_slam_request_t *ctrl_slam)
{
	...
	if (TRUSTED ==  ctrl_slam->req_orb.secure_flag) {
		...
		hipp_trusted_unmap(dev->sec_slamdrv, ctrl_slam->req_orb.streams[0][0].buf, g_share_fd_size[BI_SHARE_FD]);
		hipp_trusted_unmap(dev->sec_slamdrv, ctrl_slam->req_orb.streams[0][2].buf, g_share_fd_size[STAT_SHARE_FD]);
	} else if (NONTRUS ==  ctrl_slam->req_orb.secure_flag) {
		if (dev->ippdrv == NULL) {
			pr_err("Failed : ippdrv.%pK\n", dev->ippdrv);
			return -EINVAL;
...
static int sec_slam_process(struct hispcpe_s *dev, unsigned long args)
{
	...
	msg_req_slam_request_t *ctrl_slam_sec = NULL;
	void __user *args_slam = (void __user *)(uintptr_t)args;
	...
	ctrl_slam_sec = (msg_req_slam_request_t *)vmalloc(sizeof(msg_req_slam_request_t));

	if (NULL == ctrl_slam_sec) {
		pr_err(" fail to vmalloc ctrl_slam\n");
		goto free_slam_sec_kmalloc_memory0;
	}

	if (memset_s(ctrl_slam_sec, sizeof(msg_req_slam_request_t), 0, sizeof(msg_req_slam_request_t))) {
		pr_err(" fail to memset_s ctrl_slam\n");
		goto free_slam_sec_kmalloc_memory;
	}

	if ((ret = copy_from_user(ctrl_slam_sec, args_slam, sizeof(msg_req_slam_request_t))) != 0) {
		pr_err("[%s] copy_from_user.%d\n", __func__, ret);
		goto free_slam_sec_kmalloc_memory;
	}

	if (ctrl_slam_sec->slam_pyramid_layer > SLAM_LAYER_MAX) {
		pr_err("[%s] Failed : Invalid slam_pyramid_layer = %d\n", __func__, ctrl_slam_sec->slam_pyramid_layer);
		goto free_slam_sec_kmalloc_memory;
	}

	if ((ret = sec_slam_int(dev, ctrl_slam_sec)) != 0) {
	...
	if ((ret = hispcpe_slam_request(dev, ctrl_slam_sec)) != 0) {
		pr_err("[%s] Failed : hispcpe_slam_request.%d\n", __func__, ret);
		if (hispcpe_slam_clk_disable(dev) != 0)
			pr_err("[%s] Failed : hispcpe_slam_clk_disable\n", __func__);
	...
	if ((ret = sec_slam_deint(dev, ctrl_slam_sec)) != 0) {
		pr_err("[%s] Failed : sec_slam_int.%d\n", __func__, ret);
		goto free_slam_sec_kmalloc_memory;
	}

free_slam_sec_kmalloc_memory:
	vfree(ctrl_slam_sec);
	ctrl_slam_sec = NULL;
free_slam_sec_kmalloc_memory0:

	if ((ret1 = dev->sec_slamdrv->unlock_usecase(dev->sec_slamdrv)) != 0) {
...
static long hispcpe_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
	...
	void __user *args_map_iommu = NULL;
	...
	case HISP_CPE_PWRUP:
		...
		if (0 == para.pw_flag) {
			...
			if (para.mem.shared_fd < 0) {
				pr_err("[%s] Failed : share_fd.%d\n", __func__, para.mem.shared_fd);
				goto relax_ipp_wakelock;
			}
	...
	case HISP_CPE_MAP_IOMMU:
		args_map_iommu = (void __user *)(uintptr_t)args;
...
static int get_ipp_online_flag(unsigned int ver)
{
	...
	reg = (void __iomem *)ioremap(SOC_ACPU_PCTRL_BASE_ADDR, IPP_PCTRL_PERI_SIZE);
	if (reg == NULL) {
		pr_err("[%s] Failed : ioremap\n", __func__);
		return -ENOMEM;
	}

	pctrl_stat10 = readl(reg + IPP_PCTRL_PERI_STAT_ADDR);
	pr_alert("ISP/Camera pctrl_stat10.0x%x\n", pctrl_stat10);
	if ((pctrl_stat10 & (IPP_PCTRL_PERI_FLAG)) == 0) {
		pr_err("[%s] Failed : IPP May be not on board\n", __func__);
		ret = -ENODEV;
	} else {
		ret = 0;
	}
...
static int hispcpe_resource_init(struct hispcpe_s *dev)
{
	...
	online_flag = get_ipp_online_flag(dev->version);
	if (online_flag == 0) {
...
static int hispcpe_probe(struct platform_device *pdev)
{
	...
	if ((dev->ippdrv = hipp_register(HISI_CPE_UNIT, NONTRUS)) == NULL) {
		pr_err("[%s] Failed : hipp_register.%pK\n", __func__, dev->ippdrv);
		goto free_iommu;
	}

	if ((dev->sec_slamdrv = hipp_register(HISI_ORB_UNIT, TRUSTED)) == NULL) {
		pr_err("[%s] Failed : sec_slam_register.%pK\n", __func__, dev->sec_slamdrv);
		goto free_iommu;
	}

	if ((dev->rdrdrv = hipp_register(HISI_REORDER_UNIT, TRUSTED)) == NULL) {
		pr_err("[%s] Failed : sec_reorder_register.%pK\n", __func__, dev->rdrdrv);
		goto free_iommu;
	}

	if ((dev->cmpdrv = hipp_register(HISI_COMPARE_UNIT, TRUSTED)) == NULL) {
		pr_err("[%s] Failed : sec_compare_register.%pK\n", __func__, dev->cmpdrv);
		goto free_iommu;
	}
...
static int hispcpe_remove(struct platform_device *pdev)
{
	...
	if (hipp_unregister(HISI_REORDER_UNIT))
		pr_err("[%s] Failed : sec_slam_unregister\n", __func__);

	if (hipp_unregister(HISI_COMPARE_UNIT))
		pr_err("[%s] Failed : sec_slam_unregister\n", __func__);
