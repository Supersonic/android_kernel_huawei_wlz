...
static struct gen_pool *hipp_genpool_setup(unsigned long start, unsigned long size, unsigned int align)
{
	...
	if ((pool = gen_pool_create((int)(order_base_2(align)), -1)) == NULL) {/*lint !e666 !e835 !e747 !e516 !e866 !e712 */
...
void hipp_trusted_unmap(struct hipp_common_s *drv, unsigned int da, size_t size)
{
	...
	if (drv->mode != TRUSTED || !size || dev->bl31_vaddr == NULL) {
...
static int hippcom_probe(struct platform_device *pdev)
{
    ...
    if ((ret = atfhipp_trusted_smem((uint64_t)dev->smem_dma, (uint64_t)(dev->smem_dma + MEM_IPP_ERR_ADDR_OFFSET))) != 0) {
	...
	if ((dev->genpool = hipp_genpool_setup(dev->genpool_start, dev->genpool_size, 0x8000)) == NULL) {
