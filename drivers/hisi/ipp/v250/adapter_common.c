...
void cmdlst_update_buffer_header(schedule_cmdlst_link_t *cur_point, schedule_cmdlst_link_t *next_point, unsigned int channel_id)
{
...
void cmdlst_update_buffer_header_last(schedule_cmdlst_link_t *cur_point, schedule_cmdlst_link_t *next_point, unsigned int channel_id)
{
	cmd_buf_t *cmd_buf = &cur_point->cmd_buf;
	unsigned int i  = 0;
...
static int cmdlst_set_irq_mode(cmdlst_para_t *cmdlst_para, unsigned int stripe_index)
{
	...
	case RDR_CHANNEL: {
		...
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_CMDLST_ACPU_IRQ_REG0_REG, 0x1<<RDR_CHANNEL); //clear acpu cmdlst channel0 irq eof
	...
	case CMP_CHANNEL: {
		...
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_CMDLST_ACPU_IRQ_REG0_REG, 0x1<<CMP_CHANNEL); //clear acpu cmdlst channel0 irq eof
						  ...
						  isp_stripe_info_t *p_stripe_info, unsigned int overlap, unsigned int width)
{
	...
	do {
		...
		for (i = 0; i < stripe_cnt; i++) {
			unsigned long long int active_stripe_end = ((i + 1) * ((unsigned long long int)(in_full_width) << 16))
					/ stripe_cnt;
...
int seg_src_set_cmdlst_wr_buf(cmd_buf_t *cmd_buf, unsigned int wr_addr, unsigned int rd_addr, unsigned int data_size)
{
	...
	wr_addr = ALIGN_DOWN(wr_addr, CVDR_ALIGN_BYTES);
