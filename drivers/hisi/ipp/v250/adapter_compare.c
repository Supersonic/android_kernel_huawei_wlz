...
static int compare_update_cvdr_cfg_tab(msg_req_compare_request_t *compare_request, cfg_tab_cvdr_t *compare_cfg_tab)
{
	...
	if (compare_request->streams[BI_COMPARE_DESC_REF].buf) {
		...
		cfmt.expand = 1;
		format = PIXEL_FMT_CPE_D64;
		stride   = cfmt.width;
		cfmt.addr = compare_request->streams[BI_COMPARE_DESC_REF].buf;
		loge_if_ret(cfg_tbl_cvdr_rd_cfg_d64(compare_cfg_tab, &cfmt, (ALIGN_UP(CMP_IN_INDEX_NUM, CVDR_ALIGN_BYTES)), stride));
	}
...
int compare_request_handler(msg_req_compare_request_t *compare_request)
{
	...
	for (i = 0; i < compare_cmdlst_para->stripe_cnt; i++) {
		if (g_compare_read_st) {
			if (i % 2 != 0)
				cmdlst_read_buffer(i, &cmd_link_entry[i].cmd_buf, compare_cmdlst_para);
			else {
				cvdr_prepare_nr_cmd(&g_cvdr_devs[0], &cmd_link_entry[i].cmd_buf, compare_cvdr_cfg_tab);
				compare_prepare_cmd(&g_compare_devs[0], &cmd_link_entry[i].cmd_buf, compare_cfg_tab);
				cvdr_prepare_cmd(&g_cvdr_devs[0], &cmd_link_entry[i].cmd_buf, compare_cvdr_cfg_tab);
...
int compare_eof_handler(msg_req_compare_request_t *compare_request, unsigned int matched_kpt_num)
{
	...
	unsigned short   *user_index = compare_request->compare_index;
	unsigned short   *user_kpt_num = compare_request->compare_matched_kpt;
	...
	void __user *args_user_index = (void __user *)(uintptr_t)user_index;
	...
	if (NULL == user_kpt_num) {
		E("[%s] failed : user_kpt_num  is null", __func__);
		return -CPE_FW_ERR;
	}

	void __user *args_user_kpt_num = (void __user *)(uintptr_t)user_kpt_num;
