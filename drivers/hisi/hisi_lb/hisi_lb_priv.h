...
#define SLC_ADDR4(addr, slc_id) \
				(addr + slc_id * 0x4)
#define SLC_GID_ADDR4(addr, slc_id, gid) \
				(addr + slc_id * 0x2000 + gid * 0x4)
#define GID_ADDR4(addr, gid) \
				(addr + gid * 0x4)
#define GID_ADDR100(addr, gid) \
				(addr + gid * 0x100)
...
#define SC_POLL_TIMEOUT_US	100000

#define DEFINE_INIT_QUOTA(quota, l, h) \
		quota.value = 0;               \
		quota.reg.gid_quato.quota_l = l; \
		quota.reg.gid_quato.quota_h = h;
...
extern int lb_gid_csz_show(int gid);
