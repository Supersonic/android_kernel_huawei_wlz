...
static unsigned int l3share_enable_flag = 0;
...
int l3_cache_request(struct l3_cache_request_params *request_params)
{
	...
	l3share_notifier_call_chain(L3SHARE_ACP_ENABLE, request_params);
		...
		goto unlock;
#else
		l3_cache_enable_acp(request_params->id, request_params->request_size);
#endif
	}
...
int l3_cache_release(struct l3_cache_release_params *release_params)
{
	...
	if (0 == (g_acp_flag & BIT((u32)(release_params->id)))){
...
#else
#define l3_share_suspend	NULL
#define l3_share_resume		NULL
#endif
