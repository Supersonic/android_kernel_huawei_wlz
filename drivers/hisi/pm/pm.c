...
#ifdef CONFIG_PM_SEC_GPIO
	if (ao_gpio_info_p[index].sec_flag == 0){
#endif
...
#ifdef CONFIG_PM_SEC_GPIO
		ret_rd_sec_flag = of_property_read_u32(dn, "secure-mode", &ao_gpio_info_p[i].sec_flag);
		if (ret_rd_sec_flag != 0)
			ao_gpio_info_p[i].sec_flag = 0;
		else
