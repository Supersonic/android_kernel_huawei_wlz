...
DRV_DMA_CONFIG_STRU hi6405_soundtrigger_dma_fast_cfg[2] = {
	{.port = (HI3xxx_SLIMBUS_BASE_REG + 0x1180), .config = 0x43322067, .channel = DMA_FAST_LEFT_CH_NUM},	/*hi6405 fast data left channel*/
	{.port = (0), .config = 0, .channel = 0},	/*hi6405 fast data right channel*/
};
...
static int32_t dma_start(struct st_fast_status * fast_status)
{
	...
	if (CODEC_HI6402 == dma_drv_info->type) {
		...
		track_type = (uint32_t)SLIMBUS_6405_TRACK_SOUND_TRIGGER;
	}
#endif
	else {
		err = -EINVAL;
