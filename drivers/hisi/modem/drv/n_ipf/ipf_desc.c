...
void ipf_rd_h2s(IPF_RD_DESC_S* param, void* rd_base, unsigned int index)
{
	...
	param->InPtr		= (modem_phy_addr)(uintptr_t)rd[index].u32InPtr;
#ifdef CONFIG_PSAM
	...
	g_ipf_ctx.last_cd = (modem_phy_addr)(uintptr_t)rd[index].u32InPtr;
...
void ipf32_get_dlrd(unsigned int* pu32Num, IPF_RD_DESC_S *pstRd)
{
    ...
    for(i = 0; i < u32Num; i++)
    {
        ...
        if(ipf_enable == g_ipf_ctx.desc->cd_en_get(g_ipf_ctx.dl_info.pstIpfRDQ, u32RdqRptr)){
        	u32CdqRptr = ((unsigned long)SHD_DDR_P2V((void *)(uintptr_t)MDDR_FAMA(pstRd[i].InPtr)) -(uintptr_t)g_ipf_ctx.dl_info.pstIpfCDQ)/(unsigned long)sizeof(ipf_cd_s); //lint !e712
		...
		ipf_pm_print_packet((void *)(uintptr_t)MDDR_FAMA(pstRd[i].OutPtr), (unsigned int)pstRd[i].u16PktLen);//lint !e747
