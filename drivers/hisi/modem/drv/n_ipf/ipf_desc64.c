...
void ipf64_rd_h2s(IPF_RD_DESC_S* param, void* rd_base, unsigned int index)
{
    ...
    param->InPtr        = (modem_phy_addr)(uintptr_t)tmp.input_ptr.addr;
#ifdef CONFIG_PSAM
    param->OutPtr   = (modem_phy_addr)(uintptr_t)tmp.virt.ptr;
#else
    param->OutPtr   = (modem_phy_addr)(uintptr_t)tmp.output_ptr;
#endif
...
int ipf64_ad_s2h(IPF_AD_TYPE_E type, unsigned int n, IPF_AD_DESC_S * param)
{
	...
	for(i=0; i < n; i++)
	{
		...
		ad[wptr].output_ptr0.addr	= (uintptr_t)(param[i].OutPtr0);
		ad[wptr].output_ptr1.addr	= (uintptr_t)(param[i].OutPtr1);
...
void ipf64_ad_h2s(IPF_AD_DESC_S* param, void* ad_base, unsigned int index)
{
	...
	param->OutPtr0	= (modem_phy_addr)(uintptr_t)ad[index].output_ptr0.bits.ptr;
	param->OutPtr1	= (modem_phy_addr)(uintptr_t)ad[index].output_ptr1.bits.ptr;
}
...
void ipf64_get_dlrd(unsigned int* pu32Num, IPF_RD_DESC_S *pstRd)
{
    ...
    for(i = 0; i < u32Num; i++)
    {
        ...
        if(ipf_enable == g_ipf_ctx.desc->cd_en_get(g_ipf_ctx.dl_info.pstIpfRDQ, u32RdqRptr)){
            /* 更新CD读指针 */
            u32CdqRptr = ((unsigned long)(uintptr_t)SHD_DDR_P2V(pstRd[i].InPtr) - (unsigned 
            long)(uintptr_t)g_ipf_ctx.dl_info.pstIpfCDQ)/sizeof(ipf64_cd_s);//lint !e712
...
void ipf64_dump_callback(void)
{
    ...
    if(p == NULL) return;
    if(g_ipf_ctx.last_rd == NULL) return;
    ...
    if(g_ipf_ctx.last_bd == NULL) return;
    ...
    if(g_ipf_ctx.last_ad0 == NULL) return;
    ...
    if(g_ipf_ctx.last_ad1 == NULL) return;
    if(memcpy_s(p, size, g_ipf_ctx.last_ad1, sizeof(ipf64_ad_s)))
    {
        bsp_err("memcpy_s failed\n");
    }

    if(memcpy_s(&g_ipf_ctx.share_mem->drd, sizeof(g_ipf_ctx.share_mem->drd),
