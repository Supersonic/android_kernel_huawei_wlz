...
{
	struct drg_device *drg_dev = NULL, *tmp = NULL;
	struct drg_freq_device *freq_dev =
			container_of(work,
...
int drg_get_freq_range(struct drg_dev_freq *freq)
{
	...
	struct drg_freq_device *freq_dev = NULL;
	struct drg_device *slave = NULL;
	int idx, cluster, ret = DRG_NONE_DEV;
	unsigned int cpu;
			   ...
			   struct device_node *np, const char *prop)
{
	...
	*drg_dev = (struct drg_device *)
	...
	divider_size = sizeof(*(*drg_dev)->divider) * divider_num;
	if (divider_size > sizeof(drg_dev_spec.args)) {
		dev_err(dev, "divider size too big:%d\n", divider_size);
		return -ENOMEM;
	}
...
{
	pm_qos_add_notifier(PM_QOS_FREQ_LINK_LEVEL, &gpucpu_freq_link_notifier);
}
#else
static void freq_link_notifier_init(void)
{
