...
u32 g_wr_check_tbl[] = { EFUSE_FN_WR_MODEM_HUK_VALUE,
			 EFUSE_FN_WR_DIEID_VALUE,
			 EFUSE_FN_WR_CHIPID_VALUE,
			 EFUSE_FN_WR_AUTH_KEY_VALUE,
			 EFUSE_FN_ENABLE_DEBUG,
			 EFUSE_FN_DISABLE_DEBUG };

u32 g_rd_check_tbl[] = { EFUSE_FN_RD_MODEM_HUK_VALUE,
			 EFUSE_FN_RD_DIEID_VALUE,
			 EFUSE_FN_RD_CHIPID_VALUE,
			 EFUSE_FN_RD_AUTH_KEY_VALUE };
...
s32 get_efuse_avs_value(u8 *buf, u32 size, u32 timeout)
{
	...
	if (!buf || size != EFUSE_AVS_LENGTH_BYTES) {
		pr_err("%s: buf is NULL.\n", __func__);
		return -EFAULT;
	}
	if (g_efusec.is_init_success != EFUSE_MODULE_INIT_SUCCESS) {
