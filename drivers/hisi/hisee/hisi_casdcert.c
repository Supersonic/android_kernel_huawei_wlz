...
static int casd_process_receive(const char *buf, unsigned int curr_pack, unsigned int total_packs)
{
    ...
    if (curr_pack < total_packs ) {  /* save casd data */
        ...
        ret = memcpy_s(g_hisee_data.casd_data.buffer + g_hisee_data.casd_data.size,
				HISI_CASD_RECV_SIZE - g_hisee_data.casd_data.size, buf, data_str_size);
		if (ret != EOK) {
			pr_err("%s(): memcpy err.\n", __func__);
...
end_process:
    ...
    if(curr_pack == total_packs) {
        if(NULL != g_hisee_data.casd_data.buffer) {
            (void)memset_s(g_hisee_data.casd_data.buffer, HISI_CASD_RECV_SIZE, 0, HISI_CASD_RECV_SIZE);
