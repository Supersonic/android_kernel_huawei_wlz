...
输入：无
输出：无。
返回值：HISEE_OK:执行hisee nvm format基本操作成功；不是HISEE_OK:执行hisee nvm format基本操作失败
前置条件：cos flash镜像升级成功，启动cos flash镜像运行。
后置条件： 无
*************************************************************/
...
后置条件： 无
*************************************************************/
static int cos_flash_image_upgrade_func(const void *buf, unsigned int factory_test_flg)
{
...
int load_cos_flash_do_factory_test(const void *buf, int para)
{
	...
	if ((NULL == buf) || (HISEE_CHAR_NEWLINE == *(const char *)buf)
			|| ('\0' == *(const char *)buf)) {
		SET_COS_DEFAULT_BUF_PARA();
...
int hisee_factory_check_func(const void *buf, int para)
{
    ...
    if (HISEE_FACTORY_TEST_RUNNING != g_hisee_data.factory_test_state) {
        ...
        if (factory_check_test_task == NULL) {
            ret = HISEE_THREAD_CREATE_ERROR;
            g_hisee_data.factory_test_state = HISEE_FACTORY_TEST_FAIL;
...
ssize_t hisee_at_result_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    switch (g_at_cmd_type) {
        ...
        case HISEE_AT_CASD:
            if (HISEE_OK == ret) {
                ret = snprintf_s(buf, HISEE_BUF_SHOW_LEN,(size_t)HISEE_BUF_SHOW_LEN,
						"^HISEE:CASD,%d", g_hisee_data.casd_data.curr_pack);
                ...
                ret = snprintf_s(buf, HISEE_BUF_SHOW_LEN, (size_t)HISEE_BUF_SHOW_LEN,
						"^HISEE:CASD,%d,TRANSMIT,-1", g_hisee_data.casd_data.curr_pack);
				if (ret == HISEE_SECLIB_ERROR) {
                ...
                ret = snprintf_s(buf, HISEE_BUF_SHOW_LEN, (size_t)HISEE_BUF_SHOW_LEN,
								"^HISEE:CASD,%d,TRANSMIT,0", g_hisee_data.casd_data.curr_pack);
				if (ret == HISEE_SECLIB_ERROR) {
        ...
        case HISEE_AT_VERIFYCASD:
            if (HISEE_OK == ret) {
                ret = snprintf_s(buf, (size_t)HISEE_BUF_SHOW_LEN, (size_t)HISEE_BUF_SHOW_LEN,
								"^HISEE:VERIFYCASD,%d", g_hisee_data.casd_data.curr_pack);
				if (ret == HISEE_SECLIB_ERROR) {
                ...
                ret = snprintf_s(buf, (size_t)HISEE_BUF_SHOW_LEN, (size_t)HISEE_BUF_SHOW_LEN,
								"^HISEE:VERIFYCASD,%d,VERIFY,-2", g_hisee_data.casd_data.curr_pack);
				if (ret == HISEE_SECLIB_ERROR) {
