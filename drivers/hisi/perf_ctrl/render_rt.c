 ...
 * Do not call any fucntion that causes wakeup, such as, kmalloc/printk etc
 *
 ...
 * @exited:  flag to mark the task is destoryed
...
/*
 * is_stopped() - if the function to get the related thread is stopped
 */
static inline bool is_stopped(void)
{
	if (g_ta_render_ptr->stopped)
		return true;
	else
		return false;
}
 ...
 */
static inline bool both_render_rthread(struct task_struct *a, struct task_struct *b)
{
...
void add_render_rthread(struct task_struct *task)
{
	...
	/* MAX_TID_COUNT means the task have already added*/
	...
	if (rthreads[last_index].pid < task->pid) {
		...
		for (j = last_index; j >= 0; j--)
			rthreads[j+1] = rthreads[j]; /*lint !e679*/
		...
		if (g_ta_render_ptr->render_index >= i)
			g_ta_render_ptr->render_index++;
...
static inline void update_rthread_util(struct child_thread *thread, u64 window_size);

/*
 * remove_render_rthread() - mark the task is exited and update the util of the task
 *
...
void remove_render_rthread(struct task_struct *task)
{
	...
	/* render thread is killed accidently, uninitialized the module */
 ...
 *
 * The function will add waker to wakee's waker_array to save the wakeup relationship
 */
	...
	if (!test_bit(waker_index, thread->waker_bitmap)) {
		if (thread->waker_count < MAX_WAKER_COUNT) {
			...
			trace_add_waker_to_render_rthread(current, waker_index,
				task, wakee_index, thread->waker_count);
		}
	}
...
static inline void init_child_rthread(int index, struct task_struct *p)
{
	...
	(void)memset_s(thread->waker_bitmap, BITS_TO_LONGS(MAX_TID_COUNT) * sizeof(unsigned long),
...
/*
 * update_all_rthread_util() - update all child threads util, window_start and exited_thrad_nr
 */
...
int get_render_hrthread(void __user *uarg)
{
	...
	if (copy_from_user(&g_ht_result, uarg, sizeof(struct render_ht))) {
		pr_err("get_render_hrthread copy_from_user fail.\n");
		ret = -EFAULT;
		goto m_unlock;
	}
	...
	for(i = 0, thread_nr= 0; i < g_ta_render_ptr->total_count; i++) {
		...
		ta_threads[thread_nr].pid = g_ta_render_ptr->ta_child_threads[i].pid;
		ta_threads[thread_nr].util = g_ta_render_ptr->ta_child_threads[i].frame_demand;
		thread_nr++;
	}
...
static inline void reset_child_rthread(void)
{
	...
	for (index = 0; index < total_count; index++) {
		...
		if (unlikely(thread == NULL))
			continue ;
		thread->waker_count = 0;
		(void)memset_s(thread->waker_array, MAX_WAKER_COUNT * sizeof(int), 0, MAX_WAKER_COUNT * sizeof(int));
		(void)memset_s(thread->waker_bitmap, BITS_TO_LONGS(MAX_TID_COUNT) * sizeof(unsigned long),
					  0, BITS_TO_LONGS(MAX_TID_COUNT) * sizeof(unsigned long));
		thread->visted = false;
		thread->exited = false;
	...
	for (index = total_count - 1; index >= 0; index--) {
		...
		if (thread && thread->exited) {
			...
			for (i = index; i < last_index; i++) {
				curr_rthread = &g_ta_render_ptr->ta_child_threads[i];
				next_rthread = &g_ta_render_ptr->ta_child_threads[i+1]; /*lint !e679*/
				if (unlikely(curr_rthread == NULL || next_rthread == NULL))
					continue ;

				curr_rthread->pid = next_rthread->pid;
				curr_rthread->task = next_rthread->task;
				curr_rthread->last_walt_ravg_sum = next_rthread->last_walt_ravg_sum;
...
static inline void populate_rthread(int *rthread_results, int result_index, int index)
{
	int i = 0;
	int waker_index = 0;
	...
	do {
		if (waker_index < thread->waker_count) {
			...
			curr_waker_thread = &g_ta_render_ptr->ta_child_threads[curr_waker_index];
...
static inline void hierarchical_bfs_traverse(int *rthread_results)
{
	...
	do {
		...
		if (rthread_index != INVALID_THREAD_IDX) {
			...
			populate_rthread(rthread_results, result_start_index, rthread_index);
		}
		i++;
	} while (i < g_bfs_thread_nr);
}

static inline void populate_results(struct render_rt *result, const int *rthread_results)
{
	struct thread_util *rthread_utils = NULL;
	struct child_thread *thread = NULL;
	int thread_index, i;
 ...
 *
 * If inited params with rfps.force_init set, the function will reinit the modle any way.
 */
int init_render_rthread(const void __user *uarg)
{
	int i;
	int ret = -ENODEV;
	int count = 0;
	...
	child_tasks = kmalloc(sizeof(struct render_thread_task) * MAX_TID_COUNT, GFP_ATOMIC);
	if (!child_tasks)
		return -ENOMEM;

	write_lock_irqsave(&g_rt_rwlock, flags);

	/* if rps.force_init is ture, ignore the error */
	...
	(void)memset_s(&g_ta_render, sizeof(struct render_related_thread), 0, sizeof(struct render_related_thread));
	...
	sort(child_tasks, count,
		sizeof(struct render_thread_task), &cmp_render_task, NULL);
...
/*
 * destroy_render_rthread() - destory the module
 */
int destroy_render_rthread(const void __user *uarg)
{
	pid_t destory_pid;
	unsigned long flags;
	int ret = 0;

	if (copy_from_user(&destory_pid, uarg, sizeof(pid_t))) {
	...
	if (destory_pid != g_ta_render_ptr->pid) {
		ret = -ENODEV;
		goto unlock;
	}
...
/*
 * destroy_render_rthread() - stop the module getting bfs results
 */
int stop_render_rthread(const void __user *uarg)
{
