...
slimbus_channel_property_t audio_kws_6405[SLIMBUS_KWS_CHANNELS] = {{0,{0,},{{0,},},},};
slimbus_channel_property_t audio_capture_5mic_6405[SLIMBUS_AUDIO_CAPTURE_5MIC_CHANNELS] = {{0, {0,}, {{0,},},},};
slimbus_channel_property_t voice_up_4pa_6405[SLIMBUS_VOICE_UP_CHANNELS] = {{0, {0,}, {{0,},},},};
...
enum {
	...
	SLIMBUS_6405_TRACK_KWS_ON                    = 1 << SLIMBUS_6405_TRACK_KWS,
	SLIMBUS_6405_TRACK_5MIC_CAPTURE_ON           = 1 << SLIMBUS_6405_TRACK_5MIC_CAPTURE,
	SLIMBUS_6405_TRACK_VOICE_UP_4PA_ON           = 1 << SLIMBUS_6405_TRACK_VOICE_UP_4PA,
};
			...
			slimbus_track_param_t *params)
{
	...
	} else if (SLIMBUS_6405_TRACK_VOICE_UP_4PA == track_type) {
		dev->voice_up_chnum = params->channels;
...
static int process_normal_scene_conflict(slimbus_6405_track_type_t track, bool track_enable, int *need_callback)
{
	...
	if (track_state_is_on(SLIMBUS_6405_TRACK_VOICE_UP_4PA)
		&& (track == SLIMBUS_6405_TRACK_DEBUG)) {
		pr_info("[%s:%d] st and debug conflict\n", __FUNCTION__, __LINE__);
		return -EPERM;
	}
	if (track_state_is_on(SLIMBUS_6405_TRACK_DEBUG)
		&& (track == SLIMBUS_6405_TRACK_VOICE_UP_4PA)) {
		pr_info("[%s:%d] st and 4pa call conflict\n", __FUNCTION__, __LINE__);
		return -EPERM;
	}
	if (track_state_is_on(SLIMBUS_6405_TRACK_VOICE_UP_4PA)
		&& (track == SLIMBUS_6405_TRACK_SOUND_TRIGGER)) {
		pr_info("[%s:%d] st and sound trigger conflict\n",
			__FUNCTION__, __LINE__);
		return -EPERM;
	}
	if (track_state_is_on(SLIMBUS_6405_TRACK_SOUND_TRIGGER)
		&& (track == SLIMBUS_6405_TRACK_VOICE_UP_4PA)) {
		pr_info("[%s:%d] st and 4pa conflict\n", __FUNCTION__, __LINE__);
		return -EPERM;
	}
	if (track_state_is_on(SLIMBUS_6405_TRACK_DEBUG)
		&& (track == SLIMBUS_6405_TRACK_SOUND_TRIGGER)) {
...
static bool is_direct_play_scene(uint32_t active_tracks)
{
	...
	case SLIMBUS_6405_TRACK_DIRECT_PLAY_ON | SLIMBUS_6405_TRACK_CAPTURE_ON |
			SLIMBUS_6405_TRACK_EC_ON:
	case SLIMBUS_6405_TRACK_DIRECT_PLAY_ON | SLIMBUS_6405_TRACK_CAPTURE_ON |
			SLIMBUS_6405_TRACK_SOUND_TRIGGER_ON:
	case SLIMBUS_6405_TRACK_DIRECT_PLAY_ON | SLIMBUS_6405_TRACK_EC_ON |
			SLIMBUS_6405_TRACK_SOUND_TRIGGER_ON:
	case SLIMBUS_6405_TRACK_DIRECT_PLAY_ON | SLIMBUS_6405_TRACK_CAPTURE_ON |
...
static void direct_select_scenes(
                uint32_t active_tracks,
                slimbus_scene_config_6405_type_t *scene_config_type,
                ...
                slimbus_clock_gear_t *cg)
{
	...
	if (is_direct_play_scene(active_tracks)) {
		if (rate == SLIMBUS_SAMPLE_RATE_44K1 ||
			rate == SLIMBUS_SAMPLE_RATE_88K2 ||
			rate == SLIMBUS_SAMPLE_RATE_176K4) {
			*scene_config_type = SLIMBUS_6405_SCENE_CONFIG_DIRECT_PLAY_441;
		...
		pr_warn("[%s: %d] not direct scene, %x", __FUNCTION__, __LINE__, active_tracks);
	}
}
				...
				bool track_enable)
{
	...
	has_direct = ((active_tracks & SLIMBUS_6405_TRACK_DIRECT_PLAY_ON) == SLIMBUS_6405_TRACK_DIRECT_PLAY_ON);
	...
	if (is_play_scene(active_tracks)) {
		...
		direct_select_scenes(active_tracks, &scene_config_type, &sm, &cg);
	} else if (is_call_4pa_scene(active_tracks)) {
		scene_config_type = SLIMBUS_6405_SCENE_CONFIG_4PA_CALL;
		cg = SLIMBUS_CG_10;
		sm = SLIMBUS_SM_2_CSW_32_SL;
...
int slimbus_hi6405_track_soundtrigger_deactivate(uint32_t track)
{
	...
	status = get_slimbus_freq_update_status();
	slimbus_freq_release(status);
