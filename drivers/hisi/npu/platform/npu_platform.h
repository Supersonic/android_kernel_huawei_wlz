...
#define DEVDRV_RESMEM_MAX_RESOURCE (9)
...
enum devdrv_register_index {
	...
	//DEVDRV_REG_FW_CLUSTER0_SUBCTRL,
	//DEVDRV_REG_TS_FW_CORE_RESET_ENTRY,
...
struct devdrv_pm_adapter {
	int (*npu_power_up)(struct devdrv_dev_ctx *dev_ctx, u64 fw_load_addr, u32 is_secure);
	int (*npu_open)(void);
	int (*npu_release)(void);
	int (*npu_power_down)(void *svm_dev, u32 is_secure, u32 *stage);
};

struct devdrv_resource_adapter {
	int (*npu_fw_proc)(const struct file *fp, loff_t fsize,
