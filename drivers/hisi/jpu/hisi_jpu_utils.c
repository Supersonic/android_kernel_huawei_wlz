...
int hisi_jpu_register(struct hisi_jpu_data_type *hisijd)
{
	...
	hisijd->jpgdDrv = hipp_register(HISI_JPEGD_UNIT, NONTRUS);
	JPU_ERR_IF_COND((hisijd->jpgdDrv == NULL), "Failed: hipp_register\n");
#endif
...
#if defined(CONFIG_HISI_FB_V501) || defined(CONFIG_HISI_FB_V510)
	JPU_ERR_IF_COND(hipp_unregister(HISI_JPEGD_UNIT),
...
#if defined(CONFIG_HISI_FB_V501) || defined(CONFIG_HISI_FB_V510)
	if (hisijd->jpgdDrv && hisijd->jpgdDrv->enable_smmu) {
		if ((hisijd->jpgdDrv->enable_smmu(hisijd->jpgdDrv)) != 0)
...
static int hisi_jpu_dec_reg_init(struct hisi_jpu_data_type *hisijd)
{
	...
	// cppcheck-suppress *
	(void)memcpy_s(&(hisijd->jpu_dec_reg), sizeof(jpu_dec_reg_t),
...
static int hisi_jpu_dec_set_cvdr(struct hisi_jpu_data_type *hisijd)
{
	...
	if (cond) {
		...
		outp32(hisijd->jpu_cvdr_base +
				JPGDEC_CVDR_AXI_JPEG_CVDR_WR_QOS_CFG, 0x10333311);
		outp32(hisijd->jpu_cvdr_base +
				JPGDEC_CVDR_AXI_JPEG_CVDR_RD_QOS_CFG, 0x10333311);
...
{
	int ret = 0;
	bool cond = ((hisijd == NULL) || (jpu_req == NULL));
	...
	cond = !!(jpu_req->progressive_mode) || !!(jpu_req->arith_code);
	if (cond) {
...
static int hisi_jpu_lock_usecase(struct hisi_jpu_data_type *hisijd)
{
	...
	if (hisijd->jpgdDrv && hisijd->jpgdDrv->lock_usecase) {
		HISI_JPU_INFO("enter lock usecase\n");

		ret = hisijd->jpgdDrv->lock_usecase(hisijd->jpgdDrv);
		if (ret != 0) {
...
static void hisi_jpu_unlock_usecase(struct hisi_jpu_data_type *hisijd)
{
	...
	if (hisijd->jpgdDrv && hisijd->jpgdDrv->unlock_usecase) {
		HISI_JPU_INFO("enter unlock usecase\n");

		ret = hisijd->jpgdDrv->unlock_usecase(hisijd->jpgdDrv);
		if (ret != 0)
			HISI_JPU_ERR("failed to unlock usecase! ret is %lu\n", ret);
	}
#endif
}
