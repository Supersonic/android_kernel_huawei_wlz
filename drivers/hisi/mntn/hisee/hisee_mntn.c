...
#include "../../hisee/hisi_hisee_dcs.h"
#endif
...
struct rdr_exception_info_s hisee_excetption_info[] = {
...
#ifdef CONFIG_HISEE_SUPPORT_DCS
	{
		.e_modid = (u32)MODID_HISEE_EXC_DCS,
		.e_modid_end = (u32)MODID_HISEE_EXC_DCS,
		.e_process_priority = RDR_ERR,
		.e_reboot_priority = RDR_REBOOT_NO,
		.e_notify_core_mask = RDR_HISEE,
		.e_reset_core_mask = RDR_HISEE,
		...
		.e_reentrant = (u32)RDR_REENTRANT_DISALLOW,
		.e_exce_type = HISEE_S_EXCEPTION,
		.e_exce_subtype = EXC_DCS,
		.e_upload_flag = (u32)RDR_UPLOAD_YES,
		.e_save_log_flags   = RDR_SAVE_BL31_LOG,
		.e_from_module = "HISEE DCS",
		.e_desc = "HISEE",
	},
#endif
	{
	...
	{
		.e_modid            = (u32)MODID_HISEE_EXC_RST,
		.e_modid_end        = (u32)MODID_HISEE_EXC_RST,
		.e_process_priority = RDR_ERR,
		.e_reboot_priority  = RDR_REBOOT_NO,
		.e_notify_core_mask = RDR_HISEE,
		.e_reset_core_mask  = RDR_HISEE,
		...
		.e_reentrant        = (u32)RDR_REENTRANT_DISALLOW,
		.e_exce_type        = HISEE_S_EXCEPTION,
		.e_exce_subtype     = EXC_RST,
		.e_upload_flag      = (u32)RDR_UPLOAD_YES,
		.e_save_log_flags   = RDR_SAVE_BL31_LOG,
		.e_from_module      = "HISEE RST",
		.e_desc             = "HISEE",
	},
#endif
	{
...
static unsigned int translate_exc_type(u32 int_id)
{
	...
	for (i = 0; i < sizeof(hisee_exc_trans) /
		...
		if (int_id == hisee_exc_trans[i].int_id_value) {
...
{
	int ret = HISEE_OK;
#ifdef CONFIG_HISI_MNTN_CHECKMISC
	/*Only for the phones that has misc but low version(ver <= 0x40)*/
	if (HISEE_SW_VERSION_MAGIC_VALUE == misc_version_mntn.magic) {
		if (misc_version_mntn.img_version_num[0] <= 0x40) {
			pr_err("Old Hisee misc, do nothing !!misc ver is %d\n", misc_version_mntn.img_version_num[0]);
			ret = HISEE_ERROR;
		}
	}
#endif
...
#ifdef CONFIG_HISEE_SUPPORT_DCS
			if (hisee_exception_modid == (u32)MODID_HISEE_EXC_DCS) {
				hisee_dcs_data_load();
			}
#endif
	...
	pfn_cb_dump_done pfn_cb)
{
	...
	ret = snprintf_s(debug_cont, HISEE_MNTN_PATH_MAXLEN,
			 HISEE_MNTN_PATH_MAXLEN - 1,
	...
	/* save to 8M */
	ret = memcpy_s(hisee_mntn_addr, hisee_info.log_len,
					hisee_log_addr, (unsigned long)hisee_info.log_len);
	if (ret != EOK) {
		pr_err("%s(): memcpy err.\n", __func__);
		return;
	}
