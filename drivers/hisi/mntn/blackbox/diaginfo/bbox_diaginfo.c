...
#define	BBOX_DIAG_SPIN_LOCK(lock)   do { if (bbox_diaginfo_need_lock) spin_lock_irqsave(&lock, flags); } while (0)
#define	BBOX_DIAG_SPIN_UNLOCK(lock) do { if (bbox_diaginfo_need_lock) spin_unlock_irqrestore(&lock, flags); } while (0)
...
int bbox_diaginfo_register(unsigned int err_id, const char *date, const char *pdata, unsigned int data_len, u64 ts)
{
	...
	if (memcpy_s(&(p->diaginfo.msg), MAX_DIAGINFO_LEN - 1, pdata, data_len) != EOK)
...
static int bbox_diaginfo_save_to_buffer(struct bbox_diaginfo_param_s *pinfo)
{
	int ret = 0;
	char pmsg[DIAGINFO_STRING_MAX_LEN] = {'\0'};
	char *p_diaginfo_type = NULL;
	char *p_diaginfo_loglevel = NULL;
	char *p_diaginfo_module = NULL;
	struct diaginfo_map *p_diaginfo_map = NULL;
	...
	nRet = snprintf_s(pmsg, DIAGINFO_STRING_MAX_LEN, DIAGINFO_STRING_MAX_LEN - 1,
	"Time [%s], Module [%s], Error_ID [%s:%d], Log_Level [%s], Fault_Type [%s], Data [%s]\n",
	pinfo->diaginfo.date,
	p_diaginfo_module,
