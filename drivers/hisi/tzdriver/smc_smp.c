...
void show_cmd_bitmap(void)
{
	...
	for (idx = 0; idx < MAX_SMC_CMD; idx++) {
		if (cmd_in[idx] == -1)
			break;
		tloge("cmd[%d]: cmd_id=%d, read_pid=%d, dev_id = 0x%x, event_nr=%d, ret_val=0x%x\n",
			cmd_in[idx],
...
resleep:
			if (wait_event_timeout(pe->wq, atomic_read(&pe->run),
					RESLEEP_TIMEOUT*HZ) == 0) {
				tloge("timout %d s for spi wakeup\n", RESLEEP_TIMEOUT*HZ);
				if (!is_cmd_working_done(cmd_index))
					goto resleep;
				else
					tloge("cmd is done, maybe miss a spi wakeup!\n");
			}
			atomic_set(&pe->run, 0);
