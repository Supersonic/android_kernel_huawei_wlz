...
static unsigned int debug_mask = (INFO_BIT | ERROR_BIT);


#define rpmsg_err(fmt, args...) \
	do {                         \
		if (debug_mask & 0x01)   \
		printk(KERN_INFO "Rpmsg HISI Err: [%s] " \
			fmt, __func__, ##args); \
	} while (0)
#define rpmsg_info(fmt, args...)  \
	do {                         \
		if (debug_mask & 0x02)   \
		printk(KERN_INFO "Rpmsg HISI Info: [%s] " \
			fmt, __func__, ##args);  \
	} while (0)
#define rpmsg_dbg(fmt, args...)  \
	do {                         \
		if (debug_mask & 0x04) \
		printk(KERN_INFO "Rpmsg HISI Debug: [%s] " \
			fmt, __func__, ##args); \
	} while (0)
...
int hisp_rpmsg_rdr_init(void)
{
	...
	if (dev == NULL) {
		rpmsg_err("kzalloc failed, size.0x%lx\n",
