 ...
 */
/*
 * lint -e715 -e838 -e529 -e438 -e30 -e142 -e528 -e750 -e753 -e754 -e785
 * -e655 -e749 -e732 -e747 -e708 -e712 -e64 -e845 -e713 -e40 -e578 -e774
 ...
 *-esym(845,*) -esym(713,*) -esym(40,*) -esym(578,*) -esym(774,*)
 */
...
void wait_firmware_coredump(void)
{
	...
	do {
		...
		pr_info("[%s] exc_flag.0x%x\n", __func__, param->exc_flag);
		if ((param->exc_flag & CORE_DUMP_ALL_FINISH) ==
		     CORE_DUMP_ALL_FINISH)
			break;
...
static int isplog_open(struct inode *inode, struct file *filp)
{
	...
	if (!use_sec_isp() && !use_nonsec_isp()) {
		dev->share_mem = ioremap_nocache(get_a7sharedmem_addr(),
						 MEM_SHARED_SIZE);
		if (!dev->share_mem) {
			pr_err("[%s] Failed: share_mem.%pK\n",
				__func__, dev->share_mem);
		...
		unsigned long args)
{
	...
	case LOG_WR_OFFSET:
		ret = wait_event_timeout(dev->wait_ctl,
				...
				msecs_to_jiffies(POLLING_TIMEOUT_NS));
		if (ret == 0) {
