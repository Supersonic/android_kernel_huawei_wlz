...
int hisp_qos_dtget(struct device_node *np)
{
	...
	qos_offset = kmalloc(sizeof(unsigned int)*qos_num, GFP_KERNEL);
	if (qos_offset == NULL) {
		pr_err("[%s] Failed: alloc qos_offset\n", __func__);
		return -ENOMEM;
	}

	qos_value = kmalloc(sizeof(unsigned int)*qos_num, GFP_KERNEL);
	if (qos_value == NULL) {
...
int ispcpu_qos_cfg(void)
{
	...
	pr_info("[%s] vivobus_base.%pK, qos_num.%d",
			__func__, vivobus_base, qos_dev.num);
	...
	for (i = 0; i < qos_dev.num; i++) {
		__raw_writel(qos_dev.value[i],
		(volatile void __iomem*)(vivobus_base + qos_dev.offset[i]));
	}
