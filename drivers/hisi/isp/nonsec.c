...
#define MTCMOS_EN_GROUP0_PERI           (0x10)
...
int get_media1_subsys_power_state(void)
{
	...
	if (dev->use_actrl_reg) {
		cfg_base = get_regaddr_by_pa(ACTRL);
		if (cfg_base == NULL) {
			pr_err("[%s] cfg_base remap fail\n", __func__);
			return 0;
		}
		media_pw_stat = __raw_readl((volatile void __iomem*)(MTCMOS_EN_GROUP0_PERI + cfg_base));
		media_pw_stat &= (1 << (unsigned int)MEDIA1_MTCMOS_EN);
	} else {
		...
		media_pw_stat &= (1 << (unsigned int)MEDIASUBSYS_PWSTAT);
	}
...
int get_isptop_power_state(void)
{
	...
	if (dev->use_actrl_reg) {
		...
		isppwrstat = __raw_readl((volatile void __iomem*)(MTCMOS_EN_GROUP0_PERI + cfg_base));
		isppwrstat &= (1 << (unsigned int)ISP_TOP_MTCMOS_EN);
	} else {
		cfg_base = get_regaddr_by_pa(CRGPERI);
		if (cfg_base == NULL) {
			pr_err("[%s] cfg_base remap fail\n", __func__);
		...
		isppwrstat &= (1 << (unsigned int)PERPWRSTAT_ISP);
	}
...
static int disreset_ispcpu(u64 remap_addr)
{
	...
	__raw_writel((unsigned int)(ISP_A7_REMAP_ENABLE | (remap_addr >> 16)),
		(volatile void __iomem *)(isp_subctrl_base + ISP_SUB_CTRL_ISP_A7_CTRL_0));/*lint !e648 */
...
try_clock_down:
		if (ret != 0 && stat_machine < HISP_CLKDOWN_MAX && stat_machine >= 0)
			pr_info("[%s] Try Clock Down %lu.%lu MHz > %u.%u MHz\n",
				__func__, ispclk/1000000, ispclk%1000000,
				dev->clkdn[stat_machine][clk]/1000000,
				dev->clkdn[stat_machine][clk]%1000000);
	} while (ret != 0);
...
static int hisp_clock_enable(struct hisi_isp_nsec *dev, int clk)
{
	...
	if (ret < 0) {
		pr_err("[%s] Failed: %d.%d M, %d.%s.clk_set_rate.%d\n",
			__func__, (int)ispclock/1000000, (int)ispclock%1000000,
	...
	pr_info("[%s] %d.%s.clk_set_rate.%d.%d M\n", __func__,
		clk, dev->clk_name[clk], (int)ispclock/1000000,
		(int)ispclock%1000000);
		...
		case ISP_SYS_CLK:
			...
			if (ispclock != 0) {
				...
				pr_info("[%s] %d.%s.need_div clk_set_rate.%d.%d M\n",
					__func__, clk, dev->clk_name[clk],
					(int)ispclock/1000000, (int)ispclock%1000000);
			}
			...
			if (ret < 0) {
				pr_err("[%s] Failed: %d.%d M, %d.%s.clk_set_rate.%d\n",
					__func__, (int)ispclock/1000000,
					(int)ispclock%1000000, clk,
			...
			rproc_set_shared_clk_value(clk, (unsigned int)ispclock);
			pr_info("[%s] %d.%s.clk_set_rate.%d.%d M\n", __func__, clk,
					dev->clk_name[clk], (int)ispclock/1000000,
					(int)ispclock%1000000);
			break;
		default:
			break;
...
int secnsec_setclkrate(unsigned int type, unsigned int rate)
{
	...
	if ((rate > dev->ispclk_value[type]) || (rate == 0)) {
		pr_err("[%s] Failed : DVFS type.0x%x.%s, %d. %d.%08d M > %d. %d.%08d M\n",
				__func__, type, dev->clk_name[type], rate, rate/1000000,
				rate%1000000, dev->ispclk_value[type],
				dev->ispclk_value[type]/1000000, dev->ispclk_value[type]%1000000);
		return -EINVAL;
	}
	...
	pr_info("[%s] DVFS Set.0x%x.%s Rate.%d. %d.%08d M\n", __func__, type,
			dev->clk_name[type], rate, rate/1000000, rate%1000000);
...
int hisp_powerup(void)
{
	...
	if (use_nonsec_isp() || (!sec_process_use_ca_ta())) {
		ret = isptop_power_up(dev->useisptop);
		if (ret < 0) {
...
err_isptop:
	if (use_nonsec_isp() || (!sec_process_use_ca_ta())) {
		err = regulator_disable(dev->ispcore_supply);
		if (err != 0)
...
err_ispclk:
	...
	if (dev->useclockdep) {
		err = regulator_disable(dev->clockdep_supply);
		if (err < 0)
...
int hisp_powerdn(void)
{
	...
	if (use_nonsec_isp() || (!sec_process_use_ca_ta())) {
		ret = isptop_power_down(dev->useisptop);
		if (ret != 0) {
			pr_err("[%s] Failed: isptop_power_down.%d\n",
					__func__, ret);
			pr_err("[%s] Failed: useisptop.%d\n",
...
int hisp_nsec_jpeg_powerup(void)
{
	...
	pr_info("[%s] refs_a7.0x%x, refs_isp.0x%x, refs_ispinit.0x%x\n",
		__func__, ops->refs_a7, ops->refs_isp, ops->refs_ispinit);
	mutex_unlock(&ops->lock);
...
static int hisi_isp_nsec_getdts_dvfs(struct device_node *np,
...
static int hisi_isp_nsec_getdts(struct platform_device *pdev,
					struct hisi_isp_nsec *dev)
{
	...
	ret = of_property_read_u32(np, "useisptop",
				(unsigned int *)(&dev->useisptop));
	if (ret < 0) {
		pr_err("[%s] Failed: useisptop get from dtsi.%d\n",
				__func__, ret);
		return -EINVAL;
	}
	...
	ret = of_property_read_u32(np, "usepowerdn_clockdn",
			(unsigned int *)(&dev->usepowerdn_clockdn));
	if (ret < 0) {
		pr_err("[%s] Failed: usepowerdn_clockdn get from dtsi.%d\n",
				__func__, ret);
		return -EINVAL;
	}
	pr_info("[%s] usepowerdn_clockdn.0x%x\n", __func__,
			dev->usepowerdn_clockdn);

	ret = of_property_read_u32(np, "useclockdep",
				(unsigned int *)(&dev->useclockdep));
	if (ret < 0) {
		pr_err("[%s] Failed: useclockdep get from dtsi.%d\n",
				__func__, ret);
		return -EINVAL;
	}
	pr_info("[%s] useclockdep.0x%x\n", __func__, dev->useclockdep);

	ret = of_property_read_u32(np, "use-actrl-flag",
				(unsigned int *)(&dev->use_actrl_reg));
	if (ret < 0) {
		pr_err("[%s] Failed: use-actrl-flag get from dtsi.%d\n",
				__func__, ret);
		return -EINVAL;
	}
	pr_info("[%s] use_actrl_reg.0x%x\n", __func__,
			dev->use_actrl_reg);
	...
	ret = hisi_isp_nsec_getdts_dvfs(np, dev);
	if (ret < 0) {
...
int hisi_isp_nsec_probe(struct platform_device *pdev)
{
	...
	ret = hisi_isp_nsec_getdts(pdev, dev);
	if (ret != 0) {
