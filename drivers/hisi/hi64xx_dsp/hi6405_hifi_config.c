...
static bool dsp_48m_enable = false;

static void hi6405_hifi_runstall_cfg(bool pull_down)
{
	if (pull_down) {
		hi64xx_hifi_reg_clr_bit(DSP_CTRL0_REG, HIFI_STATVECTORSEL_OFFSET);
		/* Pull down runstall of HIFI*/
		hi64xx_hifi_reg_clr_bit(SC_DSP_CTRL0_REG, SC_DSP_SFT_RUNSTALL_OFFSET);
	} else {
		/* Pull up runstall of HIFI*/
		hi64xx_hifi_reg_set_bit(SC_DSP_CTRL0_REG, SC_DSP_SFT_RUNSTALL_OFFSET);
	}
}
...
{
	if (enable) {
		hi64xx_hifi_reg_set_bit(APB_CLK_CFG_REG, WD_PCLK_EN_OFFSET);
	} else {
		hi64xx_hifi_reg_clr_bit(APB_CLK_CFG_REG, WD_PCLK_EN_OFFSET);
	}
}

static void hi6405_i2c_enable(bool enable)
{
	if (enable)
		hi64xx_hifi_reg_set_bit(CFG_CLK_CTRL_REG, INTF_I2C_CLK_EN_OFFSET);
	else
		hi64xx_hifi_reg_clr_bit(CFG_CLK_CTRL_REG, INTF_I2C_CLK_EN_OFFSET);
}
...
static void hi6405_hifi_notify_dsp(void)
{
	...
	hi64xx_hifi_reg_set_bit(DSP_NMI_REG, HIFI_IRQ_REQ_OFFSET);

	while (wait_cnt) {
		if (0x0 == (hi64xx_hifi_read_reg(DSP_NMI_REG) & 0xc))
			break;
	...
	if (0 == wait_cnt)
		HI64XX_DSP_ERROR("dsp do not handle msg, DSP_NMI:0x%x\n",
			hi64xx_hifi_read_reg(DSP_NMI_REG));
...
{
	if (enable) {
		hi64xx_hifi_reg_set_bit(DSP_CLK_CFG_REG, HIFI_DIV_CLK_EN_OFFSET);
		hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, HIFI_PERI_CLK_EN_OFFSET);
		hi64xx_hifi_reg_set_bit(SC_DSP_CTRL0_REG, SC_HIFI_CLK_EN_OFFSET);
		...
		hi64xx_hifi_reg_clr_bit(SC_DSP_CTRL0_REG, SC_HIFI_CLK_EN_OFFSET);
		hi64xx_hifi_reg_clr_bit(DSP_CTRL6_DMAC_REG, HIFI_PERI_CLK_EN_OFFSET);
		hi64xx_hifi_reg_clr_bit(DSP_CLK_CFG_REG, HIFI_DIV_CLK_EN_OFFSET);
	}
}
...
static void hi6405_hifi_init(void)
{
	...
	hi64xx_hifi_reg_set_bit(SW_RST_REQ_REG, HIFI_CORE_SRST_REQ_OFFSET);
	hi64xx_hifi_reg_set_bit(SW_RST_REQ_REG, HIFI_SRST_REQ_OFFSET);
	hi64xx_hifi_reg_set_bit(SW_RST_REQ_REG, HIFI_PERI_SRST_REQ_OFFSET);
	hi64xx_hifi_reg_set_bit(SW_RST_REQ_REG, DSP_PD_SRST_REQ_OFFSET);

	/* close all dspif clocks */
	hi64xx_hifi_write_reg(HI6405_DSP_DSPIF_CLK_EN, 0x0);

	/* enable dsp debug */
	hi64xx_hifi_reg_clr_bit(DSP_CTRL2_REG, HIFI_DEBUGRST_N_OFFSET);
	hi64xx_hifi_reg_clr_bit(DSP_CTRL1_REG, HIFI_OCDHALTONRESET_N_OFFSET);

	/* power on dsp_top_mtcmos_ctrl*/
	hi64xx_hifi_reg_clr_bit(DSP_LP_CTRL0_REG, DSP_TOP_MTCMOS_CTRL_OFFSET);
	/* enable dsp_top_iso_ctrl */
	hi64xx_hifi_reg_clr_bit(DSP_LP_CTRL0_REG, DSP_TOP_ISO_CTRL_OFFSET);
	/* enable axi lowpower bydefault */
	hi64xx_hifi_reg_clr_bit(DSP_LP_CTRL0_REG, AXI_CSYSREQ_OFFSET);

	/* enable low power ctrl by default*/
	hi64xx_hifi_reg_set_bit(SC_DSP_CTRL0_REG, SC_DSP_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(SC_DSP_CTRL0_REG, SC_DSP_BP_OFFSET);

	/* set runstall ctrl by software */
	hi64xx_hifi_reg_set_bit(SC_DSP_CTRL0_REG, SC_DSP_RUNSTALL_BP_OFFSET);
	/* set hifi clk ctrl by software */
	hi64xx_hifi_reg_set_bit(SC_DSP_CTRL0_REG, SC_DSP_HIFI_DIV_BP_OFFSET);

	/* enable hifi clk */
	hi64xx_hifi_reg_set_bit(SC_DSP_CTRL0_REG, SC_HIFI_ACLK_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(SC_DSP_CTRL0_REG, SC_HIFI_CLK_EN_OFFSET);
	...
	/* enable dsp autoclk  */
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, DW_AXI_X2P_CG_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, DW_P2P_ASYNC_CG_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, DMAC_ACLK0_CG_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, DMAC_ACLK1_CG_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, DW_X2X_CG_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, UART_CG_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, HIFI_PERI_CLK_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL6_DMAC_REG, PERI_DW_AXI_X2P_CG_EN_OFFSET);

	hi64xx_hifi_reg_set_bit(APB_CLK_CFG_REG, APB_PD_PCLK_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(APB_CLK_CFG_REG, APB2RAM_PCLK_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(APB_CLK_CFG_REG, APB_CLK_EN_OFFSET);

	/* disable watchdog */
	hi64xx_hifi_reg_clr_bit(APB_CLK_CFG_REG, WD_PCLK_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(APB_CLK_CFG_REG, TIMER0_PCLK_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(APB_CLK_CFG_REG, TIMER1_PCLK_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(APB_CLK_CFG_REG, GPIO_PCLK_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(DSP_PERI_CLKEN1_REG, I2C_MST_CLK_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(DSP_PERI_CLKEN1_REG, UART_CLK_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(I2C_TO_CTRL1_REG, UART_DIV_EN_OFFSET);

	hi64xx_hifi_reg_set_bit(DSP_AXI_CLKEN1_REG, DW_AXI_GLB_CG_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_AXI_CLKEN1_REG, IOS_CG_EN_OFFSET);

	/* enable hifi auto clk */
	hi64xx_hifi_write_reg(DSP_CG_CTRL0_REG, 0x0);
	hi64xx_hifi_write_reg(DSP_CG_CTRL1_REG, 0x0);

	/* release dsp_pd_srst_req */
	hi64xx_hifi_reg_clr_bit(SW_RST_REQ_REG, DSP_PD_SRST_REQ_OFFSET);
	hi64xx_hifi_reg_clr_bit(SW_RST_REQ_REG, HIFI_PERI_SRST_REQ_OFFSET);
	hi64xx_hifi_reg_clr_bit(SW_RST_REQ_REG, HIFI_SRST_REQ_OFFSET);
	hi64xx_hifi_reg_clr_bit(SW_RST_REQ_REG, HIFI_CORE_SRST_REQ_OFFSET);

	/* enable dsp debug */
	hi64xx_hifi_reg_set_bit(DSP_CTRL2_REG, HIFI_DEBUGRST_N_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_CTRL1_REG, HIFI_OCDHALTONRESET_N_OFFSET);
...
static void hi6405_hifi_deinit(void)
{
	...
	/* Close HIFI clock */
	hi64xx_hifi_reg_clr_bit(SC_DSP_CTRL0_REG, SC_DSP_EN_OFFSET);

	/* Close APB clock */
	hi64xx_hifi_reg_clr_bit(APB_CLK_CFG_REG, APB_PD_PCLK_EN_OFFSET);

	/* Close DSPIF clocks, and soft reset DSPIF */
	hi64xx_hifi_write_reg(HI6405_DSP_DSPIF_CLK_EN, 0x0);

	/* Enable isolation cell */
	hi64xx_hifi_reg_set_bit(DSP_LP_CTRL0_REG, DSP_TOP_ISO_CTRL_OFFSET);
	...
	hi64xx_hifi_reg_set_bit(DSP_LP_CTRL0_REG, DSP_TOP_MTCMOS_CTRL_OFFSET);
...
static void hi6405_soundtrigger_fasttrans_ctrl(bool enable, bool fm)
{
	...
	if (enable) {
		/* DSP IF 3 */
		hi64xx_hifi_write_reg(DSPIF_VLD_CTRL1_REG, 0x36);/* 192K */
		/* DSP IF 1 */
		hi64xx_hifi_write_reg(DSPIF_VLD_CTRL0_REG, 0xC); /* 16k in 48k out */


		hi64xx_hifi_reg_write_bits(SLIM_UP_EN1_REG,
			0x1<<SLIM_UP1_EN_OFFSET | 0x1<<SLIM_UP2_EN_OFFSET ,
			0x1<<SLIM_UP1_EN_OFFSET | 0x1<<SLIM_UP2_EN_OFFSET);
		hi64xx_hifi_reg_write_bits(SLIM_UP_EN2_REG,
			0x1<<SLIM_UP9_EN_OFFSET ,
			0x1<<SLIM_UP9_EN_OFFSET );

		hi64xx_hifi_write_reg(SC_FS_SLIM_CTRL_7_REG,0x6<<FS_SLIM_UP9_OFFSET); /* 192K */

		hi64xx_hifi_write_reg(SC_FS_SLIM_CTRL_3_REG, 0x44); /* 48K */
	} else {
		/* DSP IF 3 */
		hi64xx_hifi_write_reg(DSPIF_VLD_CTRL1_REG, 0x24);/* 48K */
		...
		hi64xx_hifi_write_reg(SC_FS_SLIM_CTRL_7_REG,0x2<<FS_SLIM_UP9_OFFSET); /* 32K */
	}
...
static void hi6405_dsp_if_set_bypass(unsigned int dsp_if_id, bool bypass)
{
	...
	case HI64XX_HIFI_DSP_IF_PORT_0:
		hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL8_REG,
				bit_val << S1_I_DSP_BYPASS_OFFSET,
				1 << S1_I_DSP_BYPASS_OFFSET);
		break;
	case HI64XX_HIFI_DSP_IF_PORT_1:
		hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL22_REG,
				bit_val << S1_O_DSP_BYPASS_OFFSET,
				1 << S1_O_DSP_BYPASS_OFFSET);
		break;
	case HI64XX_HIFI_DSP_IF_PORT_2:
		hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL8_REG,
				bit_val << S2_I_DSP_BYPASS_OFFSET,
				1 << S2_I_DSP_BYPASS_OFFSET);
		break;
	case HI64XX_HIFI_DSP_IF_PORT_3:
		hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL22_REG,
				bit_val << S2_O_DSP_BYPASS_OFFSET,
				1 << S2_O_DSP_BYPASS_OFFSET);
		break;
	case HI64XX_HIFI_DSP_IF_PORT_4:
		hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL8_REG,
				bit_val << S3_I_DSP_BYPASS_OFFSET,
				1 << S3_I_DSP_BYPASS_OFFSET);
		break;
	case HI64XX_HIFI_DSP_IF_PORT_5:
		hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL22_REG,
				bit_val << S3_O_DSP_BYPASS_OFFSET,
				1 << S3_O_DSP_BYPASS_OFFSET);
		break;
	case HI64XX_HIFI_DSP_IF_PORT_6:
		hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL8_REG,
				bit_val << S4_I_DSP_BYPASS_OFFSET,
				1 << S4_I_DSP_BYPASS_OFFSET);
		break;
	case HI64XX_HIFI_DSP_IF_PORT_7:
		if (bypass) {
			hi64xx_hifi_reg_write_bits(SC_CODEC_MUX_CTRL37_REG,
...
{
	/* mad_buf_clk ->en */
	hi64xx_hifi_reg_set_bit(I2S_DSPIF_CLK_EN_REG, MAD_BUF_CLK_EN_OFFSET);
	/* mad_buffer ->en */
	hi64xx_hifi_reg_set_bit(MAD_BUFFER_CTRL1_REG, MAD_BUF_EN_OFFSET);
	/* mad_vad_ao ->en */
	hi64xx_hifi_reg_set_bit(MAD_CTRL_REG, MAD_VAD_AO_OFFSET);
	/* mad irq ->en */
	hi64xx_hifi_reg_set_bit(MAD_CTRL_REG, MAD_INT_EN_OFFSET);
	/* mad ->en */
	hi64xx_hifi_reg_set_bit(MAD_CTRL_REG, MAD_EN_OFFSET);
}

void hi6405_mad_disable(void)
{
	/* mad_buf_clk ->en */
	hi64xx_hifi_reg_clr_bit(I2S_DSPIF_CLK_EN_REG, MAD_BUF_CLK_EN_OFFSET);
	/* mad_buffer ->dis */
	hi64xx_hifi_reg_clr_bit(MAD_BUFFER_CTRL1_REG, MAD_BUF_EN_OFFSET);
	/* mad_vad_ao ->dis */
	hi64xx_hifi_reg_clr_bit(MAD_CTRL_REG, MAD_VAD_AO_OFFSET);
	/* mad irq ->dis */
	hi64xx_hifi_reg_clr_bit(MAD_CTRL_REG, MAD_INT_EN_OFFSET);
	/* mad ->dis */
	hi64xx_hifi_reg_clr_bit(MAD_CTRL_REG, MAD_EN_OFFSET);
}

static void hi6405_enable_mad_auto_clk(void)
{
	hi64xx_hifi_reg_clr_bit(SC_MAD_CTRL0_REG, SC_MAD_ANA_BP_OFFSET);
	hi64xx_hifi_reg_clr_bit(SC_MAD_CTRL0_REG, SC_MAD_PLL_BP_OFFSET);
	hi64xx_hifi_reg_clr_bit(SC_MAD_CTRL0_REG, SC_MAD_MIC_BP_OFFSET);
	hi64xx_hifi_reg_clr_bit(SC_MAD_CTRL0_REG, SC_MAD_PGA_BP_OFFSET);
}

static void hi6405_disable_mad_auto_clk(void)
{
	hi64xx_hifi_reg_set_bit(SC_MAD_CTRL0_REG, SC_MAD_ANA_BP_OFFSET);
	hi64xx_hifi_reg_set_bit(SC_MAD_CTRL0_REG, SC_MAD_PLL_BP_OFFSET);
	hi64xx_hifi_reg_set_bit(SC_MAD_CTRL0_REG, SC_MAD_MIC_BP_OFFSET);
	hi64xx_hifi_reg_set_bit(SC_MAD_CTRL0_REG, SC_MAD_PGA_BP_OFFSET);
}
...
static void hi6405_set_dsp_div(enum pll_state pll_state)
{
	...
	case PLL_HIGH_FREQ:
		...
		hi64xx_hifi_reg_clr_bit(DSP_CLK_CFG_REG, HIFI_DIV_CLK_EN_OFFSET);
		hi64xx_hifi_reg_clr_bit(CLK_SOURCE_SW_REG, DSP_CLK_SW_OFFSET);
		hi64xx_hifi_reg_write_bits(DSP_CLK_CFG_REG, 0x1, 0xF);
		hi64xx_hifi_reg_write_bits(DSP_CLK_CFG_REG, 0x51, 0xFF);
		hi64xx_hifi_reg_set_bit(DSP_CLK_CFG_REG, HIFI_DIV_CLK_EN_OFFSET);
		break;
	case PLL_LOW_FREQ:
		hi6405_enable_mad_auto_clk();
		hi64xx_hifi_reg_clr_bit(DSP_CLK_CFG_REG, HIFI_DIV_CLK_EN_OFFSET);
		hi64xx_hifi_reg_set_bit(CLK_SOURCE_SW_REG, DSP_CLK_SW_OFFSET);
		if (dsp_48m_enable)
			hi64xx_hifi_reg_write_bits(DSP_CLK_CFG_REG, 0x1, 0xF);
		else
			hi64xx_hifi_reg_write_bits(DSP_CLK_CFG_REG, 0x3, 0xF);
		hi64xx_hifi_reg_set_bit(DSP_CLK_CFG_REG, HIFI_DIV_CLK_EN_OFFSET);
		break;
	default:
		break;
...
static bool hi6405_check_dp_clk(void)
{
	...
	while(--count) {
		if(1 == hi64xx_hifi_read_reg(CODEC_DP_CLKEN_REG)) {
			return true;
		} else {
...
static bool hi6405_check_i2s2_clk(void)
{
	...
	s2_ctrl = hi64xx_hifi_read_reg(SC_CODEC_MUX_CTRL26_REG);
						...
						unsigned int sample_rate_in, unsigned int sample_rate_out)
{
	...
	if (hi6405_sc_fs_ctrl_regs[dsp_if_id].in_offset >= 0 && hi64xx_get_sample_rate_index(sample_rate_in, &sample_rate_in_index)) {
		hi64xx_hifi_reg_write_bits(hi6405_sc_fs_ctrl_regs[dsp_if_id].reg_addr,
			sample_rate_in_index<<(unsigned int)hi6405_sc_fs_ctrl_regs[dsp_if_id].in_offset, 0x7<<(unsigned int)hi6405_sc_fs_ctrl_regs[dsp_if_id].in_offset);
	} else {
	...
	if (hi6405_sc_fs_ctrl_regs[dsp_if_id].out_offset >= 0 && hi64xx_get_sample_rate_index(sample_rate_out, &sample_rate_out_index)) {
		hi64xx_hifi_reg_write_bits(hi6405_sc_fs_ctrl_regs[dsp_if_id].reg_addr,
			sample_rate_out_index<<(unsigned int)hi6405_sc_fs_ctrl_regs[dsp_if_id].out_offset, 0x7<<(unsigned int)hi6405_sc_fs_ctrl_regs[dsp_if_id].out_offset);
	} else {
...
{
	hi64xx_hifi_reg_set_bit(DSP_PERI_CLKEN1_REG, USB_HCLK_EN_OFFSET);
	hi64xx_hifi_reg_set_bit(DSP_AXI_CLKEN1_REG, USB_HRST_EN_OFFSET);
	hi64xx_hifi_reg_write_bits(SOUNDWIRE_CTRL0_REG,
		0x1<<USB_PCLK_EN_OFFSET | 0x1<<USB_PRST_EN_OFFSET,
		0x1<<USB_PCLK_EN_OFFSET | 0x1<<USB_PRST_EN_OFFSET);
	hi64xx_hifi_write_reg(USB20_CTRL_I_1, 0x400001);
	hi64xx_hifi_reg_clr_bit(DSP_PERI_CLKEN1_REG, USB_HCLK_EN_OFFSET);
	hi64xx_hifi_reg_clr_bit(DSP_AXI_CLKEN1_REG, USB_HRST_EN_OFFSET);
	hi64xx_hifi_reg_write_bits(SOUNDWIRE_CTRL0_REG, 0x0,
				...
				enum bustype_select bus_sel)
{
	...
	dsp_config.dsp_ops.i2c_enable = hi6405_i2c_enable;
	...
	if (of_property_read_bool(codec->dev->of_node, "dsp_48m_enable")) {
		dsp_48m_enable = true;
		dsp_config.dsp_48m_enable = true;
		HI64XX_DSP_INFO("48m config enable\n");
	} else {
		dsp_48m_enable = false;
		dsp_config.dsp_48m_enable = false;
		HI64XX_DSP_INFO("48m config disable\n");
	}
