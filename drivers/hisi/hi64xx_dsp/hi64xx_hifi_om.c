...
static int _add_data_to_list(struct hook_runtime *runtime)
{
	...
	if (runtime->idle_id != _get_idle_buffer_id(runtime))
...
#ifdef CONFIG_SND_SOC_HI6405
	else if (HI64XX_CODEC_TYPE_6405 == codec_type) {
		priv->slimbus_device_type = SLIMBUS_DEVICE_HI6405;
		priv->slimbus_track_type = (unsigned int)SLIMBUS_6405_TRACK_DEBUG;
	}
#endif
	else {
