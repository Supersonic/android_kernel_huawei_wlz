...
static void ivp_ipc_remove_one_packet(
	struct ivp_ipc_queue *queue __attribute__((unused)), struct ivp_ipc_packet *packet)
{
	if (packet)
...
/* trans ion fd to phyaddr */
...
#if   defined(SEC_IVP_V250)
		if ((CMD_ALGORUN == ((unsigned char)tmp_buff[3] & TMP_BUFF_MASK)) && // 3 is array element
			(tmp_buff[0] == 0 || 1 == tmp_buff[0]))
#endif
		{
			ret = ivp_trans_sharefd_to_phyaddr(&pdev->ipc_pdev->dev,
				(unsigned int *)tmp_buff, size);
			if (ret != 0) {
	...
	if (tmp_buff[0] == 0) {
		ivp_info("receive ipc cmd 0x%x",
			((unsigned char)tmp_buff[3] & TMP_BUFF_MASK)); // 3 is array element
	}
...
static int ivp_ipc_release(struct inode *inode __attribute__((unused)), struct file *file)
{
	struct ivp_ipc_device *pdev =
		(struct ivp_ipc_device *)file->private_data;
