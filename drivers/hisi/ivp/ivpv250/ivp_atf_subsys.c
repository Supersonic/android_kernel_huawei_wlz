...
noinline int atfd_hisi_service_ivp_smc(u64 _function_id,
	...
	, unsigned int len, unsigned long ddr_addr)
{
	if ((ivp_addr & MASK_1MB) != 0 || (
		ddr_addr & MASK_1MB) != 0 ||
		len >= 128 * SIZE_1MB) {
		ivp_dbg("ivp_addr:0x%x, ddr_addr:0x%lx, len:0x%x not aligned"
			, ivp_addr, ddr_addr, len);
		ivp_err("ivp_addr or ddr_addr not aligned");
		return -EINVAL;
	}
	len = (len + SIZE_1MB - 1) / SIZE_1MB - 1;
	atfd_hisi_service_ivp_smc(
		IVP_REMAP_SECMODE,
		ivp_addr / SIZE_1MB,
		ddr_addr / SIZE_1MB,
		len);
