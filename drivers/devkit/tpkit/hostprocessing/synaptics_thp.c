...
static int syna_tcm_send_cmd(struct spi_device *client,
	char *buf, unsigned int length)
{
	...
	while (retry_times) {
		...
		ret = syna_tcm_spi_read(client, resp_buf, SYNA_RESPONSE_LEN);
...
static int syna_tcm_disable_frame(struct thp_device *tdev, int value)
{
	...
	return syna_tcm_send_cmd(
		tdev->thp_core->sdev,
		cmd_buf,
		FRAME_CMD_LEN);
}

static int syna_tcm_set_dynamic_config(struct thp_device *tdev,
...
static int thp_synaptic_parse_gesture_data(struct thp_device *tdev,
...
static int thp_synaptics_wrong_touch(struct thp_device *tdev)
{
...
static int thp_synaptics_gesture_report(struct thp_device *tdev,
	unsigned int *gesture_wakeup_value)
{
	...
	retval = thp_synaptic_parse_gesture_data(tdev,
...
static int synaptics_power_off_for_3909(struct thp_device *tdev)
{
	int ret;
...
static int synaptics_power_off(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == SYNA3909)
		return synaptics_power_off_for_3909(tdev);
	gpio_set_value(tdev->gpios->rst_gpio, GPIO_LOW);
...
static void thp_synaptics_gesture_mode_enable_switch(
	struct thp_device *tdev, unsigned int value)
{
	int ret_disable_frame;
	int ret_set_config;

	ret_disable_frame = syna_tcm_disable_frame(tdev, value);
	ret_set_config = syna_tcm_set_dynamic_config(tdev,
...
static int thp_synaptics_resume(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->self_control_power) {
		if (is_pt_test_mode(tdev)) {
			...
			ret = syna_tcm_spi_write(sdev, spi_write_buf,
				SYNA_CMD_LEN);
			if (ret < 0)
...
static int thp_synaptics_suspend(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->self_control_power) {
		...
		if (gesture_status == TS_GESTURE_MODE &&
			...
			msleep(120);
			thp_synaptics_gesture_mode_enable_switch(tdev,
...
static int thp_synaptics_switch_int_sh(struct thp_device *tdev)
{
	...
	memset(spi_write_buf, 0, SPI_READ_WRITE_SIZE);
	memcpy(spi_write_buf, report_to_shb_cmd, SYNA_CMD_LEN);
	ret = syna_tcm_spi_write(sdev, spi_write_buf, SYNA_CMD_LEN);
	if (ret < 0)
		THP_LOG_ERR("%s: send cmd failed\n", __func__);
	thp_bus_unlock();
	return 0;
...
struct thp_device_ops syna_dev_ops = {
	...
	.chip_wrong_touch = thp_synaptics_wrong_touch,
	.chip_gesture_report = thp_synaptics_gesture_report,
#ifdef CONFIG_HUAWEI_SHB_THP
