...
static int thp_ilitek_spi_read_write(struct spi_device *client,
...
static void thp_ilitek_hw_reset(struct thp_device *tdev)
{
...
static int thp_ilitek_init(struct thp_device *tdev)
{
	int ret;
	...
	if (!ilitek_node) {
		THP_LOG_ERR("%s: ilitek dev not config in dts\n", __func__);
		return -ENODEV;
	}
...
static int ilitek_spi_write(struct spi_device *sdev, const void *buf, int len)
{
	int ret = 0;
	u8 *txbuf = NULL;
	...
	if (thp_ilitek_spi_read_write(sdev, txbuf, NULL, safe_size) < 0) {
...
static int ilitek_spi_read(struct spi_device *sdev, u8 *buf, int len)
{
	int i;
	int ret = 0;
	u8 *txbuf = NULL;
	...
	txbuf[0] = SPI_READ;
	if (thp_ilitek_spi_read_write(sdev, txbuf, buf, len) < 0) {
...
static int ilitek_ice_mode_read(struct spi_device *sdev, u32 addr, u32 *data,
	int len)
{
	int ret;
	u8 *rxbuf = NULL;
	...
	ret = ilitek_spi_write(sdev, txbuf, REGISTER_LEN);
	if (ret < 0) {
		THP_LOG_ERR("%s: spi write failed\n", __func__);
		goto out;
	}

	ret = ilitek_spi_read(sdev, rxbuf, len + DUMMY_LEN);
	if (ret < 0) {
...
static int ilitek_ice_mode_ctrl(struct thp_device *tdev, bool ice_enable,
	bool mcu_enable)
{
	int i;
	int ret = 0;
	...
	if (ice_enable) {
		...
		for (i = 0; i < ENABLE_ICE_MODE_RETRY; i++) {
			thp_ilitek_hw_reset(tdev);
			if (ilitek_spi_write(tdev->thp_core->sdev, cmd,
				sizeof(cmd)) < 0)
				THP_LOG_ERR("%s: write ice mode cmd error\n",
					__func__);

			if (ilitek_ice_mode_read(tdev->thp_core->sdev,
		...
		ret = ilitek_spi_write(tdev->thp_core->sdev, cmd, sizeof(cmd));
		if (ret < 0)
...
static int thp_ilitek_chip_detect(struct thp_device *tdev)
{
	int ret;

	ret = ilitek_ice_mode_ctrl(tdev, ENABLE, DISABLE);
	/* pull reset to exit ice mode */
	thp_ilitek_hw_reset(tdev);
	if (ret) {
		THP_LOG_ERR("%s: Ilitek chip is not detected\n", __func__);
		return -ENODEV;
	}
...
static int thp_ilitek_get_frame(struct thp_device *tdev,
	char *buf, unsigned int len)
{
	...
	ret = ilitek_spi_read(tdev->thp_core->sdev, buf, len);
	if (ret < 0) {
...
static int thp_ilitek_resume(struct thp_device *tdev)
{
	struct thp_core_data *cd = NULL;
...
static int thp_ilitek_suspend(struct thp_device *tdev)
{
...
static void thp_ilitek_exit(struct thp_device *tdev)
{
	THP_LOG_INFO("%s: called\n", __func__);
	kfree(tdev->tx_buff);
	tdev->tx_buff = NULL;
...
struct thp_device_ops ilitek_dev_ops = {
	.init = thp_ilitek_init,
	.detect = thp_ilitek_chip_detect,
	.get_frame = thp_ilitek_get_frame,
	.resume = thp_ilitek_resume,
	.suspend = thp_ilitek_suspend,
	.exit = thp_ilitek_exit,
};

static int __init thp_ilitek_module_init(void)
{
	int ret;
...
static void __exit thp_ilitek_module_exit(void)
{
	THP_LOG_INFO("%s: called\n", __func__);
};

module_init(thp_ilitek_module_init);
module_exit(thp_ilitek_module_exit);
