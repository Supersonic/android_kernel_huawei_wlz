...
static int thp_novatek_wrong_touch(struct thp_device *tdev)
{
...
static int thp_novatek_gesture_report(struct thp_device *tdev,
	unsigned int *gesture_wakeup_value)
{
	...
	for (i = 0; i < RETRY_NUM; i++) {
		retval = thp_nvt_spi_read_write(tdev->thp_core->sdev,
...
static int thp_novatek_read_fw_stamp(struct thp_device *tdev,
	uint8_t *stamp_val)
{
	uint8_t *r_buf = g_nvt_ts_spi_buf.r_buf;
	uint8_t *w_buf = g_nvt_ts_spi_buf.w_buf;
	...
	rw_len = SEND_CMD_LEN;
	ret = thp_nvt_spi_read_write(tdev->thp_core->sdev,
		w_buf, NULL, rw_len);
	if (ret) {
...
static int thp_novatek_write_cmd_addr(struct thp_device *tdev)
{
	uint8_t *w_buf = g_nvt_ts_spi_buf.w_buf;
	int rw_len;
	int ret;
...
static int thp_novatek_send_fw_cmd(struct thp_device *tdev, uint8_t cmd)
{
	uint8_t *w_buf = g_nvt_ts_spi_buf.w_buf;
	int rw_len;
	uint8_t cur_idx;
	...
	ret = thp_novatek_read_fw_stamp(tdev, &stamp_val);
	if (ret) {
		THP_LOG_ERR("%s, fail read fw stamp\n");
		return ret;
	}
	THP_LOG_INFO("read stamp value is %x, cmd is %x\n", stamp_val, cmd);
	ret = thp_novatek_write_cmd_addr(tdev);
	if (ret)
		return ret;
	/*
	 * 0:r/w
	...
	ret = thp_nvt_spi_read_write(tdev->thp_core->sdev, w_buf, NULL, rw_len);
	if (ret < 0) {
...
static void thp_novatech_enter_gesture_mode(
	struct thp_device *tdev)
{
	int retval;

	retval = thp_novatek_send_fw_cmd(tdev,
...
static int thp_novatech_suspend(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_gesture_mode &&
		...
		if (pt_test_mode) { /* PT test spec */
			...
			(void)thp_novatek_send_fw_cmd(tdev,
			...
			thp_novatech_enter_gesture_mode(tdev);
		} else { /* power off spec */
...
struct thp_device_ops nova_dev_ops = {
	...
	.chip_wrong_touch = thp_novatek_wrong_touch,
	.chip_gesture_report = thp_novatek_gesture_report,
};
