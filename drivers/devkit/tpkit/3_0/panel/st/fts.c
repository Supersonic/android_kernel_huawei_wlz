...
static int st_parse_dts(struct device_node *device, struct ts_kit_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, "support_self_calibrate_test",
		&chip_data->support_self_calibrate_test);
	if (retval) {
		chip_data->support_self_calibrate_test = 0;
		TS_LOG_ERR("support_self_calibrate_test %u\n",
			chip_data->support_self_calibrate_test);
	}
	retval = of_property_read_u32(device, "support_ito_cap_test",
		&chip_data->support_ito_cap_test);
	if (retval) {
		chip_data->support_ito_cap_test = 0;
		TS_LOG_ERR("support_ito_cap_test %u\n",
			chip_data->support_ito_cap_test);
	}
