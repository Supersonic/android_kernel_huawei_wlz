...
enum ts_cmd {
	...
	TS_ALGO_FINGER_PEN, /* copy kernel finger_pen data to daemon */
	TS_REPORT_FINGER_PEN_DIRECTLY, /* report finger pen directly without algo */
	TS_INVAILD_CMD = 255,
};
...
struct tp_callback_event {
	...
	int event_code; /* Bit0=1 indicates Tx1 freq need to hop, Bit1=1 indicates Tx2 freq need to hop */
	/*
	 * "freq1, freq2, paras", freq1 and freq2 is the index of HPP3.0 freq table. 1~241
	 * param is 1 byte, bit0=0 means Tx1 hopping imediately, bit0=1 means Tx1 delay hopping
	 * bit1=0 means Tx2 hopping imediately, bit1=1 hopping Tx2 delay hopping
	 */
...
struct ts_kit_platform_data {
	...
	atomic_t stylus3_ack_freq_status;
