	...
	struct ts_finger_pen_info *finger_pen)
{
	...
	if (atomic_read(&pdata->finger_pen_waitq_flag) == AFT_WAITQ_WAIT) {
		...
		if (finger_pen_flag == TS_ONLY_FINGER ||
			...
			dump_fingers_info_debug((struct ts_finger *)&(finger_pen->report_finger_info));
		/* clear slow status if daemon not busy */
		atomic_set(&pdata->aft_in_slow_status, 0);
		...
		return 0;
	} else if (atomic_read(&pdata->finger_pen_waitq_flag) != AFT_WAITQ_IDLE) {
		up(&pdata->finger_pen_aft_send);
		TS_LOG_DEBUG("[MUTI_AFT] ts_algo_calibrate hal aglo process too slow\n");
		if (finger_pen_flag == TS_ONLY_FINGER ||
			(finger_pen_flag == TS_FINGER_PEN)) {
...
void copy_stylus3_shift_freq_to_aft_daemon(struct shift_freq_info *shift_freq_info)
{
	struct ts_kit_platform_data *pdata = &g_ts_kit_platform_data;
...
void ts_update_stylu3_shif_freq_flag(unsigned int status, bool set_flag)
{
	...
	last_status = atomic_read(&g_ts_kit_platform_data.current_stylus3_status);
	if (set_flag)
		last_status |= status;
	else
		last_status &= ~status;
	TS_LOG_INFO("%s, last_status=%x\n", __func__, last_status);
	atomic_set(&g_ts_kit_platform_data.current_stylus3_status, last_status);
	up(&g_ts_kit_platform_data.stylus3_status_flag);
}
...
static int ts_ioctl_set_stylus3_ack_freq(unsigned long arg)
{
	...
	atomic_set(&g_ts_kit_platform_data.stylus3_ack_freq_status,
		STYLUS3_RECEIVE_ACK);
	return 0;

}

int ts_check_stylus3_ack_freq(void)
{
	int ack_freq_status;
	struct ts_kit_platform_data *pdata = &g_ts_kit_platform_data;
	...
	ack_freq_status =
		atomic_read(&g_ts_kit_platform_data.stylus3_ack_freq_status);
	if (ack_freq_status == STYLUS3_RECEIVE_ACK) {
		atomic_set(&g_ts_kit_platform_data.stylus3_ack_freq_status,
			STYLUS3_CLEAR_ACK);
		return 0;
	} else {
...
int ts_kit_misc_init(struct ts_kit_platform_data *pdata)
{
	...
	if ((g_ts_kit_platform_data.pen_aft_enable_flag == SUPPORT_PEN_AFT) &&
		...
		atomic_set(&g_ts_kit_platform_data.current_stylus3_status, 0);
		...
		sema_init(&pdata->stylus3_ack_freq_flag,
			STATUS_FLAG_INIT_VALUE);
		atomic_set(&g_ts_kit_platform_data.stylus3_ack_freq_status, 0);
	}
	return error;
}
