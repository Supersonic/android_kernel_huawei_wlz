			...
			if (in_info->fingers[index].status != 0)
			{
				...
				else
				{
					...
					if (stop_report_flag & (1 << index))
					{
						...
						if (temp_val)
						{
							TS_LOG_DEBUG("stopped finger index: %u, coordinate is legal1, can report again\n", index);
						...
						else
						{
							...
							TS_LOG_DEBUG("finger index: %u, keep stopped, stop_cnt: %d\n", index, finger_stop_cnt[index]);
					...
					else
					{
						...
						{
							TS_LOG_DEBUG("finger index: %u, start OK, directly report\n", index);
