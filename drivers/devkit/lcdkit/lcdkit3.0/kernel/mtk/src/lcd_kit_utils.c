...
int lcd_kit_read_project_id(void)
{
	...
	if (disp_info->project_id.support) {
		if (disp_info->otp_flag) {
			ret = lcd_kit_dsi_cmds_extern_rx_v1(project_id_flag,
				&disp_info->otp_reg_cmds);
			if (ret) {
				LCD_KIT_ERR("read project id flag fail\n");
				return LCD_KIT_FAIL;
			}

			if (disp_info->otp_value.buf[0] == project_id_flag[0]) {
				LCD_KIT_INFO("otp error, use default\n");
				return LCD_KIT_OK;
			}
		}
...
void lcd_kit_pinfo_init(struct device_node *np, struct mtk_panel_info *pinfo)
{
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,mipi-rg-vcm-adjust",
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,otp-flag",
		&disp_info->otp_flag, 0);
	if (disp_info->otp_flag) {
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,otp-reg-command",
			"lcd-kit,otp-reg-cmds-state", &disp_info->otp_reg_cmds);
		lcd_kit_parse_array_data(np, "lcd-kit,otp-value",
			&disp_info->otp_value);
	}
