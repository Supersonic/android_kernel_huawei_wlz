...
static void hisi_vibrator_ldo_ctrl(
...
static ssize_t hisi_vibrator_min_timeout_show(
...
static DEVICE_ATTR(
	vibrator_min_timeout, 0664, hisi_vibrator_min_timeout_show, NULL);
...
static int hisi_vibrator_register_led_classdev(struct hisi_vibrator_data *vdata)
{
...
{
	struct hisi_vibrator_data *vdata = NULL;
	int ret;
	...
	vdata = devm_kzalloc(
		&pdev->dev, sizeof(struct hisi_vibrator_data), GFP_KERNEL);
	if (vdata == NULL)
		return -ENOMEM;
