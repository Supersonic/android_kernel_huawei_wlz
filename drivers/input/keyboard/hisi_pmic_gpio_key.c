...
static const char
	*hisi_pmic_gpio_key_irq_table[HISI_PMIC_GPIO_KEY_IRQ_COUNTS] = {
		"vol-up", "vol-down",
};
...
int is_pmic_gpio_key_vol_updown_pressed(void)
{
	...
	if ((vol_up_active_low < 0) || (vol_down_active_low < 0)) {
		pr_err("[%s]:vol_updown gpio or act_low is invalid!", __func__);
		return 0;
	}

	mdelay(5);
	state1 = (hisi_pmic_gpio_key_read(PMIC_GPIO_KEYUP_DATA) ? 1 : 0) ^
		 (unsigned int)vol_up_active_low;
	state2 = (hisi_pmic_gpio_key_read(PMIC_GPIO_KEYDOWN_DATA) ? 1 : 0) ^
...
static int hisi_pmic_gpio_key_irq_init(
	struct platform_device *pdev, struct hisi_pmic_gpio_key *gpio_key)
{
	int i;
	int ret = 0;
...
{
	/*20ms filter*/
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ONE, PMIC_GPIO_KEYUP_DEBSEL);
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ONE, PMIC_GPIO_KEYDOWN_DEBSEL);
	/* not PD */
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ZERO, PMIC_GPIO_KEYUP_PDSEL);
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ZERO, PMIC_GPIO_KEYDOWN_PDSEL);
	/* PU */
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ONE, PMIC_GPIO_KEYUP_PUSEL);
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ONE, PMIC_GPIO_KEYDOWN_PUSEL);
	/* input */
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ZERO, PMIC_GPIO_KEYUP_DIR);
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ZERO, PMIC_GPIO_KEYDOWN_DIR);
	/* edge trigger */
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ZERO, PMIC_GPIO_KEYUP_IS);
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ZERO, PMIC_GPIO_KEYDOWN_IS);
	/* double edge trigger */
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ONE, PMIC_GPIO_KEYUP_IBE);
	hisi_pmic_gpio_key_write(
		PMIC_GPIO_KEY_REG_VALUE_ONE, PMIC_GPIO_KEYDOWN_IBE);
}

#ifdef CONFIG_VOLUME_KEY_MASK
static ssize_t hisi_pmic_key_mask_read(
