...
static struct timer_list
	dsm_gpio_key_timer; // used to reset the statistic variable
...
static void hisi_gpio_keyback_work(struct work_struct *work)
{
	...
	pr_info("[gpiokey]back key %u action %u\n", KEY_BACK,
		report_action);
...
static void hisi_gpio_keysmart_work(struct work_struct *work)
{
	...
	pr_info("[gpiokey]smart key %u action %u\n", KEY_F24,
		report_action);
...
int is_gpio_key_vol_updown_pressed(void)
{
	...
	mdelay(5);
	state1 = (gpio_get_value_cansleep(vol_up_gpio) ? 1 : 0) ^
		 ((unsigned int)vol_up_active_low);
	state2 = (gpio_get_value_cansleep(vol_down_gpio) ? 1 : 0) ^
...
static irqreturn_t hisi_gpio_key_irq_handler(int irq, void *dev_id)
{
	...
	if (irq == gpio_key->volume_up_irq) {
		...
		mod_timer(&(gpio_key->key_back_timer),
			jiffies + msecs_to_jiffies(TIMER_DEBOUNCE));
		__pm_wakeup_event(&back_key_lock, jiffies_to_msecs(50));
#endif
#ifdef CONFIG_HISI_GPIO_KEY_SUPPORT_SMART_KEY
	} else if (support_smart_key && irq == gpio_key->key_smart_irq) {
		mod_timer(&(gpio_key->key_smart_timer),
			jiffies + msecs_to_jiffies(TIMER_DEBOUNCE));
		__pm_wakeup_event(&smart_key_lock, jiffies_to_msecs(50));
#endif
	} else {
	...
	size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%d", &mask) != 1) /* [false alarm]:fortify */
		return 0;
...
static int hisi_gpio_key_probe(struct platform_device *pdev)
{
	...
	if (pdev == NULL) {
		pr_err("[gpiokey]parameter error!\n");
		return -EINVAL;
	}
	...
	INIT_DELAYED_WORK(
		&(gpio_key->gpio_keydown_work), hisi_gpio_keydown_work);
	...
	if (gpio_is_valid(gpio_key->gpio_up)) {
		err = devm_gpio_request(
			&pdev->dev, (unsigned int)gpio_key->gpio_up, "gpio_up");
		if (err < 0) {
		...
		err = gpio_direction_input((unsigned int)gpio_key->gpio_up);
		if (err < 0) {
			dev_err(&pdev->dev, "Set gpio_up direction failed!\n");
			goto err_get_gpio;
		}

		gpio_key->volume_up_irq =
			gpio_to_irq((unsigned int)gpio_key->gpio_up);
		if (gpio_key->volume_up_irq < 0) {
			dev_err(&pdev->dev, "Get gpio key press irq failed!\n");
	...
	if (gpio_is_valid(gpio_key->gpio_down)) {
		...
		err = gpio_direction_input((unsigned int)gpio_key->gpio_down);
		if (err < 0) {
			dev_err(&pdev->dev, "Set gpio_down direction failed!\n");
			goto err_get_gpio;
		}

		gpio_key->volume_down_irq =
			gpio_to_irq((unsigned int)gpio_key->gpio_down);
		if (gpio_key->volume_down_irq < 0) {
			dev_err(&pdev->dev, "Get gpio key release irq failed!\n");
...
#ifdef CONFIG_HISI_GPIO_KEY_SUPPORT_HI6XXX
	if (gpio_is_valid(gpio_key->gpio_back)) {
		err = gpio_request((unsigned int)gpio_key->gpio_back,
			"gpio_back");
		if (err) {
			dev_err(&pdev->dev, "Fail request gpio:%d\n",
				gpio_key->gpio_back);
		...
		gpio_key->key_back_irq =
			gpio_to_irq((unsigned int)gpio_key->gpio_back);
		if (gpio_key->key_back_irq < 0) {
			dev_err(&pdev->dev,
				"Failed to get gpio key release irq!\n");
	...
	if (support_smart_key && gpio_is_valid(gpio_key->gpio_smart)) {
		...
		gpio_key->key_smart_irq =
			gpio_to_irq((unsigned int)gpio_key->gpio_smart);
	 ...
	 */
	if (gpio_is_valid(gpio_key->gpio_up)) {
		err = request_irq(gpio_key->volume_up_irq,
		...
		if (err) {
			dev_err(&pdev->dev,
				"Failed to request press interupt handler!\n");
			goto err_up_irq_req;
		}
	}
	 ...
	 */
	if (gpio_is_valid(gpio_key->gpio_down)) {
		err = request_irq(gpio_key->volume_down_irq,
			hisi_gpio_key_irq_handler,
			IRQF_NO_SUSPEND |
			IRQF_TRIGGER_RISING |
			IRQF_TRIGGER_FALLING,
		...
		if (err) {
			dev_err(&pdev->dev,
			"Failed to request release interupt handler!\n");
...
#ifdef CONFIG_HISI_GPIO_KEY_SUPPORT_HI6XXX
	if (gpio_is_valid(gpio_key->gpio_back)) {
		err = request_irq(gpio_key->key_back_irq,
			hisi_gpio_key_irq_handler,
			IRQF_NO_SUSPEND |
			IRQF_TRIGGER_RISING |
			IRQF_TRIGGER_FALLING,
...
#ifdef CONFIG_HISI_GPIO_KEY_SUPPORT_SMART_KEY
	if (support_smart_key && gpio_is_valid(gpio_key->gpio_smart)) {
		err = request_irq(gpio_key->key_smart_irq,
			hisi_gpio_key_irq_handler,
			IRQF_NO_SUSPEND |
			IRQF_TRIGGER_RISING |
			IRQF_TRIGGER_FALLING,
...
#endif
	pr_info("[gpiokey]K3v3 gpio key probe failed! ret = %d.\n",
		err);
