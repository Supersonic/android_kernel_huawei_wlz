...
#ifdef VERBOSE_DEBUG
#ifndef pr_vdebug
#define pr_vdebug(fmt, arg...)  pr_debug(fmt, ##arg)
#endif /* pr_vdebug */
#else
#ifndef pr_vdebug
	...
	while (!port->write_busy && !list_empty(pool)) {
		...
		if (port->port_num == 1) {
			pr_err("[gs_start_tx_printk]%d:len=%d,%c%c%c%c%c%c\n",
				port->port_num, req->length,
				*((u8 *)req->buf), *((u8 *)req->buf + 1),
				*((u8 *)req->buf + 2), *((u8 *)req->buf + 3),
				*((u8 *)req->buf + 4), *((u8 *)req->buf + 5));
		}
...
static void gs_rx_push(unsigned long _port)
{
	...
	while (!list_empty(queue)) {
		...
		if (port->port_num == 1) {
			pr_err("[%s]%d:len=%d,%c%c%c%c%c%c%c%c\n",
				__func__, port->port_num, req->length,
				*((u8 *)req->buf), *((u8 *)req->buf + 1),
				*((u8 *)req->buf + 2), *((u8 *)req->buf + 3),
				*((u8 *)req->buf + 4), *((u8 *)req->buf + 5),
				*((u8 *)req->buf + 6), *((u8 *)req->buf + 7));
		}
...
static int gs_open(struct tty_struct *tty, struct file *file)
{
	...
	if (port->port_write_buf.buf_buf == NULL) {
		...
		if (status) {
			pr_debug("%s: ttyGS%d %p,%p no buffer\n",
...
static int gs_put_char(struct tty_struct *tty, unsigned char ch)
{
	...
	pr_vdebug("%s: %d,%p char=0x%x, called from %pf\n", __func__,
...
static int userial_init(void)
{
	...
	gs_tty_driver = alloc_tty_driver(HW_MAX_U_SERIAL_PORTS);
	if (!gs_tty_driver)
		return -ENOMEM;
