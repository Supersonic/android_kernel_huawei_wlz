...
requeue_req:
	...
	if (dev->state == STATE_BUSY) {
		...
		if (req->actual == 0)	/*lint !e613 */
			goto requeue_req;

		DBG(cdev, "rx %p %d\n", req, req->actual);
		xfer = (req->actual < count) ? req->actual : count;	/*lint !e613 */
		r = xfer;
		if (copy_to_user(buf, req->buf, xfer))	/*lint !e613 */
			r = -EFAULT;
	} else
		r = -EIO;
...
static long mtp_ioctl(struct file *fp, unsigned code, unsigned long value)
{
	...
	case MTP_SEND_FILE_WITH_HEADER:
	{
		...
		if (copy_from_user(&mfr, r_value, sizeof(mfr))) {
			ret = -EFAULT;
			goto fail;
		}
...
mtp_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (gadget_is_superspeed(c->cdev->gadget)
		...
		max_burst = min_t(unsigned, dev->bulk_buffer_size / 1024, 15);
