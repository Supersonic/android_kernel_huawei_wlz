...
#define PR_BOOL(_name)	PR("%s %s\n", #_name, tcpc_dev->_name ? "true" : "false");
	...
	PR("attach_wake_lock active %d\n",
			tcpc_dev->attach_wake_lock.active);
	PR("dettach_temp_wake_lock active %d\n",
			tcpc_dev->dettach_temp_wake_lock.active);
...
static bool typec_try_enter_norp_src(struct tcpc_device *tcpc_dev)
{
	...
	static bool under_wireless_charging = false;

	if (tcpci_tcpc_vusb_uv_det_masked(tcpc_dev)) {
		D("vusb_uv_det disabled, ignore vbus_only\n");
		under_wireless_charging = true;
		return false;
	}

	if (under_wireless_charging) {
		D("vusb_uv_det disabled, ignore vbus_only\n");
		under_wireless_charging = false;
		return false;
	}
#endif /* CONFIG_HISI_TCPC_QUIRK_V100 */
...
static inline int typec_norp_src_attached_entry(struct tcpc_device *tcpc_dev)
{
	if (tcpc_dev->no_rpsrc_state == 0) {
		...
		/*
		 * For 56K Cable Plugin without charger or PC
		 * During Wireless charging
		 */
		if (tcpci_under_wireless_charging(tcpc_dev))
...
{
	I("typec_tole %d, typec_during_role_swap %d\n", tcpc_dev->typec_role,
...
{
	I("typec_role %d, typec_during_role_swap %d\n", tcpc_dev->typec_role,
				tcpc_dev->typec_during_role_swap);
