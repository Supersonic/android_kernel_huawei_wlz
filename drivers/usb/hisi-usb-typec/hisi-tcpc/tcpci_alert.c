...
static void tcpci_alert_frswap(struct tcpc_device *tcpc_dev)
{
	D("FR_swap\n");
...
static inline int __tcpci_set_wake_lock_pd(struct tcpc_device *tcpc, bool pd_lock, bool user_lock)
{
	bool ori_lock = false, new_lock = false;
