...
static inline void print_state(pd_port_t *pd_port, bool vdm_evt, uint8_t state)
{
	/*
...
#else
	PE_STATE_INFO("%s-> %s\n", vdm_evt ? "VDM" : "PD", pd_pe_state_name(state));
	D("pe state: %s-> %s; " "power_role: %s; " "data_role: %s; " "pd_port->vconn_source %d\n",
			vdm_evt ? "VDM" : "PD", pd_pe_state_name(state),
...
static void pd_pe_state_change(pd_port_t *pd_port, pd_event_t *pd_event, bool vdm_evt)
{
	...
	uint8_t new_state = pd_port->pe_state_next;	/*assigned in hisi_pd_process_event */
	...
	I("old_state %d(%s), new_state %d(%s)\n", old_state, pd_pe_state_name(old_state),
			new_state, pd_pe_state_name(new_state));
	...
	D("new_state: pe_vdm_state %d, pe_pe_state %d\n", pd_port->pe_vdm_state, pd_port->pe_pd_state);
	D("-\n");
}
...
static int pd_handle_event(pd_port_t *pd_port, pd_event_t *pd_event, bool vdm_evt)
{
	...
	if (vdm_evt) {
		if (pd_port->reset_vdm_state && (!pd_port->dpm_ack_immediately)) {
			...
			pd_port->pe_vdm_state = pd_port->pe_pd_state;
			D("set pe_vdm_state %d\n", pd_port->pe_vdm_state);
		}

		pd_port->pe_state_curr = pd_port->pe_vdm_state;
		D("pe_state_curr = pe_vdm_state, %d\n", pd_port->pe_vdm_state);
	} else {
		pd_port->pe_state_curr = pd_port->pe_pd_state;
		D("pe_state_curr = pe_pd_state, %d\n", pd_port->pe_pd_state);
	}

	ret = hisi_pd_process_event(pd_port, pd_event, vdm_evt);/* get next state by event */
	if (ret)
		pd_pe_state_change(pd_port, pd_event, vdm_evt); /* state change */
	else
		pd_free_pd_event(pd_port, pd_event);		/* state not change */

	D("-\n");
	return 1;
}

static inline int pd_put_dpm_ack_immediately(pd_port_t *pd_port, bool vdm_evt)
{
	pd_event_t pd_event = {
...
int hisi_pd_policy_engine_run(struct tcpc_device *tcpc_dev)
{
	...
	mutex_lock(&pd_port->pd_lock);	/* global lock of pd engine */
