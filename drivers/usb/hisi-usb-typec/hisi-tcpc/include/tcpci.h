...
extern int hisi_usb_typec_otg_pwr_src(void);
...
static inline bool tcpci_under_wireless_charging(struct tcpc_device *tcpc)
{
	if (tcpc->ops->under_wireless_charging)
		return tcpc->ops->under_wireless_charging();
...
static inline int tcpci_sink_vbus(struct tcpc_device *tcpc, uint8_t type, int mv, int ma)
{
	...
	if (type >= TCP_VBUS_CTRL_PD && tcpc->pd_port.pd_prev_connected) {
		type |= TCP_VBUS_CTRL_PD_DETECT;
		tcp_noti.vbus_state.ext_power = tcpc->pd_port.dpm_flags  & DPM_FLAGS_PARTNER_EXTPOWER;
	}
#endif	/* CONFIG_USB_POWER_DELIVERY_SUPPORT */
