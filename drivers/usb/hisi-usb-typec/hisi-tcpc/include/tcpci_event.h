...
typedef struct __pd_msg {
	uint8_t frame_type;	/* come from tcpci register, SOP, SOP' ... */
	uint16_t msg_hdr;
...
bool hisi_pd_put_event(struct tcpc_device *tcpc_dev, const pd_event_t *pd_event, bool from_port_partner);
...
bool hisi_pd_put_vdm_event(struct tcpc_device *tcpc_dev, pd_event_t *pd_event, bool from_port_partner);

bool hisi_pd_put_last_vdm_event(struct tcpc_device *tcpc_dev);

extern int hisi_tcpci_event_init(struct tcpc_device *tcpc_dev);
extern int hisi_tcpci_event_deinit(struct tcpc_device *tcpc_dev);
