...
#define PD_PORT_PDOS_MAX_SIZE	7
...
typedef struct __pd_port {
	...
	/* PD */
	bool explicit_contract;	/* Assigned after power negotiation */
	...
	bool pe_ready;
	bool pd_connected;
	bool pd_prev_connected;	/* Assigned after capability-message received, both source and sink */
	...
	uint8_t msg_id_rx[PD_SOP_NR];		/* used for filter duplicate messages */
	uint8_t msg_id_rx_init[PD_SOP_NR];
	uint8_t msg_id_tx[PD_SOP_NR];		/* used for message-id to send */
...
extern int hisi_pd_core_init(struct tcpc_device *tcpc_dev);
...
extern void hisi_pd_try_put_pe_idle_event(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_transit_to_default(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_hard_reset_completed(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_send_hard_reset(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_running(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_idle(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_wait_vbus_once(pd_port_t *pd_port, int wait_evt);
extern void hisi_pd_notify_pe_error_recovery(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_execute_pr_swap(pd_port_t *pd_port, bool start_swap);
extern void hisi_pd_notify_pe_cancel_pr_swap(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_reset_protocol(pd_port_t *pd_port);
extern void hisi_pd_noitfy_pe_bist_mode(pd_port_t *pd_port, uint8_t mode);
extern void hisi_pd_notify_pe_pr_changed(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_src_explicit_contract(pd_port_t *pd_port);
extern void hisi_pd_notify_pe_transmit_msg(pd_port_t *pd_port, uint8_t type);
extern void hisi_pd_notify_pe_recv_ping_event(pd_port_t *pd_port);
...
static inline bool vdm_put_dpm_event(pd_port_t *pd_port, uint8_t dpm_event, pd_msg_t *pd_msg)
{
	pd_event_t evt = {
		.event_type = PD_EVT_DPM_MSG,
		.msg = dpm_event,
...
static inline bool pd_put_hw_event(struct tcpc_device *tcpc_dev,
			uint8_t hw_event)
{
	pd_event_t evt = {
		.event_type = PD_EVT_HW_MSG,
...
static inline bool pd_put_cc_attached_event(struct tcpc_device *tcpc_dev, uint8_t type)
{
	pd_event_t evt = {
		.event_type = PD_EVT_HW_MSG,
...
int hisi_pd_send_ctrl_msg(
	pd_port_t *pd_port, uint8_t sop_type, uint8_t msg);

int hisi_pd_send_data_msg(pd_port_t *pd_port,
...
int hisi_pd_reply_svdm_request(pd_port_t *pd_port,
...
static inline int hisi_pd_reply_svdm_request_simply(
