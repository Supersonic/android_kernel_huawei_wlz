				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case TCPC_DESC_ROLE_DEF:
		size = snprintf_s(buf, MAX_PROP_LEN, MAX_PROP_LEN - 1, "%s\n"
	...
	case TCPC_DESC_REMOTE_RP_LEVEL:
		...
		size = snprintf_s(buf, MAX_PROP_LEN, MAX_PROP_LEN - 1, " cc1 %s cc2 %s\n",
	...
	case TCPC_DESC_INFO:
		...
		size = snprintf_s(buf + size, MAX_PROP_LEN - size, MAX_PROP_LEN - size - 1,
			"rplvl = %s\n", tcpm_cc_voltage_status_string(cc2));
		break;
