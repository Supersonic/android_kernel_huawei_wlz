...
const char *hisi_tcpc_timer_name[] = {
...
static inline struct tcpc_device *tcpc_hrtimer_to_dev(struct hrtimer *timer, int index)
{
	return container_of(timer, struct tcpc_device, tcpc_timer[index]); /* [false alarm]:timer can't be NULL */
}
...
void hisi_tcpc_restart_timer(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	D("timer_id %d, name %s\n", timer_id, hisi_tcpc_timer_name[timer_id]);
	mask = tcpc_get_timer_enable_mask(tcpc);
...
void hisi_tcpc_enable_timer(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	hrtimer_start(&tcpc->tcpc_timer[timer_id], ktime_set(r, mod*1000), HRTIMER_MODE_REL); /*lint !e647 */
}
...
{
	/* FIXME: Why lock ? Why others have no lock ? */
...
int hisi_tcpci_timer_init(struct tcpc_device *tcpc_dev)
{
	...
	tcpc_dev->timer_task = kthread_create(hisi_tcpc_timer_thread, tcpc_dev,
			"hisi_tcpc_timer_%s.%p", dev_name(&tcpc_dev->dev), tcpc_dev);
