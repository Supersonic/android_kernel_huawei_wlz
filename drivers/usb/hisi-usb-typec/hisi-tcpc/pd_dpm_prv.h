...
typedef struct __svdm_svid_ops {
	...
	bool (*dfp_inform_id)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, pd_event_t *pd_event, bool ack);
	bool (*dfp_inform_svids)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, bool ack);
	bool (*dfp_inform_modes)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, bool ack);

	bool (*dfp_inform_enter_mode)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, uint8_t ops, bool ack);
	bool (*dfp_inform_exit_mode)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, uint8_t ops);

	bool (*dfp_inform_attention)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, pd_event_t *pd_event);

	void (*ufp_request_enter_mode)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, uint8_t ops);
	void (*ufp_request_exit_mode)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, uint8_t ops);

	bool (*notify_pe_startup)(pd_port_t *pd_port, svdm_svid_data_t *svid_data);
	int (*notify_pe_ready)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, pd_event_t *pd_event);
	bool (*notify_pe_shutdown)(pd_port_t *pd_port, svdm_svid_data_t *svid_data);

#ifdef CONFIG_USB_PD_UVDM_SUPPORT
	bool (*dfp_notify_uvdm)(pd_port_t *pd_port, svdm_svid_data_t *svid_data, bool ack);
	bool (*ufp_notify_uvdm)(pd_port_t *pd_port, svdm_svid_data_t *svid_data);
#endif
	bool (*reset_state)(pd_port_t *pd_port, svdm_svid_data_t *svid_data);
} svdm_svid_ops_t;

static inline svdm_svid_data_t *dpm_get_svdm_svid_data(pd_port_t *pd_port, uint16_t svid)
{
	uint8_t i;
...
static inline bool dpm_register_svdm_ops(pd_port_t *pd_port, const svdm_svid_ops_t *ops)
{
	svdm_svid_data_t *svid_data = NULL;
...
static inline bool svdm_notify_pe_startup(pd_port_t *pd_port)
{
	...
	for (i = 0; i < pd_port->svid_data_cnt; i++) {
		...
		if (svid_data->ops && svid_data->ops->notify_pe_startup)
			svid_data->ops->notify_pe_startup(pd_port, svid_data); /* hisi_dp_dfp_u_notify_pe_startup */
	}
...
static inline bool svdm_dfp_inform_modes(pd_port_t *pd_port, uint16_t svid, bool ack)
{
