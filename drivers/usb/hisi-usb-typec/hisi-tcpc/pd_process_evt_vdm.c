...
#define VDM_CMD_STATE(cmd, cmd_type)	((cmd & 0x1f) | ((cmd_type & 0x03) << 6))
...
#endif
#ifdef CONFIG_USB_PD_ALT_MODE_DFP_SUPPORT
	{ PE_DFP_VDM_DP_STATUS_UPDATE_REQUEST, PE_DFP_VDM_DP_STATUS_UPDATE_NAKED },
	{ PE_DFP_VDM_DP_CONFIGURATION_REQUEST, PE_DFP_VDM_DP_CONFIGURATION_NAKED },
#endif
#ifdef CONFIG_USB_PD_UVDM_SUPPORT
	{ PE_DFP_UVDM_SEND, PE_DFP_UVDM_NAKED },
#endif	/* CONFIG_USB_PD_UVDM_SUPPORT */
...
static inline bool pd_process_ufp_vdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+-\n");
	if (!pd_check_pe_state_ready(pd_port)) {
		PE_DBG("659 : invalid, current status\n");
...
static inline bool pd_process_dfp_vdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint32_t vdm_hdr = pd_event->pd_msg->payload[0];
...
static inline bool pd_process_sop_vdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ret = false;
...
static inline bool pd_process_sop_prime_vdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	switch (pd_port->pe_state_curr) {
...
static inline bool pd_process_data_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (!PD_VDO_SVDM(vdm_hdr))
		return pd_process_uvdm(pd_port, pd_event); /* deal with unstructed vdm */
...
static inline bool pd_process_dpm_msg_ack(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("\n");
			...
			pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	D("+\n");
	if (!pd_check_pe_state_ready(pd_port)) {
		hisi_pd_update_dpm_request_state(pd_port, DPM_REQ_ERR_NOT_READY);
	...
	if ((is_dfp && is_attention) || (!is_dfp && !is_attention)) {
		hisi_pd_update_dpm_request_state(pd_port, DPM_REQ_ERR_WRONG_ROLE);
			...
			pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (pd_event->msg == PD_HW_TX_DISCARD &&
		(pd_port->vdm_discard_retry_count < 10)) {
...
bool hisi_hisi_pd_process_event_vdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
