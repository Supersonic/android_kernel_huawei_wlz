 ...
 */
static void hisi_usb_typec_disable_vbus_control(struct tcp_ny_vbus_state *vbus_state)
{
	struct usb_typec *typec = _typec;
...
void hisi_usb_typec_dfp_inform_id(uint32_t *payload, uint32_t size)
{
	...
	if (payload && size >= 4) {
		...
		pid = (*(payload + 3) >> PD_DPM_PDT_VID_OFFSET) & PD_DPM_HW_PDO_MASK;
	}
...
static int queue_notify(struct usb_typec *typec, unsigned long action, const void *data)
{
...
static int pd_dpm_wake_lock_call(struct notifier_block *dpm_nb, unsigned long event, void *data)
{
	...
	I("%s\n", event == PD_WAKE_LOCK ? "lock" : "unlock");
...
static bool hisi_usb_check_cc_vbus_short(void)
{
	...
	return (cc1 == ANALOG_CC_WITH_UNKNOW_RESISTANCE
		|| cc2 == ANALOG_CC_WITH_UNKNOW_RESISTANCE);
}
...
static inline int __tcpc_notifier_work(struct tcpc_notify *noti)
{
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		vbus_state.remote_rp_level = noti->notify.vbus_state.remote_rp_level;
...
void hisi_usb_typec_max_power(int max_power)
{
	...
	pd_dpm_set_wireless_cover_power_status(max_power >= WIRELESS_HIGH_POWER_VALUE);
}

/**
 * hisi_usb_typec_reset_product() - Reset hw_pd product type when src startup.
 */
...
static inline int __tcpc_notifier_work(struct tcpc_notify *noti)
{
	E("[Not support typeC] >> tcpc_notify_type %lu\n", noti->tcpc_notify_type);
	return 0;
}
