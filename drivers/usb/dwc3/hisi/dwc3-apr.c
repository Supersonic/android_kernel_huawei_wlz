...
struct usb30phy_tcxo_power {
	...
	unsigned usb30phy_tcxo_power_flag;
};
...
#define TERM_DEFAULT_PARAM	0xa8

static int usb3_phy_33v_enable(struct plat_hisi_usb_phy *usb_phy);
static int usb3_phy_33v_disable(struct plat_hisi_usb_phy *usb_phy);
static int usb3_phy_tcxo_buffer_poweron(struct plat_hisi_usb_phy *usb_phy);
static int usb3_phy_tcxo_buffer_poweroff(struct plat_hisi_usb_phy *usb_phy);
...
int usb3_open_misc_ctrl_clk(void)
{
	...
	/* open aclk gate */
	ret = clk_prepare_enable(_hisi_usb_phy->gt_aclk_usb3otg);
	if (ret) {
	...
	if (__clk_is_enabled(_hisi_usb_phy->gt_aclk_usb3otg) == false) {
		clk_disable_unprepare(_hisi_usb_phy->gt_hclk_usb3otg);
...
void usb3_close_misc_ctrl_clk(void)
{
	...
	clk_disable_unprepare(_hisi_usb_phy->gt_aclk_usb3otg);
	clk_disable_unprepare(_hisi_usb_phy->gt_hclk_usb3otg);
}
...
static int config_usb_phy_controller(struct plat_hisi_usb_phy *usb_phy,
		unsigned int support_dp)
{
	struct hisi_usb_combophy *combophy = usb3_get_combophy_phandle();
...
int dwc3_set_combophy_clk(void)
{
	...
	return set_combophy_clk(_hisi_usb_phy);
}
...
static int dwc3_set_highspeed_only_step1(struct plat_hisi_usb_phy *usb_phy)
{
...
static int dwc3_release(struct plat_hisi_usb_phy *usb_phy, unsigned int support_dp)
{
	...
	if (!hisi_dwc3_is_fpga() && highspeed_only) {
...
static bool is_valid_hisi_dwc3_phy_term(u32 usb3_phy_term)
{
	int i;

	for (i = 0; i < sizeof(usb3_phy_terms) / sizeof(u32); ++i) {
		if (usb3_phy_term == usb3_phy_terms[i])
			return true;
	}
...
static int apr_set_phy_term(u32 usb3_phy_term)
{
	...
	if (is_valid_hisi_dwc3_phy_term(usb3_phy_term))
		_hisi_usb_phy->usb3_phy_term = usb3_phy_term;
...
static int apr_get_phy_term(u32 *usb3_phy_term)
{
	...
	*usb3_phy_term = _hisi_usb_phy->usb3_phy_term;
...
/*lint -e438 +esym(438,*)*/
/*lint -e529 +esym(529,*)*/
static int apr_usb31_core_enable_u3(struct plat_hisi_usb_phy *usb_phy)
{
	int ret = 0;
	int power_down_flag;
...
static int apr_usb31_core_disable_u3(struct plat_hisi_usb_phy *usb_phy)
{
	usb_dbg("+\n");
...
int hisi_usb_combophy_notify(enum phy_change_type type)
{
	...
	if (PHY_MODE_CHANGE_BEGIN == type) {
		ret = apr_usb31_core_enable_u3(_hisi_usb_phy);
	} else if (PHY_MODE_CHANGE_END == type) {
		apr_usb31_core_disable_u3(_hisi_usb_phy);
	} else {
		usb_err("Bad param!\n");
		ret = -EINVAL;
	}
...
static int apr_usb3phy_shutdown(unsigned int support_dp)
{
	...
	usb3_phy_33v_disable(_hisi_usb_phy);
		...
		unsigned int usb3_phy_tx_vboost_lvl)
{
	...
	sctrl_base = _hisi_usb_phy->sctrl_reg_base;
	...
	ret = usb3_phy_33v_enable(_hisi_usb_phy);
	if (ret) {
	...
	set_phy_term_for_usb3(_hisi_usb_phy->usb3_phy_term);
...
static int apr_get_clk_resource(struct device *dev, struct plat_hisi_usb_phy *usb_phy)
{
	/* get abb clk handler */
...
static int apr_get_dts_resource(struct device *dev, struct plat_hisi_usb_phy *usb_phy)
{
	void __iomem *regs;
	...
	if (of_property_read_u32(dev->of_node, "usb3_phy_term",
			...
			!is_valid_hisi_dwc3_phy_term(usb_phy->usb3_phy_term)) {
		usb_dbg("get usb3_phy_term form dt failed or invalid, \
				use default value\n");
		usb_phy->usb3_phy_term = TERM_DEFAULT_PARAM;
	}
...
static void __iomem *apr_get_bc_ctrl_reg(void)
{
	...
	return _hisi_usb_phy->hsdt_sctrl_reg_base + SC_USB20PHY_MISC_CTRL4;
}
		...
		unsigned int combophy_flag, unsigned int keep_power)
{
	...
	if (!keep_power) {
		usb_dbg(":DP_AUX_LDO_CTRL_HIFIUSB disable\n");
		ret = usb3_phy_33v_disable(_hisi_usb_phy);
		if (ret && ret != -ENODEV)
...
static void apr_set_hi_impedance(void)
{
	void __iomem *bc_ctrl2 = apr_get_bc_ctrl_reg();
...
static int dwc3_apr_probe(struct platform_device *pdev)
{
	...
	ret = apr_get_dts_resource(&pdev->dev, _hisi_usb_phy);
	if (ret) {
		usb_err("get_dts_resource failed\n");
		goto err_out;
	}

	_hisi_usb_phy->phy.init = apr_usb3phy_init;
	_hisi_usb_phy->phy.shutdown = apr_usb3phy_shutdown;
	_hisi_usb_phy->phy.shared_phy_init = apr_shared_phy_init;
	_hisi_usb_phy->phy.shared_phy_shutdown = apr_shared_phy_shutdown;
	_hisi_usb_phy->phy.set_hi_impedance = apr_set_hi_impedance;
	_hisi_usb_phy->phy.notify_speed = apr_notify_speed;
	_hisi_usb_phy->phy.get_bc_ctrl_reg = apr_get_bc_ctrl_reg;
	_hisi_usb_phy->phy.get_term = apr_get_phy_term;
	_hisi_usb_phy->phy.set_term = apr_set_phy_term;
