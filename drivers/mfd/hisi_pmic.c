...
static void __iomem *g_pmussi_baseaddr;
#define PMUSSI_REG(addr)        (((addr)<<2) + (char *)g_pmussi_baseaddr)

unsigned int hisi_pmic_reg_read(int addr)
{
	unsigned char regval;
	if (NULL == g_pmussi_baseaddr) {
		pr_err("hisi pmic reg read fail, pmussi baseaddr is null!\n");
		return 0;
	}
	regval =  *(volatile unsigned char*)PMUSSI_REG(addr);/*lint !e679*/
	return regval;
}
...
{
	if (NULL == g_pmussi_baseaddr) {
		pr_err("hisi pmic reg write fail, pmussi baseaddr is null!\n");
		return ;
	}
	*(volatile unsigned char*)PMUSSI_REG(addr) = val;/*lint !e679*/
}
...
#endif
	of_property_read_u32_array(np, "hisilicon,pmic_fpga_flag", (u32 *)&fpga_flag, 1);
