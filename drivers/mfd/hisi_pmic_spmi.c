...
static struct vbus_irq_info g_pmic_vbus_mask = {0};
static struct vbus_irq_info g_pmic_vbus_clear = {0};
#ifndef BIT
#define BIT(x)		(0x1U << (x))
#endif
...
u32 hisi_pmic_read(struct hisi_pmic *pmic, int reg)
{
	...
	ret = spmi_ext_register_readl(pdev->ctrl, pdev->sid, reg, (unsigned char*)&read_value, 1);/*lint !e734 !e732 */
	if (ret) {
...
void hisi_pmic_write(struct hisi_pmic *pmic, int reg, u32 val)
{
	...
	ret = spmi_ext_register_writel(pdev->ctrl, pdev->sid, reg, (unsigned char*)&val, 1);/*lint !e734 !e732 */
	if (ret) {
...
static int get_pmic_device_tree_data(struct device_node *np, struct hisi_pmic *pmic)
{
	...
	ret = of_property_read_u32_array(np,
		"hisilicon,hisi-pmic-vbus-mask-addr",
		(u32 *)&g_pmic_vbus_mask, VBUS_IRQ_INFO_MEMBERS_NUMS);
	if (ret)
...
static int get_pmic_device_tree_all_data(struct spmi_device *pdev, struct hisi_pmic *pmic)
{
	int ret =0;
	int ret1= 0;
	struct device *dev = &pdev->dev;
	...
	/*get pmic dts the first group irq*/
	ret1 = get_pmic_device_tree_data(np, pmic);
	if (ret1) {
...
static void hisi_pmic_vbus_irq(struct vbus_irq_info *vbus_info, unsigned int mask)
{
	unsigned int value;
	unsigned int temp;
...
EXPORT_SYMBOL(hisi_pmic_vbus_irq_mask);
...
EXPORT_SYMBOL(hisi_pmic_clear_vbus_irq);
...
static int hisi_pmic_probe(struct spmi_device *pdev)
{
	...
	ret = get_pmic_device_tree_all_data(pdev, pmic);
	if (ret) {
