...
static int kirin_pcie_cs_pll_ctrl(struct kirin_pcie *pcie,
		int enable, enum cspll pll)
{
	int ret;
		...
		enum rc_power_status on_flag)
{
	...
	PCIE_PR_INFO("pciectrl/pciephy pclk open Done");

	/* unset module */
	kirin_pcie_reset_ctrl(pcie, RST_DISABLE);
	PCIE_PR_INFO("module unreset Done");
