					...
					struct platform_device *pdev)
{
	...
	if (dbi == NULL) {
		PCIE_PR_ERR("Failed to get PCIe dbi resource");
		return -1;
	}
	pcie->dtsinfo.dbi_phy_base = lower_32_bits(dbi->start);

	dbi_vir_base = devm_ioremap_resource(&pdev->dev, dbi);
	if (IS_ERR(dbi_vir_base)) {
		PCIE_PR_ERR("Failed to get PCIe dbi base");
		return PTR_ERR(dbi_vir_base);
	}

	pcie->pci->dbi_base = dbi_vir_base;
...
static void kirin_pcie_get_eyeparam(struct kirin_pcie *pcie, struct platform_device *pdev)
{
	...
	PCIE_PR_INFO("Update eye params: %d", dtsinfo->eye_param_nums);

	dtsinfo->eye_param_data = (u32 *)kzalloc((u64)(dtsinfo->eye_param_nums) * TRIPLE_TUPLE * sizeof(u32), GFP_KERNEL);
	...
	ret = of_property_read_u32_array(np, "eye_param_details",
			dtsinfo->eye_param_data, (u64)(dtsinfo->eye_param_nums) * TRIPLE_TUPLE);
	if (ret) {
...
static int get_pcie_link_status(struct kirin_pcie *pcie)
{
	return dw_pcie_link_up(pcie->pci);
}

static void kirin_pcie_err_resp_enable(struct kirin_pcie *pcie)
{
	u32 val;

	/* enable err resp */
	val = kirin_elb_readl(pcie, SOC_PCIECTRL_CTRL7_ADDR);
	val |= PCIE_ERR_BRESP_ENABLE;
	val |= PCIE_ERR_RRESP_ENABLE;
...
static int kirin_pcie_establish_link(struct kirin_pcie *pcie)
{
	...
	PCIE_PR_INFO("Setup rc done");

	/* assert LTSSM enable */
	kirin_elb_writel(pcie, PCIE_LTSSM_ENABLE_BIT,
			  PCIE_APP_LTSSM_ENABLE);
	/* check if the link is up or not */
	while (!get_pcie_link_status(pcie)) {
		mdelay(1);
		count++;
	...
	if (IS_ENABLED(CONFIG_KIRIN_PCIE_MAY))
		kirin_pcie_err_resp_enable(pcie);
...
static irqreturn_t kirin_pcie_linkdown_irq_handler(int irq, void *arg)
{
	...
	PCIE_PR_ERR("RC[%u], Triggle linkdown irq[%d]", pcie->rc_id, irq);
...
static irqreturn_t kirin_pcie_cpltimeout_irq_handler(int irq, void *arg)
{
	...
	PCIE_PR_ERR("RC[%u], Triggle CPL timeout irq[%d]", pcie->rc_id, irq);
	/* clear interrupt */
...
static int kirin_pcie_probe(struct platform_device *pdev)
{
	...
	if (pcie->dtsinfo.ep_device_type != EP_DEVICE_HI110X &&
		...
		ret = kirin_pcie_power_notifiy_register(pcie->rc_id,
			kirin_pcie_pwron_default, kirin_pcie_pwroff_default, (void *)pcie);
		if (ret)
...
static int kirin_pcie_resume_noirq(struct device *dev)
{
	...
	if (atomic_read(&(pcie->is_enumerated)) &&
		...
		PCIE_PR_INFO("Begin to recover RC cfg");
		if (pcie->rc_dev)
...
static int kirin_pcie_bypass_l1ss(struct kirin_pcie *pcie)
{
#ifdef CONFIG_KIRIN_PCIE_MAR
	int ret;
	unsigned char ip[KIRIN_SOC_IP_SIZE] = {0};
	...
	ret = get_efuse_dieid_value(ip, sizeof(ip), KIRIN_DIEID_TIMEOUT_1000MS);
	if (ret) {
		PCIE_PR_INFO("Fail to get dieid");
		return 1;
	}

	val = kirin_elb_readl(pcie, SOC_PCIECTRL_STATE4_ADDR) & 0xc0;
	chip_version = ((ip[19] & 0x3) << 0x1) + ((ip[18] & 0x80) >> 0x7);
	if (val == 0x80 && chip_version < 0x5)
		return 1;
#endif
...
static void kirin_pcie_aspm_enable(struct kirin_pcie *pcie)
{
	...
	if (kirin_pcie_bypass_l1ss(pcie)) {
		PCIE_PR_INFO("Bypass L1ss");
		return;
	}
	...
	if ((pcie->dtsinfo.ep_device_type == EP_DEVICE_MODEM) &&
		pcie->ep_dev) {
		ep_l1ss_pm = pci_find_ext_capability(pcie->ep_dev,
				PCI_EXT_L1SS_CAP_ID);
		if (!ep_l1ss_pm) {
			PCIE_PR_ERR("Failed to get EP PCI_EXT_L1SS_CAP_ID");
			return;
		}
		pci_read_config_dword(pcie->ep_dev, ep_l1ss_pm + PCI_EXT_L1SS_CAP,
			&reg_val);
		if ((reg_val & PCI_EXT_L1SS_SUP_ALL) != PCI_EXT_L1SS_SUP_ALL) {
			PCIE_PR_INFO("Bypass L1ss, EP not support(0x%x)", reg_val);
			return;
		}
	}
#endif
...
int kirin_pcie_enumerate(u32 rc_idx)
{
	...
	if (pcie->rc_dev->subordinate) {
		list_for_each_entry(dev,
			&pcie->rc_dev->subordinate->devices, bus_list) {
			if (pci_is_pcie(dev)) {
				pcie->ep_dev = dev;
...
int kirin_pcie_remove_ep(u32 rc_idx)
{
	...
	list_for_each_entry_safe(dev, temp,
		&pcie->rc_dev->subordinate->devices, bus_list) {
		if (pci_is_pcie(dev)) {
...
bool kirin_pcie_bypass_pm(struct pci_dev *dev)
{
	...
	PCIE_PR_INFO("EP class version: 0x%x\n", class);
