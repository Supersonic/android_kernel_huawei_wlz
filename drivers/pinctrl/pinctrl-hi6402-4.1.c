			...
			unsigned *nmaps)
{
	...
	*nmaps = 0;
	...
	snprintf(gname, strlen(np->name) + GSUFFIX_LEN, "%s%s", np->name, GROUP_FLAG);
	...
	if (of_find_property(np, "pinmux-hi6402,func-sel", NULL)) {
		...
		snprintf(fname, strlen(np->name) + FSUFFIX_LEN, "%s%s", np->name, FUNCTION_FLAG);
			    	...
			    	const struct hi6402_pinctrl_function **func)
{
	...
	if (!pdesc) {
		dev_err(pindev->dev, "pdesc get error!\n");
		return -ENOTSUPP;
	}
...
static void hi6402_pinconf_clear_bias(struct pinctrl_dev *pctldev, unsigned pin)
{
	...
	for (i = 0; i < ARRAY_SIZE(pin_bias); i++) {
		...
		hi6402_pinconf_set(pctldev, pin, &config, 1);
	}
}
					...
					unsigned long *config, unsigned num_configs)
{
	...
	for(j=0; j < num_configs; j++) {
		for (i = 0; i < func->confs->nconfs; i++) {
			if (pinconf_to_config_param(config[j]) != func->confs->configs[i].param)
				continue;
			data = pindev->read(pindev, func->confs->reg);
			arg = pinconf_to_config_argument(config[j]);
			switch (func->confs->configs[i].param) {
				...
				unsigned group, unsigned long *config, unsigned num_configs)
{
	...
	for (i = 0; i < npins; i++)
		if (hi6402_pinconf_set(pctldev, pins[i], config, num_configs))
			return -ENOTSUPP;
						...
						char *gname)
{
	...
	snprintf(fname, strlen(cfg_np->name) + FSUFFIX_LEN, "%s%s", cfg_np->name, FUNCTION_FLAG);
				...
				struct hi6402_pinctrl_device *pindev)
{
	...
	for_each_child_of_node(dev_np, cfg_np) {
		...
		snprintf(gname, strlen(cfg_np->name) + GSUFFIX_LEN, "%s%s", cfg_np->name, GROUP_FLAG);
...
static int hi6402_pinctrl_register(struct hi6402_pinctrl_device *pindev)
{
	...
	for (pin = 0; pin < pinctrl_desc->npins; pin++) {
		snprintf(pin_names, sizeof(char) * PIN_NAME_LENGTH *
			pinctrl_desc->npins, "pin%02d", pin);
		pdesc = pindesc + pin;
