...
#ifndef SOC_RTCTIMERWDT_WDLOAD_B_ADDR
#define SOC_RTCTIMERWDT_WDLOAD_B_ADDR(base)     ((base) + (0x0018UL))
#endif
...
static unsigned int wdt_timeleft(struct watchdog_device *wdd)
{
	...
	if (!((u32)readl_relaxed(wdt->base + WDTRIS) & INT_MASK))
		load += wdt->load_val + 1;
	spin_unlock(&wdt->lock);
...
static int sp805_wdt_rdr_init(void)
{
	...
	(void)memset_s(&einfo, sizeof(struct rdr_exception_info_s),
...
void sp805_wdt_dump(void)
{
	...
	pr_crit("timer 0x%pK: next 0x%pK pprev 0x%pK expires %lu jiffies %lu "
