...
static int mipi_cmi_panel_set_backlight(struct platform_device *pdev, uint32_t bl_level)
{
	...
	if (hisifd->panel_info.bl_set_type & BL_SET_BY_PWM) {
		...
		HISI_FB_DEBUG("fb%d, backlight=%d.\n", hisifd->index, bl_level_adjust[1]);
	} else {
...
static int mipi_cmi_probe(struct platform_device *pdev)
{
	...
	HISI_FB_INFO("bl_type=0x%x, display_type=0x%x, ifbc_type=0x%x.\n", bl_type, lcd_display_type, lcd_ifbc_type);
	...
	gpio_lcd_vcc1v8 = of_get_named_gpio(np, "gpios", 0);			//gpio_16_2
	gpio_lcd_p5v5_enable = of_get_named_gpio(np, "gpios", 1); 	//gpio_16_0
	gpio_lcd_n5v5_enable = of_get_named_gpio(np, "gpios", 2);	//gpio_20_7
	gpio_lcd_reset = of_get_named_gpio(np, "gpios", 3); 			//gpio_17_3
	gpio_lcd_bl_enable = of_get_named_gpio(np, "gpios", 4); 		//gpio_20_6

	HISI_FB_INFO("vcc=%d, vsp=%d, vsn=%d, reset=%d, bl_enable=%d\n",
