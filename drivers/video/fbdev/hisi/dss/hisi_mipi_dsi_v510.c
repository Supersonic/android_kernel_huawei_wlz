...
static void mipi_cdphy_init_config(char __iomem *mipi_dsi_base, struct hisi_panel_info *pinfo)
{
	if (pinfo->mipi.phy_mode == CPHY_MODE) {
		...
		if (g_fpga_flag) {
			...
			mipi_config_phy_test_code(mipi_dsi_base, 0x0001004A, pinfo->dsi_phy_ctrl.rg_pll_fbkdiv);
		}
		...
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010058, (0x4 + pinfo->mipi.lane_nums) << 4 | 0);
		...
		if (g_fpga_flag) {
			...
			mipi_config_phy_test_code(mipi_dsi_base, 0x00010048, pinfo->dsi_phy_ctrl.rg_0p8v +
				(pinfo->dsi_phy_ctrl.rg_2p5g << 1) + (pinfo->dsi_phy_ctrl.rg_320m << 2) + (pinfo->dsi_phy_ctrl.rg_band_sel << 3));
			//PLL configuration III
			mipi_config_phy_test_code(mipi_dsi_base, 0x00010049, pinfo->dsi_phy_ctrl.rg_pre_div);
			//PLL configuration IV
			mipi_config_phy_test_code(mipi_dsi_base, 0x0001004A, pinfo->dsi_phy_ctrl.rg_div);
		...
		mipi_config_phy_test_code(mipi_dsi_base, 0x0001005B, pinfo->dsi_phy_ctrl.rg_vrefsel_vcm);
		//PLL update control
		mipi_config_phy_test_code(mipi_dsi_base, 0x0001004B, 0x1);
...
static void get_mipi_dsi_timing_config_para(struct hisi_fb_data_type *hisifd,
	struct mipi_dsi_phy_ctrl* phy_ctrl, struct mipi_dsi_timing *timing)
{
	...
	if (lane_byte_clk == pinfo->dsi_phy_ctrl.lane_byte_clk_default) {
		...
		timing->hline_time = pinfo->mipi.hline_time;
	} else {
		...
		timing->hbp = (uint32_t)ROUND1(pinfo->mipi.hbp * lane_byte_clk,
			pinfo->dsi_phy_ctrl.lane_byte_clk_default);
		timing->hline_time = (uint32_t)ROUND1(pinfo->mipi.hline_time * lane_byte_clk,
			pinfo->dsi_phy_ctrl.lane_byte_clk_default);
	}
	...
	HISI_FB_DEBUG("lanebyteclk: %llu M, %llu M, htiming: %d, %d, %d, %d. new: %d, %d, %d, %d\n",
		lane_byte_clk, pinfo->dsi_phy_ctrl.lane_byte_clk_default,
		pinfo->mipi.hsa, pinfo->mipi.hbp, pinfo->mipi.hline_time,
...
void mipi_init(struct hisi_fb_data_type *hisifd, char __iomem *mipi_dsi_base)
{
	...
	if (is_mipi_cmd_panel(hisifd)) {
		...
		set_reg(mipi_dsi_base + MIPIDSI_EDPI_CMD_SIZE_OFFSET, timing.width, 16, 0);
		...
		if (pinfo->mipi.hs_wr_to_time == 0) {
			...
			set_reg(mipi_dsi_base + MIPIDSI_HS_WR_TO_CNT_OFFSET,
				(0x1 << 24) | (pinfo->mipi.hs_wr_to_time * pinfo->dsi_phy_ctrl.lane_byte_clk / 1000000000UL), 25, 0); //lint !e712
		}
	...
	*/
	...
	set_reg(mipi_dsi_base + MIPIDSI_VID_VFP_LINES_OFFSET, timing.vfp, 10, 0);
...
static void mipi_tx_off_rx_ulps_config(struct hisi_fb_data_type *hisifd, struct platform_device *pdev)
{
	int delay_count = 0;
	...
	while ((((uint32_t)inp32(hisifd->mipi_dsi0_base + MIPI_LDI_VSTATE)) & 0x7FF) != 0x1) {
	...
	char __iomem *mipi_dsi_base, uint32_t mipi_idx)
{
	...
	while ((tmp & cmp_stopstate_val) != cmp_stopstate_val) {
		...
		if ((tmp & cmp_stopstate_val) == (cmp_stopstate_val & (~ BIT(2)))) {
			HISI_FB_INFO("fb%d, datalanes are in stop state, pull down phy_txrequestclkhs.\n", hisifd->index);
	...
	if (g_fpga_flag) {
		if (pinfo->mipi.phy_mode == CPHY_MODE) {
			...
			mipi_config_phy_test_code(mipi_dsi_base, 0x00010048, phy_ctrl->rg_0p8v + (phy_ctrl->rg_2p5g << 1) +
				(pinfo->dsi_phy_ctrl.rg_320m << 2) + (phy_ctrl->rg_band_sel << 3) + (phy_ctrl->rg_cphy_div << 4));
			/* PLL configuration III */
			...
			mipi_config_phy_test_code(mipi_dsi_base, 0x00010046, phy_ctrl->rg_cp + (phy_ctrl->rg_lpf_r << 4));
			/* PLL configuration II */
			mipi_config_phy_test_code(mipi_dsi_base, 0x00010048, phy_ctrl->rg_0p8v +
				(phy_ctrl->rg_2p5g << 1) + (phy_ctrl->rg_320m << 2) + (phy_ctrl->rg_band_sel << 3));
			/* PLL configuration III */
			mipi_config_phy_test_code(mipi_dsi_base, 0x00010049, phy_ctrl->rg_pre_div);
			/* PLL configuration IV */
			mipi_config_phy_test_code(mipi_dsi_base, 0x0001004A, phy_ctrl->rg_div);
...
static bool check_pctrl_trstop_flag(struct hisi_fb_data_type *hisifd, int time_count)
{
	...
	if (is_dual_mipi_panel(hisifd)) {
		for(count = 0; count < time_count; count++) {
			tmp = inp32(hisifd->mipi_dsi0_base + MIPIDSI_DPHYTX_TRSTOP_FLAG_OFFSET);
...
int mipi_dsi_reset_underflow_clear(struct hisi_fb_data_type *hisifd)
{
	...
	lane_byte_clk = (hisifd->panel_info.mipi.phy_mode == DPHY_MODE) ?
		hisifd->panel_info.dsi_phy_ctrl.lane_byte_clk : hisifd->panel_info.dsi_phy_ctrl.lane_word_clk;
	lane_byte_clk /= 1000000; /* unit conversion: Hz to MHz */

	if (0 == lane_byte_clk) {
		vfp_time = 200; /* exception, set 200us */
		HISI_FB_WARNING("lane_byte_clk == 0\n");
	} else {
...
int mipi_dsi_bit_clk_upt_isr_handler(struct hisi_fb_data_type *hisifd)
{
	...
	vfp_time = (uint32_t)inp32(hisifd->mipi_dsi0_base +
		MIPIDSI_VID_HLINE_TIME_OFFSET) & VFP_TIME_MASK;
	if (lane_byte_clk != 0) {
		vfp_time = vfp_time * (pinfo->mipi.vfp + VFP_TIME_OFFSET) /
	...
	HISI_FB_DEBUG("timediff = %d us, vfp_time = %d us\n", timediff, vfp_time);
