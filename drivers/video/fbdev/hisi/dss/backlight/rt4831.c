	...
	unsigned int size)
{
	...
	RT4831_INFO("rt4831_bl_mode_reg_init in\n");
	...
	RT4831_INFO("rt4831_bl_mode_reg_init success\n");
}
...
static int rt4831_check_ovp_error(void)
{
	...
	if (!g_fake_lcd_flag) {
		...
		RT4831_INFO("rt4831_check_ovp_error:regmap_read val %u\n",
			val);
		if (ret < 0) {
...
int rt4831_set_backlight_reg(uint32_t bl_level)
{
	...
	RT4831_INFO("rt4831_set_backlight_reg bl_level = %u\n", bl_level);
	...
	RT4831_INFO("rt4831_set_backlight_reg exit succ\n");
	return ret;
...
i2c_error:
	...
	dev_err(rt4831_g_chip->dev, "%s:i2c access fail to register\n",
		__func__);
	RT4831_INFO("rt4831_set_backlight_reg exit fail\n");
	return ret;
}
...
static void rt4831_bl_resume_workqueue_handler(struct work_struct *work)
{
	...
	RT4831_INFO("rt4831_bl_resume_workqueue_handler in\n");
	if (work == NULL) {
	...
	up(&(rt4831_g_chip->test_sem));
	RT4831_INFO("rt4831_bl_resume_workqueue_handler exit\n");
}
...
ssize_t rt4831_blkit_set_normal_work_mode(void)
{
	...
	RT4831_INFO("rt4831_blkit_set_normal_work_mode in\n");
	if (down_interruptible(&rt4831_g_chip->test_sem)) {
	...
	up(&(rt4831_g_chip->test_sem));
	RT4831_INFO("rt4831_blkit_set_normal_work_mode exit!\n");
	return error;

out:
	up(&(rt4831_g_chip->test_sem));
	RT4831_INFO("rt4831_blkit_set_normal_work_mode failed\n");
	return error;
}
...
ssize_t rtblkit_set_enhance_work_mode(void)
{
	...
	RT4831_INFO("rtblkit_set_enhance_work_mode in!\n");
	if (down_interruptible(&rt4831_g_chip->test_sem)) {
		RT4831_ERR("down_trylock fail return\n");
		return error;
	}
	...
	up(&(rt4831_g_chip->test_sem));
	RT4831_INFO("rtblkit_set_enhance_work_mode exit!\n");
	return error;
	...
	if (g_fake_lcd_flag) {
		...
		if (val != DEVICE_FAULT_OCCUR) {
			...
			if (!ret) {
				...
				dsm_client_record(lcd_dclient, "FLAG=%d!\n", val);
				dsm_client_notify(lcd_dclient,
					DSM_LCD_OVP_ERROR_NO);
			} else {
				dev_err(&client->dev,
