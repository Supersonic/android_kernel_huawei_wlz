...
static void hisi_fb_displayeffect_update(struct hisi_fb_data_type *hisifd)
{
	...
	hisifd->effect_updated_flag.post_xcc_effect_updated = true;
...
static void hisi_fb_unblank_wq_handle(struct work_struct *work)
{
	...
	if (is_mipi_video_panel(hisifd) &&
		...
		hisifd_list[AUXILIARY_PANEL_IDX]->panel_power_on)  {
		// if video panel aod, wait for offline blank
		HISI_FB_INFO(" wait for offline blank!\n");
		while (hisifd_list[AUXILIARY_PANEL_IDX]->panel_power_on) {
			mdelay(1);
			if (++try_times > 1000) {
				HISI_FB_ERR("wait for offline blank timeout!\n");
				break;
			}
		}
	}
...
static int hisi_fb_blank(int blank_mode, struct fb_info *info)
{
...
#if CONFIG_SH_AOD_ENABLE
	sensorhub_aod_hwlock_succ = hisi_sensorhub_aod_hw_lock(hisifd);
#endif

	hisi_log_enable_if((hisifd->index != AUXILIARY_PANEL_IDX), "fb%d, blank_mode(%d) +\n", hisifd->index, blank_mode);

#if CONFIG_SH_AOD_ENABLE
	if ((hisifd->index == PRIMARY_PANEL_IDX) && (FB_BLANK_UNBLANK == blank_mode)) {
		ret = hisi_aod_set_blank_mode(blank_mode);
		if (ret == -3) {
			HISI_FB_INFO("AOD in fast unlock mode\n");
			return 0;
		}
	}
	...
	if ((FB_BLANK_POWERDOWN == blank_mode) && (hisifd->enable_fast_unblank)) {
	...
	if ((hisifd->index == PRIMARY_PANEL_IDX) &&
		...
		(FB_BLANK_POWERDOWN == blank_mode)) {
		hisi_aod_set_blank_mode(blank_mode);
	}
...
static int hisifb_dss_get_platform_limit(struct hisi_fb_data_type *hisifd, void __user *argp)
{
	struct platform_limit get_platform_limit;
	...
	memset(&get_platform_limit, 0, sizeof(struct platform_limit));
	get_platform_limit.max_hwc_mmbuf_size = mmbuf_max_size;
	get_platform_limit.max_mdc_mmbuf_size = dss_mmbuf_reserved_info[SERVICE_MDC].size;
	get_platform_limit.fold_display_support = hisifd->panel_info.cascadeic_support;
	get_platform_limit.desktop_landscape_support = hisifd->panel_info.desktop_landscape_support;
	get_platform_limit.dummy_pixel_num = hisifd->panel_info.dummy_pixel_num;

	if (copy_to_user(argp, &get_platform_limit, sizeof(struct platform_limit))) {
		HISI_FB_ERR("copy to user fail!\n");
		return -EINVAL;
	}

	HISI_FB_INFO("max_hwc_mmbuf_size=%d, max_mdc_mmbuf_size=%d, fold_display_support=%d, dummy_pixel_num=%d!\n",
		get_platform_limit.max_hwc_mmbuf_size, get_platform_limit.max_mdc_mmbuf_size,
		get_platform_limit.fold_display_support, get_platform_limit.dummy_pixel_num);
	return 0;
}
...
static int hisi_fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case HISIFB_MDC_POWER_DOWNUP_CTRL:
		ret = hisi_mdc_power_ctrl(info, argp);
		break;
#endif
	...
	case HISIFB_PANEL_REGION_NOTIFY:
		...
		break;
	case HISIFB_PLATFORM_LIMIT_GET:
		ret = hisifb_dss_get_platform_limit(hisifd, argp);
		break;
...
static uint32_t get_initial_fps(struct hisi_fb_data_type *hisifd, struct fb_var_screeninfo *var)
{
	...
	if (hisifd->panel_info.mipi.dsi_timing_support) {
		lane_byte_clock = (hisifd->panel_info.mipi.phy_mode == DPHY_MODE) ?
				(uint64_t)(hisifd->panel_info.mipi.dsi_bit_clk * 2 / 8) :
				(uint64_t)(hisifd->panel_info.mipi.dsi_bit_clk / 7);
		lane_byte_clock = lane_byte_clock * 1000000UL;
...
static int hisi_fb_register(struct hisi_fb_data_type *hisifd)
{
	...
	HISI_FB_INFO("hisifd->index = %d", hisifd->index);
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		var->reserved[0] = get_initial_fps(hisifd, var);
		HISI_FB_INFO("dsi_timing_support %d, fps = reserved[0] = %d",
			panel_info->mipi.dsi_timing_support, var->reserved[0]);
	}
...
#if defined (CONFIG_HISI_FB_V320) || defined (CONFIG_HISI_FB_970) || defined (CONFIG_HISI_FB_V501) || \
...
static int hisi_fb_probe(struct platform_device *pdev)
{
	...
	if (!hisi_fb_resource_initialized) {
		...
		if (ret) {
			dev_err(dev, "failed to get mmbuf_size_max\n");
			return -ENXIO;
		}
		...
		if (!hisifd_irq_dptx) {
			dev_err(dev, "failed to get hisifd_irq_dptx resource.\n");
			return -ENXIO;
		}

		hisifd_irq_mdc = irq_of_parse_and_map(np, 6);
		if (!hisifd_irq_mdc) {
			dev_err(dev, "failed to get hisifd_irq_mdc resource.\n");
			return -ENXIO;
		}
	#endif
		...
		hisifd_media_common_base = of_iomap(np, 10);
		if (!hisifd_media_common_base) {
		...
		if (!hisifd_mmc0_crg_base) {
			dev_err(dev, "failed to get hisifd_mmc0_crg_base resource.\n");
			return -ENXIO;
		}

		hisifd_dp_phy_base = of_iomap(np, 12);
		if (!hisifd_dp_phy_base) {
			dev_err(dev, "failed to get hisifd_dp_phy_base resource.\n");
			return -ENXIO;
		}
		#endif
		...
		if (ret != 0) {
			dev_err(dev, "failed to get dss_clk_gate_dpctrl_16m resource! ret=%d.\n", ret);
			return -ENXIO;
		}
		ret = of_property_read_string_index(np, "clock-names", 15, &g_dss_pclk_gate_dpctrl_name); //pclk
		if (ret != 0) {
			dev_err(dev, "failed to get dss_pclk_gate_dpctrl resource! ret=%d.\n", ret);
			return -ENXIO;
		}
		ret = of_property_read_string_index(np, "clock-names", 16, &g_dss_aclk_dpctrl_name); //ack
		if (ret != 0) {
			dev_err(dev, "failed to get dss_aclk_gate_dpctrl resource! ret=%d.\n", ret);
			return -ENXIO;
		}
#endif
