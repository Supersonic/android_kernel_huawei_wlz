...
static int hisifb_effect_info_set_handler(const void __user *argp)
{
	...
	if (effect_info.modules & DSS_EFFECT_MODULE_DPPROI) {
		ret = hisi_effect_dpproi_info_set(hisifd_primary, effect_info.dpp_roi);
		if (ret) {
			HISI_FB_ERR("failed to set dpproi info\n");
			goto err_out_spin;
		}
	}
	...
	if ((effect_info.modules & DSS_EFFECT_MODULE_LCP_GMP) ||
		...
		ret = hisi_effect_dpp_buf_info_set(hisifd_primary, &effect_info);
		if (ret) {
...
int display_engine_amoled_algo_param_set(struct hisi_fb_data_type *hisifd, display_engine_amoled_param_t *param)
{
	...
	hisifd->de_info.amoled_param.lowac_dbv_thres_dc_low =
		param->lowac_dbv_thres_dc_low;
	up(&hisifd->blank_sem);
	...
	HISI_FB_DEBUG("[effect] lowac_dbv_thres_dc_low = %d",
		hisifd->de_info.amoled_param.lowac_dbv_thres_dc_low);
	return ret;
}
...
int display_engine_panel_info_get(struct hisi_fb_data_type *hisifd, display_engine_panel_info_param_t *param)
{
	...
	pinfo = &(hisifd->panel_info);
	param->width = pinfo->xres - pinfo->dummy_pixel_num;
...
static int display_engine_foldable_info_get(struct hisi_fb_data_type *hisifd, display_engine_foldable_info_t *param) {
	...
	int ret;
	ret = lcd_get_dbv_stat(hisifd, param->dbv_acc, sizeof(param->dbv_acc),
		param->screen_on_duration, sizeof(param->screen_on_duration));
	if (ret != 0) {
...
void hiace_size_config(struct hisi_fb_data_type *hisifd, uint32_t width, uint32_t height)
{
	...
	hisifd->effect_info.hiace.image_info = (height << 16) | width;
	...
	hisifd->effect_info.hiace.half_block_info = (half_block_h << 16) | half_block_w;
	...
	hisifd->effect_info.hiace.xyweight = (fixbit_y << 26) | (reciprocal_y << 16) |
