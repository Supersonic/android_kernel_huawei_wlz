...
void dumpDssOverlay(struct hisi_fb_data_type *hisifd, dss_overlay_t *pov_req)
{
	...
	if (dumpDss == NULL)
	{
		HISI_FB_INFO("dumpDSS is NULL");
		return;
	}

	if ((dumpDss->dss_buf == NULL) || (dumpDss->dss_buf_len != 0))
	{
		HISI_FB_INFO("dss_buf is NULL or dumpDss->dss_buf_len=%d", dumpDss->dss_buf_len);
		return;
	}
...
{
	uint32_t dummy_pixel_num = hisifd->panel_info.dummy_pixel_num;

	if (dummy_pixel_num != 0) {
		if ((pov_req->rog_width > 0) && (get_panel_xres(hisifd) != 0))
			dummy_pixel_num = dummy_pixel_num * (pov_req->rog_width / get_panel_xres(hisifd));

		pdst_rect->x = pdst_rect->x + dummy_pixel_num / 2;
	}
}
	...
	char __iomem *dma_base, dss_rdma_t *s_dma)
{
	...
	if (hisifd->video_idle_ctrl.idle_frame_display)
		HISI_FB_DEBUG("data_addr0=0x%x, header=0x%x, payload=0x%lx\n",
			s_dma->data_addr0, s_dma->afbcd_hreg_hdr_ptr_lo,
	...
	#if defined (CONFIG_HISI_FB_970) || defined (CONFIG_HISI_FB_V501) || defined (CONFIG_HISI_FB_V510) || defined (CONFIG_HISI_FB_V350)
	...
	dss_rect_t src_rect,int aligned_pixel)
{
	...
	if (is_YUV_P_420(layer->img.format) || is_YUV_P_422(layer->img.format)) {
		...
		aligned_rect->right = ALIGN_UP(src_right, 2 * (uint32_t)aligned_pixel) - 1;
	} else {
		aligned_rect->left = ALIGN_DOWN((uint32_t)src_rect.x, (uint32_t)aligned_pixel);
		aligned_rect->right = ALIGN_UP(src_right, (uint32_t)aligned_pixel) - 1;
	}

	if (is_YUV_SP_420(layer->img.format) || is_YUV_P_420(layer->img.format)) {
		aligned_rect->top = ALIGN_DOWN((uint32_t)src_rect.y, 2);
		aligned_rect->bottom = ALIGN_UP(src_bottom, 2) - 1;
	} else {
...
static int hisi_dss_rdma_stretch(dss_layer_t *layer, dss_rect_t *out_aligned_rect,
	uint32_t *afbcd_half_block_mode, const bool *rdma_stretch_enable,
	uint32_t *afbcd_stretch_inc, uint32_t *afbcd_stretch_acc)
{
	int rdma_transform;
	if(NULL == layer)  {
		HISI_FB_ERR("layer is NULL");
	...
	dss_rect_t *out_aligned_rect, bool *rdma_stretch_enable)
{
	...
	*rdma_stretch_enable = (stretched_line_num > 0) ? true : false;
	...
	#if defined (CONFIG_HISI_FB_970) || defined (CONFIG_HISI_FB_V501) || defined (CONFIG_HISI_FB_V510) || defined (CONFIG_HISI_FB_V350)
		ret = hisi_dss_hfbcd_config(hisifd, ovl_idx, layer, clip_rect, out_aligned_rect, rdma_stretch_enable);
		if (ret != 0) {
			HISI_FB_ERR("hisi_dss_hfbcd_config failed! ret = %d\n", ret);
			return -EINVAL;
		}
	#endif
		return 0;
	...
	if (layer->need_cap & CAP_HEBCD) {
		...
		ret = hisi_dss_hebcd_config(hisifd, ovl_idx, layer, clip_rect, out_aligned_rect, rdma_stretch_enable);
		 if (ret != 0) {
			HISI_FB_ERR("hisi_dss_hebcd_config failed! ret = %d\n", ret);
			return -EINVAL;
		}
	#endif
		return 0;
	...
	if (layer->need_cap & CAP_AFBCD) {
		...
		//aligned rect
		if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
			aligned_rect.left = ALIGN_DOWN((uint32_t)src_rect.y, AFBC_BLOCK_ALIGN);
			aligned_rect.right = ALIGN_UP(src_bottom, AFBC_BLOCK_ALIGN) - 1;
			aligned_rect.top = ALIGN_DOWN((uint32_t)src_rect.x, AFBC_BLOCK_ALIGN);
			aligned_rect.bottom = ALIGN_UP(src_right, AFBC_BLOCK_ALIGN) - 1;

			afbcd_height_bf_str = aligned_rect.right - aligned_rect.left + 1;
		} else {
		...
		afbc_rect.left = ALIGN_DOWN((uint32_t)src_rect.x, AFBC_HEADER_ADDR_ALIGN);
		afbc_rect.right = ALIGN_UP(src_right, AFBC_HEADER_ADDR_ALIGN) - 1;
		afbc_rect.top = ALIGN_DOWN((uint32_t)src_rect.y, AFBC_BLOCK_ALIGN);
		afbc_rect.bottom = ALIGN_UP(src_bottom, AFBC_BLOCK_ALIGN) - 1;
		...
		dma->afbcd_line_crop = set_bits32(dma->afbcd_line_crop,
			(afbcd_top_crop_num << 8 | afbcd_bottom_crop_num), 16, 0);
	#else
		dma->afbcd_line_crop = set_bits32(dma->afbcd_line_crop,
	...
	// aligned_rect
	ret = hisi_dss_rdma_aligned_rect(layer, &aligned_rect, src_rect, aligned_pixel);
	if(ret != 0) {
		return -EINVAL;
	}
	// aligned_mask_rect
	...
	do {
		...
		if ((dst_rect.w > (src_rect.w * SCF_UPSCALE_MAX))
			...
			HISI_FB_ERR("width out of range, original_src_rec(%d, %d, %d, %d) "
				"src_rect(%d, %d, %d, %d), dst_rect(%d, %d, %d, %d), rdma_stretch_enable=%d\n",
				layer->src_rect.x, layer->src_rect.y, layer->src_rect.w, layer->src_rect.h,
...
#if defined(CONFIG_HISI_FB_970) || defined(CONFIG_HISI_FB_V320) || defined(CONFIG_HISI_FB_V501) || \
	defined(CONFIG_HISI_FB_V510) || defined (CONFIG_HISI_FB_V330) || defined (CONFIG_HISI_FB_V350)
	...
	dss_layer_t *layer, dss_wb_layer_t *wb_layer, int ovl_idx, dss_rect_t *wb_ov_block_rect, bool has_base)
{
	...
	if (wb_layer != NULL) {
		if (wb_layer->chn_idx < 0 || wb_layer->chn_idx >= DSS_CHN_MAX_DEFINE) {
			HISI_FB_ERR("wb_layer->chn_idx exceeds array limit\n");
			return -EINVAL;
		}
		chn_idx = wb_layer->chn_idx;
		...
		if (layer->chn_idx < 0 || layer->chn_idx >= DSS_CHN_MAX_DEFINE) {
			HISI_FB_ERR("layer->chn_idx exceeds array limit\n");
			return -EINVAL;
		}
		...
		if (layer->need_cap & (CAP_DIM | CAP_PURE_COLOR)) {
			...
			if ((chn_idx != DSS_RCHN_V2)) {
				mctl_ch->chn_ov_oen = set_bits32(mctl_ch->chn_ov_oen,
					((1 << ((uint32_t)layer_idx + 1)) | (0x100 << (uint32_t)ovl_idx)), 32, 0);//lint !e701
	...
	char __iomem *dss_base, dirty_region_updt_t *s_dirty_region_updt)
{
	...
	if (s_dirty_region_updt->ldi_vrt_ctrl0 > 1023) {
		ldi_vrt_ctrl0 = 1023;
		vsync_delay_cnt = (s_dirty_region_updt->ldi_vrt_ctrl0 - 1023) * hisifd->panel_info.mipi.hline_time;
	} else {
		ldi_vrt_ctrl0 = s_dirty_region_updt->ldi_vrt_ctrl0;
		vsync_delay_cnt = 0;
	}

	set_reg(dss_base + DSS_HI_ACE_OFFSET + DPE_IMAGE_INFO,
	...
	set_reg(dss_base + DSS_MIPI_DSI0_OFFSET + MIPI_LDI_VRT_CTRL2,
...
#if defined(CONFIG_HISI_FB_970) || defined(CONFIG_HISI_FB_V501) || defined(CONFIG_HISI_FB_V510) || defined(CONFIG_HISI_FB_V330) || defined (CONFIG_HISI_FB_V320) || defined (CONFIG_HISI_FB_V350)
	set_reg(dss_base + DSS_HI_ACE_OFFSET + DPE_IMAGE_INFO,
		s_dirty_region_updt->hiace_img_size, 32, 0);
#endif
}
	...
	dss_overlay_t *pov_req)
{
	...
	dirty_region_updt->ldi_vrt_ctrl0 = set_bits32(dirty_region_updt->ldi_vrt_ctrl0,
		(pinfo->ldi.v_front_porch + v_porch_pading), 32, 0);
	...
	if ((pdata != NULL) && pdata->set_display_region) {
		if (is_dual_mipi_panel(hisifd)) {
			...
			HISI_FB_DEBUG("dirty_updt(%d,%d, %d,%d), h_porch_pading=%d, v_porch_pading=%d.\n",
				dirty_updt.x, dirty_updt.y, dirty_updt.w, dirty_updt.h, h_porch_pading, v_porch_pading);
		} else {
	...
	dss_rect_t *aligned_rect, dss_rect_t *ov_block_rect)
{
	...
	if (layer->need_cap & CAP_AFBCE) {
		...
		if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
			if (is_YUV_SP_420(layer->dst.format)) {
				aligned_rect->x = ALIGN_DOWN(in_rect.x, HEBC_BLOCK1_YUV_HEIGHT_ALIGN);
				aligned_rect->w = ALIGN_UP(in_rect.x - aligned_rect->x + in_rect.w + dfc_w, HEBC_BLOCK1_YUV_HEIGHT_ALIGN);
				aligned_rect->y = ALIGN_DOWN(in_rect.y, HEBC_BLOCK1_YUV_WIDTH_ALIGN);
				aligned_rect->h = ALIGN_UP(in_rect.y - aligned_rect->y + in_rect.h, HEBC_BLOCK1_YUV_WIDTH_ALIGN);
			} else {
				aligned_rect->x = ALIGN_DOWN(in_rect.x, HEBC_BLOCK1_RGB_HEIGHT_ALIGN);
				aligned_rect->w = ALIGN_UP(in_rect.x - aligned_rect->x + in_rect.w + dfc_w, HEBC_BLOCK1_RGB_HEIGHT_ALIGN);
				aligned_rect->y = ALIGN_DOWN(in_rect.y, HEBC_BLOCK1_RGB_WIDTH_ALIGN);
				aligned_rect->h = ALIGN_UP(in_rect.y - aligned_rect->y + in_rect.h, HEBC_BLOCK1_RGB_WIDTH_ALIGN);
			}
		} else {
			if (is_YUV_SP_420(layer->dst.format)) {
				aligned_rect->x = ALIGN_DOWN(in_rect.x, HEBC_BLOCK0_YUV_WIDTH_ALIGN);
				aligned_rect->w = ALIGN_UP(in_rect.x - aligned_rect->x + in_rect.w + dfc_w, HEBC_BLOCK0_YUV_WIDTH_ALIGN);
				aligned_rect->y = ALIGN_DOWN(in_rect.y, HEBC_BLOCK0_YUV_HEIGHT0_ALIGN);
				aligned_rect->h = ALIGN_UP(in_rect.y - aligned_rect->y + in_rect.h, HEBC_BLOCK0_YUV_HEIGHT0_ALIGN);
			} else {
				aligned_rect->x = ALIGN_DOWN(in_rect.x, HEBC_BLOCK0_RGB_WIDTH_ALIGN);
				aligned_rect->w = ALIGN_UP(in_rect.x - aligned_rect->x + in_rect.w + dfc_w, HEBC_BLOCK0_RGB_WIDTH_ALIGN);
				aligned_rect->y = ALIGN_DOWN(in_rect.y, HEBC_BLOCK0_RGB_HEIGHT_ALIGN);
				aligned_rect->h = ALIGN_UP((uint32_t)(in_rect.y - aligned_rect->y + in_rect.h), HEBC_BLOCK0_RGB_HEIGHT_ALIGN);
			}
		}
	#endif
	...
	if (left_pad || right_pad || top_pad || bottom_pad) {
		dfc->padding_ctl = set_bits32(dfc->padding_ctl, (left_pad |
			(right_pad << 8) | (top_pad << 16) | (bottom_pad << 24) | (0x1 << 31)), 32, 0); //lint !e648
	} else {
	...
	dss_wb_layer_t *layer, dss_rect_t aligned_rect, dss_rect_t *ov_block_rect, bool last_block)
{
	...
	#if defined(CONFIG_HISI_FB_970) || defined(CONFIG_HISI_FB_V501) || defined(CONFIG_HISI_FB_V510)
		ret = hisi_dss_hfbce_config(hisifd, pov_req, layer, aligned_rect, ov_block_rect, last_block);
		if (ret != 0) {
			HISI_FB_ERR("hisi_dss_hfbce_config failed! ret = %d\n", ret);
			return -EINVAL;
		}
	#endif
		return 0;
	...
	#if defined(CONFIG_HISI_FB_V510)
		ret = hisi_dss_hebce_config(hisifd, pov_req, layer, aligned_rect, ov_block_rect, last_block);
		if (ret != 0) {
			HISI_FB_ERR("hisi_dss_hebce_config failed! ret = %d\n", ret);
			return -EINVAL;
		}
	#endif
		return 0;
	...
	if (layer->need_cap & CAP_AFBCE) {
		...
		if (afbc_header_start_pos != 0) {
			HISI_FB_ERR("transform(%d), dst_rect.x(%d), afbc_header_rect.left(%d),"
				...
				layer->transform, layer->dst_rect.x, afbc_header_rect.left, in_rect.x, afbc_header_start_pos);
			return -EINVAL;
		}
	#endif
		...
		//afbc payload
		ret = hisi_dss_wdma_afbc_check_payload(layer, afbc_payload_rect, in_rect,
			&afbc_payload_addr, &afbc_payload_stride);
		if(ret != 0) {
	...
	if (layer->need_cap & CAP_TILE) {
		l2t_interleave_n = (uint32_t)hisi_get_rdma_tile_interleave(layer->dst.stride);
		if (l2t_interleave_n < MIN_INTERLEAVE) {
			HISI_FB_ERR("tile stride should be 256*2^n, error stride:%d!\n", layer->dst.stride);
			return -EINVAL;
		}
	...
	data_num = (oft_x1 - oft_x0 + 1) * (oft_y1- oft_y0 + 1); //lint !e834
	...
	bool enable_cmdlist, int task_end, int last, bool is_first_ov_block)
{
	...
	if ((i < 0) || (i >= DSS_OVL_IDX_MAX)) {
		HISI_FB_DEBUG("ovl_idx More than Array Upper Limit!\n");
		return -1;
	}
	...
	#if defined(CONFIG_HISI_FB_970) || defined(CONFIG_HISI_FB_V501) || \
		defined(CONFIG_HISI_FB_V320) || defined (CONFIG_HISI_FB_V330) || defined(CONFIG_HISI_FB_V510)
	...
	dss_overlay_t *pov_req, uint32_t cmdlist_pre_idxs, bool enable_cmdlist, int *use_comm_mmbuf)
{
	...
	for (m = 0; m < pov_req->ov_block_nums; m++) {
		...
		for (i = 0; i < pov_h_block->layer_nums; i++) {
			...
			if ((layer->need_cap & (CAP_AFBCD | CAP_HFBCD | CAP_HEBCD)) && (layer->dst_rect.y >= pov_h_block->ov_block_rect.y)) {
				if (chn_idx < DSS_CHN_MAX_DEFINE) {
					if (ovl_idx == DSS_OVL0) {
...
static void unflow_mipi_ldi_int_handler(char __iomem *mipi_ldi_base, bool unmask)
{
	...
	tmp = inp32(mipi_ldi_base + MIPI_LDI_CPU_ITF_INT_MSK);
	if (unmask) {
		tmp &= ~ BIT_LDI_UNFLOW;
	} else {
		tmp |= BIT_LDI_UNFLOW;
	}
	outp32(mipi_ldi_base + MIPI_LDI_CPU_ITF_INT_MSK, tmp);
}
	...
	dss_overlay_t *pov_req, bool unmask)
{
	...
	if (pov_req->ovl_idx == DSS_OVL0) {
		...
		if (is_dp_panel(hisifd)) {
			tmp = inp32(hisifd->dss_base + DSS_LDI_DP_OFFSET + LDI_CPU_ITF_INT_MSK);
			if (unmask) {
				tmp &= ~ BIT_LDI_UNFLOW;
			} else {
				tmp |= BIT_LDI_UNFLOW;
			}
			outp32(hisifd->dss_base + DSS_LDI_DP_OFFSET + LDI_CPU_ITF_INT_MSK, tmp);

			tmp = inp32(hisifd->dss_base + DSS_LDI_DP1_OFFSET + LDI_CPU_ITF_INT_MSK);
			if (unmask) {
				tmp &= ~ BIT_LDI_UNFLOW;
			} else {
				tmp |= BIT_LDI_UNFLOW;
			}
			outp32(hisifd->dss_base + DSS_LDI_DP1_OFFSET + LDI_CPU_ITF_INT_MSK, tmp);
		} else {
			unflow_mipi_ldi_int_handler(hisifd->mipi_dsi1_base, unmask);
		}
	} else {
...
int hisi_overlay_on(struct hisi_fb_data_type *hisifd, bool fastboot_enable)
{
	...
	if (g_dss_module_resource_initialized == 0) {
		...
		if (!hisifd->dss_module_resource_initialized) {
			if (hisifd->index == MEDIACOMMON_PANEL_IDX) {
			#if defined(CONFIG_HISI_FB_970) || defined(CONFIG_HISI_FB_V501) || defined(CONFIG_HISI_FB_V510) || defined(CONFIG_HISI_FB_V600)
				hisi_dss_mdc_module_default(hisifd);
			#endif
			} else {
...
#if defined(CONFIG_HISI_FB_V501) || defined (CONFIG_HISI_FB_V330) || defined (CONFIG_HISI_FB_V510) || defined (CONFIG_HISI_FB_V350)
		/*for gmp lut set*/
...
/*lint -e436 -e438 -e527 -e666 -e648 -e737 -e570*/
