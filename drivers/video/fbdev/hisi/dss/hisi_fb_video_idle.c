...
#define SC_AVAILABLE_SIZE (0x380000)
...
static void hisifb_video_idle_init(struct hisi_fb_data_type *hisifd)
{
	...
	video_idle_ctrl->gpu_compose_idle_frame = true;
#else
	video_idle_ctrl->gpu_compose_idle_frame = false;
#endif
	...
	video_idle_ctrl->sc_access = true;//syscache access
	video_idle_ctrl->policy_id = ION_LB_IDISPLAY;/*4*/
	/* FIXME:l3&lb size need reconfirm */
	video_idle_ctrl->l3cache_size = L3CACHE_AVAILABLE_SIZE;
	video_idle_ctrl->lb_size = SC_AVAILABLE_SIZE;
#else
	video_idle_ctrl->sc_access = false;
	video_idle_ctrl->l3cache_size = 0x0;
	video_idle_ctrl->lb_size = 0x0;
#endif
	...
	(struct hisifb_video_idle_ctrl *video_idle_ctrl)
{
	...
	video_idle_ctrl->request_params.request_size =
		video_idle_ctrl->l3cache_size / 0x100000; // 3M
...
static void hisifb_video_idle_buffer_alloc(struct hisi_fb_data_type *hisifd, uint32_t req_size)
{
	...
	if (vic->sc_access) {
		...
		if (lb_pages_attach(vic->policy_id, phys_to_page(vic->wb_phys_addr), vic->wb_buffer_size / PAGE_SIZE)) {
	...
	//HISI_FB_INFO("pa= 0x%x, va= 0x%x\n", vic->wb_phys_addr, vic->wb_vir_addr);
...
l3req_exit:
	lb_pages_detach(vic->policy_id, phys_to_page(vic->wb_phys_addr), vic->wb_buffer_size / PAGE_SIZE);
attach_exit:
...
void hisifb_video_idle_buffer_free(struct hisi_fb_data_type *hisifd)
{
	...
	if (vic->sc_access) {
		//hisi_iommu_idle_display_unmap(__hdss_get_dev(), vic->wb_vir_addr, vic->wb_buffer_size);
...
static void hisifb_video_idle_config_dpp_regs(struct hisi_fb_data_type *hisifd)
{
	...
	dss_base = hisifd->dss_base;
	dsc_en = inp32(dss_base + DSS_DSC_OFFSET + DSC_EN);
	dpp_clip_en = inp32(dss_base + DSS_DPP_CLIP_OFFSET + DPP_CLIP_EN);
	dither_ctl0 = inp32(dss_base + DSS_DPP_DITHER_OFFSET + DITHER_CTL0);
	gama_en = inp32(dss_base + DSS_DPP_GAMA_OFFSET + GAMA_EN);
	xcc_en = inp32(dss_base + DSS_DPP_XCC_OFFSET + XCC_EN);
	degama_en = inp32(dss_base + DSS_DPP_DEGAMMA_OFFSET + DEGAMA_EN);
	gmp_en = inp32(dss_base + DSS_DPP_GMP_OFFSET + GMP_EN);
	arsr_post_bypass = inp32(dss_base + DSS_POST_SCF_OFFSET + ARSR_POST_MODE);
	hiace_bypass = inp32(dss_base + DSS_HI_ACE_OFFSET + DPE_BYPASS_ACE);
	nr_bypass = inp32(dss_base + DSS_HI_ACE_OFFSET + DPE_BYPASS_NR);
	...
	HISI_FB_DEBUG("fb%d, dsc_en[0x7dc08]=0x%x, dpp_clip_en[0x70190]=0x%x, dither_ctl0[0x70204]=0x%x\n"
		...
		ifbc_en, hiace_bypass, nr_bypass);
	...
	hisifd->set_reg(hisifd, dss_base + DSS_DPP_CLIP_OFFSET + DPP_CLIP_EN, dpp_clip_en, 32, 0);
	...
	arsr_post_bypass = set_bits32(arsr_post_bypass, 0x1, 1, 0);
	hisifd->set_reg(hisifd, dss_base + DSS_POST_SCF_OFFSET + ARSR_POST_MODE, arsr_post_bypass, 32, 0);
...
static int hisifb_video_idle_alloc_cache(struct hisi_fb_data_type *hisifd)
{
	...
	if (hisifd->video_idle_ctrl.buffer_alloced) {
		HISI_FB_DEBUG("fb%d buffer alloced\n", hisifd->index);
		return 0;
	}

	layer = get_video_idle_layer_info(hisifd);
	if (layer == NULL) {
		HISI_FB_INFO("get_video_idle_layer_info fail\n");
		return 0;
	}

	buf = hisi_dss_get_dmabuf(layer->img.shared_fd);
	attach = dma_buf_attach(buf, __hdss_get_dev());
	...
	if (hisifd->video_idle_ctrl.sc_access) {
		nents = sg_nents(table->sgl);

		HISI_FB_DEBUG("lb_sg_attach nents=%d, length=0x%x, "
			"buffer_size=0x%x\n",
			nents, table->sgl->length, layer->img.buf_size);

		if (lb_sg_attach(hisifd->video_idle_ctrl.policy_id,
			table->sgl, nents)) {
		...
		hisifd->video_idle_ctrl.lb_size =
			layer->img.buf_size - hisifd->video_idle_ctrl.l3cache_size;

		hisifd->video_idle_ctrl.wb_vir_addr =
			hisi_iommu_idle_display_sg_map(__hdss_get_dev(),
			hisifd->video_idle_ctrl.policy_id, table->sgl, layer->img.buf_size,
	...
	layer->img.afbc_header_addr =
		hisifd->video_idle_ctrl.wb_vir_addr + layer->img.afbc_header_offset;
	layer->img.afbc_payload_addr =
		hisifd->video_idle_ctrl.wb_vir_addr + layer->img.afbc_payload_offset;

	HISI_FB_DEBUG("vir_addr=0x%lx, header=0x%lx, "
		"payload=0x%lx, size-all=0x%x, l3=0x%x, lb=0x%x\n",
		layer->img.vir_addr, layer->img.afbc_header_addr,
		layer->img.afbc_payload_addr, layer->img.buf_size,
		hisifd->video_idle_ctrl.l3cache_size, hisifd->video_idle_ctrl.lb_size);
...
l3cache_request_fail:
	lb_sg_detach(hisifd->video_idle_ctrl.policy_id,
...
{
	int ret = 0;
	static uint32_t last_frm_num = 0;
...
int hisifb_video_panel_idle_display_ctrl(struct hisi_fb_data_type *hisifd, uint32_t video_idle_status)
{
	...
	if (!g_enable_video_idle_l3cache && !vic->buffer_alloced) {
		HISI_FB_INFO("disable cache idle display");
		return 0;
	}
