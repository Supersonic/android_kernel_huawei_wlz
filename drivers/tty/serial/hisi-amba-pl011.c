...
int pl011_tx_stat_show(struct seq_file *s, void *arg)
{
	...
	seq_printf(s, "fifo capacity : 0x%lx(%ldk)\n",
		...
		(unsigned long)uart_tx_buf_size / 1024);
	seq_printf(s, "fifo required : 0x%lx(%ldk)\n", unit->tx_buf_max,
		unit->tx_buf_max / 1024);
	...
	struct uart_amba_port *uap, unsigned int aurt_tx_buf_size, int cpuid)
{
	...
	ret = kfifo_alloc(
		&unit->tx_fifo, aurt_tx_buf_size, GFP_KERNEL | __GFP_ZERO);
	if (ret) {
	...
	snprintf_s(dbgfs_name, sizeof(dbgfs_name), sizeof(dbgfs_name) - 1,
		"uart%d_stat", uap->port.line);
#ifdef CONFIG_HISI_DEBUG_FS
	debugfs_create_file(
		dbgfs_name, 0444, NULL, unit, &pl011_tx_stat_ops);
#endif
	return 0;
}
	...
	struct amba_device *dev, struct uart_amba_port *uap)
{
	...
	if (ret || (node == NULL)) {
		uap->reset = false;
		dev_info(&dev->dev,
		"can't get reset-enable-flag property. reset disable.\n");
	} else {
		if (flag_reset_enable) {
			/* get uart reset-base regester */
			ret = of_property_read_u32_array(dev->dev.of_node,
				"reset-reg-base", &data[0], 4);
			if (ret) {
				uap->reset_reg_base = NULL;
				uap->reset = false;
				dev_info(&dev->dev,
					"can't get reset-reg-base property.\n");
			} else {
			...
			ret = of_property_read_u32_array(dev->dev.of_node,
				"reset-controller-reg", &data[0], 4);
			if (ret) {
				uap->reset = false;
				dev_info(&dev->dev,
					"can't get reset-controller-reg.\n");
			} else {
	...
	struct uart_amba_port *uap, const char *amba_console_name)
{
	...
	if (get_console_name(console_uart_name,
		sizeof(console_uart_name)) == 0) {
		if (strncmp(console_uart_name, amba_console_name, 8) == 0)
	...
	if (console_uart_name_is_ttyAMA &&
		...
		if (console_fifo_enable_status == 1) {
			...
			pl011_tx_work_init(uap, console_fifo_size,
				console_fifo_cpuon);
			return;
		}
		/* get console fifo enable flag */
		if (of_property_read_u32(dev->dev.of_node,
			"console-fifo-enable", &uap->tx_unit.tx_valid)) {
			uap->tx_unit.tx_valid = 0;
			dev_info(&dev->dev,
				"don't have console-fifo-enable property!\n");
		} else {
			if (uap->tx_unit.tx_valid) {
				dev_info(&dev->dev,
					"enable serial console fifo!\n");
				...
				if (of_property_read_u32(
					...
					dev_err(&dev->dev,
						"%s don't have console-fifo-size property! set 0x%x as default.\n",
						__func__, UART_TX_BUF_SIZE_DEF);
				}
				/* get console fifo cpuon */
				if (of_property_read_u32(dev->dev.of_node,
						"console-fifo-cpuon",
						(u32 *)&console_fifo_cpuon)) {
					console_fifo_cpuon =
						UART_TX_CPUON_NOTSET;
					dev_err(&dev->dev,
						"%s don't have console-fifo-cpuon property! set fifo on cpu disabled.\n",
						__func__);
				} else {
					dev_info(&dev->dev,
						"%s set fifo on cpu:%d.\n",
				...
				pl011_tx_work_init(uap,
					console_fifo_size,
					console_fifo_cpuon);
			} else {
				dev_info(&dev->dev,
					"%s:disable serial console fifo!\n",
					__func__);
			}
		}
	}
