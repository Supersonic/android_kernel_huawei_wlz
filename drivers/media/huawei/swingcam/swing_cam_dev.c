...
void swing_cam_init_timestamp(void)
{
...
void swing_cam_destroy_timestamp(void)
{
...
void swing_cam_set_timestamp(u32 *timestampH, u32 *timestampL)
{
...
static int swing_cam_get_resp(const pkt_header_t *head)
{
	...
	case SUB_CMD_SWING_CAM_MATCH_ID:
		swing_cam_log_info("Received, sensor name:%s, status:%d\n",
			p_msg_ack->u.ack_query_swing_cam.name,
...
static int swing_v4l2_ioctl_cam_acquire(void *arg)
{
	...
	swing_cam_log_info("i2c_index:%d, i2c_bus_type:%s, sensor_name:%s\n",
			acqReq->i2c_index, acqReq->i2c_bus_type, acqReq->sensor_name);
...
static int swing_v4l2_ioctl_cam_match_id(void *arg)
{
	...
	swing_cam_log_info("product:%s, sensor:%s, pos:%d, i2c:%d, csi:%d\n",
			matchReq->product_name, matchReq->name,
...
static int swing_v4l2_ioctl_get_otp(void *arg)
{
	...
	if (otpReq != NULL && otpReq->sensor_name != NULL) {
		swing_cam_log_info("%s sensor_name=%s\n",
			__func__, otpReq->sensor_name);
	} else {
		swing_cam_log_err("%s sensor_name is nul\n", __func__);
		return -EFAULT;
	}
