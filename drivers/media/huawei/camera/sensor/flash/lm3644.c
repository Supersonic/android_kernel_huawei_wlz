...
#endif
/* Function define */
extern int snprintf_s(char *strDest, size_t destMax, size_t count,
		const char *format, ...);
		...
		struct hw_flash_cfg_data *cdata)
{
	...
	if (pdata->flash_current == FLASH_LED_LEVEL_INVALID) {
		...
		data_level = (pdata->flash_type == 1) ? (cdata->data) : (cdata->data / 2);
	...
	val_led1 = (pdata->flash_type == 1) ? 0 : (current_level & 0x7f);
	val_led2 = (current_level & 0x7f);
	...
	int data)
{
	...
	if (pdata->torch_current == FLASH_LED_LEVEL_INVALID) {
		...
		current_level = hw_find_match_torch_current(data_level, pdata->driver_ic_type);
		if (current_level < 0)
			current_level = LM3644_TORCH_DEFAULT_CUR_LEV;
	}
	...
	loge_if_ret(i2c_func->i2c_write(i2c_client, REG_L1_TORCH, val_led1) < 0);
	loge_if_ret(i2c_func->i2c_write(i2c_client, REG_L2_TORCH, val_led2) < 0);
	if (pdata->flash_type == 1)
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE-1,
			 "mode=%d, data=%d\n", hw_lm3644_ctrl.state.mode, hw_lm3644_ctrl.state.mode);
	if (ret < 0) {
		cam_err("%s(%d)::snprintf_s return error(%d)", __func__, __LINE__, ret);
	}
	rc = strlen(buf) + 1;
	return rc;
}
	...
	enum led_brightness brightness)
{
	...
	if (led_bright == STANDBY_MODE) {
		...
		cdata.data = MMI_TEST_BRIGHTNESS; /* cmp lm3642, just for mmi */
		if (g_flash_type == 1) {
			...
			rc = hw_lm3644_torch_mode_mmi(&hw_lm3644_ctrl, cdata.mode);
		}
		if (rc < 0) {
...
static int hw_lm3644_match(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	case KTD2687_CHIP_ID:
		...
		break;
	default:
		cam_info("%s match error.id = 0x%x\n", __func__, id);
		return -1;
	}
	cam_info("%s match ok,driver type = 0x%x\n", __func__, pdata->driver_ic_type);
