...
struct dsm_client *client_flash = NULL;
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if ('0' == buf[0]) {
        ...
        cam_info("%s flash mode=%d.", __func__, flash_ctrl->state.mode);
...
int hw_flash_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	rc = of_property_read_u32(dev_node, "huawei,flash-classtype",
		(u32 *)&flash_info->classtype);
	cam_info("%s huawei,flash-classtype %d, rc %d\n", __func__,
...
int hw_flash_config(struct hw_flash_ctrl_t *flash_ctrl, void *arg)
{
	...
	case CFG_FLASH_GET_FLASH_TYPE:
		...
		mutex_unlock(flash_ctrl->hw_flash_mutex);
		cam_info("%s flash class type =%d.", __func__, cdata->data);
		break;
