...
#define FLASH_TIMEOUT_TIME                  0x0A /* 600ms */
...
#define LM3644TT_LED1_BASE_CUR_OF_FLASH       (2304)//*100
#define LM3644TT_LED1_STEP_CUR_OF_FLASH       (1172)//*100
#define LM3644TT_LED1_BASE_CUR_OF_TORCH       (253) //*100
#define LM3644TT_LED1_STEP_CUR_OF_TORCH       (146) //*100
#define LM3644TT_STANDBY_MODE_CUR             (0)
#define REG_MASK_0XFF                       (0xFF)
/* 1 means I2C control, 0 means GPIO control. */
#define LM3644TT_CTRL_TYPE_GPIO               (0)
#define LM3644TT_CTRL_TYPE_I2C                (1)
//#define TORCH_DISABLE                       0x00
#define TX_ENABLE                           0x08
#define ENABLE_SHORT_PROTECT                0x80
#define LM3644TT_FLASH_LED_VAL_MASK          (0x7F)
#define LM3644TT_TORCH_LED_VAL_MASK          (0x7F)
#define BACK_FLASH_USE_LED2                 (1)
#define LM3644TT_TORCH_MAX_VAL_SHIFT          (4)
#define LM3644TT_FLASH_MAX_VAL_MASK           (0x0F)//(reg 0x05)max flash cur mask
#define LM3644TT_TORCH_MAX_VAL_MASK           (0x70)//(reg 0x05)max torch cur mask
...
static int hw_lm3644tt_mix_ir_set_pin(struct hw_flash_ctrl_t   *flash_ctrl,
                                    lm3644tt_mix_ir_pin_type_t  pin_type,
                                    int state)
{
...
static int gpio_request_assemble(struct hw_lm3644tt_mix_ir_private_data_t *pdata)
{
    int i  = 0;
    int rc = 0;
    const char* gpio_name[MAX_PIN]= {"flash-en", "flash-strobe", "flash-torch"};
...
static int hw_lm3644tt_mix_ir_flash_step_match(unsigned int cur, unsigned int *level)
{
    int i = 0;
...
static int hw_lm3644tt_mix_ir_flash_led_step_calculate(unsigned int cur, unsigned int *level)
{
    RETURN_ERROR_ON_NULL(level);

    *level = 0;
    if (cur * COUNT_BASE < LM3644TT_LED1_BASE_CUR_OF_FLASH) {
        *level = 0;//level 0, cur is 0
    } else {
        *level = (cur * 100 - LM3644TT_LED1_BASE_CUR_OF_FLASH)/LM3644TT_LED1_STEP_CUR_OF_FLASH + 1;//idx from 1
    }
                                                     ...
                                                     struct hw_lm3644tt_mix_ir_private_data_t *pdata)
{
    ...
    cam_info("%s led_type=%d.", __func__, pdata->led_type);
    ...
    if ( LM3644TT_STANDBY_MODE_CUR == led2_cur ) {
        *reg_mode = LED1_EN | MODE_FLASH;
		*led1_val = FLASH_CURRENT_TO_LEVEL_TI(led1_cur);
		*led2_val = FLASH_CURRENT_TO_LEVEL_TI(led1_cur) & LM3644TT_FLASH_LED_VAL_MASK;
    } else {
        *reg_mode = LED2_EN | MODE_FLASH;
		*led1_val = FLASH_CURRENT_TO_LEVEL_TI(led2_cur) & LM3644TT_FLASH_LED_VAL_MASK;
		*led2_val = FLASH_CURRENT_TO_LEVEL_TI(led2_cur);
    }

    if ( LM3644TT_CTRL_TYPE_I2C == pdata->ctrltype ) {
        *led1_val = *led1_val | STROBE_DISABLE;
        *led2_val = *led2_val | STROBE_DISABLE;
    } else {
        *led1_val = *led1_val | STROBE_ENABLE;
        *led2_val = *led2_val | STROBE_ENABLE;
    }

    return 0;
}

static int hw_lm3644tt_mix_ir_torch_step_match(unsigned int cur, unsigned int *level)
{
    int i = 0;
    RETURN_ERROR_ON_NULL(level);
...
static int hw_lm3644tt_mix_ir_torch_led_step_calculate(unsigned int cur, unsigned int *level)

{
    RETURN_ERROR_ON_NULL(level);

    *level = 0;
    if (cur * COUNT_BASE < LM3644TT_LED1_BASE_CUR_OF_TORCH) {
        *level = 0;
    } else {
        *level = (cur*100 - LM3644TT_LED1_BASE_CUR_OF_TORCH)/LM3644TT_LED1_STEP_CUR_OF_TORCH + 1;
    }
                                                     ...
                                                     struct hw_lm3644tt_mix_ir_private_data_t *pdata)
{
    ...
    unsigned int led1_level = 0;
    unsigned int total_level = 0;
    unsigned char reg_mode_val_tmp  = 0;
    int rc = 0;
    ...
    if (LM3644TT_STANDBY_MODE_CUR == led2_cur) {
        ...
        * LED2 = Off
        */
        *reg_mode = LED1_EN | MODE_TORCH;
        *led1_val = TORCH_CURRENT_TO_LEVEL_TI(led1_cur);
        *led2_val = TORCH_CURRENT_TO_LEVEL_TI(led1_cur) & LM3644TT_TORCH_LED_VAL_MASK;
    } else {
        /**
         * only LED2 active:
         * LED1 = Off
         * LED2 = LED2 Torch Current Control Value
         */
        *reg_mode = LED2_EN | MODE_TORCH;
        *led1_val = TORCH_CURRENT_TO_LEVEL_TI(led1_cur) & LM3644TT_TORCH_LED_VAL_MASK;
        *led2_val = TORCH_CURRENT_TO_LEVEL_TI(led2_cur);
    }
...
static int hw_lm3644tt_mix_ir_action_ic_flash_mode(struct hw_flash_ctrl_t *flash_ctrl)
{
    ...
    rc = hw_lm3644tt_mix_ir_flash_mode_cfg_calculate(&led1_val, &led2_val, &reg_mode, pdata);
    if (rc < 0) {
    ...
    hw_lm3644tt_mix_ir_set_pin(flash_ctrl, STROBE, HIGH);
                                             ...
                                             struct hw_flash_cfg_data *cdata)
{
    ...
    rc = hw_lm3644tt_mix_ir_update_work_mode_dispatch(flash_ctrl,ic_work_mode);
    if (rc < 0) {
...
static ssize_t hw_lm3644tt_mix_ir_flash_lightness_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t hw_lm3644tt_mix_ir_flash_lightness_f_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
__ATTR(flash_lightness, 0660, hw_lm3644tt_mix_ir_flash_lightness_show, hw_lm3644tt_mix_ir_flash_lightness_store);//660:-wr-wr---

static struct device_attribute hw_lm3644tt_mix_ir_flash_lightness_f =
__ATTR(flash_lightness_f, 0660, hw_lm3644tt_mix_ir_flash_lightness_show_f, hw_lm3644tt_mix_ir_flash_lightness_f_store);//660:-wr-wr---

static struct device_attribute hw_lm3644tt_mix_ir_flash_mask =
__ATTR(flash_mask, 0660, hw_lm3644tt_mix_ir_flash_mask_show, hw_lm3644tt_mix_ir_flash_mask_store);//660:-wr-wr---
...
static int hw_lm3644tt_mix_ir_match(struct hw_flash_ctrl_t *flash_ctrl)
{
    ...
    if ( (id != 0x02) && ((id != LM3644TT_CHIP_ID)) ) {
        cam_err("%s match error, id(0x%x) != 0x%x.",
            __func__, id, LM3644TT_CHIP_ID);
    ...
    cam_info("%s mode=0x%x. ID(0x02 is AW or KTD) MODE(0x08 is AW or LM; No val is KTD).\n", __func__, mode);
