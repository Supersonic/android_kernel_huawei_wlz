...
/* AW3642_FRONT Registers define */
#define AW3642_FRONT_SLAVE_ADDRESS      (0x63)
...
#define LED_EN                          (0x03)
#define AW3642_FRONT_FLASH_DEFAULT_CUR_LEV    (31) //754mA
#define AW3642_FRONT_TORCH_DEFAULT_CUR_LEV    (23) //141mA
#define AW3642_FRONT_FLASH_MAX_CUR            (1500)//1500mA
#define AW3642_FRONT_TORCH_MAX_CUR            (376)
#define AW3642_FRONT_TORCH_MAX_RT_CUR         (190)//190mA
#define AW3642_FRONT_TORCH_MAX_MMI_CUR        (48)//48mA
#define AW3642_FRONT_TORCH_MAX_LEV            (63)
#define FLASH_LED_MAX                         (64)
#define TORCH_LED_MAX                         (64)
#define FLASH_LED_LEVEL_INVALID               (0xff)
#define AW3642_FRONT_UNDER_VOLTAGE_LOCKOUT    (0x02)
#define AW3642_FRONT_OVER_VOLTAGE_PROTECT     (0x02)
#define AW3642_FRONT_LED_SHORT                (0x10)
#define AW3642_FRONT_VOUT_SHORT               (0x40)
#define AW3642_FRONT_OVER_TEMP_PROTECT        (0x04)
#define PARAM_LEN                             (2)

/* Internal data struct define */
struct hw_aw3642_front_private_data_t {
...
static struct hw_aw3642_front_private_data_t hw_aw3642_front_pdata;
static struct hw_flash_ctrl_t hw_aw3642_front_ctrl;
static struct i2c_driver hw_aw3642_front_i2c_driver;
extern struct dsm_client *client_flash;

DEFINE_HISI_FLASH_MUTEX(aw3642_front);
#ifdef CAMERA_FLASH_FACTORY_TEST
extern int register_camerafs_attr(struct device_attribute *attr);
#endif

static int hw_aw3642_front_clear_error_and_notify_dmd(struct hw_flash_ctrl_t *flash_ctrl)
{
    ...
    if (ocp_reg_val & AW3642_FRONT_OVER_TEMP_PROTECT) {
        if(!dsm_client_ocuppy(client_flash)) {
    ...
    if (ocp_reg_val & (AW3642_FRONT_LED_SHORT | AW3642_FRONT_VOUT_SHORT)) {
        if(!dsm_client_ocuppy(client_flash)) {
    ...
    if (ovp_reg_val & AW3642_FRONT_OVER_VOLTAGE_PROTECT) {
        if(!dsm_client_ocuppy(client_flash)) {
    ...
    if (ocp_reg_val & AW3642_FRONT_UNDER_VOLTAGE_LOCKOUT) {
        if(!dsm_client_ocuppy(client_flash)) {
...
static int hw_aw3642_front_find_match_flash_current(int cur_flash)
{
    int cur_level = 0;
    int i = 0;
    ...
    if(cur_flash >= AW3642_FRONT_FLASH_MAX_CUR){
        cam_warn("%s current set is %d", __func__, cur_flash);
        return AW3642_FRONT_FLASH_DEFAULT_CUR_LEV;
    }
...
static int hw_aw3642_front_find_match_torch_current(int cur_torch)
{
    int cur_level = 0;
    int i = 0;
    ...
    if(cur_torch > AW3642_FRONT_TORCH_MAX_CUR){
        cam_warn("%s current set is %d", __func__, cur_torch);
        return AW3642_FRONT_TORCH_MAX_LEV;
    }
...
static int hw_aw3642_front_init(struct hw_flash_ctrl_t *flash_ctrl)
{
...
static int hw_aw3642_front_exit(struct hw_flash_ctrl_t *flash_ctrl)
{
...
static int hw_aw3642_front_flash_mode(struct hw_flash_ctrl_t *flash_ctrl,
    int data)
{
    struct hw_flash_i2c_client *i2c_client;
    struct hw_flash_i2c_fn_t *i2c_func;
    struct hw_aw3642_front_private_data_t *pdata;
    unsigned char val;
    int current_level = 0;
    int rc = 0;
    ...
    {
        current_level = AW3642_FRONT_FLASH_DEFAULT_CUR_LEV;
    }
    else
    {
        current_level = hw_aw3642_front_find_match_flash_current(data);
        if(current_level < 0){
             current_level = AW3642_FRONT_FLASH_DEFAULT_CUR_LEV;
        }
    }

    rc = hw_aw3642_front_clear_error_and_notify_dmd(flash_ctrl);
    if(rc < 0)
    {
...
static int hw_aw3642_front_torch_mode(struct hw_flash_ctrl_t *flash_ctrl,
    int data)
{
    struct hw_flash_i2c_client *i2c_client;
    struct hw_flash_i2c_fn_t *i2c_func;
    ...
    i2c_client = flash_ctrl->flash_i2c_client;
    i2c_func = flash_ctrl->flash_i2c_client->i2c_func_tbl;
    pdata = (struct hw_aw3642_front_private_data_t *)flash_ctrl->pdata;
    if (FLASH_LED_LEVEL_INVALID == pdata->torch_current)
    {
        current_level = AW3642_FRONT_TORCH_DEFAULT_CUR_LEV;
    }
    else
    {
        current_level = hw_aw3642_front_find_match_torch_current(data);
        if(current_level < 0){
              current_level = AW3642_FRONT_TORCH_DEFAULT_CUR_LEV;
        }
    }
...
static int hw_aw3642_front_on(struct hw_flash_ctrl_t *flash_ctrl, void *data)
{
    ...
    if ((FLASH_MODE == cdata->mode) || (FLASH_STROBE_MODE == cdata->mode)) {
        rc = hw_aw3642_front_flash_mode(flash_ctrl, cdata->data);
    } else {
        rc = hw_aw3642_front_torch_mode(flash_ctrl, cdata->data);
    }
...
static int hw_aw3642_front_off(struct hw_flash_ctrl_t *flash_ctrl)
{
    struct hw_flash_i2c_client *i2c_client;
    struct hw_flash_i2c_fn_t *i2c_func;
...
static int hw_aw3642_front_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
    struct hw_aw3642_front_private_data_t *pdata;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
static ssize_t hw_aw3642_front_lightness_show(struct device *dev,
    struct device_attribute *attr,char *buf)
{
    ...
    hw_aw3642_front_ctrl.state.mode, hw_aw3642_front_ctrl.state.data);
    rc = strlen(buf)+1;
    return rc;
}

static int hw_aw3642_front_param_check(char *buf, unsigned long *param,
    int num_of_par)
{
...
static ssize_t hw_aw3642_front_lightness_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    rc = hw_aw3642_front_param_check((char *)buf, param, PARAM_LEN);
    if (rc < 0) {
    ...
    if (cdata.mode == STANDBY_MODE) {
        rc = hw_aw3642_front_off(&hw_aw3642_front_ctrl);
        if (rc < 0) {
            cam_err("%s aw3642 flash off error.", __func__);
            return rc;
        }
    } else if(cdata.mode == TORCH_MODE){
        cdata.data = AW3642_FRONT_TORCH_MAX_RT_CUR;//hardware test requiring the max torch mode current
        cam_info("%s mode=%d, max_current=%d.", __func__, cdata.mode, cdata.data);

        rc = hw_aw3642_front_on(&hw_aw3642_front_ctrl, &cdata);
        if (rc < 0) {
...
static ssize_t hw_aw3642_front_flash_mask_show(struct device *dev,
    struct device_attribute *attr,char *buf)
{
    int rc=0;
    if ((NULL == dev) || (NULL == attr) || (NULL == buf))
    ...
    snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE-1, "flash_mask_disabled=%d.\n", hw_aw3642_front_ctrl.flash_mask_enable);
    rc = strlen(buf)+1;
    return rc;
}

static ssize_t hw_aw3642_front_flash_mask_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    if ((NULL == dev) || (NULL == attr) || (NULL == buf))
    {
    ...
    if ('0' == buf[0]) {
        hw_aw3642_front_ctrl.flash_mask_enable = false;
    } else {
        hw_aw3642_front_ctrl.flash_mask_enable = true;
    }
...
static void hw_aw3642_front_torch_brightness_set(struct led_classdev *cdev,
    enum led_brightness brightness)
{
    ...
    if (STANDBY_MODE == led_bright) {
        ...
        cdata.data = AW3642_FRONT_TORCH_MAX_MMI_CUR;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
static struct device_attribute hw_aw3642_front_lightness =
    __ATTR(flash_lightness_f, 0660, hw_aw3642_front_lightness_show, hw_aw3642_front_lightness_store);
#endif

static struct device_attribute hw_aw3642_front_flash_mask =
    __ATTR(flash_mask, 0660, hw_aw3642_front_flash_mask_show, hw_aw3642_front_flash_mask_store);

static int hw_aw3642_front_register_attribute(struct hw_flash_ctrl_t *flash_ctrl, struct device *dev)
{
    int rc = 0;
    ...
    flash_ctrl->cdev_torch.max_brightness
        = ((struct hw_aw3642_front_private_data_t *)(flash_ctrl->pdata))->torch_led_num;
    flash_ctrl->cdev_torch.brightness_set = hw_aw3642_front_torch_brightness_set;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
    rc = device_create_file(dev, &hw_aw3642_front_lightness);
    if (rc < 0) {
        cam_err("%s failed to creat lightness attribute.", __func__);
        goto err_create_lightness_file;
    }
#endif
    rc = device_create_file(dev, &hw_aw3642_front_flash_mask);
    if (rc < 0) {
...
#ifdef CAMERA_FLASH_FACTORY_TEST
    device_remove_file(dev, &hw_aw3642_front_lightness);
#endif
...
static int hw_aw3642_front_match(struct hw_flash_ctrl_t *flash_ctrl)
{
    struct hw_flash_i2c_client *i2c_client;
    struct hw_flash_i2c_fn_t *i2c_func;
    struct hw_aw3642_front_private_data_t *pdata;
    ...
    i2c_client->client->addr = AW3642_FRONT_SLAVE_ADDRESS;
    i2c_func = flash_ctrl->flash_i2c_client->i2c_func_tbl;
    pdata = (struct hw_aw3642_front_private_data_t *)flash_ctrl->pdata;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
    register_camerafs_attr(&hw_aw3642_front_lightness);
#endif
    return 0;
}

static int hw_aw3642_front_remove(struct i2c_client *client)
{
    ...
    hw_aw3642_front_ctrl.func_tbl->flash_exit(&hw_aw3642_front_ctrl);

    client->adapter = NULL;
    return 0;
}

static void hw_aw3642_front_shutdown(struct i2c_client *client)
{
    int rc = -1;
    if (NULL == client) {
        cam_err("%s invalid param", __func__);
...
static const struct i2c_device_id hw_aw3642_front_id[] = {
    {"aw3642_front", (unsigned long)&hw_aw3642_front_ctrl},
    {}
};

static const struct of_device_id hw_aw3642_front_dt_match[] = {
    {.compatible = "huawei,aw3642_front"},
    {}
};
MODULE_DEVICE_TABLE(of, aw3642_front_dt_match);

static struct i2c_driver hw_aw3642_front_i2c_driver = {
    .probe = hw_flash_i2c_probe,
    .remove = hw_aw3642_front_remove,
    .shutdown = hw_aw3642_front_shutdown,
    .id_table = hw_aw3642_front_id,
    .driver = {
        .name = "hw_aw3642_front",
        .of_match_table = hw_aw3642_front_dt_match,
    },
};

static int __init hw_aw3642_front_module_init(void)
{
    cam_info("%s erter.\n", __func__);
    return i2c_add_driver(&hw_aw3642_front_i2c_driver);
}

static void __exit hw_aw3642_front_module_exit(void)
{
    cam_info("%s enter.", __func__);
    i2c_del_driver(&hw_aw3642_front_i2c_driver);
    return;
}

static struct hw_flash_i2c_client hw_aw3642_front_i2c_client;

static struct hw_flash_fn_t hw_aw3642_front_func_tbl = {
    .flash_config = hw_flash_config,
    .flash_init = hw_aw3642_front_init,
    .flash_exit = hw_aw3642_front_exit,
    .flash_on = hw_aw3642_front_on,
    .flash_off = hw_aw3642_front_off,
    .flash_match = hw_aw3642_front_match,
    .flash_get_dt_data = hw_aw3642_front_get_dt_data,
    .flash_register_attribute = hw_aw3642_front_register_attribute,
};

static struct hw_flash_ctrl_t hw_aw3642_front_ctrl = {
    .flash_i2c_client = &hw_aw3642_front_i2c_client,
    .func_tbl = &hw_aw3642_front_func_tbl,
    .hw_flash_mutex = &flash_mut_aw3642_front,
    .pdata = (void*)&hw_aw3642_front_pdata,
...
module_init(hw_aw3642_front_module_init);
module_exit(hw_aw3642_front_module_exit);
MODULE_DESCRIPTION("AW3642_FRONT FLASH");
