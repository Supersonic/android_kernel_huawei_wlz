...
int mclk_config(sensor_t *s_ctrl, unsigned int id, unsigned int clk, int on)
{
	...
	if (fsnclk0) {
		...
		if (ret) {
			dev_err(dev, "cloud not enalbe clk_isp_snclk0\n");
			return ret;
		}
	} else if (fsnclk1) {
		...
		if (ret) {
			dev_err(dev, "cloud not enalbe clk_isp_snclk1\n");
			return ret;
		}
	} else if (fSnclk2) {
		...
		if (ret) {
			dev_err(dev, "cloud not enalbe clk_isp_snclk2\n");
			return ret;
		}
	}
	return 0;
	...
	struct sensor_power_setting *power_setting, int on_off)
{
	...
	case SENSOR_CHECK_LEVEL:
		...
		break;
	case SENSOR_RST:
		cam_info("%s, seq_type:%u SENSOR_RST",
			__func__, power_setting->seq_type);
	...
	case SENSOR_BOOT_5V:
		cam_info("%s, seq_type:%u SENSOR_BOOT_5V",
			__func__, power_setting->seq_type);
		if (on_off == POWER_ON)
			rc = boost_5v_enable(BOOST_5V_ENABLE,
				BOOST_CTRL_CAMERA);
		else if (on_off == POWER_OFF)
			rc = boost_5v_enable(BOOST_5V_DISABLE,
				BOOST_CTRL_CAMERA);
		break;
	default:
