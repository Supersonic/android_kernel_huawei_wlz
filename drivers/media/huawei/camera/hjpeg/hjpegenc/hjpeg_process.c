...
static int is_prefetch_bypass = PREFETCH_BYPASS_NO;   // default open prefetch mechanism
...
static int __check_buffer_vaild(int share_fd, unsigned int req_addr, unsigned int req_size)
{
    ...
    if (req_addr != fmt.iova || req_size > fmt.size) {
		cam_err("%s: fd:%d, iova:%#llx, size:%#llx", __func__, share_fd, fmt.iova, fmt.size);
        ret = -ERANGE;
    }
...
void dump_reg(void)
{
	...
	if (s_hjpeg.hw_ctl.cvdr_prop.wr_port == WR_PORT_25) {
	...
	} else if (s_hjpeg.hw_ctl.cvdr_prop.wr_port == WR_PORT_30) {
		cam_debug("%s: CVDR reg: 0x%08x=0x%08x", __func__,
			CVDR_AXI_JPEG_VP_WR_CFG_30_OFFSET,
			get_reg_val((void __iomem*)((char*)viraddr + CVDR_AXI_JPEG_VP_WR_CFG_30_OFFSET)));
		cam_debug("%s: CVDR reg: 0x%08x=0x%08x", __func__,
			CVDR_AXI_JPEG_VP_WR_AXI_FS_30_OFFSET,
			get_reg_val((void __iomem*)((char*)viraddr + CVDR_AXI_JPEG_VP_WR_AXI_FS_30_OFFSET)));
		cam_debug("%s: CVDR reg: 0x%08x=0x%08x", __func__,
			CVDR_AXI_JPEG_VP_WR_AXI_LINE_30_OFFSET,
			get_reg_val((void __iomem*)((char*)viraddr + CVDR_AXI_JPEG_VP_WR_AXI_LINE_30_OFFSET)));
		cam_debug("%s: CVDR reg: 0x%08x=0x%08x", __func__,
			CVDR_AXI_JPEG_VP_WR_IF_CFG_30_OFFSET,
			get_reg_val((void __iomem*)((char*)viraddr + CVDR_AXI_JPEG_VP_WR_IF_CFG_30_OFFSET)));
		cam_debug("%s: CVDR reg: 0x%08x=0x%08x", __func__,
		...
		cam_debug("%s: CVDR reg: 0x%08x=0x%08x", __func__,
			...
			get_reg_val((void __iomem*)((char*)viraddr + CVDR_AXI_JPEG_LIMITER_VP_WR_30_OFFSET)));
	}
...
static void hjpeg_disabe_irq(void)
{
    switch (is_irq_merge) {
        ...
        case JPEG_IRQ_MERGE:
            ...
            break;
        default:
            set_reg_val((void __iomem*)((char*)s_hjpeg.hw_ctl.jpegenc_viraddr + JPGENC_JPE_STATUS_IMR_REG), 0x00);
            break;
    }
}
...
static void hjpeg_enable_irq(void)
{
     switch (is_irq_merge) {
        ...
        case JPEG_IRQ_MERGE:
            ...
            break;
        default:
            set_reg_val(s_hjpeg.hw_ctl.jpegenc_viraddr + JPGENC_JPE_STATUS_IMR_REG, 0x30);
            break;
    }
}

static int hjpeg_remap_addr_for_diff_domain(jpgenc_config_t* pcfg)
{
    int rc = 0;
    struct dma_buf *dmabuf = NULL;
    unsigned long iova_addr, iova_size = 0, offset = 0;
    struct device *pdev = &(s_hjpeg.pdev->dev);
    if (pcfg == NULL) {
        cam_err("%s: pcfg is NULL pointer!", __func__);
        return -ENOENT;
    }

    if (pdev != NULL) {
        offset = pcfg->buffer.input_buffer_uv - pcfg->buffer.input_buffer_y;
        dmabuf = dma_buf_get(pcfg->buffer.input_ion_fd);
        if (IS_ERR(dmabuf)) {
            cam_err("%s: fail to get dma dmabuf.", __func__);
            return -ENOENT;
        }

        iova_addr = hisi_iommu_map_dmabuf(pdev, dmabuf, 0, &iova_size);
        pcfg->buffer.input_buffer_y = iova_addr;
        pcfg->buffer.input_buffer_uv = iova_addr + offset;
        cam_debug("%s: fd:%d, dmabuf:%pK.", __func__, pcfg->buffer.input_ion_fd, dmabuf);
        dma_buf_put(dmabuf);

        dmabuf = dma_buf_get(pcfg->buffer.ion_fd);
        if (IS_ERR(dmabuf)) {
            cam_err("%s: fail to get dma dmabuf.", __func__);
            return -ENOENT;
        }
        ...
        iova_addr = hisi_iommu_map_dmabuf(pdev, dmabuf, 0, &iova_size);
        pcfg->buffer.output_buffer = iova_addr;

        cam_debug("%s: fd:%d, dmabuf:%pK.", __func__, pcfg->buffer.ion_fd, dmabuf);
        dma_buf_put(dmabuf);
    }
    return rc;
}

// unmap smmu v3 addr for diff domain
static int hjpeg_unmap_addr_for_diff_domain(jpgenc_config_t* pcfg)
{
    int rc = 0;
    struct dma_buf *dmabuf = NULL;
    struct device *pdev = &(s_hjpeg.pdev->dev);
    ...
    if (s_hjpeg.pdev != NULL) {
        ...
        rc = hisi_iommu_unmap_dmabuf(pdev, dmabuf, pcfg->buffer.input_buffer_y);
        if (rc) {
            cam_err("%s:unmap input_buffer_y(%u) fail.", __func__, pcfg->buffer.input_buffer_y);
            return -ENOENT;
        }
        cam_debug("%s: fd:%d, dmabuf:%pK.", __func__, pcfg->buffer.input_ion_fd, dmabuf);
        dma_buf_put(dmabuf);
        ...
        rc = hisi_iommu_unmap_dmabuf(pdev, dmabuf, pcfg->buffer.output_buffer);
        if (rc) {
            cam_err("%s:unmap output_buffer(%u) fail.", __func__, pcfg->buffer.output_buffer);
            return -ENOENT;
        }
        cam_debug("%s: fd:%d, dmabuf:%pK.", __func__, pcfg->buffer.ion_fd, dmabuf);
        dma_buf_put(dmabuf);
...
static int hjpeg_encode_process(hjpeg_intf_t *i, void *cfg)
{
    ...
    int    ret;
    u32 byte_cnt;
	unsigned int tmp_input_buffer_y;
	unsigned int tmp_input_buffer_uv;
	unsigned int tmp_output_buffer;
	...
	{
		tmp_input_buffer_y = pcfg->buffer.input_buffer_y;
		tmp_input_buffer_uv = pcfg->buffer.input_buffer_uv;
		tmp_output_buffer = pcfg->buffer.output_buffer;
	}
    ...
    if (s_hjpeg.hw_ctl.smmu_type == ST_SMMUV3) {
       if( hjpeg_remap_addr_for_diff_domain(pcfg) != 0) {
           cam_err("%s(%d) hjpeg remap addr for different domain error!", __func__, __LINE__);
           return -ENOENT;
       }
    }
    ...
    // upmap smmu v3 addr for diff domain
    if (s_hjpeg.hw_ctl.smmu_type == ST_SMMUV3) {
       if (hjpeg_unmap_addr_for_diff_domain(pcfg) != 0) {
           cam_err("%s(%d) hjpeg unmap addr for different domain error!", __func__, __LINE__);
           return -ENOENT;
       }
	   {
		   pcfg->buffer.input_buffer_y = tmp_input_buffer_y;
		   pcfg->buffer.input_buffer_uv = tmp_input_buffer_uv;
		   pcfg->buffer.output_buffer = tmp_output_buffer;
	   }
    }
...
static int hjpeg_poweroff(hjpeg_base_t* pJpegDev)
{
    ...
    if (pJpegDev->hw_ctl.smmu_type == ST_SMMUV3) {
        ret = do_cfg_smmuv3_tbuclose(&pJpegDev->hw_ctl);
        if (ret != 0) {
            cam_err("%s(%d) failed to close connection of tbu and tcu!", __func__, __LINE__);
        }
    }
...
static int get_dts_secadapt_prop(struct device *pdev)
{
    ...
    ret = of_property_read_u32_array(np, "huawei,secadapt-base", base_array, count);
    if (ret < 0) {
        cam_err("%s failed get subctrl base %d", __func__, __LINE__);
        return -1;
    }
    ...
    if (IS_ERR_OR_NULL(s_hjpeg.hw_ctl.secadapt_viraddr)) {
        cam_err("%s secadapt phyaddr ioremap fail", __func__);
        return -1;
    }

    cam_debug("%s secadapt base address = 0x%x. secadapt-base size = 0x%x. sec_viraddr: 0x%08x",
            __func__,base_array[0], base_array[1], s_hjpeg.hw_ctl.secadapt_viraddr);

    /* property(huawei,sid-ssid) = <sid ssid>, so count is 2 */
    ret = of_property_read_u32_array(np, "huawei,sid-ssid", base_array, count);
    if (ret < 0) {
        cam_err("%s failed get sid-ssid property %d", __func__, __LINE__);
        return -1;
    }

    s_hjpeg.hw_ctl.secadapt_prop.sid = base_array[0];
    s_hjpeg.hw_ctl.secadapt_prop.ssid_ns = base_array[1];
    cam_debug("%s(%d) sid: %u, ssidv_ns: %u", __func__, __LINE__, base_array[0], base_array[1]);

    count = 3;
    ret = of_property_read_u32_array(np, "huawei,jpgen-swid", base_array, count);
    if (ret < 0) {
        cam_err("%s failed get sid-ssid property %d", __func__, __LINE__);
        return -1;
    }

    s_hjpeg.hw_ctl.secadapt_prop.swid[0] = base_array[0];
    s_hjpeg.hw_ctl.secadapt_prop.swid[1] = base_array[1];
    s_hjpeg.hw_ctl.secadapt_prop.swid[2] = base_array[2];
    cam_info("%s(%d) swid: %u %u %u", __func__, __LINE__, base_array[0], base_array[1], base_array[2]);
...
static int hjpeg_get_dts(struct platform_device* pDev)
{
    ...
    // prefetch_bypass
    ret = of_property_read_u32(np, "huawei,prefetch_bypass", &is_prefetch_bypass);
    if (ret == 0) {
        cam_info("%s: update prefetch_bypass=%u", __func__, is_prefetch_bypass);
    }
    ...
    if (s_hjpeg.hw_ctl.smmu_type == ST_SMMUV3) {
        ...
        if (ret < 0) {
            cam_err("%s: get secadapt property  failed.", __func__);
            return -1;
        }
...
static int hjpeg_clk_ctrl(void __iomem* subctrl1, bool enable)
{
    ...
    cam_info("%s enter [0x%x] \n", __func__, get_reg_val(subctrl_clk));

    if (enable) {
        set_reg_val(subctrl_clk, get_reg_val(subctrl_clk) | 0x1);
    } else {
        set_reg_val(subctrl_clk, get_reg_val(subctrl_clk) & 0xFFFFFFFE);   /* [false alarm]:it is a dead code */
    }
    set_clk = enable ? 0x1 : 0x0;
    cur_clk = get_reg_val(subctrl_clk);
    if (set_clk != cur_clk) {
        cam_err("%s(%d) write fail jpeg clk status 0x%x", __func__, __LINE__, cur_clk);
        ret = -EIO;
    }
...
{
    // 1. clear inner jpgen
    set_reg_val((void __iomem*)((char*)jpegenc + JPGENC_JPE_STATUS_ICR_REG), IRQ_IN_CLR_VAL);
    // 2. clear ipp top register
    set_reg_val((void __iomem*)((char*)(subctl + JPGENC_IRQ_REG0)), IRQ_OUT_CLR_VAL);
}
...
static void hjpeg_irq_inout_mask(bool enable)
{
    u32 set_val;
    u32 cur_val;
    void __iomem* subctrl1_irq_mask = (void __iomem*)((char*)s_hjpeg.hw_ctl.subctrl_viraddr + JPGENC_IRQ_REG1);
    void __iomem* jpg_irq_mask = (void __iomem*)((char*)s_hjpeg.hw_ctl.jpegenc_viraddr + JPGENC_JPE_STATUS_IMR_REG);

    set_val = enable ? (IRQ_OUT_MASK_ENABLE) : (IRQ_OUT_MASK_DISABLE);
    set_reg_val(subctrl1_irq_mask, set_val);
    cur_val = get_reg_val(subctrl1_irq_mask);
    if (set_val != cur_val) {
        cam_err("%s(%d) isp subcrl irq mask status %x, mask write failed set_val=%x", __func__, __LINE__, cur_val, set_val);
    }
    cam_info("%s isp subcrl irq mask(%x) status %u enable(%u)", __func__, subctrl1_irq_mask, cur_val, enable);
    ...
    cur_val = get_reg_val(jpg_irq_mask);
    if (set_val != cur_val) {
        cam_err("%s(%d) isp jpeg irq mask(0x%08x) status %u, mask write failed set_val=%u", __func__, __LINE__, jpg_irq_mask, cur_val, set_val);
    }
    cam_info("%s isp jpeg irq mask(%x)  setval(0x%08x)status curval(0x%08x), enable(%u)", __func__, jpg_irq_mask, set_val, cur_val, enable);
...
int is_hjpeg_prefetch_bypass(void)
{
	cam_debug("%s: is_prefetch_bypass=%d", __func__, is_prefetch_bypass);
	return is_prefetch_bypass;
}
