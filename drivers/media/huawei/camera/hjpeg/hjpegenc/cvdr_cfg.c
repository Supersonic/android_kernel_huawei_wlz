...
void hjpeg_vp_wr_axi_line_update3(void __iomem *cvdr_srt_base, cvdr_wr_fmt_desc_t *desc, u32 offset)
{
    ...
    tmp_line.reg32 = get_reg_val((void __iomem*)((char*)cvdr_srt_base + offset));
    tmp_line.bits.vpwr_line_stride = desc->line_stride;
    tmp_line.bits.vpwr_line_start_wstrb = CVDR_AXI_JPEG_LINE_START_WSTRB_VAL;
    tmp_line.bits.vpwr_line_wrap = desc->line_wrap;
    set_reg_val((void __iomem*)((char*)cvdr_srt_base + offset), tmp_line.reg32);
}
...
void hjpeg_config_axi_line(hjpeg_hw_ctl_t *hw_ctl, cvdr_wr_fmt_desc_t *desc)
{
    ...
    switch(hw_ctl->cvdr_prop.wr_port) {
        ...
        case WR_PORT_30:
        {
            hjpeg_vp_wr_axi_line_update3(hw_ctl->cvdr_viraddr, desc, CVDR_AXI_JPEG_VP_WR_AXI_LINE_30_OFFSET);
            break;
        }
        default:
        {
...
void hjpeg_config_axi_fs(hjpeg_hw_ctl_t *hw_ctl, jpgenc_config_t* config)
{
    ...
    switch(wr_port) {
        ...
        case WR_PORT_30:
        {
            hjpeg_config_vp_wr_axi_fs_for34(hw_ctl->cvdr_viraddr, CVDR_AXI_JPEG_VP_WR_AXI_FS_30_OFFSET, buf_addr);
            break;
        }
        default:
        {
...
static int set_vp_wr_ready(hjpeg_hw_ctl_t *hw_ctl, jpgenc_config_t* config)
{
    ...
    switch(wr_port) {
        ...
        case WR_PORT_30:
        {
            ...
            cvdr_axi_limiter_vp_wr_offset = CVDR_AXI_JPEG_LIMITER_VP_WR_30_OFFSET;
            break;
        }
        default:
            cam_err("%s, invalid wr port[%d]", __func__, wr_port);
    ...
    // smmu bypass
    if (hw_ctl->smmu_bypass == BYPASS_YES || is_hjpeg_prefetch_bypass() == PREFETCH_BYPASS_YES)
    {
