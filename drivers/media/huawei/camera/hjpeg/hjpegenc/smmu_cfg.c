...
static void do_cfg_smmuv3_tbu_swid(bool bypass, hjpeg_hw_ctl_t *hw_ctl)
{
    ...
    // open tbu prefetch
    for (i = 0; i < MAX_SECADAPT_SWID_NUM; i++) {
        tbu_swidcfg_addr = tbu_base_addr + hw_ctl->secadapt_prop.swid[i] * SECADAPT_SWID_CFG_NS_PER_OFFSET + SMMUV3_SWID_CFG_OFFSET;
...
static int do_cfg_secadapt_sid(hjpeg_hw_ctl_t *hw_ctl)
{
    ...
    for (i = 0; i < MAX_SECADAPT_SWID_NUM; i++) {
        ...
        swid_cfg_ns_addr = sec_adapt_base_addr + SECADAPT_SWID_CFG_NS_OFFSET +
            hw_ctl->secadapt_prop.swid[i] * SECADAPT_SWID_CFG_NS_PER_OFFSET;
        ...
        if (hw_ctl->smmu_bypass == BYPASS_YES) {
            ...
            // current version only support ssid = 0, otherwise will error
            sec_adapt_swid_cfg_ns.reg.ssid_ns = 0; // hw_ctl->secadapt_prop.ssid_ns;
        }
...
void dump_smmu_reg(hjpeg_hw_ctl_t *hw_ctl)
{
    ...
    for(i = 0; i < MAX_SECADAPT_SWID_NUM; i++) {
        offset = SECADAPT_SWID_CFG_NS_OFFSET + hw_ctl->secadapt_prop.swid[i] * SECADAPT_SWID_CFG_NS_PER_OFFSET;
    ...
    for(i = 0; i < MAX_SECADAPT_SWID_NUM; i++) {
        offset = SMMUV3_SWID_CFG_OFFSET + hw_ctl->secadapt_prop.swid[i] * SECADAPT_SWID_CFG_NS_PER_OFFSET;
