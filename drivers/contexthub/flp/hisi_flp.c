		...
		unsigned int count, unsigned char cmd_type)
{
	...
	case FLP_GENL_CMD_PDR_DATA:
		...
		if ((pdata->read_index + count) >  pdata->buf_size) {
			result = memcpy_s(data, (size_t)count, pdata->data_buf + pdata->read_index,
							 (size_t)(pdata->buf_size - pdata->read_index));
			if (result != EOK) {
				nlmsg_free(skb);
				pr_err("[%s]memset_s fail[%d]\n", __func__, result);
				return result;
			}
			result = memcpy_s(data + pdata->buf_size - pdata->read_index,
								(size_t)count - (size_t)(pdata->buf_size - pdata->read_index),
								pdata->data_buf, (size_t)(count + pdata->read_index - pdata->buf_size));


		} else {
			result = memcpy_s(data, (size_t)count, pdata->data_buf + pdata->read_index, (size_t)count);
			if (result != EOK) {
				nlmsg_free(skb);
				pr_err("[%s]memset_s fail[%d]\n", __func__, result);
				return result;
...
static int  flp_pdr_stop_cmd(flp_port_t *flp_port, unsigned long arg)
{
	...
	flp_port_t      *port = NULL;
	unsigned int delay = 0;
	int ret;

	pr_info("flp_stop_cmd pdr count[%d]--dalay[%d]\n", g_flp_dev.pdr_start_count, delay);
	if (flp_port->pdr_buf.data_buf == NULL) {
		pr_err("Repeat stop is not permit \n");
		return -EPERM;
	}

	if (copy_from_user(&delay, (void *)((uintptr_t)arg), sizeof(unsigned int))) {
	    pr_err("flp_ioctl copy_from_user error\n");
	    return -EFAULT;
	}
	...
	(void)memset_s((void *)&flp_port->pdr_config, sizeof(flp_port->pdr_config), 0, sizeof(pdr_start_config_t));
	if (0 == g_flp_dev.pdr_start_count) {
		(void)memset_s((void *)&g_flp_dev.pdr_config, sizeof(g_flp_dev.pdr_config), 0, sizeof(pdr_start_config_t));
		delay = 0;
...
static int get_pdr_cfg(pdr_start_config_t *pdr_config, const char __user *buf, size_t len)
{
	...
	if (copy_from_user(pdr_config, buf, len)) {
		pr_err("flp_start_cmd copy_from_user error\n");
		return -EIO;
	}
	...
	if (pdr_config->report_interval <
		...
		pr_err("flp_start_cmd error  line[%d]\n", __LINE__);
		return -EINVAL;
	}
...
static int  flp_pdr_start_cmd(flp_port_t *flp_port, const char __user *buf, size_t len, unsigned int cmd)
{
	...
	pr_err("flp:interval:%u,precise:%u,count:%u\n",
...
static int  flp_pdr_update_cmd(flp_port_t *flp_port, const char __user *buf, size_t len)
{
	...
	if (g_flp_dev.pdr_start_count > 1) {
		g_flp_dev.pdr_config.report_precise = PDR_PRECISE_MIN;
		g_flp_dev.pdr_config.report_interval =
				calc_GCD(g_flp_dev.pdr_config.report_interval,
					flp_port->pdr_update_config.report_interval);
		g_flp_dev.pdr_config.report_count = PDR_DATA_MIX_COUNT;
		g_flp_dev.pdr_config.report_times = 0;
	}   else {
		ret = memcpy_s(&g_flp_dev.pdr_config, sizeof(g_flp_dev.pdr_config),
						&flp_port->pdr_update_config, sizeof(pdr_start_config_t));
		if (ret != EOK) {
			pr_err("[%s]memset_s fail[%d]\n", __func__, ret);
		}
		g_flp_dev.pdr_config.report_times = 0;
			...
			unsigned int len, unsigned int align)
{
	...
	if ((pdata->data_count + len) > pdata->buf_size) {
		...
		if (deta % align) {
			pr_err("copy_data_to_buf data not align\n");
...
static void __get_pdr_data_from_mcu(flp_pdr_data_t *data, unsigned int count)
{
	...
	flp_port_t *flp_port = NULL ;
	struct list_head *pos = NULL;
	flp_pdr_data_t *pevent = NULL;
	flp_pdr_data_t *pinsert = NULL;
	int ret;
...
static int get_pdr_data_from_mcu(const pkt_header_t *head)
{
	...
	if (list_empty(&g_flp_dev.list)) {
		pr_err("flp pdr no port be opened\n");
		mutex_unlock(&g_flp_dev.lock);
		return -EFAULT;
	}
...
#else
	return send_cmd_from_kernel(TAG_PDR, CMD_CMN_CONFIG_REQ, CMD_FLP_PDR_FLUSH_REQ, NULL, 0);
#endif
}
/*lint +e845 */
/*
...
static int flp_pdr_step(flp_port_t *flp_port, unsigned long arg)
{
	...
	(void)memset_s((void *)&step_report, sizeof(step_report), 0, sizeof(step_report_t));
	flp_port->need_report = TRUE;
	if (copy_from_user(&step_report, (char __user *)((uintptr_t)arg), sizeof(step_report_t))) {
		pr_err("[%s]copy_from_user error\n", __func__);
		return -EIO;
	}
#ifdef CONFIG_INPUTHUB_20
	return send_cmd_from_kernel(TAG_PDR, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_PDR_STEPCFG_REQ,
				    (char  *)&step_report, sizeof(step_report_t));
#else
	return send_cmd_from_kernel(TAG_PDR, CMD_CMN_CONFIG_REQ, CMD_FLP_PDR_STEPCFG_REQ,
				    (char  *)&step_report, sizeof(step_report_t));
#endif
}
...
static int flp_pdr_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	case FLP_IOCTL_PDR_START(0):
		ret = flp_pdr_start_cmd(flp_port, (char __user *)((uintptr_t)arg),
					sizeof(pdr_start_config_t), cmd);
		break;
	case FLP_IOCTL_PDR_STOP(0):
		ret = flp_pdr_stop_cmd(flp_port, arg);
		break;
	case FLP_IOCTL_PDR_UPDATE(0):
		ret = flp_pdr_update_cmd(flp_port, (char __user *)((uintptr_t)arg),
					 sizeof(pdr_start_config_t));
		break;
	...
	case FLP_IOCTL_PDR_STEP_CFG(0):
		...
		break;
	default:
		pr_err("flp_pdr_ioctl input cmd[0x%x] error\n", cmd);
		return -EFAULT;
	}
	return ret;
}
...
static int flp_stop_service_type(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = send_cmd_from_kernel_response(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_COMMON_STOP_SERVICE_REQ,
...
static int flp_iomcu_template (flp_port_t *flp_port, unsigned long arg, unsigned int iomcu_sub_cmd)
{
	...
	return send_cmd_from_kernel(TAG_FLP, CMD_CMN_CONFIG_REQ, iomcu_sub_cmd,
...
static int flp_common_push_location_status(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = send_cmd_from_kernel(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_BATCH_PUSH_GNSS_REQ,
					(char *)&flp_port->location_status, sizeof(loc_source_status));
...
static int flp_common_set_debug_config(unsigned long arg)
{
	...
	ret = send_cmd_from_kernel(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_COMMON_DEBUG_CONFIG_REQ,
				   (char *)&cfg, sizeof(cfg));
...
static int flp_common_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	default:
			...
			return ret;
#else
			pr_err("flp_common_ioctl input cmd[0x%x] error\n", cmd);
			return -EFAULT;
#endif
	}
	return 0;
...
static int flp_open_send_status_flp(flp_port_t *flp_port)
{
	...
	if (!(flp_port->channel_type & IOMCU_APP_FLP)) {
		...
		pr_info("%s:line[%d] send source[0x%x] status[0][0x%x] status[1][0x%x] ret[%d]\n",
...
{
	FLP_IOMCU_SHMEM_TYPE *iomcu_data = NULL;
...
static int flp_ioctl_remove_geofence(flp_port_t *flp_port, unsigned long arg)
{
	...
	if (copy_from_user(&hal_config, (const void __user *)((uintptr_t)arg), sizeof(hal_config_t))) {
		pr_err("%s copy_from_user error\n", __func__);
		return -EFAULT;
	}
	...
	cmd_data = kzalloc((size_t)hal_config.length, GFP_KERNEL);//lint !e838
	if (cmd_data == NULL) {
...
#ifdef CONFIG_INPUTHUB_20
		send_cmd_from_kernel(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_BATCHING_MULT_LASTLOCATION_REQ, (char *)&data, sizeof(int));
#endif
		break ;
...
#ifdef CONFIG_INPUTHUB_20
		send_cmd_from_kernel(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_BATCHING_MULT_FLUSH_REQ, (char *)&data, sizeof(int));
#endif
		break ;
...
#endif
		{
			if (copy_to_user((void *)((uintptr_t)arg), rd.data, sizeof(unsigned int))) {
...
static int cellfence_add(flp_port_t *flp_port, unsigned long arg, unsigned short int cmd)
{
	...
	FLP_IOMCU_SHMEM_TYPE *shmem_block = NULL;
	char *cptr = NULL;
	...
	usr_data = (char *)memdup_user(cfence_hdr.buf, (size_t)cfence_hdr.len);//lint !e838
	if (IS_ERR(usr_data)) {
		pr_err("[%s] memdup_user error\n", __func__);
		return (int)PTR_ERR(usr_data);
	}

	cptr = usr_data;//lint !e838
...
static int flp_diag_send_cmd(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = send_cmd_from_kernel(TAG_FLP, CMD_CMN_CONFIG_REQ,	SUB_CMD_FLP_DIAG_SEND_CMD_REQ,
				   (char *)(data_buf + sizeof(unsigned int)), (size_t)data_len);
	if (ret != 0) {
		pr_err("[%s]GEOFENCE_RM_REQ error\n", __func__);
		goto FLP_DIAG_EXIT;
	}
...
static int flp_release(struct inode *inode, struct file *file)/*lint -e715*/
{
	...
	pr_info("%s pdr_count[%d]:service_type [%d] \n", __func__, g_flp_dev.pdr_start_count,
...
int  hisi_flp_register(void)
{
	...
	if (getSensorMcuMode() == 0) {
		pr_err("cannot register hisi flp %d\n", __LINE__);
