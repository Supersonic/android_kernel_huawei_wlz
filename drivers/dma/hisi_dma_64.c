	...
	unsigned long flags)
{
	...
	ds->desc_hw_lli =
		__virt_to_phys((uintptr_t)&ds->desc_hw[0]); /*lint !e648*/
	ds->size = len;
...
void show_dma64_reg(struct dma_chan *chan)
{
	...
	for (i = d->dma_min_chan; i < d->dma_channels; i++) {
		p = &d->phy[i];
		pr_err("idx[%2d]:CX_CONFIG:[0x%32x], CX_AXI_CONF:[0x%32x], "
			"SRC_H:[0x%32x], SRC_L:[0x%32x], DST_H:[0x%32x], "
			"DST_L:[0x%32x], CNT0:[0x%32x], CX_CURR_CNT0:[0x%32x]\n",
			i, readl(p->base + CX_CONFIG),
			readl(p->base + AXI_CONFIG), readl(p->base + CX_SRC_H),
			readl(p->base + CX_SRC_L), readl(p->base + CX_DST_H),
...
noinline int atfd_hisi_service_dmac_smc(
...
static int hisi_dma_probe(struct platform_device *op)
{
	...
	of_property_read_u32((&op->dev)->of_node, "dma-min-chan",
		&d->dma_min_chan);
	atfd_hisi_service_dmac_smc(DMAC_REGISTER_FN_ID, d->dma_min_chan, 0, 0);
	of_property_read_u32((&op->dev)->of_node, "dma-used-chans",
		&d->dma_used_chans);
	...
	op->dev.coherent_dma_mask = DMA_BIT_MASK(64); /*lint !e598 !e648*/

	d->clk = devm_clk_get(&op->dev, NULL);
	if (IS_ERR(d->clk)) {
		dev_err(&op->dev, "no dma clk\n");
		return (int)PTR_ERR(d->clk); /*lint !e429*/
	}

	irq = (u32)platform_get_irq(op, 0);
	ret = devm_request_irq(
		&op->dev, irq, hisi_dma_int_handler, 0, DRIVER_NAME, d);
	if (ret)
		return ret; /*lint !e429*/
