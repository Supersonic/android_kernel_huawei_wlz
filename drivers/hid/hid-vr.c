...
static int ovr_hidraw_release(struct inode *inode, struct file *file)
{
	...
	/* VR may disconnect or plug again when /dev/hwvr being closed,
	 * access struct hidraw using list->hidraw instead of
	 * ovr_hidraw_table[minor]. */
	mutex_lock(&minors_lock);
				...
				unsigned long arg)
{
	...
	if (user_arg == NULL) return -EINVAL;
	mutex_lock(&minors_lock);
	dev = ovr_hidraw_table[minor];
	if (!dev) {
		ret = -ENODEV;
