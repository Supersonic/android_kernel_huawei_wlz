...
#define IS_HOST   (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1102_HOST)
#define IS_DEVICE (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1102_DEV)
...
/* 业务VAP个数(原本应该用AP个数+STA个数（这个表示芯片最大支持能力）,软件约束和资源上命令成3即可) */
 ...
 * WLAN_ACTIVE_USER_MAX_NUM对应g_us_active_max_user
 */
/* 关联用户的最大个数 */
 ...
 * 使用规则:这里三个用于oal oam hal初始化成员，或者数组下标，
 * 与之对应mac res的单播和组播，以及整board user个数会封装成函数供业务层代码调用
 */
...
/* 接收512(数据帧描述符) + 64(管理帧描述符) */ /* 注意! 新增一个子内存池要更新oal_mem.c里的OAL_MEM_BLK_TOTAL_CNT */
...
/* 2.4.7 用户内存池 */
#define WLAN_MEM_MIB_SIZE1 1184 /* mib结构体大小 */
