...
/* 全局变量定义 */
sdt_drv_mng_stru sdt_drv_mng_entry;
oam_sdt_func_hook_stru sdt_drv_func_hook;
...
oal_void sdt_drv_set_mng_entry_usepid(oal_uint32 ulpid)
{
    ...
    oal_spin_lock_irq_save(&sdt_drv_mng_entry.st_spin_lock, &ui_irq_save);

    sdt_drv_mng_entry.ul_usepid = ulpid;

    oal_spin_unlock_irq_restore(&sdt_drv_mng_entry.st_spin_lock, &ui_irq_save);
}
...
OAL_STATIC OAL_INLINE oal_void sdt_drv_netbuf_add_to_list(oal_netbuf_stru *pst_netbuf)
{
    ...
    oal_netbuf_add_to_list_tail(pst_netbuf, &sdt_drv_mng_entry.rx_wifi_dbg_seq);
...
oal_netbuf_stru *sdt_drv_netbuf_delist(oal_void)
{
    ...
    pst_netbuf = oal_netbuf_delist(&sdt_drv_mng_entry.rx_wifi_dbg_seq);
                                                    ...
                                                    oam_primid_type_enum_uint8 en_prim_id)
{
    ...
    us_tmp_data = sdt_drv_mng_entry.us_sn_num;
                                                        ...
                                                        oam_primid_type_enum_uint8 en_prim)
{
    ...
    l_ret = oal_workqueue_schedule(&sdt_drv_mng_entry.rx_wifi_work);
...
{
    return (oal_int32)oal_netbuf_list_len(&sdt_drv_mng_entry.rx_wifi_dbg_seq);
}
...
oal_int32 sdt_drv_send_data_to_wifi(oal_uint8 *puc_param, oal_int32 l_len)
{
    ...
    memcpy_s((oal_void *)pc_buf, l_len, (const oal_void *)puc_param, (oal_uint32)l_len); /* i_len >= l_len */
...
oal_uint32 sdt_drv_netlink_send(oal_netbuf_stru *pst_netbuf, oal_uint32 ul_len)
{
    ...
    if (sdt_drv_mng_entry.ul_usepid == 0) {
        if ((oal_atomic_read(&sdt_drv_mng_entry.ul_unconnect_cnt) % SDT_DRV_REPORT_NO_CONNECT_FREQUENCE) == 0) {
            OAL_IO_PRINT("Info:waitting app_sdt start...\r\n");
            oal_atomic_inc(&sdt_drv_mng_entry.ul_unconnect_cnt);
        }
    ...
    l_unicast_bytes = oal_netlink_unicast(sdt_drv_mng_entry.pst_nlsk, pst_copy_netbuf,
                                          sdt_drv_mng_entry.ul_usepid, OAL_MSG_DONTWAIT);
...
oal_void sdt_drv_netlink_recv(oal_netbuf_stru *pst_netbuf)
{
    ...
    memset_s(sdt_drv_mng_entry.puc_data, DATA_BUF_LEN, 0, DATA_BUF_LEN);
    ...
    if (OAL_NETBUF_LEN(pst_netbuf) >= OAL_NLMSG_SPACE(0)) {
        ...
        if (ul_len <= DATA_BUF_LEN && ul_len >= (oal_uint32)OAL_SIZEOF(st_msg_hdr)) {
            ret = memcpy_s((oal_void *)sdt_drv_mng_entry.puc_data, DATA_BUF_LEN,
        ...
        ret = memcpy_s((oal_void *)&st_msg_hdr, (oal_uint32)OAL_SIZEOF(st_msg_hdr),
                       (const oal_void *)sdt_drv_mng_entry.puc_data,
        ...
        if (st_msg_hdr.ul_cmd == NETLINK_MSG_HELLO) {
            sdt_drv_mng_entry.ul_usepid = pst_nlhdr->nlmsg_pid; /* pid of sending process */
            OAL_IO_PRINT("sdt_drv_netlink_recv::%s pid is-->%d \n", OAL_FUNC_NAME, sdt_drv_mng_entry.ul_usepid);
        } else {
#if defined(PLATFORM_DEBUG_ENABLE) || (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1151)
            sdt_drv_send_data_to_wifi(&sdt_drv_mng_entry.puc_data[OAL_SIZEOF(st_msg_hdr)],
...
{
    sdt_drv_mng_entry.pst_nlsk = oal_netlink_kernel_create(&OAL_INIT_NET, NETLINK_TEST,
                                                           0, sdt_drv_netlink_recv,
                                                           OAL_PTR_NULL, OAL_THIS_MODULE);
    if (sdt_drv_mng_entry.pst_nlsk == OAL_PTR_NULL) {
...
{
    sdt_drv_func_hook.p_sdt_report_data_func = sdt_drv_report_data2app;
    sdt_drv_func_hook.p_sdt_get_wq_len_func = sdt_drv_get_wq_len;
}
...
oal_int32 sdt_drv_main_init(oal_void)
{
    ...
    memset_s((void *)&sdt_drv_mng_entry, OAL_SIZEOF(sdt_drv_mng_entry),
             0, OAL_SIZEOF(sdt_drv_mng_entry));

    sdt_drv_mng_entry.puc_data = oal_memalloc(DATA_BUF_LEN);
    if (sdt_drv_mng_entry.puc_data == OAL_PTR_NULL) {
        OAL_IO_PRINT("alloc sdt_drv_mng_entry.puc_data fail!\n");
        return -OAL_EFAIL;
    }
    ...
    OAL_INIT_WORK(&sdt_drv_mng_entry.rx_wifi_work, sdt_drv_push_wifi_log_work);
    oal_spin_lock_init(&sdt_drv_mng_entry.st_spin_lock);
    oal_netbuf_list_head_init(&sdt_drv_mng_entry.rx_wifi_dbg_seq);

    /* sdt模块钩子函数初始化 */
    sdt_drv_func_hook_init();

    /* 将sdt钩子函数注册至oam模块 */
    oam_sdt_func_fook_register(&sdt_drv_func_hook);
...
oal_void sdt_drv_main_exit(oal_void)
{
    ...
    if (sdt_drv_mng_entry.pst_nlsk != OAL_PTR_NULL) {
        oal_netlink_kernel_release(sdt_drv_mng_entry.pst_nlsk);
    }

    if (sdt_drv_mng_entry.puc_data != OAL_PTR_NULL) {
        oal_free(sdt_drv_mng_entry.puc_data);
    }

    oal_cancel_work_sync(&sdt_drv_mng_entry.rx_wifi_work);
    oal_netbuf_queue_purge(&sdt_drv_mng_entry.rx_wifi_dbg_seq);
