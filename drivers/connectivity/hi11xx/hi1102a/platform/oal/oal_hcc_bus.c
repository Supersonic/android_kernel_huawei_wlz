...
oal_uint32 pm_wakeup_event_flag = OAL_FALSE;
/*lint -e19*/
oal_module_symbol(pm_wakeup_event_flag);
/*lint +e19*/
#endif
    ...
    for (;;) {
        ...
        if (ret != OAL_SUCC || switch_pwr_ret != OAL_SUCC) {
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "hcc_bus_switch_thread init bus %s falied=%d, ret=%d",
                                 hcc_bus_get_bus_type_str(target_bus->bus_type), ret, switch_pwr_ret);
...
oal_int32 hcc_add_bus(hcc_bus *pst_bus, const char *bus_name)
{
    ...
    if (OAL_TRUE == oal_dlist_is_empty(&hcc_bus_res_hdr)) {
        oal_spin_unlock_irq_restore(&hcc_bus_res_lock, &irqsave);
...
struct st_wifi_dump_mem_info hi1102a_pilot_meminfo[] = {
    {
        .file_name = "wifi_device_panic_mem",
...
void hcc_bus_exception_handler(oal_work_stru *work)
{
    ...
    mem_info = hi1102a_pilot_meminfo;
    size = sizeof(hi1102a_pilot_meminfo) / sizeof(struct st_wifi_dump_mem_info);
#else
