...
{
    uart_state.tty_tx_cnt += cnt;
}

/* call only in one place!!! */
STATIC void ps_uart_tty_rx_add(uint32 cnt)
{
    uart_state.tty_rx_cnt += cnt;
}
...
uint32 ps_uart_state_cur(uint32 index)
{
    ...
    switch (index) {
        case STATE_TTY_TX:
            return uart_state.tty_tx_cnt;
        case STATE_TTY_RX:
            return uart_state.tty_rx_cnt;
        case STATE_UART_TX:
...
STATIC void ps_uart_state_print(struct ps_uart_state_s *state)
{
    ...
    PS_PRINT_INFO("uart cts:%x,dsr:%x,rng:%x,dcd:%x,frame:%x,overrun:%x,parity:%x,brk:%x,buf_overrun:%x\n",
                  state->uart_cnt.cts, state->uart_cnt.dsr, state->uart_cnt.rng,
                  state->uart_cnt.dcd, state->uart_cnt.frame,
                  state->uart_cnt.overrun, state->uart_cnt.parity,
