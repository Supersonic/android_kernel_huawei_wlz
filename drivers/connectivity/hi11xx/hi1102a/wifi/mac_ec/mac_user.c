                ...
                oal_uint8       uc_vap_id)
{
    ...
    if (OAL_RETURN == g_st_mac_user_rom_cb.p_user_init_cb(pst_mac_user, &ul_cb_ret))
    {
        return ul_cb_ret;
    }
...
oal_void  mac_user_get_sta_cap_bandwidth(mac_user_stru *pst_mac_user, wlan_bw_cap_enum_uint8 *pen_bandwidth_cap)
{
    ...
    if (OAL_TRUE == pst_mac_vht_hdl->en_vht_capable)
    {
        ...
        {
            if ((1 == pst_mac_ht_hdl->bit_supported_channel_width)&& (OAL_TRUE == pst_mac_ht_hdl->en_ht_capable))
            {
