...
oal_bool_enum_uint8 mac_is_ftm_req_frame(oal_netbuf_stru *pst_netbuf)
{
    ...
    {
        OAM_ERROR_LOG0(0, OAM_SF_FTM, "{mac_is_ftm_req_frame: input pointer is null!}");
        return OAL_FALSE;
    }

    puc_mac_header  = oal_netbuf_header(pst_netbuf);
    puc_mac_payload = oal_netbuf_data(pst_netbuf);

    /* Management frame */
    if ((WLAN_FC0_SUBTYPE_ACTION|WLAN_FC0_TYPE_MGT) == mac_get_frame_sub_type(puc_mac_header))
    {
...
oal_bool_enum_uint8 mac_is_ftm_frame(oal_netbuf_stru *pst_netbuf)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_netbuf))
    {
        OAM_ERROR_LOG0(0, OAM_SF_FTM, "{mac_is_ftm_frame: input pointer is null!}");
        return OAL_FALSE;
    }
    ...
    /* Management frame */
    if (WLAN_FC0_TYPE_MGT == mac_get_frame_type(puc_mac_header) && WLAN_FC0_SUBTYPE_ACTION == mac_get_frame_sub_type(puc_mac_header))
    {
...
oal_bool_enum_uint8 mac_is_ftm_related_frame(oal_netbuf_stru *pst_netbuf)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_netbuf))
    {
        OAM_ERROR_LOG0(0, OAM_SF_FTM, "{mac_is_ftm_related_frame: input pointer is null!}");
        return OAL_FALSE;
    }
...
oal_uint32 mac_set_rsn_ie(oal_void *pst_vap, oal_uint8 *puc_pmkid, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    *puc_ie_len = 0;    /* 初始化IE 长度为0 */
    ...
    if (OAL_TRUE == mac_mib_get_dot11RSNAMFPC(pst_mac_vap) && 0 != uc_group_mgmt_suit)
    {
    ...
    *puc_ie_len = uc_index;
    ...
    if (OAL_RETURN == g_st_mac_frame_rom_cb.p_mac_set_rsn_ie(pst_vap, puc_pmkid, puc_buffer, puc_ie_len, &ul_ret))
    {
        return ul_ret;
    }
...
oal_uint32 mac_set_wpa_ie(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    *puc_ie_len = uc_index;
    ...
    if (OAL_RETURN == g_st_mac_frame_rom_cb.p_mac_set_wpa_ie(pst_vap, puc_buffer, puc_ie_len, &ul_ret))
    {
        return ul_ret;
    }
...
oal_uint8 mac_calc_smps_field(oal_void *pst_vap)
{
    mac_vap_stru *pst_mac_vap = (mac_vap_stru *)pst_vap;
...
oal_uint32  mac_set_csa_ie(oal_uint8 uc_mode, oal_uint8 uc_channel, oal_uint8 uc_csa_cnt, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    if (OAL_UNLIKELY((OAL_PTR_NULL == puc_buffer) || (OAL_PTR_NULL == puc_ie_len)))
    {
        return OAL_ERR_CODE_PTR_NULL;
...
oal_uint16 mac_get_rsn_capability(const oal_uint8 *puc_rsn_ie)
{
    ...
    if (OAL_FALSE == MAC_IE_REAMIN_LEN_IS_ENOUGH(2, us_index, puc_rsn_ie[1], 2))
    {
        return 0;
    }
...
oal_void mac_set_rrm_enabled_cap_field(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    *puc_ie_len = MAC_IE_HDR_LEN + MAC_RRM_ENABLE_CAP_IE_LEN;
    ...
    g_st_mac_frame_rom_cb.p_mac_set_rrm_enabled_cap_field(pst_vap, puc_buffer, puc_ie_len);
}
...
oal_uint16  mac_encap_2040_coext_mgmt(oal_void *pst_vap, oal_netbuf_stru *pst_buffer, oal_uint8 uc_coext_info, oal_uint32 ul_chan_report)
{
    ...
    return (us_index + MAC_80211_FRAME_LEN);    /* [false alarm]:fortify误报,返回是无符号数  */
}
