...
oal_void  wal_inform_all_bss(oal_wiphy_stru  *pst_wiphy, hmac_bss_mgmt_stru  *pst_bss_mgmt, oal_uint8   uc_vap_id)
{
    ...
    if(1 == uc_voe_11r_auth)
    {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_SCAN,
                             "{wal_inform_all_bss::uc_11r_auth=[%d]!}",uc_voe_11r_auth);
    }
#endif
    ...
    OAM_WARNING_LOG3(uc_vap_id, OAM_SF_SCAN,
                     "{wal_inform_all_bss::there are %d bss not in regdomain, so inform kernal bss num is [%d] in [%d]!}",
