...
OAL_STATIC oal_uint32 hmac_sync_tcp_ack_filter_switch(mac_vap_stru *pst_mac_vap, oal_bool_enum_uint8 en_tcp_ack_filter)
{
    oal_uint32 ul_ret;
...
OAL_STATIC oal_bool_enum_uint8 hmac_get_tcp_ack_filter_status(mac_vap_stru *pst_mac_vap, oal_uint32 ul_tx_large_pps,
...
OAL_STATIC oal_bool_enum_uint8 hmac_tcp_ack_filter_enable(mac_device_stru *pst_mac_device, oal_uint32 ul_tx_large_pps, oal_uint32 ul_rx_large_pps,
    oal_uint32 ul_tx_small_pps, oal_uint32 ul_rx_small_pps)
{
    ...
    if (!IS_LEGACY_VAP(pst_mac_vap) ||
        (IS_AP(pst_mac_vap) && (pst_mac_vap->us_user_nums != 1 || !g_st_wifi_tx_opt.en_txopt_on))) {
        return OAL_FALSE;
    }

    return hmac_get_tcp_ack_filter_status(pst_mac_vap, ul_tx_large_pps, ul_rx_large_pps, ul_tx_small_pps, ul_rx_small_pps);
}
...
oal_void hmac_rx_tcp_ack_filter_switch(oal_uint32 ul_tx_large_pps, oal_uint32 ul_rx_large_pps, oal_uint32 ul_tx_small_pps, oal_uint32 ul_rx_small_pps)
{
    ...
    en_tcp_ack_filter = hmac_tcp_ack_filter_enable(pst_mac_device, ul_tx_large_pps, ul_rx_large_pps, ul_tx_small_pps, ul_rx_small_pps);
    ...
    if (hmac_tcp_ack_filter_change_state(en_tcp_ack_filter)) {
        ...
        if (pst_mac_vap == OAL_PTR_NULL) {
            OAM_ERROR_LOG0(pst_mac_device->uc_cfg_vap_id, OAM_SF_ANY, "{hmac_rx_tcp_ack_filter_switch::pst_mac_vap NULL}");
            return;
        }
