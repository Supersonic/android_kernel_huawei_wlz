...
OAL_STATIC oal_uint32  hmac_roam_connect_notify_wpas(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_mac_hdr, oal_uint16 us_msg_len)
{
    ...
    st_asoc_rsp.ul_asoc_req_ie_len   = pst_hmac_vap->ul_asoc_req_ie_len;
...
OAL_STATIC oal_uint32  hmac_roam_ft_notify_wpas(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_mac_hdr, oal_uint16 us_msg_len)
{
    ...
    oal_memcopy(puc_ft_ie_buff, puc_mac_hdr + us_ie_offset, pst_ft_event->us_ft_ie_len);
...
OAL_STATIC oal_uint32 hmac_roam_trigger_sae_auth(hmac_roam_info_stru *pst_roam_info)
{
    ...
    oal_workqueue_schedule(&(pst_hmac_vap->st_sae_report_ext_auth_worker));
...
#ifdef _PRE_WLAN_FEATURE_SAE
    if (pst_hmac_vap->en_auth_mode == WLAN_WITP_AUTH_SAE) {
        hmac_rx_mgmt_send_to_host(pst_hmac_vap, pst_crx_event->pst_netbuf);

        OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
                        "{hmac_roam_process_auth_seq2::rx sae auth frame, status_code %x, seq_num %d.}",
...
#ifdef _PRE_WLAN_1102A_CHR
                CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV, CHR_WIFI_DRV_EVENT_11R_AUTH_FAIL, ul_ret);
#endif
                return ul_ret;
            }
            hmac_roam_connect_change_state(pst_roam_info, ROAM_CONNECT_STATE_WAIT_ASSOC_COMP);
...
OAL_STATIC oal_uint32 hmac_roam_process_assoc_rsp(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if ((WLAN_FC0_SUBTYPE_REASSOC_RSP   != uc_frame_sub_type) && (WLAN_FC0_SUBTYPE_ASSOC_RSP != uc_frame_sub_type)) {
        return OAL_SUCC;
    }
...
OAL_STATIC oal_uint32 hmac_roam_connect_fail(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    else if (ROAM_CONNECT_STATE_WAIT_ASSOC_COMP == connect_state || ROAM_CONNECT_STATE_WAIT_AUTH_COMP  == connect_state)
    {
