                                       ...
                                       oal_uint16 us_type)
{
    ...
    if ((OAL_PTR_NULL == pst_mac_ap) || (OAL_PTR_NULL == puc_sta_addr) || (OAL_PTR_NULL == puc_asoc_rsp)) {
        OAM_ERROR_LOG3(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::pst_mac_ap[0x%x], puc_data[0x%x], puc_asoc_req[0x%x].}", (uintptr_t)pst_mac_ap, (uintptr_t)puc_sta_addr, (uintptr_t)puc_asoc_rsp);
    ...
    if (OAL_PTR_NULL == pst_mac_device) {
        OAM_ERROR_LOG1(pst_mac_ap->uc_vap_id, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::pst_mac_device is a null ptr.[0x%x]}", (uintptr_t)pst_mac_device);
        return us_asoc_rsp_len;
    }

    pst_mac_user = mac_res_get_mac_user(us_assoc_id);
    if (OAL_PTR_NULL == pst_mac_user) {
        OAM_WARNING_LOG1(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::pst_mac_user[%d] is a null ptr.}", us_assoc_id);
        return us_asoc_rsp_len;
    }
    pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(us_assoc_id);
    if (OAL_PTR_NULL == pst_hmac_user) {
        OAM_WARNING_LOG1(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::pst_hmac_user(idx=%d) is a null ptr.}", us_assoc_id);
        return us_asoc_rsp_len;
    }
    ...
    if (OAL_TRUE == pst_hmac_user->en_user_epigram_vht_capable) {
        mac_set_epigram_vht_ie((oal_void *)pst_mac_ap, puc_asoc_rsp, &uc_ie_len);
        puc_asoc_rsp += uc_ie_len;
    }
    /* 5G时，epigram私有vendor ie中不携带vht,需按照此格式组帧 */
    if (OAL_TRUE == pst_hmac_user->en_user_epigram_novht_capable) {
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_mgmt_is_challenge_txt_equal(oal_uint8 *puc_data, oal_uint8 *puc_chtxt)
{
    ...
    if (OAL_PTR_NULL == puc_data || OAL_PTR_NULL == puc_chtxt) {
        OAM_ERROR_LOG2(0, OAM_SF_ANY, "{hmac_mgmt_is_challenge_txt_equal::puc_data[0x%x] or puc_chtxt[0x%x] is NULL.}", (uintptr_t)puc_data, (uintptr_t)puc_chtxt);
        return OAL_FALSE;
    }
                                                   ...
                                                   oal_uint16 *pus_user_index)
{
    ...
    if ((OAL_PTR_NULL == pst_hmac_vap) || (uc_mac_len != ETHER_ADDR_LEN)) {
...
oal_uint16 hmac_encap_auth_rsp(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_auth_rsp, oal_netbuf_stru *pst_auth_req, oal_uint8 *puc_chtxt)
{
...
#endif
    {
        OAM_ERROR_LOG4(0, OAM_SF_AUTH, "{hmac_encap_auth_rsp::pst_mac_vap[0x%x], puc_data[0x%x], puc_auth_req[0x%x] and puc_chtxt[0x%x]}", (uintptr_t)pst_mac_vap, (uintptr_t)pst_auth_rsp, (uintptr_t)pst_auth_req, (uintptr_t)puc_chtxt);
    ...
    if (OAL_PTR_NULL == pst_hmac_user_sta) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "{hmac_encap_auth_rsp::pst_hmac_user_sta[%d] is NULL}", us_user_index);
        puc_frame[4] = MAC_UNSPEC_FAIL;
#ifdef _PRE_WLAN_1102A_CHR
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV, CHR_WIFI_DRV_EVENT_SOFTAP_CONNECT, puc_frame[4]);
#endif
