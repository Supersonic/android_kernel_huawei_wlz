                ...
                oal_uint8           *puc_tid)
{
        ...
        (pst_judge_list->ul_to_judge_num >= 1))
    {
        ...
        if(RTP_VERSION == uc_rtp_ver)
        {
            ...
            {
                OAM_WARNING_LOG2(0,0, "hmac_tx_traffic_classify:rtp seq=0x%x tid=%d to WLAN_TIDNO_VOICE", pst_rtp_hdr->us_RTP_idx,  *puc_tid );
                *puc_tid = WLAN_TIDNO_VOICE;
            }
            ...
            {
                OAM_WARNING_LOG2(0,0, "hmac_tx_traffic_classify:rtp seq=0x%x tid=%d to WLAN_TIDNO_VIDEO", pst_rtp_hdr->us_RTP_idx,  *puc_tid );
                *puc_tid = WLAN_TIDNO_VIDEO;
            }
        }
    }
