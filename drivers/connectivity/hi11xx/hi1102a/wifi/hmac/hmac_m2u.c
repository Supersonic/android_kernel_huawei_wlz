                                          ...
                                          oal_uint32 ul_nowtimestamp)
{
    ...
    oal_dlist_head_stru      *pst_grp_member_entry = OAL_PTR_NULL;
    oal_dlist_head_stru      *pst_grp_member_entry_temp = OAL_PTR_NULL;
...
{
    hmac_m2u_snoop_list_stru         *pst_snp_list = OAL_PTR_NULL;
    hmac_m2u_grp_list_entry_stru     *pst_grp_list_member = OAL_PTR_NULL;
    oal_dlist_head_stru              *pst_grp_list_entry = OAL_PTR_NULL;
    hmac_m2u_stru                    *pst_m2u = (hmac_m2u_stru *)(pst_hmac_vap->pst_m2u);
...
oal_uint32 hmac_m2u_update_snoop_list(hmac_m2u_list_update_stru *pst_list_entry)
{
    ...
    hmac_m2u_grp_member_stru      *pst_grp_member_list = OAL_PTR_NULL;
    oal_uint32                     ul_ret = OAL_SUCC;
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_grp_list)) {
        OAM_WARNING_LOG0(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                         "{hmac_m2u_update_snoop_list::pst_grp_list null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 如果待更新的节点有指定的组播源，如果链表中存在指定该组播源的该节点，则更新该节点的状态，否则重新创建加入链表 */
    if (pst_list_entry->ul_src_ip_addr) {
        OAM_INFO_LOG4(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                      "{hmac_m2u_update_snoop_list::pst_list_entry is assigned src_ip_addr [%x].[%x].[%x].[%x]}\r\n",
        ...
        if (OAL_PTR_NULL != pst_grp_member_list) {
            ...
            if (pst_grp_member_list->ul_src_ip_addr) {
                ...
                if (HMAC_M2U_IGMP_CMD_ADD_INC_LIST == pst_grp_member_list->en_mode) {
                    OAM_INFO_LOG1(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                                  "{hmac_m2u_update_snoop_list::pst_grp_member_list->en_mode is [%d].}",
                                  pst_grp_member_list->en_mode);
                    ul_ret = hmac_m2u_add_member_list(pst_grp_list, pst_list_entry);
                }
            }
            /* 原组中的该节点未指定组播源，如果待更新节点的CMD为EXC，则删除原组播组中的该节点 */
                ...
                OAM_INFO_LOG1(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                              "{hmac_m2u_update_snoop_list::pst_list_entry->en_cmd is [%d].}",
                              pst_list_entry->en_cmd);
        ...
        /* 特定组中不存在该节点，并且更新节点的CMD不为EXC则ADD */
            ...
            if (HMAC_M2U_IGMP_CMD_ADD_EXC_LIST != pst_list_entry->en_cmd) {
                OAM_INFO_LOG1(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                              "{hmac_m2u_update_snoop_list::pst_grp_member_list == NULL && pst_list_entry->en_cmd is [%d].}",
                              pst_list_entry->en_cmd);
                ul_ret = hmac_m2u_add_member_list(pst_grp_list, pst_list_entry);
            }
        }
...
oal_void hmac_m2u_show_snoop_deny_table(hmac_vap_stru *pst_hmac_vap)
{
    ...
    for (uc_idx = 0; uc_idx < pst_snp_list->uc_deny_count; uc_idx++) {
        ...
        OAM_INFO_LOG4(0, OAM_SF_M2U,
                      "{hmac_m2u_show_snoop_deny_table:: %d - deny addr = %x.%x.%x.%x}\r\n",
        ...
        /* 打印黑名单的子网掩码 */
        OAM_INFO_LOG4(0, OAM_SF_M2U,
                      "{hmac_m2u_show_snoop_deny_table:: deny mask = %x.%x.%x.%x}\r\n",
    ...
    if ((WLAN_VAP_MODE_BSS_AP == pst_hmac_vap->st_vap_base_info.en_vap_mode) &&
        ...
        OAL_DLIST_SEARCH_FOR_EACH(pst_grp_list_entry, &(pst_snp_list->st_grp_list))
        {
            ...
            OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_grp_member_entry, pst_grp_member_entry_temp, &(pst_grp_list_member->st_src_list)) {
                ...
                memset_s(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), 0, OAL_SIZEOF(ac_tmp_buff));
                OAL_SPRINTF(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff),
                            "member_mac_addr and mode = [%x]:[%x]:[%x]:[%x]:[%x]:[%x]:%d\n",
        ...
        OAM_WARNING_LOG2(0, OAM_SF_M2U,
                         "{hmac_m2u_print_all_snoop_list::en_snoop_enable en_vap_mode = [%d].[%d]}\r\n",
...
oal_uint32 hmac_m2u_check(hmac_vap_stru *pst_hmac_vap, mac_ether_header_stru *pst_ether_header)
{
    ...
    if (OAL_HOST2NET_SHORT(ETHER_TYPE_IP) != pst_ether_header->us_ether_type) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                         "{hmac_m2u_check::not ip protocol:ether_type is [0x%x].}",
                         OAL_HOST2NET_SHORT(pst_ether_header->us_ether_type));
        return OAL_FAIL;
    }
    /* lint +e778 */
    return ul_ret;
...
oal_uint32 hmac_m2u_igmp_v1v2_update(hmac_vap_stru *pst_hmac_vap,
                                        hmac_m2u_list_update_stru *pst_list_entry,
                                        mac_igmp_header_stru *pst_igmp)
{
    ...
    if (hmac_m2u_snoop_is_denied(pst_hmac_vap, ul_group_addr)) {
        OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                         "{hmac_m2u_igmp_v1v2_update::ul_group_addr [%x].[%x].[%x].[%x] is denied}\r\n}",
...
oal_uint32 hmac_m2u_igmp_v3_update(hmac_vap_stru *pst_hmac_vap, hmac_m2u_list_update_stru *pst_list_entry, mac_igmp_v3_report_stru *pst_igmpr3)
{
    ...
    oal_uint32                          *pul_src_addr = OAL_PTR_NULL;                      /* 组播源的IP地址 */
    mac_igmp_v3_grec_stru               *pst_grec = OAL_PTR_NULL;                          /* igmp group record */
    hmac_m2u_grp_list_entry_stru        *pst_grp_list = OAL_PTR_NULL;
    hmac_m2u_grp_member_stru            *pst_grp_member_list = OAL_PTR_NULL;
    oal_uint32                           ul_ret = OAL_SUCC;
    ...
    while (us_no_grec) {
        ...
        if (hmac_m2u_snoop_is_denied(pst_hmac_vap, ul_group_addr)) {
            OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                             "{hmac_m2u_igmp_v3_update::ul_group_addr [%x].[%x].[%x].[%x] is denied}\r\n}",
                             (oal_uint32)((ul_group_addr >> 24) & 0xff),
                             (oal_uint32)((ul_group_addr >> 16) & 0xff),
                             (oal_uint32)((ul_group_addr >> 8) & 0xff),
                             (oal_uint32)((ul_group_addr)&0xff));
        ...
        if (!IS_IGMPV3_MODE(pst_grec->uc_grec_type)) {
            OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                             "{hmac_m2u_igmp_v3_update::uc_grec_type is [%x] not inside the six cmd.}",
                             pst_grec->uc_grec_type);
            /* move the grec to next group record */
            pst_grec = (mac_igmp_v3_grec_stru *)((oal_uint8 *)pst_grec + IGMPV3_GRP_REC_LEN(pst_grec));
            us_no_grec--;
            continue;
...
oal_void hmac_m2u_snoop_inspecting(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user, oal_netbuf_stru *pst_buf)
{
    ...
    mac_ip_header_stru                   *pst_ip_hdr = OAL_PTR_NULL;
    mac_igmp_header_stru                 *pst_igmp = OAL_PTR_NULL;                             /* igmp header for v1 and v2 */
    mac_igmp_v3_report_stru              *pst_igmpr3 = OAL_PTR_NULL;                           /* igmp header for v3 */
    oal_uint8                            *puc_src_addr = OAL_PTR_NULL;                         /* source address which send the report and it is the member */
...
OAL_STATIC oal_void hmac_m2u_clean_snp_list(hmac_vap_stru *pst_hmac_vap)
{
    ...
    hmac_m2u_grp_list_entry_stru  *pst_grp_list = OAL_PTR_NULL;
    oal_dlist_head_stru           *pst_grp_list_entry = OAL_PTR_NULL;
    oal_dlist_head_stru           *pst_grp_list_entry_temp = OAL_PTR_NULL;
...
{
    mcast_tunnel_hdr_stru  *pst_mcast_tunHdr = OAL_PTR_NULL;
...
oal_uint32 hmac_m2u_snoop_convert(hmac_vap_stru *pst_vap, oal_netbuf_stru *pst_buf)
{
    ...
    oal_uint8              *puc_dstmac = OAL_PTR_NULL;
    ...
    mac_ip_header_stru     *pst_ip_header = OAL_PTR_NULL;
    ...
    if (hmac_m2u_snoop_is_denied(pst_vap, ul_grp_addr)) {
        OAM_INFO_LOG4(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                      "{hmac_m2u_snoop_convert::group_addr [%x].[%x].[%x].[%x] is denied}\r\n}",
    ...
    {
        OAM_INFO_LOG2(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                      "{hmac_m2u_snoop_convert:: %d user found , mcast mode is %d}",
                      uc_newmaccnt, pst_m2u->en_mcast_mode);
        OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_mcast_cnt, 1);
        return HMAC_TX_PASS;
    }
    ...
    /* 如果没有STA需要此netbuf,则不需要发送，丢弃该组播帧 */
        ...
        OAM_INFO_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                      "{hmac_m2u_snoop_convert:: %d user found}", uc_newmaccnt);
        OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_mcast_droped, 1);
    ...
    do {
        ...
        if (!oal_compare_mac_addr(puc_dstmac, auc_empty_entry_mac)) {
            OAM_INFO_LOG2(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                          "{hmac_m2u_snoop_convert::dstmac == NULL && newmaccnt is %d && discard mcast is %d}",
                          uc_newmaccnt, pst_m2u->en_discard_mcast);
        ...
        if (OAL_UNLIKELY(MAC_USER_STATE_ASSOC != pst_user->st_user_base_info.en_user_asoc_state)) {
            ...
            OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                             "{hmac_m2u_snoop_convert::asoc state is [%d].}",
                             pst_user->st_user_base_info.en_user_asoc_state);
            /* 组播转单播发送循环的末尾处理 */
            hmac_m2u_convert_loop_end(pst_copy_buf, &pst_buf, &uc_newmaccnt, &uc_newmacidx);
            continue;
        }
        ...
        if (uc_newmaccnt > 0) {
            ...
            if (OAL_UNLIKELY(HMAC_TX_PASS != ul_ret)) {
                if ((HMAC_TX_BUFF != ul_ret)) {
                    ...
                    OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                                     "{hmac_m2u_snoop_convert::hmac_tx_ucast_process not pass or buff, ul_ret = [%d]}", ul_ret);
                    OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_ucast_droped, 1);
                    hmac_free_netbuf_list(pst_buf);
                }
...
oal_void hmac_m2u_attach(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_m2u)) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                       "{hmac_m2u_attach::pst_m2u is null!}");
        return;
    }
    ...
    memset_s(pst_m2u, OAL_SIZEOF(hmac_m2u_stru), 0, OAL_SIZEOF(hmac_m2u_stru));
    /* 启动定时器 */
