                                               ...
                                               hmac_user_stru *pst_hmac_user)
{
    ...
    /* 关联的STA mac地址 */
    oal_memcopy((oal_uint8 *)pst_asoc_user_req_info->auc_user_mac_addr,
                pst_hmac_user->st_user_base_info.auc_user_mac_addr,
                WLAN_MAC_ADDR_LEN);
#else
    /* 去关联的STA mac地址 */
    oal_memcopy((oal_uint8 *)pst_event->auc_event_data,
                pst_hmac_user->st_user_base_info.auc_user_mac_addr,
                WLAN_MAC_ADDR_LEN);
#endif
...
OAL_STATIC oal_void hmac_ap_rx_auth_req(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_auth_req, dmac_wlan_crx_event_stru *pst_mgmt_rx_event)
{
    ...
    if (OAL_PTR_NULL == pst_hmac_vap || OAL_PTR_NULL == pst_auth_req || OAL_PTR_NULL == pst_mgmt_rx_event) {
        OAM_ERROR_LOG3(0, OAM_SF_AUTH,
                       "{hmac_ap_rx_auth_req::param null, %x %x %x.}",
    ...
    if (OAL_SUCC != ul_ret) {
        oal_netbuf_free(pst_auth_rsp);
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                         "{hmac_ap_rx_auth_req::hmac_tx_mgmt_send_event failed[%d].}", ul_ret);
    }
}
...
OAL_STATIC oal_uint32 hmac_ap_process_obss_erp_ie(hmac_vap_stru *pst_hmac_vap,
                                                  oal_uint8 *puc_payload,
                                                  oal_uint32 ul_payload_len)
{
    /* 存在non erp站点 */
...
OAL_STATIC oal_uint32 hmac_ap_process_obss_ht_operation_ie(hmac_vap_stru *pst_hmac_vap,
                                                           oal_uint8 *puc_payload,
                                                           oal_uint32 ul_payload_len)
{
    /* 如果存在non-ht站点 */
                                                               ...
                                                               mac_status_code_enum_uint16 *pen_status_code)
{
    ...
    for (us_pmkid_idx = 0; us_pmkid_idx < us_pmkid_count; us_pmkid_idx++) {
        if (OAL_FALSE == MAC_IE_REAMIN_LEN_IS_ENOUGH(2, uc_index, puc_ie[1], 16)) {
...
OAL_STATIC oal_uint32 hmac_ap_up_update_asoc_entry_prot(oal_uint8 *puc_mac_hdr,
                                                 ...
                                                 mac_status_code_enum_uint16 *pen_status_code)
{
    ...
    ***************************************************************************/
    ...
    if (OAL_PTR_NULL != puc_ie_tmp) {
        ...
        if (0 != oal_memcmp(&puc_ie_tmp[2], st_cfg_ssid.ac_ssid, st_cfg_ssid.uc_ssid_len)) {
            *pen_status_code = MAC_UNSPEC_FAIL;
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
...
OAL_STATIC oal_uint32 hmac_check_sa_query_trigger(hmac_user_stru *pst_hmac_user,
                                                  hmac_vap_stru *pst_hmac_vap,
                                                  mac_status_code_enum_uint16 *pen_status_code)
{
    oal_uint32 ul_rslt;
...
OAL_STATIC oal_bool_enum_uint8 hmac_ap_update_2040_chan_info(mac_device_stru *pst_mac_device,
                                                             mac_vap_stru *pst_mac_vap,
                                                             oal_uint8 *puc_payload,
...
OAL_STATIC oal_void hmac_ap_up_rx_obss_beacon(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if ((OAL_TRUE == mac_mib_get_FortyMHzOperationImplemented(&(pst_hmac_vap->st_vap_base_info))) &&
        ...
        (OAL_TRUE == pst_hmac_vap->st_vap_base_info.st_cap_flag.bit_2040_autoswitch)) {
        ...
        case MAC_ACTION_CATEGORY_PUBLIC: {
            ...
            switch (puc_data[MAC_ACTION_OFFSET_ACTION]) {
                ...
                case MAC_PUB_COEXT_MGMT:
                    if (OAL_TRUE == pst_hmac_vap->st_vap_base_info.st_cap_flag.bit_2040_autoswitch) {
...
oal_uint32 hmac_ap_up_rx_mgmt(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    if ((OAL_PTR_NULL == pst_hmac_vap) || (OAL_PTR_NULL == p_param)) {
        OAM_ERROR_LOG2(0, OAM_SF_RX,
                       "{hmac_ap_up_rx_mgmt::param null, %x %x.}",
...
OAL_STATIC oal_void hmac_ap_store_network(mac_device_stru *pst_mac_device, oal_netbuf_stru *pst_netbuf)
{
    ...
    if ((OAL_FALSE == en_already_present) &&
