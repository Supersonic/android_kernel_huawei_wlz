...
extern oal_uint32 g_aul_cpumaxfreq[OAL_BUS_MAXCPU_NUM];
#endif
...
oal_void hmac_wifi_statistic_rx_amsdu(oal_uint32 ul_frame_len)
{
    if (ul_frame_len > WLAN_PKT_MEM_OPT_MIN_PKT_LEN) {
        g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_large_amsdu_pkt++;
    } else {
        g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_small_amsdu_pkt++;
    }
}
...
{
    g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_small_pkt += ul_filtered_small_pkt_cnt;
    g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_large_pkt += ul_filtered_large_pkt_cnt;
    g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_bytes += ul_filtered_rx_bytes;
}


oal_void hmac_wifi_statistic_rx_packets(oal_uint32 ul_pkt_bytes)
{
    g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_bytes += ul_pkt_bytes;

    if (ul_pkt_bytes <= WLAN_PKT_MEM_OPT_MIN_PKT_LEN)
    {
        g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_small_pkt++;
    }
    else
    {
        g_st_wifi_rxtx_statis.st_pkgs_info.ul_rx_large_pkt++;
    }
}
...
{
    g_st_wifi_rxtx_statis.st_pkgs_info.ul_tx_bytes += ul_pkt_bytes;

    if (ul_pkt_bytes <= WLAN_PKT_MEM_OPT_MIN_PKT_LEN)
    {
        g_st_wifi_rxtx_statis.st_pkgs_info.ul_tx_small_pkt++;
    }
    else
    {
        g_st_wifi_rxtx_statis.st_pkgs_info.ul_tx_large_pkt++;
    }
}
...
/* 所有核都锁定最高频率 */
    ...
    {
        hisi_cpufreq_update_req(&g_ast_cpufreq[uc_cpuid], g_aul_cpumaxfreq[uc_cpuid]);
        OAM_WARNING_LOG2(0, OAM_SF_ANY, "hmac_thread_bind_fast_cpu:lock cpu freq cpu[%d]->%d",uc_cpuid,g_aul_cpumaxfreq[uc_cpuid]);
    }
#endif
#endif
}
...
oal_void hmac_thread_bindcpu(oal_uint32 ul_total_pps)
{
    ...
    if (OAL_TRUE == g_st_thread_bindcpu.en_btcoex_flag)
    {
...
#if defined(_PRE_FEATURE_PLAT_LOCK_CPUFREQ) && !defined(CONFIG_HI110X_KERNEL_MODULES_BUILD_SUPPORT)
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
        core_ctl_set_boost(2*WLAN_STATIS_DATA_TIMER_PERIOD);
    ...
    {
        OAM_WARNING_LOG3(0,OAM_SF_ANY, "{hmac_rxdata_thread_bindcpu:rxdata_thread bind cpu:[%d] total_pps[%d],btcoex_flag[%d]}",
            en_irq_cpu_state, ul_total_pps, g_st_thread_bindcpu.en_btcoex_flag);

        /* 流量较大时将收发线程绑定在大核 */
        if (WLAN_IRQ_AFFINITY_BUSY_CPU == en_irq_cpu_state)
        {
...
OAL_STATIC oal_void hmac_wifi_reset_stat(oal_void)
{
    ...
    OAL_MEMZERO(&g_st_wifi_rxtx_statis.st_pkgs_info, OAL_SIZEOF(wifi_statis_pkgs_info_stru));
}
...
OAL_STATIC oal_uint32 hmac_wifi_calculate_stat(wifi_txrx_calculated_stat_stru *pst_calculated_stat)
{
    ...
    pst_calculated_stat->ul_tx_large_pps = (pst_pkgs_info->ul_tx_large_pkt * 1000) / ul_dur_ms;
    pst_calculated_stat->ul_rx_large_pps = (pst_pkgs_info->ul_rx_large_pkt * 1000) / ul_dur_ms;
    pst_calculated_stat->ul_tx_small_pps = (pst_pkgs_info->ul_tx_small_pkt * 1000) / ul_dur_ms;
    pst_calculated_stat->ul_rx_small_pps = (pst_pkgs_info->ul_rx_small_pkt * 1000) / ul_dur_ms;
    pst_calculated_stat->ul_rx_throughput_mbps = (pst_pkgs_info->ul_rx_bytes >> 7) / ul_dur_ms;
    pst_calculated_stat->ul_tx_throughput_mbps = (pst_pkgs_info->ul_tx_bytes >> 7) / ul_dur_ms;
    pst_calculated_stat->ul_rx_large_amsdu_pps = (pst_pkgs_info->ul_rx_large_amsdu_pkt * 1000) / ul_dur_ms;
...
oal_void hmac_wifi_calculate_throughput(oal_void)
{
    ...
    if (OAL_SUCC != hmac_wifi_calculate_stat(&st_calculated_stat))
    {
        return;
    }
