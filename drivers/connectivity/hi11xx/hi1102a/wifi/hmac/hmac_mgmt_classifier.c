...
oal_uint32 hmac_mgmt_send_disasoc_deauth_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_vap)) {
        OAM_ERROR_LOG0(pst_event_hdr->uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_mgmt_send_disasoc_deauth_event::pst_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (ul_rslt != OAL_SUCC) {
        OAM_WARNING_LOG4(0, OAM_SF_RX,
                         "{hmac_mgmt_send_disasoc_deauth_event::Hmac cannot find USER by addr[%02X:XX:XX:%02X:%02X:%02X], just del DMAC user}",
                         puc_da[0],
                         puc_da[3],
                         puc_da[4],
                         puc_da[5]);
...
oal_uint32 hmac_proc_disasoc_misc_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (WLAN_VAP_MODE_BSS_AP == pst_hmac_vap->st_vap_base_info.en_vap_mode) {
        ...
        if (OAL_PTR_NULL == pst_hmac_user) {
            OAM_WARNING_LOG1(0, OAM_SF_ASSOC,
                             "{hmac_proc_disasoc_misc_event ap::pst_hmac_user[%d] is null.}",
                             pdmac_disasoc_misc_stru->us_user_idx);
            return OAL_ERR_CODE_PTR_NULL;
        }
        ...
        if (OAL_PTR_NULL == pst_hmac_user) {
            OAM_WARNING_LOG1(0, OAM_SF_ASSOC,
                             "{hmac_proc_disasoc_misc_event::pst_hmac_user[%d] is null.}",
