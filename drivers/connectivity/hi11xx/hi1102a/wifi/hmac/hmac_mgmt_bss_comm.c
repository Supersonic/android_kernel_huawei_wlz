...
#endif
#if defined(_PRE_WLAN_FEATURE_FTM_RAM) || defined(_PRE_WLAN_FEATURE_LOCATION_RAM)
...
oal_uint16 hmac_btcoex_get_ba_size(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    if (MAC_IS_XIAOMI_R1D(pst_hmac_user->st_user_base_info.auc_user_mac_addr) ||
        ...
        if ((WLAN_BAND_5G == pst_hmac_vap->st_vap_base_info.st_channel.en_band) &&
            (WLAN_BAND_WIDTH_80PLUSPLUS <= pst_hmac_vap->st_vap_base_info.st_channel.en_bandwidth) &&
            (WLAN_BAND_WIDTH_80MINUSMINUS >= pst_hmac_vap->st_vap_base_info.st_channel.en_bandwidth)) {
            /* 如果非电话结束场景 */
            if (HMAC_BA_SIZE_64 != pst_hmac_user_btcoex->uc_ba_size) {
                us_ba_size = HMAC_BA_SIZE_2;
            }
        }
    }
    if (BTCOEX_BA_TYPE_SIZE_1 == pst_hmac_user->st_hmac_user_btcoex.en_ba_type) {
                                   ...
                                   oal_uint16 *pus_ba_param)
{
    ...
    pst_hmac_user_btcoex = &(pst_hmac_user->st_hmac_user_btcoex);
    if ((OAL_TRUE == pst_hmac_user_btcoex->en_delba_btcoex_trigger) && (WLAN_AMPDU_RX_BUFFER_SIZE != pst_hmac_user_btcoex->uc_ba_size)) {
        *pus_ba_param &= (~BIT0);
    }
    if (WLAN_VAP_MODE_BSS_STA == pst_hmac_vap->st_vap_base_info.en_vap_mode) {
        ...
        } else if ((OAL_TRUE == pst_hmac_user_btcoex->en_delba_btcoex_trigger) && (0 != pst_hmac_user_btcoex->uc_ba_size)) {
            us_ba_size = hmac_btcoex_get_ba_size(pst_hmac_vap, pst_hmac_user);
            if (0 == us_ba_size) {
                us_ba_size = (oal_uint16)pst_hmac_user_btcoex->uc_ba_size;
            } else {
                us_ba_size = (us_ba_size > pst_hmac_user_btcoex->uc_ba_size ? pst_hmac_user_btcoex->uc_ba_size : us_ba_size);
            }
                                     ...
                                     oal_uint8 uc_tid)
{
    ...
    if ((OAL_PTR_NULL == pst_vap) || (OAL_PTR_NULL == puc_data) || (OAL_PTR_NULL == pst_tx_ba)) {
        OAM_ERROR_LOG3(0, OAM_SF_BA, "{hmac_mgmt_encap_addba_req::null param.vap:%x data:%x ba:%x}", (uintptr_t)pst_vap, (uintptr_t)puc_data, (uintptr_t)pst_tx_ba);
                                     ...
                                     oal_uint8 uc_status)
{
    ...
    if ((OAL_PTR_NULL == pst_vap) || (OAL_PTR_NULL == puc_data) || (OAL_PTR_NULL == pst_addba_rsp)) {
        OAM_ERROR_LOG3(0, OAM_SF_BA, "{hmac_mgmt_encap_addba_req::null param.vap:%x data:%x rsp:%x}", (uintptr_t)pst_vap, (uintptr_t)puc_data, (uintptr_t)pst_addba_rsp);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_PTR_NULL != pst_hmac_user) {
        hmac_btcoex_set_ba_size(pst_vap, pst_hmac_user, pst_addba_rsp, &us_ba_param);
    } else {
    ...
    us_ba_param |= (oal_uint16)(pst_addba_rsp->us_baw_size << 6); /* BIT6 */
#endif
    OAM_WARNING_LOG2(0, OAM_SF_BA, "hmac_mgmt_encap_addba_rsp:tid[%d],ba_size[%d]", uc_tid, ((us_ba_param & 0xffc0) >> 6));
    puc_data[us_index++] = (oal_uint8)(us_ba_param & 0xFF);
    puc_data[us_index++] = (oal_uint8)((us_ba_param >> 8) & 0xFF);
                                 ...
                                 oal_uint8 reason)
{
    ...
    if ((OAL_PTR_NULL == pst_vap) || (OAL_PTR_NULL == puc_data) || (OAL_PTR_NULL == puc_addr)) {
        OAM_ERROR_LOG3(0, OAM_SF_BA, "{hmac_mgmt_encap_delba::null param, %x %x %x.}", (uintptr_t)pst_vap, (uintptr_t)puc_data, (uintptr_t)puc_addr);
        return OAL_ERR_CODE_PTR_NULL;
    }
                                  ...
                                  mac_action_mgmt_args_stru *pst_action_args)
{
    ...
    if ((OAL_PTR_NULL == pst_hmac_vap) || (OAL_PTR_NULL == pst_hmac_user) || (OAL_PTR_NULL == pst_action_args)) {
        OAM_ERROR_LOG3(0, OAM_SF_BA, "{hmac_mgmt_tx_addba_req::null param, %x %x %x.}", (uintptr_t)pst_hmac_vap, (uintptr_t)pst_hmac_user, (uintptr_t)pst_action_args);
        return OAL_ERR_CODE_PTR_NULL;
    }
                                  ...
                                  oal_uint8 uc_status)
{
    ...
    if ((OAL_PTR_NULL == pst_hmac_vap) || (OAL_PTR_NULL == pst_hmac_user) || (OAL_PTR_NULL == pst_ba_rx_info)) {
        OAM_ERROR_LOG3(0, OAM_SF_BA,
                       "{hmac_mgmt_tx_addba_rsp::send addba rsp failed, null param, %x %x %x.}",
...
oal_uint32 hmac_mgmt_tx_delba(hmac_vap_stru *pst_hmac_vap,
                              hmac_user_stru *pst_hmac_user,
                              mac_action_mgmt_args_stru *pst_action_args)
{
    mac_device_stru                  *pst_device = OAL_PTR_NULL;
    ...
    if ((OAL_PTR_NULL == pst_hmac_vap) || (OAL_PTR_NULL == pst_hmac_user) || (OAL_PTR_NULL == pst_action_args)) {
        OAM_ERROR_LOG3(0, OAM_SF_BA,
                       "{hmac_mgmt_tx_delba::null param, %x %x %x.}",
    ...
    if (MAC_VAP_STATE_BUTT == pst_mac_vap->en_vap_state) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_BA,
                         "{hmac_mgmt_tx_delba:: vap has been down/del, vap_state[%d].}", pst_mac_vap->en_vap_state);
        return OAL_FAIL;
    }
    ...
    /* 调用封装管理帧接口 */
    us_frame_len = hmac_mgmt_encap_delba(pst_hmac_vap, (oal_uint8 *)OAL_NETBUF_HEADER(pst_delba), pst_action_args->puc_arg5, uc_tidno, en_initiator, (oal_uint8)pst_action_args->ul_arg3);
    memset_s((oal_uint8*)&st_wlan_ctx_action, OAL_SIZEOF(st_wlan_ctx_action), 0, OAL_SIZEOF(st_wlan_ctx_action));
    st_wlan_ctx_action.us_frame_len        = us_frame_len;
    st_wlan_ctx_action.uc_hdr_len          = MAC_80211_FRAME_LEN;
    st_wlan_ctx_action.en_action_category  = MAC_ACTION_CATEGORY_BA;
                                  ...
                                  oal_uint8 *puc_payload)
{
    ...
    if ((OAL_PTR_NULL == pst_hmac_vap) || (OAL_PTR_NULL == pst_hmac_user) || (OAL_PTR_NULL == puc_payload)) {
        OAM_ERROR_LOG3(0, OAM_SF_BA,
                       "{hmac_mgmt_rx_addba_req::addba req receive failed, null param, 0x%x 0x%x 0x%x.}",
    ...
    if (OAL_PTR_NULL == pst_hmac_user->ast_tid_info[uc_tid].pst_ba_rx_info) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_BA,
                         "{hmac_mgmt_rx_addba_req::addba req receive failed, pst_ba_rx_hdl mem alloc failed.tid[%d]}", uc_tid);
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_BA,
                     "{hmac_mgmt_rx_addba_req::process addba req receive and send addba rsp, uc_tid[%d], uc_status[%d], baw_start[%d], baw_size[%d]}\r\n",
                                  ...
                                  oal_uint8 *puc_payload)
{
    ...
    if (DMAC_BA_COMPLETE == pst_tid->st_ba_tx_info.en_ba_status) {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_BA,
                         "{hmac_mgmt_rx_addba_rsp::addba rsp is received whne ba status is DMAC_BA_COMPLETE or uc_dialog_token wrong.tid[%d], status[%d], rsp dialog[%d], req dialog[%d]}",
...
oal_uint32 hmac_start_sa_query(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user, oal_bool_enum_uint8 en_is_protected)
{
    ...
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_start_sa_query::SA query, trans_id %d.}", pst_hmac_user->st_sa_query_info.us_sa_query_trans_id);
...
oal_void hmac_mgmt_send_deauth_frame(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_da, oal_uint16 us_err_code, oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (OAL_PTR_NULL == pst_mac_vap || OAL_PTR_NULL == puc_da) {
        OAM_ERROR_LOG2(0, OAM_SF_AUTH,
                       "{hmac_mgmt_send_deauth_frame::param null, %x %x.}",
...
oal_void hmac_mgmt_send_disassoc_frame(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_da, oal_uint16 us_err_code, oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (OAL_PTR_NULL == pst_mac_vap || OAL_PTR_NULL == puc_da) {
        OAM_ERROR_LOG2(0, OAM_SF_ASSOC,
                       "{hmac_mgmt_send_disassoc_frame::param null, %x %x.}",
                       (uintptr_t)pst_mac_vap, (uintptr_t)puc_da);
        return;
    }
                                ...
                                oal_int l_freq)
{
    ...
    if (OAL_PTR_NULL == pst_mgmt_data) {
        FRW_EVENT_FREE(pst_event_mem);
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
                       "{hmac_send_mgmt_to_host::pst_mgmt_data null.}");
        return;
    }
...
OAL_STATIC oal_bool_enum_uint8 hmac_sae_auth_frame(oal_netbuf_stru *pst_netbuf)
{
    oal_uint8 *puc_mac_hdr = oal_netbuf_data(pst_netbuf);

    return mac_frame_get_type_value(puc_mac_hdr) == WLAN_MANAGEMENT &&
           mac_frame_get_subtype_value(puc_mac_hdr) == WLAN_AUTH &&
           mac_get_auth_alg(puc_mac_hdr) == WLAN_MIB_AUTH_ALG_SAE;
}


OAL_STATIC oal_uint32 hmac_wpas_sae_auth_update_cb(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_netbuf)
{
    hmac_user_stru  *pst_hmac_user_ap;
    mac_tx_ctl_stru *pst_tx_ctl;

    if (!hmac_sae_auth_frame(pst_netbuf)) {
        return OAL_SUCC;
    }

    pst_hmac_user_ap = mac_res_get_hmac_user(pst_mac_vap->uc_assoc_vap_id);
...
#ifdef _PRE_WLAN_FEATURE_SAE
    if (hmac_wpas_sae_auth_update_cb(pst_mac_vap, pst_netbuf_mgmt_tx) != OAL_SUCC) {
        oal_netbuf_free(pst_netbuf_mgmt_tx);
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "{hmac_wpas_mgmt_tx::sae auth cb update failed}");
        return OAL_FAIL;
    }
#endif
...
oal_uint32 hmac_proc_ht_cap_ie(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, oal_uint8 *puc_ht_cap_ie)
{
    ...
    for (uc_mcs_bmp_index = 0; uc_mcs_bmp_index < WLAN_HT_MCS_BITMASK_LEN; uc_mcs_bmp_index++) {
        pst_ht_hdl->uc_rx_mcs_bitmask[uc_mcs_bmp_index] = (*(oal_uint8 *)(puc_ht_cap_ie + us_offset + uc_mcs_bmp_index));
    }
...
OAL_STATIC oal_uint32 hmac_netlink_location_send(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_hmac_vap || OAL_PTR_NULL == pst_netbuf || OAL_PTR_NULL == puc_send_csi_buf || OAL_PTR_NULL == puc_send_ftm_buf)) {
        OAM_ERROR_LOG0(0, OAM_SF_FTM, "{hmac_netlink_location_send: param NULL}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    pst_rx_ctrl = (mac_rx_ctl_stru *)oal_netbuf_cb(pst_netbuf);
    pst_location_event = (mac_location_event_stru *)((oal_uint8 *)pst_rx_ctrl->pul_mac_hdr_start_addr + pst_rx_ctrl->uc_mac_header_len);
    ...
    switch (pst_location_event->uc_location_type) {
        ...
        case MAC_HISI_LOCATION_CSI_IE:
            ...
            OAM_WARNING_LOG2(0, OAM_SF_FTM,
                             "{hmac_netlink_location_send::len[%d], copy_len[%d].}",
                             *pul_len, us_action_len - MAC_CSI_LOCATION_INFO_LEN);
            OAM_WARNING_LOG3(0, OAM_SF_FTM,
                             "{hmac_netlink_location_send::frag [%d], [%d] ,[%d].}",
...
OAL_STATIC oal_uint32 hmac_proc_location_action(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (OAL_UNLIKELY((OAL_PTR_NULL == pst_hmac_vap) || (OAL_PTR_NULL == pst_netbuf))) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_proc_location_action: input pointer is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (IS_ERR_OR_NULL(f_file)) {
        OAM_ERROR_LOG1(0, OAM_SF_FTM,
                       "{hmac_proc_location_action: ****************save file failed %d }", l_str_len);
    ...
    if (MAC_HISI_LOCATION_CSI_IE == pst_location_event->uc_location_type) {
        if ((0 == pst_location_event->auc_payload[0]) &&
            ...
            oal_kernel_file_print(f_file, "%04d-%02d-%02d-", st_local_time.tm_year + 1900, st_local_time.tm_mon + 1, st_local_time.tm_mday);
            oal_kernel_file_print(f_file, "%02d-%02d-%02d-%08d : ", st_local_time.tm_hour, st_local_time.tm_min, st_local_time.tm_sec, st_tv.tv_usec);
            /* rssi snr */
            puc_payload = (oal_uint8 *)(pst_location_event->auc_payload);
            l_str_len = 0;
