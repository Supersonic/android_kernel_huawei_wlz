...
oal_uint32 hmac_config_fbt_set_mode(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (uc_fbt_cfg_mode == HMAC_FBT_MODE_CLOSE) {
        for (uc_user_index = 0; uc_user_index < HMAC_FBT_MAX_USER_NUM; uc_user_index++) {
            memset_s(pst_hmac_fbt_mgmt->ast_fbt_disable_connect_user_list + uc_user_index, 
                OAL_SIZEOF(hmac_fbt_disable_user_info_stru), 0, OAL_SIZEOF(hmac_fbt_disable_user_info_stru));
        }
...
OAL_STATIC oal_uint32 hmac_fbt_scan_report_timeout(oal_void *p_arg)
{
    ...
    for (uc_user_idx = 0; uc_user_idx < HMAC_FBT_MAX_USER_NUM; uc_user_idx++)  // HMAC_FBT_MAX_USER_NUM 3 for test
    {
        ...
        if (OAL_TRUE == pst_fbt_scan_result->uc_is_found) {
            ...
            pst_fbt_notify->uc_user_rssi = (oal_get_real_rssi(pst_fbt_scan_mgmt->ast_fbt_scan_user_list[uc_user_idx].s_rssi) + HMAC_FBT_RSSI_ADJUST_VALUE);
            ...
            OAM_INFO_LOG4(pst_mac_device->uc_device_id, OAM_SF_HILINK,
                          "{hmac_fbt_scan_report_timeout::uc_user_idx=%d MAC:0x%x:0x%x:0x%x is found!}",
                          uc_user_idx,
            ...
            OAM_INFO_LOG4(pst_mac_device->uc_device_id, OAM_SF_HILINK,
                          "{hmac_fbt_scan_report_timeout::bssid MAC:0x%x:0x%x:0x%x:0x%x.}",
            ...
            OAM_INFO_LOG4(pst_mac_device->uc_device_id, OAM_SF_HILINK,
                          "{hmac_fbt_scan_report_timeout::channel =%d,c_rssi=%d,ul_rssi_timestamp=%d,ul_total_pkt_cnt=%d.}",
                          pst_fbt_scan_mgmt->ast_fbt_scan_user_list[uc_user_idx].uc_user_channel,
                          pst_fbt_scan_mgmt->ast_fbt_scan_user_list[uc_user_idx].s_rssi,
...
oal_uint32 hmac_fbt_start_scan(mac_vap_stru *pst_mac_vap)
{
    ...
    if (HMAC_FBT_SCAN_OPEN == pst_fbt_scan_mgmt->uc_fbt_scan_enable) {
        ...
        for (uc_user_index = 0; uc_user_index < HMAC_FBT_MAX_USER_NUM; uc_user_index++) {
            memset_s(pst_fbt_scan_mgmt->ast_fbt_scan_user_list + uc_user_index, 
                OAL_SIZEOF(mac_fbt_scan_result_stru), 0, OAL_SIZEOF(mac_fbt_scan_result_stru));
        }
...
oal_uint32 hmac_fbt_init(hmac_vap_stru *pst_hmac_vap)
{
     ...
     * 监控已关联用户的时间为5s
     */
    memset_s(pst_hmac_fbt_mgmt, OAL_SIZEOF(hmac_fbt_mgmt_stru), 0, OAL_SIZEOF(hmac_fbt_mgmt_stru));
    ...
    memset_s(pst_fbt_scan_mgmt, OAL_SIZEOF(mac_fbt_scan_mgmt_stru), 0, OAL_SIZEOF(mac_fbt_scan_mgmt_stru));
...
oal_uint32 hmac_fbt_notify(hmac_vap_stru *pst_hmac_vap, hmac_fbt_notify_stru *pst_hmac_fbt_notify)
{
    ...
    if (OAL_SUCC != ul_ret) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_HILINK,
                       "{hmac_fbt_notify::oal_cfg80211_fbt_notify_find_sta fail[%d]!}\r\n",
                       ul_ret);
        return ul_ret;
    }

    OAM_INFO_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_HILINK,
                  "{hmac_fbt_notify::mac[%x:**:**:%x:%x:%x] OK!}\r\n",
                  auc_user_addr[0],
                  auc_user_addr[3],
                  auc_user_addr[4],
                  auc_user_addr[5]);
