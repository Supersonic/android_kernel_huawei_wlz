...
#define FBT_DEFAULT_SCAN_REPORT_PERIOD                  (1000)  /* 默认配置未关联用户侦听上报周期1000ms -- 需要修改为200ms */
#endif
...
typedef struct
{
    ...
    oal_uint32 ul_scan_interval;                /* 配置未关联用户每个信道侦听的时长，单位为毫秒 */
    ...
    frw_timeout_stru st_timer;                  /* 侦听未关联用户使用的定时器 */
...
typedef struct
{
    unsigned char   mac[WLAN_MAC_ADDR_LEN];                     /* 需要侦听的sta的mac地址 */
...
#ifdef _PRE_WLAN_FEATURE_PROXYSTA
    oal_uint8                           auc_vap_id[WLAN_REPEATER_SERVICE_VAP_MAX_NUM_PER_DEVICE];   /* device下的业务vap，此处只记录VAP ID */
#else
...
/* end: P2P */
    ...
    oal_void                            *pst_green_ap_mgr;                     /*device结构下的green ap特性结构体*/
#endif
...
{
    return  (oal_bool_enum_uint8)((OAL_TRUE == pst_device->en_dbac_enabled) && (MAC_VAP_STATE_PAUSE == pst_vap->en_vap_state));
}
#endif
#ifdef _PRE_SUPPORT_ACS
...
extern  mac_fcs_err_enum_uint8    dmac_fcs_start_nb(mac_fcs_mgr_stru *pst_fcs_mgr,
                                                mac_fcs_cfg_stru *pst_fcs_cfg,
                                                hal_one_packet_status_stru *pst_status);
extern oal_uint32  dmac_fcs_get_prot_mode(mac_vap_stru *pst_mac_vap, hal_one_packet_cfg_stru *pst_one_packet_cfg);
#endif
