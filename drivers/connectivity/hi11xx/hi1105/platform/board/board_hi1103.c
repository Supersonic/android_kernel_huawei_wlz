...
#ifdef _PRE_H2D_GPIO_WKUP
    board_info_etc.host_wakeup_bfg = UDP_HOST_WKUP_BFG_GPIO;
    PS_PRINT_INFO("hi110x host wkup bfg gpio is %d\n", board_info_etc.host_wakeup_bfg);
#ifdef GPIOF_OUT_INIT_LOW
    ret = gpio_request_one(board_info_etc.host_wakeup_bfg, GPIOF_OUT_INIT_LOW, PROC_NAME_GPIO_HOST_WAKEUP_BFG);
    if (ret) {
        PS_PRINT_ERR("%s gpio_request failed\n", PROC_NAME_GPIO_HOST_WAKEUP_BFG);
    }
#else
    ret = gpio_request(board_info_etc.host_wakeup_bfg, PROC_NAME_GPIO_HOST_WAKEUP_BFG);
    if (ret) {
        PS_PRINT_ERR("%s gpio_request failed\n", PROC_NAME_GPIO_HOST_WAKEUP_BFG);
    } else {
        gpio_direction_input(board_info_etc.host_wakeup_bfg);
    }
#endif
#else //_PRE_H2D_GPIO_WKUP
    board_info_etc.host_wakeup_bfg = 0;
#endif
...
/* 共享外置buck上下电接口 */
int32  buck_power_ctrl(uint32 enable)
{
#define BUCK_1P15V    1150000
#ifndef WBG_PMIC_REQ
#define WBG_PMIC_REQ (1 << 8)
#endif
    ...
    if (buck_mode_get() == EXT_BUCK_HOST_CTL) {
        ...
        fp_pmic_ldo_set.pmic_num = 0;                       // 目前暂时固定位为0
        fp_pmic_ldo_set.pmic_power_type = VOUT_BUCK_1;      // 控制的BUCK号
        fp_pmic_ldo_set.pmic_power_voltage = BUCK_1P15V;    //要设置的电压，单位 uV
        fp_pmic_ldo_set.pmic_power_state = enable;          // 打开或关闭 0/1

        ret =  hw_pmic_power_cfg(WBG_PMIC_REQ, &fp_pmic_ldo_set);

        PS_PRINT_INFO("share buck mode, buck_ctrl[%d],ret[%d]\n",enable,ret);
    }
 #endif
    return ret;
}
