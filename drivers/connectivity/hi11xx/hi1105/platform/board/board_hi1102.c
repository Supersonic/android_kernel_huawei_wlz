...
static void hi1102_prepare_to_power_on(void)
{
    ...
    if (OAL_IS_ERR_OR_NULL(board_info_etc.pctrl) || OAL_IS_ERR_OR_NULL(board_info_etc.pins_idle)) {
        PS_PRINT_ERR("power pinctrl is err, pctrl is %p, pins_idle is %p\n",
                     board_info_etc.pctrl, board_info_etc.pins_idle);
        return;
    }
#ifdef _PRE_CONFIG_USE_DTS
    /* set LowerPower mode */
    ret = pinctrl_select_state(board_info_etc.pctrl, board_info_etc.pins_idle);
    if (ret != BOARD_SUCC) {
...
#endif
#ifdef GPIOF_OUT_INIT_LOW
    ret = gpio_request_one(board_info_etc.xldo_pinmux, GPIOF_OUT_INIT_LOW, PROC_NAME_GPIO_XLDO_PINMUX);
    if (ret) {
...
#else
    ret = gpio_request(board_info_etc.xldo_pinmux, PROC_NAME_GPIO_XLDO_PINMUX);
    if (ret) {
        PS_PRINT_ERR("%s gpio_request failed\n", PROC_NAME_GPIO_XLDO_PINMUX);
        CHR_EXCEPTION(CHR_WIFI_DEV(CHR_WIFI_DEV_EVENT_CHIP, CHR_WIFI_DEV_ERROR_GPIO));
        return;
    }
    gpio_direction_output(board_info_etc.xldo_pinmux, 0);
#endif

    gpio_direction_output(board_info_etc.xldo_pinmux, board_info_etc.gpio_xldo_level);
    board_info_etc.pinmux_set_result = PINMUX_SET_SUCC;
...
static void hi1102_post_to_power_on(void)
{
    ...
    if (board_info_etc.pinmux_set_result == PINMUX_SET_SUCC) {
        board_info_etc.pinmux_set_result = PINMUX_SET_INIT;
        gpio_free(board_info_etc.xldo_pinmux);
    }

    if (OAL_IS_ERR_OR_NULL(board_info_etc.pctrl) || OAL_IS_ERR_OR_NULL(board_info_etc.pins_normal)) {
        PS_PRINT_ERR("power pinctrl is err, pctrl is %p, pins_idle is %p\n",
                     board_info_etc.pctrl, board_info_etc.pins_normal);
        return;
    }
#ifdef _PRE_CONFIG_USE_DTS
    /* set NORMAL mode */
...
int32 hi1102_wlan_power_off(void)
{
    ...
    if (bfgx_is_shutdown_etc()) {
        ...
        if (uart_wifi_close_etc() != OAL_SUCC) {
...
int32 hi1102_wlan_power_on(void)
{
    ...
    if (bfgx_is_shutdown_etc()) {
        ...
        if (firmware_download_function_etc(BFGX_AND_WIFI_CFG) == WIFI_POWER_SUCCESS) {
        ...
        if (uart_wifi_open_etc() != WIFI_POWER_SUCCESS) {
            ...
            hcc_bus_power_action(hcc_get_current_110x_bus(), HCC_BUS_POWER_PATCH_LOAD_PREPARE);
            if (firmware_download_function_etc(WIFI_CFG) == WIFI_POWER_SUCCESS) {
                pm_data->pst_wlan_pm_info->ul_wlan_power_state = POWER_STATE_OPEN;
            } else {
                PS_PRINT_ERR("firmware download fail\n");
...
{
    int32 ret = -EFAIL;
    uintptr_t gpio = board_info_etc.power_on_enable;
...
int32 hi1102_get_board_uart_port(void)
{
    ...
    PS_PRINT_INFO(" board_info_etc.uart_port=%s\n", board_info_etc.uart_port);
    return BOARD_SUCC;
#else
    return BOARD_SUCC;
#endif
...
int32 hi1102_board_ir_ctrl_pmic_init(struct platform_device *pdev)
{
    ...
    board_info_etc.bfgn_ir_ctrl_ldo = regulator_get(&pdev->dev, DTS_IRLED_LDO_POWER);
...
int32 hi1102_board_ir_ctrl_init(struct platform_device *pdev)
{
    ...
    if (!board_info_etc.have_ir) {
        ...
        ret = of_property_read_u32(np, DTS_PROP_IR_LDO_TYPE, &board_info_etc.irled_power_type);
