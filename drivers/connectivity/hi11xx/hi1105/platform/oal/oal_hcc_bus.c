...
oal_int32 oal_register_gpio_flowctrl_intr_etc(hcc_bus_dev *pst_bus_dev)
{
    ...
    ret = request_irq(flowctrl_irq, hcc_bus_flowctrl_gpio_irq_etc,
...
oal_uint32 hcc_bus_flowctrl_init_etc(oal_uint8 uc_hcc_flowctrl_type)
{
    ...
    /* Step1: 申请管脚 */
    if (board_flowctrl_gpio_init_etc() != BOARD_SUCC) {
    ...
    /* Step3: 使用中断号注册GPIO中断 */
    if (oal_register_gpio_flowctrl_intr_etc(pst_bus_dev) != OAL_SUCC) {
