 ...
 */
/*lint -e695*/
oal_bool_enum_uint8 oal_netbuf_is_dhcp_port_etc(oal_udp_header_stru *pst_udp_hdr)
{
    ...
    switch (pst_ether_header->us_ether_type) {
        ...
        case OAL_HOST2NET_SHORT(ETHER_TYPE_IP):
            ...
            if (pst_ip->uc_protocol == MAC_UDP_PROTOCAL) {
                ...
                if (((pst_ip->us_frag_off & 0xFF1F) == 0) && (oal_netbuf_is_dhcp_port_etc(pst_udp_hdr) == OAL_TRUE)) {
        ...
        case OAL_HOST2NET_SHORT(ETHER_TYPE_IPV6):
            ...
            if (pst_ipv6->nexthdr == MAC_UDP_PROTOCAL) { /* UDP报文 */
                ...
                if (oal_netbuf_is_tcp_ack6_etc(pst_ipv6) == OAL_TRUE) {
                    us_queue = WLAN_TCP_ACK_QUEUE;
                } else {
                    us_queue = WLAN_TCP_DATA_QUEUE;
                }
            ...
            else if (oal_netbuf_is_dhcp6_etc((oal_ipv6hdr_stru *)(pst_ether_header + 1)) == OAL_TRUE) {
                us_queue = WLAN_DATA_VIP_QUEUE;
            }
            break;
...
void dev_netlink_rev_etc(oal_netbuf_stru *skb)
{
    ...
    memset_s(dev_excp_handler_data_etc.data, OAL_EXCP_DATA_BUF_LEN, 0, OAL_EXCP_DATA_BUF_LEN);
    ...
    if (pst_skb->len >= OAL_NLMSG_SPACE(0)) {
        ...
        ret = memcpy_s(dev_excp_handler_data_etc.data, OAL_EXCP_DATA_BUF_LEN, OAL_NLMSG_DATA(pst_nlh), ul_len);
        if (ret != EOK) {
        ...
        ret = memcpy_s((void *)&st_msg_hdr, sizeof(st_msg_hdr), dev_excp_handler_data_etc.data, sizeof(st_msg_hdr));
        if (ret != EOK) {
...
{
    dev_excp_handler_data_etc.nlsk = oal_netlink_kernel_create(&OAL_INIT_NET, NETLINK_DEV_ERROR, 0,
                                                               dev_netlink_rev_etc, OAL_PTR_NULL, OAL_THIS_MODULE);
    if (dev_excp_handler_data_etc.nlsk == OAL_PTR_NULL) {
...
oal_int32 init_dev_excp_handler_etc(oal_void)
{
    ...
    memset_s((oal_uint8 *)&dev_excp_handler_data_etc, OAL_SIZEOF(dev_excp_handler_data_etc),
             0, OAL_SIZEOF(dev_excp_handler_data_etc));

    dev_excp_handler_data_etc.data = (oal_uint8 *)kzalloc(OAL_EXCP_DATA_BUF_LEN, GFP_KERNEL);
    if (dev_excp_handler_data_etc.data == OAL_PTR_NULL) {
