...
OAL_STATIC oal_pcie_res *pci_res = NULL;

OAL_STATIC oal_kobject *conn_syfs_pci_object = NULL;

char *pcie_link_state_str[PCI_WLAN_LINK_BUTT + 1] = {
...
OAL_STATIC oal_pcie_bar_info en_bar_table[] = {
    /*
...
/* factory test */
/* mpw2 */
oal_ulong mpw2_wmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
...
/* pilot */
oal_ulong pilot_wmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x00040000, 0x000A7FFF }, /* itcm ram */
    { 0x20018000, 0x2007FFFF }, /* dtcm */
    { 0x60000000, 0x6007FFFF }, /* share mem */
};

oal_ulong mpw2_bmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x80000000, 0x800FFFFF }, /* itcm */
    { 0x80100000, 0x801DFFFF }, /* dtcm */
};

/* pilot */
oal_ulong pilot_bmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x80040000, 0x8010FFFF }, /* itcm */
    { 0x80200000, 0x8030FFFF }, /* dtcm */
};

/* 1105 */
oal_ulong hi1105_asic_wmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x00040000, 0x000BFFFF }, /* itcm ram */
    { 0x20018000, 0x2007FFFF }, /* dtcm */
    { 0x60000000, 0x600FFFFF }, /* share mem */
};

oal_ulong hi1105_fpga_wmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
...
oal_ulong hi1105_asic_bmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x80040000, 0x8014FFFF }, /* itcm */
    { 0x80200000, 0x8030FFFF }, /* dtcm */
};

oal_ulong hi1105_fpga_bmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x80040000, 0x80040000 }, /* itcm */
    { 0x80200000, 0x80200000 }, /* dtcm */
};
...
OAL_STATIC oal_pcie_region hi1103_pcie_mpw2_regions[] = {
...
OAL_STATIC oal_pcie_region hi1103_pcie_pilot_regions[] = {
    {   .pci_start = 0x00000000,
        .pci_end = 0x000A7FFF,
        .cpu_start = 0x00000000,
...
OAL_STATIC oal_pcie_region hi1105_pcie_asic_regions[] = {
    {   .pci_start = 0x00000000,
        .pci_end = 0x000BFFFF,
        .cpu_start = 0x00000000,
        .cpu_end = 0x000BFFFF,
...
OAL_STATIC oal_pcie_region hi1105_pcie_fpga_regions[] = {
    {   .pci_start = 0x00000000,
        .pci_end = 0x000BFFFF,
        .cpu_start = 0x00000000,
        .cpu_end = 0x000BFFFF,
...
oal_reg_bits_stru pcie_phy_0v9_bits[] = {
    { 700, 0x0, "0.7v" },
    { 750, 0x1, "0.75v" },
    { 775, 0x2, "0.775v" },
    { 800, 0x3, "0.8v" },
...
oal_reg_bits_stru pcie_phy_1v8_bits[] = {
    { 1600, 0x0, "1.6v" },
    { 1625, 0x1, "1.625v" },
    { 1650, 0x2, "1.650v" },
    { 1675, 0x3, "1.675v" },
...
oal_uint32 h2d_bypass_pkt_num = 0;
oal_netbuf_stru *h2d_pst_netbuf = NULL;
dma_addr_t h2d_pci_dma_addr = 0x0;
#endif

#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE_D2H_BYPASS
oal_uint32 d2h_bypass_pkt_num = 0;
oal_uint32 d2h_bypass_total_pkt_num = 0;
oal_netbuf_stru *d2h_pst_netbuf = NULL;
dma_addr_t d2h_pci_dma_addr = 0x0;
oal_completion d2h_test_done;
#endif
...
/* 函数定义 */oal_void oal_pcie_io_trans64_sub(oal_uint64 *dst, oal_uint64 *src, oal_int32 size)
{
    oal_int32 remain = size;

    for (;;) {
        if (remain < 8) { /* 最少传输字节数，剩余数据量不足时退出 */
        ...
        if (remain >= 128) { /* 根据剩余数据量拷贝，一次性拷贝128字节 */
            ...
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            remain -= 128;
        } else if (remain >= 64) { /* 根据剩余数据量拷贝，一次性拷贝64字节 */
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            ...
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            remain -= 64;
        } else if (remain >= 32) { /* 根据剩余数据量拷贝，一次性拷贝32字节 */
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            remain -= 32;
        } else if (remain >= 16) { /* 根据剩余数据量拷贝，一次性拷贝16字节 */
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            *((volatile oal_uint64 *)dst++) = *((volatile oal_uint64 *)src++);
            remain -= 16;
        } else {
...
oal_void oal_pcie_io_trans32(oal_uint32 *dst, oal_uint32 *src, oal_int32 size)
{
    oal_int32 remain = size;

    for (;;) {
        if (remain < 4) { /* 最少传输字节数，剩余数据量不足时退出 */
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
            rmb();
            wmb(); /* dsb */
#endif
        ...
        if (remain >= 64) { /* 根据剩余数据量拷贝，一次性拷贝64字节 */
            ...
            *((volatile oal_uint32 *)dst++) = *((volatile oal_uint32 *)src++);
            remain -= 16;
        } else if (remain >= 8) { /* 根据剩余数据量拷贝，一次性拷贝8字节 */
            *((volatile oal_uint32 *)dst++) = *((volatile oal_uint32 *)src++);
            *((volatile oal_uint32 *)dst++) = *((volatile oal_uint32 *)src++);
            remain -= 8;
        } else {
            *((volatile oal_uint32 *)dst++) = *((volatile oal_uint32 *)src++);
            remain -= 4; /* 剩余数据量为4到7字节时，只拷贝4字节 */
        }
    }
}

oal_void oal_pcie_io_trans64(oal_void *dst, oal_void *src, oal_int32 size)
{
    ...
    for (;;) {
        ...
        if ((!((uintptr_t)src & 0x7)) && (remain >= 8)) { /* 8bytes */
            copy_size = OAL_ROUND_DOWN((oal_uint32)remain, 8); /* 清除低3bit，保证8字节对齐 */
            remain -= copy_size;
            oal_pcie_io_trans64_sub(dst, src, (oal_int32)copy_size);
            src += copy_size;
            dst += copy_size;
            ...
            *((volatile oal_uint32 *)dst) = *((volatile oal_uint32 *)src);
            /* 每次偏移4字节 */
            dst += sizeof(oal_uint32);
            src += sizeof(oal_uint32);
        } else {
...
oal_int32 oal_pcie_memport_copy_test(oal_void)
{
    ...
    oal_void *buff_src = NULL;
    oal_void *buff_dst = NULL;
...
OAL_STATIC ktime_t mips_arry_total[PCIE_MIPS_BUTT];
OAL_STATIC ktime_t mips_arry[PCIE_MIPS_BUTT];
...
oal_void oal_pcie_mips_end(PCIE_MIPS_TYPE type)
{
    ...
    mips_arry_total[type] = ktime_add(mips_arry_total[type], ktime_sub(end, mips_arry[type]));
}

oal_void oal_pcie_mips_clear(oal_void)
{
    memset_s(mips_arry, sizeof(mips_arry), 0, sizeof(mips_arry));
    memset_s(mips_arry_total, sizeof(mips_arry_total), 0, sizeof(mips_arry_total));
}
...
oal_void oal_pcie_mips_show(oal_void)
{
    ...
    for (i = 0; i < PCIE_MIPS_BUTT; i++) {
        trans_us = (oal_uint64)ktime_to_us(mips_arry_total[i]);
        total_us += trans_us;
...
oal_int32 oal_pcie_set_inbound_by_viewport(oal_pcie_res *pst_pci_res)
{
    ...
    for (index = 0; index < region_num; index++, region_base++) {
        ...
        ret |= (oal_uint32)oal_pci_write_config_dword(pst_pci_dev,
            HI_PCI_IATU_REGION_CTRL_1_OFF_INBOUND_I(HI_PCI_IATU_BOUND_BASE_OFF),
            0x0);
        ...
        ret |= (oal_uint32)oal_pci_write_config_dword(pst_pci_dev,
            HI_PCI_IATU_REGION_CTRL_2_OFF_INBOUND_I(HI_PCI_IATU_BOUND_BASE_OFF),
            ctr2.AsDword);
        ...
        ret |= (oal_uint32)oal_pci_write_config_dword(pst_pci_dev,
            HI_PCI_IATU_LWR_BASE_ADDR_OFF_INBOUND_I(HI_PCI_IATU_BOUND_BASE_OFF),
            start.bits.low_addr);
        ret |= (oal_uint32)oal_pci_write_config_dword(pst_pci_dev,
            HI_PCI_IATU_UPPER_BASE_ADDR_OFF_INBOUND_I(HI_PCI_IATU_BOUND_BASE_OFF),
        ...
        ret |= (oal_uint32)oal_pci_write_config_dword(pst_pci_dev,
            HI_PCI_IATU_LWR_TARGET_ADDR_OFF_INBOUND_I(HI_PCI_IATU_BOUND_BASE_OFF), target.bits.low_addr);
        ret |= (oal_uint32)oal_pci_write_config_dword(pst_pci_dev,
            HI_PCI_IATU_UPPER_TARGET_ADDR_OFF_INBOUND_I(HI_PCI_IATU_BOUND_BASE_OFF),
            target.bits.high_addr);
    }

    /* 配置命令寄存器                                                                         */
    /* BIT0 = 1(I/O Space Enable), BIT1 = 1(Memory Space Enable), BIT2 = 1(Bus Master Enable) */
    ret |= (oal_uint32)oal_pci_write_config_word(pst_pci_dev, 0x04, 0x7);
    if (ret) {
...
oal_int32 oal_pcie_iatu_bar_init(oal_pcie_res *pst_pci_res)
{
    ...
    if (pst_pci_res->st_iatu_bar.st_bar_info.size == 0) {
        return OAL_SUCC;
    }
...
oal_int32 oal_pcie_bar_init(oal_pcie_res *pst_pci_res)
{
    ...
    /* 暂时只考虑1103 */
    bar_num = OAL_ARRAY_SIZE(en_bar_table);
    bar_base = &en_bar_table[0];

    if (get_hi110x_subchip_type() == BOARD_VERSION_HI1105) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "1105 %s region map", isAsic_etc() ? "asic" : "fpga");
        if (isAsic_etc()) {
            /* 1105 Asic */
            region_num = OAL_ARRAY_SIZE(hi1105_pcie_asic_regions);
            region_base = &hi1105_pcie_asic_regions[0];
        } else {
            /* 1105 Fpga */
            region_num = OAL_ARRAY_SIZE(hi1105_pcie_fpga_regions);
            region_base = &hi1105_pcie_fpga_regions[0];
        }
    } else {
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
            region_num = OAL_ARRAY_SIZE(hi1103_pcie_mpw2_regions);
            region_base = &hi1103_pcie_mpw2_regions[0];
        } else if (pst_pci_res->revision == PCIE_REVISION_5_00A) {
            region_num = OAL_ARRAY_SIZE(hi1103_pcie_pilot_regions);
            region_base = &hi1103_pcie_pilot_regions[0];
        } else {
                                            ...
                                            oal_uint32 request_cnt)
{
    ...
    for (i = 0; i < request_cnt; i++) {
        ...
        if (d2h_pci_dma_addr == 0) {
            break;
        }
        st_write_item.buff_paddr.addr = d2h_pci_dma_addr;

        PCI_PRINT_LOG(PCI_LOG_DBG, "d2h ringbuf write [netbuf:0x%p, data:[va:0x%lx,pa:0x%llx]",
                      d2h_pst_netbuf, (oal_ulong)OAL_NETBUF_DATA(d2h_pst_netbuf), st_write_item.buff_paddr.addr);
    ...
    *ret = OAL_SUCC;
    ...
    for (i = 0; i < request_cnt; i++) {
        ...
        if (pcie_dma_data_check_enable) {
            ...
            value = (oal_uint32)(uintptr_t)OAL_NETBUF_DATA(pst_netbuf) + HCC_HDR_TOTAL_LEN;
...
oal_int32 oal_pcie_tx_dma_addr_match(oal_netbuf_stru *pst_netbuf, edma_paddr_t dma_addr)
{
    ...
    ret = memcpy_s(&st_cb_dma, sizeof(pcie_cb_dma_res),
                   (oal_uint8 *)OAL_NETBUF_CB(pst_netbuf) + sizeof(struct hcc_tx_cb_stru),
...
oal_void oal_pcie_rx_netbuf_submit(oal_pcie_res *pst_pci_res, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (pcie_dma_data_check_enable) {
        if (OAL_UNLIKELY(oal_readl(OAL_NETBUF_DATA(pst_netbuf)) == 0xffffffff)) {
            ...
            oal_print_hex_dump ((oal_uint8 *)(OAL_NETBUF_DATA(pst_netbuf)),
                                HCC_HDR_TOTAL_LEN, HEX_DUMP_GROUP_SIZE, "hdr ");
            oal_print_hex_dump ((oal_uint8 *)(OAL_NETBUF_DATA(pst_netbuf) + HCC_HDR_TOTAL_LEN),
                                HCC_HDR_TOTAL_LEN / 2, HEX_DUMP_GROUP_SIZE, "payload "); /* 第二参数为打印字节数 */
            oal_netbuf_free(pst_netbuf);
...
oal_int32 oal_pcie_unmap_tx_netbuf(oal_pcie_res *pst_pci_res, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* Debug */
    memset_s((oal_uint8 *)OAL_NETBUF_CB(pst_netbuf) + sizeof(struct hcc_tx_cb_stru),
    ...
    if (OAL_LIKELY((st_cb_dma.paddr.addr != 0) && (st_cb_dma.len != 0))) {
        ...
        oal_print_hex_dump((oal_uint8 *)OAL_NETBUF_CB(pst_netbuf), OAL_NETBUF_CB_SIZE(),
...
oal_void oal_pcie_h2d_transfer_done(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < PCIE_H2D_QTYPE_BUTT; i++) {
        ...
        for (j = 0; j < PCIE_EDMA_MAX_CHANNELS; j++) {
            ...
            if (h2d_pst_netbuf == NULL) {
                PCI_PRINT_LOG(PCI_LOG_DBG, "h2d_pst_netbuf is null!");
                break;
            }
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE_FIFO_ADDRESS
            if ((oal_uint64)h2d_pci_dma_addr != (oal_uint64)addr[j].addr) {
                /* 地址不匹配，遍历下一个队列 */
                PCI_PRINT_LOG(PCI_LOG_ERR, "unkown bypass netbuf addr:0x%llu, should be :0x%llu",
                              (oal_uint64)h2d_pci_dma_addr, addr[j]);
                break;
            }
#endif
...
oal_void oal_pcie_d2h_transfer_done(oal_pcie_res *pst_pci_res)
{
    ...
    if (d2h_bypass_pkt_num >= d2h_bypass_total_pkt_num) {
        OAL_COMPLETE(&d2h_test_done);
    }

    oal_pcie_rx_ringbuf_bypass_supply(pst_pci_res, OAL_TRUE, OAL_TRUE, PCIE_RX_RINGBUF_SUPPLY_ALL);
}
#else
oal_uint32 rx_addr_count_err_cnt = 0;
/* 接收完成中断 */
oal_void oal_pcie_d2h_transfer_done(oal_pcie_res *pst_pci_res)
{
    oal_ulong flags;
    oal_int32 i, flag, cnt, total_cnt;
    const oal_uint32 ul_max_dump_bytes = 128;
    oal_netbuf_stru *pst_netbuf = NULL;
    ...
    if (!flag) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_D2H_TRANSFER_PCIE_LINK_DOWN);
    } else {
        oal_pcie_linux_res *pst_pci_lres = NULL;
    ...
    for (;;) {
        ...
        for (;;) {
            ret = oal_pcie_ringbuf_read(pst_pci_res, PCIE_COMM_RINGBUF_DMA_READ_FIFO,
                                        (oal_uint8 *)&soft_rd_item, sizeof(soft_rd_item));
            if (ret <= 0) {
                break;
            }
            ...
            if ((oal_uint16)pst_pci_res->stat.intx_tx_count != soft_rd_item.isr_count) {
                PCI_PRINT_LOG(PCI_LOG_WARN, "tx count:%u not equal to isr count:%u",
                              (oal_uint16)pst_pci_res->stat.intx_tx_count, soft_rd_item.isr_count);
            }
            ...
            if (OAL_UNLIKELY(soft_rd_item.qid >= (oal_uint16)PCIE_H2D_QTYPE_BUTT)) {
                ...
                oal_print_hex_dump((oal_uint8 *)&soft_rd_item, OAL_SIZEOF(soft_rd_item),
...
oal_void oal_pcie_d2h_edma_isr(oal_pcie_res *pst_pci_res)
{
    ...
    for (;;) {
        ...
        for (;;) {
            ret = oal_pcie_ringbuf_read(pst_pci_res, PCIE_COMM_RINGBUF_DMA_WRITE_FIFO,
                                        (oal_uint8 *)&soft_wr_item, sizeof(soft_wr_item));
            if (ret <= 0) {
                break;
            }
            ...
            if ((oal_uint16)pst_pci_res->stat.intx_rx_count != soft_wr_item.isr_count) {
                PCI_PRINT_LOG(PCI_LOG_WARN, "rx count:%u not equal to isr count:%u",
                              (oal_uint16)pst_pci_res->stat.intx_rx_count, soft_wr_item.isr_count);
            }
            ...
            if (OAL_UNLIKELY(netbuf_cnt < total_cnt)) {
                ...
                oal_print_hex_dump((oal_uint8 *)&soft_wr_item, OAL_SIZEOF(soft_wr_item),
    ...
    if (!flag) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_D2H_EDMA_PCIE_LINK_DOWN);
    } else {
        PCI_PRINT_LOG(PCI_LOG_DBG, "d2h trigger hcc_sched_transfer, dev:%p, lres:%p",
                      PCIE_RES_TO_DEV(pst_pci_res), oal_pci_get_drvdata(PCIE_RES_TO_DEV(pst_pci_res)));
        /* 发送完成,唤醒发送线程 */
...
oal_int32 oal_pcie_check_link_state(oal_pcie_res *pst_pci_res)
{
    ...
    if (share_mem_address == 0xFFFFFFFF) {
        ...
        do {
            ...
            if (board_get_wlan_wkup_gpio_val_etc() == 0) {
...
oal_void oal_pcie_share_mem_res_unmap(oal_pcie_res *pst_pci_res)
{
    memset_s((oal_void *)&pst_pci_res->dev_share_mem, OAL_SIZEOF(pst_pci_res->dev_share_mem),
...
{
    oal_int32 ret;
    oal_void *pst_share_mem_vaddr = NULL;
 ...
 * Output       : void
 */
OAL_STATIC oal_void oal_pcie_devce_l1_restore(oal_pcie_res *pst_pci_res, oal_bool_enum l1_disable)
{
    LOW_POWER_CFG_U low_power_cfg;
 ...
 */
OAL_STATIC oal_bool_enum oal_pcie_device_phy_addr_range_vaild(oal_uint16 addr)
{
 ...
 * Output       : oal_int32 : OAL_TURE means write succ
 */
OAL_STATIC oal_int32 oal_pcie_device_phy_write(oal_pcie_res *pst_pci_res,
                                               oal_uint16 addr, oal_uint16 value)
{
    oal_int32 i;
 ...
 * Input        : addr:phy地址，16bit位宽
 * Output       : oal_int32 : OAL_TURE means write succ , *pst_value:回读值
 */
OAL_STATIC oal_int32 oal_pcie_device_phy_read(oal_pcie_res *pst_pci_res,
                                              oal_uint16 addr, oal_uint16 *pst_value)
{
    oal_int32 i;
    PCIE_PHY_CFG_U phy_cfg;
    PCIE_PHY_CFG_ADDR_U phy_cfg_addr;
    ...
    for (i = 0; i < PCIE_EP_PHY_RW_TIMEOUT_CNT; i++) {
        ...
        if (phy_cfg.bits.phy_config_rdy == 1) { /* read done */
            ...
            *pst_value = (oal_uint16)phy_rd_data.bits.phy_cr_rd_data;
...
* Input        : pst_pci_res
* Output       : oal_int32 : OAL_TURE means set succ
*/
OAL_STATIC oal_int32 oal_pcie_device_phy_los_en_ovrd(oal_pcie_res *pst_pci_res)
{
    oal_uint16 value;
    oal_int32 ret;
...
* Output       : oal_int32 : OAL_TURE means set succ
*/
oal_int32 oal_pcie_device_phy_config(oal_pcie_res *pst_pci_res)
{
    oal_int32 ret;
    oal_pci_dev_stru *pst_pci_dev;
...
{
    oal_int32 ret;
    ret = oal_pcie_get_vol_reg_bits_value(request_vol, pcie_phy_1v8_bits,
                                          OAL_ARRAY_SIZE(pcie_phy_1v8_bits), pst_value);
    if (ret != OAL_SUCC) {
...
{
    oal_int32 ret;
    ret = oal_pcie_get_vol_reg_bits_value(request_vol, pcie_phy_0v9_bits,
                                          OAL_ARRAY_SIZE(pcie_phy_0v9_bits), pst_value);
    if (ret != OAL_SUCC) {
...
oal_int32 oal_pcie_voltage_bias_init(oal_pcie_res *pst_pci_res)
{
    ...
    pci_addr_map addr_map;
    oal_void *pst_pmu_cmu_ctrl = NULL;     /* 0x50002000 */
    oal_void *pst_pmu2_cmu_ir_ctrl = NULL; /* 0x50003000 */

    if (OAL_WARN_ON(pst_pci_res == NULL)) {
        return -OAL_ENODEV;
    }

    if (pcie_ldo_phy_0v9_param == 0 ||
        pcie_ldo_phy_1v8_param == 0) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "phy 0v9=%u mV, 1v8=%u mV invaild, pcie vol bias bypass",
                             pcie_ldo_phy_0v9_param, pcie_ldo_phy_1v8_param);
        return OAL_SUCC;
    }

    ret = oal_pcie_get_vol_reg_bits_value(pcie_ldo_phy_0v9_param, pcie_phy_0v9_bits,
                                          OAL_ARRAY_SIZE(pcie_phy_0v9_bits), &phy_0v9_bias);
    if (ret != OAL_SUCC) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "invalid pcie ldo bias phy 0v9 param =%d mV", pcie_ldo_phy_0v9_param);
        return -OAL_EINVAL;
    }

    ret = oal_pcie_get_vol_reg_bits_value(pcie_ldo_phy_1v8_param, pcie_phy_1v8_bits,
                                          OAL_ARRAY_SIZE(pcie_phy_1v8_bits), &phy_1v8_bias);
    if (ret != OAL_SUCC) {
...
oal_int32 oal_pcie_device_changeto_high_cpufreq(oal_pcie_res *pst_pci_res)
{
    oal_int32 ret;
    pci_addr_map addr_map;
    oal_void *pst_glb_ctrl = NULL;         /* 0x50000000 */
    oal_void *pst_pmu_cmu_ctrl = NULL;     /* 0x50002000 */
    oal_void *pst_pmu2_cmu_ir_ctrl = NULL; /* 0x50003000 */
    oal_void *pst_wctrl = NULL;
    const oal_uint32 ul_dev_read_timeout = 100;
    ...
    if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
        ...
        oal_pcie_device_read32_timeout(HI1103_PMU2_CMU_IR_PMU2_CMU_ABB_STS_6_REG,
            (HI1103_PMU2_CMU_IR_PMU_LNALDO0_EN_DELAY_STS_MSK | HI1103_PMU2_CMU_IR_PMU_LNALDO0_EN_STS_MSK),
            (HI1103_PMU2_CMU_IR_PMU_LNALDO0_EN_DELAY_STS_MSK | HI1103_PMU2_CMU_IR_PMU_LNALDO0_EN_STS_MSK),
            ul_dev_read_timeout);
        ...
        oal_pcie_device_read32_timeout(HI1103_PMU2_CMU_IR_FBDIV_FRAC_SLP_REG, 0x0,
            (HI1103_PMU2_CMU_IR_FBDIV_FRAC_WKUP_MSK | HI1103_PMU2_CMU_IR_FBDIV_FRAC_SLP_STS_MSK), ul_dev_read_timeout);
...
{
    oal_uint32 value = (oal_uint32)oal_readl(pst_pci_res->pst_pci_ctrl_base + PCIE_STATUS2_OFF);
    value = ((value >> 9) & 0x1);
    if (value == 0) {
        return PCIE_GEN1;
    } else {
        ...
        for (i = 0; i < copy_size; i += sizeof(oal_uint32)) {
            ...
            if (value != verify) {
                oal_print_hi11xx_log(HI11XX_LOG_ERR,
                    "mem check failed, cpu address : 0x%lx , write 0x%x , read 0x%x, offset =%lu, i = %u",
                    start + offset + i, verify, value, offset, i);
                oal_print_hex_dump((oal_uint8 *)pst_ddr_buf, copy_size, HEX_DUMP_GROUP_SIZE, "burst check ");
                oal_print_hex_dump((oal_uint8 *)(uintptr_t)(addr_map.va + offset),
                                         ...
                                         oal_ulong start, oal_ulong length, oal_uint8 test_value)
{
    ...
    for (i = 0; i < (oal_int32)(oal_uint32)length; i += sizeof(oal_uint32)) { /* 每次偏移4字节 */
...
oal_int32 oal_pcie_device_mem_check(oal_pcie_res *pst_pci_res, oal_ulong start, oal_ulong length)
{
    oal_int32 ret;
    oal_int32 i;
    const oal_uint32 ul_burst_size = 4096;
...
oal_int32 oal_pcie_device_mem_write_address_onetime(oal_pcie_res *pst_pci_res, oal_ulong start, oal_ulong length)
{
    ...
    for (;;) {
        ...
        for (i = 0; i < copy_size; i += sizeof(oal_uint32)) {      /* 每次偏移4字节 */
    ...
    for (;;) {
        ...
        for (i = 0; i < copy_size; i += sizeof(oal_uint32)) { /* 每次偏移4字节 */
            ...
            if (OAL_UNLIKELY(value != cpu_address)) {
                ...
                oal_print_hex_dump((oal_uint8 *)pst_ddr_buf, copy_size, HEX_DUMP_GROUP_SIZE, "address check ");
                oal_print_hex_dump((oal_uint8 *)(uintptr_t)(addr_map.va + offset),
...
oal_int32 oal_pcie_device_mem_performance(oal_pcie_res *pst_pci_res, oal_ulong start,
                                          oal_ulong length, oal_ulong runtime)
{
    oal_int32 ret;
    oal_ulong timeout;
    oal_ulong offset = 0;
    oal_ulong remain_size, copy_size, total_size;
    oal_ulong burst_size = 4096;
    oal_uint64 trans_size, us_to_s;
    declare_time_cost_stru(cost);
...
oal_int32 oal_pcie_device_get_scan_wmem_info(oal_pcie_res *pst_pci_res, oal_ulong (**pst_scan_base)[],
                                             oal_uint32 *pst_scan_nums)
{
    *pst_scan_base = NULL;
    *pst_scan_nums = 0;
    if (get_hi110x_subchip_type() == BOARD_VERSION_HI1105) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "1105 %s wmem scan", isAsic_etc() ? "asic" : "fpga");
        if (isAsic_etc()) {
            *pst_scan_nums = sizeof(hi1105_asic_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * sizeof(hi1105_asic_wmem_scan_array[0][0]));
            *pst_scan_base = hi1105_asic_wmem_scan_array;
        } else {
            *pst_scan_nums = sizeof(hi1105_fpga_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * sizeof(hi1105_fpga_wmem_scan_array[0][0]));
            *pst_scan_base = hi1105_fpga_wmem_scan_array;
        }
    } else {
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
            *pst_scan_nums = sizeof(mpw2_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * sizeof(mpw2_wmem_scan_array[0][0]));
            *pst_scan_base = mpw2_wmem_scan_array;
        } else if (pst_pci_res->revision == PCIE_REVISION_5_00A) {
            *pst_scan_nums = sizeof(pilot_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * sizeof(pilot_wmem_scan_array[0][0]));
            *pst_scan_base = pilot_wmem_scan_array;
        } else {
...
oal_int32 oal_pcie_device_scan_wmem(oal_pcie_res *pst_pci_res)
{
    ...
    oal_ulong(*pst_scan_base)[PCIE_MEM_MSG_SIZE];
...
oal_int32 oal_pcie_device_get_scan_bmem_info(oal_pcie_res *pst_pci_res, oal_ulong (**pst_scan_base)[],
                                             oal_uint32 *pst_scan_nums)
{
    *pst_scan_base = NULL;
    *pst_scan_nums = 0;
    if (get_hi110x_subchip_type() == BOARD_VERSION_HI1105) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "1105 %s bmem scan", isAsic_etc() ? "asic" : "fpga");
        if (isAsic_etc()) {
            *pst_scan_nums = sizeof(hi1105_asic_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * sizeof(hi1105_asic_bmem_scan_array[0][0]));
            *pst_scan_base = hi1105_asic_bmem_scan_array;
        } else {
        ...
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
            *pst_scan_nums = sizeof(mpw2_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * sizeof(mpw2_bmem_scan_array[0][0]));
            *pst_scan_base = mpw2_bmem_scan_array;
        } else if (pst_pci_res->revision == PCIE_REVISION_5_00A) {
            *pst_scan_nums = sizeof(pilot_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * sizeof(pilot_bmem_scan_array[0][0]));
            *pst_scan_base = pilot_bmem_scan_array;
        } else {
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "unkown pcie ip revision :0x%x", pst_pci_res->revision);
            return -OAL_ENODEV;
        }
                                             ...
                                             oal_uint32 data_size)
{
    ...
    for (i = 0; i < (oal_uint32)length; i += sizeof(oal_uint32)) { /* 每次偏移4字节 */
        value = oal_readl((oal_void *)(addr_map.va + i));
...
oal_int32 oal_pcie_comm_ringbuf_res_map(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < PCIE_COMM_RINGBUF_BUTT; i++) {
        ...
        memcpy_s((oal_void *)&pst_pci_res->st_ringbuf_res.comm_rb_res[i].data_daddr,
                 sizeof(pci_addr_map), (oal_void *)&st_map, sizeof(pci_addr_map));
...
oal_int32 oal_pcie_ringbuf_h2d_refresh(oal_pcie_res *pst_pci_res)
{
    ...
    if (ret != OAL_SUCC) {
        ...
        oal_print_hex_dump((oal_uint8 *)&st_share_mem, OAL_SIZEOF(st_share_mem), HEX_DUMP_GROUP_SIZE, "st_share_mem: ");
        return -OAL_ENOMEM;
    }
    ...
    for (i = 0; i < PCIE_H2D_QTYPE_BUTT; i++) {
        ...
        memcpy_s(&pst_pci_res->st_tx_res[i].ringbuf_data_dma_addr, sizeof(pci_addr_map), &st_map, sizeof(pci_addr_map));
...
oal_int32 oal_pcie_ringbuf_res_map(oal_pcie_res *pst_pci_res)
{
    ...
    oal_pcie_io_trans((uintptr_t)&st_share_mem, pst_pci_res->dev_share_mem.va, sizeof(pcie_share_mem_stru));
    oal_print_hex_dump((oal_uint8 *)pst_pci_res->dev_share_mem.va, sizeof(pcie_share_mem_stru),
    ...
    memcpy_s((oal_void *)&pst_pci_res->st_rx_res.ringbuf_data_dma_addr, sizeof(pci_addr_map),
             (oal_void *)&st_map, sizeof(pci_addr_map));
    ...
    memcpy_s((oal_void *)&pst_pci_res->st_message_res.h2d_res.ringbuf_data_dma_addr,
             sizeof(pci_addr_map), (oal_void *)&st_map, sizeof(pci_addr_map));
    ...
    memcpy_s((oal_void *)&pst_pci_res->st_message_res.d2h_res.ringbuf_data_dma_addr,
             sizeof(pci_addr_map), (oal_void *)&st_map, sizeof(pci_addr_map));
...
oal_int32 oal_pcie_send_netbuf(oal_pcie_res *pst_pci_res, oal_netbuf_stru *pst_netbuf, PCIE_H2D_RINGBUF_QTYPE qtype)
{
    ...
    /* 64bits 传输, 不考虑大小端 */
    st_item.buff_paddr.addr = (oal_uint64)h2d_pci_dma_addr;
                                        ...
                                        pci_addr_map *pst_ringbuf_base, oal_uint32 *message)
{
    ...
    oal_pcie_io_trans((uintptr_t)(message), (uintptr_t)(pst_ringbuf_base->va + (oal_ulong)real_rd),
                      pst_ringbuf->item_len);
...
oal_int32 oal_pcie_ringbuf_read(oal_pcie_res *pst_pci_res, PCIE_COMM_RINGBUF_TYPE type, oal_uint8 *buf, oal_uint32 len)
{
    ...
    oal_pcie_io_trans ((uintptr_t)(buf), (pst_ringbuf_base->va + (oal_ulong)real_rd), pst_ringbuf->item_len);
...
static oal_int32 pcie_send_check_param(oal_pcie_res *pst_pci_res, oal_netbuf_head_stru *pst_head)
{
...
oal_int32 oal_pcie_send_netbuf_list(oal_pcie_res *pst_pci_res, oal_netbuf_head_stru *pst_head,
                                    PCIE_H2D_RINGBUF_QTYPE qtype)
{
    oal_ulong flags;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    ...
    for (;;) {
        ...
        ret = memcpy_s((oal_uint8 *)OAL_NETBUF_CB(pst_netbuf) + sizeof(struct hcc_tx_cb_stru),
...
oal_int32 oal_pcie_print_device_transfer_info(oal_pcie_res *pst_pcie_res)
{
    ...
    if (pst_pcie_res->st_device_stat_map.va != 0) {
...
#ifdef CONFIG_PCIE_MEM_WR_CACHE_ENABLE
        oal_pci_cache_inv(pst_pci_dev, (oal_void *)pst_pcie_res->st_device_stat_map.pa,
        ...
        PCI_PRINT_LOG(PCI_LOG_INFO,
                      "d2h push_fifo_count:%u done_isr_count:%u dma_work_list_stat:%u dma_free_list_stat:%u dma_pending_list_stat:%u",
        ...
        PCI_PRINT_LOG(PCI_LOG_INFO,
                      "h2d push_fifo_count:%u done_isr_count:%u dma_work_list_stat:%u dma_free_list_stat:%u dma_pending_list_stat:%u",
        ...
        PCI_PRINT_LOG(PCI_LOG_INFO,
                      "comm_stat l1_wake_force_push_cnt:%u l1_wake_l1_hit:%u l1_wake_l1_miss:%u l1_wake_state_err_cnt:%u l1_wake_timeout_cnt:%u l1_wake_timeout_max_cnt:%u",
...
oal_void oal_pcie_print_transfer_info(oal_pcie_res *pst_pci_res, oal_uint64 print_flag)
{
    ...
    if (OAL_UNLIKELY(pst_pci_res->link_state >= PCI_WLAN_LINK_WORK_UP)) {
        ...
        PCI_PRINT_LOG(PCI_LOG_INFO, "pcie is %s", pcie_link_state_str[pst_pci_res->link_state]);
    }
}
...
oal_void oal_pcie_reset_transfer_info(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < PCIE_H2D_QTYPE_BUTT; i++) {
        ...
        memset_s((oal_void *)pst_pci_res->st_tx_res[i].stat.tx_burst_cnt,
                 sizeof(pst_pci_res->st_tx_res[i].stat.tx_burst_cnt), 0,
                 sizeof(pst_pci_res->st_tx_res[i].stat.tx_burst_cnt));
    }

    memset_s((oal_void *)pst_pci_res->st_rx_res.stat.rx_burst_cnt,
             sizeof(pst_pci_res->st_rx_res.stat.rx_burst_cnt), 0, sizeof(pst_pci_res->st_rx_res.stat.rx_burst_cnt));
}
...
/* Debug functions */
oal_int32 oal_pcie_dump_all_regions_mem(struct kobject *dev, struct kobj_attribute *attr,
                                        const char *buf, oal_pcie_res *pst_pcie_res)
{
    ...
    for (index = 0; index < region_num; index++, region_base++) {
        ...
        if (region_base->vaddr != NULL) {
            oal_uint32 size = region_base->size > 256 ? 256 : region_base->size; /* 表示最多打印256字节 */
...
#endif
            oal_print_hex_dump((oal_uint8 *)region_base->vaddr, size, HEX_DUMP_GROUP_SIZE, "pci mem  ");
        }
    }
...
oal_int32 oal_pcie_dump_all_regions(struct kobject *dev, struct kobj_attribute *attr,
                                    const char *buf, oal_pcie_res *pst_pcie_res)
{
    if (pst_pcie_res == NULL) {
...
oal_int32 oal_pcie_debug_read32(struct kobject *dev, struct kobj_attribute *attr,
                                const char *buf, oal_pcie_res *pst_pcie_res)
{
...
oal_int32 oal_pcie_debug_write32(struct kobject *dev, struct kobj_attribute *attr,
                                 const char *buf, oal_pcie_res *pst_pcie_res)
{
...
oal_int32 oal_pcie_debug_read_dsm32(struct kobject *dev, struct kobj_attribute *attr,
                                    const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
    oal_uint32 val, type;
...
oal_int32 oal_pcie_debug_write_dsm32(struct kobject *dev, struct kobj_attribute *attr,
                                     const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
    oal_uint32 val, type;
...
oal_int32 oal_pcie_debug_host_read32(struct kobject *dev, struct kobj_attribute *attr,
                                     const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo host_read32 address > debug */
    oal_ulong cpu_address, align_addr;
    oal_void *pst_vaddr = NULL;
    oal_int32 value;
...
oal_int32 oal_pcie_debug_host_write32(struct kobject *dev, struct kobj_attribute *attr,
                                      const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo host_write32 address value > debug */
    oal_ulong cpu_address, align_addr;
    oal_void *pst_vaddr = NULL;
    oal_void *pst_align_addr = NULL;
...
oal_int32 oal_pcie_debug_read16(struct kobject *dev, struct kobj_attribute *attr,
                                const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo read32 address > debug */
    oal_int32 ret;
...
oal_int32 oal_pcie_debug_write16(struct kobject *dev, struct kobj_attribute *attr,
                                 const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo write32 address value > debug */
    oal_int32 ret;
...
oal_int32 oal_pcie_saveconfigmem(oal_pcie_res *pst_pcie_res, const char *file_name,
                                 oal_uint32 cpu_address, oal_uint32 length)
{
    struct file *fp = NULL;
    oal_uint32 index;
    oal_int32 ret = OAL_SUCC;
    ...
    length = PADDING(length, 4); /* 计算4字节对齐后的长度，默认进位 */
    ...
    for (index = 0; index < length; index += sizeof(oal_uint32)) { /* 每次偏移4字节 */
        oal_uint32 reg = 0;
...
oal_int32 oal_pcie_savemem(oal_pcie_res *pst_pcie_res, const char *file_name, oal_uint32 cpu_address, oal_uint32 length)
{
    struct file *fp = NULL;
...
oal_int32 oal_pcie_save_hostmem(oal_pcie_res *pst_pcie_res, const char *file_name,
                                oal_ulong host_address, oal_uint32 length)
{
    struct file *fp = NULL;
    oal_uint32 index, value;
    oal_int32 ret = OAL_SUCC;
    ...
    for (index = 0; index < length; index += sizeof(oal_uint32)) {
        /* 这里有可能保存的是寄存器区域，按4字节对齐访问 */
...
oal_int32 oal_pcie_debug_savemem(struct kobject *dev, struct kobj_attribute *attr,
                                 const char *buf, oal_pcie_res *pst_pcie_res)
{
...
oal_int32 oal_pcie_debug_saveconfigmem(struct kobject *dev, struct kobj_attribute *attr,
                                       const char *buf, oal_pcie_res *pst_pcie_res)
{
    char file_name[PCIE_DEBUG_MSG_LEN];
    oal_uint32 cpu_address, length;
...
oal_int32 oal_pcie_debug_save_hostmem(struct kobject *dev, struct kobj_attribute *attr,
                                      const char *buf, oal_pcie_res *pst_pcie_res)
{
    char file_name[PCIE_DEBUG_MSG_LEN];
...
oal_int32 oal_pcie_debug_readmem(struct kobject *dev, struct kobj_attribute *attr,
                                 const char *buf, oal_pcie_res *pst_pcie_res)
{
...
oal_int32 oal_pcie_debug_readmem_config(struct kobject *dev, struct kobj_attribute *attr,
                                        const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    oal_uint32 cpu_address, length, index;
 ...
 * 1103 Slave 空间为256M
 */
oal_int32 oal_pcie_outbound_test(oal_pcie_res *pst_pcie_res, const char *buf)
{
    OAL_STATIC dma_addr_t outbound_dma_addr = 0;
    oal_uint32 outbound_size = 4096;
    ...
    if ((sscanf(buf, "%s", casename) != 1)) {
        ...
        /* dump */
        if (outbound_vaddr) {
            oal_print_hex_dump((oal_uint8 *)outbound_vaddr, outbound_size, HEX_DUMP_GROUP_SIZE, "outbound: ");
        }
    }
        ...
        if (outbound_vaddr) {
            ...
            if (pst_pcie_res->revision == PCIE_REVISION_4_70A) {
                ...
                oal_pci_write_config_dword(pst_pci_dev, 0x700 + 0x218,
                                           (oal_uint32)outbound_dma_addr); /* target lower */
                oal_pci_write_config_dword(pst_pci_dev, 0x700 + 0x21c, 0); /* target upper */
            } else if (pst_pcie_res->revision == PCIE_REVISION_5_00A) {
                IATU_REGION_CTRL_2_OFF ctr2;
                oal_uint32 index = 0;
...
oal_int32 oal_pcie_device_memcheck(oal_pcie_res *pst_pcie_res, oal_uint32 cpuaddr, oal_uint32 size, oal_uint32 data)
{
    ...
    for (i = 0, mode = 0; i < (size); i += sizeof(oal_uint32), mode++) { /* 每次偏移4字节，直到超过size */
...
oal_void pcie_change_2G_init(oal_void)
{
    ...
    oal_void *reg5115_sys_ctl;
    oal_uint32 ul_val;
    pst_5115_pci0 = oal_ioremap_nocache(0x10A00000, 0x1000);
    pst_5115_pcie_phy = oal_ioremap_nocache(0x14880030, 0x1000);
    reg5115_sys_ctl = oal_ioremap_nocache(0x10100000, 0x1000);
    if (reg5115_sys_ctl == NULL) {
        OAL_IO_PRINT("remap addr fail\n");
        return;
    }

    ul_val = oal_readl(reg5115_sys_ctl + 0xBC);
    ul_val |= BIT21;
    oal_writel(ul_val, reg5115_sys_ctl + 0xBC);

    ul_val = oal_readl(reg5115_sys_ctl + 0xC0);
    ul_val |= BIT21;
    oal_writel(ul_val, reg5115_sys_ctl + 0xC0);
...
#define PCIE_CHANGE_LOOP 100
    for (ul_loop = 0; ul_loop < PCIE_CHANGE_LOOP; ul_loop++) {
        ul_val = oal_readl(reg5115_sys_ctl + 0x38);
    ...
    if (ul_loop == PCIE_CHANGE_LOOP) {
        OAL_IO_PRINT("0x38 change timeout\n");
        oal_pcie_print_bits(&ul_val, sizeof(ul_val));
    }
...
oal_int32 oal_pcie_debug_wlan_power_on(struct kobject *dev, struct kobj_attribute *attr,
                                       const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_pci_dev_stru *pst_pci_dev = NULL;
...
OAL_STATIC oal_int32 oal_pcie_testcase_transfer_res_init(oal_pcie_res *pst_pcie_res,
                                                         oal_pci_dev_stru *pst_pci_dev,
                                                         const char *buf)
{
    oal_int32 ret;
...
OAL_STATIC oal_int32 oal_pcie_testcase_send_test_pkt(oal_pcie_res *pst_pcie_res,
                                                     oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_host_sm_monitor(oal_pcie_res *pst_pcie_res,
                                                       oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_send_test_pkt2(oal_pcie_res *pst_pcie_res,
                                                      oal_pci_dev_stru *pst_pci_dev,
                                                      const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_send_test_pkt3(oal_pcie_res *pst_pcie_res,
                                                      oal_pci_dev_stru *pst_pci_dev,
                                                      const char *buf)
{
...
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE_PERFORMANCE
OAL_STATIC oal_int32 oal_pcie_testcase_pcie_mips_show(oal_pcie_res *pst_pcie_res,
                                                      oal_pci_dev_stru *pst_pci_dev,
                                                      const char *buf)
{
    oal_pcie_mips_show();
...
OAL_STATIC oal_int32 oal_pcie_testcase_pcie_mips_clear(oal_pcie_res *pst_pcie_res,
                                                       oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
    oal_pcie_mips_clear();
...
#ifdef CONFIG_ARCH_SD56XX
OAL_STATIC oal_int32 oal_pcie_testcase_gen2_test(oal_pcie_res *pst_pcie_res,
                                                 oal_pci_dev_stru *pst_pci_dev,
                                                 const char *buf)
{
    pcie_change_5G();
...
OAL_STATIC oal_int32 oal_pcie_testcase_wlan_poweron_2g(oal_pcie_res *pst_pcie_res,
                                                       oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
    /* FPGA 默认切换到Gen 1 */
...
OAL_STATIC oal_int32 oal_pcie_testcase_wlan_poweron(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    oal_pci_card_detect();
...
OAL_STATIC oal_int32 oal_pcie_testcase_pm_aspm_l0(oal_pcie_res *pst_pcie_res,
                                                  oal_pci_dev_stru *pst_pci_dev,
                                                  const char *buf)
{
    oal_pcie_rc_mem_map();
...
OAL_STATIC oal_int32 oal_pcie_testcase_pm_aspm_l1(oal_pcie_res *pst_pcie_res,
                                                  oal_pci_dev_stru *pst_pci_dev,
                                                  const char *buf)
{
    oal_pcie_rc_mem_map();
...
OAL_STATIC oal_int32 oal_pcie_testcase_pm_aspm_l0_l1(oal_pcie_res *pst_pcie_res,
                                                     oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
    oal_pcie_rc_mem_map();
...
OAL_STATIC oal_int32 oal_pcie_testcase_pm_aspm_l1_1(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    oal_pcie_rc_mem_map();
...
OAL_STATIC oal_int32 oal_pcie_testcase_pm_aspm_l1_2(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    oal_pcie_rc_mem_map();
...
OAL_STATIC oal_int32 oal_pcie_testcase_pm_aspm_l1ss(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    oal_pcie_rc_mem_map();
...
OAL_STATIC oal_int32 oal_pcie_testcase_read_rc_reg32(oal_pcie_res *pst_pcie_res,
                                                     oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
    oal_uint32 reg = 0;
    for (; *buf == ' '; buf++)
        ;
    if ((sscanf(buf, "0x%x", &reg) != 1) || reg > PAGE_SIZE) {
...
OAL_STATIC oal_int32 oal_pcie_testcase_pcie_powerdown(oal_pcie_res *pst_pcie_res,
                                                      oal_pci_dev_stru *pst_pci_dev,
                                                      const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_pcie_powerup(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    oal_int32 ret;
    ...
    /* 检查建链是否完成 */
    if (oal_pcie_check_link_up() != OAL_SUCC) {
...
OAL_STATIC oal_int32 oal_pcie_testcase_gpio_87_test(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    ...
    if (pst_gpio_base == OAL_PTR_NULL || pst_gpio_mode == NULL) {
...
OAL_STATIC oal_int32 oal_pcie_testcase_d2h_doorbell(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_h2d_doorbell(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_outbound_test(oal_pcie_res *pst_pcie_res,
                                                     oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_ram_memcheck(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_pme_enable(oal_pcie_res *pst_pcie_res,
                                                  oal_pci_dev_stru *pst_pci_dev,
                                                  const char *buf)
{
    oal_int32  ret;
...
OAL_STATIC oal_int32 oal_pcie_testcase_ringbuf_stat(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    oal_int32 i;
...
OAL_STATIC oal_int32 oal_pcie_testcase_clear_trans_info(oal_pcie_res *pst_pcie_res,
                                                        oal_pci_dev_stru *pst_pci_dev,
                                                        const char *buf)
{
    memset_s((oal_void *)pst_pcie_res->st_tx_res[PCIE_H2D_QTYPE_NORMAL].stat.tx_burst_cnt,
    ...
    memset_s((oal_void *)pst_pcie_res->st_rx_res.stat.rx_burst_cnt,
...
OAL_STATIC oal_int32 oal_pcie_testcase_show_trans_info(oal_pcie_res *pst_pcie_res,
                                                       oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
    oal_int32 i;
...
OAL_STATIC oal_int32 oal_pcie_testcase_send_message(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    oal_int32 ret;
...
OAL_STATIC oal_int32 oal_pcie_testcase_sched_rx_task(oal_pcie_res *pst_pcie_res,
                                                     oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
...
OAL_STATIC oal_int32 oal_pcie_testcase_pcie_poweron_test(oal_pcie_res *pst_pcie_res,
                                                         oal_pci_dev_stru *pst_pci_dev,
                                                         const char *buf)
{
    oal_int32 ret;
...
OAL_STATIC oal_int32 oal_pcie_testcase_pcie_enum_download(oal_pcie_res *pst_pcie_res,
                                                          oal_pci_dev_stru *pst_pci_dev,
                                                          const char *buf)
{
    oal_int32 ret;
...
OAL_STATIC oal_int32 oal_pcie_testcase_firmware_cfg_init_etc(oal_pcie_res *pst_pcie_res,
                                                             oal_pci_dev_stru *pst_pci_dev,
                                                             const char *buf)
{
    oal_int32 ret;
...
OAL_STATIC oal_int32 oal_pcie_testcase_firmware_download_etc(oal_pcie_res *pst_pcie_res,
                                                             oal_pci_dev_stru *pst_pci_dev,
                                                             const char *buf)
{
    ...
    for (; *buf == ' '; buf++)
        ;
    if ((sscanf(buf, "%u", &cfg_index) != 1) || cfg_index >= CFG_FILE_TOTAL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "invaild input:[%s]", buf);
    } else {
...
OAL_STATIC oal_int32 oal_pcie_testcase_turnoff_message(oal_pcie_res *pst_pcie_res,
                                                       oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
#ifdef CONFIG_ARCH_KIRIN_PCIE
...
OAL_STATIC oal_int32 oal_pcie_testcase_suspend_test(oal_pcie_res *pst_pcie_res,
                                                    oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
#ifdef CONFIG_ARCH_KIRIN_PCIE
    ...
    kirin_pcie_power_notifiy_register(kirin_rc_idx, NULL, NULL, NULL);
    ret__ = kirin_pcie_pm_control(0, kirin_rc_idx);
...
OAL_STATIC oal_int32 oal_pcie_testcase_resume_test(oal_pcie_res *pst_pcie_res,
                                                   oal_pci_dev_stru *pst_pci_dev,
                                                   const char *buf)
{
#ifdef CONFIG_ARCH_KIRIN_PCIE
    ...
    ret__ = kirin_pcie_pm_control(1, kirin_rc_idx);
...
OAL_STATIC oal_int32 oal_pcie_testcase_rc_send_polling(oal_pcie_res *pst_pcie_res,
                                                       oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
#ifdef CONFIG_ARCH_KIRIN_PCIE
...
OAL_STATIC oal_int32 oal_pcie_testcase_ep_send_polling(oal_pcie_res *pst_pcie_res,
                                                       oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
    oal_uint16 val = 0;
    oal_int32 ret = oal_pci_read_config_word(pst_pci_dev, oal_pci_pcie_cap(pst_pci_dev) + PCI_EXP_LNKCTL2, &val);
    if (ret == 0) {
...
typedef struct _pcie_debug_testcase_ {
    char *name;
    oal_int32 (*testcase)(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev, const char *buf);
} pcie_debug_testcase;

OAL_STATIC pcie_debug_testcase pci_debug_testcases[] = {
    { "transfer_res_init", oal_pcie_testcase_transfer_res_init },
    { "send_test_pkt",     oal_pcie_testcase_send_test_pkt },
    { "host_sm_monitor",   oal_pcie_testcase_host_sm_monitor },
...
#endif
#ifdef CONFIG_ARCH_SD56XX
    { "gen2_test",       oal_pcie_testcase_gen2_test },
    { "wlan_poweron_2g", oal_pcie_testcase_wlan_poweron_2g },
    { "wlan_poweron",    oal_pcie_testcase_wlan_poweron },
    { "pm_aspm_l0",      oal_pcie_testcase_pm_aspm_l0 },
    { "pm_aspm_l1",      oal_pcie_testcase_pm_aspm_l1 },
    { "pm_aspm_l1_1",    oal_pcie_testcase_pm_aspm_l1_1 },
    { "pm_aspm_l1_2",    oal_pcie_testcase_pm_aspm_l1_2 },
    { "pm_aspm_l1ss",    oal_pcie_testcase_pm_aspm_l1ss },
    { "read_rc_reg32",   oal_pcie_testcase_read_rc_reg32 },
    { "gpio_87_test",    oal_pcie_testcase_gpio_87_test },
    { "pcie_powerdown",  oal_pcie_testcase_pcie_powerdown },
    { "pcie_powerup",    oal_pcie_testcase_pcie_powerup },
#endif
    ...
    { "pcie_poweron_test",  oal_pcie_testcase_pcie_poweron_test },
    { "pcie_enum_download", oal_pcie_testcase_pcie_enum_download },
    { "firmware_cfg_init",  oal_pcie_testcase_firmware_cfg_init_etc },
    { "firmware_download",  oal_pcie_testcase_firmware_download_etc },
...
oal_int32 oal_pcie_debug_testcase(struct kobject *dev, struct kobj_attribute *attr,
                                  const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    oal_int32 i;
    ...
    for (i = 0; i < OAL_ARRAY_SIZE(pci_debug_testcases); i++) {
        if (pci_debug_testcases[i].name) {
            if (!oal_strcmp(pci_debug_testcases[i].name, casename)) {
                break;
            }
        }
    }

    if (i == OAL_ARRAY_SIZE(pci_debug_testcases)) {
        PCI_PRINT_LOG(PCI_LOG_WARN, "testcase: %s not found", casename);
        return -OAL_ENODEV;
    }

    if (pci_debug_testcases[i].testcase == NULL) {
        PCI_PRINT_LOG(PCI_LOG_WARN, "testcase: %s no func", casename);
        return -OAL_ENODEV;
    }

    PCI_PRINT_LOG(PCI_LOG_INFO, "testcase:[%s] run", casename);
    pci_debug_testcases[i].testcase(pst_pcie_res, pst_pci_dev, buf);
                                   ...
                                   oal_uint32 length, oal_uint32 times, oal_uint32 burst_size)
{
    ...
    oal_uint64 trans_size, us_to_s, total_size;
    oal_uint32 size, copy_size, remainder;
    oal_void *pst_burst_buf_src = NULL;
    oal_void *pst_burst_buf_dst = NULL;
    oal_pci_dev_stru *pst_pci_dev = NULL;
    if (pst_pcie_res == NULL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "pst_pcie_res is null");
        return -OAL_EBUSY;
                                     ...
                                     oal_uint32 length, oal_uint32 times, oal_uint32 burst_size, oal_uint32 value)
{
    ...
    memset_s(pst_burst_buf, length, (oal_int32)value, length);
...
oal_int32 oal_pcie_debug_performance_read(struct kobject *dev, struct kobj_attribute *attr,
                                          const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    oal_int32 ret;
...
oal_int32 oal_pcie_debug_performance_cpu(struct kobject *dev, struct kobj_attribute *attr,
                                         const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
    oal_uint32 length, times, burst_size;
...
oal_int32 oal_pcie_debug_performance_netbuf_alloc(struct kobject *dev, struct kobj_attribute *attr,
                                                  const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
    oal_uint32 test_time;
...
oal_int32 oal_pcie_debug_performance_netbuf_queue(struct kobject *dev, struct kobj_attribute *attr,
                                                  const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
                                          ...
                                          oal_uint32 pkt_num, oal_uint32 pkt_len)
{
    ...
    h2d_pst_netbuf = oal_pcie_rx_netbuf_alloc(HCC_HDR_TOTAL_LEN + pkt_len, GFP_ATOMIC);
    if (h2d_pst_netbuf == NULL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "alloc netbuf %u failed!", HCC_HDR_TOTAL_LEN + pkt_len);
        return -OAL_ENOMEM;
    }

    oal_netbuf_put(h2d_pst_netbuf, HCC_HDR_TOTAL_LEN + pkt_len);

    h2d_pci_dma_addr = dma_map_single(&pst_pci_dev->dev, OAL_NETBUF_DATA(h2d_pst_netbuf),
                                      OAL_NETBUF_LEN(h2d_pst_netbuf), PCI_DMA_FROMDEVICE);
    if (dma_mapping_error(&pst_pci_dev->dev, h2d_pci_dma_addr)) {
        PCI_PRINT_LOG(PCI_LOG_INFO, "h2d bypass dma map netbuf failed, len=%u",
                      OAL_NETBUF_LEN(h2d_pst_netbuf));
        oal_netbuf_free(h2d_pst_netbuf);
        h2d_pst_netbuf = NULL;
        return -OAL_ENOMEM;
    }

    pst_cb_res = (pcie_cb_dma_res *)OAL_NETBUF_CB(h2d_pst_netbuf);
    pst_cb_res->paddr.addr = h2d_pci_dma_addr;
    pst_cb_res->len = OAL_NETBUF_LEN(h2d_pst_netbuf);
    ...
    for (i = 0; i < pkt_num; i++) {
        for (;;) {
            if (oal_pcie_send_netbuf(pst_pcie_res, h2d_pst_netbuf, PCIE_H2D_QTYPE_NORMAL) == 0) {
                if (flag) {
                    /* 更新device侧wr指针,刷ringbuf cache */
    ...
    dma_unmap_single(&pst_pci_dev->dev, (dma_addr_t)h2d_pci_dma_addr,
                     OAL_NETBUF_LEN(h2d_pst_netbuf), PCI_DMA_FROMDEVICE);
    h2d_pci_dma_addr = NULL;
    oal_netbuf_free(h2d_pst_netbuf);
    h2d_pst_netbuf = NULL;
    return OAL_SUCC;
}

oal_int32 oal_pcie_debug_performance_h2d_bypass(struct kobject *dev, struct kobj_attribute *attr,
                                                const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
                                          ...
                                          oal_uint32 pkt_num, oal_uint32 pkt_len)
{
    ...
    d2h_pst_netbuf = oal_pcie_rx_netbuf_alloc(HCC_HDR_TOTAL_LEN + PCIE_EDMA_TRANS_MAX_FRAME_LEN, GFP_ATOMIC);
    if (d2h_pst_netbuf == NULL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "alloc netbuf %u failed!", HCC_HDR_TOTAL_LEN + PCIE_EDMA_TRANS_MAX_FRAME_LEN);
        return -OAL_ENOMEM;
    }

    oal_netbuf_put(d2h_pst_netbuf, HCC_HDR_TOTAL_LEN + PCIE_EDMA_TRANS_MAX_FRAME_LEN);

    d2h_pci_dma_addr = dma_map_single(&pst_pci_dev->dev, OAL_NETBUF_DATA(d2h_pst_netbuf),
                                      OAL_NETBUF_LEN(d2h_pst_netbuf), PCI_DMA_FROMDEVICE);
    if (dma_mapping_error(&pst_pci_dev->dev, d2h_pci_dma_addr)) {
        PCI_PRINT_LOG(PCI_LOG_INFO, "h2d bypass dma map netbuf failed, len=%u",
                      OAL_NETBUF_LEN(d2h_pst_netbuf));
        oal_netbuf_free(d2h_pst_netbuf);
        d2h_pst_netbuf = NULL;
        return -OAL_ENOMEM;
    }

    pst_cb_res = (pcie_cb_dma_res *)OAL_NETBUF_CB(d2h_pst_netbuf);
    pst_cb_res->paddr.addr = d2h_pci_dma_addr;
    pst_cb_res->len = OAL_NETBUF_LEN(d2h_pst_netbuf);
    INIT_COMPLETION(d2h_test_done);
    ...
    oal_writel((oal_uint16)pkt_len << 16 | ((oal_uint16)pkt_num),
    ...
    start_time = ktime_get();
    /* 补充内存 */
    ret = wait_for_completion_interruptible(&d2h_test_done);
    if (ret < 0) {
        PCI_PRINT_LOG(PCI_LOG_INFO, "d2h_test_done wait interrupt!, rx cnt:%u", d2h_bypass_pkt_num);
    }
    ...
    trans_size = (oal_uint64)pkt_len * d2h_bypass_pkt_num;
    trans_size = trans_size * 1000u;
    trans_size = trans_size * 1000u;
    trans_size = (trans_size >> PCIE_TRANS_US_OFFSET_BITS);
    trans_size = div_u64(trans_size, trans_us);
    ...
    dma_unmap_single(&pst_pci_dev->dev, (dma_addr_t)d2h_pci_dma_addr,
                     OAL_NETBUF_LEN(d2h_pst_netbuf), PCI_DMA_FROMDEVICE);
    d2h_pci_dma_addr = NULL;
    oal_netbuf_free(d2h_pst_netbuf);
    d2h_pst_netbuf = NULL;
    return OAL_SUCC;
}

oal_int32 oal_pcie_debug_performance_d2h_bypass(struct kobject *dev, struct kobj_attribute *attr,
                                                const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
    oal_uint32 pkt_num, pkt_len;
...
oal_int32 oal_pcie_debug_performance_write(struct kobject *dev, struct kobj_attribute *attr,
                                           const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    oal_int32 ret;
...
oal_int32 oal_pcie_debug_loadfile(struct kobject *dev, struct kobj_attribute *attr,
                                  const char *buf, oal_pcie_res *pst_pcie_res)
{
    oal_int32 ret;
    char file_name[PCIE_DEBUG_MSG_LEN];
...
typedef struct _pcie_sysfs_debug_info_ {
    char *name;
    char *usage;
    oal_int32 (*debug)(struct kobject *dev, struct kobj_attribute *attr, const char *buf, oal_pcie_res *pst_pcie_res);
} pcie_sysfs_debug_info;

oal_int32 oal_pcie_print_debug_info(struct kobject *dev, struct kobj_attribute *attr,
                                    const char *buf, oal_pcie_res *pst_pcie_res)
{
...
OAL_STATIC pcie_sysfs_debug_info pci_debug[] = {
                                           ...
                                           const char *buf, size_t count)
{
    ...
    for (i = 0; i < OAL_ARRAY_SIZE(pci_debug); i++) {
        if (pci_debug[i].name) {
            if ((count >= OAL_STRLEN(pci_debug[i].name)) &&
                !oal_memcmp(pci_debug[i].name, buf, OAL_STRLEN(pci_debug[i].name))) {
                /* 判断最后一个字符是回车还是空格 */
                char last_c = *(buf + OAL_STRLEN(pci_debug[i].name));
                if (last_c == '\n' || last_c == ' ' || last_c == '\0') {
                    break;
                }
            }
...
#endif
    buf += OAL_STRLEN(pci_debug[i].name);
    if (*buf != '\0') {  // count > OAL_STRLEN(pci_debug[i].name)
        buf += 1;        /* EOF */
    }
    ...
    if (pci_debug[i].debug(dev, attr, (const char *)buf, pst_pcie_res) == -OAL_EINVAL) {
        oal_pcie_print_debug_usage(i);
    }
    return count;
}
OAL_STATIC struct kobj_attribute dev_attr_debug =
...
oal_int32 oal_pcie_sysfs_init(oal_pcie_res *pst_pcie_res)
{
    ...
    conn_syfs_pci_object = kobject_create_and_add("pci", pst_root_object);
    if (conn_syfs_pci_object == NULL) {
        ret = -OAL_ENODEV;
        PCI_PRINT_LOG(PCI_LOG_ERR, "sysfs create kobject_create_and_add pci fail\n");
        goto fail_g_conn_syfs_pci_object;
    }

    ret = oal_debug_sysfs_create_group(conn_syfs_pci_object, &hpci_attribute_group);
    if (ret) {
        ret = -OAL_ENOMEM;
...
fail_create_pci_group:
    kobject_put(conn_syfs_pci_object);
    conn_syfs_pci_object = NULL;
...
oal_void oal_pcie_sysfs_exit(oal_pcie_res *pst_pcie_res)
{
    ...
    oal_debug_sysfs_remove_group(conn_syfs_pci_object, &hpci_attribute_group);
    kobject_put(conn_syfs_pci_object);
    conn_syfs_pci_object = NULL;
}
...
OAL_STATIC oal_irq_handler_t msi_110x_callback[] = {
...
oal_pcie_res *oal_pcie_host_init(oal_void *data, oal_pcie_msi_stru *pst_msi, oal_uint32 revision)
{
    ...
    memset_s((oal_void *)pst_pci_res, sizeof(oal_pcie_res), 0, sizeof(oal_pcie_res));
    pst_pci_res->data = data;
    pst_pci_res->revision = revision;

    pst_msi->func = msi_110x_callback;
    pst_msi->msi_num = (oal_int32)(OAL_SIZEOF(msi_110x_callback) / OAL_SIZEOF(oal_irq_handler_t));
...
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE_D2H_BYPASS
    OAL_INIT_COMPLETION(&d2h_test_done);
#endif
