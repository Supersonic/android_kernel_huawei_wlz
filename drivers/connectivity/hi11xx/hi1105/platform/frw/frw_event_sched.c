...
/*
 * 函 数 名  : frw_event_sched_deactivate_queue_no_lock_etc
 * 功能描述  : 从调度队列删除一个事件队列
 * 输入参数  : pst_sched_queue: 调度队列指针
 *             pst_event_queue: 事件队列指针
 */
oal_uint32 frw_event_sched_deactivate_queue_no_lock_etc(frw_event_sched_queue_stru *pst_sched_queue,
...
/*
 * 函 数 名  : frw_event_sched_activate_queue_no_lock_etc
 * 功能描述  : 向调度队列添加一个新的事件队列
 *             封装为不加锁接口，保持接口对称性，供加锁接口调用
 * 输入参数  : past_sched_queue: 调度队列指针
 *             pst_event_queue : 事件队列指针
 */
oal_uint32 frw_event_sched_activate_queue_no_lock_etc(frw_event_sched_queue_stru *pst_sched_queue,
                                                      frw_event_queue_stru *pst_event_queue)
{
...
/*
 * 函 数 名  : frw_event_sched_pick_next_event_queue_wrr_etc
...
/*
 * 函 数 名  : frw_event_sched_init_etc
 * 功能描述  : 调度器初始化
 * 输入参数  : pst_sched_queue: 调度队列指针
 */
...
/*
 * 函 数 名  : frw_event_queue_init_etc
 * 功能描述  : 事件队列初始化
 ...
 */
oal_uint32 frw_event_queue_init_etc(frw_event_queue_stru *pst_event_queue,
                                    oal_uint8 uc_weight,
                                    frw_sched_policy_enum_uint8 en_policy,
                                    frw_event_queue_state_enum_uint8 en_state,
...
/*
 * 函 数 名  : frw_event_sched_deactivate_queue_etc
 * 功能描述  : 从调度队列删除一个事件队列
 * 输入参数  : pst_sched_queue: 调度队列指针
 *             pst_event_queue: 事件队列指针
 */
oal_uint32 frw_event_sched_deactivate_queue_etc(frw_event_sched_queue_stru *pst_sched_queue,
                                                frw_event_queue_stru *pst_event_queue)
{
    oal_uint ul_flag = 0;
    oal_uint32 ul_ret;
...
/*
 * 函 数 名  : frw_event_sched_activate_queue_etc
 * 功能描述  : 向调度队列添加一个新的事件队列
 *             封装调用不加锁实现，保持接口对称性
 * 输入参数  : past_sched_queue: 调度队列指针
 *             pst_event_queue : 事件队列指针
 */
oal_uint32 frw_event_sched_activate_queue_etc(frw_event_sched_queue_stru *pst_sched_queue,
                                              frw_event_queue_stru *pst_event_queue)
{
    oal_uint ul_flag = 0;
    oal_uint32 ul_ret;
...
/*
 * 函 数 名  : frw_event_sched_pause_queue_etc
 * 功能描述  : 暂停队列调度状态
 * 输入参数  : past_sched_queue: 调度队列指针
 *             pst_event_queue : 事件队列指针
 */
oal_void frw_event_sched_pause_queue_etc(frw_event_sched_queue_stru *pst_sched_queue,
                                         frw_event_queue_stru *pst_event_queue)
{
    oal_uint ul_flag = 0;
...
/*
 * 函 数 名  : frw_event_sched_resume_queue_etc
 * 功能描述  : 恢复队列调度状态
 * 输入参数  : past_sched_queue: 调度队列指针
 *             pst_event_queue : 事件队列指针
 */
oal_void frw_event_sched_resume_queue_etc(frw_event_sched_queue_stru *pst_sched_queue,
                                          frw_event_queue_stru *pst_event_queue)
{
    oal_uint ul_flag = 0;
