...
oal_uint32 os_time_etc = 0;
frw_timeout_track_stru timeout_track_etc[FRW_TIMEOUT_TRACK_NUM];
oal_uint8 timeout_track_idx_etc = 0;
#endif
...
oal_void frw_timer_init_etc(oal_uint32 ul_delay, oal_timer_func p_func, oal_ulong arg)
{
    ...
    for (ul_core_id = 0; ul_core_id < WLAN_FRW_MAX_NUM_CORES; ul_core_id++) {
        oal_dlist_init_head(&timer_list_etc[ul_core_id]);
        oal_spin_lock_init(&timer_list_spinlock_etc[ul_core_id]);

        oal_timer_init(&st_timer_etc[ul_core_id], ul_delay, p_func, arg);
        frw_timer_start_stamp[ul_core_id] = 0;
    }

#if defined(_PRE_DEBUG_MODE) && (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
    memset_s(timeout_track_etc, OAL_SIZEOF(frw_timeout_track_stru) * FRW_TIMEOUT_TRACK_NUM,
...
oal_void frw_timer_exit_etc(oal_void)
{
    ...
    for (ul_core_id = 0; ul_core_id < WLAN_FRW_MAX_NUM_CORES; ul_core_id++) {
        oal_timer_delete_sync(&st_timer_etc[ul_core_id]);
        frw_timer_start_stamp[ul_core_id] = 0;
    }
}
...
oal_void frw_timer_restart_etc(oal_void)
{
    ...
    for (ul_core_id = 0; ul_core_id < WLAN_FRW_MAX_NUM_CORES; ul_core_id++) {
        oal_timer_start(&st_timer_etc[ul_core_id], FRW_TIMER_DEFAULT_TIME);
    }
    need_restart_etc = OAL_FALSE;
}
...
oal_void frw_timer_stop_etc(oal_void)
{
    ...
    stop_timestamp_etc = (oal_uint32)OAL_TIME_GET_STAMP_MS();

    for (ul_core_id = 0; ul_core_id < WLAN_FRW_MAX_NUM_CORES; ul_core_id++) {
        oal_timer_delete(&st_timer_etc[ul_core_id]);
        frw_timer_start_stamp[ul_core_id] = 0;
    }

    need_restart_etc = OAL_TRUE;
}
...
oal_uint32 frw_timer_timeout_proc_etc(frw_event_mem_stru *pst_timeout_event)
{
    ...
    if (en_event_track_switch_etc == OAL_TRUE) {
        memset_s(&timeout_track_etc[timeout_track_idx_etc], OAL_SIZEOF(frw_timeout_track_stru),
    ...
    while (pst_timeout_entry != &timer_list_etc[ul_core_id]) {
        ...
        if (frw_time_after(ul_present_time, pst_timeout_element->ul_time_stamp)) {
            ...
            if (pst_timeout_element->en_is_enabled) {
                oal_spin_unlock_bh(&timer_list_spinlock_etc[ul_core_id]);
            ...
            if (ul_runtime_func_end >= (oal_uint32)OAL_JIFFIES_TO_MSECS(2)) {
                OAM_WARNING_LOG4(0, OAM_SF_FRW,
                                 "{frw_timer_timeout_proc_etc:: fileid=%u, linenum=%u, moduleid=%u, runtime=%u}",
                                 pst_timeout_element->ul_file_id,
                                 pst_timeout_element->ul_line_num,
                                 pst_timeout_element->en_module_id,
                                 ul_runtime_func_end);
            ...
            if (en_event_track_switch_etc == OAL_TRUE) {
                timeout_track_etc[timeout_track_idx_etc].st_timer_track[timeout_track_etc[timeout_track_idx_etc].uc_timer_cnt].ul_file_id = pst_timeout_element->ul_file_id;
                timeout_track_etc[timeout_track_idx_etc].st_timer_track[timeout_track_etc[timeout_track_idx_etc].uc_timer_cnt].ul_line_num = pst_timeout_element->ul_line_num;
                timeout_track_etc[timeout_track_idx_etc].st_timer_track[timeout_track_etc[timeout_track_idx_etc].uc_timer_cnt].ul_execute_time = ul_start_time - oal_5115timer_get_10ns();
                ul_sum_time += timeout_track_etc[timeout_track_idx_etc].st_timer_track[timeout_track_etc[timeout_track_idx_etc].uc_timer_cnt].ul_execute_time;
            }
#endif
        } else {
            break;
        ...
        if (en_event_track_switch_etc == OAL_TRUE) {
            OAL_INCR(timeout_track_etc[timeout_track_idx_etc].uc_timer_cnt, FRW_TIMER_TRACK_NUM);
        }
#endif
    ...
    if (oal_dlist_is_empty(&timer_list_etc[ul_core_id]) == OAL_FALSE) {
        ...
        oal_timer_start(&st_timer_etc[ul_core_id], ul_frw_timer_start);
    } else {
        frw_timer_start_stamp[ul_core_id] = 0;
    }
    ...
    if (en_event_track_switch_etc == OAL_TRUE) {
        if ((ul_sum_time * 10) > 300000) {
            OAL_INCR(timeout_track_idx_etc, FRW_TIMEOUT_TRACK_NUM);
        }
    }
#endif
...
oal_void frw_timer_add_timer_etc(frw_timeout_stru *pst_timeout)
{
    ...
    if (oal_dlist_is_empty(&timer_list_etc[pst_timeout->ul_core_id]) == OAL_TRUE) {
    ...
    if ((frw_timer_start_stamp[pst_timeout->ul_core_id] == 0) || (l_val > 0)) {
        oal_timer_start(&st_timer_etc[pst_timeout->ul_core_id], pst_timeout->ul_timeout);
                                    ...
                                    oal_uint32 ul_core_id)
{
    ...
    if (ul_timeout == 0 && en_is_periodic == OAL_TRUE) {
        OAM_ERROR_LOG4(0, OAM_SF_FRW,
            "{frw_timer_create_timer_etc::timeout value invalid! fileid: %d, line: %d, module: %d, core: %d}",
    ...
    oal_spin_lock_bh(&timer_list_spinlock_etc[pst_timeout->ul_core_id]);
    ...
    oal_spin_unlock_bh(&timer_list_spinlock_etc[pst_timeout->ul_core_id]);
                                                                   ...
                                                                   frw_timeout_stru *pst_timeout)
{
    ...
    if (pst_timeout->en_is_registerd == OAL_FALSE) {
        OAM_WARNING_LOG0(0, OAM_SF_FRW,
                         "{frw_timer_immediate_destroy_timer_etc::This timer is not enabled it should not be deleted}");
...
oal_void frw_timer_restart_timer_etc(frw_timeout_stru *pst_timeout, oal_uint32 ul_timeout,
                                     oal_bool_enum_uint8 en_is_periodic)
{
    if (pst_timeout == OAL_PTR_NULL) {
    ...
    if (pst_timeout->en_is_registerd == OAL_FALSE) {
        OAM_ERROR_LOG4(0, OAM_SF_FRW,
                       "{frw_timer_restart_timer_etc::This timer is not registerd!file_id=%d,line=%d,core=%d,mod=%d}",
                       pst_timeout->ul_file_id, pst_timeout->ul_line_num,
                       pst_timeout->ul_core_id, pst_timeout->en_module_id);
        return;
    }
...
oal_uint8 timer_pause_etc = OAL_FALSE;
#if defined(_PRE_FRW_TIMER_BIND_CPU) && defined(CONFIG_NR_CPUS)
oal_uint32 frw_timer_cpu_count_etc[CONFIG_NR_CPUS] = {0};
#endif

oal_void frw_timer_timeout_proc_event_etc(oal_ulong arg)
{
    ...
    if (en_event_track_switch_etc == OAL_TRUE) {
        if (os_time_etc == 0) {
            os_time_etc = oal_5115timer_get_10ns();
        } else {
            ul_time = oal_5115timer_get_10ns();
            timeout_track_etc[timeout_track_idx_etc].ul_os_timer_interval = os_time_etc - ul_time;
            os_time_etc = ul_time;
        }
    }
#endif
    ...
    do {
        ...
        if (cpu_id < CONFIG_NR_CPUS) {
            frw_timer_cpu_count_etc[cpu_id]++;
        }
    } while (0);
#endif

    if (timer_pause_etc == OAL_TRUE) {
        return;
    }
...
oal_void frw_timer_dump_timer_etc(oal_uint32 ul_core_id)
{
    ...
    OAM_WARNING_LOG1(0, OAM_SF_ANY, "frw_timer_dump_timer_etc::frw_timer_start_stamp [%u]",
                     frw_timer_start_stamp[ul_core_id]);

    OAL_DLIST_SEARCH_FOR_EACH(pst_dlist_entry, &timer_list_etc[ul_core_id])
    {
...
oal_void frw_timer_clean_timer(oam_module_id_enum_uint16 en_module_id)
{
    ...
    for (ul_core_id = 0; ul_core_id < WLAN_FRW_MAX_NUM_CORES; ul_core_id++) {
        ...
        if (oal_dlist_is_empty(&timer_list_etc[ul_core_id])) {
            frw_timer_start_stamp[ul_core_id] = 0;
        }
        oal_spin_unlock_bh(&timer_list_spinlock_etc[ul_core_id]);
    }
...
oal_module_symbol(timer_pause_etc);
oal_module_symbol(frw_timer_clean_timer);

#if defined(_PRE_DEBUG_MODE) && (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
oal_module_symbol(timeout_track_etc);
oal_module_symbol(timeout_track_idx_etc);
#endif
