...
typedef enum {
    ...
    IEEE80211_RADIOTAP_MCS_HAVE_BUTT
...
typedef enum {
    ...
    IEEE80211_RADIOTAP_VHT_KNOWN_BUTT
...
typedef enum {
    ...
    IEEE80211_RADIOTAP_VHT_FLAG_BUTT
...
typedef enum {
    ...
    IEEE80211_RADIOTAP_CODING_LDPC_BUTT
...
typedef oal_int32 (*oal_iw_handler)(struct oal_net_device *dev, oal_iw_request_info_stru *info,
...
typedef struct oal_net_device {
    ...
    struct oal_ethtool_ops *ethtool_ops;
    oal_uint last_rx;
    oal_uint flags;
...
typedef struct oal_ethtool_ops {
    oal_int32 (*get_regs_len)(struct oal_net_device *);
} oal_ethtool_ops_stru;
...
typedef struct oal_cfg80211_ops_tag {
    ...
    oal_int32 (*connect)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev, oal_cfg80211_conn_stru *pst_sme);
    oal_int32 (*disconnect)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev, oal_uint16 us_reason_code);
    oal_int32 (*set_channel)(oal_wiphy_stru *pst_wiphy, oal_ieee80211_channel *pst_chan,
    ...
    oal_int32 (*change_virtual_intf)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
                                     enum nl80211_iftype en_type, oal_uint32 *pul_flags,
                                     oal_vif_params_stru *pst_params);
    oal_int32 (*add_station)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
                             oal_uint8 *puc_mac, oal_station_parameters_stru *pst_sta_parms);
    oal_int32 (*del_station)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev, oal_uint8 *puc_mac);
    oal_int32 (*change_station)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
                                oal_uint8 *puc_mac, oal_station_parameters_stru *pst_sta_parms);
    oal_int32 (*get_station)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
                             oal_uint8 *puc_mac, oal_station_info_stru *pst_sta_info);
    oal_int32 (*dump_station)(oal_wiphy_stru *wiphy, oal_net_device_stru *dev, oal_int32 idx,
                              oal_uint8 *mac, oal_station_info_stru *pst_sta_info);
    /* lm add new code begin */
    oal_int32 (*change_beacon)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev,
                               oal_beacon_data_stru *pst_beacon_info);
    oal_int32 (*start_ap)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev,
                          oal_ap_settings_stru *pst_ap_settings);
    oal_int32 (*stop_ap)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev);
    oal_int32 (*change_bss)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev,
                            oal_bss_parameters *pst_bss_params);
    oal_int32 (*set_power_mgmt)(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_ndev,
                                oal_bool_enum enabled, oal_int32 ul_timeout);
    /* lm add new code end */
...
extern oal_sock_stru sock;

typedef struct {
    oal_proc_dir_entry_stru *proc_net;
} oal_net_stru;

extern oal_net_stru init_net;
...
extern oal_net_device_stru *past_net_device[];
 ...
 */
EXTERN_C OAL_INLINE oal_net_device_stru *oal_dev_get_by_name(const oal_int8 *pc_name)
{
    oal_uint32 u_i;

    for (u_i = 0; u_i < WLAN_VAP_SUPPORT_MAX_NUM_LIMIT; u_i++) {
        if (past_net_device[u_i] != OAL_PTR_NULL) {
            if (!(oal_strcmp(past_net_device[u_i]->name, pc_name))) {
                return past_net_device[u_i];
            }
        }
    }
                                                         ...
                                                         oal_void *p_set_up, oal_uint32 ul_txqs, oal_uint32 ul_rxqs)
{
    ...
    memset(pst_net_dev, 0, OAL_SIZEOF(oal_net_device_stru));

    /* 将name保存到netdeivce */
    memcpy(pst_net_dev->name, puc_name, ul_size);
...
OAL_STATIC OAL_INLINE oal_void oal_net_free_netdev(oal_net_device_stru *pst_netdev)
{
    ...
    if (pst_netdev->priv != OAL_PTR_NULL) {
...
OAL_INLINE oal_int32 oal_net_device_set_macaddr(oal_net_device_stru *pst_dev, oal_void *pst_addr)
{
    ...
    memcpy((oal_void *)pst_dev->dev_addr, (oal_void *)pst_mac->sa_data, 6);
...
OAL_INLINE oal_int32 oal_net_register_netdev(oal_net_device_stru *pst_net_dev)
{
    ...
    for (u_i = 0; u_i < WLAN_VAP_SUPPORT_MAX_NUM_LIMIT; u_i++) {
        if (past_net_device[u_i] == OAL_PTR_NULL) {
            past_net_device[u_i] = pst_net_dev;
...
OAL_STATIC OAL_INLINE oal_void oal_net_unregister_netdev(oal_net_device_stru *p_net_device)
{
    ...
    for (u_i = 0; u_i < WLAN_VAP_SUPPORT_MAX_NUM_LIMIT; u_i++) {
        if (past_net_device[u_i] == p_net_device) {
            past_net_device[u_i] = OAL_PTR_NULL;
 ...
 * 输入参数  : ul_size: 分配内存的大小
             ...
             ul_align: 需要几字节对齐
 */
OAL_INLINE oal_netbuf_stru *oal_netbuf_alloc(oal_uint32 ul_size, oal_int32 l_reserve, oal_uint32 l_align)
{
    oal_int32 l_offset;
    ...
    memset(pst_netbuf, 0, OAL_SIZEOF(oal_netbuf_stru));

    /* 分配数据空间 */
    pst_netbuf->head = (oal_uint8 *)oal_memalloc(ul_size);

    if (pst_netbuf->head == OAL_PTR_NULL) {
        oal_free(pst_netbuf);
    ...
    /* 结构体初始化 */
    memset(pst_netbuf->head, 0, ul_size);
    ...
    /* 字节对齐 */
    if (l_align) {
        l_offset = (oal_int32)(((oal_uint)pst_netbuf->data) % l_align);
...
OAL_INLINE oal_uint32 oal_netbuf_free(oal_netbuf_stru *pst_netbuf)
{
    ...
    if (oal_atomic_read(&pst_netbuf->users) == 0) {
        return OAL_SUCC;
    }

    if (pst_netbuf->cloned != 1) {
...
OAL_STATIC OAL_INLINE oal_void oal_netbuf_copy_header(oal_netbuf_stru *pst_newbuf, oal_netbuf_stru *pst_oldbuf)
{
    ...
    memcpy((oal_void *)pst_newbuf->cb, (oal_void *)pst_oldbuf->cb, sizeof(pst_oldbuf->cb));
}
...
OAL_INLINE oal_netbuf_stru *oal_netbuf_copy(oal_netbuf_stru *pst_netbuf, oal_gfp_enum_uint8 en_priority)
{
    ...
    /* 数据拷贝 */
    memcpy((oal_void *)pst_newbuf->head, (oal_void *)pst_netbuf->head, ul_size);
...
{
    oal_netbuf_stru *pst_newbuf = NULL;
...
OAL_INLINE oal_netbuf_stru *oal_netbuf_realloc_headroom(oal_netbuf_stru *pst_netbuf, oal_uint32 ul_headroom)
{
    ...
    pst_nbuf = oal_netbuf_alloc((pst_netbuf->truesize - OAL_SIZEOF(oal_netbuf_stru) + ul_headroom),
                                (oal_int32)ul_headlen, 4);

    oal_netbuf_copy_header(pst_nbuf, pst_netbuf);

    oal_netbuf_put(pst_nbuf, pst_netbuf->len);

    memcpy(pst_nbuf->data, pst_netbuf->data, pst_netbuf->len);
...
OAL_INLINE oal_netbuf_stru *oal_netbuf_realloc_tailroom(oal_netbuf_stru *pst_netbuf, oal_uint32 ul_tailroom)
{
    ...
    pst_nbuf = oal_netbuf_alloc((pst_netbuf->truesize - OAL_SIZEOF(oal_netbuf_stru) + ul_tailroom),
                                (oal_int32)ul_headlen, 4);
...
{
    return oal_netbuf_delist(pst_list_head);
}
 ...
 */
OAL_INLINE oal_uint32 oal_netbuf_get_appointed_netbuf(oal_netbuf_stru *pst_netbuf, oal_uint8 uc_num,
                                                      oal_netbuf_stru **pst_expect_netbuf)
{
    oal_uint8 uc_buf_num;

    if (OAL_ANY_NULL_PTR2(pst_netbuf, pst_expect_netbuf)) {
        return OAL_ERR_CODE_PTR_NULL;
    }
...
OAL_INLINE oal_netbuf_stru *oal_netbuf_clone(oal_netbuf_stru *pst_buf)
{
    ...
    if (OAL_UNLIKELY(pst_nbuf == OAL_PTR_NULL)) {
        return OAL_PTR_NULL;
    }
    ...
    memcpy((oal_void *)pst_nbuf->cb, (oal_void *)pst_buf->cb, sizeof(pst_buf->cb));
...
{
    // 由于在Windows 中，需要申请空间才能继续执行，故将 oal_wiphy_new 返回值为真。
                                                       ...
                                                       oal_int32 l_payload, oal_int32 l_flags)
{
    ...
    if (OAL_NLMSG_ALIGN(l_size) - l_size != 0) {
        memset((oal_uint8 *)OAL_NLMSG_DATA(pst_nlmsghdr) + l_payload, 0, OAL_NLMSG_ALIGN(l_size) - l_size);
    }
 ...
 */
OAL_STATIC OAL_INLINE oal_uint32 oal_netbuf_copydata(oal_netbuf_stru *pst_netbuf_sc, oal_uint32 ul_offset,
                                                     oal_void *p_dst, oal_uint32 ul_len)
{
    oal_void *p_scr;
    p_scr = OAL_NETBUF_DATA(pst_netbuf_sc) + ul_offset;

    memcpy(p_dst, p_scr, ul_len);
...
{
    memcpy(pst_ipv6_dst, pst_ipv6_src, OAL_SIZEOF(oal_in6_addr));
}
                                                      ...
                                                      oal_uint8 *puc_target_hw)
{
    ...
    if (puc_src_hw == NULL) {
        memcpy(puc_src_hw, pst_dev->dev_addr, 6);
    }

    if (puc_dest_hw == NULL) {
        memcpy(puc_dest_hw, ac_bcast, 6);
    }
    ...
    memcpy(pst_arp_ptr, puc_src_hw, 6);
    pst_arp_ptr += 6;
    memcpy(pst_arp_ptr, &ul_src_ip, 4);
    pst_arp_ptr += 4;
    if (puc_target_hw != NULL) {
        memcpy(pst_arp_ptr, puc_target_hw, 6);
    } else {
        memset(pst_arp_ptr, 0, 6);
    }

    pst_arp_ptr += 6;
    memcpy(pst_arp_ptr, &ul_dest_ip, 4);
