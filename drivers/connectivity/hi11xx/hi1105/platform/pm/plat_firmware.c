...
/*
 * 函 数 名  : msg_send_and_recv_except_etc
...
int32 string_to_num_etc(uint8 *string, int32 *number)
{
    ...
    for (i = 0; (string[i] >= '0') && (string[i] <= '9'); i++) {
        l_num = (l_num * 10) + (string[i] - '0'); /* 字符串转整数逻辑需要 */
    }
...
/*
 * 函 数 名  : number_type_cmd_send_etc
...
/*
 * 函 数 名  : update_device_cali_count_etc
...
/*
 * 函 数 名  : download_bfgx_cali_data_etc
...
int32 wifi_device_mem_dump(struct st_wifi_dump_mem_info *pst_mem_dump_info, uint32 count)
{
    ...
    for (i = 0; i < count; i++) {
        *pcount = pst_mem_dump_info[i].size;
        ...
        wifi_memdump_enquenue_etc(buff, 4); /* 后续申请sk_buff的大小 */
...
/*
 * 函 数 名  : sdio_read_device_mem_etc
...
int32 exec_number_type_cmd_etc(uint8 *Key, uint8 *Value)
{
    ...
    if (!OS_STR_CMP((int8 *)Key, WMEM_CMD_KEYWORD)) {
        ...
        l_delay_ms = simple_strtol(Value, NULL, 10); /* 将字符串转换成10进制数 */
...
int32 exec_file_type_cmd_etc(uint8 *Key, uint8 *Value)
{
    ...
    transmit_limit = firmware_down_buf_len_etc;  // DataBufLen_etc在调用前保证为大于0
...
/*
 * 函 数 名  : exec_shutdown_type_cmd_etc
...
/*
 * 函 数 名  : execute_download_cmd_etc
...
#define BUCK_PARA_LEN   16
    ...
    if (OS_STR_STR((int8 *)puc_cfg_buffer, QUIT_CMD_KEYWORD) != NULL) {
        ...
        if (INI_FAILED == get_cust_conf_string_etc(INI_MODU_PLAT, "buck_custom_addr", buff, sizeof(buff) - 1)) {
            PS_PRINT_ERR("can't found buck_mode ini");
            return ERROR_TYPE_CMD;
        }

        oal_itoa(board_info_etc.buck_param, buck_para, OAL_SIZEOF(buck_para));
        ...
        ret = memcpy_s(puc_cmd_name, cmd_name_len, "WRITEM", 6);
        if (ret != EOK) {
...
int32 firmware_download_etc(uint32 ul_index)
{
    ...
    firmware_down_buf_len_etc = OAL_ROUND_DOWN(firmware_down_buf_len_etc, 8); /* 清除低3bit，保证8字节对齐 */
...
int32 firmware_cfg_path_init(void)
{
    ...
    if (get_hi110x_subchip_type() == BOARD_VERSION_HI1103) {
        ...
        if (isAsic_etc()) {
            cfg_path_etc = hi1105_asic_cfg_patch_in_vendor;
            set_hi1103_asic_type(HI1105_ASIC);
        } else {
...
/*
 * 函 数 名  : nfc_buffer_data_recv_etc
 * 功能描述  : 保存nfc buffer数据
 */
...
int32 get_device_ram_test_result(int32 is_wcpu, uint32 *cost)
{
    ...
    *cost = time_cost * 31;
    ...
    if (ram_test_detail_tsensor_dump) {
        ...
        l_temp_val = ((((oal_int32)us_temp_data - 118) * 165) / 815) - 40; /* 温度码转换公式 */
