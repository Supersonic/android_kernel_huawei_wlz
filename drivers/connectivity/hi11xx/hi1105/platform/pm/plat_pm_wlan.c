 ...
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
OAL_STATIC oal_int32 wlan_pm_custom_cali(struct wlan_pm_s *pst_wlan_pm)
{
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
...
#endif
        {
            if (oal_wait_for_completion_timeout(&pst_wlan_pm->pst_bus->st_device_ready,
...
oal_int32 wlan_device_mem_check_etc(oal_int32 l_runing_test_mode)
{
    ...
    if (l_chip_type == BOARD_VERSION_HI1105) {
        ul_ret = set_wlan_mem_check_mdelay(5000);
        ul_ret += set_bfgx_mem_check_mdelay(4000);
    } else {
        ul_ret = set_wlan_mem_check_mdelay(3000);
        ul_ret += set_bfgx_mem_check_mdelay(5000);
    }
