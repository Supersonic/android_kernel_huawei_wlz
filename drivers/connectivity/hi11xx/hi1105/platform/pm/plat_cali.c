...
static bfgx_ini_cmd bfgx_ini_config_cmd_1103[BFGX_BT_CUST_INI_SIZE / 4] = {
    { "bt_maxpower",                       0 },
    { "bt_edrpow_offset",                  0 },
    { "bt_blepow_offset",                  0 },
...
/* 定义不能超过BFGX_BT_CUST_INI_SIZE/4 (128) 最后使用 { NULL, 0 } 结尾 */
static bfgx_ini_cmd bfgx_ini_config_cmd_1105[BFGX_BT_CUST_INI_SIZE / 4] = {
    { "bt_maxpower",                       0 },
    { "bt_edrpow_offset",                  0 },
    { "bt_blepow_offset",                  0 },
    { "bt_cali_txpwr_pa_ref_num",          0 },
...
int32 bfgx_cust_ini_init(void)
{
    ...
    switch (l_subchip_type) {
        case BOARD_VERSION_HI1103:
            p_bfgx_ini_cmd = (bfgx_ini_cmd *)&bfgx_ini_config_cmd_1103;
            PS_PRINT_DBG("bfgx load ini 1103");
            break;
        case BOARD_VERSION_HI1105:
            p_bfgx_ini_cmd = (bfgx_ini_cmd *)&bfgx_ini_config_cmd_1105;
    ...
    for (i = 0; i < (BFGX_BT_CUST_INI_SIZE / 4); i++) {
        /* 定制化项目列表名称以 null 结尾 */
        ...
        /* 获取ini的配置值 */
        l_ret = get_cust_conf_int32_etc(INI_MODU_DEV_BT, p_bfgx_ini_cmd[i].name, &l_cfg_value);
        if (l_ret == INI_FAILED) {
