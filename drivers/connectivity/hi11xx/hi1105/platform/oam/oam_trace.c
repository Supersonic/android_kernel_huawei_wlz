 ...
 * 输入参数  : en_func_index : rx tx 节点的索引
 */
 ...
 *             past_profiling_data : rx或tx全局变量中的记录节点信息的数组，比如
...
oal_void oam_profiling_rx_show_offset(oal_uint8 uc_show_level)
{
    ...
    uc_packet_num = (oam_profiling_statistic_rx.uc_skb_id_idx < OAM_PROFILING_RX_PACKET_MAX_COUNT ?
                     oam_profiling_statistic_rx.uc_skb_id_idx : OAM_PROFILING_TX_PACKET_MAX_COUNT);
    ...
    for (uc_rx_profiling_point_idx = 0; uc_rx_profiling_point_idx < OAM_PROFILING_RX_OFFSET_COUNT;
         uc_rx_profiling_point_idx++) {
        OAL_IO_PRINT("%d-%d ", ast_rx_profiling_offset_section[uc_rx_profiling_point_idx][0],
 ...
 *             (2)函数索引
 */
oal_void oam_profiling_tx_save_data(oal_void *pst_netbuf, oam_profiling_tx_func_enum_uint8 en_func_index)
{
    /* 开关判断 */
...
oal_void oam_profiling_tx_show_offset(oal_uint8 uc_show_level)
{
    ...
    uc_packet_num = (oam_profiling_statistic_tx.uc_skb_id_idx < OAM_PROFILING_TX_PACKET_MAX_COUNT ?
                     oam_profiling_statistic_tx.uc_skb_id_idx : OAM_PROFILING_TX_PACKET_MAX_COUNT);
    ...
    for (uc_tx_profiling_point_idx = 0; uc_tx_profiling_point_idx < OAM_PROFILING_TX_OFFSET_COUNT;
        ...
        OAL_IO_PRINT("%d-%d ", ast_tx_profiling_offset_section[uc_tx_profiling_point_idx][0],
...
oal_uint32 oam_profiling_statistic_init(oal_void)
{
    ...
    memset_s(&oam_profiling_statistic_chswitch, OAL_SIZEOF(oam_profiling_statistic_chswitch),
             0, OAL_SIZEOF(oam_profiling_statistic_chswitch));
