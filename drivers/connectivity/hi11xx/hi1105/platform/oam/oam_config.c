...
/* 全局变量定义 */
oam_customize_stru oam_customize_etc;

/* 函数实现 */
oal_void oam_register_init_hook_etc(oam_msg_moduleid_enum_uint8 en_moduleid, p_oam_customize_init_func p_func)
{
    oam_customize_etc.customize_init[en_moduleid] = p_func;
}

/*
 * 函 数 名  : oam_cfg_get_one_item_etc
 ...
 *             pl_val         :配置项的值
 */
oal_int32 oam_cfg_get_one_item_etc(oal_int8 *pc_cfg_data_buf,
                                   oal_int8 *pc_section,
                                   oal_int8 *pc_key,
                                   oal_int32 *pl_val)
{
...
/*
 * 函 数 名  : oam_cfg_read_file_to_buf_etc
 * 功能描述  : 从配置文件中读取所有数据，保存到一个buf中
 * 输入参数  : pc_cfg_data_buf:从文件读出数据后存放的buf
 *             ul_file_size   :文件大小(字节数)
 */
oal_int32 oam_cfg_read_file_to_buf_etc(oal_int8 *pc_cfg_data_buf,
...
/*
 * 函 数 名  : oam_cfg_decrypt_all_item_etc
 ...
 *             ul_cipher_len :密文长度
 */
oal_uint32 oam_cfg_decrypt_all_item_etc(oal_aes_key_stru *pst_aes_key,
