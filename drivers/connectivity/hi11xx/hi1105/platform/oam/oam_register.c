...
/* 全局变量定义 */
oam_reg_manage_stru oam_reg_mng_etc;
...
oal_uint32 oam_reg_malloc_etc(oam_reg_type_enum_uint8 en_type)
{
    ...
    if (oam_reg_mng_etc.past_reg[en_type] != OAL_PTR_NULL) {
        OAL_IO_PRINT("Err! register %u already exist!\n", en_type);
        return OAL_SUCC;
    }

    ul_reg_num = oam_reg_get_num_etc(en_type);
    pst_reg = (oam_reg_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, (oal_uint16)(ul_reg_num * sizeof(oam_reg_stru)),
                                            OAL_TRUE);
    oam_reg_mng_etc.past_reg[en_type] = pst_reg;
    ...
    oam_reg_mng_etc.aul_reg_num[en_type] = ul_reg_num;
    return OAL_SUCC;
}

/*
 * 函 数 名  : oam_reg_free_etc
 * 功能描述  : 释放某类寄存器的内存
 */
oal_uint32 oam_reg_free_etc(oam_reg_type_enum_uint8 en_type)
{
    ...
    /* 不存在，返回成功 */
    pst_reg = oam_reg_mng_etc.past_reg[en_type];
    if (pst_reg == OAL_PTR_NULL) {
        return OAL_SUCC;
    }

    OAL_MEM_FREE(pst_reg, OAL_TRUE);
    oam_reg_mng_etc.past_reg[en_type] = OAL_PTR_NULL;
    oam_reg_mng_etc.aul_reg_num[en_type] = 0;

    return OAL_SUCC;
}

/*
 * 函 数 名  : oam_reg_set_flag_etc
 * 功能描述  : 设置寄存器是否需要刷新的标志
 * 输入参数  : 事件类型，事件子类型，TRUE/FALSE
 */
oal_void oam_reg_set_flag_etc(oam_reg_type_enum_uint8 en_type, oam_reg_subtype_enum_uint32 en_subtype,
                              oal_bool_enum_uint8 en_flag)
{
    ...
    ul_subreg_num = oam_reg_mng_etc.aul_reg_num[en_type];
    ...
    for (ul_loop = 0; ul_loop < ul_subreg_num; ul_loop++) {
        if (pst_reg->en_rpt_flag) {
            oam_reg_mng_etc.ul_reg_flag_bitmap |= (1 << en_type);
            return;
        }
        pst_reg++;
    }
    oam_reg_mng_etc.ul_reg_flag_bitmap &= (~(oal_uint32)(1 << en_type));
}
 ...
 */
oal_void oam_reg_set_flag_addr_etc(oam_reg_type_enum_uint8 en_type, oal_uint32 ul_startaddr,
...
{
    return oam_reg_mng_etc.en_refresh_flag;
}

/*
 * 函 数 名  : oam_reg_is_need_refresh_etc
 * 功能描述  : 判断该事件是否触发寄存器刷新
 */
oal_uint8 oam_reg_is_need_refresh_etc(oam_reg_evt_enum_uint32 en_evt_type)
{
    ...
    if (oam_reg_mng_etc.ul_reg_flag_bitmap == 0) {
        return OAL_FALSE;
    }

    /* 判断该事件上报开关是否打开  */
    /* 用户没有设置该事件，不需要上报 */
    if (oam_reg_mng_etc.aul_refresh_evt[en_evt_type] != OAL_TRUE) {
        return OAL_FALSE;
    }

    /* 事件需要判断evt tick有没有计到0，如果计到0，则刷新上报数据，并更新evt_tick  */
    if (oam_reg_mng_etc.aul_evt_tick[en_evt_type] == 0) {
        return OAL_FALSE;
    }

    oam_reg_mng_etc.aul_evt_tick[en_evt_type]--;
    if (oam_reg_mng_etc.aul_evt_tick[en_evt_type] != 0) {
        return OAL_FALSE;
    }

    oam_reg_mng_etc.aul_evt_tick[en_evt_type] = oam_reg_mng_etc.aul_evt_tick_cfg[en_evt_type];
    return OAL_TRUE;
}

/*
 * 函 数 名  : oam_reg_get_evt_name_etc
 * 功能描述  : 获取事件字符名
 * 输入参数  : 事件
 * 输出参数  : 事件字符名
 */
...
oal_void oam_reg_set_evt_etc(oam_reg_evt_enum_uint32 en_evt_type, oal_uint32 ul_tick)
{
    ...
    if (en_evt_type >= OAM_REG_EVT_BUTT) {
        for (ul_evt_type = 0; ul_evt_type < OAM_REG_EVT_BUTT; ul_evt_type++) {
            oam_reg_mng_etc.aul_refresh_evt[ul_evt_type] = ((0 == ul_tick) ? OAL_FALSE : OAL_TRUE);
            oam_reg_mng_etc.aul_evt_tick_cfg[ul_evt_type] = ul_tick;
            oam_reg_mng_etc.aul_evt_tick[ul_evt_type] = ul_tick;
        }
        return;
    }

    oam_reg_mng_etc.aul_refresh_evt[en_evt_type] = ((0 == ul_tick) ? OAL_FALSE : OAL_TRUE);
    oam_reg_mng_etc.aul_evt_tick_cfg[en_evt_type] = ul_tick;
    oam_reg_mng_etc.aul_evt_tick[en_evt_type] = ul_tick;
}

/*
 * 函 数 名  : oam_reg_recv_msg_etc
 * 功能描述  : netlink接收处理入口函数
 */
oal_uint32 oam_reg_recv_msg_etc(oal_uint8 *puc_data, oal_uint32 ul_len)
{
    oam_reg_mng_etc.uc_ack_flag = OAL_TRUE;

    return OAL_SUCC;
}

/*
 * 函 数 名  : oam_reg_rpt_buff_etc
...
oal_uint32 oam_reg_rpt_buff_etc(oal_void)
{
    ...
    pst_send_head = (oam_reg_send_head_stru *)oam_reg_mng_etc.puc_send_buff;
    ...
    oam_reg_mng_etc.ul_rpt_count++;
    ret = oam_netlink_kernel_send_etc((oal_uint8 *)pst_send_head,
...
oal_uint32 oam_reg_nb2buf_etc(oal_netbuf_stru *pst_netbuf)
{
    ...
    pst_send_buff = (oam_reg_send_head_stru *)oam_reg_mng_etc.puc_send_buff;
    ...
    if (memcpy_s((oal_uint8 *)pst_send_buff + sizeof(oam_reg_send_head_stru) + ul_pkt_num * sizeof(oam_reg_rpt),
                 ...
                 oal_netbuf_data(pst_netbuf), sizeof(oam_reg_rpt)) != EOK) {
...
oal_void oam_reg_wq_etc(oal_work_stru *pst_work)
{
    ...
    ul_nb_num = oam_reg_mng_etc.ul_nb;
    ...
    while (ul_nb_num >= OAM_REG_MAX_PKT_ONE_BUFF) {
        ...
        for (ul_loop = 0; ul_loop < OAM_REG_MAX_PKT_ONE_BUFF; ul_loop++) {
            pst_netbuf = oal_netbuf_delist(&oam_reg_mng_etc.st_wq.st_netbuf_head);
            /* 没有元素，break掉 */
            if (pst_netbuf == OAL_PTR_NULL) {
                break;
            }
...
{
    memset_s(&oam_reg_mng_etc, sizeof(oam_reg_manage_stru), 0, sizeof(oam_reg_manage_stru));

    oam_reg_mng_etc.puc_send_buff = (oal_uint8 *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, OAM_REG_MAX_SEND_BUF_SIZE,
                                                               OAL_TRUE);
    if (oam_reg_mng_etc.puc_send_buff == OAL_PTR_NULL) {
        OAL_IO_PRINT("oam_reg_init_etc: puc_send_buff is NULL!\n");
        return;
    }
    memset_s(oam_reg_mng_etc.puc_send_buff, OAM_REG_MAX_SEND_BUF_SIZE, 0, OAM_REG_MAX_SEND_BUF_SIZE);
    ...
    /* 初始化工作队列 */
    memset_s((void *)&oam_reg_mng_etc.st_wq, OAL_SIZEOF(oam_reg_workqueue_stru),
             0, OAL_SIZEOF(oam_reg_workqueue_stru));
    OAL_INIT_WORK(&oam_reg_mng_etc.st_wq.st_wk, oam_reg_wq_etc);
    oal_netbuf_list_head_init(&oam_reg_mng_etc.st_wq.st_netbuf_head);
}

/*
 * 函 数 名  : oam_reg_exit_etc
 * 功能描述  : 析构初始化
 */
oal_void oam_reg_exit_etc(oal_void)
{
    ...
    if (oam_reg_mng_etc.puc_send_buff != OAL_PTR_NULL) {
        OAL_MEM_FREE(oam_reg_mng_etc.puc_send_buff, OAL_TRUE);
        oam_reg_mng_etc.puc_send_buff = OAL_PTR_NULL;
    }

    /* 去注册netlink接收函数 */
    oam_netlink_ops_unregister_etc(OAM_NL_CMD_REG);

    oal_cancel_work_sync(&oam_reg_mng_etc.st_wq.st_wk);
    oal_netbuf_queue_purge(&oam_reg_mng_etc.st_wq.st_netbuf_head);
}

/*
 * 函 数 名  : oam_reg_add_netbuf_etc
...
oal_uint32 oam_reg_add_netbuf_etc(oam_reg_rpt *pst_reg_rpt)
{
    ...
    *pst_reg_rpt_nb = *pst_reg_rpt;
    oal_netbuf_add_to_list_tail(pst_queue_netbuf, &oam_reg_mng_etc.st_wq.st_netbuf_head);
    oam_reg_mng_etc.ul_nb++;

    return OAL_SUCC;
}

/*
 * 函 数 名  : oam_reg_report_etc
...
oal_void oam_reg_report_etc(oal_void)
{
    ...
    st_rpt.us_sn = 0;
    st_rpt.un_rpt.st_rpt_head.en_evt = oam_reg_mng_etc.en_evt_type;
    st_rpt.un_rpt.st_rpt_head.ul_timestamp = oam_reg_mng_etc.ul_time_stamp_start;
    oam_reg_add_netbuf_etc(&st_rpt);
    oam_reg_mng_etc.ul_rpt_count++;

    for (ul_reg_type = 0; ul_reg_type < OAM_REG_BUTT; ul_reg_type++) {
        pst_reg = oam_reg_mng_etc.past_reg[ul_reg_type];
        if (pst_reg == OAL_PTR_NULL) {
            continue;
        }
        OAL_IO_PRINT("oam_reg_report_etc regtype %u, regnum %u \n",
                     ul_reg_type, oam_reg_mng_etc.aul_reg_num[ul_reg_type]);
        for (ul_reg_num = 0; ul_reg_num < oam_reg_mng_etc.aul_reg_num[ul_reg_type]; ul_reg_num++) {
    ...
    st_rpt.us_sn++;
    st_rpt.un_rpt.st_rpt_head.en_evt = oam_reg_mng_etc.en_evt_type;
    st_rpt.un_rpt.st_rpt_head.ul_timestamp = oam_reg_mng_etc.ul_time_stamp_end;
    oam_reg_add_netbuf_etc(&st_rpt);

    /* 启动workqueue */
    oal_workqueue_schedule(&oam_reg_mng_etc.st_wq.st_wk);
...
/*lint -e19*/
oal_module_symbol(oam_reg_mng_etc);
