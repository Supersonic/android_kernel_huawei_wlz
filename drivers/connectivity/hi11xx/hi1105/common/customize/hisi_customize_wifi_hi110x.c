                                                                                          ...
                                                                                          oal_uint32 *pul_data_len)
{
    ...
    st_syn_msg.en_syn_id = CUSTOM_CFGID_PRIV_INI_HAL_PS_RSSI;
                                                                                        ...
                                                                                        oal_uint32 *pul_data_len)
{
    ...
    st_syn_msg.en_syn_id = CUSTOM_CFGID_PRIV_INI_HAL_PS_PPS;
...
OAL_STATIC oal_uint8 hwifi_custom_cali_ini_param(oal_uint8 uc_priv_cfg_value)
{
    ...
    } else if (wlan_cal_disable_switch == OAL_TRUE) {
...
{
    if (hcc_bus_flowctrl_init_etc(*puc_priv_cfg_value) != OAL_SUCC) {
                                                             ...
                                                             oal_uint8 *puc_priv_cfg_value)
{
    switch (uc_cfg_id) {
        ...
        case WLAN_CFG_PRIV_MBO_SWITCH:
            ...
            OAL_IO_PRINT("hwifi_custom_adapt_mac_device_priv_ini_param::mbo switch[%d].\r\n", *puc_priv_cfg_value);
            break;
        case WLAN_CFG_PRIV_DYNAMIC_DBAC_SWITCH:
            pst_syn_msg->en_syn_id = CUSTOM_CFGID_PRIV_INI_DYNAMIC_DBAC_ID;
            OAL_IO_PRINT("hwifi_custom_adapt_mac_device_priv_ini_param::dynamic_dbac switch[%d].\r\n", *puc_priv_cfg_value);
            break;
...
OAL_STATIC oal_void hwifi_config_init_iq_lpf_nvram_param(oal_void)
{
    ...
    /*  5g iq cali lvl  */
    l_ret += memcpy_s(pst_cust_nv_params->auc_5g_iq_cali_lpf_params,
                      NUM_OF_NV_5G_LPF_LVL, &l_nvram_params, NUM_OF_NV_5G_LPF_LVL);

    /* 参数有效性检查 */
    for (l_cfg_id = 0; l_cfg_id < NUM_OF_NV_5G_LPF_LVL; l_cfg_id++) {
        if (CUS_VAL_INVALID(pst_cust_nv_params->auc_5g_iq_cali_lpf_params[l_cfg_id],
            CUS_IQ_CALI_LPF_MAX, CUS_IQ_CALI_LPF_MIN)) {
                                           ...
                                           oal_uint8 uc_param_num)
{
    ...
    if (INI_SUCC == get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[uc_cus_id].name, &ul_nvram_params)) {
    ...
    for (l_cfg_idx_one = 0; l_cfg_idx_one < WLAN_BW_CAP_BUTT; l_cfg_idx_one++) {
        for (l_cfg_idx_two = 0; l_cfg_idx_two < WLAN_HE_RU_SIZE_BUTT; l_cfg_idx_two++) {
            OAL_IO_PRINT("%s[%d][%d]:%d \n", "5g_cfg_tpc_ru_pow", l_cfg_idx_one, l_cfg_idx_two,
                pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[l_cfg_idx_one][l_cfg_idx_two]);
        }
    }
    for (l_cfg_idx_one = 0; l_cfg_idx_one < WLAN_BW_CAP_80M; l_cfg_idx_one++) {
        for (l_cfg_idx_two = 0; l_cfg_idx_two < WLAN_HE_RU_SIZE_996; l_cfg_idx_two++) {
            OAL_IO_PRINT("%s[%d][%d]:%d \n", "2g_cfg_tpc_ru_pow", l_cfg_idx_one, l_cfg_idx_two,
                pst_cust_nv_params->ac_fullbandwidth_to_ru_power_2g[l_cfg_idx_one][l_cfg_idx_two]);
        }
    }
...
static oal_void hwifi_get_delt_ru_pow_val_5g(void)
{
    ...
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_20M_5G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_20M_5G];
    }
    l_ret = memcpy_s(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_20M], sizeof(l_nvram_params), \
             &l_nvram_params, sizeof(l_nvram_params));

    /* 40m 26~242tone ru_power */
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_40M_5G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_40M_5G];
    }
    l_ret += memcpy_s(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_40M], sizeof(l_nvram_params), \
             &l_nvram_params, sizeof(l_nvram_params));

    /* 40m 484tone/80m 26~106tone ru_power */
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_80M_L_5G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_80M_L_5G];
    }
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_40M][WLAN_HE_RU_SIZE_484] = CUS_GET_FIRST_BYTE(l_nvram_params);
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_80M][WLAN_HE_RU_SIZE_26] = CUS_GET_SECOND_BYTE(l_nvram_params);
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_80M][WLAN_HE_RU_SIZE_52] = CUS_GET_THIRD_BYTE(l_nvram_params);
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_80M][WLAN_HE_RU_SIZE_106] = CUS_GET_FOURTH_BYTE(l_nvram_params);

    /* 80m 242~996tone/160m 26tone ru_power */
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_80M_H_5G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_80M_H_5G];
    }
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_80M][WLAN_HE_RU_SIZE_242] = CUS_GET_FIRST_BYTE(l_nvram_params);
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_80M][WLAN_HE_RU_SIZE_484] = CUS_GET_SECOND_BYTE(l_nvram_params);
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_80M][WLAN_HE_RU_SIZE_996] = CUS_GET_THIRD_BYTE(l_nvram_params);
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_160M][WLAN_HE_RU_SIZE_26] = CUS_GET_FOURTH_BYTE(l_nvram_params);

    /* 160m 52~996tone ru_power */
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_160M_L_5G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_160M_L_5G];
    }
    l_ret += memcpy_s(&(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_160M][WLAN_HE_RU_SIZE_52]), sizeof(l_nvram_params), \
                &l_nvram_params, sizeof(l_nvram_params));
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_160M_H_5G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_160M_H_5G];
    }
    l_ret += memcpy_s(&(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[WLAN_BW_CAP_160M][WLAN_HE_RU_SIZE_996]), sizeof(oal_uint8)*2, \
                &l_nvram_params, sizeof(oal_uint8)*2);

    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_CUSTOM, "hwifi_get_delt_ru_pow_val_5g::memcpy failed!");
        memset_s(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g, sizeof(oal_int8) * WLAN_BW_CAP_BUTT * WLAN_HE_RU_SIZE_BUTT, 0, \
    ...
    for (l_cfg_idx_one = 0; l_cfg_idx_one < WLAN_BW_CAP_BUTT; l_cfg_idx_one++) {
        for (l_cfg_idx_two = 0; l_cfg_idx_two < WLAN_HE_RU_SIZE_BUTT; l_cfg_idx_two++) {
            if (CUS_VAL_INVALID(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[l_cfg_idx_one][l_cfg_idx_two],
                ...
                OAM_ERROR_LOG4(0, OAM_SF_CUSTOM, "hwifi_get_delt_ru_pow_val_5g:cfg bw[%d]ru[%d] val[%d] invalid! max[%d]",
                               l_cfg_idx_one, l_cfg_idx_two, pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[l_cfg_idx_one][l_cfg_idx_two],
                               CUS_RU_DELT_POW_MAX);
                pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g[l_cfg_idx_one][l_cfg_idx_two] = 0;
            }
        }
    }
...
static oal_void hwifi_get_delt_ru_pow_val_2g(void)
{
    ...
    /* 20m 26~242tone ru_power */
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_20M_2G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_20M_2G];
    }
    l_ret = memcpy_s(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_2g[WLAN_BW_CAP_20M], sizeof(l_nvram_params), \
             &l_nvram_params, sizeof(l_nvram_params));

    /* 40m 26~242tone ru_power */
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_40M_0_2G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_40M_0_2G];
    }
    l_ret += memcpy_s(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_2g[WLAN_BW_CAP_40M], sizeof(l_nvram_params), \
             &l_nvram_params, sizeof(l_nvram_params));

    /* 40m 484tone ru_power */
    if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_40M_1_2G].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = nvram_init_params[NVRAM_CFG_TPC_RU_POWER_40M_1_2G];
    }
    pst_cust_nv_params->ac_fullbandwidth_to_ru_power_2g[WLAN_BW_CAP_40M][WLAN_HE_RU_SIZE_484] = CUS_GET_FIRST_BYTE(l_nvram_params);

    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_CUSTOM, "hwifi_get_delt_ru_pow_val_2g::memcpy failed!");
        memset_s(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_5g, sizeof(oal_int8) * WLAN_BW_CAP_80M * WLAN_HE_RU_SIZE_996, \
    ...
    for (l_cfg_idx_one = 0; l_cfg_idx_one < WLAN_BW_CAP_80M; l_cfg_idx_one++) {
        for (l_cfg_idx_two = 0; l_cfg_idx_two < WLAN_HE_RU_SIZE_996; l_cfg_idx_two++) {
            if (CUS_VAL_INVALID(pst_cust_nv_params->ac_fullbandwidth_to_ru_power_2g[l_cfg_idx_one][l_cfg_idx_two],
                                CUS_RU_DELT_POW_MAX, CUS_RU_DELT_POW_MIN)) {
                OAM_ERROR_LOG4(0, OAM_SF_CUSTOM, "hwifi_get_delt_ru_pow_val_2g:cfg bw[%d]ru[%d] val[%d] invalid! max[%d]",
                               l_cfg_idx_one, l_cfg_idx_two, pst_cust_nv_params->ac_fullbandwidth_to_ru_power_2g[l_cfg_idx_one][l_cfg_idx_two],
                               CUS_RU_DELT_POW_MAX);
                pst_cust_nv_params->ac_fullbandwidth_to_ru_power_2g[l_cfg_idx_one][l_cfg_idx_two] = 0;
            }
        }
    }
...
OAL_STATIC int32 hwifi_config_init_nvram(void)
{
    ...
    hwifi_get_max_txpwr_base_delt_val(NVRAM_PARAMS_INDEX_DELT_BASE_POWER_23,
                                      &pst_cust_nv_params->ac_delt_txpwr_base_params[0][0],
                                      OAL_SIZEOF(pst_cust_nv_params->ac_delt_txpwr_base_params));
