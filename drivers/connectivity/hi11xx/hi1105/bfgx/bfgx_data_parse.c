...
static void ps_decode_bt_packet(struct ps_core_s *ps_core_d, uint8 *ptr, struct pm_drv_data *pm_data)
{
    // check whether package type is right or not
    if ((*ptr < BT_PACKET_TYPE_MIN) || (*ptr > BT_PACKET_TYPE_MAX)) {
...
int32 ps_decode_packet_func_etc(struct ps_core_s *ps_core_d, uint8 *buf_ptr)
{
    ...
    switch (ps_core_d->rx_pkt_type) {
        ...
        case BT_MSG:
            ...
            ps_decode_bt_packet(ps_core_d, ptr, pm_data);
            break;
