...
oal_void mac_vap_init_mib_11ax(mac_vap_stru *pst_mac_vap)
{
    ...
    mac_mib_set_he_BQRSupport(pst_mac_vap, OAL_TRUE);
    ...
    mac_mib_set_he_TriggeredMUBeamformingPartialBWFeedback(pst_mac_vap_rom, OAL_FALSE);
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        ...
        mac_mib_set_he_BQRSupport(pst_mac_vap, OAL_FALSE);
        mac_mib_set_he_SRPBaseSR(pst_mac_vap, OAL_FALSE);
        mac_mib_set_he_SUBeamformer(pst_mac_vap, OAL_FALSE);
...
oal_void mac_vap_tx_data_set_user_htc_cap(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    ...
    if ((pst_mac_vap->bit_htc_include_custom_switch == OAL_FALSE) ||
        !IS_11AX_VAP(pst_mac_vap) || !MAC_USER_IS_HE_USER(pst_mac_user)) {
        return;
    }
...
OAL_STATIC oal_void mac_vap_11ax_cap_init(mac_vap_stru *pst_mac_vap)
{
    ...
    pst_mac_vap->bit_htc_include_custom_switch    = g_st_mac_11ax_custom_param.bit_htc_include;
    pst_mac_vap->bit_om_include_custom_switch     = g_st_mac_11ax_custom_param.bit_om_in_data;
    pst_mac_vap->bit_rom_custom_switch            = g_st_mac_11ax_custom_param.bit_rom_cap_switch;
}
#endif
                            ...
                            mac_cfg_add_vap_param_stru *pst_param)
{
    ...
    if ((pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) ||
        ...
        if (pst_vap->pst_mib_info == OAL_PTR_NULL) {
            OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY,
                "{mac_vap_init_etc::pst_mib_info alloc null, size[%d].}",
        ...
        if (pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
            ...
            switch (pst_mac_device->en_protocol_cap) {
                ...
                case WLAN_PROTOCOL_CAP_HE:
                    if (g_pst_mac_device_capability[0].en_11ax_switch == OAL_TRUE) {
                ...
                default:
                    OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG,
                        "{mac_vap_init_etc::en_protocol_cap[%d] is not supportted.}",
                        pst_mac_device->en_protocol_cap);
...
oal_uint8 mac_vap_get_ap_usr_opern_bandwidth(mac_vap_stru *pst_mac_sta, mac_user_stru *pst_mac_user)
{
    ...
    **********************************************************************/
    ...
    if (g_st_mac_regdomain_rom_cb.p_mac_regdomain_channel_is_support_bw(en_sta_new_bandwidth,
