...
oal_uint32 mac_pm_arbiter_alloc_id(mac_device_stru *pst_device,
                                   oal_uint8 *pst_name,
...
oal_uint32 mac_pm_arbiter_free_id(mac_device_stru *pst_device, oal_uint32 ul_arbiter_id)
{
    ...
    if (ul_arbiter_id >= MAC_PM_ARBITER_MAX_REQUESTORS) {
        OAM_ERROR_LOG2(pst_device->uc_cfg_vap_id, OAM_SF_PWR,
                       "hmac_pm_arbiter_free_id FAIL:invalid id %d,total %d",
                                 ...
                                 oal_uint8 uc_state_from, oal_uint8 uc_state_to)
{
    ...
    if (ul_arbiter_id >= MAC_PM_ARBITER_MAX_REQUESTORS) {
        OAM_ERROR_LOG2(pst_device->uc_cfg_vap_id, OAM_SF_PWR,
            "mac_pm_arbiter_to_state FAIL:invalid id %d,total %d",
            ul_arbiter_id, pst_pm_arbiter->uc_requestor_num);
        return;
    }

    if ((uc_state_from >= DEV_PWR_STATE_BUTT) ||
        (uc_state_to >= DEV_PWR_STATE_BUTT) ||
        (uc_state_from == uc_state_to)) {
        OAM_WARNING_LOG2(pst_device->uc_cfg_vap_id, OAM_SF_PWR,
...
oal_uint32 mac_pm_wow_prepare_probe_resp(dmac_vap_stru *pst_dmac_vap)
{
    ...
    if (pst_dmac_vap->pst_wow_probe_resp == OAL_PTR_NULL) {
        pst_dmac_vap->pst_wow_probe_resp = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF,
    ...
    /* 封装probe response帧 */
    pst_dmac_vap->us_wow_probe_resp_len =
        dmac_mgmt_encap_probe_response(pst_dmac_vap, pst_dmac_vap->pst_wow_probe_resp, &ast_dest_addr[0], OAL_FALSE);

    /* 增加1024 QAM IE */
#ifdef _PRE_WLAN_FEATURE_1024QAM
    mac_set_1024qam_vendor_ie(pst_dmac_vap,
                              mac_netbuf_get_payload(pst_dmac_vap->pst_wow_probe_resp) +
                              pst_dmac_vap->us_wow_probe_resp_len - MAC_80211_FRAME_LEN,
                              &uc_ie_len);
...
oal_uint32 mac_pm_wow_prepare_null_data(dmac_vap_stru *pst_dmac_vap)
{
    ...
    if (pst_dmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        pst_dmac_user = mac_res_get_dmac_user(pst_dmac_vap->st_vap_base_info.us_assoc_vap_id);
        if (pst_dmac_user == OAL_PTR_NULL) {
            return OAL_FAIL;
        }

        if (pst_dmac_vap->pst_wow_null_data == OAL_PTR_NULL) {
            /* 申请net_buff */
            pst_dmac_vap->pst_wow_null_data = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF,
        ...
        mac_hdr_set_frame_control(oal_netbuf_header(pst_dmac_vap->pst_wow_null_data),
            (oal_uint16)(WLAN_PROTOCOL_VERSION | WLAN_FC0_TYPE_DATA | WLAN_FC0_SUBTYPE_NODATA) | 0x0200);

        /* 设置ADDR1为目的地址 */
        oal_set_mac_addr((oal_netbuf_header(pst_dmac_vap->pst_wow_null_data) + 4),
                         pst_dmac_user->st_user_base_info.auc_user_mac_addr);

        /* 设置ADDR2为SA */
        oal_set_mac_addr((oal_netbuf_header(pst_dmac_vap->pst_wow_null_data) + 10),
                         mac_mib_get_StationID(&pst_dmac_vap->st_vap_base_info));

        /* 设置ADDR3为BSSID */
        oal_set_mac_addr((oal_netbuf_header(pst_dmac_vap->pst_wow_null_data) + 16),
...
oal_uint32 mac_pm_set_wow_para(dmac_vap_stru *pst_dmac_vap,
                               hal_wow_param_stru *pst_wow_para,
                               mac_cfg_lpm_wow_en_stru st_wow_en)
{
    ...
    if (pst_dmac_device == OAL_PTR_NULL) {
        return OAL_FAIL;
    }
    ...
    if (st_wow_en.uc_en) {
        ...
        for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
            ...
            if (pst_dmac_vap == OAL_PTR_NULL) {
                OAM_WARNING_LOG0(pst_mac_device->auc_vap_id[uc_vap_idx], OAM_SF_CFG,
                                 "{mac_pm_set_wow_para::pst_dmac_vap null.");
                return OAL_ERR_CODE_PTR_NULL;
            }
            mac_pm_wow_prepare_probe_resp(pst_dmac_vap);
            mac_pm_wow_prepare_null_data(pst_dmac_vap);

            if (pst_dmac_vap->pst_hal_vap->uc_vap_id == 0) {
                if (pst_dmac_vap->pst_wow_probe_resp == OAL_PTR_NULL) {
                    continue;
                }
                pst_wow_para->ul_ap0_probe_resp_address =
                    OAL_VIRT_TO_PHY_ADDR((oal_uint32 *)oal_netbuf_data(pst_dmac_vap->pst_wow_probe_resp));
                pst_wow_para->ul_ap0_probe_resp_len = pst_dmac_vap->us_wow_probe_resp_len;
                pst_wow_para->ul_ap0_probe_resp_phy = 0;
                /* 110x废弃 */
                // mac_fcs_get_prot_datarate(&pst_dmac_vap->st_vap_base_info);
                pst_wow_para->ul_ap0_probe_resp_rate = 0x490010 ul_wow_set_bitmap |= HAL_WOW_PARA_AP0_PROBE_RESP;
            ...
            } else if (pst_dmac_vap->pst_hal_vap->uc_vap_id == 1) {
                if (pst_dmac_vap->pst_wow_probe_resp == OAL_PTR_NULL) {
                    continue;
                }
                pst_wow_para->ul_ap1_probe_resp_address =
                    OAL_VIRT_TO_PHY_ADDR((oal_uint32 *)oal_netbuf_data(pst_dmac_vap->pst_wow_probe_resp));
                pst_wow_para->ul_ap1_probe_resp_len = pst_dmac_vap->us_wow_probe_resp_len;
                pst_wow_para->ul_ap1_probe_resp_phy = 0;
                /* 110x废弃 */
                // mac_fcs_get_prot_datarate(&pst_dmac_vap->st_vap_base_info);
                pst_wow_para->ul_ap1_probe_resp_rate = 0x490010 ul_wow_set_bitmap |= HAL_WOW_PARA_AP1_PROBE_RESP;
                pst_wow_para->ul_nulldata_interval = 0;
                ul_wow_set_bitmap |= HAL_WOW_PARA_NULLDATA_INTERVAL;
            } else if (pst_dmac_vap->pst_hal_vap->uc_vap_id == 4) {
                /* 110x废弃 */
                // mac_fcs_get_prot_datarate(&pst_dmac_vap->st_vap_base_info);
                pst_wow_para->ul_nulldata_rate = 0x490010 pst_wow_para->ul_nulldata_phy_mode =
                                                     mac_fcs_get_prot_mode(&pst_dmac_vap->st_vap_base_info);
                ...
                pst_wow_para->ul_nulldata_address =
                    OAL_VIRT_TO_PHY_ADDR((oal_uint32 *)oal_netbuf_data(pst_dmac_vap->pst_wow_null_data));
...
oal_uint32 mac_pm_set_hal_state(mac_device_stru *pst_device, mac_vap_stru *pst_mac_vap, oal_uint8 uc_state_to)
{
    ...
    switch (uc_state_to) {
        case DEV_PWR_STATE_WORK:
            ...
            if (pst_ap_pm_handler->st_oal_fsm.uc_prev_state == DEV_PWR_STATE_WOW) {
                st_wow_en.uc_en = 0;
            ...
            /* 从深睡唤醒，重新启动定时器 */
            else if (pst_ap_pm_handler->st_oal_fsm.uc_prev_state == DEV_PWR_STATE_DEEP_SLEEP) {
                frw_timer_restart_etc();
            }
            break;
        ...
        case DEV_PWR_STATE_WOW:
            ...
            if ((pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
                (pst_mac_vap->st_protection.bit_obss_non_ht_present == OAL_TRUE)) {
                ...
                dmac_encap_beacon(pst_dmac_vap, pst_dmac_vap->pauc_beacon_buffer[pst_dmac_vap->uc_beacon_idx],
                                  &(pst_dmac_vap->us_beacon_len));
                mac_pm_wow_prepare_probe_resp(pst_dmac_vap);

                OAM_WARNING_LOG0(pst_device->uc_cfg_vap_id, OAM_SF_PWR,
                                 "{mac_pm_set_hal_state: clear non ht obss setting}");
