    ...
    if (uc_channel_number <= 14) {
        ...
        if (uc_channel_number <= 64) {
            ...
            OAM_WARNING_LOG1(0, OAM_SF_ANY,
                             "{mac_regdomain_get_channel_to_bw_mode_idx::unknow channel=%d, force uc_idx = chan 36",
                             uc_channel_number);
            uc_idx = 0;
        }
    }
...
wlan_channel_bandwidth_enum_uint8 mac_regdomain_get_support_bw_mode(
    wlan_channel_bandwidth_enum_uint8 en_cfg_bw, oal_uint8 uc_channel)
{
    oal_uint8 uc_idx;
    oal_uint8 i;
    ...
    if (uc_channel <= 14) {
        if (en_cfg_bw == WLAN_BAND_WIDTH_40M) {
            // 未配置带宽扩展方向时，默认用第一种带宽扩展方式
            en_bw_mode = g_bw_mode_table_2g[uc_idx].aen_supp_bw[1];
        } else if ((en_cfg_bw == WLAN_BAND_WIDTH_40PLUS) || (en_cfg_bw == WLAN_BAND_WIDTH_40MINUS)) {
            ...
            OAM_ERROR_LOG1(0, OAM_SF_ANY,
                           "{mac_regdomain_get_support_bw_mode::2G not support bw_mode=%d,force to 20M", en_cfg_bw);
        }
    } else {
        if (en_cfg_bw == WLAN_BAND_WIDTH_40M) {
            // 未配置带宽扩展方向时，使用该信道支持的40M带宽扩展模式
            en_bw_mode = g_bw_mode_table_5g[uc_idx].aen_supp_bw[1];
        } else if (en_cfg_bw == WLAN_BAND_WIDTH_80M) {
            ...
            if (i == g_bw_mode_table_5g[uc_idx].uc_cnt) {
                // 该信道不支持设置的带宽模式,提示用户带宽扩展方向被驱动自适应调整了
                if ((en_cfg_bw == WLAN_BAND_WIDTH_40PLUS) || (en_cfg_bw == WLAN_BAND_WIDTH_40MINUS)) {
                    en_bw_mode = g_bw_mode_table_5g[uc_idx].aen_supp_bw[1];
                } else {
                    en_bw_mode = g_bw_mode_table_5g[uc_idx].aen_supp_bw[2];
                }
...
oal_void mac_get_ext_chan_info(oal_uint8 uc_pri20_channel_idx,
...
oal_bool_enum_uint8 mac_is_cover_dfs_channel(oal_uint8 uc_band,
                                             wlan_channel_bandwidth_enum_uint8 en_bandwidth,
