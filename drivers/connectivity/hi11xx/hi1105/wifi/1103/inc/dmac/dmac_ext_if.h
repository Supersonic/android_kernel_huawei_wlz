...
typedef enum
{
    ...
    CUSTOM_CFGID_PRIV_INI_HAL_PS_RSSI,
    CUSTOM_CFGID_PRIV_INI_HAL_PS_PPS,
...
#ifdef _PRE_WLAN_FEATURE_11AX
typedef enum {
    DMAC_TX_NOT_TB_FLAG = 0,   /* 最后一次上报不是以he_tb的格式发送的 */
    DMAC_TX_TB_ALL_FLAG,       /* 最后一次上报是以he_tb的格式发送的，且全部聚合包都已经发送 */
    DMAC_TX_TB_PART_FLAG,      /* 最后一次上报是以he_tb的格式发送的，只有部分聚合包发送 */
} dmac_tx_desc_last_tb_flag_enum;
typedef oal_uint8 dmac_tx_desc_last_tb_flag_enum_uint8;
#endif
...
struct  mac_tx_ctl
{
    ...
    /* byte13-16 */
    oal_uint16                              us_timestamp_ms;                         /* 维测使用入TID队列的时间戳, 单位1ms精度 */
    oal_uint8                               bit_is_qosnull              :1;          /* 用于标记qos null帧 */
    oal_uint8                               uc_reserved1                :7;
    oal_uint8                               uc_reserved2;
    /* byte17-18 */
...
#ifdef _PRE_WLAN_FEATURE_AGGR_OPTIMIZE_BYTES
    oal_uint32                      ul_aggr_bytes;           /* 算法调度出的聚合字节数，单位：字节 */
#endif
...
#define DMAC_TIME_USEC_INT64(_pst_time) \
    ((((oal_uint32)((_pst_time)->i_sec)) << 20) + (oal_uint32)((_pst_time)->i_usec))
/* 1ms精度 */
#define DMAC_TIME_USEC_INT32(_pst_time) \
    ((((oal_uint32)((_pst_time)->i_sec)) << 10) + (((oal_uint32)((_pst_time)->i_usec)) >> 10))
/* us->ms精度转换 */
#define MAC_TIME_TRANSFORM_MS(_str)    ((oal_uint16)((_str) >> 10))

/* 不区分offload架构的CB字段 */
#define MAC_GET_CB_IS_4ADDRESS(_pst_tx_ctrl)             ((_pst_tx_ctrl)->en_use_4_addr)
#define MAC_GET_CB_IS_AMSDU(_pst_tx_ctrl)                ((_pst_tx_ctrl)->bit_is_amsdu)           /* 标记是否是小包AMSDU帧 */
#define MAC_GET_CB_AMSDU_LEVEL(_pst_tx_ctrl)             ((_pst_tx_ctrl)->bit_large_amsdu_level)  /* 自适应算法决定的聚合度 */
#define MAC_GET_CB_ETHER_HEAD_PADDING(_pst_tx_ctrl)      ((_pst_tx_ctrl)->bit_align_padding)      /* MSDU帧头部对齐字节 */
#define MAC_GET_CB_HAS_MSDU_NUMBER(_pst_tx_ctrl)         ((_pst_tx_ctrl)->bit_msdu_num_in_amsdu)  /* 实际组成的大包AMSDU帧包含的子帧数 */
#define MAC_GET_CB_IS_LARGE_SKB_AMSDU(_pst_tx_ctrl)      ((_pst_tx_ctrl)->bit_is_large_skb_amsdu) /* 标记是否组成了大包AMSDU帧 */
