...
extern oal_void hmac_tx_ba_setup_etc(hmac_vap_stru *pst_vap,
                                     hmac_user_stru *pst_user,
                                     oal_uint8 uc_tidno);

extern oal_void hmac_tx_ba_cnt_vary_etc(hmac_vap_stru *pst_hmac_vap,
                                        hmac_user_stru *pst_hmac_user,
                                        oal_uint8 uc_tidno,
                                        oal_netbuf_stru *pst_buf);

#if defined(_PRE_PRODUCT_ID_HI110X_HOST)
extern oal_uint8 hmac_tx_wmm_acm_etc(oal_bool_enum_uint8 en_wmm,
                                     hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_tid);
                                                     ...
                                                     mac_ieee80211_qos_htc_frame_addr4_stru *pst_hdr_addr4)
{
    ...
    /* 若himit使能，HTC字段信息填写HIMIT信息 目前芯片调试不会打开himit开关，所以不影响原有代码逻辑
     * TODO：由于HE模式时与其他特性填写冲突，关于HTC字段后面会讨论后再适配
     */
 #ifdef _PRE_WLAN_FEATURE_HIMIT
    if (pst_mac_vap_rom->uc_himit_enable == OAL_TRUE) {
        if (pst_hmac_user->st_user_base_info.en_cur_protocol_mode == WLAN_HE_MODE) {
            ul_htc_val = HTC_HIMIT_HE_VALUE;
        } else if (pst_hmac_user->st_user_base_info.en_cur_protocol_mode == WLAN_VHT_MODE) {
            ul_htc_val = HTC_HIMIT_VHT_VALUE;
        } else {
            ul_htc_val = HTC_HIMIT_HT_OR_LEGACY_VALUE;
        }
...
OAL_STATIC OAL_INLINE oal_void hmac_tx_ba_del(hmac_vap_stru *pst_hmac_vap,
                                              hmac_user_stru *pst_hmac_user,
                                              oal_uint8 uc_tidno)
{
                                                                   ...
                                                                   oal_netbuf_stru *pst_buf)
{
    ...
    if ((WLAN_WME_AC_VO == WLAN_WME_TID_TO_AC(uc_tidno)) &&
