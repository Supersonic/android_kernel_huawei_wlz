...
oal_void hmac_report_ext_auth_worker_etc(oal_work_stru *pst_sae_report_ext_auth_worker);
#endif
/* 3 函数实现 */

extern oal_uint8 wlan_ps_mode;
oal_uint32 hmac_vap_init_etc(hmac_vap_stru *pst_hmac_vap,
                             ...
                             mac_cfg_add_vap_param_stru *pst_param)
{
    ...
    switch (pst_param->en_vap_mode) {
        case WLAN_VAP_MODE_BSS_AP:
            ...
            FRW_TIMER_CREATE_TIMER(&(pst_hmac_vap->st_edca_opt_timer), hmac_edca_opt_timeout_fn_etc,
...
#endif
#if (_PRE_WLAN_FEATURE_BLACKLIST_LEVEL != _PRE_WLAN_FEATURE_BLACKLIST_NONE)
            mac_blacklist_get_pointer(pst_param->en_vap_mode, uc_chip_id,
                                      uc_device_id, uc_vap_id, &(pst_hmac_vap->pst_blacklist_info));
            if (pst_hmac_vap->pst_blacklist_info != OAL_PTR_NULL) {
                memset_s(pst_hmac_vap->pst_blacklist_info->ast_black_list,
                         sizeof(mac_blacklist_stru) * WLAN_BLACKLIST_MAX, 0,
                         sizeof(mac_blacklist_stru) * WLAN_BLACKLIST_MAX);
        ...
        default:
            OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                             "{hmac_vap_init_etc::unsupported mod=%d.}", pst_param->en_vap_mode);
...
#endif
#ifdef _PRE_WLAN_FEATURE_SAE
    OAL_INIT_WORK(&(pst_hmac_vap->st_sae_report_ext_auth_worker), hmac_report_ext_auth_worker_etc);
#endif
#ifdef _PRE_WLAN_TCP_OPT
...
#endif
    {
        ...
        pst_hmac_vap->bit_11r_over_ds = g_st_mac_voe_custom_param.en_11r_ds;
        pst_hmac_vap->bit_adaptive11r = g_st_mac_voe_custom_param.en_adaptive11r;
        pst_hmac_vap->bit_nb_rpt_11k = g_st_mac_voe_custom_param.en_nb_rpt_11k;
    }
#endif
...
oal_uint32 hmac_vap_get_priv_cfg_etc(mac_vap_stru *pst_mac_vap, hmac_vap_cfg_priv_stru **ppst_cfg_priv)
{
    ...
    if (OAL_UNLIKELY(pst_hmac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY,
                       "{hmac_vap_get_priv_cfg_etc::mac_res_get_hmac_vap fail.vap_id = %u}",
...
oal_uint32 hmac_vap_creat_netdev_etc(hmac_vap_stru *pst_hmac_vap, oal_int8 *puc_netdev_name,
                                     oal_uint8 uc_netdev_name_len, oal_int8 *puc_mac_addr)
{
    ...
    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR2(pst_hmac_vap, puc_netdev_name))) {
        OAM_ERROR_LOG2(0, OAM_SF_ASSOC,
                       "{hmac_vap_creat_netdev_etc::param null %d %d.}",
    ...
    if (OAL_UNLIKELY(ul_return != OAL_SUCC)) {
        ...
        OAM_WARNING_LOG0(pst_vap->uc_vap_id, OAM_SF_ANY,
                         "{hmac_vap_creat_netdev_etc::oal_net_register_netdev failed.}");
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_vap_creat_netdev_etc::memcpy fail!");
        return OAL_FAIL;
    }

#ifdef _PRE_WLAN_REPORT_PRODUCT_LOG
    // 保存vap id 对应的 chip id
    vapid_to_chipid[pst_vap->uc_vap_id] = pst_vap->uc_chip_id;
#endif
...
oal_uint32 hmac_vap_destroy_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                         "{hmac_vap_destroy_etc::hmac_config_down_vap_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                         "{hmac_vap_destroy_etc::hmac_config_del_vap_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
    return OAL_SUCC;
}
...
oal_uint16 hmac_vap_check_ht_capabilities_ap_etc(hmac_vap_stru *pst_hmac_vap,
...
oal_uint32 hmac_search_ht_cap_ie_ap_etc(hmac_vap_stru *pst_hmac_vap,
                                        hmac_user_stru *pst_hmac_user_sta,
                                        oal_uint8 *puc_payload,
                                        ...
                                        oal_bool_enum en_prev_asoc_ht)
{
    ...
    ***************************************************************************/
    ...
    pst_ht_hdl->bit_supported_channel_width = mac_ie_proc_ht_supported_channel_width_etc(pst_mac_user, pst_mac_vap,
    ...
    pst_ht_hdl->bit_ht_green_field = mac_ie_proc_ht_green_field_etc(pst_mac_user, pst_mac_vap,
    ...
    pst_ht_hdl->bit_lsig_txop_protection = mac_ie_proc_lsig_txop_protection_support_etc(pst_mac_user, pst_mac_vap,
    ...
    ***************************************************************************/
    ...
    OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                     "{hmac_search_ht_cap_ie_ap_etc::en_40M_intol_user[%d].}",
...
oal_void hmac_set_ampdu_hw_worker(oal_delayed_work *pst_work)
{
    ...
    /***************************************************************************
            抛事件到DMAC层, 同步DMAC数据
     ***************************************************************************/
    ul_ret = hmac_config_send_event_etc(&pst_hmac_vap->st_vap_base_info, WLAN_CFGID_AMPDU_TX_ON,
                                        OAL_SIZEOF(mac_cfg_ampdu_tx_on_param_stru), (oal_uint8 *)&st_ampdu_tx_on);
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                         "hmac_set_ampdu_hw_worker::hmac_config_send_event_etc fail(%u)", ul_ret);
    }
            ...
            for (uc_vap_index = 0; uc_vap_index < pst_dev->uc_vap_num; uc_vap_index++) {
                ...
                if (pst_vap == OAL_PTR_NULL) {
                    OAM_INFO_LOG1(0, OAM_SF_BA,
                        "{hmac_flowctl_check_device_is_sta_mode_etc::mac_res_get_mac_vap fail.vap_index = %u}",
                        uc_vap_index);
                    continue;
                }
...
oal_void hmac_del_virtual_inf_worker_etc(oal_work_stru *pst_del_virtual_inf_work)
{
    ...
    if (pst_net_dev == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_P2P,
                       "{hmac_del_virtual_inf_worker_etc:: net_dev is null}");
        return;
    }
...
oal_void hmac_report_ext_auth_worker_etc(oal_work_stru *pst_sae_report_ext_auth_worker)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{hmac_report_ext_auth_worker_etc:: hmac_vap is null}");
        return;
    }

    OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                     "{hmac_report_ext_auth_worker_etc:: hmac_report_external_auth_req_etc auth start}");
...
oal_uint32 hmac_tx_get_mac_vap_etc(oal_uint8 uc_vap_id, mac_vap_stru **pst_vap_stru)
{
    ...
    if (pst_vap->en_vap_mode != WLAN_VAP_MODE_BSS_AP) {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_TX,
                         "hmac_tx_get_mac_vap_etc::vap_mode error[%d]",
                         pst_vap->en_vap_mode);
    ...
    if (pst_vap->en_vap_state != MAC_VAP_STATE_UP && pst_vap->en_vap_state != MAC_VAP_STATE_PAUSE) {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_TX,
                         "hmac_tx_get_mac_vap_etc::vap_state[%d] error",
...
oal_uint32 hmac_restart_all_work_vap(oal_uint8 uc_chip_id)
{
    ...
    for (uc_dev_idx = 0; uc_dev_idx < pst_mac_chip->uc_device_nums; uc_dev_idx++) {
        ...
        for (uc_vap_idx = 0; uc_vap_idx < pst_mac_dev->uc_vap_num; uc_vap_idx++) {
            ...
            if (ul_ret != OAL_SUCC) {
                OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                                 "{hmac_restart_all_work_vap::hmac_config_down_vap_etc failed[%d].}", ul_ret);
                return ul_ret;
            }
            /* down netdev */
            ...
            if (ul_ret != OAL_SUCC) {
                OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                                 "{hmac_config_wifi_enable::hmac_config_start_vap_etc failed[%d].}", ul_ret);
                return ul_ret;
            }
            /* up netdev */
...
oal_void hmac_vap_state_restore_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                         "{hmac_vap_state_restore_etc::pst_hmac_device[%d] null.}",
                         pst_hmac_vap->st_vap_base_info.uc_device_id);
        return ;
    }
