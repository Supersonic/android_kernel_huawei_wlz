...
oal_uint32 hmac_config_wapi_add_key_etc(mac_vap_stru *pst_mac_vap,
    ...
    if (en_pairwise) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                "{hmac_config_wapi_add_key_etc::mac_vap_find_user_by_macaddr_etc failed. %u}", ul_ret);
...
oal_uint32 hmac_config_wapi_add_key_and_sync_etc(mac_vap_stru *pst_mac_vap,
    mac_addkey_param_stru *pst_payload_addkey_params)
{
    hmac_vap_stru *pst_hmac_vap = OAL_PTR_NULL;
    oal_uint32 ul_ret;

    OAM_WARNING_LOG2(0, OAM_SF_WPA, "{hmac_config_wapi_add_key_and_sync_etc:: key idx==%u, pairwise==%u}",
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_add_key_etc::hmac_config_wapi_add_key_etc fail[%d].}", ul_ret);
        return ul_ret;
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_add_key_etc::WLAN_CFGID_ADD_WAPI_KEY send fail[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32 hmac_config_11i_add_key_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (uc_key_index >= WLAN_NUM_TK + WLAN_NUM_IGTK) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_add_key_etc::invalid uc_key_index[%d].}", uc_key_index);
    ...
    OAM_INFO_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
        "{hmac_config_11i_add_key_etc::mac addr=%02X:XX:XX:%02X:%02X:%02X}",
    ...
    if (en_pairwise == OAL_TRUE) {
        ...
        if (ul_ret != OAL_SUCC) {
            ...
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                "{hmac_config_11i_add_key_etc::us_user_idx[%d] find_user_by_macaddr fail[%d].}",
                us_user_idx, ul_ret);
            return ul_ret;
        }
    } else {
    ...
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_add_key_etc::get_mac_user null.idx:%u}", us_user_idx);
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_add_key_etc::mac_11i_add_key fail[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32 hmac_config_11i_get_key_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_ANY_NULL_PTR2(pst_mac_vap, puc_param)) {
        OAM_ERROR_LOG2(0, OAM_SF_WPA,
            "{hmac_config_11i_get_key_etc::param null, pst_mac_vap=%x, puc_param=%x.}",
            (uintptr_t)pst_mac_vap, (uintptr_t)puc_param);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (uc_key_index >= WLAN_NUM_TK + WLAN_NUM_IGTK) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_get_key_etc::uc_key_index invalid[%d].}", uc_key_index);
        return OAL_ERR_CODE_SECURITY_KEY_ID;
    }
    ...
    if (pst_priv_key == OAL_PTR_NULL) {
        OAM_ERROR_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_get_key_etc::key is null.pairwise[%d], key_idx[%d]}",
    ...
    if (pst_priv_key->ul_key_len == 0) {
        OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_get_key_etc::key len = 0.pairwise[%d], key_idx[%d]}",
            en_pairwise, uc_key_index);
...
oal_uint32 hmac_config_11i_remove_key_etc(mac_vap_stru *pst_mac_vap,
    oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 1.1 入参检查 */
    if (OAL_ANY_NULL_PTR2(pst_mac_vap, puc_param)) {
        OAM_ERROR_LOG2(0, OAM_SF_WPA, "{hmac_config_11i_remove_key_etc::param null,pst_mac_vap=%x, puc_param=%x.}",
            (uintptr_t)pst_mac_vap, (uintptr_t)puc_param);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
        "{hmac_config_11i_remove_key_etc::uc_key_index=%d, en_pairwise=%d.}",
        uc_key_index, en_pairwise);
    ...
    if (uc_key_index >= WLAN_NUM_TK + WLAN_NUM_IGTK) {
        ...
        OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_remove_key_etc::invalid uc_key_index[%d].}", uc_key_index);
        return OAL_SUCC;
    }
    ...
    if (pst_priv_key == OAL_PTR_NULL) {
        ...
        if (us_user_idx == MAC_INVALID_USER_ID) {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                "{hmac_config_11i_remove_key_etc::user already del.}");
            return OAL_SUCC;
        } else {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                "{hmac_config_11i_remove_key_etc::pst_priv_key null.}");
            return OAL_ERR_CODE_SECURITY_USER_INVAILD;
        }
    }
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_ERROR_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_remove_key_etc::hmac_config_send_event_etc failed[%d], en_cfgid=%d .}",
            ul_ret, en_cfgid);
        return ul_ret;
    }
...
oal_uint32 hmac_config_11i_set_default_key_etc(mac_vap_stru *pst_mac_vap,
    oal_uint16 us_len, oal_uint8 *puc_param)
{
    oal_uint32 ul_ret = OAL_SUCC;
    oal_uint8 uc_key_index;
    ...
    if (uc_key_index >= (WLAN_NUM_TK + WLAN_NUM_IGTK)) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_set_default_key_etc::invalid uc_key_index[%d].}",
            uc_key_index);
        return OAL_ERR_CODE_SECURITY_KEY_ID;
    }
    ...
    if ((en_multicast == OAL_FALSE) && (en_unicast == OAL_FALSE)) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_set_default_key_etc::not ptk or gtk,invalid mode.}");
    ...
    ***************************************************************************/
    ...
    OAM_INFO_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
        "{hmac_config_11i_set_default_key_etc::key_id[%d] un[%d] mu[%d] OK}",
...
oal_uint32 hmac_config_11i_add_wep_entry_etc(mac_vap_stru *pst_mac_vap,
    oal_uint16 us_len, oal_uint8 *puc_param)
{
    mac_user_stru *pst_mac_user = OAL_PTR_NULL;
    oal_uint32 ul_ret;
                                                        ...
                                                        mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    if (OAL_ANY_NULL_PTR2(pst_mac_vap, pst_bss_dscr)) {
        OAM_WARNING_LOG2(0, OAM_SF_WPA, "{hmac_check_capability_mac_phy_supplicant_etc::input null %x %x.}",
...
oal_uint32 hmac_en_mic_etc(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user,
    oal_netbuf_stru *pst_netbuf, oal_uint8 *puc_iv_len)
{
    ...
    if (OAL_ANY_NULL_PTR4(pst_hmac_vap, pst_hmac_user, pst_netbuf, puc_iv_len)) {
        OAM_ERROR_LOG4(0, OAM_SF_WPA, "{hmac_en_mic_etc::hmac_vap=%d hmac_user=%d netbuf=%d iv_len=%d.}",
    ...
    *puc_iv_len = 0;
    ...
    pst_key = mac_user_get_key_etc(&pst_hmac_user->st_user_base_info,
...
oal_uint32 hmac_rx_tkip_mic_failure_process_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (pst_hmac_event_mem == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_event_hdr->uc_vap_id, OAM_SF_WPA,
            "{hmac_rx_tkip_mic_failure_process_etc::pst_hmac_event_mem null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 去关联的STA mac地址 */
    if (EOK != memcpy_s((oal_uint8 *)frw_get_event_payload(pst_hmac_event_mem), sizeof(dmac_to_hmac_mic_event_stru),
                        (oal_uint8 *)pst_mic_event, sizeof(dmac_to_hmac_mic_event_stru))) {
        OAM_ERROR_LOG0(0, OAM_SF_WPA, "hmac_rx_tkip_mic_failure_process_etc::memcpy fail!");
...
oal_uint32 hmac_11i_ether_type_filter_etc(hmac_vap_stru *pst_vap,
    hmac_user_stru *pst_hmac_user, oal_uint16 us_ether_type)
{
...
oal_module_symbol(hmac_config_11i_add_wep_entry_etc); /*lint +e578*/ /*lint +e19*/
