...
oal_uint32 hmac_res_free_mac_dev_etc(oal_uint32 ul_dev_idx)
{
    ...
    if (g_st_hmac_res_etc.st_hmac_dev_res.auc_user_cnt[ul_dev_idx] != 0) {
        return OAL_SUCC;
    }

    /* 入队索引值需要加1操作 */
    oal_queue_enqueue (&(g_st_hmac_res_etc.st_hmac_dev_res.st_queue),
                       (oal_void *)(uintptr_t)((oal_uint) ul_dev_idx + 1));
...
oal_uint32 hmac_res_init_etc(oal_void)
{
    ...
    ***************************************************************************/
    ...
    for (ul_loop = 0; ul_loop < MAC_RES_MAX_DEV_NUM; ul_loop++) {
        /* 初始值保存的是对应数组下标值加1 */
        oal_queue_enqueue(&(g_st_hmac_res_etc.st_hmac_dev_res.st_queue),
                          (oal_void *)(uintptr_t)((oal_uint)ul_loop + 1));
