...
OAL_STATIC oal_void hmac_amsdu_prepare_to_send(hmac_vap_stru *pst_vap,
                                               hmac_user_stru *pst_user,
...
OAL_STATIC oal_uint32 hmac_amsdu_send(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, hmac_amsdu_stru *pst_amsdu)
{
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        ...
        OAM_ERROR_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMPDU,
                       "{hmac_amsdu_send::hmac_tx_encap_etc failed[%d]}", ul_ret);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        ...
        OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMPDU,
                         "hmac_amsdu_send::frw_event_dispatch_event_etc fail[%d]", ul_ret);
    }
...
OAL_STATIC OAL_INLINE oal_uint32 hmac_amsdu_tx_encap_mpdu(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user,
                                                          hmac_amsdu_stru *pst_amsdu, oal_netbuf_stru *pst_buf)
{
    ...
    pst_ether_head->us_ether_type =
        oal_byteorder_host_to_net_uint16((oal_uint16)(ul_frame_len - ETHER_HDR_LEN + SNAP_LLC_FRAME_LEN));
    ...
    if (pst_amsdu->uc_msdu_num == 0) {
        ...
        if (OAL_SUCC != hmac_amsdu_alloc_netbuf(pst_amsdu, pst_buf)) {
            OAM_WARNING_LOG0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
                             "{hmac_amsdu_tx_process_etc::failed to alloc netbuf.}");
            return HMAC_TX_PASS;
        }
                                       ...
                                       oal_netbuf_stru *pst_buf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    if (g_st_tx_large_amsdu.en_tx_amsdu_level == WLAN_TX_AMSDU_NONE) {
        return;
    }
    ...
    pst_ether_hdr_temp->us_ether_type =
        oal_byteorder_host_to_net_uint16((oal_uint16)(us_mpdu_len - ETHER_HDR_LEN + SNAP_LLC_FRAME_LEN));
}
#endif
...
oal_uint32 hmac_amsdu_notify_etc(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
{
    ...
    if ((g_en_2g_tx_amsdu == OAL_FALSE) &&
        (pst_vap->st_vap_base_info.st_channel.en_band == WLAN_BAND_2G)) {
        OAM_INFO_LOG0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
                      "{hmac_amsdu_notify_etc::2g tx amsdu forbidden due to 5G traffic}");
        return HMAC_TX_PASS;
    }
#endif
    ...
    if ((OAL_TRUE != mac_mib_get_CfgAmsduTxAtive(&pst_vap->st_vap_base_info)) ||
        ...
        OAM_INFO_LOG0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
                      "{hmac_amsdu_notify_etc::amsdu is unenable in amsdu notify}");
        return HMAC_TX_PASS;
    }
    ...
    if (OAL_FALSE == HMAC_USER_IS_AMSDU_SUPPORT(pst_user, uc_tid_no)) {
        OAM_INFO_LOG2(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
                      "{hmac_amsdu_notify_etc::AMPDU NOT SUPPORT AMSDU uc_tid_no=%d uc_amsdu_supported=%d}",
    ...
    if (OAL_FALSE == hmac_user_xht_support(pst_user)) {
        OAM_INFO_LOG0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
                      "{hmac_amsdu_notify_etc::user is not qos in amsdu notify}");
        return HMAC_TX_PASS;
    }

    if (OAL_UNLIKELY(uc_tid_no >= WLAN_TID_MAX_NUM)) {
        OAM_ERROR_LOG0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
                       "{hmac_amsdu_notify_etc::invalid tid number obtained from the cb in asmdu notify function}");
        return HMAC_TX_PASS;
    }

    if (uc_tid_no == WLAN_TIDNO_VOICE) {
        OAM_INFO_LOG2(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
                      "{hmac_amsdu_notify_etc::VO TID NOT SUPPORT AMSDU uc_tid_no=%d uc_amsdu_supported=%d",
                      uc_tid_no, pst_user->uc_amsdu_supported);
        return HMAC_TX_PASS;
    }
...
OAL_STATIC oal_uint32 hmac_amsdu_tx_timeout_process(oal_void *p_arg)
{
    ...
    if (OAL_UNLIKELY(pst_user == OAL_PTR_NULL)) {
        ...
        OAM_ERROR_LOG1(0, OAM_SF_AMPDU,
                       "{hmac_amsdu_tx_timeout_process::pst_user[%d] null}",
