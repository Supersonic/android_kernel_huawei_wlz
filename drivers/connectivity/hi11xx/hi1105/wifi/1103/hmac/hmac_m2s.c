...
oal_void hmac_m2s_vap_arp_probe_process(oal_void *p_arg, oal_bool_enum_uint8 en_arp_detect_on)
{
    ...
    OAM_WARNING_LOG1(0, OAM_SF_M2S, "{hmac_m2s_vap_arp_probe_process: en_arp_probe_on[%d].}",
                     pst_hmac_vap->st_hmac_vap_m2s.en_arp_probe_on);
}
...
OAL_STATIC oal_uint32 hmac_m2s_arp_probe_timeout(oal_void *p_arg)
{
    ...
    if (ui_val == 0) {
        ...
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2S,
                         "{hmac_m2s_arp_probe_timeout::rx_arp_pkt fail cnt[%d]!}",
                         pst_hmac_vap_m2s->uc_rx_no_pkt_count);
        ...
        if (pst_hmac_vap_m2s->uc_rx_no_pkt_count > M2S_ARP_FAIL_REASSOC_NUM) {
            ...
            if (pst_hmac_user == OAL_PTR_NULL) {
                OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2S,
                               "hmac_m2s_arp_probe_timeout: pst_hmac_user is null ptr. user id:%d",
            ...
            OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2S,
                             "{hmac_m2s_arp_probe_timeout::user rssi[%d] threhold[%d] need to reassoc to resume.}",
        ...
        OAM_WARNING_LOG1(0, OAM_SF_M2S, "{hmac_m2s_arp_probe_timeout: detect succ, en_arp_probe_on[%d].}",
                         pst_hmac_vap_m2s->en_arp_probe_on);
    }
...
oal_void hmac_m2s_arp_fail_process(oal_netbuf_stru *pst_netbuf, oal_void *p_arg)
{
    ...
    /* 只要统计功能打开，就需要做一次探测 */
    if (pst_hmac_vap_m2s->en_arp_probe_on == OAL_TRUE) {
        ...
        /* 发送方向创建定时器，多次创建定时器 */
        if ((uc_data_type == MAC_DATA_ARP_REQ) && (pst_hmac_vap_m2s->st_arp_probe_timer.en_is_registerd == OAL_FALSE)) {
            /* 每次重启定时器之前清零,保证统计的时间 */
            oal_atomic_set(&(pst_hmac_vap_m2s->ul_rx_unicast_pkt_to_lan), 0);
