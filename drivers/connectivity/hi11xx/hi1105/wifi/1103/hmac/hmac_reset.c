...
extern oal_uint32 hmac_config_send_event_etc(mac_vap_stru *pst_mac_vap,
    ...
    if (pst_query_station_reponse_event->query_event == OAL_QUERY_STATION_INFO_EVENT) {
        ...
        */
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 0))
        {
            uc_flag |= ((pst_query_station_reponse_event->st_txrate.flags & MAC_RATE_INFO_FLAGS_MCS) ?
                RATE_INFO_FLAGS_MCS : 0);
            uc_flag |= ((pst_query_station_reponse_event->st_txrate.flags & MAC_RATE_INFO_FLAGS_40_MHZ_WIDTH) ?
                RATE_INFO_FLAGS_40_MHZ_WIDTH : 0);
            uc_flag |= ((pst_query_station_reponse_event->st_txrate.flags & MAC_RATE_INFO_FLAGS_SHORT_GI) ?
                RATE_INFO_FLAGS_SHORT_GI : 0);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0))
            uc_flag |= ((pst_query_station_reponse_event->st_txrate.flags & MAC_RATE_INFO_FLAGS_60G) ?
                RATE_INFO_FLAGS_60G : 0);
#endif
        }
...
#else
        {
            uc_flag |= ((pst_query_station_reponse_event->st_txrate.flags & MAC_RATE_INFO_FLAGS_MCS) ?
                RATE_INFO_FLAGS_MCS : 0);
            uc_flag |= ((pst_query_station_reponse_event->st_txrate.flags & MAC_RATE_INFO_FLAGS_VHT_MCS) ?
            ...
            pst_hmac_vap->station_info.txrate.bw =
                ...
                (pst_query_station_reponse_event->st_txrate.flags & MAC_RATE_INFO_FLAGS_160_MHZ_WIDTH ?
...
oal_uint32 hmac_config_reset_operate_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (st_reset_sys.en_reset_sys_type == MAC_RESET_SWITCH_SET_TYPE) {
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
        ul_ret = hmac_config_send_event_etc(pst_mac_vap, WLAN_CFGID_RESET_HW_OPERATE,
                                            us_len, (oal_uint8 *)&st_reset_sys);
        if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                             "{hmac_config_reset_operate_etc::hmac_config_send_event_etc failed[%d].}", ul_ret);
        }
        ...
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_reset_operate_etc::the reset switch is %d.}",
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_reset_operate_etc::the reset status is %d, the reset num is %d.}",
                         pst_mac_dev->uc_device_reset_in_progress, pst_mac_dev->us_device_reset_num);
    } else {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_reset_operate_etc::the reset tpye is %d, out of limit.}",
