...
OAL_STATIC oal_uint32 hmac_roam_alg_add_bsslist(hmac_roam_bss_list_stru *pst_roam_bss_list,
                                                oal_uint8 *puc_bssid, roam_blacklist_type_enum_uint8 list_type)
{
...
oal_uint32 hmac_roam_alg_add_blacklist_etc(hmac_roam_info_stru *pst_roam_info,
                                           oal_uint8 *puc_bssid,
...
oal_uint32 hmac_roam_alg_scan_one_channel_init_etc(hmac_roam_info_stru *pst_roam_info,
                                                   mac_scan_req_stru *pst_scan_params)
{
    ...
    OAM_INFO_LOG3(0, OAM_SF_ROAM,
                  "{hmac_roam_alg_scan_channel_init_etc::scan one channel: ChanNum[%d], Band[%d], ChanIdx[%d]}",
    ...
    if (pst_roam_info->en_roam_trigger == ROAM_TRIGGER_HOME_NETWORK) {  // scan 5G home network
        ...
        ul_ret = mac_get_channel_idx_from_num_etc(pst_scan_params->ast_channel_list[0].en_band,
                                                  pst_scan_params->ast_channel_list[0].uc_chan_number,
                                                  &(pst_scan_params->ast_channel_list[0].uc_chan_idx));
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(0, OAM_SF_SCAN,
                             "hmac_roam_alg_scan_channel_init_etc::mac_get_channel_idx_from_num_etc fail=%d", ul_ret);
        }
    }
#endif
    else {
...
OAL_STATIC oal_uint32 hmac_roam_alg_get_capacity_by_rssi(wlan_protocol_enum_uint8 en_protocol,
                                                         wlan_bw_cap_enum_uint8 en_bw_cap, oal_int8 c_rssi)
{
...
OAL_STATIC oal_uint32 hmac_roam_alg_calc_avail_channel_capacity(mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    puc_obss_ie = mac_find_ie_etc(MAC_EID_QBSS_LOAD, (oal_uint8 *)(pst_bss_dscr->auc_mgmt_buff + uc_ie_offset),
                                  (oal_int32)(pst_bss_dscr->ul_mgmt_len - uc_ie_offset));
    /*lint +e416*/
...
OAL_STATIC void hmac_roam_alg_record_bss_info_etc(hmac_roam_record_stru *pst_roam_record, mac_bss_dscr_stru *pst_bss_dscr)
{
    mac_channel_stru *pst_channel;
...
oal_uint32 hmac_roam_alg_bss_in_ess_etc(hmac_roam_info_stru *pst_roam_info, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    if (pst_bss_dscr->st_channel.en_band == WLAN_BAND_5G) {
        hmac_roam_alg_record_bss_info_etc(&(pst_roam_alg->st_home_network.st_5g_bssid), pst_bss_dscr);
    } else if (pst_bss_dscr->st_channel.en_band == WLAN_BAND_2G) {
        hmac_roam_alg_record_bss_info_etc(&(pst_roam_alg->st_home_network.st_2g_bssid), pst_bss_dscr);
    } else {
...
oal_uint32 hmac_roam_check_cipher_limit_etc(hmac_roam_info_stru *pst_roam_info, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    if (pst_roam_info->pst_hmac_vap->bit_11r_enable) {
        ...
        if ((en_auth_mode == WLAN_WITP_AUTH_SAE) && (puc_mde != OAL_PTR_NULL)) {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ROAM,
                             "hmac_roam_check_cipher_limit_etc::SAE do not support 11r roam");
            return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
            /* 不支持ft到非ft */
        } else if ((en_auth_mode == WLAN_WITP_AUTH_FT) && (puc_mde == OAL_PTR_NULL)) {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ROAM,
                             "hmac_roam_check_cipher_limit_etc::not support roam from FT  to Non_FT");
            return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
        }
    }
#endif
...
oal_bool_enum_uint8 hmac_roam_alg_neighbor_rpt_is_target_ap_etc(hmac_roam_info_stru *pst_roam_info,
...
oal_uint32 hmac_roam_alg_check_bssid_limit(hmac_roam_info_stru *pst_roam_info, oal_uint8 *puc_mac_addr)
{
    ...
    if (pst_roam_info->st_scan_h2d_params.st_scan_params.uc_neighbor_report_process_flag == OAL_TRUE) {
        ul_ret = hmac_roam_alg_neighbor_rpt_is_target_ap_etc(pst_roam_info, puc_mac_addr);
        if (ul_ret != OAL_TRUE) {
            OAM_WARNING_LOG3(0, OAM_SF_ROAM,
                "{hmac_roam_alg_check_bssid_limit:: [%02X:XX:XX:XX:%02X:%02X] is not neighbor report target ap!}",
                puc_mac_addr[0], puc_mac_addr[4], puc_mac_addr[5]);
            return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
        }
    }
...
oal_uint32 hmac_roam_alg_bss_check_etc(hmac_roam_info_stru *pst_roam_info, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    /* 检查特定BSSID的漫游 */
    if (oal_memcmp(pst_roam_info->auc_target_bssid, pst_bss_dscr->auc_bssid, OAL_MAC_ADDR_LEN) &&
