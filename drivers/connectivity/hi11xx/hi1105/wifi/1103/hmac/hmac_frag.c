                             ...
                             oal_uint32 ul_max_tx_unit)
{
    ...
    do {
        ...
        /* 防止反复扩充头部空间预留32字节 */
        pst_netbuf = oal_netbuf_alloc(ul_frag_size + MAC_80211_QOS_HTC_4ADDR_FRAME_LEN + 32,
                                      MAC_80211_QOS_HTC_4ADDR_FRAME_LEN + 32, 4);
        if (pst_netbuf == OAL_PTR_NULL) {
            /* 在外部释放之前申请的报文 */
            OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                           "{hmac_frag_process::pst_netbuf null.}");
...
oal_netbuf_stru *hmac_defrag_process_etc(hmac_user_stru *pst_hmac_user,
    ...
    if (pst_hmac_user->pst_defrag_netbuf == OAL_PTR_NULL) {
        ...
        if (pst_new_buf == OAL_PTR_NULL) {
            OAM_ERROR_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ANY,
                           "{hmac_defrag_process_etc::Alloc new_buf null,size[%d].}",
                           HMAC_MAX_FRAG_SIZE);
            return OAL_PTR_NULL;
        }
