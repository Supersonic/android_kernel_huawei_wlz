                                            ...
                                            oal_void *pst_mask_addr)
{
    ...
    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR3(pst_mac_vap, pst_ip_addr, pst_mask_addr))) {
        OAM_ERROR_LOG3(0, OAM_SF_PWR,
            ...
            (uintptr_t)pst_mac_vap, (uintptr_t)pst_ip_addr, (uintptr_t)pst_mask_addr);
    ...
    if (en_type == DMAC_CONFIG_IPV4) {
        ...
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PWR,
                       "{hmac_arp_offload_set_ip_addr_etc::The type[%d] is error.}", en_type);
    ...
    if (OAL_UNLIKELY(en_oper >= DMAC_IP_OPER_BUTT)) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PWR,
                       "{hmac_arp_offload_set_ip_addr_etc::The operation[%d] is error.}", en_type);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    st_ip_addr_config.en_oper = en_oper;

    ul_ret = hmac_config_set_ip_addr_etc(pst_mac_vap,
