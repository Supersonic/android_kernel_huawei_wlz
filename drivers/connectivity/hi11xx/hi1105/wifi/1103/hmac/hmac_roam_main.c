...
/* 2 全局变量定义 */
OAL_STATIC hmac_roam_homenetwork_thr_stru g_st_roam_homenetwork_thr_etc = { -50, -60, -74};
...
OAL_STATIC oal_uint32 hmac_roam_connecting_timeout(hmac_roam_info_stru *pst_roam_info,
                                                   oal_void *p_param);
...
OAL_STATIC oal_uint32 hmac_roam_neighbor_response_process(hmac_roam_info_stru *pst_roam_info,
                                                          oal_void *p_param);
OAL_STATIC oal_uint32 hmac_wait_neighbor_response_timeout(hmac_roam_info_stru *pst_roam_info,
                                                          oal_void *p_param);
#endif
...
oal_uint32 hmac_roam_main_fsm_action_etc(hmac_roam_info_stru *pst_roam_info,
                                         roam_main_fsm_event_type_enum en_event, oal_void *p_param)
{
...
oal_void hmac_roam_home_network_set_thr_etc(oal_int8 c_trigger_rssi, oal_int8 c_5g_rssi_th, oal_int8 c_5g_hold_th)
{
    g_st_roam_homenetwork_thr_etc.c_trigger_rssi = c_trigger_rssi;
    g_st_roam_homenetwork_thr_etc.c_5g_rssi_th = c_5g_rssi_th;
    g_st_roam_homenetwork_thr_etc.c_5g_hold_th = c_5g_hold_th;
    OAM_WARNING_LOG3(0, OAM_SF_ROAM, "2g c_trigger_rssi[%d], c_5g_rssi_th[%d], c_5g_hold_th[%d]",
                     (oal_int32)g_st_roam_homenetwork_thr_etc.c_trigger_rssi,
                     (oal_int32)g_st_roam_homenetwork_thr_etc.c_5g_rssi_th,
                     (oal_int32)g_st_roam_homenetwork_thr_etc.c_5g_hold_th);
}


oal_void hmac_roam_check_psk_etc(hmac_vap_stru *pst_hmac_vap, mac_conn_security_stru *pst_conn_sec)
{
    oal_uint32 ul_akm_suite = OAL_NTOH_32(pst_conn_sec->st_crypto.aul_akm_suite[0]);
    oal_uint8 uc_akm_type = ul_akm_suite & 0xFF;
...
OAL_STATIC oal_void hmac_roam_check_home_network_etc(hmac_vap_stru *pst_hmac_vap, hmac_roam_info_stru *pst_roam_info)
{
...
OAL_STATIC oal_void hmac_roam_home_network_init_etc(hmac_vap_stru *pst_hmac_vap, hmac_roam_info_stru *pst_roam_info)
{
    ...
    pst_roam_info->st_alg.st_home_network.uc_is_homenetwork = OAL_FALSE;
    pst_roam_info->st_alg.st_home_network.c_trigger_rssi = g_st_roam_homenetwork_thr_etc.c_trigger_rssi;
    pst_roam_info->st_alg.st_home_network.c_5g_rssi_th = g_st_roam_homenetwork_thr_etc.c_5g_rssi_th;
    pst_roam_info->st_alg.st_home_network.c_5g_hold_th = g_st_roam_homenetwork_thr_etc.c_5g_hold_th;
    ...
    hmac_roam_check_home_network_etc(pst_hmac_vap, pst_roam_info);
...
oal_uint32 hmac_roam_init_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_hmac_vap->pul_roam_info == OAL_PTR_NULL) {
        ...
        if (pst_hmac_vap->pul_roam_info == OAL_PTR_NULL) {
            OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                           "{hmac_roam_init_etc::OAL_MEM_ALLOC fail.}");
    ...
    hmac_roam_home_network_init_etc(pst_hmac_vap, pst_roam_info);
...
oal_uint32 hmac_roam_info_init_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_roam_info->pst_hmac_user == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_info_init_etc::assoc_vap_id[%d] can't found.}",
...
oal_uint32 hmac_roam_exit_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_hmac_vap->pul_roam_info == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                         "{hmac_roam_exit_etc::pul_roam_info is NULL.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32 hmac_roam_show_info_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pc_print_buff == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                       "{hmac_config_vap_info_etc::pc_print_buff null.}");
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
...
oal_uint32 hmac_roam_enable_etc(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_enable)
{
    ...
    if (pst_roam_info == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_enable_etc::pst_roam_info null .}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }

    if (uc_enable == pst_roam_info->uc_enable) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_enable_etc::SET[%d] fail .}", uc_enable);
        return OAL_FAIL;
    }
    ...
    OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                     "{hmac_roam_enable_etc::SET[%d] OK!}", uc_enable);
...
oal_uint32 hmac_roam_org_etc(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_scan_orthogonal)
{
    ...
    if (pst_roam_info == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_org_etc::pst_roam_info null .}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }

    /* 设置漫游正交 */
    pst_roam_info->st_config.uc_scan_orthogonal = uc_scan_orthogonal;
    OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                  "{hmac_roam_org_etc::SET[%d] OK!}", uc_scan_orthogonal);
...
oal_uint32 hmac_roam_band_etc(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_scan_band)
{
    ...
    if (pst_roam_info == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_band_etc::pst_roam_info null .}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }
    ...
    OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                  "{hmac_roam_band_etc::SET[%d] OK!}", uc_scan_band);
...
oal_uint32 hmac_roam_check_signal_bridge_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_check_signal_bridge_etc::pst_mac_device ptr is null!}\r\n");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_other_vap == OAL_PTR_NULL) {
            OAM_WARNING_LOG0(pst_mac_device->auc_vap_id[uc_vap_idx], OAM_SF_ROAM,
                             "{hmac_roam_check_signal_bridge_etc::vap is null!}");
            continue;
        }

        /* 终端需求: 打开信号桥，禁止漫游 */
        if ((pst_other_vap->en_p2p_mode == WLAN_P2P_GO_MODE) &&
            (pst_other_vap->ast_app_ie[OAL_APP_BEACON_IE].ul_ie_len == 0)) {
            /* 如果是P2P GO模式且Beacon帧不包含P2P ie即为信号桥 */
            OAM_WARNING_LOG0(pst_other_vap->uc_vap_id, OAM_SF_ROAM,
                             "{hmac_roam_check_signal_bridge_etc::vap is GO Signal Bridge, disable STA roaming!}");
            return OAL_ERR_CODE_ROAM_DISABLED;
        }
    }
                               ...
                               roam_trigger_enum_uint8 en_roam_trigger)
{
    ...
    if (ROAM_SCAN_CHANNEL_ORG_0 == uc_scan_type) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                           "{hmac_roam_start_etc::START fail[%d].}", ul_ret);
            return ul_ret;
        }
    } else {
...
oal_uint32 hmac_roam_handle_home_network_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if ((pst_roam_info->st_config.uc_scenario_enable == OAL_TRUE) &&
        ...
        if (oal_time_after32(ul_current_timestamp,
            ...
            OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                "{hmac_roam_handle_home_network_etc::roam_to home network env:%d sign:%d max_rssi:%d current_rssi:%d}",
            ...
            return hmac_roam_start_etc(pst_hmac_vap, ROAM_SCAN_CHANNEL_ORG_1,
...
oal_uint32 hmac_roam_show_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_roam_info->uc_rssi_type == ROAM_ENV_SPARSE_AP) {
        OAM_WARNING_LOG4(uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_show_etc::default env band[%d] scan_orthogonal[%d], c_trigger_rssi_2G=%d c_trigger_rssi_5G=%d}",
...
OAL_STATIC oal_uint32 hmac_roam_scan_channel(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    /* 发起背景扫描 */
    ul_ret = hmac_fsm_call_func_sta_etc(pst_roam_info->pst_hmac_vap, HMAC_FSM_INPUT_SCAN_REQ,
                                        (oal_void *)(&pst_roam_info->st_scan_h2d_params));
    if (ul_ret != OAL_SUCC) {
...
#ifdef _PRE_WLAN_FEATURE_11V_ENABLE
OAL_STATIC oal_uint32 hmac_roam_check_11v_scan_result(hmac_roam_info_stru *pst_roam_info,
                                                      oal_bool_enum_uint8 en_find_bss)
{
    ...
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
                         "{hmac_roam_check_11v_scan_result::mac_res_get_hmac_user_etc failed.}");
    ...
    if (en_find_bss == OAL_FALSE) {
        ...
        OAM_WARNING_LOG1(0, OAM_SF_ROAM,
                         "{roam_to::hmac_roam_check_11v_scan_result:: 11v candidate list no bss valid, scan fail=%d}",
                         pst_roam_info->st_static.ul_roam_11v_scan_fail);
...
OAL_STATIC oal_uint32 hmac_roam_check_scan_result(hmac_roam_info_stru *pst_roam_info,
                                                  oal_void *p_param,
                                                  mac_bss_dscr_stru **ppst_bss_dscr_out)
{
    oal_uint32 ul_ret;
...
OAL_STATIC oal_uint32 hmac_roam_handle_scan_result(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (ul_ret == OAL_SUCC) {
        ...
        /* 扫描到可用的bss，开始connect */
        return hmac_roam_main_fsm_action_etc(pst_roam_info,
...
OAL_STATIC oal_uint32 hmac_roam_resume_pm(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    ul_ret = hmac_config_set_pm_by_module_etc(&pst_roam_info->pst_hmac_vap->st_vap_base_info,
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_roam_info->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                         "{hmac_roam_resume_pm::hmac_config_set_pm_by_module_etc failed[%d].}", ul_ret);
    }
#endif
...
OAL_STATIC oal_uint32 hmac_roam_resume_security_port(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    // 发送同步信息
    ul_ret = hmac_config_send_event_etc(&pst_roam_info->pst_hmac_vap->st_vap_base_info,
                                        WLAN_CFGID_ROAM_HMAC_SYNC_DMAC,
                                        OAL_SIZEOF(mac_h2d_roam_sync_stru),
                                        (oal_uint8 *)&st_h2d_sync);
    if (ul_ret != OAL_SUCC) {
...
OAL_STATIC oal_uint32 hmac_roam_connecting_fail(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (pst_roam_info->st_alg.c_current_rssi > ROAM_RSSI_MAX_TYPE) {
        ...
        return hmac_roam_trigger_handle_etc(pst_roam_info->pst_hmac_vap,
...
OAL_STATIC oal_uint32 hmac_roam_connect_to_bss(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_connect_to_bss::pst_mac_device ptr is null!}\r\n");
        return -OAL_EINVAL;
    }
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_other_vap == OAL_PTR_NULL) {
            OAM_WARNING_LOG0(pst_mac_device->auc_vap_id[uc_vap_idx], OAM_SF_ROAM,
                             "{hmac_roam_connect_to_bss::vap is null!}");
            continue;
        }
        ...
        if (OAL_VALUE_IN_VALID_RANGE(pst_other_vap->en_vap_state,
            ...
            OAM_WARNING_LOG1(pst_other_vap->uc_vap_id, OAM_SF_ROAM,
                             "{hmac_roam_connect_to_bss::vap is connecting, state=[%d]!}",
    ...
    if (OAL_TRUE == (mac_mib_get_ft_trainsistion(&pst_hmac_vap->st_vap_base_info) &&
        ...
        OAM_WARNING_LOG1(0, OAM_SF_ROAM,
                         "{hmac_roam_connect_to_bss::cmd bit_11r_over_ds = [%d]!}",
...
OAL_STATIC oal_uint32 hmac_roam_to_new_bss(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                         "{hmac_roam_to_new_bss::hmac_fsm_call_func_sta_etc fail! erro code is %u}", ul_ret);
    }
    hmac_config_set_mgmt_log_etc(&pst_hmac_vap->st_vap_base_info, &pst_hmac_user->st_user_base_info, OAL_FALSE);
    ...
    ul_ret = hmac_config_send_event_etc(&pst_hmac_vap->st_vap_base_info,
                                        WLAN_CFGID_ROAM_SUCC_H2D_SYNC,
                                        OAL_SIZEOF(oal_uint8),
                                        (oal_uint8 *)&uc_roam_band_state);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
...
oal_uint32 hmac_roam_pause_user_etc(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
...
#ifdef _PRE_WLAN_FEATURE_STA_PM
    ul_ret = hmac_config_set_pm_by_module_etc(&pst_hmac_vap->st_vap_base_info,
                                              MAC_STA_PM_CTRL_TYPE_ROAM,
                                              MAC_STA_PM_SWITCH_OFF);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_CFG,
                       "{hmac_roam_pause_user_etc::hmac_config_set_pm_by_module_etc failed[%d].}", ul_ret);
    ...
    uc_roaming_mode = 1;
    ul_ret = hmac_config_send_event_etc(&pst_hmac_vap->st_vap_base_info, WLAN_CFGID_SET_ROAMING_MODE,
                                        OAL_SIZEOF(oal_uint8), (oal_uint8 *)&uc_roaming_mode);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_CFG,
                       "{hmac_roam_pause_user_etc::send event[WLAN_CFGID_SET_ROAMING_MODE] failed[%d].}", ul_ret);
        oal_net_tx_wake_all_queues(pst_net_device);
        return ul_ret;
    }
...
oal_uint32 hmac_roam_resume_user_etc(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    if (pst_hmac_vap->st_vap_base_info.en_vap_state != MAC_VAP_STATE_ROAMING) {
        ...
        oal_net_tx_wake_all_queues(pst_net_device);
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ROAM,
                         "{hmac_roam_resume_user_etc::vap state[%d] NOT ROAMING!}",
                         pst_hmac_vap->st_vap_base_info.en_vap_state);
        return OAL_SUCC;
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_CFG,
                       "{hmac_roam_resume_user_etc::send event[WLAN_CFGID_SET_ROAMING_MODE] failed[%d].}", ul_ret);
    }
...
oal_uint32 hmac_roam_trigger_handle_etc(hmac_vap_stru *pst_hmac_vap, oal_int8 c_rssi,
                                        oal_bool_enum_uint8 en_current_bss_ignore)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                         "{hmac_roam_trigger_handle_etc::START fail[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32 hmac_roam_reassoc_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_roam_info->uc_enable == 0) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_reassoc_etc::roam disabled.}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }
    ...
    if (OAL_TRUE == (mac_mib_get_ft_trainsistion(&pst_hmac_vap->st_vap_base_info))) {
        if (mac_mib_get_ft_over_ds(&pst_hmac_vap->st_vap_base_info) &&
            ...
            if (ul_ret != OAL_SUCC) {
                OAM_WARNING_LOG1(0, OAM_SF_ROAM,
                                 "{hmac_roam_reassoc_etc::hmac_fsm_call_func_sta_etc fail[%ld]!}", ul_ret);
                return ul_ret;
            }
        }
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ROAM,
                         "{hmac_roam_reassoc_etc::ft_over_ds=[%d],to send reassoc!}",
                         mac_mib_get_ft_over_ds(&pst_hmac_vap->st_vap_base_info));
    ...
    if (l_framebody_len > MAC_NEIGHBOR_REPORT_ACTION_RSP_FIX_LEN) {
        ...
        while (l_framebody_len >= MAC_NEIGHBOR_RPT_FIX_LEN) {
            ...
            if (puc_neighbor_rpt_ie != OAL_PTR_NULL) {
                ...
                if (ul_ret == OAL_SUCC) {
                    hmac_roam_neighbor_report_add_chan(pst_scan_params, pst_neighbor_rpt_ie->uc_channum,
                                                       en_band, uc_channel_idx);
                }
...
oal_void hmac_roam_connect_complete_etc(hmac_vap_stru *pst_hmac_vap, oal_uint32 ul_result)
{
    ...
    if (pst_roam_info == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_connect_complete_etc::pst_roam_info null!}");
        return;
    }
    ...
    if (pst_roam_info->uc_enable == 0) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_connect_complete_etc::roam disabled!}");
        return;
    }
    ...
    if (pst_roam_info->uc_roaming_scenario == ROAMING_SCENARIO_HOME) {
        pst_roam_info->st_alg.st_home_network.ul_5g_scan_timestamp = (oal_uint32)OAL_TIME_GET_STAMP_MS();
    }
