...
oal_uint32 hmac_check_opmode_notify_etc(hmac_vap_stru *pst_hmac_vap,
    ...
    if ((puc_opmode_notify_ie != OAL_PTR_NULL) && (puc_opmode_notify_ie[1] == MAC_OPMODE_NOTIFY_LEN)) {
        ...
        if (pst_mac_user->en_avail_num_spatial_stream > pst_opmode_notify->bit_rx_nss ||
            (pst_mac_user->en_avail_num_spatial_stream == WLAN_SINGLE_NSS &&
             pst_opmode_notify->bit_rx_nss != WLAN_SINGLE_NSS)) {
            OAM_WARNING_LOG0(0, OAM_SF_OPMODE, "{hmac_check_opmode_notify_etc::SMPS and OPMODE show different nss!}");
            if (pst_mac_user->en_cur_protocol_mode == WLAN_HT_MODE ||
                pst_mac_user->en_cur_protocol_mode == WLAN_HT_ONLY_MODE) {
                return OAL_FAIL;
            }
        }

        ul_relt = mac_ie_proc_opmode_field_etc(pst_mac_vap, pst_mac_user, pst_opmode_notify, OAL_FALSE);

        if (OAL_UNLIKELY(ul_relt != OAL_SUCC)) {
            OAM_WARNING_LOG1(pst_mac_user->uc_vap_id, OAM_SF_OPMODE,
                             "{hmac_check_opmode_notify_etc::mac_ie_proc_opmode_field_etc failed[%d].}", ul_relt);
            return ul_relt;
        }
    }
    return OAL_SUCC;
...
oal_uint32 hmac_mgmt_rx_opmode_notify_frame_etc(hmac_vap_stru *pst_hmac_vap,
                                                hmac_user_stru *pst_hmac_user,
                                                oal_netbuf_stru *pst_netbuf)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_netbuf)) {
        OAM_ERROR_LOG3(0, OAM_SF_OPMODE,
            "{hmac_mgmt_rx_opmode_notify_frame_etc::pst_hmac_vap = [%x],pst_hmac_user = [%x], pst_netbuf = [%x]!}\r\n",
    ...
    if ((OAL_FALSE == mac_mib_get_VHTOptionImplemented(&pst_hmac_vap->st_vap_base_info)) ||
        ...
        OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_OPMODE,
                      "{hmac_mgmt_rx_opmode_notify_frame_etc::the vap is not support OperatingModeNotification!}\r\n");
        return OAL_SUCC;
    }
    ...
    if (pst_mac_user == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_OPMODE,
                       "{hmac_mgmt_rx_opmode_notify_frame_etc::pst_mac_user null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ul_ret = mac_ie_proc_opmode_field_etc(&(pst_hmac_vap->st_vap_base_info),
                pst_mac_user, pst_opmode_notify, OAL_FALSE);
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_mac_user->uc_vap_id, OAM_SF_OPMODE,
                         "{hmac_mgmt_rx_opmode_notify_frame_etc::hmac_config_send_event_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
