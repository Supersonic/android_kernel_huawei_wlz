...
OAL_STATIC OAL_INLINE oal_void hmac_lock_max_cpu_freq(oal_void)
{
    ...
    for (uc_cpuid_loop = 0; uc_cpuid_loop < OAL_BUS_MAXCPU_NUM; uc_cpuid_loop++) {
        ...
        OAM_WARNING_LOG2(0, OAM_SF_ANY, "hmac_adjust_set_irq:lock cpu freq cpu[%d]->%d",
                         uc_cpuid_loop, g_aul_cpumaxfreq_etc[uc_cpuid_loop]);
    }
}
...
#if defined(_PRE_FEATURE_PLAT_LOCK_CPUFREQ) && !defined(CONFIG_HI110X_KERNEL_MODULES_BUILD_SUPPORT)
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    if (g_uc_lock_max_cpu_freq_switch == OAL_TRUE) {
                                  ...
                                  oal_uint32 ul_trx_pps)
{
    ...
    if (g_freq_lock_control_etc.uc_req_irq_cpu != g_freq_lock_control_etc.uc_cur_irq_cpu) {
        OAM_WARNING_LOG4(0, OAM_SF_ANY,
                         "{hmac_perform_calc_throughput: rx = %d , tx = %d, pps_high = %d, pps low = %d!}",
                         ul_rx_throughput_mbps, ul_tx_throughput_mbps, ul_limit_pps_high, ul_limit_pps_low);
        OAM_WARNING_LOG4(0, OAM_SF_ANY,
                         "{hmac_perform_calc_throughput: req cpu id[%d],cur cpu id[%d],high_th = %d,low_th = %d!}",
                         g_freq_lock_control_etc.uc_req_irq_cpu, g_freq_lock_control_etc.uc_cur_irq_cpu,
                         ul_limit_throughput_high, ul_limit_throughput_low);
        OAM_WARNING_LOG3(0, OAM_SF_ANY,
                         "{hmac_perform_calc_throughput: rx pps = %d , tx pps = %d, trx pps = %d,!}",
