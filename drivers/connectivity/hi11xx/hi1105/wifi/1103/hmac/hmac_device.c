...
oal_uint32 hmac_device_exit_etc(mac_board_stru *pst_board, mac_chip_stru *pst_chip, hmac_device_stru *pst_hmac_device)
{
    ...
    if (ul_return != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                         "{hmac_device_exit_etc::hmac_config_del_vap_etc failed[%d].}", ul_return);
        return ul_return;
    }
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_vap == OAL_PTR_NULL) {
            OAM_WARNING_LOG1(0, OAM_SF_ANY,
                             "{hmac_device_exit_etc::mac_res_get_hmac_vap failed vap_idx[%u].}", uc_vap_idx);
            continue;
        }
        ...
        if (ul_return != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                             "{hmac_device_exit_etc::hmac_vap_destroy_etc failed[%d].}", ul_return);
            return ul_return;
        }
...
OAL_STATIC oal_uint32 hmac_cfg_vap_init(mac_device_stru *pst_device)
{
...

    {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_device->uc_cfg_vap_id, OAM_SF_ANY,
                             "{hmac_cfg_vap_init::hmac_vap_init_etc failed[%d].}", ul_ret);
            return ul_ret;
        }
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_device->uc_cfg_vap_id, OAM_SF_ANY,
                         "{hmac_cfg_vap_init::hmac_vap_creat_netdev_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_void hmac_do_suspend_action_etc(mac_device_stru *pst_mac_device, oal_uint8 uc_in_suspend)
{
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_mac_device->uc_cfg_vap_id, OAM_SF_ANY,
                         "{hmac_do_suspend_action_etc::pst_hmac_device null.}");
        return;
    }
    ...
    if (wlan_pm_switch_etc && ul_is_wlan_poweron) {
        ...
        ***************************************************************************/
        ul_ret = hmac_config_send_event_etc(pst_cfg_mac_vap, WLAN_CFGID_SUSPEND_ACTION_SYN,
                                            OAL_SIZEOF(mac_cfg_suspend_stru), (oal_uint8 *)&st_suspend);
        if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
...
oal_uint32 hmac_send_evt2wal_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_evtid,
                                 oal_uint8 *puc_evt, oal_uint32 ul_evt_len)
{
...
oal_uint32 hmac_config_host_dev_init_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_device_init_etc:: pst_mac_device[%d] NULL pointer!}",
                       pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_hmac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_config_host_dev_init_etc::pst_hmac_device[%d] null!}",
                       pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32 hmac_config_host_dev_exit_etc(mac_vap_stru *pst_mac_vap)
{
    ...
    if (OAL_UNLIKELY(pst_hmac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_config_host_dev_exit_etc::pst_hmac_device[%d] null!}",
                       pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    for (uc_chip = 0; uc_chip < ul_chip_max_num; uc_chip++) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG2(0, OAM_SF_ANY,
                             "{hmac_init_event_process_etc::hmac_chip_init failed[%d], uc_chip_id[%d].}",
                             ul_ret, uc_chip);
            return OAL_FAIL;
        }
