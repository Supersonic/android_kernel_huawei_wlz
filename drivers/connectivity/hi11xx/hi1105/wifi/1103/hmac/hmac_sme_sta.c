                                                       ...
                                                       oal_int8 *puc_param)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_device_id, OAM_SF_SCAN,
                         "{hmac_cfg80211_prepare_scan_req_sta::mac_res_get_dev_etc fail.device_id:%u}",
    ...
    for (uc_loop = 0; uc_loop < pst_cfg80211_scan_param->l_ssid_num; uc_loop++) {
        ...
        /* ssid末尾置'\0' */
        pst_scan_params->ast_mac_ssid_set[uc_loop].auc_ssid[pst_cfg80211_scan_param->st_ssids[uc_loop].uc_ssid_len] = '\0';
    }
...
oal_void hmac_handle_auth_rsp_sta_etc(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_msg)
{
    ...
    if (pst_auth_rsp->us_status_code == HMAC_MGMT_SUCCESS) {
        OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                      "{hmac_handle_auth_rsp_sta_etc::auth succ.}");
