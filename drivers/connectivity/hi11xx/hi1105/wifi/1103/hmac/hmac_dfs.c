...
OAL_STATIC oal_void hmac_scan_switch_channel_back(mac_vap_stru *pst_mac_vap);
OAL_STATIC oal_void hmac_scan_switch_channel_off(mac_vap_stru *pst_mac_vap);
#endif
...
oal_uint32 hmac_dfs_recalculate_channel_etc(mac_device_stru *pst_mac_device,
                                            oal_uint8 *puc_freq,
                                            wlan_channel_bandwidth_enum_uint8 *pen_bandwidth)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG2(pst_mac_device->uc_device_id, OAM_SF_DFS,
                       "{hmac_dfs_recalculate_channel_etc::mac_get_channel_idx_from_num_etc fail.max_band:%d  freq:%x}",
                       pst_mac_device->en_max_band, (uintptr_t)puc_freq);
        return ul_ret;
    }
...
OAL_STATIC OAL_INLINE oal_uint32 hmac_dfs_cac_event_report(mac_vap_stru *pst_mac_vap,
...
OAL_STATIC oal_uint32 hmac_dfs_update_available_channel_list(mac_device_stru *pst_mac_device,
                                                             oal_uint8 uc_chan_idx,
                                                             oal_bool_enum_uint8 en_radar_detected)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_DFS,
                       "{hmac_dfs_update_available_channel_list::chan_idx(%d) invalid!}", uc_chan_idx);
...
oal_uint32 hmac_dfs_cac_timeout_fn_etc(oal_void *p_arg)
{
    ...
    for (uc_idx = 0; uc_idx < st_chan_info.ul_channels; uc_idx++) {
        ul_ret = hmac_dfs_update_available_channel_list(pst_mac_device,
                                                        st_chan_info.ast_channels[uc_idx].uc_idx, OAL_FALSE);
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG0(0, OAM_SF_DFS,
                             "{hmac_dfs_cac_timeout_fn_etc::hmac_dfs_update_available_channel_list fail.}");
        }
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_DFS,
                         "{[DFS]hmac_dfs_cac_timeout_fn_etc::mac_get_channel_idx_from_num_etc failed=%d}", ul_ret);
    }
...
OAL_STATIC oal_uint32 hmac_chan_get_cac_time(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_rd_info->en_dfs_domain == MAC_DFS_DOMAIN_ETSI) {
        ul_ret = mac_get_channel_idx_from_num_etc(pst_mac_vap->st_channel.en_band,
                                                  pst_mac_vap->st_channel.uc_chan_number, &uc_idx);
        if (ul_ret != OAL_SUCC) {
            OAM_ERROR_LOG2(0, OAM_SF_DFS,
                           "{hmac_chan_get_cac_time::mac_get_channel_idx_from_num_etc fail. _band:%d, offchan_num:%u}",
...
OAL_STATIC oal_uint32 hmac_chan_get_off_chan_cac_time(mac_device_stru *pst_mac_device,
                                                      mac_vap_stru *pst_mac_vap)
{
    mac_channel_list_stru st_chan_info;
    oal_uint8 uc_idx = 0;

    mac_get_channel_idx_from_num_etc(pst_mac_vap->st_channel.en_band,
...
oal_void hmac_dfs_cac_stop_etc(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_mac_device->st_dfs.st_dfs_cac_timer.en_is_enabled == OAL_TRUE) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(0, OAM_SF_DFS,
                             "{[DFS]hmac_dfs_cac_stop_etc::hmac_dfs_cac_event_report failed=%d}", ul_ret);
        }
...
oal_void hmac_dfs_cac_start_etc(mac_device_stru *pst_mac_device, hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (ul_scan_time < HMAC_DFS_SIXTY_TWO_SEC_IN_MS) {
    ...
    OAM_WARNING_LOG2(0, OAM_SF_DFS,
                     "{[DFS]hmac_dfs_cac_start_etc, CAC start, channel number:%d, timer cnt:%d.}",
                     pst_hmac_vap->st_vap_base_info.st_channel.uc_chan_number,
...
oal_void hmac_dfs_radar_wait_etc(mac_device_stru *pst_mac_device, hmac_vap_stru *pst_hmac_vap)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (OAL_UNLIKELY(pst_mac_vap == OAL_PTR_NULL)) {
            OAM_WARNING_LOG1(0, OAM_SF_DFS,
                             "{hmac_dfs_radar_wait_etc::pst_mac_vap null, vap id=%d.",
    ...
    if (0 == uc_5g_vap_cnt) {
        OAM_WARNING_LOG1(0, OAM_SF_DFS,
                         "{hmac_dfs_radar_wait_etc::reset dfs status for device[%d] after all 5g vap down",
...
OAL_STATIC oal_uint32 hmac_dfs_nol_timeout_fn(oal_void *p_arg)
{
    ...
    OAM_WARNING_LOG1(0, OAM_SF_DFS,
                     "{[DFS]hmac_dfs_nol_timeout_fn, Non-Occupancy Period expired, remove channel %d from NOL.}",
                     uc_chan_num);
...
OAL_STATIC oal_uint32 hmac_dfs_nol_addchan(mac_device_stru *pst_mac_device, oal_uint8 uc_chan_idx)
{
    ...
    if (ul_ret == OAL_SUCC) {
        pst_nol_node = (mac_dfs_nol_node_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
...
OAL_STATIC oal_uint32 hmac_dfs_nol_delchan(mac_device_stru *pst_mac_device,
...
OAL_STATIC oal_uint32 hmac_dfs_nol_clear(oal_dlist_head_stru *pst_dlist_head)
{
    ...
    if (OAL_UNLIKELY(pst_dlist_head == OAL_PTR_NULL)) {
    ...
    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_dlist_pos, pst_dlist_temp, pst_dlist_head)
    {
        ...
        ul_ret = hmac_dfs_nol_delchan(pst_mac_device, pst_nol_node);
        if (ul_ret != OAL_SUCC) {
...
OAL_STATIC oal_uint32 hmac_dfs_channel_mark_radar(mac_device_stru *pst_mac_device,
                                                  mac_vap_stru *pst_mac_vap)
{
    mac_channel_list_stru st_chan_info;
    oal_uint8 uc_idx;
...
OAL_STATIC oal_uint32 hmac_dfs_is_channel_support_bw(mac_vap_stru *pst_mac_vap,
...
OAL_STATIC oal_uint32 hmac_dfs_select_random_channel(mac_vap_stru *pst_mac_vap,
...
hmac_dfs_select_random_channel_update:
    ...
    /* 20MHz */
    if (en_try_bandwidth == WLAN_BAND_WIDTH_20M) {
        ul_window = 0x1; /* 1b */
        uc_step = 1;
...
oal_uint32 hmac_dfs_ap_wait_start_radar_handler_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum) {
        ...
        if (hmac_dfs_is_channel_support_bw(pst_mac_vap, uc_new_channel, en_new_bandwidth) != OAL_SUCC) {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                             "{hmac_dfs_switch_channel_for_radar_etc::new chan(%d) not support curr bw mode(%d).}",
                             uc_new_channel, en_new_bandwidth);
    ...
    if ((ul_ret != OAL_SUCC) || (en_new_bandwidth >= WLAN_BAND_WIDTH_BUTT)) {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
            "{hmac_dfs_ap_wait_start_radar_handler_etc::cann't start network using the selected chan[%d] or bw[%d]}",
            uc_new_channel, en_new_bandwidth);
        return ul_ret;
    }
...
oal_uint32 hmac_dfs_switch_channel_for_radar_etc(mac_device_stru *pst_mac_device,
                                                 mac_vap_stru *pst_mac_vap)
{
    oal_uint8 uc_new_channel = 0;
    wlan_channel_bandwidth_enum_uint8 en_new_bandwidth = WLAN_BAND_WIDTH_BUTT;
    ...
    if (pst_mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum) {
        uc_new_channel = pst_mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum;
        if (pst_mac_device->st_dfs.st_dfs_info.en_next_ch_width_type != WLAN_BAND_WIDTH_BUTT) {
        ...
        if (hmac_dfs_is_channel_support_bw(pst_mac_vap, uc_new_channel, en_new_bandwidth) != OAL_SUCC) {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                             "{hmac_dfs_switch_channel_for_radar_etc::new chan(%d) not support current bw mode(%d).}",
                             uc_new_channel, en_new_bandwidth);
            // 如果应用设定的下一跳信道不支持当前的带宽模式，那么按照驱动的随机策略选择
            ul_ret = hmac_dfs_select_random_channel(pst_mac_vap, &uc_new_channel, &en_new_bandwidth);
            if (ul_ret != OAL_SUCC) {
...
oal_bool_enum_uint8 hmac_dfs_try_cac_etc(hmac_device_stru *pst_hmac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                       "{hmac_dfs_try_cac_etc::mac_res_get_hmac_vap fail.vap_id[%u]}", pst_mac_vap->uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
                                       ...
                                       hmac_device_stru *pst_hmac_device)
{
    ...
    for (uc_vap_id = 0; uc_vap_id < pst_mac_device->uc_vap_num; uc_vap_id++) {
        ...
        if (pst_vap && pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
