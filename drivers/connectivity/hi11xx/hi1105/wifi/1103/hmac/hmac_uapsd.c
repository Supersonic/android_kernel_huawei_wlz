...
oal_void hmac_uapsd_update_user_para_etc(oal_uint8 *puc_payload,
                                         ...
                                         hmac_user_stru *pst_hmac_user)
{
    ...
    if (OAL_UNLIKELY(pst_mac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_CFG,
                       "{hmac_uapsd_update_user_para_etc::get mac_vap [%d] null.}",
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_uapsd_update_user_para_etc::hmac_config_send_event_etc fail. erro code is %u}", ul_ret);
    }
    OAL_MEM_FREE(puc_param, OAL_TRUE);
