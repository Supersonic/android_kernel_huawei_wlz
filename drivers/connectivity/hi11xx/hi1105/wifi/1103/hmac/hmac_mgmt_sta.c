...
oal_uint32 hmac_sta_wait_auth_seq4_rx_etc(hmac_vap_stru *pst_sta, oal_void *p_msg)
{
    ...
    if ((WLAN_FC0_SUBTYPE_AUTH | WLAN_FC0_TYPE_MGT) == mac_get_frame_type_and_subtype(puc_mac_hdr)) {
        ...
        if ((WLAN_AUTH_TRASACTION_NUM_FOUR == mac_get_auth_seq_num(puc_mac_hdr)) &&
            ...
            hmac_fsm_change_state_etc(pst_sta, MAC_VAP_STATE_STA_AUTH_COMP);
            OAM_INFO_LOG0(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                          "{hmac_sta_wait_auth_seq4_rx_etc::auth succ.}");
            /* 将认证结果上报SME */
            hmac_send_rsp_to_sme_sta_etc(pst_sta, HMAC_SME_AUTH_RSP, (oal_uint8 *)&st_auth_rsp);
        } else {
            OAM_WARNING_LOG1(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
...
oal_uint32 hmac_p2p_remain_on_channel_etc(hmac_vap_stru *pst_hmac_vap_sta, oal_void *p_param)
{
    ...
    if (OAL_UNLIKELY(pst_mac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY,
                       "{hmac_p2p_listen_timeout_etc::pst_mac_device[%d](%d) null!}",
                       pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
                                                    ...
                                                    oal_netbuf_stru *pst_netbuf)
{
    switch (en_public_action_field) {
        case MAC_PUB_GAS_INIT_RESP:
            OAM_WARNING_LOG0(0, OAM_SF_RX, "{hmac_sta_not_up_rx_public_action::receive gas init response}");
            hmac_ftm_rx_gas_init_resp(pst_hmac_vap_sta, pst_netbuf);
            break;
...
oal_uint32 hmac_sta_up_update_rrm_capability(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user,
                                             oal_uint8 *puc_payload, oal_uint32 us_rx_len)
{
    ...
    if (pst_user_cap_info->bit_radio_measurement == OAL_FALSE) {
                                      ...
                                      oal_uint16 us_msg_len)
{
    ...
    if (pst_hmac_sta->st_wds_table.en_wds_vap_mode == WDS_MODE_REPEATER_STA) {
        ...
        /* 打桩用于调试，只要检测到相应的hilinkie即认为用户支持4地址 */
            ...
            if (OAL_PTR_NULL != mac_find_vendor_ie_etc(MAC_WLAN_OUI_HUAWEI,
                                                       MAC_WLAN_OUI_TYPE_HAUWEI_4ADDR,
    ...
    ul_change |= hmac_ie_check_ht_sta(&pst_hmac_sta->st_vap_base_info, puc_payload, us_msg_len,
    ...
    if (OAL_TRUE == hmac_user_ht_support(pst_hmac_user)) {
        ...
        memset_s(pst_he_hdl, OAL_SIZEOF(mac_he_hdl_stru),
        ...
        puc_tmp_ie = mac_find_ie_ext_ie(MAC_EID_HE,
...
oal_uint32 hmac_sta_update_join_req_params_etc(hmac_vap_stru *pst_hmac_vap, hmac_join_req_stru *pst_join_req)
{
    ...
    if (pst_hmac_vap->bit_sta_protocol_cfg == OAL_SWITCH_OFF) {
        ...
        if (IS_LEGACY_STA(pst_mac_vap) &&
            (pst_join_req->st_bss_dscr.en_he_capable == OAL_TRUE) &&
            (IS_11AX_VAP(pst_mac_vap))) {
    ...
    if (pst_event_mem == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                       "{hmac_sta_update_join_req_params_etc::pst_event_mem null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
                                             ...
                                             oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (OAL_ANY_NULL_PTR2(pst_hmac_vap, puc_mac_hdr)) {
        OAM_ERROR_LOG2(0, OAM_SF_AUTH, "{hmac_sta_rx_deauth_req::param null,%d %d.}",
