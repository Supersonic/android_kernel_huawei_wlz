...
OAL_STATIC oal_uint32 hmac_btcoex_delba_foreach_tid(mac_vap_stru *pst_mac_vap,
                                                    ...
                                                    mac_cfg_delba_req_param_stru *pst_mac_cfg_delba_param)
{
    ...
    for (pst_mac_cfg_delba_param->uc_tidno = 0;
         pst_mac_cfg_delba_param->uc_tidno < WLAN_TID_MAX_NUM; pst_mac_cfg_delba_param->uc_tidno++) {
        ul_ret = hmac_config_delba_req_etc(pst_mac_vap, 0, (oal_uint8 *)pst_mac_cfg_delba_param);
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_COEX,
...
oal_uint32 hmac_btcoex_rx_delba_trigger_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_UNLIKELY(pst_hmac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_COEX,
                       "{hmac_btcoex_rx_delba_trigger_etc::pst_hmac_vap is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_COEX,
                     "{hmac_btcoex_rx_delba_trigger_etc:delba size:%d, need_delba:%d, delba_trigger:%d.}",
                     pst_dmac_to_hmac_btcoex_rx_delba->uc_ba_size,
                     pst_dmac_to_hmac_btcoex_rx_delba->en_need_delba,
    ...
    if (pst_dmac_to_hmac_btcoex_rx_delba->en_need_delba) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_COEX,
                             "{hmac_btcoex_rx_delba_trigger_etc:delba send failed:ul_ret: %d.}", ul_ret);
            return ul_ret;
        }
    }
...
OAL_STATIC oal_uint32 hmac_btcoex_delba_send_timeout(oal_void *p_arg)
{
    ...
    if (ui_val == 0) {
        ...
        if (OAL_UNLIKELY(pst_hmac_vap == OAL_PTR_NULL)) {
            OAM_ERROR_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_COEX,
                           "{hmac_btcoex_delba_send_timeout::pst_hmac_vap is null!}");
            return OAL_ERR_CODE_PTR_NULL;
        }
        ...
        OAM_WARNING_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_COEX,
                         "{hmac_btcoex_delba_send_timeout::rx_pkt[%d]!}",
                         pst_hmac_user_btcoex->uc_rx_no_pkt_count);

        if (pst_hmac_user_btcoex->uc_rx_no_pkt_count > BTCOEX_ARP_FAIL_REASSOC_NUM) {
            /* 重关联逻辑暂时关闭，等统计出现哪些场景出现不通，才限制场景放开 */
            OAM_WARNING_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_COEX,
                             "{hmac_btcoex_delba_send_timeout::need to reassoc to resume.}");
...
oal_void hmac_btcoex_arp_fail_delba_process_etc(oal_netbuf_stru *pst_netbuf, mac_vap_stru *pst_mac_vap)
{
    ...
    /* 只要统计功能打开，就需要做一次探测 */
    if ((pst_hmac_user_btcoex->us_ba_size > 0) && (pst_hmac_user->st_hmac_user_btcoex.en_arp_probe_on == OAL_TRUE)) {
...
oal_uint32 hmac_btcoex_check_exception_in_list_etc(oal_void *p_arg, oal_uint8 *auc_addr)
{
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_COEX,
                       "{hmac_btcoex_check_exception_in_list_etc::pst_hmac_device null}");
        return OAL_FALSE;
    }
...
OAL_STATIC oal_void hmac_btcoex_add_exception_to_list(hmac_vap_stru *pst_hmac_vap, oal_uint8 *auc_mac_addr)
{
    ...
    if (pst_hmac_device_btcoex->uc_exception_bss_index >= BTCOEX_BSS_NUM_IN_BLACKLIST) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_COEX,
                         "{hmac_btcoex_add_exception_to_list::already reach max num:%d.}",
...
oal_uint32 hmac_config_print_btcoex_status_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_COEX,
                       "{hmac_config_print_btcoex_status_etc::pst_hmac_device null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
