...
OAL_STATIC oal_uint32 hmac_wds_find_node(hmac_vap_stru *pst_hmac_vap,
                                         oal_uint8 *puc_addr,
                                         hmac_wds_node_stru **ppst_wds_node)
{
    oal_uint32 ul_hash_value;
...
oal_uint32 hmac_wds_find_sta(hmac_vap_stru *pst_hmac_vap,
                             oal_uint8 *puc_addr,
...
oal_uint32 hmac_wds_del_sta_entry(hmac_vap_stru *pst_hmac_vap,
                                  oal_uint8 *puc_addr,
                                  hmac_wds_stas_stru *pst_sta)
{
    if ((pst_sta == OAL_PTR_NULL) && (OAL_SUCC != hmac_wds_find_sta(pst_hmac_vap, puc_addr, &pst_sta))) {
...
OAL_STATIC oal_uint32 hmac_wds_add_sta_entry(hmac_vap_stru *pst_hmac_vap,
                                             oal_uint8 *puc_addr,
...
OAL_STATIC oal_uint32 hmac_wds_update_sta_entry(hmac_vap_stru *pst_hmac_vap,
...
OAL_STATIC oal_uint32 hmac_wds_del_node_entry(hmac_vap_stru *pst_hmac_vap,
                                              oal_uint8 *puc_addr,
...
OAL_STATIC oal_uint32 hmac_wds_add_node_entry(hmac_vap_stru *pst_hmac_vap,
                                              oal_uint8 *puc_addr,
                                              hmac_wds_node_stru **ppst_wds_node)
{
    oal_uint32 ul_hash_value;
...
oal_uint32 hmac_wds_add_node(hmac_vap_stru *pst_hmac_vap,
...
oal_uint32 hmac_wds_del_node(hmac_vap_stru *pst_hmac_vap,
                             oal_uint8 *puc_node_mac)
{
    oal_uint32 ul_ret = OAL_SUCC;
    hmac_wds_node_stru *pst_wds_node = OAL_PTR_NULL;
...
oal_uint32 hmac_wds_add_sta(hmac_vap_stru *pst_hmac_vap,
                            oal_uint8 *puc_node_mac,
...
oal_uint32 hmac_wds_del_sta(hmac_vap_stru *pst_hmac_vap,
                            oal_uint8 *puc_addr)
{
    hmac_wds_stas_stru *pst_sta = OAL_PTR_NULL;
    hmac_wds_node_stru *pst_node = OAL_PTR_NULL;
...
oal_uint32 hmac_find_valid_user_by_wds_sta(hmac_vap_stru *pst_hmac_vap,
                                           ...
                                           oal_uint16 *pus_user_idx)
{
    ...
    ul_ret = mac_vap_find_user_by_macaddr_etc(&(pst_hmac_vap->st_vap_base_info),
                                              pst_wds_sta->pst_related_node->auc_mac, pus_user_idx);
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_find_valid_user_by_wds_sta::mac_vap_find_user_by_macaddr_etc failed. mac[%02x:%02x:%02x:%02x]}",
            puc_sta_mac_addr[2], puc_sta_mac_addr[3], puc_sta_mac_addr[4], puc_sta_mac_addr[5]);
    }
    oal_rw_lock_write_unlock(&pst_hmac_vap->st_wds_table.st_lock);
...
oal_uint32 hmac_wds_update_table(hmac_vap_stru *pst_hmac_vap,
                                 oal_uint8 *puc_node_mac,
                                 oal_uint8 *puc_sta_mac,
...
oal_uint32 hmac_wds_node_ergodic(hmac_vap_stru *pst_hmac_vap,
                                 oal_uint8 *src_addr,
                                 p_hmac_wds_node_func pst_hmac_wds_node,
                                 oal_void *pst_arg)
{
    oal_uint32 find_sta_ret;
    hmac_wds_stas_stru *pst_wds_sta = OAL_PTR_NULL;
...
OAL_STATIC oal_uint32 hmac_wds_find_neigh(hmac_vap_stru *pst_hmac_vap,
                                          oal_uint8 *puc_addr,
...
oal_uint32 hmac_wds_update_neigh(hmac_vap_stru *pst_hmac_vap,
                                 oal_uint8 *puc_mac)
{
    oal_uint32 ul_hash_value;
    hmac_wds_neigh_stru *pst_neigh = OAL_PTR_NULL;
...
oal_uint32 hmac_wds_neigh_not_expired(hmac_vap_stru *pst_hmac_vap,
                                      oal_uint8 *puc_mac)
{
    oal_uint32 ul_ret;
    hmac_wds_neigh_stru *pst_neigh = OAL_PTR_NULL;
...
oal_uint32 hmac_wds_vap_show_all(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_param)
{
    ...
    if (pc_print_buff == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_CFG,
                       "{hmac_wds_vap_show_all::pc_print_buff null,string_len=%d.}",
    ...
    for (uc_hash_value = 0; uc_hash_value < WDS_HASH_NUM; uc_hash_value++) {
        OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_hmac_vap->st_wds_table.st_wds_stas[uc_hash_value]))
        {
            ...
            if (OAL_ANY_NULL_PTR2(pst_sta, pst_node)) {
                OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                    "{hmac_wds_vap_show_all::pst_sta|pst_node is null.sta idx %d, pst_sta: %x, pst_node: %x}",
                    uc_hash_value, (uintptr_t)(oal_uint32 *)pst_sta, (uintptr_t)(oal_uint32 *)pst_node);
                break;
            }
    ...
    for (uc_hash_value = 0; uc_hash_value < WDS_HASH_NUM; uc_hash_value++) {
        OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_hmac_vap->st_wds_table.st_peer_node[uc_hash_value]))
        {
            ...
            if (pst_node == OAL_PTR_NULL) {
                OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                                 "{hmac_wds_vap_show_all::pst_node is null.sta idx %d}", uc_hash_value);
                break;
            }
    ...
    for (uc_hash_value = 0; uc_hash_value < WDS_HASH_NUM; uc_hash_value++) {
        OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_hmac_vap->st_wds_table.st_neigh[uc_hash_value]))
        {
            ...
            if (pst_br_sa == OAL_PTR_NULL) {
                OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                                 "{hmac_wds_vap_show_all::pst_br_sa is null.sta idx %d}", uc_hash_value);
                break;
            }
...
oal_uint32 hmac_vap_set_wds_user(hmac_vap_stru *pst_hmac_vap,
                                 oal_uint8 *pst_addr)
{
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_ERROR_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                       "{hmac_vap_set_wds_user::mac_vap_get_user_by_addr_etc failed. mac[XX%02x:%02x:%02x%02x]}",
                                                   ...
                                                   oal_uint8 *pst_addr)
{
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_ERROR_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_vmsta_get_user_a4_support::mac_vap_get_user_by_addr_etc failed. mac[XX%02x:%02x:%02x%02x]}",
            pst_addr[2], pst_addr[3], pst_addr[4], pst_addr[5]);
        return OAL_FALSE;
    }
...
OAL_STATIC oal_uint32 hmac_wds_tx_event(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
{
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX,
                         "{hmac_wds_tx_event::frw_event_dispatch_event_etc failed[%d].}", ul_ret);
        OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_abnormal_msdu_dropped, 1);
    }
