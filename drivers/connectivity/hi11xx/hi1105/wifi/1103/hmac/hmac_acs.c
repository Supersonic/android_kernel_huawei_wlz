...
OAL_STATIC oal_void hmac_acs_get_simple_bss_stats_info(
    mac_scan_bss_stats_simple_stru *pst_simple_bss_stats, hmac_scan_record_stru *pst_scan_record)
{
...
OAL_STATIC oal_void hmac_acs_get_simple_channel_stats_info(
    mac_scan_chan_stats_simple_stru *pst_simple_channel_stats, hmac_scan_record_stru *pst_scan_record)
{
    oal_uint32 ul_idx = 0;
    ...
    if (!pst_mac_dev) {
        OAM_WARNING_LOG1(0, OAM_SF_ACS,
                         "{hmac_acs_get_simple_channel_stats_info:pst_mac_dev is null, device id=%d}",
    ...
    for (ul_idx = 0; ul_idx < ul_chan_numbers; ul_idx++) {
        pst_simple_channel_stats[ul_idx].us_total_stats_time_ms =
            pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us / 1000;
        pst_simple_channel_stats[ul_idx].uc_bandwidth_mode =
            pst_scan_record->ast_chan_results[ul_idx].uc_bandwidth_mode;
        pst_simple_channel_stats[ul_idx].uc_radar_detected =
            pst_scan_record->ast_chan_results[ul_idx].uc_radar_detected;
        pst_simple_channel_stats[ul_idx].uc_radar_bw =
            pst_scan_record->ast_chan_results[ul_idx].uc_radar_bw;
        pst_simple_channel_stats[ul_idx].uc_radar_type =
            pst_scan_record->ast_chan_results[ul_idx].uc_radar_type;
        pst_simple_channel_stats[ul_idx].uc_radar_freq_offset =
            pst_scan_record->ast_chan_results[ul_idx].uc_radar_freq_offset;
        pst_simple_channel_stats[ul_idx].uc_channel_number =
            pst_scan_record->ast_chan_results[ul_idx].uc_channel_number;

#ifdef _PRE_WLAN_FEATURE_DFS
        if (pst_mac_dev && pst_simple_channel_stats[ul_idx].uc_channel_number > MAX_CHANNEL_NUM_FREQ_2G) {
            if (OAL_SUCC == mac_get_channel_idx_from_num_etc(WLAN_BAND_5G,
                pst_simple_channel_stats[ul_idx].uc_channel_number, &uc_chan_idx)) {
                pst_simple_channel_stats[ul_idx].uc_dfs_check_needed =
                    (MAC_CHAN_DFS_REQUIRED == pst_mac_dev->st_ap_channel_list[uc_chan_idx].en_ch_status) ?
                    OAL_TRUE : OAL_FALSE;
            }
        }
#endif
        pst_simple_channel_stats[ul_idx].s_free_power_20M =
            (oal_int8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].s_free_power_stats_20M,
                              pst_scan_record->ast_chan_results[ul_idx].uc_free_power_cnt);
        pst_simple_channel_stats[ul_idx].s_free_power_40M =
            (oal_int8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].s_free_power_stats_40M,
                              pst_scan_record->ast_chan_results[ul_idx].uc_free_power_cnt);
        pst_simple_channel_stats[ul_idx].s_free_power_80M =
            (oal_int8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].s_free_power_stats_80M,
                              pst_scan_record->ast_chan_results[ul_idx].uc_free_power_cnt);

        pst_simple_channel_stats[ul_idx].uc_free_time_20M_rate =
            (oal_uint8)ACS_DIV(ACS_MUL255(pst_scan_record->ast_chan_results[ul_idx].ul_total_free_time_20M_us),
                               pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
        pst_simple_channel_stats[ul_idx].uc_free_time_40M_rate =
            (oal_uint8)ACS_DIV(ACS_MUL255(pst_scan_record->ast_chan_results[ul_idx].ul_total_free_time_40M_us),
                               pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
        pst_simple_channel_stats[ul_idx].uc_free_time_80M_rate =
            (oal_uint8)ACS_DIV(ACS_MUL255(pst_scan_record->ast_chan_results[ul_idx].ul_total_free_time_80M_us),
                               pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);

        pst_simple_channel_stats[ul_idx].uc_total_send_time_rate =
            (oal_uint8)ACS_DIV(ACS_MUL255(pst_scan_record->ast_chan_results[ul_idx].ul_total_send_time_us),
                               pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
        pst_simple_channel_stats[ul_idx].uc_total_recv_time_rate =
            (oal_uint8)ACS_DIV(ACS_MUL255(pst_scan_record->ast_chan_results[ul_idx].ul_total_recv_time_us),
                               pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
    }
...
oal_bool_enum_uint8 hmac_acs_try_switch_channel(hmac_device_stru *pst_hmac_device)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        /* 只有running ap的信道信息可以作为判断是否切换信道的依据 */
...
oal_uint32 hmac_acs_got_init_rank(hmac_device_stru *pst_hmac_device,
                                  mac_vap_stru *pst_mac_vap, mac_acs_cmd_stru *pst_cmd)
{
    ...
    if (OAL_SUCC != mac_get_channel_idx_from_num_etc(pst_ch->en_band,
        ...
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ACS,
                         "{hmac_acs_got_init_rank::invalid 2G channel band=%d bw=%d ch=%d",
    ...
    if (OAL_SUCC != mac_get_channel_idx_from_num_etc(pst_ch->en_band,
                                                     ...
                                                     &pst_ch->uc_chan_idx)) {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ACS,
                         "{hmac_acs_got_init_rank::invalid 5G channel band=%d bw=%d ch=%d",
                         pst_ch->en_band, pst_ch->en_bandwidth, pst_ch->uc_chan_number);
        memset_s(pst_ch, OAL_SIZEOF(mac_channel_stru), 0, OAL_SIZEOF(mac_channel_stru));
    }
    ...
    if (pst_hmac_device->en_init_scan == OAL_TRUE) {
        ...
        if (mac_vap_get_dfs_enable(pst_mac_vap)) {
            if (OAL_SUCC != hmac_dfs_init_scan_hook_etc(&pst_hmac_device->st_scan_mgmt.st_scan_record_mgmt,
                                                        pst_hmac_device)) {
...
oal_uint32 hmac_acs_rescan_handler(void *p_arg)
{
    ...
    if (pst_dev == OAL_PTR_NULL ||
        pst_dev->pst_device_base_info == OAL_PTR_NULL ||
        MAC_ACS_SW_NONE == mac_get_acs_switch(pst_dev->pst_device_base_info) ||
        pst_dev->en_init_scan) {
        return OAL_SUCC;
    }
...
oal_void hmac_acs_register_rescan_timer(oal_uint32 ul_dev_id)
{
...
oal_uint32 hmac_acs_process_rescan_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    return hmac_acs_rescan_handler((oal_void *)(oal_uint32) pst_req_param->uc_device_id);
}
