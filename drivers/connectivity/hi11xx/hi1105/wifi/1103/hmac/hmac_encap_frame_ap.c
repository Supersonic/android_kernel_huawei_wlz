...
oal_void hmac_set_supported_rates_ie_asoc_rsp(mac_user_stru *pst_mac_user,
...
oal_uint32 hmac_mgmt_encap_asoc_rsp_ap_etc(mac_vap_stru *pst_mac_ap,
                                           ...
                                           oal_uint16 us_type)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_mac_ap, puc_sta_addr, puc_asoc_rsp)) {
        OAM_ERROR_LOG3(0, OAM_SF_ASSOC,
                       "{hmac_mgmt_encap_asoc_rsp_ap_etc::pst_mac_ap[0x%x], puc_data[0x%x], puc_asoc_req[0x%x].}",
                       (uintptr_t)pst_mac_ap, (uintptr_t)puc_sta_addr, (uintptr_t)puc_asoc_rsp);
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_ap->uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_mgmt_encap_asoc_rsp_ap_etc::pst_mac_device is a null ptr.[0x%x]}",
    ...
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_ASSOC,
                         "{hmac_mgmt_encap_asoc_rsp_ap_etc::pst_hmac_user(idx=%d) is a null ptr.}", us_assoc_id);
        return us_asoc_rsp_len;
    }
    ...
    mac_set_he_ie_in_assoc_rsp((oal_void *)pst_mac_ap, us_assoc_id, puc_asoc_rsp, &uc_ie_len);
    puc_asoc_rsp += uc_ie_len;
#endif
                                                       ...
                                                       oal_uint16 *pus_user_index)
{
    ...
    if ((pst_hmac_vap == OAL_PTR_NULL) || (uc_mac_len != ETHER_ADDR_LEN)) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                       "{hmac_encap_auth_rsp_get_user_idx_etc::mac_res_get_hmac_vap failed!}");
        return OAL_FAIL;
    }
    ...
    *puc_auth_resend = OAL_FALSE;
    ...
    if (ul_ret == OAL_SUCC) {
        ...
        if (pst_hmac_user == OAL_PTR_NULL) {
            OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                           "{hmac_encap_auth_rsp_get_user_idx_etc::mac_res_get_hmac_user_etc return null}");
            return OAL_FAIL;
        }
    ...
    if (OAL_SUCC == mac_chip_find_user_by_macaddr(pst_hmac_vap->st_vap_base_info.uc_chip_id,
        ...
        if (pst_user != OAL_PTR_NULL) {
            ...
            if (pst_hmac_other_vap != OAL_PTR_NULL && pst_hmac_other_vap != pst_hmac_vap) {
                /* 抛事件上报内核，已经删除某个STA */
                hmac_mgmt_send_disassoc_frame_etc(&(pst_hmac_other_vap->st_vap_base_info),
    ...
    if (!uc_is_seq1) {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                         "{hmac_encap_auth_rsp_get_user_idx_etc::user should have been added at seq1!}");
        return OAL_FAIL;
    }
...
hmac_ap_auth_process_code_enum_uint8 hmac_encap_auth_rsp_seq1_etc(mac_vap_stru *pst_mac_vap,
...
hmac_ap_auth_process_code_enum_uint8 hmac_encap_auth_rsp_seq3_etc(mac_vap_stru *pst_mac_vap,
                                                                  hmac_auth_rsp_param_stru *pst_auth_rsp_param,
                                                                  oal_uint8 *puc_code,
                                                                  mac_user_asoc_state_enum_uint8 *pst_usr_ass_stat)
{
...
oal_void hmac_tid_clear_etc(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    for (uc_loop = 0; uc_loop < WLAN_TID_MAX_NUM; uc_loop++) {
        ...
        if (pst_tid->pst_ba_rx_info != OAL_PTR_NULL) {
            hmac_ba_reset_rx_handle_etc(pst_device, pst_hmac_user, uc_loop, OAL_TRUE);
        }
...
oal_uint16 hmac_encap_auth_rsp_etc(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_auth_rsp,
                                   oal_netbuf_stru *pst_auth_req, oal_uint8 *puc_chtxt, oal_uint8 uc_chtxt_len)
{
    ...
    if (OAL_ANY_NULL_PTR4(pst_mac_vap, pst_auth_rsp, pst_auth_req, puc_chtxt)) {
        OAM_ERROR_LOG4(0, OAM_SF_AUTH,
                       "{hmac_encap_auth_rsp_etc::pst_mac_vap[0x%x], p_data[0x%x], p_auth_req[0x%x], p_chtxt[0x%x]}",
    ...
    if (us_auth_seq > HMAC_AP_AUTH_SEQ3_WEP_COMPLETE) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                         "{hmac_encap_auth_rsp_etc::auth recieve invalid seq, auth seq [%d]}",
                         us_auth_seq);
        return 0;
    }
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CONN,
                     "{hmac_encap_auth_rsp_etc:: AUTH_RSP tx : user mac:%02X:XX:XX:%02X:%02X:%02X}",
    ...
    if (mac_addr_is_zero_etc(auc_addr2)) {
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                         "{hmac_encap_auth_rsp_etc::user mac:%02X:XX:XX:%02X:%02X:%02X is all 0 and invaild!}",
                         auc_addr2[0], auc_addr2[3], auc_addr2[4], auc_addr2[5]);
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                         "{hmac_encap_auth_rsp_etc::hmac_ap_get_user_idx fail[%d]! (1002:exceed config spec)}", ul_ret);
    ...
    if (pst_hmac_user_sta == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                       "{hmac_encap_auth_rsp_etc::pst_hmac_user_sta[%d] is NULL}",
                       us_user_index);
        puc_frame[4] = MAC_UNSPEC_FAIL;
#ifdef _PRE_WLAN_1103_CHR
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI,
    ...
    if (ul_alg_suppt != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                         "{hmac_encap_auth_rsp_etc::auth type[%d] not support!}",
                         us_auth_type);
    ...
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                     "{hmac_encap_auth_rsp_etc::ul_auth_proc_rst:%d}", ul_auth_proc_rst);
        ...
        case HMAC_AP_AUTH_SEQ3_WEP_COMPLETE: {
            ...
            if (hmac_mgmt_is_challenge_txt_equal(puc_chtxt,
                ...
                OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                                 "{hmac_encap_auth_rsp_etc::challenage txt not equal.}");
    ...
    ul_ret = hmac_config_user_asoc_state_syn_etc(&(pst_hmac_vap->st_vap_base_info),
