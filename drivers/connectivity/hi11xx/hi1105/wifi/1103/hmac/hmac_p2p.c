    ...
    */
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_CFG,
                         "{hmac_p2p_check_can_enter_state_etc::mac_res_get_dev_etc fail.device_id = %u}",
    ...
    for (uc_vap_idx = 0; uc_vap_idx < uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_other_vap == OAL_PTR_NULL) {
            OAM_WARNING_LOG1(0, OAM_SF_CFG,
                             "{hmac_p2p_check_can_enter_state_etc::hmac_res_get_mac_vap fail.vap_idx = %u}",
                                         ...
                                         wlan_channel_bandwidth_enum_uint8 *pen_home_channel_bandwidth)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_CFG,
                         "{hmac_p2p_get_home_channel_etc::mac_res_get_dev_etc fail.device_id = %u}",
                         pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    *pul_home_channel = 0;
    *pen_home_channel_bandwidth = WLAN_BAND_WIDTH_20M;
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_mac_vap == OAL_PTR_NULL) {
            OAM_WARNING_LOG1(0, OAM_SF_CFG,
                             "{hmac_p2p_get_home_channel_etc::hmac_res_get_mac_vap fail.vap_idx = %u}",
                             uc_vap_idx);
            continue;
        }
        ...
        if (pst_mac_vap->en_vap_state == MAC_VAP_STATE_UP) {
            ...
            if (ul_last_home_channel == 0) {
                ...
                OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                    "{hmac_p2p_get_home_channel_etc::failed.home_channel:%d,last_home_chan:%d,home bw:%d,last bw:%d.}",
...
oal_uint32 hmac_check_p2p_vap_num_etc(mac_device_stru *pst_mac_device, wlan_p2p_mode_enum_uint8 en_p2p_mode)
{
    if (WLAN_P2P_DEV_MODE == en_p2p_mode) {
        ...
        if (pst_mac_device->uc_sta_num >= WLAN_SERVICE_STA_MAX_NUM_PER_DEVICE) {
            /* 已有AP创建 */
            OAM_WARNING_LOG1(0, OAM_SF_CFG,
                             "{hmac_check_p2p_vap_num_etc::can not create p2p_device any more[uc_sta_num:%d].}",
        ...
        if (pst_mac_device->st_p2p_info.uc_p2p_goclient_num >= WLAN_MAX_SERVICE_P2P_GOCLIENT_NUM) {
            /* 已创建的P2P_DEV个数达到最大值 */
            OAM_WARNING_LOG1(0, OAM_SF_CFG,
                             "{hmac_check_p2p_vap_num_etc::have created 1 p2p_GO/Client.can not createany more[%d].}",
        ...
        if (pst_mac_device->st_p2p_info.uc_p2p_device_num != WLAN_MAX_SERVICE_P2P_DEV_NUM) {
            /* 已有AP创建 */
            OAM_WARNING_LOG1(0, OAM_SF_CFG,
                             "{hmac_check_p2p_vap_num_etc::there ia no p2p0 %d,so can not create p2p_CL.}",
                             pst_mac_device->st_p2p_info.uc_p2p_device_num);
            return OAL_ERR_CODE_CONFIG_EXCEED_SPEC;
        }
...
oal_uint32 hmac_add_p2p_cl_vap_etc(mac_vap_stru *pst_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_add_p2p_cl_vap_etc::hmac_config_check_vap_num failed[%d].}", ul_ret);
        return ul_ret;
    }
    ...
    if (OAL_UNLIKELY(pst_hmac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG,
                       "{hmac_add_p2p_cl_vap_etc::mac_res_get_hmac_vap failed.vap_id:[%d].}", uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        /* 异常处理，释放内存 */
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG,
                       "{hmac_add_p2p_cl_vap_etc::hmac_config_alloc_event failed[%d].}", ul_ret);
        return ul_ret;
    }
    ...
    OAM_INFO_LOG3(pst_vap->uc_vap_id, OAM_SF_ANY,
                  "{hmac_add_p2p_cl_vap_etc::func out.vap_mode[%d], p2p_mode[%d}, vap_id[%d]",
...
oal_uint32 hmac_del_p2p_cl_vap_etc(mac_vap_stru *pst_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR2(pst_vap, puc_param))) {
        OAM_ERROR_LOG2(0, OAM_SF_CFG, "{hmac_config_del_vap_etc::param null,pst_vap=%d puc_param=%d.}",
                       (uintptr_t)pst_vap, (uintptr_t)puc_param);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG,
                       "{hmac_config_del_vap_etc::pst_device null.devid[%d]}",
    ...
    if (OAL_UNLIKELY(pst_hmac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG,
                       "{hmac_config_del_vap_etc::mac_res_get_hmac_vap fail.vap_id[%d]}", uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_del_p2p_cl_vap_etc::hmac_config_send_event_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32 hmac_p2p_send_listen_expired_to_host_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_hmac_vap->pst_p2p0_net_device && pst_hmac_vap->pst_p2p0_net_device->ieee80211_ptr) {
        ...
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_P2P,
                         "{hmac_send_mgmt_to_host_etc::vap has deleted.}");
        return OAL_FAIL;
    }
...
oal_uint32 hmac_p2p_send_listen_expired_to_device_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(pst_hmac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_p2p_send_listen_expired_to_device_etc::pst_hmac_device[%d] null!}",
                       pst_hmac_vap->st_vap_base_info.uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_P2P,
            "{hmac_p2p_send_listen_expired_to_device_etc::hmac_config_send_event_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_void hmac_disable_p2p_pm_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    ul_ret = hmac_config_set_p2p_ps_noa_etc(pst_mac_vap,
                                            OAL_SIZEOF(mac_cfg_p2p_noa_param_stru),
                                            (oal_uint8 *)&st_p2p_noa);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_P2P,
    ...
    ul_ret = hmac_config_set_p2p_ps_ops_etc(pst_mac_vap,
                                            OAL_SIZEOF(mac_cfg_p2p_ops_param_stru),
                                            (oal_uint8 *)&st_p2p_ops);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_P2P,
