...
oal_uint32 hmac_check_ap_channel_follow_sta(mac_vap_stru *pst_check_mac_vap,
                                            const mac_channel_stru *pst_set_mac_channel,
                                            oal_uint8 *puc_ap_follow_channel)
{
    mac_device_stru *pst_mac_device = OAL_PTR_NULL;
    ...
    if (!IS_LEGACY_VAP(pst_check_mac_vap)) {
        OAM_WARNING_LOG1(pst_check_mac_vap->uc_vap_id, OAM_SF_2040,
                         "{hmac_check_ap_channel_follow_sta:: check_vap_p2p_mode=%d, not neet to check}",
                         pst_check_mac_vap->en_p2p_mode);
        return OAL_FAIL;
    }
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_check_mac_vap->uc_vap_id, OAM_SF_2040,
                       ...
                       pst_check_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
        ...
        if (IS_STA(pst_check_mac_vap) && IS_AP(pst_index_mac_vap)) { /* AP先启动;STA后启动 */
            if ((pst_set_mac_channel->en_band == pst_index_mac_vap->st_channel.en_band) &&
                ...
                OAM_WARNING_LOG2(pst_index_mac_vap->uc_vap_id, OAM_SF_2040,
                    "{hmac_check_ap_channel_follow_sta::<vap_current_mode=STA vap_index_mode=Ap> SoftAp CSA Operate, Channel from [%d] To [%d]}.\n",
            ...
            if ((pst_set_mac_channel->en_band == pst_index_mac_vap->st_channel.en_band) &&
                ...
                *puc_ap_follow_channel = pst_index_mac_vap->st_channel.uc_chan_number;
                OAM_WARNING_LOG2(pst_index_mac_vap->uc_vap_id, OAM_SF_2040,
                    "{hmac_check_ap_channel_follow_sta::<vap_current_mode=Ap vap_index_mode=Sta> SoftAp change Channel from [%d] To [%d]}.\n",
...
oal_void hmac_dfs_set_channel_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_channel)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                       "{hmac_dfs_set_channel_etc::pst_device(%d) null.}",
                       pst_mac_vap->uc_device_id);
        return;
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                         "{hmac_dfs_set_channel_etc::mac_get_channel_idx_from_num_etc failed[%d].}", ul_ret);
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if ((pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
            ...
            if (uc_channel == pst_vap->st_channel.uc_chan_number) {
                OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                                 "{hmac_dfs_set_channel_etc::same channel num %d, no need to change.}",
                                 uc_channel);
                return;
            }
...
oal_void hmac_chan_multi_switch_to_new_channel_etc(mac_vap_stru *pst_mac_vap,
                                                   ...
                                                   wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040,
                       "{hmac_chan_multi_switch_to_new_channel_etc::pst_device(%d) null.}",
                       pst_mac_vap->uc_device_id);
        return;
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040,
            "{hmac_chan_multi_switch_to_new_channel_etc::mac_get_channel_idx_from_num_etc failed[%d].}", ul_ret);
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_vap == OAL_PTR_NULL) {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040,
                           "{hmac_chan_multi_switch_to_new_channel_etc::pst_ap(%d) null.}",
                           pst_mac_device->auc_vap_id[uc_vap_idx]);
            continue;
        }
                                                   ...
                                                   wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    ...
    if (pst_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                       "{hmac_chan_multi_switch_to_new_channel_etc::pst_device null.}");
        return;
    }
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_ap == OAL_PTR_NULL) {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                           "{hmac_chan_multi_switch_to_new_channel_etc::pst_ap null.}",
...
oal_void hmac_chan_do_sync_etc(mac_vap_stru *pst_mac_vap, dmac_set_chan_stru *pst_set_chan)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                         "{hmac_chan_sync_etc::mac_get_channel_idx_from_num_etc failed[%d].}", ul_ret);
...
oal_void hmac_chan_multi_select_channel_mac_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_channel,
                                                wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    oal_uint8 uc_vap_idx;
    mac_device_stru *pst_device = OAL_PTR_NULL;
    ...
    OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_2040,
                     "{hmac_chan_multi_select_channel_mac_etc::uc_channel=%d,en_bandwidth=%d}",
                     uc_channel, en_bandwidth);

    pst_device = mac_res_get_dev_etc(pst_mac_vap->uc_device_id);
    if (pst_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040,
                       "{hmac_chan_multi_select_channel_mac_etc::pst_device null,device_id=%d.}",
                       pst_mac_vap->uc_device_id);
        return;
    }
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_mac_vap->st_mac_user_list_head))
    {
        ...
        if (OAL_UNLIKELY(pst_mac_user == OAL_PTR_NULL)) {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_2040,
                             "{hmac_chan_update_40M_intol_user_etc::pst_user null pointer.}");
            continue;
        } else {
...
oal_void hmac_chan_reval_bandwidth_sta_etc(mac_vap_stru *pst_mac_vap, oal_uint32 ul_change)
{
    ...
    if (MAC_BW_CHANGE & ul_change) {
        hmac_chan_multi_select_channel_mac_etc(pst_mac_vap, pst_mac_vap->st_channel.uc_chan_number,
...
OAL_STATIC oal_void hmac_chan_ctrl_machw_tx(mac_vap_stru *pst_mac_vap, oal_uint8 uc_sub_type)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_chan_ctrl_machw_tx::frw_event_dispatch_event_etc failed[%d].}", ul_ret);
        FRW_EVENT_FREE(pst_event_mem);
        return;
    }
...
OAL_STATIC oal_void hmac_cac_chan_ctrl_machw_tx_event(mac_vap_stru *pst_mac_vap, oal_uint8 uc_cac_machw_en)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_cac_chan_ctrl_machw_tx::frw_event_dispatch_event_etc failed[%d].}", ul_ret);
        FRW_EVENT_FREE(pst_event_mem);
        return;
    }
...
oal_uint32 hmac_chan_start_bss_etc(hmac_vap_stru *pst_hmac_vap, mac_channel_stru *pst_channel,
                                   wlan_protocol_enum_uint8 en_protocol)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
                         "{hmac_chan_start_bss_etc::hmac_sta_sync_vap failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32 hmac_chan_restart_network_after_switch_etc(mac_vap_stru *pst_mac_vap)
{
    ...
    if (OAL_UNLIKELY(pst_event_mem == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                       "{hmac_chan_restart_network_after_switch_etc::pst_event_mem null.}");
    ...
    if (IS_STA(pst_mac_vap)) {
        ...
        if (ul_ret == OAL_SUCC) {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                "{hmac_chan_switch_to_new_chan_complete_etc::ap channel change from %d to %d}",
...
oal_void hmac_40M_intol_sync_data(mac_vap_stru *pst_mac_vap,
