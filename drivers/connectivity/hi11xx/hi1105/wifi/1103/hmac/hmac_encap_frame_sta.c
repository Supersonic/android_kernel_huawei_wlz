...
oal_uint32 hmac_mgmt_encap_asoc_req_sta_etc(hmac_vap_stru *pst_hmac_sta,
                                            ...
                                            oal_uint8 *puc_curr_bssid)
{
    ...
    if (OAL_ANY_NULL_PTR2(pst_hmac_sta, puc_req_frame)) {
        OAM_ERROR_LOG2(0, OAM_SF_ASSOC,
                       "{hmac_mgmt_encap_asoc_req_sta_etc::null param, pst_hmac_sta=%x puc_req_frame=%x.}",
    ...
    if (OAL_ANY_NULL_PTR2(pst_mac_device, pst_scaned_bss)) {
        OAM_ERROR_LOG0(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_mgmt_encap_asoc_req_sta_etc::pst_mac_device or pst_scaned_bss null.}");
        return us_asoc_rsq_len;
    }
...
#endif
#ifdef _PRE_WLAN_FEATURE_M2S
    if ((g_en_mimo_blacklist_etc == OAL_TRUE)
        && (0 != uc_ie_len)) {
    ...
    if ((pst_scaned_bss != OAL_PTR_NULL) && (pst_scaned_bss->st_bss_dscr_info.en_support_1024qam == OAL_TRUE)) {
    ...
    if ((pst_scaned_bss != OAL_PTR_NULL) && (pst_scaned_bss->st_bss_dscr_info.en_vendor_vht_capable == OAL_TRUE)) {
        mac_set_vendor_vht_ie(pst_mac_vap, puc_req_frame, &uc_ie_len);
        puc_req_frame += uc_ie_len;
    }
    if ((pst_scaned_bss != OAL_PTR_NULL) && (pst_scaned_bss->st_bss_dscr_info.en_vendor_novht_capable == OAL_TRUE)) {
    ...
    if ((pst_scaned_bss != OAL_PTR_NULL) && (pst_scaned_bss->st_bss_dscr_info.en_nb_capable == OAL_TRUE) &&
        (pst_mac_vap->st_nb.en_open == OAL_TRUE) &&
        (pst_mac_vap->st_cap_flag.bit_nb == OAL_TRUE)) {
