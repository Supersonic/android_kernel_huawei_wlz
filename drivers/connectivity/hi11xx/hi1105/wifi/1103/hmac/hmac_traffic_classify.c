...
OAL_STATIC oal_uint32 hmac_tx_add_cfm_traffic(hmac_user_stru *pst_hmac_user,
                                              oal_uint8 uc_tid,
...
oal_void hmac_tx_traffic_classify_etc(mac_tx_ctl_stru *pst_tx_ctl,
    ...
    if ((pst_hmac_user->us_clear_judge_count < MAX_CLEAR_JUDGE_TH) &&
        ...
        if (uc_rtp_ver == RTP_VERSION) {
            if (uc_rtp_payload_type == RTP_PT_VO_G729) {
                OAM_WARNING_LOG2(0, 0, "hmac_tx_traffic_classify_etc:rtp seq=0x%x tid=%d to WLAN_TIDNO_VOICE",
                ...
                *puc_tid = WLAN_TIDNO_VOICE;
                ...
                OAM_WARNING_LOG2(0, 0, "hmac_tx_traffic_classify_etc:rtp seq=0x%x tid=%d to WLAN_TIDNO_VIDEO",
                                 pst_rtp_hdr->us_RTP_idx, *puc_tid);
                *puc_tid = WLAN_TIDNO_VIDEO;
            }
        }
