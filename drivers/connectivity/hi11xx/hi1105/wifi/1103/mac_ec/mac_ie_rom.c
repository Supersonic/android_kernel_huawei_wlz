...
mac_ie_cb g_st_mac_ie_rom_cb = {
    ...
    .ht_opern_cb = mac_proc_ht_opern_ie_cb
};
#else
mac_ie_cb g_st_mac_ie_rom_cb = {OAL_PTR_NULL,
    OAL_PTR_NULL,
    mac_proc_ht_opern_ie_cb
};
#endif
/* 3 函数实现 */

oal_uint8 mac_ie_get_chan_num_etc(oal_uint8 *puc_frame_body, oal_uint16 us_frame_len,
                                  oal_uint16 us_offset, oal_uint8 uc_curr_chan)
{
...
oal_uint32 mac_set_second_channel_offset_ie_etc(wlan_channel_bandwidth_enum_uint8 en_bw,
                                                oal_uint8 *pauc_buffer,
                                                oal_uint8 *puc_output_len)
{
    if (OAL_ANY_NULL_PTR2(pauc_buffer, puc_output_len)) {
...
oal_uint32 mac_set_11ac_wideband_ie_etc(oal_uint8 uc_channel,
...
oal_uint32 mac_ie_proc_chwidth_field_etc(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, oal_uint8 uc_chwidth)
{
    ...
    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR2(pst_mac_vap, pst_mac_user))) {
        OAM_ERROR_LOG2(0, OAM_SF_2040,
                       "{mac_ie_proc_opmode_field_etc::pst_mac_user = [%x],   \
...
oal_uint32 mac_proc_ht_opern_ie_etc(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload, mac_user_stru *pst_mac_user)
{
    ...
    if (puc_payload[1] < 6) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                         "{mac_proc_ht_opern_ie_etc::invalid ht opern ie len[%d].}", puc_payload[1]);
        return ul_change;
    }
    ...
    **************************************************************************/
    ...
    if ((uc_secondary_chan_offset_old != st_ht_hdl.bit_secondary_chan_offset) ||
        ...
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                         "mac_proc_ht_opern_ie_etc:usr_bw is updated second_chan_offset from [%d] to [%d],   \
                          chan_with from [%d] to [%d]",
                         uc_secondary_chan_offset_old, st_ht_hdl.bit_secondary_chan_offset,
                         uc_sta_chan_width_old, st_ht_hdl.bit_sta_chan_width);
    }

    if (g_st_mac_ie_rom_cb.ht_opern_cb != OAL_PTR_NULL) {
...
oal_uint32 mac_ie_proc_obss_scan_ie_etc(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload)
{
    ...
    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR2(pst_mac_vap, puc_payload))) {
...
oal_uint32 mac_ie_proc_vht_opern_ie_etc(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload, mac_user_stru *pst_mac_user)
{
    ...
    if (st_vht_hdl.en_channel_width > WLAN_MIB_VHT_OP_WIDTH_80PLUS80) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
                         "{mac_ie_proc_vht_opern_ie_etc::invalid channel width[%d], use 20M chn width.}",
                         st_vht_hdl.en_channel_width);
    ...
    if ((en_channel_width_old != st_vht_hdl.en_channel_width) ||
        ...
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                         "mac_ie_proc_vht_opern_ie_etc:usr_bw is updated chanl_with from [%d] to [%d], \
                         chanl_center_freq_seg0 from [%d] to [%d]",
                         en_channel_width_old, st_vht_hdl.en_channel_width,
                         uc_channel_center_freq_seg0_old, st_vht_hdl.uc_channel_center_freq_seg0);
    }

    if (g_st_mac_ie_rom_cb.vht_opern_cb != OAL_PTR_NULL) {
...
oal_uint32 mac_ie_get_wpa_cipher(oal_uint8 *puc_ie, mac_crypto_settings_stru *pst_crypto)
{
    ...
    memset_s(pst_crypto, OAL_SIZEOF(mac_crypto_settings_stru), 0, OAL_SIZEOF(mac_crypto_settings_stru));
    /**************************************************************************/
    /*                  WPA Element Format                                    */
    /* ---------------------------------------------------------------------- */
    ...
    for (uc_suite_idx = 0; uc_suite_idx < us_suites_count; uc_suite_idx++) {
        if (MAC_IE_REAMIN_LEN_IS_ENOUGH(puc_src_ie, puc_ie, uc_ie_len, 4)) {
            ...
            OAM_WARNING_LOG2(0, OAM_SF_ANY,
                "mac_ie_get_wpa_cipher: Pairwise Cipher, invalid ie len[%d], us_suites_count[%d]",
...
oal_uint32 mac_ie_get_rsn_cipher(oal_uint8 *puc_ie, mac_crypto_settings_stru *pst_crypto)
{
    ...
    if (pst_crypto->ul_wpa_versions != 1) {
        return OAL_FAIL;
    }
    ...
    us_suites_count = OAL_MAKE_WORD16(puc_ie[0], puc_ie[1]);
    puc_ie += 2;
    if ((us_suites_count == 0) || (us_suites_count > MAC_RSN_CIPHER_COUNT_LEN)) {
        return OAL_FAIL;
    }
    ...
    us_suites_count = OAL_MAKE_WORD16(puc_ie[0], puc_ie[1]);
    puc_ie += 2;
    if ((us_suites_count == 0) || (us_suites_count > OAL_NL80211_MAX_NR_AKM_SUITES)) {
        return OAL_FAIL;
    }
    ...
    if (((*(oal_uint32 *)puc_ie) & 0xFFFFFF) == MAC_RSN_CHIPER_OUI(0)) {
...
oal_module_symbol(mac_ie_proc_vht_opern_ie_etc); /*lint +e19*/
