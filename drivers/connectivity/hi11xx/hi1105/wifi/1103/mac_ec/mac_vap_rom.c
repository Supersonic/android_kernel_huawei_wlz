...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)/* host */
mac_vap_cb  g_st_mac_vap_rom_cb = {
    ...
    .init_rates_by_prot_cb              = mac_vap_init_rates_by_protocol_etc_cb,              // init_rates_by_prot_cb
...
oal_uint32 mac_mib_set_ssid_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "mac_mib_set_ssid_etc::memcpy fail!");
        return OAL_FAIL;
    }
...
oal_uint32 mac_mib_get_ssid_etc(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_len, oal_uint8 *puc_param)
{
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "mac_mib_get_ssid_etc::memcpy fail!");
        return OAL_FAIL;
    }
...
oal_uint32 mac_vap_add_assoc_user_etc(mac_vap_stru *pst_vap, oal_uint16 us_user_idx)
{
    ...
    if (OAL_UNLIKELY(pst_user == OAL_PTR_NULL)) {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ASSOC,
                         "{mac_vap_add_assoc_user_etc::pst_user[%d] null.}", us_user_idx);
    ...
    if (OAL_SUCC == mac_vap_user_exist_etc(&(pst_user->st_user_dlist), &(pst_vap->st_mac_user_list_head))) {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ASSOC,
                       "{mac_vap_add_assoc_user_etc::user[%d] already exist.}", us_user_idx);
...
oal_uint32 mac_vap_del_user_etc(mac_vap_stru *pst_vap, oal_uint16 us_user_idx)
{
    ...
    if (OAL_UNLIKELY(pst_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC,
                       "{mac_vap_del_user_etc::pst_vap null,us_user_idx is %d}", us_user_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_user == OAL_PTR_NULL)) {
        ...
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ASSOC,
                         "{mac_vap_del_user_etc::pst_user[%d] null.}", us_user_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (pst_user->us_user_hash_idx >= MAC_VAP_USER_HASH_MAX_VALUE) {
        ...
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ASSOC,
                       "{mac_vap_del_user_etc::hash idx invaild %u}",
    ...
    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_dlist_tmp, pst_hash_head)
    {
        ...
        if (pst_user_temp == OAL_PTR_NULL) {
            OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ASSOC,
                           "{mac_vap_del_user_etc::pst_user_temp null,us_user_idx is %d}", us_user_idx);
    ...
    OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ASSOC,
                     "{mac_vap_del_user_etc::delete user failed,user idx is %d.}", us_user_idx);
    return OAL_FAIL;
}


oal_uint32 mac_vap_find_user_by_macaddr_etc(mac_vap_stru *pst_vap,
            ...
            OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_vap->ast_user_hash[ul_user_hash_value]))
            {
                ...
                if (pst_mac_user == OAL_PTR_NULL) {
                    OAM_ERROR_LOG0(pst_vap->uc_vap_id, OAM_SF_ANY,
                                   "{mac_vap_find_user_by_macaddr_etc::pst_mac_user null.}");
                    continue;
                }
                /*lint -restore */
                /* 相同的MAC地址 */
...
oal_uint32 mac_device_find_user_by_macaddr_etc(oal_uint8 uc_device_id,
                                               const unsigned char *puc_sta_mac_addr,
                                               oal_uint16 *pus_user_idx)
{
...
oal_uint32 mac_chip_find_user_by_macaddr(oal_uint8 uc_chip_id,
                                         const unsigned char *puc_sta_mac_addr,
                                         oal_uint16 *pus_user_idx)
{
    ...
    for (uc_device_idx = 0; uc_device_idx < pst_mac_chip->uc_device_nums; uc_device_idx++) {
        ul_ret = mac_device_find_user_by_macaddr_etc(pst_mac_chip->auc_device_id[uc_device_idx],
...
oal_uint32 mac_board_find_user_by_macaddr(const unsigned char *puc_sta_mac_addr,
                                          oal_uint16 *pus_user_idx)
{
...
{
    if (g_st_mac_vap_rom_cb.init_11ax_mib_cb != OAL_PTR_NULL) {
...
oal_void mac_vap_init_p2p_rates_etc(mac_vap_stru *pst_vap,
                                    ...
                                    mac_data_rate_stru *pst_rates)
{
    ...
    if (pst_mac_dev == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY,
                       "{mac_vap_init_p2p_rates_etc::pst_mac_dev[%d] null.}",
    ...
    l_ret += memcpy_s(&pst_vap->ast_sta_sup_rates_ie[WLAN_BAND_2G],
                      ...
                      OAL_SIZEOF(pst_vap->st_curr_sup_rates));
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "mac_vap_init_p2p_rates_etc::memcpy fail!");
        return;
    }
...
oal_void mac_vap_init_rates_by_protocol_etc(mac_vap_stru *pst_vap,
                                            wlan_protocol_enum_uint8 en_vap_protocol,
                                            mac_data_rate_stru *pst_rates)
{
    mac_device_stru *pst_mac_dev;
...
#ifdef _PRE_WLAN_FEATURE_11AX
    if (pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA && en_vap_protocol == WLAN_HE_MODE) {
        /* 用于STA全信道扫描 5G时 填写支持速率集ie */
        mac_vap_init_legacy_rates(pst_vap, pst_rates);
        l_res = memcpy_s(&pst_vap->ast_sta_sup_rates_ie[WLAN_BAND_5G],
                         OAL_SIZEOF(mac_curr_rateset_stru),
                         &pst_vap->st_curr_sup_rates,
                         OAL_SIZEOF(pst_vap->st_curr_sup_rates));
        ...
        l_res += memcpy_s(&pst_vap->ast_sta_sup_rates_ie[WLAN_BAND_2G],
                          OAL_SIZEOF(mac_curr_rateset_stru),
                          &pst_vap->st_curr_sup_rates,
                          OAL_SIZEOF(pst_vap->st_curr_sup_rates));
        if (l_res != EOK) {
    ...
    if (g_st_mac_vap_rom_cb.init_rates_by_prot_cb != OAL_PTR_NULL) {
...
oal_void mac_vap_init_rates_etc(mac_vap_stru *pst_vap)
{
    ...
    if (pst_mac_dev == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY,
                       "{mac_vap_init_rates_etc::pst_mac_dev[%d] null.}",
                       pst_vap->uc_device_id);
...
oal_void mac_vap_vowifi_init(mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_mac_vap->pst_vowifi_cfg_param == OAL_PTR_NULL) {
        pst_mac_vap->pst_vowifi_cfg_param = OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
...
oal_uint32 mac_vap_save_app_ie_etc(mac_vap_stru *pst_mac_vap, oal_app_ie_stru *pst_app_ie, en_app_ie_type_uint8 en_type)
{
    ...
    if (en_type >= OAL_APP_IE_NUM) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{mac_vap_save_app_ie_etc::invalid en_type[%d].}", en_type);
    ...
    if ((pst_mac_vap->ast_app_ie[en_type].ul_ie_max_len < ul_ie_len) ||
        ...
        if (pst_mac_vap->ast_app_ie[en_type].puc_ie == NULL && pst_mac_vap->ast_app_ie[en_type].ul_ie_max_len != 0) {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                           "{mac_vap_save_app_ie_etc::invalid len[%d].}",
        ...
        if (puc_ie == OAL_PTR_NULL) {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                             "{mac_vap_save_app_ie_etc::LOCAL_MEM_POOL is empty!,len[%d], en_type[%d].}",
    ...
    l_ret = memcpy_s((oal_void *)pst_mac_vap->ast_app_ie[en_type].puc_ie, ul_ie_len,
                     (oal_void *)pst_app_ie->auc_ie, ul_ie_len);
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "mac_vap_save_app_ie_etc::memcpy fail!");
        return OAL_FAIL;
    }

    pst_mac_vap->ast_app_ie[en_type].ul_ie_len = ul_ie_len;
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                     "{mac_vap_save_app_ie_etc::IE:[0x%2x][0x%2x][0x%2x][0x%2x]}",
...
oal_uint32 mac_vap_exit_etc(mac_vap_stru *pst_vap)
{
    ...
    if (OAL_UNLIKELY(pst_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY,
                       "{mac_vap_exit_etc::pst_device[%d] null.}",
                       pst_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_void mac_init_mib_etc(mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_mac_dev == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                       "{mac_init_mib_etc::pst_mac_dev[%d] null.}",
                       pst_mac_vap->uc_device_id);
        return;
    }
    ...
    if (g_st_mac_vap_rom_cb.mac_init_mib_cb != OAL_PTR_NULL) {
...
oal_uint32 mac_vap_config_vht_ht_mib_by_protocol_etc(mac_vap_stru *pst_mac_vap)
{
    if (pst_mac_vap->pst_mib_info == OAL_PTR_NULL) {
        OAM_ERROR_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                       "{mac_vap_config_vht_ht_mib_by_protocol_etc::pst_mib_info null,  \
...
#ifdef _PRE_WLAN_FEATURE_11AX
    else if (pst_mac_vap->en_protocol == WLAN_HE_MODE) {
        mac_mib_set_HighThroughputOptionImplemented(pst_mac_vap, OAL_TRUE);
        mac_mib_set_VHTOptionImplemented(pst_mac_vap, OAL_TRUE);
...
oal_void mac_vap_init_rx_nss_by_protocol_etc(mac_vap_stru *pst_mac_vap)
{
    ...
    if (OAL_UNLIKELY(pst_mac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                       "{mac_vap_init_rx_nss_by_protocol_etc::pst_mac_device[%d] null.}",
                       pst_mac_vap->uc_device_id);
        return;
    }
...
oal_uint32 mac_vap_init_by_protocol_etc(mac_vap_stru *pst_mac_vap, wlan_protocol_enum_uint8 en_protocol)
{
    ...
    /* 根据协议模式更新mib值 */
    if (mac_vap_config_vht_ht_mib_by_protocol_etc(pst_mac_vap) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
oal_void mac_vap_change_mib_by_bandwidth_etc(mac_vap_stru *pst_mac_vap,
...
oal_uint32 mac_vap_set_bssid_etc(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_bssid)
{
    if (EOK != memcpy_s(pst_mac_vap->auc_bssid, WLAN_MAC_ADDR_LEN, puc_bssid, WLAN_MAC_ADDR_LEN)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "mac_vap_set_bssid_etc::memcpy fail!");
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
oal_void mac_vap_state_change_etc(mac_vap_stru *pst_mac_vap, mac_vap_state_enum_uint8 en_vap_state)
{
    ...
    OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                     "{mac_vap_state_change_etc:from[%d]to[%d]}",
...
oal_uint32 mac_vap_set_current_channel_etc(mac_vap_stru *pst_vap,
                                           wlan_channel_band_enum_uint8 en_band,
...
oal_bool_enum_uint8 mac_vap_check_bss_cap_info_phy_ap_etc(oal_uint16 us_cap_info, mac_vap_stru *pst_mac_vap)
{
    ...
    if ((OAL_FALSE == mac_mib_get_PBCCOptionImplemented(pst_mac_vap)) &&
        ...
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                         "{mac_vap_check_bss_cap_info_phy_ap_etc::PBCC is different.}");
    }
...
wlan_prot_mode_enum_uint8 mac_vap_get_user_protection_mode_etc(mac_vap_stru *pst_mac_vap_sta,
...
oal_bool_enum mac_protection_lsigtxop_check_etc(mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        pst_mac_user =
            (mac_user_stru *)mac_res_get_mac_user_etc(pst_mac_vap->us_assoc_vap_id); /* user保存的是AP的信息 */
        if ((pst_mac_user == OAL_PTR_NULL) ||
...
oal_void mac_protection_set_rts_tx_param_etc(mac_vap_stru *pst_mac_vap,
                                             oal_switch_enum_uint8 en_flag,
                                             wlan_prot_mode_enum_uint8 en_prot_mode,
                                             mac_cfg_rts_tx_param_stru *pst_rts_tx_param)
{
...
oal_uint32 mac_vap_add_wep_key(mac_vap_stru *pst_mac_vap, oal_uint8 us_key_idx,
                               oal_uint8 uc_key_len, oal_uint8 *puc_key)
{
    ...
    if (ul_ret != OAL_SUCC) {
        ...
        if (puc_mac_addr != OAL_PTR_NULL) {
            OAM_WARNING_LOG3(0, OAM_SF_ANY,
                             "{mac_vap_get_user_by_addr_etc::mac[%x:XX:XX:XX:%x:%x] cant be found!}",
...
oal_uint32 mac_vap_add_key_etc(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user,
                               oal_uint8 uc_key_id, mac_key_params_stru *pst_key)
{
    oal_uint32 ul_ret;
...
oal_uint8 mac_vap_get_default_key_id_etc(mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_multi_user == OAL_PTR_NULL) {
        ...
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                         "{mac_vap_get_default_key_id_etc::multi_user[%d] NULL}",
                         pst_mac_vap->us_multi_user_idx);
        return 0;
    }
    ...
    if ((pst_multi_user->st_key_info.en_cipher_type != WLAN_80211_CIPHER_SUITE_WEP_40) &&
        ...
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                       "{mac_vap_get_default_key_id_etc::unexpectd cipher_type[%d]}",
    ...
    if (uc_default_key_id >= WLAN_NUM_TK) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                       "{mac_vap_get_default_key_id_etc::unexpectd keyid[%d]}",
                       uc_default_key_id);
        return 0;
    }
...
oal_uint32 mac_vap_set_vowifi_param_etc(mac_vap_stru *pst_mac_vap,
    ...
    switch (en_vowifi_cfg_cmd) {
        ...
        case VOWIFI_SET_LOW_THRESHOLD: {
            ...
            if ((c_value < MAC_VOWIFI_LOW_THRESHOLD_MIN) || (c_value > MAC_VOWIFI_LOW_THRESHOLD_MAX)) {
                OAM_ERROR_LOG1(0, OAM_SF_VOWIFI,
                               "{mac_vap_set_vowifi_param_etc::LOW_THRESHOLD Value[%d] error!}", c_value);
                return OAL_ERR_CODE_VOWIFI_SET_INVALID;
            }
        ...
        case VOWIFI_SET_HIGH_THRESHOLD: {
            ...
            if ((c_value < MAC_VOWIFI_HIGH_THRESHOLD_MIN) || (c_value > MAC_VOWIFI_HIGH_THRESHOLD_MAX)) {
                OAM_ERROR_LOG1(0, OAM_SF_VOWIFI,
                               "{mac_vap_set_vowifi_param_etc::HIGH_THRESHOLD Value[%d] error!}", c_value);
                return OAL_ERR_CODE_VOWIFI_SET_INVALID;
            }
        ...
        case VOWIFI_SET_TRIGGER_COUNT: {
            ...
            if ((uc_value < MAC_VOWIFI_TRIGGER_COUNT_MIN) || (uc_value > MAC_VOWIFI_TRIGGER_COUNT_MAX)) {
                OAM_ERROR_LOG1(0, OAM_SF_VOWIFI,
                               "{mac_vap_set_vowifi_param_etc::TRIGGER_COUNT Value[%d] error!}", uc_value);
                return OAL_ERR_CODE_VOWIFI_SET_INVALID;
            }
        ...
        default:
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                             "{mac_vap_set_vowifi_param_etc::invalid cmd = %d!!}",
                             en_vowifi_cfg_cmd);
...
oal_void mac_device_set_vap_id_etc(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap,
                                   oal_uint8 uc_vap_idx, wlan_vap_mode_enum_uint8 en_vap_mode,
                                   wlan_p2p_mode_enum_uint8 en_p2p_mode, oal_uint8 is_add_vap)
{
#ifdef _PRE_WLAN_FEATURE_P2P
...
oal_uint32 mac_device_find_2up_vap_etc(mac_device_stru *pst_mac_device,
