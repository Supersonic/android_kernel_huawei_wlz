                                                             ...
                                                             oal_uint32 *pul_num_channels)
{
    ...
    ***************************************************************************/
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfgvendor_get_current_channel_list:: wal_send_cfg_event_etc return err code %d!}\r\n", l_ret);
                                                    ...
                                                    oal_int32 len)
{
    ...
    if (OAL_UNLIKELY(err)) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "wal_cfgvendor_get_channel_list::oal_cfg80211_vendor_cmd_reply_etc failed ret:%d.\r\n", err);
        return err;
    }
                                                ...
                                                oal_wireless_dev_stru *wdev, OAL_CONST oal_void *data, oal_int32 len)
{
    ...
    if (OAL_UNLIKELY(!skb)) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "wal_cfgvendor_dbg_get_ring_status_etc::skb alloc failed.len %d\r\n",
            l_buf_len + 100);
        return -OAL_ENOMEM;
    }
    ...
    if (OAL_UNLIKELY(l_ret)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY,
            "wal_cfgvendor_dbg_get_ring_status_etc::Vendor Command reply failed. ret:%d.\r\n", l_ret);
    }

    OAM_WARNING_LOG1(0, OAM_SF_ANY,
        "wal_cfgvendor_dbg_get_ring_status_etc::SUCCESS. driver version len %d\r\n", l_buf_len);
    return l_ret;
}
                                                              ...
                                                              oal_int32 l_len)
{
    ...
    if (uc_ip_pkt_len > 0 && puc_ip_pkt != OAL_PTR_NULL) {
        ...
        pst_start_info =
            (mac_vowifi_nat_keep_alive_start_info_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, uc_msg_len, OAL_TRUE);
                                                             ...
                                                             oal_int32 l_len)
{
    ...
    if (en_find_keepid_flag == OAL_TRUE) {
        ...
        l_ret = wal_send_vowifi_nat_keep_alive_params(pst_wdev->netdev,
            (oal_uint8 *)&st_stop_info, OAL_SIZEOF(mac_vowifi_nat_keep_alive_basic_info_stru));
        if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
            return -OAL_EFAIL;
        }
    }
    ...
    ********************************************************************************/
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ANY,
            "{wal_cfgvendor_lstats_get_station_info::wal_send_cfg_event_etc return err code %d!}", l_ret);
        return -OAL_EFAIL;
    }
            ...
            case APF_ATTRIBUTE_PROGRAM:
                ...
                ***************************************************************************/
                ...
                if (OAL_SUCC != wal_check_and_release_msg_resp_etc(pst_rsp_msg)) {
                    OAM_WARNING_LOG0(0, OAM_SF_ANY,
                        "{wal_cfgvendor_apf_set_filter::wal_check_and_release_msg_resp_etc fail!}");
                    return -OAL_EFAIL;
                }
                break;
            default:
