...
oal_void  mac_set_cap_info_sta_etc(oal_void *pst_vap, oal_uint8 *puc_cap_info)
{
    ...
    ***************************************************************************/
    /* 学习对端的能力信息 */
    memcpy(puc_cap_info, (oal_uint8 *)(&pst_mac_vap->us_assoc_user_cap_info), OAL_SIZEOF(mac_cap_info_stru));
...
oal_void  mac_set_ssid_ie_etc(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len, oal_uint16 us_frm_type)
{
    ...
    *(puc_buffer + 1) = uc_ssid_len;
    ...
    memcpy(puc_buffer + MAC_IE_HDR_LEN, puc_ssid, uc_ssid_len);
...
oal_void  mac_set_nb_ie( oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    memcpy(puc_buffer + uc_index, g_auc_huawei_oui, MAC_OUI_LEN);
...
oal_void mac_set_11ntxbf_vendor_ie_etc(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    memset(&pst_vendor_ie->st_11ntxbf, 0, OAL_SIZEOF(mac_11ntxbf_info_stru));
...
oal_void mac_set_rsn_ie_etc(oal_void *pst_vap, oal_uint8 *puc_pmkid, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    memcpy(&puc_buffer[uc_index], (oal_uint8 *)(&ul_group_suit), 4);
    uc_index += 4;
    ...
    {
        memcpy(&puc_buffer[uc_index], (oal_uint8 *)(&aul_pcip[uc_loop]), 4);
        uc_index += 4;
    }
    ...
    {
        memcpy(&puc_buffer[uc_index], (oal_uint8 *)(&aul_akm[uc_loop]), 4);
        uc_index += 4;
    }
    ...
    memset(pst_rsn_cap, 0, OAL_SIZEOF(mac_rsn_cap_stru));
    ...
    if (puc_pmkid != OAL_PTR_NULL)
    {
        ...
        memcpy(&(puc_buffer[uc_index]), puc_pmkid, WLAN_PMKID_LEN);
    ...
    if ((OAL_TRUE == mac_mib_get_dot11RSNAMFPC((mac_vap_stru *)pst_mac_vap) && ul_group_mgmt_suit))
    {
        ...
        memcpy(&puc_buffer[uc_index], (oal_uint8 *)(&ul_group_mgmt_suit), 4);
        uc_index += 4;
...
oal_void mac_set_wpa_ie_etc(oal_void  *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    memcpy(puc_buffer + uc_index, g_auc_wpa_oui_etc, MAC_OUI_LEN);
...
//    if (OAL_FALSE == pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.en_dot11QosOptionImplemented)
    ...
    /* OUI */
    memcpy(&puc_buffer[uc_index], g_auc_wmm_oui_etc, MAC_OUI_LEN);
    uc_index += MAC_OUI_LEN;
...
oal_void mac_set_sup_mcs_set_field(oal_void * pst_vap, oal_uint8 *puc_buffer)
{
    ...
    *************************************************************************/
    ...
    /* 初始清零 */
    memset(puc_buffer, 0, OAL_SIZEOF(mac_sup_mcs_set_stru));

    memcpy(pst_sup_mcs_set->auc_rx_mcs, mac_mib_get_SupportedMCSRx(pst_mac_vap), WLAN_HT_MCS_BITMASK_LEN);
...
//#ifdef _PRE_WLAN_FEATURE_20_40_80_COEXIST
    ...
    memset(pst_ht_opern->auc_basic_mcs_set, 0, MAC_HT_BASIC_MCS_SET_LEN);
...
oal_void mac_set_ext_capabilities_ie_etc(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    ***************************************************************************/
    ...
    /* 初始清零 */
    memset(puc_buffer + MAC_IE_HDR_LEN, 0, OAL_SIZEOF(mac_ext_cap_ie_stru));
...
{
    mac_vap_stru              *pst_mac_vap    = (mac_vap_stru *)pst_vap;
    mac_vht_sup_mcs_set_rom_stru  *pst_vht_mcsset = (mac_vht_sup_mcs_set_rom_stru *)puc_buffer;
    ...
    ----------------------------------------------------------------------------
     ...
     |-----------------------------------------------------------------------|
    ***************************************************************************/
    pst_vht_mcsset->bit_rx_mcs_rom_map      = mac_mib_get_vht_rx_mcs_map(pst_mac_vap);
    pst_vht_mcsset->bit_rx_highest_rom_rate = mac_mib_get_us_rx_highest_rate(pst_mac_vap);
    pst_vht_mcsset->bit_tx_mcs_rom_map      = mac_mib_get_vht_tx_mcs_map(pst_mac_vap);
    pst_vht_mcsset->bit_tx_highest_rom_rate = mac_mib_get_us_tx_highest_rate(pst_mac_vap);
...
oal_void mac_set_tspec_info_field(oal_uint8 *pst_vap, mac_wmm_tspec_stru *pst_addts_args, oal_uint8 *puc_buffer)
{
   ...
   ***************************************************************************************************/
    ...
    /* 初始化TSPEC结构内存信息 */
    memset(puc_buffer, 0, MAC_WMMAC_TSPEC_LEN);
                            ...
                            en_app_ie_type_uint8 en_type)
{
    ...
    {
        memcpy(puc_buffer, puc_app_ie, ul_app_ie_len);
                        ...
                        en_app_ie_type_uint8 en_type)
{
    ...
    memcpy(puc_buffer, puc_wps_ie, puc_wps_ie[1] + MAC_IE_HDR_LEN);
...
oal_void mac_set_opmode_field_etc(oal_uint8 *pst_vap, oal_uint8 *puc_buffer, wlan_nss_enum_uint8 en_nss)
{
    ...
    memset(pst_opmode_notify, 0, OAL_SIZEOF(mac_opmode_notify_stru));
...
oal_void mac_set_tspec_ie_etc(oal_void *pst_mac_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len, oal_uint8 uc_tid)
{
    ...
    *(puc_buffer + 1) = 55;
    ...
    memset(puc_buffer + uc_len, 0, *(puc_buffer + 1));
...
oal_void mac_set_rrm_enabled_cap_field_etc(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    memset(pst_rrm_enabled_cap_ie, 0, OAL_SIZEOF(oal_rrm_enabled_cap_ie_stru));
...
oal_void mac_set_wfa_tpc_report_ie_etc(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    memcpy(puc_buffer + uc_index, g_auc_wfa_oui_etc, MAC_OUI_LEN);
    uc_index += MAC_OUI_LEN;
