...
oal_void  mac_res_vap_init(oal_void)
{
    ...
    for (ul_loop = 0; ul_loop < WLAN_VAP_SUPPORT_MAX_NUM_LIMIT; ul_loop++)
    {
        ...
        memset(g_pst_mac_res->st_vap_res.past_vap_info[ul_loop], 0, OAL_SIZEOF(dmac_vap_stru));
...
oal_uint32  mac_res_user_init_etc(oal_void)
{
    ...
    /* 内存初始清0 */
    memset(g_pst_dmac_user, 0, (OAL_SIZEOF(dmac_user_stru) * MAC_RES_MAX_USER_LIMIT));
    memset(g_pst_dmac_user_idx_size, 0, (OAL_SIZEOF(oal_uint) * MAC_RES_MAX_USER_LIMIT));
    memset(g_pst_dmac_user_cnt_size, 0, (OAL_SIZEOF(oal_uint8) * MAC_RES_MAX_USER_LIMIT));
...
oal_uint32  mac_res_init_etc(oal_void)
{
    ...
    memset(g_pst_mac_res, 0, OAL_SIZEOF(mac_res_stru));
    /***************************************************************************
            初始化DEV的资源管理内容
    ***************************************************************************/
...
oal_void  mac_res_vap_init(oal_void)
{
    ...
    memset(g_pst_mac_res_vap, 0, WLAN_VAP_SUPPORT_MAX_NUM_LIMIT * OAL_SIZEOF(mac_res_mem_vap_stru));
#endif
    ...
    for (ul_loop = 0; ul_loop < WLAN_VAP_SUPPORT_MAX_NUM_LIMIT; ul_loop++)
    {
          ...
          memset(g_pst_mac_res->st_vap_res.past_vap_info[ul_loop], 0, OAL_SIZEOF(hmac_vap_stru));
          #else
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1151) && (_PRE_TEST_MODE != _PRE_TEST_MODE_UT)
...
#endif
		  memset(g_pst_mac_res->st_vap_res.past_vap_info[ul_loop], 0, OAL_SIZEOF(mac_res_mem_vap_stru));
          #endif
...
oal_uint32  mac_res_user_init_etc(oal_void)
{
    ...
    ***************************************************************************/
    ...
    /* 内存初始清0 */
    #if (defined(_PRE_PRODUCT_ID_HI110X_HOST))
    memset(p_user_info, 0, (OAL_SIZEOF(hmac_user_stru) * MAC_RES_MAX_USER_LIMIT));
    #else
    memset(p_user_info, 0, (OAL_SIZEOF(mac_res_mem_user_stru) * MAC_RES_MAX_USER_LIMIT));
    #endif

    memset(p_idx, 0, (OAL_SIZEOF(oal_uint) * MAC_RES_MAX_USER_LIMIT));
    memset(p_user_cnt, 0, (OAL_SIZEOF(oal_uint8) * MAC_RES_MAX_USER_LIMIT));
