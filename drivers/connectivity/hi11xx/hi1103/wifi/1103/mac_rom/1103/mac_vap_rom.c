...
oal_uint32  mac_mib_get_station_id(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_len, oal_uint8 *puc_param)
{
    ...
    memcpy(pst_param->auc_station_id,
...
oal_uint32  mac_mib_set_ssid_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    memcpy(puc_mib_ssid, pst_param->ac_ssid, uc_ssid_len);
...
oal_uint32  mac_mib_get_ssid_etc(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_len, oal_uint8 *puc_param)
{
    ...
    memcpy(pst_param->ac_ssid, puc_mib_ssid, uc_ssid_len);
...
oal_uint32  mac_vap_add_assoc_user_etc(mac_vap_stru *pst_vap, oal_uint16 us_user_idx)
{
    ...
    {
        memset(&st_hilink_white_node, 0, OAL_SIZEOF(oal_hilink_white_node_stru));
        memcpy(&(st_hilink_white_node.auc_mac), pst_user->auc_user_mac_addr, WLAN_MAC_ADDR_LEN);
...
oal_uint32  mac_vap_del_user_etc(mac_vap_stru *pst_vap, oal_uint16 us_user_idx)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_dlist_tmp, pst_hash_head)
    {
        ...
        if (!oal_compare_mac_addr(pst_user->auc_user_mac_addr, pst_user_temp->auc_user_mac_addr))
        {
            ...
            memset(pst_user->auc_user_mac_addr, 0, WLAN_MAC_ADDR_LEN);
...
oal_uint32  mac_vap_find_user_by_macaddr_etc( mac_vap_stru *pst_vap, const unsigned char *puc_sta_mac_addr, oal_uint16 *pus_user_idx)
{
    ...
    else
    {
        ...
        else
        {
            ...
            OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_vap->ast_user_hash[ul_user_hash_value]))
            {
                ...
                if (OAL_PTR_NULL == pst_mac_user)
                {
                    OAM_ERROR_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY, "{mac_vap_find_user_by_macaddr_etc::pst_mac_user null.user idx %d}",pst_mac_user->us_assoc_id);
                    continue;
                }
                /*lint -restore */
...
#ifdef _PRE_WLAN_FEATURE_11AX_ROM
...
oal_void mac_vap_init_legacy_rates(mac_vap_stru *pst_vap,mac_data_rate_stru *pst_rates)
{
    ...
    for (uc_rate_index = 0; uc_rate_index < uc_rates_num; uc_rate_index++)
    {
        ...
        {
            memcpy(puc_curr_rate,puc_orig_rate,sizeof(mac_data_rate_stru));
...
oal_void mac_vap_init_11n_rates(mac_vap_stru *pst_mac_vap,mac_device_stru *pst_mac_dev)
{
    ...
    /* 将MIB值的MCS MAP清零 */
    memset(mac_mib_get_SupportedMCSTx(pst_mac_vap), 0, WLAN_HT_MCS_BITMASK_LEN);
    memset(mac_mib_get_SupportedMCSRx(pst_mac_vap), 0, WLAN_HT_MCS_BITMASK_LEN);
...
oal_void mac_vap_init_p2p_rates_etc(mac_vap_stru *pst_vap, wlan_protocol_enum_uint8 en_vap_protocol, mac_data_rate_stru  *pst_rates)
{
    ...
    memcpy(&pst_vap->ast_sta_sup_rates_ie[WLAN_BAND_5G], &pst_vap->st_curr_sup_rates, OAL_SIZEOF(pst_vap->st_curr_sup_rates));
    memcpy(&pst_vap->ast_sta_sup_rates_ie[WLAN_BAND_2G], &pst_vap->st_curr_sup_rates, OAL_SIZEOF(pst_vap->st_curr_sup_rates));
...
#ifdef _PRE_WLAN_FEATURE_11AX_ROM
    //mac_he_hdl_rom_rsv_stru             *pst_he_hdl_rom_rsv;
#endif
...
#ifdef _PRE_WLAN_FEATURE_11AX_ROM
    //pst_he_hdl_rom_rsv = &(pst_mac_user->st_he_hdl_rom_rsv);
#endif
...
#ifdef _PRE_WLAN_FEATURE_11AX_ROM
    if((OAL_TRUE == pst_mac_user->en_he_hdl_rom_no_use) &&
        (OAL_TRUE == pst_mac_user->bit_he_hdl_rom_vht_present_no_use))
    {
        en_bandwidth_ap = mac_get_bandwith_from_center_freq_seg0(pst_mac_user->uc_he_hdl_rom_bw_no_use,
                                          pst_mac_sta->st_channel.uc_chan_number, pst_mac_user->uc_he_hdl_rom_center_freq_seg0_no_use);
    }
#endif
...
{
    memset(&pst_vap->st_vap_dft, 0, OAL_SIZEOF(mac_vap_dft_stru));
...
oal_void mac_vap_vowifi_init(mac_vap_stru *pst_mac_vap)
{
    ...
    memset(pst_mac_vap->pst_vowifi_cfg_param, 0, OAL_SIZEOF(mac_vowifi_param_stru));
...
oal_uint32 mac_vap_save_app_ie_etc(mac_vap_stru *pst_mac_vap, oal_app_ie_stru *pst_app_ie, en_app_ie_type_uint8 en_type)
{
    ...
    memset(&st_tmp_app_ie, 0, OAL_SIZEOF(st_tmp_app_ie));
    ...
    memcpy((oal_void*)pst_mac_vap->ast_app_ie[en_type].puc_ie, (oal_void*)pst_app_ie->auc_ie, ul_ie_len);
...
{
    memcpy (pst_mac_vap->auc_bssid, puc_bssid, WLAN_MAC_ADDR_LEN);
    return OAL_SUCC;
}
                                        ...
                                        wlan_prot_mode_enum_uint8 en_prot_mode,mac_cfg_rts_tx_param_stru *pst_rts_tx_param)
{
    ...
    {
        OAM_ERROR_LOG2(0, OAM_SF_ASSOC, "{mac_protection_set_rts_tx_param_etc::param null,pst_mac_vap[%p] pst_rts_tx_param[%p]}",
...
oal_uint32 mac_vap_add_wep_key(mac_vap_stru *pst_mac_vap, oal_uint8 us_key_idx, oal_uint8 uc_key_len, oal_uint8 *puc_key)
{
    ...
    memcpy(pst_wep_key->auc_key, puc_key, WLAN_WPA_KEY_LEN); /*lint !e420*/
...
oal_uint32 mac_mib_set_wep_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_key_id, oal_uint8 uc_key_value)
{
    ...
    /*初始化wep相关MIB信息*/
    memset(pst_mac_vap->pst_mib_info->st_wlan_mib_privacy.ast_wlan_mib_wep_dflt_key[uc_key_id].auc_dot11WEPDefaultKeyValue, 0, WLAN_MAX_WEP_STR_SIZE);
...
oal_uint32 mac_vap_set_security(mac_vap_stru *pst_mac_vap, mac_beacon_param_stru *pst_beacon_param)
{
    ...
    memset(&st_crypto, 0, OAL_SIZEOF(mac_crypto_settings_stru));
    ...
    if (MAC_EID_RSN == pst_beacon_param->auc_rsn_ie[0])
    {
        ...
        mac_mib_set_rsnaactivated(pst_mac_vap, OAL_TRUE);
        mac_mib_set_AuthenticationMode(pst_mac_vap, WLAN_WITP_AUTH_OPEN_SYSTEM);
        mac_ie_get_rsn_cipher(pst_beacon_param->auc_rsn_ie, &st_crypto);
...
oal_uint32 mac_vap_update_hilink_white_list(mac_vap_stru *pst_mac_vap, oal_hilink_white_node_stru *pst_white_node)
{
    ...
    if (pst_white_node->en_white_list_operate == OAL_WHITE_LIST_OPERATE_ADD)
    {
        ...
        memset(pst_mac_hilink_white_temp, 0, OAL_SIZEOF(mac_okc_white_list_member_stru));
        memcpy(pst_mac_hilink_white_temp->auc_mac_addr, pst_white_node->auc_mac, WLAN_MAC_ADDR_LEN);
#ifdef _PRE_WLAN_FEATURE_HILINK_HERA_PRODUCT
