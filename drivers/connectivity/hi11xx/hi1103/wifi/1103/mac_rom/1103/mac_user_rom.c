...
oal_uint32 mac_user_add_wep_key_etc(mac_user_stru *pst_mac_user, oal_uint8 uc_key_index, mac_key_params_stru *pst_key)
{
    ...
    memcpy(&pst_mac_user->st_key_info.ast_key[uc_key_index].auc_key, pst_key->auc_key, (oal_uint32)pst_key->key_len);
    memcpy(&pst_mac_user->st_key_info.ast_key[uc_key_index].auc_seq, pst_key->auc_seq, (oal_uint32)pst_key->seq_len);
...
oal_uint32 mac_user_init_key_etc(mac_user_stru *pst_mac_user)
{
    ...
    memset(&pst_mac_user->st_key_info, 0, sizeof(mac_key_mgmt_stru));
...
oal_uint32  mac_user_update_ap_bandwidth_cap(mac_user_stru *pst_mac_user)
{
    ...
    if (OAL_TRUE == pst_mac_vht_hdl->en_vht_capable)
    {
        /* en_channel_width的取值，0 -- 20/40M, 1 -- 80M, 2 -- 160M */
                                    ...
                                    oam_user_info_change_type_enum_uint8 en_type)
{
    ...
    oam_event_report_etc(auc_user_macaddr, uc_vap_id, en_mod, OAM_EVENT_USER_INFO_CHANGE,
                            auc_event_info, sizeof(oam_user_info_change_stru));
...
oal_void mac_user_set_avail_op_rates_etc(mac_user_stru *pst_mac_user, oal_uint8 uc_rs_nrates, oal_uint8 *puc_rs_rates)
{
    ...
    memcpy(pst_mac_user->st_avail_op_rates.auc_rs_rates, puc_rs_rates, WLAN_MAX_SUPP_RATES);
}

oal_void mac_user_set_vht_hdl_etc(mac_user_stru *pst_mac_user, mac_vht_hdl_stru *pst_vht_hdl)
{
   memcpy((oal_uint8 *)(&pst_mac_user->st_vht_hdl), (oal_uint8 *)pst_vht_hdl, OAL_SIZEOF(mac_vht_hdl_stru));
}

oal_void mac_user_get_vht_hdl_etc(mac_user_stru *pst_mac_user, mac_vht_hdl_stru *pst_vht_hdl)
{
    memcpy((oal_uint8 *)pst_vht_hdl, (oal_uint8 *)(&pst_mac_user->st_vht_hdl), OAL_SIZEOF(mac_vht_hdl_stru));
}


oal_void mac_user_set_ht_hdl_etc(mac_user_stru *pst_mac_user, mac_user_ht_hdl_stru *pst_ht_hdl)
{
   memcpy((oal_uint8 *)(&pst_mac_user->st_ht_hdl), (oal_uint8 *)pst_ht_hdl, OAL_SIZEOF(mac_user_ht_hdl_stru));
}

oal_void mac_user_get_ht_hdl_etc(mac_user_stru *pst_mac_user, mac_user_ht_hdl_stru *pst_ht_hdl)
{
    memcpy((oal_uint8 *)pst_ht_hdl, (oal_uint8 *)(&pst_mac_user->st_ht_hdl), OAL_SIZEOF(mac_user_ht_hdl_stru));
}
...
oal_uint32 mac_user_update_wep_key_etc(mac_user_stru *pst_mac_usr, oal_uint16 us_multi_user_idx)
{
    ...
    memcpy(&pst_mac_usr->st_key_info, &pst_multi_user->st_key_info, OAL_SIZEOF(mac_key_mgmt_stru));
