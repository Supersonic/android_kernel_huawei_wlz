...
oal_void mac_init_regdomain_etc(oal_void)
{
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "mac_init_regdomain_etc::memcpy fail!");
        return;
    }
...
oal_uint32 mac_set_country_ie_2g_etc(mac_regdomain_info_stru *pst_rd_info,
    ...
    for (uc_rc_idx = 0; uc_rc_idx < pst_rd_info->uc_regclass_num; uc_rc_idx++) {
        ...
        if (ul_ret != OAL_SUCC) {
            ...
            OAM_ERROR_LOG1(0, OAM_SF_ANY,
                           "{mac_set_country_ie_2g_etc::mac_get_channel_num_from_idx_etc failed[%d].}", ul_ret);
            return ul_ret;
        }
...
oal_uint32 mac_set_country_ie_5g_etc(mac_regdomain_info_stru *pst_rd_info,
                                     oal_uint8 *puc_buffer,
                                     oal_uint8 *puc_len)
{
    mac_regclass_info_stru      *pst_reg_class = OAL_PTR_NULL;
...
oal_uint32 mac_get_channel_num_from_idx_etc(oal_uint8 uc_band,
                                            oal_uint8 uc_idx,
...
oal_uint32 mac_get_channel_idx_from_num_etc(oal_uint8 uc_band,
...
oal_uint32 mac_is_channel_idx_valid_etc(oal_uint8 uc_band, oal_uint8 uc_ch_idx)
{
    ...
    if ((uc_band == MAC_RC_START_FREQ_2) && (uc_ch_idx == MAC_CHANNEL14)) {
        return OAL_SUCC;
    }
#endif
...
mac_regclass_info_stru *mac_get_channel_num_rc_info_etc(oal_uint8 uc_band, oal_uint8 uc_ch_num)
{
    ...
    if (mac_get_channel_idx_from_num_etc(uc_band, uc_ch_num, &uc_channel_idx) != OAL_SUCC) {
        OAM_WARNING_LOG2(0, OAM_SF_ANY,
                         "{mac_get_channel_num_rc_info_etc::get_channel_idx_from_num failed. band:%d, ch_num:%d",
                         uc_band, uc_ch_num);
...
oal_uint32 mac_regdomain_set_max_power_etc(oal_uint8 uc_pwr, oal_bool_enum_uint8 en_exceed_reg)
{
    ...
    for (uc_rc_idx = 0; uc_rc_idx < g_st_mac_regdomain.uc_regclass_num; uc_rc_idx++) {
        ...
        if (uc_pwr <= uc_reg_pwr || en_exceed_reg == OAL_TRUE) {
            ...
            OAM_WARNING_LOG3(0, OAM_SF_TPC,
                             "uc_pwr[%d] exceed reg_tx_pwr[%d], rc_idx[%d]",
                             uc_pwr, uc_reg_pwr, uc_rc_idx);
            g_st_mac_regdomain.ast_regclass[uc_rc_idx].us_max_tx_pwr =
               (oal_uint16)(g_st_mac_regdomain.ast_regclass[uc_rc_idx].uc_max_reg_tx_pwr * 10);
        }
    }
...
oal_module_symbol(mac_regdomain_set_max_power_etc); /*lint +e19*/
