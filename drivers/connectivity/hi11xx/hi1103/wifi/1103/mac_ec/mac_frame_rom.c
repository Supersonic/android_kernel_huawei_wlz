...
mac_frame_other_cb g_st_mac_frame_other_rom_cb = {OAL_PTR_NULL, OAL_PTR_NULL };
#endif
...
oal_uint8 *mac_find_ie_sec_etc(oal_uint8 uc_eid, oal_uint8 *puc_ies, oal_int32 l_len, oal_uint32 *pul_len)
{
    if (puc_ies == OAL_PTR_NULL) {
        return OAL_PTR_NULL;
    }
...
oal_uint16 mac_set_wmmac_ie_sta_etc(oal_uint8 *pst_vap, oal_uint8 *puc_buffer, mac_wmm_tspec_stru *pst_addts_args)
{
    ...
    *************************************************************************************/
    ...
    l_ret = memcpy_s(&puc_buffer[uc_index], MAC_OUI_LEN, g_auc_wmm_oui_etc, MAC_OUI_LEN);
    if (l_ret != EOK) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "mac_set_wmmac_ie_sta_etc::memcpy fail!");
    }
...
oal_module_symbol(mac_find_ie_sec_etc);
