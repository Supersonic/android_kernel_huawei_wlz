    ...
    if ((l_ssid_num > 0) && (OAL_PTR_NULL != pst_request->ssids))
    {
        ...
        for (l_loop = 0; l_loop < l_ssid_num; l_loop++)
        {
            ...
            if (EOK != memcpy_s(pst_scan_param->st_ssids[l_loop].auc_ssid, pst_scan_param->st_ssids[l_loop].uc_ssid_len,
                ...
                OAM_WARNING_LOG0(0, OAM_SF_ANY, "wal_set_scan_ssid::memcpy fail!");
            }
        }
	}
}
...
oal_int32 wal_stop_sched_scan_etc(oal_net_device_stru *pst_netdev)
{
    ...
    OAM_WARNING_LOG2(0, OAM_SF_SCAN, "{wal_stop_sched_scan_etc::sched scan req[0x%x],sched scan complete[%d]}",(uintptr_t)(pst_scan_mgmt->pst_sched_scan_req),pst_scan_mgmt->en_sched_scan_complete);
