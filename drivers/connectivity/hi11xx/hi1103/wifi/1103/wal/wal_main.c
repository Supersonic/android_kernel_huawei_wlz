...
st_rx_auth_to_host g_st_rx_auth_to_host;
#endif
#ifdef _PRE_WLAN_FEATURE_IP_FILTER
...
OAL_STATIC ssize_t  wal_get_packet_statistics_wlan0_info(struct kobject *dev, struct kobj_attribute *attr, char*buf)
{
    ...
    l_ret = wal_atcmsrv_ioctl_get_rx_pckg_etc(pst_net_dev, &l_rx_pckg_succ_num);
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_packet_statistics_wlan0_info_print::wal_atcmsrv_ioctl_get_rx_pckg_etc failed, failed err:%d!}\r\n", l_ret);
        return ret;
    }
...
oal_void wal_send_rx_auth_to_host_work(oal_work_stru *pst_work)
{
    st_rx_auth_to_host     *pst_rx_auth_to_host;

    pst_rx_auth_to_host = OAL_CONTAINER_OF(pst_work, st_rx_auth_to_host, st_rx_auth_work);

    oal_cfg80211_send_rx_auth(pst_rx_auth_to_host->pst_dev, pst_rx_auth_to_host->puc_buf, pst_rx_auth_to_host->ul_len);
}
#endif
...
oal_int32  wal_main_init_etc(oal_void)
{
    ...
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_main_init_etc::en_init_state has a invalid value [%d]!}\r\n", en_init_state);
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_main_init_etc::wal_hipriv_create_proc_etc has a wrong return value [%d]!}\r\n", ul_ret);
...
#ifdef _PRE_WLAN_FEATURE_11R_AP
    oal_cancel_work_sync(&g_st_rx_auth_to_host.st_rx_auth_work);
#endif
#ifdef _PRE_WLAN_FEATURE_IP_FILTER
