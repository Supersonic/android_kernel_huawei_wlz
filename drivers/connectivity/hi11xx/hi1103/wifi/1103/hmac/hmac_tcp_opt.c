        ...
        case OAL_HOST2NET_SHORT(ETHER_TYPE_IP):
            ...
            if (MAC_TCP_PROTOCAL == pst_ip_hdr->uc_protocol)
            {
                if (OAL_TRUE == oal_netbuf_is_tcp_ack_etc(pst_ip_hdr))
                {
                    ...
                    {
#ifdef _PRE_WLAN_TCP_OPT_DEBUG
                        OAM_WARNING_LOG0(0, OAM_SF_RX, "{hmac_judge_rx_netbuf_is_tcp_ack_etc:  **specific tcp pkt, can't be filter**!}");
#endif
                    }
                    else
                    {
...
#ifdef _PRE_WLAN_TCP_OPT_DEBUG
    OAM_WARNING_LOG1(0,OAM_SF_ANY,"**hmac_judge_rx_netbuf_classify_etc, us_ether_type = %d**",pst_snap->us_ether_type);
#endif
