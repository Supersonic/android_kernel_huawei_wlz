...
oal_void hmac_uapsd_update_user_para_etc (oal_uint8  *puc_payload,oal_uint8  uc_sub_type,oal_uint32 ul_msg_len,
                                            hmac_user_stru *pst_hmac_user)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_mac_vap)) {
        OAM_ERROR_LOG1(0, OAM_SF_CFG, "{hmac_uapsd_update_user_para_etc::get mac_vap [%d] null.}", pst_hmac_user->st_user_base_info.uc_vap_id);
        return;
    }
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == puc_param)) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_uapsd_update_user_para_etc::puc_param null.}");
        return;
    }
    // uc_user_index
    ...
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_uapsd_update_user_para_etc::hmac_config_send_event_etc fail.  erro code is %u}", ul_ret);
    }
    OAL_MEM_FREE(puc_param, OAL_TRUE);
