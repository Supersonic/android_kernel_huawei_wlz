                    ...
                    mac_wmm_tspec_stru                 *pst_addts_args)
{
    ...
    /* 设置WMM AC参数 */
    us_index += mac_set_wmmac_ie_sta_etc((oal_void *)pst_vap, puc_data + us_index, pst_addts_args);
                                                        ...
                                                        oal_uint8         uc_tsid)
{
    ...
    us_ie_len = mac_set_wmmac_ie_sta_etc((oal_void *)pst_vap, puc_data + us_index, &st_addts_args);
                       ...
                       mac_wmm_tspec_stru                 *pst_delts_args)
{
    ...
    if (EOK != memcpy_s((oal_uint8 *)(oal_netbuf_data(pst_delts) + us_frame_len),
    ...
    if (ul_ret != OAL_SUCC)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_tx_delts::frw_event_dispatch_event_etc failed[%d].}", ul_ret);
        oal_netbuf_free(pst_delts);
...
OAL_STATIC oal_uint32  hmac_mgmt_tx_addts_req_timeout(oal_void *p_arg)
{
    ...
    pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user_etc(pst_ts_args->us_mac_user_idx);
    ...
    if(OAL_TRUE == pst_ts_args->uc_second_flag)
    {
        return OAL_SUCC;
    }
                ...
                mac_wmm_tspec_stru                 *pst_addts_args)
{
    ...
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_tx_addba_req_etc::pst_device null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (EOK != memcpy_s((oal_uint8 *)(oal_netbuf_data(pst_addts_req) + us_frame_len),
                         WLAN_MEM_NETBUF_SIZE2 - us_frame_len,
                         (oal_uint8 *)&st_wlan_ctx_action,
                         OAL_SIZEOF(dmac_ctx_action_event_stru))) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hwifi_config_init_fcc_ce_txpwr_nvram::memcpy fail!");
    ...
    if (ul_ret != OAL_SUCC)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_tx_addts_req::frw_event_dispatch_event_etc failed[%d].}", ul_ret);
        oal_netbuf_free(pst_addts_req);
        FRW_EVENT_FREE(pst_event_mem);
        return ul_ret;
    }
    ...
    if(MAC_TS_SUCCESS == pst_ts_use->en_ts_status)
    {
    ...
    /* 启动ADDTS REQ超时计时器 */
    FRW_TIMER_CREATE_TIMER(&pst_ts_timer->st_addts_timer,
                                                ...
                                                oal_uint16         us_frame_len)
{
    ...
    /* 拷贝整个ADDTS REQ帧 */
    if (EOK != memcpy_s(puc_data, us_frame_len, puc_addts_req, us_frame_len)) {
...
oal_uint32  hmac_mgmt_rx_addts_rsp_etc(
                hmac_vap_stru              *pst_hmac_vap,
                hmac_user_stru             *pst_hmac_user,
                oal_uint8                  *puc_payload)
{
    ...
    {
        OAM_ERROR_LOG3(0, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_rsp_etc::null param, %p %p %p.}", (uintptr_t)pst_hmac_vap, (uintptr_t)pst_hmac_user, (uintptr_t)puc_payload);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        /* 对于tid > 7的resp直接忽略 */
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_rsp_etc::addts rsp tsid[%d]} token[%d] status[%d]", uc_user_prio, puc_payload[2], puc_payload[3]);
        return OAL_SUCC;
    }
    ...
    pst_ts_tmp = &pst_hmac_user->st_ts_tmp_info[uc_ac_num];
    if(MAC_TS_SUCCESS == pst_ts->en_ts_status)
    {
        if(MAC_TS_INPROGRESS == pst_ts_tmp->en_ts_status)
        {
            if(OAL_TRUE == pst_ts_tmp->st_addts_timer.en_is_registerd)
            {
                FRW_TIMER_IMMEDIATE_DESTROY_TIMER(&pst_ts_tmp->st_addts_timer);
            }
    ...
    {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_rsp_etc::addba rsp tsid[%d],old_tsid[%d]} ", uc_tsid, pst_ts->uc_tsid);
        return OAL_SUCC;
    }

    if(uc_dialog_token != pst_ts->uc_ts_dialog_token)
    {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_rsp_etc::addts rspuc_dialog_token wrong.rsp dialog[%d], req dialog[%d]}",uc_dialog_token, pst_ts->uc_ts_dialog_token);
        return OAL_SUCC;
    }
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_rsp_etc::medium_time[%d], tsid[%d], uc_up[%d], en_direction[%d].}",
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_rsp_etc::ts status[%d], dialog_token[%d], vap id[%d], ac num[%d].}",
...
oal_uint32  hmac_mgmt_rx_delts_etc(
                hmac_vap_stru              *pst_hmac_vap,
                hmac_user_stru             *pst_hmac_user,
                oal_uint8                  *puc_payload)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap,pst_hmac_user,puc_payload))
    {
        OAM_ERROR_LOG3(0, OAM_SF_WMMAC, "{hmac_mgmt_rx_delts_etc::null param, %p %p %p.}", (uintptr_t)pst_hmac_vap, (uintptr_t)pst_hmac_user, (uintptr_t)puc_payload);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        /* 对于tid > 7的resp直接忽略 */
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_delts_etc::tsid[%d]} token[%d] status[%d]", uc_tsid, puc_payload[2], puc_payload[3]);
        return OAL_SUCC;
    }
    ...
    for(uc_ac_num = 0; uc_ac_num < WLAN_WME_AC_BUTT; uc_ac_num++)
    {
        ...
        if(pst_ts->uc_tsid != uc_tsid)
        {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_delts_etc::tsid[%d],old_tsid[%d]} ", uc_tsid, pst_ts->uc_tsid);
            continue;
        }
...
oal_uint32  hmac_mgmt_rx_addts_req_frame_etc(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    {
        OAM_ERROR_LOG2(0, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_req_frame_etc::pst_hmac_vap = [%x], pst_netbuf = [%x]!}\r\n",
    ...
    mac_get_address2((oal_uint8 *)MAC_GET_RX_CB_MAC_HEADER_ADDR(pst_rx_ctrl), auc_sta_addr, WLAN_MAC_ADDR_LEN);

    ul_ret = mac_vap_find_user_by_macaddr_etc(&(pst_hmac_vap->st_vap_base_info), auc_sta_addr, &us_user_idx);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC,
                         "{hmac_mgmt_rx_addts_req_frame_etc::mac_vap_find_user_by_macaddr_etc failed[%d].}", ul_ret);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_req_frame_etc::pst_mac_user null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_req_frame_etc::pst_addts_rsp null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 调用封装管理帧接口 */
    us_frame_len = hmac_mgmt_encap_addts_rsp(pst_hmac_vap, oal_netbuf_data(pst_addts_rsp), oal_netbuf_data(pst_netbuf), pst_rx_ctrl->us_frame_len);
    if (0 == us_frame_len)
    {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_req_frame_etc::encap_addts_rsp, but frame len is zero.}");
    ...
    ul_ret = hmac_tx_mgmt_send_event_etc(&(pst_hmac_vap->st_vap_base_info), pst_addts_rsp, us_frame_len);
    if (OAL_SUCC != ul_ret)
    {
        oal_netbuf_free(pst_addts_rsp);
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_req_frame_etc::hmac_tx_mgmt_send_event_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32  hmac_config_addts_req_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_FALSE == g_en_wmmac_switch_etc)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_addts_req_etc::wmmac switch is false[%d].}", g_en_wmmac_switch_etc);
        return OAL_SUCC;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_addts_req_etc::pst_hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_PTR_NULL == pst_hmac_user)
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_addts_req_etc::pst_hmac_user null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_addts_req_etc::user prio[%d] is false.}", uc_ac);
        return OAL_SUCC;
    }
...
oal_uint32  hmac_config_delts_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_FALSE == g_en_wmmac_switch_etc)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_delts_etc::wmmac switch is false[%d].}", g_en_wmmac_switch_etc);
        return OAL_SUCC;
    }
    ...
    if (OAL_PTR_NULL == pst_hmac_vap)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_delts_etc::pst_hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_PTR_NULL == pst_hmac_user)
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_delts_etc::pst_hmac_user null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32  hmac_config_wmmac_switch_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    oal_uint32                           ul_ret;
    oal_uint8                            uc_ac_idx;
    hmac_user_stru                      *pst_hmac_user = OAL_PTR_NULL;
    ...
    g_en_wmmac_switch_etc                = pst_wmm_ac_param->en_wmm_ac_switch;
    pst_hmac_vap->en_wmmac_auth_flag = pst_wmm_ac_param->en_auth_flag;

    pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user_etc(pst_mac_vap->us_assoc_vap_id);
    if (OAL_PTR_NULL == pst_hmac_user)
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_wmmac_switch_etc::pst_hmac_user is deleted already.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if(!g_en_wmmac_switch_etc)
    {
        for(uc_ac_idx = 0;uc_ac_idx < WLAN_WME_AC_BUTT; uc_ac_idx++)
        {
            ...
            if(OAL_TRUE == pst_hmac_user->st_ts_tmp_info[uc_ac_idx].st_addts_timer.en_is_registerd)
            {
                FRW_TIMER_IMMEDIATE_DESTROY_TIMER(&(pst_hmac_user->st_ts_tmp_info[uc_ac_idx].st_addts_timer));
            }
    ...
    抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ul_ret = hmac_config_send_event_etc(pst_mac_vap, WLAN_CFGID_WMMAC_SWITCH, us_len, puc_param);
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_wmmac_switch_etc::hmac_config_send_event_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
#endif
...
oal_uint32  hmac_tx_mgmt_reassoc_req_etc(hmac_vap_stru *pst_hmac_vap_sta)
{
    ...
    {
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_tx_mgmt_reassoc_req_etc,%p.}", (uintptr_t)pst_hmac_vap_sta);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_tx_mgmt_reassoc_req_etc::pst_asoc_req_frame null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ul_asoc_frame_len = hmac_mgmt_encap_asoc_req_sta_etc(pst_hmac_vap_sta,(oal_uint8 *)(OAL_NETBUF_HEADER(pst_asoc_req_frame)), puc_curr_bssid);
    oal_netbuf_put(pst_asoc_req_frame, ul_asoc_frame_len);

    if (0 == ul_asoc_frame_len)
    {
        OAM_WARNING_LOG0(pst_hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_tx_mgmt_reassoc_req_etc::hmac_mgmt_encap_asoc_req_sta_etc null.}");
        oal_netbuf_free(pst_asoc_req_frame);

        return OAL_FAIL;
    }
    pst_hmac_user_ap = (hmac_user_stru *)mac_res_get_hmac_user_etc(pst_hmac_vap_sta->st_vap_base_info.us_assoc_vap_id);
    if (OAL_PTR_NULL == pst_hmac_user_ap)
    {
        OAM_ERROR_LOG0(pst_hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_tx_mgmt_reassoc_req_etc::pst_hmac_user_ap null.}");
        oal_netbuf_free(pst_asoc_req_frame);
    ...
    /* 抛事件让DMAC将该帧发送 */
    ul_ret = hmac_tx_mgmt_send_event_etc(&(pst_hmac_vap_sta->st_vap_base_info), pst_asoc_req_frame, (oal_uint16)ul_asoc_frame_len);
    if (OAL_SUCC != ul_ret)
    {
        oal_netbuf_free(pst_asoc_req_frame);
        OAM_WARNING_LOG1(pst_hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{mac_tx_mgmt_reassoc_req::hmac_tx_mgmt_send_event_etc failed[%d].}", ul_ret);
        return ul_ret;
    }
    return OAL_SUCC;
}
...
oal_uint32  hmac_config_reassoc_req_etc(mac_vap_stru *pst_mac_vap)
{
    hmac_vap_stru                       *pst_hmac_vap = OAL_PTR_NULL;

    if (OAL_FALSE == g_en_wmmac_switch_etc)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_reassoc_req_etc::wmmac switch is false[%d].}", g_en_wmmac_switch_etc);
        return OAL_SUCC;
    }
    ...
    if (OAL_PTR_NULL == pst_hmac_vap)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_reassoc_req_etc::pst_hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_tx_mgmt_reassoc_req_etc(pst_hmac_vap);
    return OAL_SUCC;
}
