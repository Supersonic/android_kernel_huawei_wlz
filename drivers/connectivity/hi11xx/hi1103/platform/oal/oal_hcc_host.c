...
oal_uint32 tcp_ack_wait_sche_cnt_etc = 1;

#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
struct custom_process_func_handler custom_process_func_etc;
EXPORT_SYMBOL_GPL(custom_process_func_etc);  //lint !e132 !e745 !e578
#endif

/* use lint -e16 to mask the error19! */
oal_uint32 pm_wifi_rxtx_count = 0;  // pm收发包统计变量
/*lint -e19 */
...
{
    return &custom_process_func_etc;
}
/*lint -e19 */
                                               ...
                                               oal_uint32 remain_len)
{
    ...
    oal_print_hex_dump((oal_uint8 *)buf, HISDIO_HOST2DEV_SCATT_SIZE, HEX_DUMP_GROUP_SIZE, "h2d assem info");
#endif
...
/*
 * Prototype    : hcc_tx_param_check_etc
...
/*lint +e19 */
oal_void hcc_tx_wlan_queue_map_set_etc(struct hcc_handler *hcc, hcc_queue_type hcc_queue_id,
...
static oal_int32 hcc_tx_para_check(struct hcc_handler *hcc, oal_netbuf_stru *netbuf,
                                   struct hcc_transfer_param *param)
{
    if (OAL_WARN_ON(hcc == NULL)) {
                     ...
                     struct hcc_transfer_param *param)
{
    ...
    if (OAL_UNLIKELY(hcc_tx_param_check_etc(hcc, param) != OAL_SUCC)) {
        return -OAL_EINVAL;
    }
    ...
    if (OAL_UNLIKELY(!OAL_IS_ALIGNED((uintptr_t)hdr, sizeof(oal_uint32)))) { /* 判断是不是4字节对齐 */
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "hdr:%ld not aligned len:%d", (uintptr_t)hdr, (int)sizeof(oal_uint32));
        return -OAL_EFAIL;
    }
...
oal_int32 hcc_send_descr_control_data_etc(struct hcc_handler *hcc, hcc_descr_type descr_type,
...
oal_void (*hcc_event_info_print_cb)(oal_netbuf_stru*);
oal_module_symbol(hcc_event_info_print_cb);

OAL_STATIC OAL_INLINE oal_void hcc_host_wkup_dev_debug_info(oal_netbuf_stru *pst_netbuf) {
    struct wlan_pm_s *pst_wlan_pm;
    if (wlan_pm_wkup_dev_src_debug_get() == OAL_TRUE) {
        wlan_pm_wkup_dev_src_debug_set(OAL_FALSE);
        pst_wlan_pm = wlan_pm_get_drv_etc();
        if (pst_wlan_pm->pst_bus->st_bus_wakelock.lock_count == 0 &&
           hcc_event_info_print_cb) {
            hcc_event_info_print_cb(pst_netbuf);
        }
    }
}
#endif
...
/*
 * Prototype    : hcc_send_tx_queue_etc
...
#ifdef _PRE_WLAN_WAKEUP_SRC_PARSE
        hcc_event_info_print_cb(oal_netbuf_peek(head));
#endif
...
#ifdef _PRE_WLAN_FEATURE_OFFLOAD_FLOWCTL
        oal_print_hi11xx_log(HI11XX_LOG_DBG,
                             "before_update: hcc_send_tx_queue_etc: queue_type = %d, burst_limit = %d, remain_len = %d",
        ...
        for (;;) {
            ...
            if (pst_hcc_queue->flow_ctrl.flow_type == HCC_FLOWCTRL_CREDIT) {
                ...
                if (ul_pool_type_flag == OAL_FALSE) {
                    if (hcc_tx_netbuf_test_and_switch_high_pri_queue_etc(hcc, pool_type) == OAL_SUCC) {
...
OAL_STATIC oal_int32 hcc_rx(struct hcc_handler *hcc, oal_netbuf_stru *netbuf)
{
    ...
    if (OAL_UNLIKELY(hcc_check_rx_netbuf_vaild(hdr, netbuf_len) != OAL_TRUE)) {
        oal_print_hex_dump((oal_uint8 *)hdr, (oal_int32)OAL_NETBUF_LEN(netbuf), HEX_DUMP_GROUP_SIZE,
...
oal_void hcc_host_set_flowctl_param_etc(oal_uint8 uc_queue_type, oal_uint16 us_burst_limit,
...
oal_void hcc_host_get_flowctl_param_etc(oal_uint8 uc_queue_type, oal_uint16 *pus_burst_limit,
...
oal_void hcc_host_update_vi_flowctl_param_etc(oal_uint32 be_cwmin, oal_uint32 vi_cwmin)
{
    ...
    if (be_cwmin == vi_cwmin) {
        ...
        hcc_host_set_flowctl_param_etc(DATA_UDP_VI_QUEUE, UDP_VI_BURST_LIMIT,
...
/*
 * Prototype    : hcc_thread_process_etc
...
{
    tcp_ack_wait_sche_cnt_etc = ul_val;
    return;
}
...
oal_int32 hcc_transfer_thread_etc(oal_void *data)
{
    ...
    for (;;) {
        ...
        if (hcc->p_hmac_tcp_ack_process_func != OAL_PTR_NULL) {
            ack_loop_count++;
            if (ack_loop_count >= tcp_ack_wait_sche_cnt_etc) {
                ack_loop_count = 0;
                hcc->p_hmac_tcp_ack_process_func();
            }
        }
#endif
        group_ret |= (oal_uint32)hcc_bus_pending_signal_process(hcc->bus_dev->cur_bus);

        group_ret |= (oal_uint32)hcc_thread_process_etc(hcc);
...
oal_module_symbol(tcp_ack_wait_sche_cnt_etc);
/*lint +e19*/
OAL_STATIC oal_void hcc_dev_flowctr_timer_del(struct hcc_handler *hcc)
{
    if (oal_in_interrupt()) {
...
OAL_STATIC oal_void hcc_trans_flow_ctrl_info_init_etc(struct hcc_handler *hcc)
{
    oal_int32 i;
    hcc->hcc_transer_info.tx_flow_ctrl.flowctrl_flag = D2H_MSG_FLOWCTRL_ON;
...
/*
 * 函 数 名  : hcc_credit_update_callback_etc
...
struct hcc_handler *hcc_module_init_etc(hcc_bus_dev *pst_bus_dev)
{
    ...
    if (hcc_tx_assem_descr_init_etc(hcc) != OAL_SUCC) {
        OAL_IO_PRINT("hcc tx assem descrt alloc failed!\n");
        goto failed_tx_assem_descr_alloc;
    }
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    if (hcc_message_register_etc(hcc, D2H_MSG_FLOWCTRL_ON, hcc_flow_on_callback_etc, hcc) != OAL_SUCC) {
        OAL_IO_PRINT("register flow ctrl on failed!\n");
        goto failed_reg_flowon_msg;
    }

    if (hcc_message_register_etc(hcc, D2H_MSG_FLOWCTRL_OFF, hcc_flow_off_callback_etc, hcc) != OAL_SUCC) {
...
#endif
    if (hcc_test_init_module_etc(hcc) != OAL_SUCC) {
        OAL_IO_PRINT("register flow ctrl off failed!\n");
