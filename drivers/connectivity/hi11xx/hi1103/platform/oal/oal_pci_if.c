...
oal_void *5115_cfg_base = OAL_PTR_NULL;
oal_void *reg5115_sys_ctl = OAL_PTR_NULL;
oal_void *5610_mode = OAL_PTR_NULL;
oal_void *5610_gpio = OAL_PTR_NULL;
#if (_PRE_TARGET_PRODUCT_TYPE_5610DMB == _PRE_CONFIG_TARGET_PRODUCT)
oal_void *5610_gpio1 = OAL_PTR_NULL; /* 0x10106000地址 */
oal_void *5610_gpio2 = OAL_PTR_NULL; /* 0x10108000地址 */
#endif
#ifdef _PRE_WLAN_FEATURE_PCIE_ADAPT_5116
oal_void *5115_sys_ctl1 = OAL_PTR_NULL; /* v200 5116 pcie1的sys ctrl */
#endif
 ...
 * 功能描述  : 设置滑窗后的EP端iATU表:FPGA有0x30000000的地址空间，需要进行划窗访问
 */
...
OAL_STATIC oal_int32 oal_pci_witp_init(oal_pci_dev_stru *pst_pci_dev, oal_uint32 ul_dev_id)
{
    ...
    oal_bus_dev_stru *pst_bus_dev = OAL_PTR_NULL;
    oal_uint32 ul_ret;
...
{
    reg5115_sys_ctl = oal_ioremap_nocache(OAL_PCIE_SYS_BASE_PHYS, 0x1000);
    if (reg5115_sys_ctl == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_5115_pci_init, Cannot map system controller register base!\n");

        return OAL_ERR_CODE_PTR_NULL;
    }
#ifdef _PRE_WLAN_FEATURE_PCIE_ADAPT_5116
    5115_sys_ctl1 = oal_ioremap_nocache(OAL_PCIE_SYS_BASE_PHYS + OAL_PCIE_PORT_OFFSET, 0x1000);
    if (5115_sys_ctl1 == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_5115_pci_init, Cannot map system controller register base for pcie1!\n");

        return OAL_ERR_CODE_PTR_NULL;
    }
#endif
    OAL_IO_PRINT("oal_5115_pci_init,OAL_PCIE_SYS_BASE_PHYS=0x%x,map reg5115_sys_ctl = 0x%x\n",
                 OAL_PCIE_SYS_BASE_PHYS, (oal_uint32)reg5115_sys_ctl);

#if (_PRE_TARGET_PRODUCT_TYPE_E5 != _PRE_CONFIG_TARGET_PRODUCT) && \
    (_PRE_TARGET_PRODUCT_TYPE_CPE != _PRE_CONFIG_TARGET_PRODUCT)
    5115_cfg_base = oal_ioremap_nocache(OAL_PCIE_CRG_BASE, 0x300);
    if (5115_cfg_base == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_5115_pci_init, cannot map cfg base register!\n");
        return OAL_ERR_CODE_PTR_NULL;
    }
#endif

    5610_mode = oal_ioremap_nocache(0x14900000, 0x200);
    if (5610_mode == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_5115_pci_init, Cannot map 5610_mode register base!\n");

        return OAL_ERR_CODE_PTR_NULL;
    }

    5610_gpio = oal_ioremap_nocache(0x10107000, 0x100);
    if (5610_gpio == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_5115_pci_init, Cannot map 5610_mode register base!\n");
...
#if (_PRE_TARGET_PRODUCT_TYPE_5610DMB == _PRE_CONFIG_TARGET_PRODUCT)
    5610_gpio1 = oal_ioremap_nocache(0x10106000, 0x100);
    if (5610_gpio1 == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_5115_pci_init, Cannot map 5610_gpio1 register base!\n");

        return OAL_ERR_CODE_PTR_NULL;
    }

    5610_gpio2 = oal_ioremap_nocache(0x10108000, 0x100);
    if (5610_gpio2 == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_5115_pci_init, Cannot map 5610_gpio2 register base!\n");
...
#ifdef _PRE_WLAN_FEATURE_PCIE_ADAPT_5116
    if (5115_sys_ctl1) {
        oal_iounmap(5115_sys_ctl1);
    }
#endif

    if (5610_mode) {
        oal_iounmap(5610_mode);
    }

    if (5610_gpio) {
        oal_iounmap(5610_gpio);
    }

#if (_PRE_TARGET_PRODUCT_TYPE_5610DMB == _PRE_CONFIG_TARGET_PRODUCT)
    if (5610_gpio1) {
        oal_iounmap(5610_gpio1);
    }

    if (5610_gpio2) {
        oal_iounmap(5610_gpio2);
    }
#endif
}
...
oal_void oal_pcie_dbi_enable(oal_uint32 id)
{
    ...
    if (id == 0) {
        ...
        ul_val = oal_readl(5115_sys_ctl1 + OAL_PERI_R_PCIE1);
        ul_val |= BIT13;
        oal_writel(ul_val, 5115_sys_ctl1 + OAL_PERI_R_PCIE1);
    }
#else
    if (id == 0) {
        ul_val = oal_readl(reg5115_sys_ctl + OAL_PERI_R_PCIE0);
        ...
        ul_val = oal_readl(reg5115_sys_ctl + OAL_PERI_W_PCIE0);
        ul_val |= BIT21;
        oal_writel(ul_val, reg5115_sys_ctl + OAL_PERI_W_PCIE0);
    } else {
        ul_val = oal_readl(reg5115_sys_ctl + OAL_PERI_R_PCIE1);
        ul_val |= BIT21;
        oal_writel(ul_val, reg5115_sys_ctl + OAL_PERI_R_PCIE1);

        ul_val = oal_readl(reg5115_sys_ctl + OAL_PERI_W_PCIE1);
        ul_val |= BIT21;
        oal_writel(ul_val, reg5115_sys_ctl + OAL_PERI_W_PCIE1);
    }
#endif
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    oal_void *pst_virt_addr = OAL_PTR_NULL;
    oal_bus_chip_stru *pst_bus_chip = OAL_PTR_NULL;
...
oal_uint32 oal_pci_linkup_check(oal_pci_dev_stru *pst_pci_dev)
{
    ...
    ul_value = oal_readl(reg5115_sys_ctl + OAL_PCIE_LINK_STATUS_OFFSET);
...
oal_uint32 oal_pci_hand_reset(oal_uint8 uc_chip_id)
{
    ...
    ul_val = oal_readl(5115_cfg_base + 0x20);
    ul_val = ul_val | PCIE_DEV_CLK_ENABLE_BIT(uc_chip_id);
    oal_writel(ul_val, 5115_cfg_base + 0x20);

    /* pcie0控制寄器12，初始值为420c0802 */
    ul_val = 0x420539F3;
    oal_writel(ul_val, 5115_cfg_base + PCIE_CTRL_REG12(uc_chip_id));

    /* 0x30外复位控制器，bit23代表 PCIE0 复位(1代表复位，0代表不复位) */
    ul_val = oal_readl(5115_cfg_base + 0x30);
    ul_val = ul_val & PCIE_RESET_CTRL_BIT(uc_chip_id);
    oal_writel(ul_val, 5115_cfg_base + 0x30);
    oal_udelay(1000);
...
#if (_PRE_CONFIG_TARGET_PRODUCT == _PRE_TARGET_PRODUCT_TYPE_E5)
/*
 * 函 数 名  : oal_drv_pcie_pm_sys_resume
 * 功能描述  : syscore resume的回调函数,恢复ep侧寄存器
 */
OAL_STATIC oal_void oal_drv_pcie_pm_sys_resume(oal_void)
{
...
/*
 * 函 数 名  : oal_register_syscore_ops
 * 功能描述  : 注册syscore回调函数
 */
...
oal_module_symbol(reg5115_sys_ctl);
#ifdef _PRE_WLAN_FEATURE_PCIE_ADAPT_5116
oal_module_symbol(5115_sys_ctl1);
#endif
oal_module_symbol(5610_mode);
oal_module_symbol(5610_gpio);
#if (_PRE_TARGET_PRODUCT_TYPE_5610DMB == _PRE_CONFIG_TARGET_PRODUCT)
oal_module_symbol(5610_gpio1);
oal_module_symbol(5610_gpio2);
#endif
#ifdef _PRE_WLAN_PRODUCT_1151V200
