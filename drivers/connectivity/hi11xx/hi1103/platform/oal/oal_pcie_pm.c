...
oal_void oal_pcie_aspm_enable(oal_pci_dev_stru *pst_dev, oal_pcie_aspm_enum mode)
{
    ...
    if (pos) {
        ...
        oal_pcie_print_bits((oal_void *)&tmp, sizeof(tmp));
    } else {
...
oal_void *pci_rc_sys_ctl = OAL_PTR_NULL;
oal_void *pci_dbi_0 = OAL_PTR_NULL;
oal_void *pci_dbi_1 = OAL_PTR_NULL;

oal_uint32 oal_pcie_rc_mem_map(oal_void)
{
    pci_rc_sys_ctl = oal_ioremap_nocache(OAL_PCIE_RC_SYS_BASE, 0x1000);
    if (pci_rc_sys_ctl == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_pci_rc_map, Cannot map system controller register base!\n");

        return OAL_ERR_CODE_PTR_NULL;
    }

    OAL_IO_PRINT("oal_pci_rc_map,OAL_PCIE_SYS_BASE = 0x%x,map pci_rc_sys_ctl = 0x%lx\n",
                 OAL_PCIE_RC_SYS_BASE, (oal_ulong)pci_rc_sys_ctl);

    pci_dbi_0 = oal_ioremap_nocache(OAL_DBI_RC_BASE_ADDR_0, 0x1000);
    if (pci_dbi_0 == OAL_PTR_NULL) {
        OAL_IO_PRINT("oal_pci_rc_map, Cannot map dbi 0 register base!\n");

        return OAL_ERR_CODE_PTR_NULL;
    }
    OAL_IO_PRINT("oal_pci_rc_map,OAL_DBI_BASE_ADDR_0=0x%x,map pci_dbi_0 = 0x%lx\n",
                 (unsigned int)OAL_DBI_RC_BASE_ADDR_0, (oal_ulong)pci_dbi_0);
...
{
    if (pci_rc_sys_ctl != NULL) {
        oal_iounmap(pci_rc_sys_ctl);
    }

    if (pci_dbi_0 != NULL) {
        oal_iounmap(pci_dbi_0);
    }
}
...
oal_void oal_pcie_rc_dbi_enable(oal_uint32 id)
{
    ...
    if (id == 0) {
        ul_val = oal_readl(pci_rc_sys_ctl + OAL_PERI_R_PCIE0);
        ul_val |= BIT21;
        oal_writel(ul_val, pci_rc_sys_ctl + OAL_PERI_R_PCIE0);

        ul_val = oal_readl(pci_rc_sys_ctl + OAL_PERI_W_PCIE0);
        ul_val |= BIT21;
        oal_writel(ul_val, pci_rc_sys_ctl + OAL_PERI_W_PCIE0);
    } else {
        ul_val = oal_readl(pci_rc_sys_ctl + OAL_PERI_R_PCIE1);
        ul_val |= BIT21;
        oal_writel(ul_val, pci_rc_sys_ctl + OAL_PERI_R_PCIE1);

        ul_val = oal_readl(pci_rc_sys_ctl + OAL_PERI_W_PCIE1);
        ul_val |= BIT21;
        oal_writel(ul_val, pci_rc_sys_ctl + OAL_PERI_W_PCIE1);
    }
#endif
}
...
oal_void oal_pcie_rc_aspm_enable(oal_pcie_aspm_enum mode)
{
    ...
    switch (mode) {
        case PCIE_ASPM_CAP_L0S:
            reg32 = oal_readl(pci_dbi_0 + OAL_PCIE_CAP_POS + PCI_EXP_LNKCTL);
            reg32 |= BIT0;
...
oal_void oal_pcie_rc_clkpm_set(int enable)
{
    ...
    reg32 = oal_readl(pci_dbi_0 + OAL_PCIE_CAP_POS + PCI_EXP_LNKCAP);
    if (!(reg32 & PCI_EXP_LNKCAP_CLKPM)) {
        OAL_IO_PRINT("oal_pcie_clkpm_set fail:device do not support clk pm \r\n");
        return;
    }
...
oal_void oal_pcie_rc_common_clk_set(oal_uint32 nfts)
{
    ...
    reg32 = oal_readl(pci_dbi_0 + OAL_PCIE_CAP_POS + PCI_EXP_LNKSTA);
    ...
    REG32_SETBITS(reg32, COMMON_CLK_NFTS_BIT_OFFSET, 8, nfts);
    REG32_SETBITS(reg32, ACK_NFTS_BIT_OFFSET, 8, nfts);
    oal_writel(reg32, pci_dbi_0 + PCIE_PL_ASPM_CTRL_OFFSET);
    OAL_IO_PRINT("rc 0x70c: ");
    oal_pcie_print_bits(&reg32, sizeof(reg32));

    reg32 = oal_readl(pci_dbi_0 + 0x80c);
    OAL_IO_PRINT("rc 0x80c: old");
    oal_pcie_print_bits(&reg32, sizeof(reg32));
    REG32_SETBITS(reg32, 0, 8, nfts);
    oal_writel(reg32, pci_dbi_0 + 0x80c);
...
oal_uint32 oal_pcie_rc_l1ss_set(oal_uint32 enable)
{
    ...
    reg_hdr = oal_readl(pci_dbi_0 + pos);
    while ((ttl--) > 0) {
    ...
    reg_cap = oal_readl(pci_dbi_0 + reg_hdr + PCI_L1SS_CAP);
    reg_cap &= 0x0F;
    reg_ctrl1 = oal_readl(pci_dbi_0 + reg_hdr + PCI_L1SS_CTRL1);
    if (enable) {
        oal_writel((reg_cap | reg_ctrl1), pci_dbi_0 + reg_hdr + PCI_L1SS_CTRL1);
    } else {
        oal_writel((reg_cap | 0x00), pci_dbi_0 + reg_hdr + PCI_L1SS_CTRL1);
    }
...
oal_void oal_pcie_rc_complete_timeout_set(oal_pcie_comp_timeout_enum value)
{
    ...
    oal_pcie_rc_dbi_enable(0);
    reg16 = readw(pci_dbi_0 + PCI_EXP_DEVCTL2);
    REG16_SETBITS(reg16, 0, 4, value);
    REG16_SETBITS(reg16, 10, 1, 0x1);
    writew(reg16, pci_dbi_0 + PCI_EXP_DEVCTL2);
