...
OAL_STATIC OAL_DLIST_CREATE_HEAD(hcc_bus_res_hdr);
OAL_STATIC OAL_DEFINE_SPINLOCK(hcc_bus_res_lock);
OAL_STATIC OAL_DLIST_CREATE_HEAD(hcc_bus_switch_hdr);
...
char str_ini_hcc_bus_switch[OAL_BUS_INI_STR_LEN] = {0};
...
#else
oal_dlist_head_stru hcc_bus_res_hdr;
oal_spin_lock_stru hcc_bus_res_lock;
oal_dlist_head_stru hcc_bus_switch_hdr;
char *hisi_wifi_bus_select = "";
oal_uint32 hcc_exception_enable = 1;
oal_uint32 hcc_bus_switch_bypass = 0;
oal_uint32 hcc_bus_switch_test_delay_time = 0;
oal_uint32 hcc_bus_switch_test_break = 0;
char str_ini_hcc_bus_switch[OAL_BUS_INI_STR_LEN];
oal_uint32 hcc_bus_auto_switch = 0;
oal_uint32 hcc_bus_auto_bindcpu = 0;
oal_uint32 hcc_bus_manual_bindcpu = 1;
oal_uint32 hcc_bus_current_pps = 0x0;
...
oal_uint32 switch_ip_pwrdown_bypass = 1;

#ifdef WIN32
oal_uint32 jiffies;
#else
oal_debug_module_param(switch_ip_pwrdown_bypass, uint, S_IRUGO | S_IWUSR);
#endif

oal_int32 switch_pwr_ret;
...
#else
OAL_STATIC hcc_bus_dev bus_dev_res[OAL_BUS_DEV_RES_SIZE];
#endif
...
OAL_STATIC oal_int32 lCpuId = -1;

#ifdef WIN32
char *hcc_bus_type_str[HCC_BUS_BUTT] = {
    "sdio",
    "pcie",
    "usb"
};
...
char *hcc_bus_get_bus_type_str(oal_uint32 bus_type)
{
    ...
    return hcc_bus_type_str[bus_type] ? hcc_bus_type_str[bus_type] : "uninit";
}
...
oal_int32 hcc_switch_action_register(hcc_switch_action *action, void *data)
{
    ...
    action->data = data;
    oal_dlist_add_tail(&action->list, &hcc_bus_switch_hdr);
...
oal_void hcc_bus_chan_init_from_string(hcc_bus_dev *pst_bus_dev, const char *bus_select)
{
    int len;
    ...
    oal_uint32 tmp_bus_type;

    if (OAL_STRLEN(str_ini_hcc_bus_switch) != 0) {
...
oal_int32 hcc_bus_cap_init(oal_uint32 dev_id, const char *bus_select)
{
    hcc_bus_dev *pst_bus_dev;
    ...
    if (hisi_wifi_bus_select != NULL &&
        OAL_STRLEN(hisi_wifi_bus_select) != 0) {
...
oal_int32 hcc_bus_check_dev_id(oal_uint32 dev_id)
{
    ...
    if (OAL_WARN_ON(dev_id >= (oal_uint32)OAL_ARRAY_SIZE(bus_dev_res))) {
...
hcc_bus_dev *hcc_get_bus_dev(oal_uint32 dev_id)
{
    ...
    for (i = 0; i < (oal_int32)OAL_ARRAY_SIZE(bus_dev_res); i++) {
        pst_bus_dev = &bus_dev_res[i];
...
oal_int32 hcc_foreach_switch_actions(oal_uint32 dev_id, hcc_bus *old_bus, hcc_bus *new_bus)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &hcc_bus_switch_hdr)
    {
...
hcc_bus *hcc_find_bus_by_devid(oal_uint32 dev_id, oal_uint32 bus_type)
{
    ...
    oal_spin_lock_irq_save(&hcc_bus_res_lock, &irqsave);
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &hcc_bus_res_hdr)
    {
...
OAL_STATIC oal_int32 hcc_bus_switch_thread(oal_void *data)
{
    ...
    for (;;) {
        ...
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "switch to %s: switch_total_count:%d",
                             hcc_bus_get_bus_type_str(target_bus->bus_type), ++switch_total_count);
        oal_get_time_cost_start(cost);
...
/*
 * Prototype    : hcc_message_register_etc
...
OAL_STATIC oal_void hcc_dev_res_exit_by_id(oal_int32 dev_max)
{
    oal_int32 i;
    hcc_bus_dev *pst_bus_dev = NULL;
    dev_max = OAL_MIN(dev_max, (oal_int32)OAL_ARRAY_SIZE(bus_dev_res));
    for (i = 0; i < dev_max; i++) {
        pst_bus_dev = &bus_dev_res[i];
...
{
    hcc_dev_res_exit_by_id((oal_int32)OAL_ARRAY_SIZE(bus_dev_res));
}
...
oal_int32 hcc_add_bus(hcc_bus *pst_bus, const char *bus_name)
{
    ...
    if (oal_dlist_is_empty(&hcc_bus_res_hdr) == OAL_TRUE) {
        oal_spin_unlock_irq_restore(&hcc_bus_res_lock, &irqsave);
    ...
    oal_dlist_add_tail(&pst_bus->list, &hcc_bus_res_hdr);
...
oal_int32 hcc_bus_printbindcpu(oal_void)
{
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "hcc cpu mask:0x%lx",
                         cpumask_bits(&(hcc->hcc_transer_info.hcc_transfer_thread_etc->cpus_allowed))[0]);
...
unsigned long long hcc_bus_excetpion_ssi_module_set(oal_void)
{
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "bfgx is %s  %s",
                         (bfgx_is_shutdown_etc() == true) ? "shutdown" : "poweron",
#if defined(CONFIG_LOG_EXCEPTION) && !defined(CONFIG_HI110X_KERNEL_MODULES_BUILD_SUPPORT)
...
oal_int32 hcc_bus_device_panic_callback(void *data)
{
    ...
    CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                         CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_DEVICE_PANIC);
...
struct st_wifi_dump_mem_info hi1102_meminfo[] = {
...
struct st_wifi_dump_mem_info hi1103_mpw2_meminfo[] = {
    {
        .file_name = "wifi_device_panic_mem",
...
struct st_wifi_dump_mem_info hi1103_pilot_meminfo[] = {
    {
        .file_name = "wifi_device_panic_mem",
...
#define WCPU_1105_ASIC_TICM_RAM_MAX_SIZE 0xa0000 /* 640KB */
#define WCPU_1105_ASIC_TICM_RAM_MAX_ADDR 0xb0000
/* 0x10000 */
#define WCPU_1105_ASIC_TICM_RAM_STORE_ADDR (WCPU_1105_ASIC_TICM_RAM_MAX_ADDR - WCPU_1105_ASIC_TICM_RAM_MAX_SIZE)
...
struct st_wifi_dump_mem_info hi1105_pilot_asic_meminfo[] = {
    {
        .file_name = "wifi_device_panic_mem",
    ...
    {
        .file_name = "wifi_device_itcm_ram",
        .mem_addr = WCPU_1105_ASIC_TICM_RAM_STORE_ADDR,
        .size = WCPU_1105_ASIC_TICM_RAM_MAX_SIZE, /* 640KB */
    },
    {
        .file_name = "wifi_device_dtcm_ram",
...
struct st_wifi_dump_mem_info hi1105_pilot_fpga_meminfo[] = {
    {
        .file_name = "wifi_device_panic_mem",
...
void hcc_bus_exception_handler(oal_work_stru *work)
{
    ...
    mem_info = hi1102_meminfo;
    size = sizeof(hi1102_meminfo) / sizeof(struct st_wifi_dump_mem_info);
#elif (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1103_HOST)
    if (get_hi110x_subchip_type() == BOARD_VERSION_HI1105) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "1105 %s memdump ", isAsic_etc() ? "asic" : "fpga");
        if (isAsic_etc()) {
            /* 1105 Asic */
            mem_info = hi1105_pilot_asic_meminfo;
            size = sizeof(hi1105_pilot_asic_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        } else {
            /* 1105 Fpga */
            mem_info = hi1105_pilot_fpga_meminfo;
            size = sizeof(hi1105_pilot_fpga_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        }
    } else {
        if (get_hi1103_asic_type() == HI1103_ASIC_PILOT) {
            mem_info = hi1103_pilot_meminfo;
            size = sizeof(hi1103_pilot_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        } else {
            mem_info = hi1103_mpw2_meminfo;
            size = sizeof(hi1103_mpw2_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        }
    }
#else
...
#ifdef CONFIG_PRINTK
    hwifi_panic_log_dump_etc(KERN_INFO);
...
/*
 * Prototype    : oal_register_gpio_intr_etc
...
/*
 * Prototype    : oal_unregister_gpio_intr_etc
 * Description  : initialize sdio pm interface
 */
oal_void oal_unregister_gpio_intr_etc(hcc_bus_dev *pst_bus_dev)
{
...
OAL_STATIC oal_void oal_flowctrl_gpio_intr_enable_etc(hcc_bus_dev *pst_bus_dev, oal_bool_enum_uint8 en_enable)
{
    if (en_enable)
    {
        /* 当前流控状态为关闭 */
...
OAL_STATIC irqreturn_t hcc_bus_flowctrl_gpio_irq_etc(oal_int32 l_irq, oal_void *p_dev_id)
{
    hcc_bus *pst_bus = NULL;
    hcc_bus_dev *pst_bus_dev = NULL;
...
oal_int32 oal_register_gpio_flowctrl_intr_etc(hcc_bus_dev *pst_bus_dev)
{
    oal_int32 ret = OAL_SUCC;
    BOARD_INFO *pst_board = get_hi110x_board_info_etc();
...
oal_uint32 hcc_bus_flowctrl_init_etc(oal_uint8 uc_hcc_flowctrl_type)
{
...
oal_int32 hcc_bus_resource_alloc(hcc_bus *pst_bus)
{
    ...
    pst_bus->pst_rx_tsk = oal_thread_create_etc(hcc_bus_rxdata_thread,
                                                ...
                                                name,
                                                OAL_BUS_RX_THREAD_POLICY,
                                                OAL_BUS_RXDATA_THREAD_PRIORITY, lCpuId);
...
oal_int32 hcc_dev_init(oal_void)
{
    ...
    oal_int32 ret;
    hcc_bus_dev *pst_bus_dev = NULL;
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "hcc_dev_init we had %d devs", (oal_int32)OAL_ARRAY_SIZE(bus_dev_res));
...
oal_int32 oal_load_dev_retry(oal_void)
{
    ...
    if (load_sdio == OAL_SUCC) {
        ...
        hw_1103_dsm_client_notify(SYSTEM_TYPE_PLATFORM, DSM_PCIE_SWITCH_SDIO_SUCC,
            "probe:pcie switch to sdio succ\n");
#endif
    } else {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "hisi wifi bus pcie and sdio all fail");
#ifdef CONFIG_HUAWEI_DSM
        hw_1103_dsm_client_notify(SYSTEM_TYPE_PLATFORM, DSM_PCIE_SWITCH_SDIO_FAIL,
...
/*
 * Prototype    :oal_thread_create_etc
...
oal_module_symbol(hcc_bus_flowctrl_init_etc);
