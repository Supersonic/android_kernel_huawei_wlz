...
oal_int32 oal_sdio_send_msg_etc(hcc_bus *pst_bus, oal_uint32 val)
{
    ...
    if (hi110x_get_wifi_power_stat_etc() == 0) {
...
oal_int32 sdio_dev_init_etc(struct oal_sdio *hi_sdio)
{
    int32 ret;
...
oal_int32 oal_sdio_reinit(hcc_bus *pst_bus)
{
    ...
    if (ret < 0) {
        ...
        if (wifi_sdio_fail_count >= wifi_sdio_fail_painc_limit - 1) {
            ...
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "sdio dev init failed, fail count %u , limit %u",
                                 wifi_sdio_fail_count, wifi_sdio_fail_painc_limit);
        }
#endif
...
/*
 * Prototype    : oal_sdio_data_sg_irq_etc(struct oal_sdio *hi_sdio)
...
/*
 * Prototype    : oal_sdio_msg_irq_etc
 * Description  : msg irq
 */
...
oal_void oal_sdio_isr_etc(struct sdio_func *func)
{
    ...
    do {
        ...
        if (OAL_UNLIKELY(ret)) {
            ...
            CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                                 CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_INIT_ISR);
        }
        oal_sdio_release_host(hi_sdio);
        oal_sdio_claim_host(hi_sdio);
...
oal_int32 oal_sdio_rxdata_proc(struct oal_sdio *hi_sdio)
{
    ...
    if (OAL_UNLIKELY(ret)) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_INIT_RX_DATA_PROC);
    }
    oal_sdio_release_host(hi_sdio);
...
/*
 * Prototype    : oal_sdio_interrupt_register_etc
...
/*
 * Prototype    : oal_sdio_interrupt_unregister_etc
...
/*
 * Prototype    : oal_sdio_get_dev_pm_state_etc
 * Description  : get device low power state by sdio 0xf1~0xf4 registers for debug
 */
oal_void oal_sdio_get_dev_pm_state_etc(struct oal_sdio *hi_sdio, oal_uint *pst_ul_f1, oal_uint *pst_ul_f2,
...
/*
 * Prototype    : oal_sdio_dev_init_etc
...
failed_sdio_alloc:
    ...
    CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                         CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_INIT_PROB_FAIL);
...
oal_int32 oal_mmc_io_rw_scat_extended_etc(struct oal_sdio *hi_sdio,
                                          oal_int32 write,
                                          oal_uint32 fn,
                                              ...
                                              oal_uint32 rw_sz)
{
    ...
    if (OAL_UNLIKELY(ret)) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_SDIO_WR_FAIL);
    }
    oal_sdio_release_host(hi_sdio);
    return ret;
}
...
/*
 * Prototype    : oal_sdio_transfer_scatt_etc
                                      ...
                                      oal_uint32 rw_sz)
{
    ...
    OAL_WARN_ON((rw_sz < HISDIO_BLOCK_SIZE) && (rw_sz & (sizeof(oal_uint32) - 1)));
...
void oal_sdio_tc_buf_tx_001_etc(void)
{
    ...
    ret = oal_sdio_single_transfer(hi_sdio_debug, SDIO_WRITE, buf, SDIO_TC_BUF_TX_LEN);
    if (ret) {
...
void oal_sdio_tc_mem_cp_from_etc(unsigned int offset, unsigned int len)
{
    int ret;
    char *buf = NULL;
...
OAL_STATIC oal_int32 oal_sdio_gpio_flowctrl_irq_etc(hcc_bus *pst_hi_bus, oal_int32 l_irq)
{
    oal_int32 l_gpio_val;
    struct hcc_handler *pst_hcc = NULL;

    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_hi_bus))
    {
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_hcc))
    {
    ...
    /* 获取流控GPIO管脚当前电平 */
    l_gpio_val = oal_gpio_get_value(board_info_etc.flowctrl_gpio);

    if (1 == l_gpio_val)
    {
        /* 高电平, 流控开启, 无法发送 */
        hcc_dev_flowctrl_off_etc(pst_hcc);
    }
    else if (0 == l_gpio_val)
    {
        /* 低电平, 流控关闭, 可以发送 */
        hcc_dev_flowctrl_on_etc(pst_hcc, OAL_FALSE);
    }
    else
    {
...
OAL_STATIC hcc_bus_opt_ops sdio_opt_ops = {
    ...
    .flowctrl_gpio_handler = oal_sdio_gpio_flowctrl_irq_etc,
...
oal_int32 oal_wifi_platform_load_sdio(oal_void)
{
    ...
    if (oal_sdio_trigger_probe() != OAL_SUCC) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DEV_EVENT_CHIP, CHR_WIFI_DEV_ERROR_SDIO_ENUM);
