...
oal_completion probe_complete;    /* 初始化信号量 */
OAL_VOLATILE oal_int32 probe_ret; /* probe 返回值 */

#ifdef CONFIG_ARCH_SD56XX
oal_void *pcie_sys_ctrl = NULL;
#endif

oal_int32 pcie_enum_fail_reg_dump_flag = 0;
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
oal_debug_module_param(hipci_msi_enable, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(hipci_gen_select, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(ft_pcie_aspm_check_bypass, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(ft_pcie_gen_check_bypass, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(hipcie_probe_fail_powerdown_bypass, int, S_IRUGO | S_IWUSR);
#endif

#ifdef CONFIG_ARCH_KIRIN_PCIE
oal_int32 kirin_rc_idx = 0;
oal_debug_module_param(kirin_rc_idx, int, S_IRUGO | S_IWUSR);
#endif

oal_int32 pcie_print_once = 0;
oal_debug_module_param(pcie_print_once, int, S_IRUGO | S_IWUSR);

oal_int32 pcie_aspm_enable = 1;
oal_debug_module_param(pcie_aspm_enable, int, S_IRUGO | S_IWUSR);
...
module_param(pcie_shutdown_panic, int, S_IRUGO | S_IWUSR);
#endif
/* Function Declare */
...
OAL_STATIC OAL_CONST oal_pci_device_id_stru pcie_id_table[] = {
    { 0x19e5, 0x1103, PCI_ANY_ID, PCI_ANY_ID }, /* 1103 PCIE */
    { 0x19e5, 0x1105, PCI_ANY_ID, PCI_ANY_ID }, /* 1105 PCIE */
    {
        0,
    }
};
...
oal_int32 oal_pcie_enable_device(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    if (OAL_WARN_ON(pst_pci_lres->default_state == NULL)) {
...
oal_int32 oal_pcie_save_default_resource(oal_pcie_linux_res *pst_pci_lres)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))
    if (pst_pci_lres->default_state != NULL) {
    ...
    if (OAL_UNLIKELY(pst_pci_lres->default_state == NULL)) {
        oal_pci_disable_device(pst_pci_lres->pst_pcie_dev);
        return -OAL_ENOMEM;
    }
#endif
...
irqreturn_t oal_pcie_intx_isr(int irq, void *dev_id)
{
    ...
    if (OAL_UNLIKELY(oal_pcie_transfer_done(pst_pci_lres->pst_pci_res) < 0)) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_INTX_ISR_PCIE_LINK_DOWN);
        /* 关闭低功耗 */
    }
    return IRQ_HANDLED;
}

oal_void oal_pcie_intx_task(uintptr_t data)
{
...
{
    OAL_STATIC oal_uint32 linkdown_count = 0;

    if (pcie_sys_ctrl != NULL) {
        /* Clear linkdown Int. */
        OAL_IO_PRINT("int[0xe8]=0x%8x, [0x38=0x%8x]\n",
                     oal_readl(pcie_sys_ctrl + 0xe8), oal_readl(pcie_sys_ctrl + 0x38));
        oal_writel(0xffffffff, (pcie_sys_ctrl + 0xe8));
        oal_writel(0x0, (pcie_sys_ctrl + 0xe8));
    } else {
...
OAL_STATIC oal_void oal_pcie_linkdown_callback(struct kirin_pcie_notify *noti)
{
    ...
    if (board_get_wlan_wkup_gpio_val_etc() == 0) {
        ...
        hw_1103_dsm_client_notify(SYSTEM_TYPE_PLATFORM, DSM_WIFI_PCIE_LINKDOWN, "%s: pcie linkdown\n", __FUNCTION__);
#endif
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_WKUP_GPIO_PCIE_LINK_DOWN);
    } else {
...
OAL_STATIC oal_int32 oal_pcie_probe(oal_pci_dev_stru *pst_pci_dev, OAL_CONST oal_pci_device_id_stru *pst_id)
{
    ...
    memset_s((oal_void *)pst_pci_lres, sizeof(oal_pcie_linux_res), 0, sizeof(oal_pcie_linux_res));
...
#ifdef CONFIG_ARCH_SD56XX
    pcie_sys_ctrl = oal_ioremap_nocache(0x10100000, 0x1000);
...
#ifdef CONFIG_ARCH_KIRIN_PCIE
OAL_STATIC oal_int32 oal_pcie_device_wakeup_handler(const void *data)
{
    OAL_REFERENCE(data);
...
OAL_STATIC oal_int32 oal_pcie_device_wake_powerup_handler(void *data)
{
    ...
    const oal_uint32 ul_max_retry_cnt = 3;
...
retry:
    ...
    oal_atomic_set(&bus_powerup_dev_wait_ack, 1);
    /* 这里保证解复位EP控制器时efuse已经稳定 */
    board_host_wakeup_dev_set(1);
    if (pst_wlan_pm != NULL) {
        ul_ret = oal_wait_for_completion_timeout(&pst_wlan_pm->st_wifi_powerup_done,
                                                 (oal_uint32)OAL_MSECS_TO_JIFFIES(2000));
    ...
    oal_atomic_set(&bus_powerup_dev_wait_ack, 0);
...
OAL_STATIC oal_int32 oal_pcie_resume(oal_pci_dev_stru *pst_pci_dev)
{
    ...
    if (ret) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
                             CHR_WIFI_DRV_EVENT_PLAT, CHR_PLAT_DRV_ERROR_RESUME_FIRMWARE_DOWN);
        return OAL_SUCC;
    }
...
OAL_STATIC oal_void oal_pcie_shutdown(oal_pci_dev_stru *pst_pcie_dev)
{
    ...
    if (pcie_shutdown_panic) {
        ...
        if (time_cost_var_sub(cost) > (oal_uint64)(oal_uint32)pcie_shutdown_panic) {
...
OAL_STATIC oal_pci_driver_stru pcie_drv = {
    .name = "hi110x_pci",
...
{
    return (qtype == HCC_NETBUF_HIGH_QUEUE) ? PCIE_H2D_QTYPE_HIGH : PCIE_H2D_QTYPE_NORMAL;
}
...
#else
        oal_print_hi11xx_log(HI11XX_LOG_DBG, "pcie bindcpu fail, cpu nums:%d irq =%u",
                             CONFIG_NR_CPUS, pst_pci_dev->irq);
#endif
#endif
...
#define TRY_MAX 2000000
    ...
    for (;;) {
        ...
        for (loop = 0; loop < TRY_MAX; loop++) {
            ...
            if (link_up_stable_counter >= OAL_CONTINUOUS_LINK_UP_SUCC_TIMES) {
                break;
            }
            val = readl(pcie_sys_base_virt + 0x18);
        }
...
OAL_STATIC oal_int32 oal_pcie_power_ctrl(hcc_bus *hi_bus, HCC_BUS_POWER_CTRL_TYPE ctrl,
                                         int (*func)(void *data), oal_void *data)
{
...
oal_int32 oal_pcie_enable_device_func(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    for (i = 0; i < retry; i++) {
        ...
        if (oal_wait_for_completion_timeout(&pst_pci_lres->st_pcie_ready,
            ...
            up(&pst_bus->rx_sema);
            if (oal_wait_for_completion_timeout(&pst_pci_lres->st_pcie_ready,
                                                (oal_uint32)OAL_MSECS_TO_JIFFIES(5000)) == 0) {
...
OAL_STATIC oal_int32 oal_pcie_mps_init(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    mps = (oal_uint32)OAL_MIN(rc_mps, ep_mps);
    mrq = mps;
...
OAL_STATIC oal_int32 oal_pcie_host_ip_init(oal_pcie_linux_res *pst_pci_lres)
{
    /* rc bar init */
#ifdef CONFIG_ARCH_KIRIN_PCIE
    if ((pst_pci_lres->pst_pci_res->revision == PCIE_REVISION_5_00A) && (pcie_rc_bar_bypass)) {
        oal_pci_dev_stru *pst_rc_dev;
        pst_rc_dev = pci_upstream_bridge(pst_pci_lres->pst_pcie_dev);
        if (pst_rc_dev == NULL) {
            PCI_PRINT_LOG(PCI_LOG_ERR, "no upstream dev");
...
oal_void oal_pcie_print_chip_info(oal_pcie_linux_res *pst_pci_lres, oal_uint32 is_full_log)
{
    /* L1 recovery count */
    oal_uint32 l1_err_cnt = oal_readl(pst_pci_lres->pst_pci_res->pst_pci_ctrl_base +
                                      PCIE_STAT_CNT_LTSSM_ENTER_RCVRY_OFF);
    oal_uint32 l1_recvy_cnt = oal_readl(pst_pci_lres->pst_pci_res->pst_pci_ctrl_base +
                                        PCIE_STAT_CNT_L1_ENTER_RCVRY_OFF);
    const oal_uint32 l1_max_err_cnt = 3;
...
oal_int32 oal_pcie_gen_mode_check(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "current link is %s",
                         (gen_select == PCIE_GEN2) ? "GEN2" : "GEN1");
...
oal_void oal_pcie_power_down(hcc_bus *pst_bus)
{
    ...
    /* 下电之前关闭 PCIE HOST 控制器 */
    {
        ...
        ret = kirin_pcie_pm_control(2, kirin_rc_idx);/*power off without L3 message*/
#else
        ret = kirin_pcie_pm_control(0, kirin_rc_idx);/*power off with L3 message*/
#endif
...
OAL_STATIC oal_int32 oal_pcie_power_action(hcc_bus *pst_bus, HCC_BUS_POWER_ACTION_TYPE action)
{
    ...
    if (action == HCC_BUS_POWER_PATCH_LAUCH) {
        ...
        if (pcie_ringbuf_bugfix_enable == 0) {
...
OAL_STATIC oal_int32 oal_pcie_gpio_irq(hcc_bus *hi_bus, oal_int32 irq)
{
    ...
    if (oal_atomic_read(&bus_powerup_dev_wait_ack)) {
...
OAL_STATIC hcc_bus_opt_ops pcie_opt_ops = {
...
oal_int32 wlan_first_power_off_fail_callback(void *data)
{
    ...
    pcie_enum_fail_reg_dump_flag = 1;
    if (hi11xx_get_os_build_variant() == HI1XX_OS_BUILD_VARIANT_USER) {
...
{
    char *token = NULL;
    const oal_uint32 bias_param_size = 100;
    const oal_uint32 param_buff_size = 10;
    char bias_param[bias_param_size];
    oal_uint32 param[param_buff_size];
    ...
    if (get_cust_conf_string_etc(INI_MODU_PLAT, "pcie_vol_bias_param", bias_param, sizeof(bias_param) - 1) ==
        INI_FAILED) {
        return;
    }
...
{
    char buff[100]; /* 因编译原因，暂不支持用const变量定义数组大小 */

    memset_s(buff, sizeof(buff), 0, sizeof(buff));

    if (get_cust_conf_string_etc(INI_MODU_PLAT, "pcie_ringbuf_bugfix", buff, sizeof(buff) - 1) == INI_FAILED) {
...
oal_int32 oal_pcie_dts_init(oal_void)
{
...
oal_int32 oal_pcie_110x_init(oal_void)
{
    ...
    OAL_INIT_COMPLETION(&probe_complete);

    probe_ret = -OAL_EFAUL;

    pcie_enum_fail_reg_dump_flag = 0;

    oal_pcie_dts_init();

    ret = oal_pci_register_driver(&pcie_drv);
...
#ifdef CONFIG_ARCH_KIRIN_PCIE
    /* 打开参考时钟 */
    ret = kirin_pcie_power_notifiy_register(kirin_rc_idx, wlan_first_power_on_callback,
                                            wlan_first_power_off_fail_callback, NULL);
    if (ret != OAL_SUCC) {
        OAL_IO_PRINT("enumerate_prepare failed ret=%d\n", ret);
        oal_pci_unregister_driver(&pcie_drv);
        return ret;
    }

    ret = oal_pcie_enumerate();
    if (ret != OAL_SUCC) {
        if (!pcie_enum_fail_reg_dump_flag) {
            if (hi11xx_get_os_build_variant() == HI1XX_OS_BUILD_VARIANT_USER) {
                if (oal_print_rate_limit(PRINT_RATE_MINUTE)) { /* 1分钟打印一次 */
                    ssi_dump_device_regs(SSI_MODULE_MASK_ARM_REG | SSI_MODULE_MASK_AON_CUT | SSI_MODULE_MASK_PCIE_CUT);
                }
    ...
    if (oal_wait_for_completion_timeout(&probe_complete, 10 * HZ)) {
        if (probe_ret != OAL_SUCC) {
            OAL_IO_PRINT("pcie driver probe failed ret=%d, driname:%s\n", probe_ret, pcie_drv.name);
#ifdef CONFIG_ARCH_KIRIN_PCIE
            kirin_pcie_power_notifiy_register(kirin_rc_idx, NULL, NULL, NULL);
#endif
            return probe_ret;
...
#ifdef CONFIG_ARCH_SD56XX
    oal_pcie_save_default_resource(pcie_linux_res);
#else
    /* Power off Wlan Chip */
    if (pcie_linux_res) {
...
#endif
        oal_disable_pcie_irq(pcie_linux_res);
        oal_pcie_power_action(pcie_linux_res->pst_bus, HCC_BUS_POWER_DOWN);
