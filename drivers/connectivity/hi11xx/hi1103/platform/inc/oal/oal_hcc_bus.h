...
/* 全局变量声明 */
extern oal_atomic wakeup_dev_wait_ack_etc;
...
struct _hcc_bus_dev_ {
    ...
    oal_int32                   l_flowctrl_irq;             /* gpio irq for hcc non-high priority flowctrl */
...
extern oal_atomic bus_powerup_dev_wait_ack;
extern oal_uint32 hcc_bus_flowctrl_init_etc(oal_uint8 uc_hcc_flowctrl_type);
...
extern oal_int32 hcc_bus_cap_init(oal_uint32 dev_id, const char *bus_select);
...
extern oal_int32 oal_register_gpio_flowctrl_intr_etc(hcc_bus_dev *pst_bus_dev);
extern oal_void oal_unregister_gpio_flowctrl_intr_etc(hcc_bus_dev *pst_bus_dev);
...
{
    oal_int32 ret;
    const oal_uint32 ul_long_delay_tx_err_times = 2;
    ...
    if (OAL_LIKELY(hi_bus->opt_ops->tx_netbuf_list)) {
        ...
        if (OAL_UNLIKELY(ret < 0)) {
            hcc_tx_err_cnt++;
            if (hcc_tx_err_cnt >= ul_long_delay_tx_err_times) {
                oal_msleep(1000);
            } else {
                oal_msleep(100);
            }
