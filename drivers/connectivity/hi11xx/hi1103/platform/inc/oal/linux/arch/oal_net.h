...
typedef struct cfg80211_crypto_settings {
    ...
    oal_uint8 auc_arry[OAL_NL80211_MAX_NR_ARRY_LEN];
 ...
 *           : p_set_up:启动函数指针
 */
OAL_STATIC OAL_INLINE oal_net_device_stru *oal_net_alloc_netdev(oal_uint32 ul_sizeof_priv, const char *puc_name,
                                                                oal_void *p_set_up)
{
 ...
 *           : p_set_up:启动函数指针
 */
OAL_STATIC OAL_INLINE oal_net_device_stru *oal_net_alloc_netdev_mqs(oal_uint32 ul_sizeof_priv, const char *puc_name,
...
OAL_STATIC OAL_INLINE oal_int32 oal_net_register_netdev(oal_net_device_stru *p_net_device)
{
    ...
    const oal_uint16 us_needed_headroom_size = 76;
#else
    const oal_uint16 us_needed_headroom_size = 64;
#endif
    const oal_uint16 us_needed_tailroom_size = 32;
#endif
...
OAL_STATIC OAL_INLINE oal_netbuf_stru *oal_netbuf_alloc_skpool(oal_uint32 ul_size, oal_int32 l_reserve,
                                                               oal_int32 l_align)
{
 ...
 *             为了使接口统一，在oal_netbuf_free/oal_netbuf_free_any中调用
 */
...
OAL_STATIC OAL_INLINE oal_uint32 oal_notice_sta_join_result(oal_uint8 uc_chip_id, oal_bool_enum_uint8 en_succ)
{
    ...
    ifIndex = uc_chip_id;
    link_status = (en_succ == OAL_TRUE) ? WLAN_UPMODE_UP : WLAN_UPMODE_DOWN;
    if (g_pf_wifi_report(ifIndex, HW_KER_WIFI_EVENT_CLIENT_LINK_STATUS, &link_status, sizeof(int)) != HW_RET_SUCCESS) {
        ul_ret = OAL_FAIL;
    }
#endif
...
{
#if (_PRE_TARGET_PRODUCT_TYPE_ONT == _PRE_CONFIG_TARGET_PRODUCT)
    if (g_wifi_80211_mirror_pkt == OAL_PTR_NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
...
#if (_PRE_TARGET_PRODUCT_TYPE_ONT == _PRE_CONFIG_TARGET_PRODUCT)
/*
 * 函 数 名  : oal_net_huawei_ie_report
 * 功能描述  : 将华为私有IE上报到上层
 * 输入参数  : pst_ie_buf: ie指针
 */
...
#ifdef _PRE_WLAN_FEATURE_HILINK
/*
 * 函 数 名  : oal_unconnect_sta_report
 * 功能描述  : 将未关联sta信息上报到上层
 * 输入参数  : pst_ie_buf: ie指针
 */
