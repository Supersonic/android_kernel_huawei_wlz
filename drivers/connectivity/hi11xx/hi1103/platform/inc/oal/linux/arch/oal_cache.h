...
/* 全局变量声明 */
extern void __iomem *l2cache_base_etc;
...
OAL_STATIC OAL_INLINE oal_void oal_l2cache_dcache_lock(oal_bool_enum_uint8 en_flag, oal_uint32 ul_way)
{
    oal_uint32 ul_reg;
     ...
     (_PRE_TARGET_PRODUCT_TYPE_VSPM310DMB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_WS835DMB == _PRE_CONFIG_TARGET_PRODUCT))
    ul_reg = readl(l2cache_base_etc + REG_L2_DLOCKWAY);
#else
    ul_reg = readl(l2cache_base_etc + HI_REG_L2_DLOCKWAY);
#endif
    ...
    ul_reg = (ul_reg & (~(1 << ul_way))) | (en_flag << ul_way);
     ...
     (_PRE_TARGET_PRODUCT_TYPE_VSPM310DMB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_WS835DMB == _PRE_CONFIG_TARGET_PRODUCT))
    writel(ul_reg, l2cache_base_etc + REG_L2_DLOCKWAY);
#else
    writel(ul_reg, l2cache_base_etc + HI_REG_L2_DLOCKWAY);
#endif
...
OAL_STATIC OAL_INLINE oal_void oal_l2cache_icache_lock(oal_bool_enum_uint8 en_flag, oal_uint32 ul_way)
{
    oal_uint32 ul_reg;
     ...
     (_PRE_TARGET_PRODUCT_TYPE_VSPM310DMB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_WS835DMB == _PRE_CONFIG_TARGET_PRODUCT))
    ul_reg = readl(l2cache_base_etc + REG_L2_ILOCKWAY);
#else
    ul_reg = readl(l2cache_base_etc + HI_REG_L2_ILOCKWAY);
#endif
    ul_reg = (ul_reg & (~(1 << ul_way))) | (en_flag << ul_way);
     ...
     (_PRE_TARGET_PRODUCT_TYPE_VSPM310DMB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_WS835DMB == _PRE_CONFIG_TARGET_PRODUCT))
    writel(ul_reg, l2cache_base_etc + REG_L2_ILOCKWAY);
#else
    writel(ul_reg, l2cache_base_etc + HI_REG_L2_ILOCKWAY);
#endif
    return;
}
...
OAL_STATIC OAL_INLINE oal_void oal_cache_rxtx_lock(oal_void)
{
     ...
     (_PRE_TARGET_PRODUCT_TYPE_VSPM310DMB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_WS835DMB == _PRE_CONFIG_TARGET_PRODUCT))
    l2cache_base_etc = (void __iomem *)ioremap(REG_BASE_L2CACHE, OAL_SIZEOF(void *));
#else
    l2cache_base_etc = (void __iomem *)ioremap(HI_REG_BASE_L2CACHE, OAL_SIZEOF(void *));
#endif
}
#endif
