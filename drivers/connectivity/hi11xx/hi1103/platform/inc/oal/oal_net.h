...
/* is address mcast? */
#define ETHER_IS_MULTICAST(_a) ((oal_uint8)(*(_a))&0x01)
...
typedef struct oal_vap_status_diag_info {
    ...
    oal_uint8 auc_hisi_cca_shreshold[OAL_VAP_CCA_LEN];  // 待确认含义   3.10.1
    ...
    /* 信道繁忙度:(信道测量时间 - 信道发送时间 - 信道空闲时间 - 节点自身接收时间)*1000 / 信道测量时间 */
...
typedef struct oal_sta_status_diag_info {
    ...
    oal_uint8 auc_chip_vendor[OAL_STA_CHIP_VENDOR_LEN]; /* 芯片厂家代码: */
    ...
    oal_uint32 aul_sta_tx_mcs_cnt[OAL_STA_TX_MCS_LEN]; /* how many tx mcs count 两次读取间隔内的每个MCS统计值,读清0 */
    oal_uint32 aul_sta_rx_mcs_cnt[OAL_STA_RX_MCS_LEN]; /* how many rx mcs count 两次读取间隔内的每个MCS统计值,读清0 */
