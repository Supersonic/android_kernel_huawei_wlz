...
extern frw_event_table_item_stru event_table_etc[FRW_EVENT_TABLE_MAX_ITEMS];
...
extern oal_void frw_event_deploy_register_etc(oal_uint32 (*p_func)(frw_event_mem_stru *pst_event_mem,
...
extern frw_event_sched_queue_stru *frw_event_get_sched_queue_etc(oal_uint32 ul_core_id,
...
extern oal_uint32 frw_event_vap_flush_event_etc(oal_uint8 uc_vap_id, frw_event_type_enum_uint8 en_event_type,
                                                       ...
                                                       oal_uint32 ul_core_id)
{
    ...
    event_manager_etc[ul_core_id].pst_frw_trace->pst_func_name = pst_func_name;
    event_manager_etc[ul_core_id].pst_frw_trace->line_num = line_num;
}
...
OAL_STATIC OAL_INLINE oal_void frw_event_trace(frw_event_mem_stru *pst_event_mem, oal_uint32 ul_core_id)
{
    ...
    ul_pos = event_manager_etc[ul_core_id].pst_frw_trace->ul_current_pos;
    ...
    pst_trace_item = &(event_manager_etc[ul_core_id].pst_frw_trace->st_trace_item[ul_pos]);
    ...
    if ((++ul_pos) >= CONFIG_FRW_MAX_TRACE_EVENT_NUMS) {
        event_manager_etc[ul_core_id].pst_frw_trace->ul_current_pos = 0;
        event_manager_etc[ul_core_id].pst_frw_trace->ul_over_flag = 1;
    } else {
        event_manager_etc[ul_core_id].pst_frw_trace->ul_current_pos++;
    }
}
#endif

#ifdef _PRE_OAL_FEATURE_TASK_NEST_LOCK
extern oal_task_lock_stru event_task_lock_etc;
#define frw_event_task_lock()                    \
    do {                                         \
        oal_smp_task_lock(&event_task_lock_etc); \
    } while (0)
#define frw_event_task_unlock()                    \
    do {                                           \
        oal_smp_task_unlock(&event_task_lock_etc); \
    } while (0)
#else
#define frw_event_task_lock() \
    do {                      \
