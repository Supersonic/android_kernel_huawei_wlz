			...
			if (ret == 0) {
				...
				if (time_after(time, timeout)) {
					dev_err(&pl022->adev->dev,
						"get hardware_mutex wait for complete timeout\n");
					return -ETIME;
				}
...
int spi_init_secos(unsigned int spi_bus_id)
{
	...
	if (spi_smc_flag[spi_bus_id]) {
		...
		if (!master) {
			pr_err("secos get spi master:%d wrong\n", spi_bus_id);
			return -EINVAL;
		}

		pl022 = spi_master_get_devdata(master);
		if (!pl022) {
			pr_err("secos pl022:%d is NULL!\n", spi_bus_id);
			return -EINVAL;
		}

		if (!pl022->clk) {
			dev_err(&pl022->adev->dev,
				"secos pl022->clk is NULL for spi%d!\n",
				spi_bus_id);
			return -EINVAL;
		}
		...
		if (ret) {
			dev_err(&pl022->adev->dev,
				"secos can not enable SPI%d bus clock\n",
				spi_bus_id);
			ret = -EINVAL;
		} else {
...
int spi_exit_secos(unsigned int spi_bus_id)
{
	...
	if (spi_smc_flag[spi_bus_id]) {
		master = spi_busnum_to_master(spi_bus_id);
		if (!master) {
			pr_err("kernel get spi master:%d wrong\n", spi_bus_id);
			return -EINVAL;
		}

		pl022 = spi_master_get_devdata(master);
		if (!pl022) {
			pr_err("kernel pl022:%d is NULL!\n", spi_bus_id);
			return -EINVAL;
		}

		if (!pl022->clk) {
			dev_err(&pl022->adev->dev,
				"kernel pl022->clk is NULL for spi:%d\n",
				spi_bus_id);
			return -EINVAL;
		}
...
static void hisi_spi_txrx_buffer_check(
	struct pl022 *pl022, struct spi_transfer *transfer)
{
	if (virt_to_phys(pl022->cur_transfer->tx_buf) > SPI_4G_PHYS_ADDR) {
		...
		if (pl022->tx_buffer != NULL) {
			memcpy(pl022->tx_buffer, transfer->tx_buf,
...
{
	if (pl022->rx_buffer != NULL) {
		memcpy(pl022->cur_transfer->rx_buf, pl022->rx,
			pl022->cur_transfer->len);
