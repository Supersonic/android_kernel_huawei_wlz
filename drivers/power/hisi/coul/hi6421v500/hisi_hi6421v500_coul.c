...
static unsigned int last_eco_in;
static unsigned int last_eco_out;
...
void hi6421v500_coul_set_nv_save_flag(int nv_flag)
{
	...
	if (NV_SAVE_SUCCESS == nv_flag) {
		HI6421V500_REG_WRITE(
			HI6421V500_NV_SAVE_SUCCESS, (val | NV_SAVE_BITMASK));
	} else {
		HI6421V500_REG_WRITE(
			HI6421V500_NV_SAVE_SUCCESS, (val & (~NV_SAVE_BITMASK)));
	}
...
int hi6421v500_coul_get_delta_rc_ignore_flag(void)
{
	...
	if (use_delta_rc_flag & DELTA_RC_SCENE_BITMASK) {
		...
		HI6421V500_REG_WRITE(
			HI6421V500_DELTA_RC_SCENE, use_delta_rc_flag);
		return 1;
	}
	return 0;
}
...
int hi6421v500_coul_is_battery_moved(void)
{
	...
	if (val == BATTERY_MOVE_MAGIC_NUM) {
		...
		HI6421V500_REG_WRITE(
			HI6421V500_BATTERY_MOVE_ADDR, BATTERY_MOVE_MAGIC_NUM);
		return 1;
	}
}
...
/*******************************************************
  ...
  Remark:        1 bit = 225.1196 uv = 4.8/21322 V = 4.8/21322 * 1000 * 1000 uV
...
/*******************************************************
  ...
  Remark:       1bit = 1bit current * 0.11 c = 5/10661 * 11/100 c = 5/10661 *
11/100 * 1000/3600 mAh
...
static int hi6421v500_coul_calculate_cc_uah(void)
{
	...
	HI6421V500_COUL_INF("ccout_reg=0x%x,ccin_reg = 0x%x, cl_out_time=%d, "
			    "cl_in_time=%d, cc_adjust=%d uah\n",
			    (unsigned int)cc_out, (unsigned int)cc_in, cl_out_time,
			    cl_in_time, cc_uah);
...
int hi6421v500_coul_get_battery_vol_uv_from_fifo(unsigned int fifo_order)
{
	...
	HI6421V500_REGS_READ(
		(HI6421V500_VOL_FIFO_BASE + 2 * fifo_order), &regval, 2);
	return (hi6421v500_coul_convert_regval2uv(regval));
}
...
int hi6421v500_coul_get_battery_cur_ua_from_fifo(unsigned int fifo_order)
{
	...
	HI6421V500_REGS_READ(
		(HI6421V500_CUR_FIFO_BASE + 2 * fifo_order), &regval, 2);
	return (hi6421v500_coul_convert_regval2ua(regval));
}
...
/*******************************************************
  Function:      hi6421v500_coul_get_offset_vol_mod
  Description:   get vol offset mod
  Intput:        NULL
  Output:        NULL
  Return:        vol offset mod value
********************************************************/
short hi6421v500_coul_get_offset_vol_mod(void)
{
	short regval;

	HI6421V500_REGS_READ(HI6421V500_OFFSET_VOL_MODIFY_BASE, &regval, 2);
	return regval;
}

/*******************************************************
  Function:      hi6421v500_coul_set_offset_vol_mod
  Description:   set vol offset mod
  Intput:        NULL
  Output:        NULL
  Return:        vol offset mod value
********************************************************/
void hi6421v500_coul_set_offset_vol_mod(void)
{
	short regval = 0;

	HI6421V500_REGS_WRITE(HI6421V500_OFFSET_VOL_MODIFY_BASE, &regval, 2);
}
/*******************************************************
...
{
	int i;
	short vol_reset_value = 0xffff; /*lint !e604*/
	...
	for (i = 0; i < FIFO_DEPTH; i++) {
		HI6421V500_REGS_READ(
			HI6421V500_VOL_FIFO_BASE + i * 2, &vol_fifo[i], 2);
		HI6421V500_REGS_READ(
			HI6421V500_CUR_FIFO_BASE + i * 2, &cur_fifo[i], 2);
	}
	...
	for (i = 1; i < FIFO_DEPTH; i++) {
		if (vol_fifo[i] != vol_reset_value) {
			vol = hi6421v500_coul_convert_regval2uv(vol_fifo[i]) /
			      1000;
			cur = hi6421v500_coul_convert_regval2ua(cur_fifo[i]) /
			      1000;
...
static int hi6421v500_coul_get_ate_a(void)
{
	...
	regval = (a_low | ((a_high & VOL_OFFSET_A_HIGH_VALID_MASK) << 1)) &
...
static int hi6421v500_coul_check_version(void)
{
	...
	do {
		HI6421V500_REGS_READ(
			HI6421V500_COUL_VERSION_ADDR, &version_reg, 1);
		HI6421V500_COUL_INF(
			"do a dummy read, version is 0x%x\n", version_reg);
...
static void hi6421v500_coul_enable_irq(void)
{
	unsigned char irq_enable_flag =
		~((unsigned char)(COUL_VBAT_INT_MASK | COUL_CL_IN_MASK |
				  COUL_CL_OUT_MASK)); /* disable cl_int
							 interrupt */
...
{
	char irq_disable_flag = (COUL_VBAT_INT_MASK | COUL_CL_IN_MASK |
...
void hi6421v500_coul_show_key_reg(void)
{
	...
	HI6421V500_COUL_INF(
		"\n"
		"0x1af(state)        = 0x%x, 0x179-0x17a(fifo0) = 0x%x,  "
		"0x17b-0x17c(fifo1) = 0x%x\n"
		"0x16b-0x16c(offset) = 0x%x, 0x151(ctrl)        = 0x%x,  "
		"0x152(reflash)     = 0x%x\n"
...
{
	struct hi6421v500_coul_device_info *di = container_of(
				work, struct hi6421v500_coul_device_info, irq_work.work);
...
static int hi6421v500_coul_calculate_eco_leak_uah(void)
{
	...
	if ((last_eco_in != in_val) || (last_eco_out != out_val)) {
		...
		HI6421V500_COUL_ERR("eco_in=%d,eco_out=%d,present_in=%d, "
				    "present_out=%d,leak cc=%d .\n",
				    eco_in_uah, eco_out_uah, present_in_uah,
				    present_out_uah, rst_uah);
	} else {
		rst_uah = 0;
		HI6421V500_COUL_INF("Not the FIRST time to calc soc after "
				    "exiting from ECO Model, leak cc=0 .\n");
	}
	return rst_uah;
}
...
static void hi6421v500_coul_clear_fifo(void)
{
	...
	HI6421V500_REG_WRITE(
		HI6421V500_FIFO_CLEAR, (reg_value | COUL_FIFO_CLEAR));
}
/*******************************************************
...
static void hi6421v500_coul_enter_eco(void)
{
	...
	HI6421V500_REGS_READ(
		HI6421V500_ECO_OUT_CLOUT_REG_BASE, &last_eco_out, 4);
...
int hi6421v500_coul_set_hltherm_flag(int temp_protect_flag)
{
	...
	if (1 == temp_protect_flag) {
		HI6421V500_REG_WRITE(HI6421V500_COUL_TEMP_PROTECT,
...
static void hi6421v500_coul_save_last_soc(short soc)
{
	...
	HI6421V500_REG_WRITE(HI6421V500_SAVE_LAST_SOC,
...
static void hi6421v500_coul_get_last_soc_flag(bool *valid)
{
	...
	val = SAVE_LAST_SOC_FLAG &
	      HI6421V500_REG_READ(HI6421V500_SAVE_LAST_SOC);
	*valid = val;
}

static void hi6421v500_coul_auto_cali(void)
{
	return;
}

static void hi6421v500_coul_save_ocv_level(u8 level)
{
	u8 val;

	val = HI6421V500_REG_READ(HI6421V500_OCV_LEVEL_ADDR);
	val &= ~SAVE_OCV_LEVEL;
	val |= ((level << OCV_LEVEL_SHIFT) & SAVE_OCV_LEVEL);
	HI6421V500_REG_WRITE(HI6421V500_OCV_LEVEL_ADDR, val);
}

static void hi6421v500_coul_get_ocv_level(u8 *level)
{
	u8 val = 0;
...
ssize_t hi6421v500_coul_show_reg_info(
...
static DEVICE_ATTR(
	sel_reg, (S_IWUSR | S_IRUGO), NULL, hi6421v500_coul_set_reg_sel);
static DEVICE_ATTR(set_reg, (S_IWUSR | S_IRUGO), hi6421v500_coul_show_reg_info,
...
struct coul_device_ops hi6421v500_coul_ops = {
	...
	.convert_ocv_regval2ua = hi6421v500_coul_convert_regval2ua,
	.convert_ocv_regval2uv = hi6421v500_coul_convert_regval2uv,
	...
	hi6421v500_coul_get_battery_vol_uv_from_fifo,
	.get_battery_cur_ua_from_fifo =
	hi6421v500_coul_get_battery_cur_ua_from_fifo,
	.get_offset_current_mod = hi6421v500_coul_get_offset_current_mod,
	.get_offset_vol_mod = hi6421v500_coul_get_offset_vol_mod,
	.set_offset_vol_mod = hi6421v500_coul_set_offset_vol_mod,
	.get_ate_a = hi6421v500_coul_get_ate_a,
	.get_ate_b = hi6421v500_coul_get_ate_b,
	.irq_enable = hi6421v500_coul_enable_irq,
	.irq_disable = hi6421v500_coul_disable_irq,
	...
	.clear_drained_battery_flag =
	hi6421v500_coul_clear_drained_battery_flag,
};
...
static int hi6421v500_coul_probe(struct platform_device *pdev)
{
	...
	di = (struct hi6421v500_coul_device_info *)devm_kzalloc(
		     &pdev->dev, sizeof(*di), GFP_KERNEL);
	if (!di) {
		HI6421V500_COUL_ERR(
			"%s failed to alloc di struct\n", __FUNCTION__);
		return -1;
	}
	di->dev = &pdev->dev;
	np = di->dev->of_node;
	...
	INIT_DELAYED_WORK(
		&di->irq_work, hi6421v500_coul_interrupt_notifier_work);
	retval = request_irq(di->irq, hi6421v500_coul_irq_cb, IRQF_NO_SUSPEND,
			     pdev->name, di);
	if (retval) {
	...
	retval =
		sysfs_create_group(&di->dev->kobj, &hi6421v500_coul_attr_group);
	if (retval) {
		HI6421V500_COUL_ERR(
	...
	if (power_class) {
		...
		if (coul_dev) {
			retval = sysfs_create_link(&coul_dev->kobj,
						   &di->dev->kobj, "hi6421v500_coul");
			if (0 != retval)
				HI6421V500_COUL_ERR(
