...
struct hi6xxx_coul_device_info *g_hi6xxx_dev;
static u64 last_eco_in;
static u64 last_eco_out;
static int hisi_saved_abs_cc_mah;
static int r_coul_mohm = R_COUL_MOHM;

static void hi6xxx_coul_cc_write_pro(u8 lock)
{
#if defined(CONFIG_HISI_COUL_HI6421V700) ||                                    \
	defined(CONFIG_HISI_COUL_HI6421V600) ||                                \
...
void hi6xxx_coul_set_nv_save_flag(int nv_flag)
{
	...
	if (nv_flag == NV_SAVE_SUCCESS)
		HI6XXX_REG_WRITE(
			HI6XXX_NV_SAVE_SUCCESS, (val | NV_SAVE_BITMASK));
	else
		HI6XXX_REG_WRITE(
			HI6XXX_NV_SAVE_SUCCESS, (val & (~NV_SAVE_BITMASK)));
}
...
static int hi6xxx_coul_convert_regval2temp(unsigned int reg_val)
{
	...
	if (di->chip_version != COUL_HI6421V700)
		temp += 650000;
 ...
 *  Output:        NULL
 *  Return:        value of register in uV
 *  Remark:      mv_val = code(23bit) * 1.3 * 5 *a / 2^23 + b  = code * 13 *a /
 * 2^24 + b
 ********************************************************/
 ...
 * Return:        battery voltage in uv
 *********************************************************/
int hi6xxx_coul_get_battery_vol_uv_from_fifo(short fifo_order)
{
	int uv = 0;
	unsigned int regval = 0;
	...
	if (fifo_order > FIFO_DEPTH)
		fifo_order = 0;
	HI6XXX_REGS_READ((HI6XXX_VOL_FIFO_BASE + REG_NUM * fifo_order), &regval,
			 REG_NUM);
 ...
 *  Return:        battery current in ua
 *********************************************************/
int hi6xxx_coul_get_battery_cur_ua_from_fifo(short fifo_order)
{
	int ua = 0;
	unsigned int regval = 0;
	...
	if (fifo_order > FIFO_DEPTH)
		fifo_order = 0;
	HI6XXX_REGS_READ((HI6XXX_CUR_FIFO_BASE + REG_NUM * fifo_order), &regval,
			 REG_NUM);
...
{
	short i;
	unsigned int vol_reset_value = COUL_FIFO_VOL_DEFAULT;
	...
	for (i = 0; i < FIFO_DEPTH; i++) {
		HI6XXX_REGS_READ(HI6XXX_VOL_FIFO_BASE + i * REG_NUM,
				 &vol_fifo[i], REG_NUM);
		HI6XXX_REGS_READ(HI6XXX_CUR_FIFO_BASE + i * REG_NUM,
				 &cur_fifo[i], REG_NUM);
	}
...
static void hi6xxx_coul_chip_init(void)
{
	...
	hi6xxx_coul_set_i_in_event_gate(DEFAULT_I_GATE_VALUE);
	hi6xxx_coul_set_i_out_event_gate(DEFAULT_I_GATE_VALUE);
 ...
 *  Description:     interrupt_notifier_work - send a notifier event to battery
 *                   monitor.
...
{
	struct hi6xxx_coul_device_info *di = container_of(
			work, struct hi6xxx_coul_device_info, irq_work.work);
	unsigned char intstat = 0;

	if (NULL == di || NULL == work) {
		HI6XXX_COUL_ERR(" [%s] NULL point\n", __func__);
		return;
	}
	intstat = di->irq_mask;
	di->irq_mask = 0;
 ...
 *  Description:     calculate capacity leak from existing ECO MODE to calc soc
 *                   first time
 *  Input:           NULL
 *  Output:          NULL
...
static void hi6xxx_coul_clear_enable_eco_fifo(void)
{
	...
	HI6XXX_REG_WRITE(HI6XXX_ECO_FIFO_CLEAR,
...
static void hi6xxx_coul_enter_eco(void)
{
	...
	HI6XXX_REGS_READ(
		HI6XXX_ECO_OUT_CLOUT_REG_BASE, &eco_out_reg, CC_REG_NUM);
...
int hi6xxx_coul_set_hltherm_flag(int temp_protect_flag)
{
	...
	if (temp_protect_flag == 1)
		HI6XXX_REG_WRITE(
			HI6XXX_COUL_TEMP_PROTECT, (val | TEMP_PROTECT_BITMASK));
	else
		HI6XXX_REG_WRITE(HI6XXX_COUL_TEMP_PROTECT,
...
static void hi6xxx_coul_save_last_soc(short soc)
{
	...
	HI6XXX_REG_WRITE(HI6XXX_SAVE_LAST_SOC,
...
static void hi6xxx_coul_get_last_soc_flag(bool *valid)
{
	...
	val = SAVE_LAST_SOC_FLAG & HI6XXX_REG_READ(HI6XXX_SAVE_LAST_SOC);
	*valid = val;
}

static void hi6xxx_coul_cancel_auto_cali(void)
{
	u8 val = 0;
...
static int hi6xxx_coul_get_normal_mode_chip_temp(void)
{
	...
	do {
		...
		if (retry == 0) {
			HI6XXX_COUL_ERR(
				"%s  val is 0x%x, wait coul working fail!\n",
				__func__, val);
			ret = INVALID_TEMP;
	...
	do {
		...
		if (retry == 0) {
			HI6XXX_COUL_ERR(
				"%s  val is 0x%x, wait temp ready fail !\n",
				__func__, val);
			ret = INVALID_TEMP;
			goto reset_vol_sel;
		}
...
reset_vol_sel:
	...
	do {
		...
		if (retry == 0) {
			HI6XXX_COUL_ERR("%s val is 0x%x, wait vout fail!\n",
					__func__, val);
			ret = INVALID_TEMP;
			goto reset_auto_cali;
		}
 ...
 *  Input:            type: start up ocv chip temp; eco out chip temp; normal
 * mode
 *                  chip temp
 *  Output:          NA
 ...
 *******************************************************/
static int hi6xxx_coul_get_chip_temp(void)
{
	int ret = 0;
#ifdef HI6XXX_COUL_TEST
...
#else
/*******************************************************
 *  Function:      hi6xxx_reg_write_mask
 *  Description:   wrrite reg by mask
 *  Input:         reg addr, mask bit, bit value
 *  Output:        NULL
 *  Return:        void
 *********************************************************/
static void hi6xxx_reg_write_mask(u16 addr, u8 mask, u8 value)
{
	u8 data;

	data = HI6XXX_REG_READ(addr);
	data = data & ~mask;
	data = data | value;
	HI6XXX_REG_WRITE(addr, data);
}
 ...
 *    Return:        battery voltage in uv
 *********************************************************/
int hi6xxx_coul_get_eco_vol_uv_from_fifo(short fifo_order)
{
	unsigned int regval = 0;

	if (fifo_order > ECO_FIFO_DEPTH)
		fifo_order = ECO_FIFO_DEPTH;
	HI6XXX_REGS_READ((HI6XXX_ECO_VOL_FIFO_BASE + 3 * fifo_order), &regval,
			 3);
 ...
 *    Return:        battery voltage in uv
 *********************************************************/
int hi6xxx_coul_get_eco_cur_ua_from_fifo(short fifo_order)
{
	unsigned int regval = 0;

	if (fifo_order > ECO_FIFO_DEPTH)
		fifo_order = ECO_FIFO_DEPTH;
	HI6XXX_REGS_READ((HI6XXX_ECO_I_FIFO_BASE + REG_NUM * fifo_order),
			 &regval, REG_NUM);
 ...
 *    Return:        battery temp
 *********************************************************/
int hi6xxx_coul_get_eco_temp_from_fifo(short fifo_order)
{
	unsigned int regval = 0;

	if (fifo_order > ECO_FIFO_DEPTH)
		fifo_order = ECO_FIFO_DEPTH;
	HI6XXX_REGS_READ((HI6XXX_ECO_TEMP_FIFO_BASE + 3 * fifo_order), &regval,
			 3);
 ...
 *******************************************************/
static int hi6xxx_coul_get_tdie(void)
{
	u16 tdie_code = 0;
	int tdie;

	/* get adc data */
	HI6XXX_REGS_READ(HI6XXX_SOH_TDIE_DATA_BASE, &tdie_code, 2);

	tdie_code = soh_adc_a * tdie_code / 1000 + soh_adc_b;
	tdie = (int)(((s32)tdie_code * 180000 / 4096 - 35868) * 100 / 13427);
	HI6XXX_COUL_DBG("%s code %d, tdie %d\n", __func__, tdie_code, tdie);

	return tdie;
}

/*******************************************************
 * Function:        hi6xxx_coul_soh_adc_get
 * Description:    get soh adc code
 * Input:          chanel 15 and 16 for calibration
 * Output:         NULL
 * Return:         adc code
 *******************************************************/
static int hi6xxx_coul_soh_adc_get(u8 chanel)
{
	int retry = ADC_RETRY;
	u8 reg_value;
	u16 code = 0;

	HI6XXX_REG_WRITE(SOH_EN_ADDR, SOH_DIS);
	/*slect adc chenel and enable adc*/
	hi6xxx_reg_write_mask(SOH_ADC_CTRL_ADDR, ADC_CHANEL_MASK, chanel);
	hi6xxx_reg_write_mask(SOH_ADC_CTRL_ADDR, ADC_EN_MASK, ADC_EN);

	udelay(SOH_ADC_WAIT_US);
	HI6XXX_REG_WRITE(SOH_ADC_START_ADDR, SOH_ADC_START);

	do {
		if (retry == 0) {
			HI6XXX_COUL_ERR("[%s] retry fail!\n", __func__);
			goto reset_adc;
		}
		udelay(SOH_ADC_WAIT_US);/*chip requires 12.5us*/
		reg_value = HI6XXX_REG_READ(SOH_ADC_STATUS_ADDR);
		retry--;
	} while (!(reg_value & SOH_ADC_READY));


	HI6XXX_REGS_READ(SOH_ADC_CALI_DATA0_ADDR, &code, 2);

	hi6xxx_reg_write_mask(SOH_ADC_CTRL_ADDR, ADC_EN_MASK, ADC_DIS);

	/*close chip adc channel*/
reset_adc:
	HI6XXX_REG_WRITE(SOH_ADC_CTRL_ADDR, SOH_ADC_EN_DEFAULT);

	HI6XXX_REG_WRITE(SOH_EN_ADDR, SOH_EN);
	return (int)code;
}
#endif

static void hi6xxx_coul_soh_adc_cali(void)
{
#if defined(CONFIG_HISI_COUL_HI6421V700) ||\
		defined(CONFIG_HISI_COUL_HI6421V600) ||\
		defined(CONFIG_HISI_COUL_HI6555V200)
	...
	Y1 = hi6xxx_coul_soh_adc_get(ADC_CALI_PATH1);
	Y2 = hi6xxx_coul_soh_adc_get(ADC_CALI_PATH2);

	if (Y2 - Y1 == 0) {
		HI6XXX_COUL_INF("[%s] fail Y1 %d, Y2 %d\n", __func__, Y1, Y2);
		return;
	}
...
ssize_t hi6xxx_coul_show_reg_info(
...
static DEVICE_ATTR(sel_reg, 0640, NULL, hi6xxx_coul_set_reg_sel);
static DEVICE_ATTR(
	set_reg, 0640, hi6xxx_coul_show_reg_info, hi6xxx_coul_set_reg_value);
...
struct coul_device_ops hi6xxx_coul_ops = {
	...
	.get_battery_vol_uv_from_fifo =
	hi6xxx_coul_get_battery_vol_uv_from_fifo,
	.get_battery_cur_ua_from_fifo =
	hi6xxx_coul_get_battery_cur_ua_from_fifo,
	...
	.cali_auto_off = hi6xxx_coul_cancel_auto_cali,
...
#else
	.get_chip_temp = hi6xxx_coul_get_tdie,
#endif
};
...
static int hi6xxx_coul_create_sysfs(struct hi6xxx_coul_device_info *di)
{
	...
	if (power_class) {
		if (coul_dev == NULL) {
			coul_dev = device_create(
					   power_class, NULL, 0, "%s", "coul");
			if (IS_ERR(coul_dev))
				coul_dev = NULL;
		}
		if (coul_dev) {
...
static int hi6xxx_coul_probe(struct spmi_device *pdev)
{
	...
	di = (struct hi6xxx_coul_device_info *)devm_kzalloc(
	...
	retval = request_irq(di->irq, hi6xxx_coul_irq_cb, IRQF_NO_SUSPEND,
			     "hi6xxx_coul_irq", di);
	if (retval) {
	...
	retval = of_property_read_u32(
			 of_find_compatible_node(NULL, NULL, "hisi,coul_core"),
...
static const struct of_device_id hi6xxx_coul_match_table[] = {
	{
