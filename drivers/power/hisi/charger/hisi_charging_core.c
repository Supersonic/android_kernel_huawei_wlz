...
#define HWLOG_TAG hisi_charging_core
HWLOG_REGIST();
#else
#define hwlog_debug(fmt, args...)do { printk(KERN_DEBUG   "[hisi_charging_core]" fmt, ## args); } while (0)
#define hwlog_info(fmt, args...) do { printk(KERN_INFO    "[hisi_charging_core]" fmt, ## args); } while (0)
#define hwlog_warn(fmt, args...) do { printk(KERN_WARNING"[hisi_charging_core]" fmt, ## args); } while (0)
#define hwlog_err(fmt, args...)  do { printk(KERN_ERR   "[hisi_chargeing_core]" fmt, ## args); } while (0)
#endif
static struct charge_core_info *g_core_info = NULL;
/**********************************************************
...
*  return value:  NULL
**********************************************************/
static void charge_core_vdpm_handler(int vbat, struct charge_vdpm_data vdpm_data[], struct charge_core_data *data)
{
	int i = 0;
	static int last_i = 0;
	...
	else if (vbat < last_vbat)
	{
		for (i = 0; i < VDPM_PARA_LEVEL; i++)
		{
			if (((vbat + vdpm_data[i].cap_back) >= vdpm_data[i].cap_min) && ((vbat + vdpm_data[i].cap_back) < vdpm_data[i].cap_max))
			{
				data->vdpm = vdpm_data[i].vin_dpm;
				break;
			}
	...
	last_vbat = vbat;
	last_vdpm = data->vdpm;
	hwlog_debug("battery voltage is %d mv, vindpm set to %d mV\n", vbat, data->vdpm);
}
/**********************************************************
...
struct charge_core_data *charge_core_get_params(void)
{
	...
	hwlog_info("[%s] tbatt = %d, ichg = %d, iin = %d, vterm = %d\n", __func__, tbatt, di->data.ichg, di->data.iin, di->data.vterm);
	return &di->data;
}
/**********************************************************
...
static int charge_core_parse_dts(struct device_node* np, struct charge_core_info *di)
{
	...
	for(i = 0; i < array_len ; i++)
	{
		...
		case INDUCTANCE_PARA_CAP_BACK:
			...
			break;
		default:
			hwlog_err("inductance_para not match\n");
			break;
		}
