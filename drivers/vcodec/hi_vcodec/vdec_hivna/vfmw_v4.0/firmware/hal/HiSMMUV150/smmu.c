...
#define rd_smmu_common_vreg(reg, dat) \
	do { \
		...
		((SINT8 *)g_smmu_reg_vir.smmu_common_base_vir_addr + (reg)))); \
	} while (0)
...
#define rd_smmu_master_vreg(reg, dat) \
	do { \
		(dat) = readl(((volatile SINT32 *) \
		((SINT8 *)g_smmu_reg_vir.smmu_master_base_vir_addr + (reg)))); \
	} while (0)
...
#define rd_smmu_mfde_vreg(reg, dat) \
	do { \
		(dat) = readl(((volatile SINT32 *) \
		((SINT8 *)g_smmu_reg_vir.smmu_mfde_reg_vir_addr + (reg)))); \
	} while (0)
...
#define rd_smmu_bpd_vreg(reg, dat) \
	do { \
		(dat) = readl(((volatile SINT32 *) \
		((SINT8 *)g_smmu_reg_vir.smmu_bpd_reg_vir_addr + (reg)))); \
	} while (0)
...
#define rd_smmu_scd_vred(reg, dat) \
	do { \
		(dat) = readl(((volatile SINT32 *) \
		((SINT8 *)g_smmu_reg_vir.smmu_scd_reg_vir_addr + (reg)))); \
	} while (0)
...
static VOID smmu_mem_dealloc(mem_desc_s *mem_desc)
{
	...
	kfree((hi_void *)(uintptr_t)mem_desc->vir_addr);
...
SINT32 smmu_init(void)
{
	...
	g_smmu_reg_vir.smmu_bpd_reg_vir_addr =
		(SINT32 *) mem_phy_2_vir(g_bpd_reg_base_addr);
	if (g_smmu_reg_vir.smmu_bpd_reg_vir_addr == NULL) {
		dprint(PRN_FATAL, "%s smmu_bpd_reg_vir_addr is NULL, SMMU Init failed\n", __func__);
		return SMMU_ERR;
	}
