...
static SINT32 vdmhal_check_rpcfg_address(omxvdh_reg_cfg_s *vdh_reg_cfg,
...
VOID vdmhal_imp_reset_vdm(SINT32 vdh_id)
{
	...
	UINT32 reg;
	UINT32 reg_rst_ok;
	UINT32 *vdm_reset_vir_addr;
	UINT32 *vdm_reset_ok_vir_addr;
	...
	read_vreg(VREG_INT_MASK, tmp, vdh_id);
									...
									mem_buffer_s *vdh_mem_map)
{
	...
	for (index = VDH_SHAREFD_FRM_BUF; index < VDH_SHAREFD_MAX; index++) {
		if (vdh_mem_map[index].is_mapped == 0)
			break;
		if (((USIGN)(vdh_reg_cfg->vdh_yst_addr)
			>= vdh_mem_map[index].start_phy_addr)
			&& ((vdh_reg_cfg->vdh_uv_offset > 0)
			&& ((hi_u32)vdh_reg_cfg->vdh_uv_offset
			< vdh_mem_map[index].size))
			&& ((USIGN)(vdh_reg_cfg->vdh_yst_addr)
				< (vdh_mem_map[index].start_phy_addr
				   + vdh_mem_map[index].size
...
SINT32 vdmhal_hw_dec_proc(omxvdh_reg_cfg_s *vdh_reg_cfg,
						  mem_buffer_s *vdh_mem_map)
{
