...
static SINT32 vctrl_get_msg_pool_addr(
	mem_buffer_s *mem_map, mem_buffer_s *com_msg_map,
...
SINT32 vctrl_vdm_hal_process(omxvdh_reg_cfg_s *vdm_reg_cfg,
							 ...
							 mem_buffer_s *com_msg_map)
{
	hi_s32 ret;
	vdmdrv_sleep_stage_e sleep_state;

	if (vdm_reg_cfg == NULL || vdm_reg_state == NULL || vdh_mem_map == NULL || com_msg_map == NULL) {
	...
	ret = vctrl_vdh_get_addr_info(&vdh_mem_map[0], com_msg_map, vdm_reg_cfg);
	if (ret) {
