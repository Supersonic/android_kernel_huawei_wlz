...
static int hisi_get_mdm_temp(struct thermal_zone_device *thermal, int *temp)
{
	...
	if (volt == DEFULT_ERROR_VALUE) { // mdm has not registered
	...
	if (mdm_adc.channel <= 0xf) { //0-0xe is adc channel; 0xf is xo channel
	...
	} else if (mdm_adc.channel <= 0x13) { // 0x10-0x13 is mdm tsensor, volt means temperature
		if (volt <= -30000) {
		...
		*temp = volt;
	} else if (mdm_adc.channel == 0x14) { // 0x14 is mdm ddr
		if (volt < 0) {
