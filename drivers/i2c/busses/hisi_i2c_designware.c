...
static int i2c_dw_dma_rx_trigger_dma(struct dw_i2c_dev *dev)
{
	...
	dev_dbg(dev->dev, "%s, %d bytes to read\n",
...
error:
	if (ret < 0) {
		...
		dev_err(dev->dev, "%s return erron %d.\n",
			__func__, ret);
	}
	return ret;
}
...
int i2c_init_secos(struct i2c_adapter *adap)
{
	...
	if (IS_ERR(dev->clk)) {
		dev_err(dev->dev, "%s: i2c clk is error!\n", __func__);
		return -EINVAL;
	}
...
void i2c_frequency_division(struct dw_i2c_dev *dev, u32 clk)
{
	...
	dev->hs_hcnt = i2c_dw_scl_hcnt(clk, 100, /* tHD;STA = tHIGH = 0.1 us */
		sda_falling_time,		 /* tf = 0.3 us */
		0,				 /* 0: DW default, 1: Ideal */
		0);				 /* No offset */
	dev->hs_lcnt = i2c_dw_scl_lcnt(clk, 200, /* tLOW = 0.2 us */
...
static int hs_dw_i2c_probe(struct platform_device *pdev)
{
	...
	ctrl = devm_kzalloc(dev, sizeof(struct dw_hisi_controller),
		GFP_KERNEL);
	if (ctrl == NULL)
	...
	r = of_property_read_u32_array(dev->of_node, "reset-reg-base",
		&data[0], 4);
	if (r) {
	...
	r = of_property_read_u32_array(
		dev->of_node, "delay-off", &ctrl->delay_off, 1);
	if (r)
		dev_err(dev, "doesn't have delay-off property!\n");

	r = of_property_read_u32_array(
		dev->of_node, "reset-controller-reg", &data[0], 4);
	if (r)
...
cs_gpio_err:
	...
	if (ctrl->delay_off == I2C_DELAY_70NS)
	...
	r = devm_request_irq(dev, d->irq, i2c_dw_isr, 0, pdev->name, d);
	if (r) {
