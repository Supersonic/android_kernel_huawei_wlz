...
/*lint -e753 -esym(753,*)*/
/*lint -e528 -esym(528,*)*/
/*lint -save -e713 -e734 -e502 -e774 -e838 -e438 -e701 -e64 */
/*lint -e826 -e838 -e715 -e613 -e747 -e838 -e732 -e785 -e647 */
/*lint -e528 -e753 -e752 */
/*
...
extern void led_config_get_current_setting(
	struct hisi_led_platform_data *hisi_leds);
#else
...
static int hisi_led_probe(struct spmi_device *pdev)
{
	...
	for (index = 0; index < HISI_LEDS_MAX; index++) {
		snprintf(compatible_string, LED_DTS_ATTR_LEN,
			"hisilicon,hisi-led%d", index);
		root = of_find_compatible_node(
			pdev->dev.of_node, NULL, compatible_string);
		if (root == NULL) {
			dev_err(&pdev->dev,
				"hisilicon,hisi-led%d root error!\n", index);
			goto err;
		}

		ret = of_property_read_string(
			root, "hisilicon,dr_ctrl", &hisi_leds.leds[index].name);
		if (ret < 0) {
			dev_err(&pdev->dev, "led%d's colour fail!\n", index);
			goto err;
		}

		ret = of_property_read_u32(root, "hisilicon,each_max_iset",
			&hisi_leds.leds[index].each_maxdr_iset);
		if (ret < 0) {
			dev_info(&pdev->dev,
				"led%d's each_maxdr_iset fail!\n", index);
			hisi_leds.leds[index].each_maxdr_iset =
				hisi_leds.max_iset;
		}

		ret = of_property_read_u32(root, "hisilicon,dr_start_del",
			&hisi_leds.leds[index].dr_start_del);
		if (ret < 0) {
			dev_err(&pdev->dev,
				"config led%d's dr_start_del fail!\n", index);
			goto err;
		}

		ret = of_property_read_u32(root, "hisilicon,dr_iset",
			&hisi_leds.leds[index].dr_iset);
		if (ret < 0) {
		...
		ret = of_property_read_u32(root, "hisilicon,dr_time_config0",
			&hisi_leds.leds[index].dr_time_config0);
		if (ret < 0) {
			dev_err(&pdev->dev,
				"cfg led%d's dr_time_config0 fail!\n", index);
			goto err;
		}

		ret = of_property_read_u32(root, "hisilicon,dr_time_config1",
			&hisi_leds.leds[index].dr_time_config1);
		if (ret < 0) {
			dev_err(&pdev->dev,
				"cfg led%d's dr_time_config1 fail!\n", index);
			goto err;
		}
	}
