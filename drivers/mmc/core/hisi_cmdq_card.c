...
static int mmc_blk_cmdq_switch(
	struct mmc_card *card, struct mmc_blk_data *md, bool enable)
{
	int ret = 0;
	...
	if (card->ext_csd.cmdq_mode_en && (!!mmc_card_cmdq(card))) {
		...
		if (!ret) {
			...
			pr_err("%s: halt fail, ret = %d\n", __func__, ret);
				clear_bit(CMDQ_STATE_QUEUE_HUNGUP,
				&ctx_info->curr_state);
...
static int mmc_blk_cmdq_start_req(
	struct mmc_host *host, struct mmc_cmdq_req *cmdq_req)
{
	...
	struct mmc_queue_req *mqrq, struct mmc_queue *mq)
{
	...
	pr_debug("%s: %s: mrq: 0x%pK req: 0x%pK mqrq: 0x%pK "
		"bytes to xf: %d mmc_cmdq_req: 0x%pK card-addr: 0x%08x "
		"data_sg_len: %d map_sg_len: %d dir(r-1/w-0): %d\n",
...
#else
	trace_mmc_blk_cmdq_rw_start(cmdq_rq->cmdq_req_flags,
		cmdq_rq->tag, cmdq_rq->blk_addr,
		(cmdq_rq->data.blocks * cmdq_rq->data.blksz));
#endif
...
static int mmc_blk_cmdq_issue_rw_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if ((req->tag < 0) ||
		((unsigned int)(req->tag) > card->ext_csd.cmdq_depth))
...
static int mmc_blk_cmdq_wait_for_dcmd(
	struct mmc_host *host, struct mmc_cmdq_req *cmdq_req)
{
	struct mmc_request *mrq = &cmdq_req->mrq;
	int ret;
...
static int mmc_cmdq_do_erase(struct mmc_card *card, struct mmc_queue *mq,
	struct request *req, unsigned int from,
	...
	struct mmc_queue *mq, struct request *req)
{
	...
	if (!host)
		rdr_syserr_process_for_ap(
			(u32)MODID_AP_S_PANIC_STORAGE, 0ull, 0ull);
	if ((req->tag < 0) || ((unsigned int)(req->tag) > card->ext_csd.cmdq_depth)) /*lint !e613 */
		rdr_syserr_process_for_ap(
			(u32)MODID_AP_S_PANIC_STORAGE, 0ull, 0ull);
	...
	if (card->quirks & MMC_QUIRK_INAND_CMD38) { /*lint !e613 */
		...
		err = __mmc_switch_cmdq_mode(cmdq_req->mrq.cmd,
			...
			arg == MMC_TRIM_ARG ?
				INAND_CMD38_ARG_TRIM : INAND_CMD38_ARG_ERASE,
			0, true, true);
		if (err)
			goto out;
...
static int mmc_blk_cmdq_issue_secdiscard_rq(
	struct mmc_queue *mq, struct request *req)
{
	struct mmc_queue_req *active_mqrq = NULL;
	struct mmc_card *card = mq->card;
	...
	if (card->quirks & MMC_QUIRK_INAND_CMD38) { /*lint !e613 */
		...
		err = __mmc_switch_cmdq_mode(cmdq_req->mrq.cmd,
			EXT_CSD_CMD_SET_NORMAL, INAND_CMD38_ARG_EXT_CSD,
			arg == MMC_SECURE_TRIM1_ARG ?
				INAND_CMD38_ARG_SECTRIM1 : INAND_CMD38_ARG_SECERASE,
			0, true, true);
		if (err)
			goto out;
		err = mmc_blk_cmdq_wait_for_dcmd(card->host, cmdq_req); /*lint !e613 */
	...
	if (arg == MMC_SECURE_TRIM1_ARG) {
		...
		err = mmc_cmdq_erase(card, mq, req,
			from, nr, MMC_SECURE_TRIM2_ARG);
		if (err)
			goto out;
	}
...
void mmc_blk_cmdq_complete_rq(struct request *rq)
{
	...
	if (err || cmdq_req->resp_err) {
		pr_err("%s: request with req: 0x%pK, tag: %d, flags: 0x%llx,"
			...
			ctx_info->curr_state, ctx_info->active_reqs); /*lint !e559*/
...
enum blk_eh_timer_return mmc_blk_cmdq_req_timed_out(struct request *req)
{
	...
	pr_err("%s: issue time:%lld, start time:%lld, end time:%lld\n",
		__func__, ktime_to_ns(cmdq_task_info[req->tag].issue_time),
	...
	if (!mq_rq) {
		pr_err("%s:req 0x%pK restart timer for tag: %d, curr_state:0x%lx, active reqs:0x%lx\n",
			__func__, req, req->tag, ctx_info->curr_state,
...
#else
		card->host->cmdq_task_info[req->tag].start_dbr_time = 0UL;
		card->host->cmdq_task_info[req->tag].end_dbr_time = 0UL;
#endif
	}
}

void mmc_cmdq_switch_error(struct mmc_card *card,
	struct mmc_queue *mq, struct request *req)
{
	struct mmc_host *host = card->host;
	struct mmc_cmdq_context_info *ctx_info = &host->cmdq_ctx;
...
{
	struct mmc_queue *mq =
		container_of(work, struct mmc_queue, cmdq_err_work);
...
enum blk_eh_timer_return mmc_cmdq_rq_timed_out(struct request *req)
{
	...
	pr_err("%s: request with req: 0x%pK, tag: %d, flags: 0x%llx timed out\n",
		__func__, req, req->tag, req->cmd_flags); /*lint !e559*/
