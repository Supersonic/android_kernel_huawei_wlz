...
void sdhci_mshc_config_strobe_clk(struct sdhci_host *host, unsigned timing)
{
	...
	if (timing == MMC_TIMING_MMC_DDR52 || timing == MMC_TIMING_MMC_HS400) {
		...
		reg_u32 |= ((DLY_1_CODE_MASK & (delaymeas_code))
			    << DLY_1_CODE_SHIFT);
		reg_u32 &= ~INV_TX_CLK;
...
static void sdhci_mshc_save_tuning_phase(struct sdhci_host *host)
{
	...
	if (sdhci_mshc->tuning_count < TUNING_LOOP_64)
		sdhci_mshc->tuning_phase_record_low |=
			(u64)0x1 << (unsigned int)(sdhci_mshc->tuning_count);
	else
...
static int sdhci_mshc_tuning_move_clk(struct sdhci_host *host)
{
	...
	for (loop = 0; loop < 2; loop++) {
		...
		sdhci_mshc->tuning_move_phase =
			...
			: -move_step) * (sdhci_mshc->tuning_move_count / 2);
...
int sdhci_mshc_soft_tuning(struct sdhci_host *host, u32 opcode, bool set)
{
	...
	if (set) {
		sdhci_mshc_set_tuning_phase(
			host, sdhci_mshc->tuning_phase_best);
	} else {
...
static int sdhci_mshc_suspend(struct device *dev)
{
	...
	sdhci_sctrl_writel(host, RG_EMMC_LHEN_IN_RET | RG_EMMC_LHEN_MASK,
		SCTRL_SCPERCTRL7);
	...
	struct platform_device *pdev, struct sdhci_mshc_data *sdhci_mshc)
{
	...
	if (of_device_is_available(np)) {
		if (of_property_read_u32(np, "clk-xin", &xin_clk))
			dev_err(&pdev->dev, "clk-xin  cannot get from dts, use "
					    "the default value!\n");
	}
...
static void sdhci_mshc_cmdq_init(struct platform_device *pdev)
{
	...
	if (host->mmc->caps2 & MMC_CAP2_CMD_QUEUE) {
		cmdq_reg_offset =
			0xFFF & sdhci_readw(host, SDHCI_VENDOR2_SPECIFIC_AREA);
		host->cq_host =
			cmdq_pltfm_init(pdev, host->ioaddr + cmdq_reg_offset);
...
static int sdhci_mshc_probe(struct platform_device *pdev)
{
	...
	/*
	 * import, ADMA support 64 or 32 bit address, here we use 32 bit. SDMA
	 * only support 32 bit mask.
	 */
	if (!(host->quirks2 & SDHCI_QUIRK2_BROKEN_64_BIT_DMA))
