...
#ifdef CONFIG_MMC_DW_MUX_SDSIM_LIBRA
int set_sd_sim_gpio_direction(
	int gpio_num, const char *gpio_name, int direction, int power_level)
{
	/*
	...
	if (direction == 0) {
		...
		if (err)
			pr_err("%s %s gpio_direction_input failed",
				MUX_SDSIM_LOG_TAG, __func__);
	} else {
		err = gpio_direction_output(gpio_num, power_level);
		if (err)
			pr_err("%s %s gpio_direction_output failed",
				MUX_SDSIM_LOG_TAG, __func__);
	}
	gpio_free(gpio_num);
...
out:
	pr_info("%s %s set gpio%d(%s) %s(%s) %s err = %d)", MUX_SDSIM_LOG_TAG,
		...
		power_level ? "high" : "low", (err < 0) ? "failed" : "success",
		err);
	return err;
}
#endif
...
int set_sd_sim_group_io_register(
	int gpionumber, int function_select, int pulltype, int driverstrenth)
{
	...
	if (unlikely(iomg_ctrl_reg == NULL))
		iomg_ctrl_reg = ioremap(IOMG_BASE_GPIO_for_SD_CLK, 0x1000);

	if (unlikely(iocg_ctrl_reg == NULL))
	...
	reg = readl(iocg_ctrl_reg +
		    0x004 * (gpionumber - GPIO_current_number_for_SD_CLK));
	reg = reg & 0x08;
	reg = reg | driverstrenth_this;
	reg = reg | pulltype_this;
	writel(reg, iocg_ctrl_reg +
		0x004 * (gpionumber - GPIO_current_number_for_SD_CLK));
...
int config_sdsim_gpio_mode(enum sdsim_gpio_mode gpio_mode)
{
	...
	if (gpio_mode == SDSIM_MODE_GPIO_DETECT) {
		...
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_CMD,
			FUNCTION0, PULL_TYPE_PU, DRIVER_STRENGTH_2MA_0); // detect pin
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_DATA0,
			FUNCTION0, PULL_TYPE_PD, DRIVER_STRENGTH_2MA_0); // detect pin
		...
		printk("%s %s set SDSIM_MODE_SD_NORMAL.\n",
			MUX_SDSIM_LOG_TAG,__func__);
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_CLK,
			FUNCTION1, PULL_TYPE_NP, sd_clk_driver_strength);
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_CMD,
			FUNCTION1, PULL_TYPE_PU, sd_cmd_driver_strength);
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_DATA0,
		...
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_DATA3,
			FUNCTION1, PULL_TYPE_PU, sd_data_driver_strength);
#ifdef CONFIG_MMC_DW_MUX_SDSIM_LIBRA
		set_sd_sim_group_io_register(gpio_current_number_for_sim_clk,
			FUNCTION0, PULL_TYPE_NP, DRIVER_STRENGTH_4MA_0);
		set_sd_sim_group_io_register(gpio_current_number_for_sim_rst,
			FUNCTION0, PULL_TYPE_NP, DRIVER_STRENGTH_4MA_0);
		set_sd_sim_group_io_register(gpio_current_number_for_sim_data,
			FUNCTION0, PULL_TYPE_NP, DRIVER_STRENGTH_4MA_0);
		(void)set_sd_sim_gpio_direction(
			gpio_current_number_for_sim_clk, "sim_clk", 0, 0);
		(void)set_sd_sim_gpio_direction(
			gpio_current_number_for_sim_rst, "sim_rst", 0, 0);
		(void)set_sd_sim_gpio_direction(
			gpio_current_number_for_sim_data, "sim_data", 0, 0);
#endif
	} else if (gpio_mode == SDSIM_MODE_SD_IDLE) {
		printk("%s %s set SDSIM_MODE_SD_IDLE.\n", MUX_SDSIM_LOG_TAG, __func__);
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_CLK,
			FUNCTION0, PULL_TYPE_NP, sd_clk_driver_strength);
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_CMD,
			FUNCTION0, PULL_TYPE_NP, sd_cmd_driver_strength);
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_DATA0,
		...
		set_sd_sim_group_io_register(gpio_current_number_for_sim_data,
			FUNCTION1, PULL_TYPE_NP, DRIVER_STRENGTH_4MA_0);
		(void)set_sd_sim_gpio_direction(
			GPIO_current_number_for_SD_CLK, "sd_clk", 0, 0);
		(void)set_sd_sim_gpio_direction(
			GPIO_current_number_for_SD_CMD, "sd_cmd", 0, 0);
		(void)set_sd_sim_gpio_direction(
			GPIO_current_number_for_SD_DATA0, "sd_data0", 0, 0);
		(void)set_sd_sim_gpio_direction(
			GPIO_current_number_for_SD_DATA1, "sd_data1", 0, 0);
		(void)set_sd_sim_gpio_direction(
			GPIO_current_number_for_SD_DATA2, "sd_data2", 0, 0);
		(void)set_sd_sim_gpio_direction(
			GPIO_current_number_for_SD_DATA3, "sd_data3", 0, 0);
#else
		set_sd_sim_group_io_register(GPIO_current_number_for_SD_CLK,
...
static detect_gpio_status_array_s g_array[] = {
	/* status type 1, NORMAL_SIM for one type card slot,like LAYA */
	{GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, GPIO_VALUE_LOW,  SD_SIM_DETECT_STATUS_SIM },
	/* status type 2, NORMAL_SD */
	{GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SD  },
	/* status type 2, SLOT_EMPTY,same status with NORMAL_SD,set as SD_SIM_DETECT_STATUS_SD */
	{GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SD  },
	/* status type 3, ERROR_1_SIM */
	{GPIO_VALUE_HIGH, GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SIM },
	/*status type 2, ERROR_1_SD */
	{GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SD  },
	/* status type 4, ERROR_2_1_SIM */
	{GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  SD_SIM_DETECT_STATUS_SIM },
	/* status type 2, ERROR_2_1_SD */
	{GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SD  },
	/* status type 2, ERROR_2_2_SIM */
	{GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SIM },
	/* status type 2, ERROR_2_2_SD */
	{GPIO_VALUE_LOW,  GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SD  },
	/* status type 5, NORMAL_SIM for another type card slot,like HIMA */
	{GPIO_VALUE_LOW,  GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, GPIO_VALUE_HIGH, SD_SIM_DETECT_STATUS_SIM },
};
...
int sd_sim_detect_run(
	void *dw_mci_host, int status, int current_module, int need_sleep)
{
	int ret = 0;
	int i = 0;
	...
	printk("%s %s argument list: status = %d current_module = %d "
	...
	if (status == STATUS_PLUG_OUT) {
		...
		if (SD_SIM_DETECT_STATUS_UNDETECTED !=
			...
			printk("%s %s For plug out "
			       "event,sd_sim_detect_status_current=%d,"
			       "now is not SD_SIM_DETECT_STATUS_UNDETECTED,"
			       "so config as SDSIM_MODE_GPIO_DETECT mode for "
			       "lowpower.\n",
				MUX_SDSIM_LOG_TAG, __func__,
				sd_sim_detect_status_current);
		}
		...
		printk("%s %s For plug out event,update "
		       "sd_sim_detect_status_current here.\n",
			MUX_SDSIM_LOG_TAG, __func__);
	...
	if (status == STATUS_PLUG_IN &&
		sd_sim_detect_status_current == SD_SIM_DETECT_STATUS_SD&&
		current_module == MODULE_SIM) {
		printk("%s %s MODULE_SIM find SD card already detected,just "
	...
	if (host) {
		...
		printk("%s %s host=NULL,current_module=%d,This is Error,maybe "
		       "MODULE_SIM,just return STATUS_PLUG_IN and let SIM go "
	...
	if (MODULE_SD == current_module) {
		...
		if (dw_mci_host == NULL) {
			printk("%s %s MODULE_SD call but dw_mci_host = NULL,"
		...
		printk("%s %s host_temp->hw_mmc_id "
		       "=%d,priv_temp->mux_sdsim=%d\n",
			MUX_SDSIM_LOG_TAG, __func__, host_temp->hw_mmc_id,
			priv_temp->mux_sdsim);

		/*
		 * for those non DWMMC_SD_ID device or mux_sdsim not
		 * enabled,just return former status value
		 */
		if (host_temp->hw_mmc_id != DWMMC_SD_ID) {
			printk("%s %s for those non DWMMC_SD_ID "
			       "device,just return former status value.\n",
				MUX_SDSIM_LOG_TAG, __func__);
			up(&sem_mux_sdsim_detect);
		...
		if (!priv_temp->mux_sdsim) {
			printk("%s %s for device mux_sdsim not "
			       "enabled,just return former status "
			       "value.\n", MUX_SDSIM_LOG_TAG, __func__);
			up(&sem_mux_sdsim_detect);
			mmc_release_host(mmc_host_temp);
			return status;
	...
	if (status == STATUS_PLUG_IN) {
		if (sd_sim_detect_status_current == SD_SIM_DETECT_STATUS_SIM &&
			current_module == MODULE_SD) {
			printk("%s %s MODULE_SD find SIM card already "
			       "detected,just return STATUS_PLUG_OUT and do "
			       "nothing.\n",
				MUX_SDSIM_LOG_TAG, __func__);
			up(&sem_mux_sdsim_detect);
			mmc_release_host(mmc_host_temp);
			return STATUS_PLUG_OUT;
		} else if ((SD_SIM_DETECT_STATUS_UNDETECTED ==
				   sd_sim_detect_status_current) ||
 ...
 */
					...
					ret = regulator_set_voltage(
						host->vqmmc, 1750000, 1750000);
					printk("%s %s vqmmc set 1.75v ret=%d\n",
						MUX_SDSIM_LOG_TAG, __func__,
						ret);
#else
					ret = regulator_set_voltage(
						host->vqmmc, 1775000, 1775000);
					printk("%s %s vqmmc set 1.775v "
					       "ret=%d\n",
						MUX_SDSIM_LOG_TAG, __func__,
						ret);
#endif
				...
				if (host->vmmc) {
					/*
					 * temp value for work around after modem
					 * set raw register here,but ap can't set
					 * same value again
					 */
					ret = regulator_set_voltage(
						host->vmmc, 1850000, 1850000);
				...
				if (host->vmmcmosen) {
					/*
					 * temp value for work around after
					 * modem set raw register here,
					 * but ap can't set same value again
					 */
					ret = regulator_set_voltage(
						host->vmmcmosen, 2950000,
						2950000);
					printk("%s %s vmmcmosen set 2.95v "
					       "ret=%d\n",
						MUX_SDSIM_LOG_TAG, __func__,
						ret);

					ret = regulator_set_voltage(
						host->vmmcmosen, 3000000,
						3000000);
					printk("%s %s vmmcmosen set 3.00v "
					       "ret=%d\n",
						MUX_SDSIM_LOG_TAG, __func__,
						ret);

					ret = regulator_enable(host->vmmcmosen);
					printk("%s %s vmmcmosen enable "
					       "ret=%d\n",
						MUX_SDSIM_LOG_TAG, __func__,
						ret);
					usleep_range(1000, 1500);
				...
				/*
				 * now sim module just use mmc host LDO config
				 * interface,and host_from_sd_module
				 */
			}
			...
			(void)gpio_request(GPIO_current_number_for_SD_CMD,
				"sd_cmd_and_sim1_vpp");
			(void)gpio_request(GPIO_current_number_for_SD_DATA0,
				"sd_data0_and_sim1_rst");
			(void)gpio_request(
				GPIO_current_number_for_SD_DATA2, "sd_data2");
			(void)gpio_request(
				GPIO_current_number_for_SD_DATA3, "sd_data3");

			(void)gpio_direction_input(
				GPIO_current_number_for_SD_CMD);
			(void)gpio_direction_input(
				GPIO_current_number_for_SD_DATA0);
			(void)gpio_direction_input(
				GPIO_current_number_for_SD_DATA2);
			(void)gpio_direction_input(
				GPIO_current_number_for_SD_DATA3);
			...
			sd_cmd_and_sim1_vpp_value =
				gpio_get_value(GPIO_current_number_for_SD_CMD);
			sd_data0_and_sim1_rst_value = gpio_get_value(
				GPIO_current_number_for_SD_DATA0);
			sd_data2_value = gpio_get_value(
				GPIO_current_number_for_SD_DATA2);
			sd_data3_value = gpio_get_value(
				GPIO_current_number_for_SD_DATA3);

			printk("%s %s <GPIO %d> "
			       "sd_data0_and_sim1_rst_value=%d\n",
				MUX_SDSIM_LOG_TAG, __func__,
				GPIO_current_number_for_SD_DATA0,
				sd_data0_and_sim1_rst_value);
			...
			msleep(20);
			printk("%s %s enter CMD1-RESPONSE STATUS detect stage "
			       "after sleep 20 ms.\n", MUX_SDSIM_LOG_TAG, __func__);

			former_present_flag =
				test_bit(DW_MMC_CARD_PRESENT, &cur_slot->flags);
			...
			if (!former_present_flag)
				clear_bit(
					DW_MMC_CARD_PRESENT, &cur_slot->flags);
			else
				set_bit(DW_MMC_CARD_PRESENT, &cur_slot->flags);

			printk("%s %s mmc_detect_sd_or_mmc() cmd1_result = "
			       "%d.\n", MUX_SDSIM_LOG_TAG, __func__, cmd1_result);
			priv_temp->mux_sdsim_vcc_status = MUX_SDSIM_VCC_STATUS_2_9_5_V;

			msleep(20);
			printk("%s %s enter OVER-ALL STATUS detect stage after "
			       "sleep 20 ms.\n", MUX_SDSIM_LOG_TAG, __func__);
...
#endif
			for (i = 0; i < (int)(sizeof(g_array) /
				sizeof(detect_gpio_status_array_s)); i++) {
				if ((sd_cmd_and_sim1_vpp_value ==
					g_array[i].sd_cmd_and_sim1_vpp_value) &&
					(sd_data0_and_sim1_rst_value ==
						g_array[i].sd_data0_and_sim1_rst_value) &&
					(sd_data2_value ==
						g_array[i].sd_data2_value) &&
					(sd_data3_value ==
						g_array[i].sd_data3_value)) {
					printk("%s %s GPIO_STATUS_LIST compare "
					       "ok when i = %d\n",
						MUX_SDSIM_LOG_TAG, __func__, i);

					if (g_array[i].actual_card_type ==
					...
					} else if (g_array[i].actual_card_type ==
						...
						printk("%s %s SD is inserted "
						       "and detected now(4-PIN "
						       "status success),"
						       "go with SD.\n",
							MUX_SDSIM_LOG_TAG,
							__func__);
			...
			if (i >= (int)(sizeof(g_array) /
					 sizeof(detect_gpio_status_array_s))) {
				printk("%s %s SD_SIM_DETECT_STATUS_ERROR "
				       "detected,but just go with SIM.\n",
					MUX_SDSIM_LOG_TAG, __func__);
#ifndef CONFIG_HISI_ESIM
				switch_gpio_change(SWITCH_GPIO_SIM_SIDE);
				config_sdsim_gpio_mode(SDSIM_MODE_SIM_NORMAL);
			...
			printk("%s %s current_module=%d now re-entered but same detect status,"
		...
		printk("%s %s input argument status=%d is not correct,just "
		       "return STATUS_PLUG_OUT.\n",
...
void notify_sim1_card_in(void)
{
	...
	ret = kernel_ioctl(filp, SIMHOTPLUG_IOC_INFORM_CARD_INOUT,
...
void notify_sim_while_sd_success(struct mmc_host *mmc)
{
	...
	if (sd_sim_detect_status_current == SD_SIM_DETECT_STATUS_SD) {
		...
		printk("%s %s SD card init success "
		       "now,sd_sim_detect_status_current = %d but not "
		       "SD_SIM_DETECT_STATUS_SD,no need retry with SIM,just "
		       "return.\n",
			MUX_SDSIM_LOG_TAG, __func__,
			sd_sim_detect_status_current);
	}
...
void notify_sim_while_sd_fail(struct mmc_host *mmc)
{
	...
	if (sd_sim_detect_status_current == SD_SIM_DETECT_STATUS_SD) {
		...
		printk("%s %s SD card init fail now, "
		       "sd_sim_detect_status_current = %d,"
		       "but not SD_SIM_DETECT_STATUS_SD,no need retry with "
		       "SIM,just return.\n",
			MUX_SDSIM_LOG_TAG, __func__,
			sd_sim_detect_status_current);
	}
...
int get_card1_type(void)
{
	...
	if (jtag_enable == 1) {
		...
		if (sd_sim_detect_status_current == SD_SIM_DETECT_STATUS_SIM) {
			status = STATUS_SIM;
		} else if (SD_SIM_DETECT_STATUS_SD ==
			   sd_sim_detect_status_current) {
			status = STATUS_SD;
		} else {
