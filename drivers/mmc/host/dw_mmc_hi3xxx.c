...
static int hs_timing_config_kirin970_cs_sd_ppll3[TUNING_INIT_CONFIG_NUM] = {
	1200000000, 6, 4, 3, 13, 0, 171000000};
static int hs_timing_config_kirin970_cs_sdio_ppll3[TUNING_INIT_CONFIG_NUM] = {
	1200000000, 6, 4, 3, 13, 0, 171000000};
...
static int hs_timing_config_hi6250
...
static int hs_timing_config_hi3660
	[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
		/* bus_clk, div, drv_phase, sam_dly, sam_phase_max, sam_phase_min, input_clk */
		{
			/* MMC */
...
static int hs_timing_config_kirin970
	[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
		/* bus_clk, div, drv_phase, sam_dly, sam_phase_max, sam_phase_min, input_clk */
		{
			/* MMC */
...
static int hs_timing_config_kirin970_cs
	[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
		/* bus_clk, div, drv_phase, sam_dly, sam_phase_max, sam_phase_min, input_clk */
		{
			/* MMC */
...
static int hs_timing_config_cancer
	[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
		/* bus_clk, div, drv_phase, sam_dly, sam_phase_max, sam_phase_min, input_clk */
		{
			/* MMC */
...
static int hs_timing_config_taurus
	[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
		/* bus_clk, div, drv_phase, sam_dly, sam_phase_max, sam_phase_min, input_clk */
		{
			/* MMC */
...
#ifdef CONFIG_SD_SDIO_CRC_RETUNING
void dw_mci_sd_crc_retuning_set(struct dw_mci *host, int sam_phase_val,
	int *use_sam_dly, int *enable_shift)
{
	...
	if (host->use_samdly_flag) {
		...
		if (host->enable_shift_range[0] <= sam_phase_val &&
			...
			*enable_shift = 1;
		dev_info(host->dev,
			"%s:use_samdly_range_min:%d,use_samdly_range_max:%d!\n",
	...
	if (sam_phase_val >= 2 && sam_phase_val <= 8)
		*enable_shift = 1;
	dev_info(host->dev,
		"%s:no data crc occur in kirin970, no need retune!\n",
		__func__);
}
#endif

#ifdef CONFIG_SD_SDIO_CRC_RETUNING
void dw_mci_sdio_crc_retuning_set(struct dw_mci *host, int sam_phase_val,
	int *use_sam_dly, int *enable_shift)
{
	/* used for libra,cancer. but not for kirin970 */
	if (host->use_samdly_flag) {
...
void dw_mci_shift_set(struct dw_mci *host, int sam_phase_val, int d_value,
	int *use_sam_dly, int *enable_shift)
{
	*use_sam_dly = 0;
	*enable_shift = 0;
...
void dw_mci_shift_sdio_set(struct dw_mci *host, int sam_phase_val, int d_value,
	int *use_sam_dly, int *enable_shift)
{
	*use_sam_dly = 0;
	*enable_shift = 0;
	...
	if (host->wifi_sdio_sdr104_177M == 0xaaaa) {
...
static void dw_mci_hs_set_timing(
	struct dw_mci *host, int id, int timing, int sam_phase, int clk_div)
{
	...
	case DW_MCI_SDIO_ID:
		...
		case MMC_TIMING_UHS_SDR50:
			...
			break;
		case MMC_TIMING_UHS_SDR104:
			dw_mci_shift_sdio_set(host, sam_phase_val, d_value,
				&use_sam_dly, &enable_shift);
			break;
		}
		break;
	...
	reg_value = SDMMC_GPIO_VALUE(
		(unsigned int)cclk_div, (unsigned int)use_sam_dly);
	udelay(50);
	mci_writel(host, GPIO, (unsigned int)reg_value | GPIO_CLK_ENABLE);

	if (!(slot && slot->sdio_wakelog_switch))
		dev_info(host->dev, "id=%d,timing=%d,UHS_REG_EXT=0x%x,ENABLE_"
				    "SHIFT=0x%x,GPIO=0x%x\n",
...
void dsm_wifi_client_notify(int dsm_id, const char *format, ...)
{
	...
	if (!dsm_client_ocuppy(wifi_client)) {
		...
		printk(KERN_ERR "dsm client notify failed, dsm id is %d.\n", dsm_id);
	}
}
#endif

static void dw_mci_lever_shit_voltage(int lever_value)
{
	int lever_shift_number = LEVER_SHIFT_90;
	(void)gpio_request(lever_shift_number, "lever_shift");
	gpio_direction_output(lever_shift_number, lever_value);
	gpio_set_value(lever_shift_number, lever_value);
	gpio_free(lever_shift_number);
}
...
static int dw_mci_set_sel18(struct dw_mci *host, bool set)
{
	...
	if (sd_hwlock == NULL || host->lever_shift)
		return 0;
	...
	pr_err(" __func__ reg = 0x%x\n", reg);
	return 0;
}

static inline void dw_mci_hs_check_result(
	struct dw_mci *host, int ret, char *log)
{
	if (ret)
...
void dw_mci_set_sd_gpio_to_high(struct dw_mci *host)
{
	...
	if ((0 != priv->set_sd_io) && (-1 != priv->set_sd_io)) {
		/* set GPIO169 to High */
...
static void dw_mci_hs_power_off(struct dw_mci *host, struct mmc_ios *ios)
{
	...
	if ((host->hw_mmc_id == DWMMC_SD_ID) &&
		...
		if (priv->mux_sdsim) {
			...
			if ((host->pinctrl) && (host->pins_idle)) {
				ret = pinctrl_select_state(
					host->pinctrl, host->pins_idle);
				dw_mci_hs_check_result(host, ret,
					"could not set sd idle pins\n");
			}
		}
#else
		if ((host->pinctrl) && (host->pins_idle)) {
			ret = pinctrl_select_state(
				host->pinctrl, host->pins_idle);
			dw_mci_hs_check_result(
				host, ret, "could not set sd idle pins\n");
		}
#endif
		...
		if ((host->pinctrl) && (host->pins_idle)) {
			ret = pinctrl_select_state(
				host->pinctrl, host->pins_idle);
			dw_mci_hs_check_result(
				host, ret, "could not set idle pins\n");
		}
	}
	...
	if (host->vqmmc) {
		...
		dw_mci_hs_check_result(
			host, ret, "egulator_disable vqmmc failed\n");
	}
	...
	if (host->hw_mmc_id == DWMMC_SD_ID && priv->mux_sdsim) {
		...
		if (host->vmmcmosen) {
			...
			dw_mci_hs_check_result(host, ret,
				"egulator_disable vmmcmosen failed\n");
		}
	} else {
		if (host->vmmc) {
			ret = regulator_disable(host->vmmc);
			dw_mci_hs_check_result(
				host, ret, "egulator_disable vmmc failed\n");
		}
	}
	...
	if (host->lever_shift)
	  dw_mci_lever_shit_voltage(LEVER_SHIFT_1_8V);
...
void dw_mci_set_sd_gpio_to_low(struct dw_mci *host)
{
	...
	/* 0 and -1 means we dont need to operate gpio */
	if ((0 != priv->set_sd_io2) && (-1 != priv->set_sd_io2)) {
		/* set GPIO169 to Low */
		ret = gpio_request(priv->set_sd_io2, "set_sd_io2");
		if (ret < 0) {
			dev_err(host->dev, "get set_sd_io2 error");
...
static void dw_mci_hs_power_up(struct dw_mci *host, struct mmc_ios *ios)
{
	...
	if (host->hw_mmc_id == DWMMC_SD_ID && priv->mux_sdsim) {
		...
		dev_info(host->dev,
			...
			MUX_SDSIM_LOG_TAG, __func__,
			sd_sim_detect_status_current);
		if (MUX_SDSIM_VCC_STATUS_2_9_5_V ==
			priv->mux_sdsim_vcc_status) {
	...
	if (host->hw_mmc_id == DWMMC_SD_ID) {
...
#endif
		dw_mci_hs_check_result(
			host, ret, "ios dw_mci_set_sel18 error!\n");
		/* Wait for 5ms */
		...
		if (host->lever_shift) {
			if (priv->mux_sdsim)
				...
				dw_mci_lever_shit_voltage(LEVER_SHIFT_3_0V);
		}
#endif
...
#endif
			dw_mci_hs_check_result(host, ret,
				"regulator_set_voltage vqmmc failed!\n");

			ret = regulator_enable(host->vqmmc);
			dw_mci_hs_check_result(
				host, ret, "regulator_enable vqmmc failed!\n");
		...
		if (host->vmmc) {
			...
			if (priv->mux_sdsim) {
				...
				if (host->vmmcmosen) {
					ret = regulator_set_voltage(
						host->vmmcmosen, 3000000, 3000000);
...
#endif
			dw_mci_hs_check_result(host, ret,
				"regulator_set_voltage vmmc failed!\n");
			...
			if (priv->mux_sdsim) {
				if (!(host->vmmcmosen) ||
					...
					dw_mci_hs_check_result(
						host, ret, "regulator_enable vmmc failed!\n");
					usleep_range(1000, 1500);
				}
				...
				if (host->vmmcmosen) {
					...
					dw_mci_hs_check_result(host, ret,
						"regulator_enable vmmcmosen failed!\n");
					usleep_range(1000, 1500);
				}
			} else {
				ret = regulator_enable(host->vmmc);
				dw_mci_hs_check_result(host, ret,
					"regulator_enable vmmc failed!\n");
				usleep_range(1000, 1500);
			}
#else
			ret = regulator_enable(host->vmmc);
		...
		if (!(dw_mci_check_himntn(HIMNTN_SD2JTAG) ||
			...
			if (priv->mux_sdsim) {
				if (SD_SIM_DETECT_STATUS_SIM !=
					sd_sim_detect_status_current)
					config_sdsim_gpio_mode(
						SDSIM_MODE_SD_NORMAL);
				else
					pr_err("%s %s SIM has detected,but SD "
					       "module want to config "
		...
		if (host->vmmc) {
			ret = regulator_set_voltage(
				host->vmmc, 2950000, 2950000);
			dw_mci_hs_check_result(
				host, ret, "set voltage vmmc failed!\n");

			ret = regulator_enable(host->vmmc);
			dw_mci_hs_check_result(
				host, ret, "enable regulator vmmc failed!\n");
		}
		...
		if (host->vqmmc) {
			ret = regulator_set_voltage(
				host->vqmmc, 2950000, 2950000);
			dw_mci_hs_check_result(
				host, ret, "set voltage vqmmc failed!\n");

			ret = regulator_enable(host->vqmmc);
			dw_mci_hs_check_result(
				host, ret, "enable regulator vqmmc failed!\n");
		}
	}
}
...
static void dw_mci_put_regulator(struct dw_mci *host)
{
	if (host->hw_mmc_id == DWMMC_SD_ID) {
...
static void dw_mci_get_regulator(struct dw_mci *host)
{
	if ((host->hw_mmc_id == DWMMC_SD_ID) &&
		(host->need_get_mmc_regulator == 1)) {
		...
		if (host->sd_vmmcmosen_switch == 1) {
			host->vmmcmosen =
				devm_regulator_get(host->dev, "vmmcmosen");
			if (IS_ERR(host->vmmcmosen)) {
				dev_info(host->dev,
					"no vmmcmosen regulator found\n");
				host->vmmcmosen = NULL;
			}
		} else {
			host->vmmcmosen = NULL;
	...
	struct dw_mci *host, struct device_node *of_node)
{
	...
	memcpy(hs_timing_config, hs_timing_config_capricorn,
...
{
	host->lever_shift = 1;
	memcpy(hs_timing_config, hs_timing_config_pisces,
		sizeof(hs_timing_config)); /* unsafe_function_ignore: memcpy */
	...
	struct dw_mci *host, struct device_node *of_node)
{
	...
	if (of_find_property(of_node, "cs_sd_timing_config_taurus", (int *)NULL)) {
		...
		memcpy(hs_timing_config, /* unsafe_function_ignore: memcpy */
		hs_timing_config_taurus, sizeof(hs_timing_config));
	}
	...
	struct dw_mci *host, struct device_node *of_node)
{
	...
	if (of_find_property(of_node, "cs_sd_timing_config_cancer", (int *)NULL)) {
		...
		if (value_clock == MMC_CLOCK_FREQUENCY_137) {
			...
			dev_info(host->dev,
				"hs_timing_config_cancer_cs[1][9][6] = %d\n",
		...
		memcpy(hs_timing_config, /* unsafe_function_ignore: memcpy */
		hs_timing_config_cancer, sizeof(hs_timing_config));
	}
...
*/
static int dw_mci_hs_get_dt_kirin970_pltfm_resource(
	struct dw_mci *host, struct device_node *of_node)
{
	host->scperctrls |= BIT_VOLT_OFFSET;
	host->bit_sdcard_o_sel18 |= BIT_VOLT_VALUE_18;
 ...
 */
static int dw_mci_hs_get_dt_hi3660_pltfm_resource(
	struct dw_mci *host, struct device_node *of_node)
{
	host->scperctrls |= BIT_VOLT_OFFSET;
	host->bit_sdcard_o_sel18 |= BIT_VOLT_VALUE_18;
	host->sdio_rst |= BIT_RST_SDIO_CHICAGO;
	memcpy(hs_timing_config, hs_timing_config_hi3660,
		sizeof(hs_timing_config)); /* unsafe_function_ignore: memcpy */
 ...
 */
static int dw_mci_hs_get_dt_hi6250_pltfm_resource(
	struct dw_mci *host, struct device_node *of_node)
{
	host->scperctrls |= BIT_VOLT_OFFSET;
	host->bit_sdcard_o_sel18 |= BIT_VOLT_VALUE_18;
	host->sdio_rst |= BIT_RST_SDIO;
	memcpy(hs_timing_config, hs_timing_config_hi6250,
		sizeof(hs_timing_config)); /* unsafe_function_ignore: memcpy */
 ...
 */
static int dw_mci_hs_get_dt_hi3650_pltfm_resource(
	struct dw_mci *host, struct device_node *of_node)
{
...
static void dw_mci_hs_set_rst_m(struct dw_mci *host, bool set)
{
	...
	if (set) {
		if (mmc0_crg_base)
			writel(BIT_RST_SD_TAURUS,
				mmc0_crg_base + MMC0_CRG_SD_HRST);
		...
		if (mmc0_crg_base)
			writel(BIT_RST_SD_TAURUS,
				mmc0_crg_base + MMC0_CRG_SD_HURST);
		else if (hsdt1_crg_base)
...
void dw_mci_hs_reset(struct dw_mci *host, int id)
{
	if (id == 0) {
		if (hsdt_crg_base)
			writel(host->sdio_rst,
				hsdt_crg_base + HSDT_CRG_PERRSTEN0);
		else if (mmc1_sys_ctrl_base)
			writel(BIT_HRST_SDIO_CANCER,
				mmc1_sys_ctrl_base + MMC1_SYSCTRL_PERRSTEN0);
		else
			writel(BIT_RST_EMMC, pericrg_base + PERI_CRG_RSTEN4);
		dev_info(host->dev, "rest emmc\n");
	} else if (id == 1) {
		if (mmc0_crg_base)
			writel(BIT_HRST_SD_TAURUS,
				mmc0_crg_base + MMC0_CRG_SD_HRST);
		else if (hsdt1_crg_base)
...
void dw_mci_hs_unreset(struct dw_mci *host, int id)
{
	if (id == 0) {
		if (hsdt_crg_base)
			writel(host->sdio_rst,
				hsdt_crg_base + HSDT_CRG_PERRSTDIS0);
		else if (mmc1_sys_ctrl_base)
			writel(BIT_HRST_SDIO_CANCER,
				mmc1_sys_ctrl_base + MMC1_SYSCTRL_PERRSTDIS0);
		else
...
static int dw_mci_hs_priv_init(struct dw_mci *host)
{
	...
	case MMC_SD:
		...
		pr_err("%s host_from_sd_module is inited when "
		       "__func__.\n", MUX_SDSIM_LOG_TAG);
...
static void dw_mci_hs_get_retuning_config(struct dw_mci *host)
{
	...
	if (host->hw_mmc_id == DWMMC_SD_ID) {
		...
		if (of_find_property(
			    np, "cs_sdio_timing_config", (int *)NULL)) {
			host->retuning_flag = SD_RETUNING_ON;
			dev_info(host->dev, "%s:SDIO retuning on!\n", __func__);
		} else {
			host->retuning_flag = SD_RETUNING_OFF;
			dev_info(
				host->dev, "%s:SDIO retuning off!\n", __func__);
		}
	} else
		return;
}
...
void dw_mci_hs_get_timing_config(struct dw_mci *host)
{
	...
	ret = of_property_read_u32_array(
		np, "use_samdly_range", &(host->use_samdly_range[0]), 2);
	if (ret) {
		host->use_samdly_flag = false;
		dev_info(host->dev,
			"%s:get use_sample_dly range range failed!\n", __func__);
	} else {
	 ...
	 */
	ret = of_property_read_u32_array(
		np, "enable_shift_range", &(host->enable_shift_range[0]), 2);
	if (ret) {
		...
		dev_info(host->dev,
			"%s:ena_shift_range_min:%d,ena_shift_range_max:%d!\n",
...
static int dw_mci_hs_parse_dt(struct dw_mci *host)
{
	...
	if (of_find_property(np, "enable_hpd_irq", NULL)) {
		dev_info(host->dev, "hpd_irq enable\n");
		host->flags |= DWMMC_HPD_IRQ;
	}

	if (of_property_read_u32(
		    np, "is-resetable", &host->is_reset_after_retry))
	...
	if (of_find_property(np, "caps2-mmc-gpio167-low", NULL)) {
		...
		dev_info(host->dev, "caps2-mmc-gpio167-low is %d",
			priv->set_sd_gpio167_low);
	}
#endif

	if (of_property_read_u32(
		    np, "debug_percrg_sd_sdio_reg_print", &value)) {
		dev_info(host->dev,
			"debug_percrg_sd_sdio_reg_print get value failed\n");
		value = 0;
	}
	if (value == CRG_PRINT)
	...
	if (of_property_read_u32(np, "mux-sdsim", &value)) {
		dev_info(host->dev, "mux-sdsim property not found, using "
				    "value of 0 as default\n");
		value = 0;
	}
	...
	if (priv->mux_sdsim == 1) {
		...
		if (of_property_read_u32(np, "vmmcmosen_switch", &value)) {
			dev_info(host->dev,
				"vmmcmosen_switch property not found, using "
				"value of 0 as default\n");
			value = 0;
		}
		...
		if (of_property_read_u32(np, "driverstrength_clk", &value)) {
			dev_info(host->dev,
				"driverstrength_clk property not found, using "
		...
		if (of_property_read_u32(np, "driverstrength_cmd", &value)) {
			dev_info(host->dev,
				"driverstrength_cmd property not found, using "
		...
		if (of_property_read_u32(np, "driverstrength_data", &value)) {
			dev_info(host->dev,
				"driverstrength_data property not found, using "
		...
		if (of_property_read_u32(np, "gpionumber_sdclk", &value)) {
			dev_info(host->dev,
				"gpionumber_sdclk property not found, using "
		...
		if (of_property_read_u32_index(
			    np, "switch_gpio_nums", 0, &value)) {
			dev_info(host->dev,
				"switch_gpio_number_0 property not found, "
		...
		value = SWITCH_GPIO_DEFAULT_NUMBER;
		if (of_property_read_u32_index(
			    np, "switch_gpio_nums", 1, &value)) {
			dev_info(host->dev,
				"switch_gpio_number_1 property not found, "
				"using value of SWITCH_GPIO_DEFAULT_NUMBER as default\n");
			value = SWITCH_GPIO_DEFAULT_NUMBER;
		}
		...
		value = SWITCH_GPIO_DEFAULT_NUMBER;
		if (of_property_read_u32_index(
			    np, "switch_gpio_nums", 2, &value)) {
			dev_info(host->dev,
				"switch_gpio_number_2 property not found, "
				"using value of SWITCH_GPIO_DEFAULT_NUMBER as default\n");
			value = SWITCH_GPIO_DEFAULT_NUMBER;
		}
		...
		value = SWITCH_GPIO_DEFAULT_NUMBER;
		if (of_property_read_u32_index(
			    np, "switch_gpio_nums", 3, &value)) {
			dev_info(host->dev,
				"switch_gpio_number_3 property not found, "
				"using value of SWITCH_GPIO_DEFAULT_NUMBER as default\n");
			value = SWITCH_GPIO_DEFAULT_NUMBER;
		}
	...
	if (of_property_read_u32(
		    np, "board-mmc-bus-clk", &(priv->dw_mmc_bus_clk))) {
	...
	priv->dw_voltage_switch_gpio =
		of_get_named_gpio(np, "board-sd-voltage-switch-gpio", 0);
...
static int dw_mci_hs_get_cd(struct dw_mci *host, u32 slot_id)
{
	...
	if ((host->hw_mmc_id == DWMMC_SD_ID) &&
		...
		dev_info(host->dev,
			" sd status set to 1 here because jtag is enabled.\n");
		status = 1;
	}

#ifdef CONFIG_MMC_DW_MUX_SDSIM
	if (host->hw_mmc_id == DWMMC_SD_ID) {
		if (first_mux_sdsim_probe_init == 0) {
...
static int dw_mci_hs_cd_detect_init(struct dw_mci *host)
{
	...
	if (host->flags & DWMMC_HPD_IRQ) {
		hpd_irq = hisi_get_pmic_irq_byname(SIM1_HPD_F);
	...
	if (gpio_is_valid(gpio)) {
		if (devm_gpio_request_one(
			...
			if (of_property_read_u32(
				...
				dev_info(host->dev,
					"shared-irq property not found, using "
...
static int dw_mci_1_8v_signal_voltage_switch(struct dw_mci_slot *slot)
{
	 ...
	 */
	mci_writel(
		host, CTRL, (mci_readl(host, CTRL) & ~SDMMC_CTRL_INT_ENABLE));
...
static void dw_mci_hi3xxx_mci_send_cmd(struct dw_mci *host, u32 cmd, u32 arg)
{
	...
	dev_err(host->dev,
		"hi3xxx_dw_mmc Timeout sending command (cmd %#x arg %#x status %#x)\n",
		cmd, arg, cmd_status);
}
...
void dw_mci_hi3xxx_work_fail_reset(struct dw_mci *host)
{
	...
	mci_writel(
		host, CTRL, SDMMC_CTRL_INT_ENABLE); /* Enable mci interrupt */
	...
	/* inform CIU */
	dw_mci_hi3xxx_mci_send_cmd(
		host, SDMMC_CMD_UPD_CLK | SDMMC_CMD_PRV_DAT_WAIT, 0);
...
#ifdef CONFIG_SDIO_HI_CLOCK_RETUNING
void dw_mci_hs_get_timing_config_normal(struct dw_mci *host)
{
	struct device_node *np = host->dev->of_node;
	int ret;
...
void dw_mci_hs_get_timing_config_high_speed_clk(struct dw_mci *host)
{
	struct device_node *np = host->dev->of_node;
	int ret;

	ret = of_property_read_u32_array(
		np, "use_samdly_range_hi", &(host->use_samdly_range[0]), 2);
	if (ret) {
		dev_info(host->dev,
			"%s:get use_samdly_range_hi range range failed!\n", __func__);
		host->use_samdly_range[0] = 9;
		host->use_samdly_range[1] = 18;
	} else {
		dev_info(host->dev, "%s:use_samdly_range_hi_min:%d,use_samdly_"
				    "range_hi_max:%d!\n",
			__func__, host->use_samdly_range[0],
			host->use_samdly_range[1]);
	}
	...
	ret = of_property_read_u32_array(
		np, "enable_shift_range_hi", &(host->enable_shift_range[0]), 2);
	if (ret) {
		dev_info(host->dev,
			"%s:get enable_shift_range_hi range range failed!\n", __func__);
		host->enable_shift_range[0] = 8;
		host->enable_shift_range[1] = 17;
	} else {
		dev_info(host->dev, "%s:ena_shift_range_hi_min:%d,ena_shift_"
				    "range_hi_max:%d!\n",
			__func__, host->enable_shift_range[0],
			host->enable_shift_range[1]);
	}
 ...
 */
void dw_mci_hs_960_clock_source_retuning(struct dw_mci *host)
{
	struct dw_mci_hs_priv_data *priv = host->priv;
	...
	host->tuning_init_sample = host->tuning_init_sample / 2;
	priv->old_timing = -1;
	priv->in_resume = STATE_KEEP_PWR;
	host->sd_reinit = 0;

	dw_mci_hs_get_timing_config_normal(host);
	dw_mci_hs_set_clk(host, &host->cur_slot->mmc->ios);
 ...
 */
void dw_mci_hs_1920_clock_source_retuning(struct dw_mci *host)
{
	struct dw_mci_hs_priv_data *priv = host->priv;
	...
	if (!IS_ERR(host->volt_hold_sdio_clk)) {
		if (clk_prepare_enable(host->volt_hold_sdio_clk))
			dev_err(host->dev, "volt_hold_sdio_clk "
	...
	struct dw_mci *host, int *ret, int sample_max, int timing)
{
	if ((host->tuning_flag_all_pass == TUNING_FLAG_ALL_PASS) &&
		...
		} else if (((2 << (unsigned int)sample_max) - 1) ==
			   host->tuning_sample_flag) {
		...
		dev_info(host->dev,
			"all tuning sample return ok,try to retuning\n");
...
static int dw_mci_hs_tuning_find_condition(struct dw_mci *host, int timing)
{
	...
	if (host->tuning_current_sample >= sample_max) {
		...
		for (mask_lenth =
				(((unsigned int)(sample_max - sample_min) >> 1) << 1) + 1;
			...
			if (host->tuning_init_sample != -1) {
				if ((host->hw_mmc_id == DWMMC_SD_ID) &&
					...
					dev_info(host->dev,
						"sd card tuning need slow down clk, timing is %d, "
					...
					ret = 0;
#ifdef CONFIG_SDIO_HI_CLOCK_RETUNING
					dw_mci_hs_tuning_all_pass(
						host, &ret, sample_max, timing);
#endif
		...
		if (-1 == host->tuning_init_sample) {
			...
			if (host->hw_mmc_id == DWMMC_EMMC_ID) {
				DSM_EMMC_LOG(host->cur_slot->mmc,
...
#ifdef CONFIG_HUAWEI_DSM
			if (host->hw_mmc_id == DWMMC_SDIO_ID)
				dsm_wifi_client_notify(DSM_SDIO_TUNNING_ERR,
					"sdio tuning error: timing %d, flag 0x%x\n",
					timing, host->tuning_sample_flag);
#endif
			ret = -1;
		}
...
static void dw_mci_hisi_change_timing_config_kirin970(struct dw_mci *host)
{
	if (host->hw_mmc_id == DWMMC_SD_ID) {
		memcpy(hs_timing_config[DWMMC_SD_ID][MMC_TIMING_UHS_SDR104],
			hs_timing_config_kirin970_cs_sd_ppll3,
			sizeof(hs_timing_config_kirin970_cs_sd_ppll3)); /* unsafe_function_ignore: memcpy */
	} else if (host->hw_mmc_id == DWMMC_SDIO_ID) {
		memcpy(hs_timing_config[DWMMC_SDIO_ID][MMC_TIMING_UHS_SDR104],
			hs_timing_config_kirin970_cs_sdio_ppll3,
			sizeof(hs_timing_config_kirin970_cs_sdio_ppll3)); /* unsafe_function_ignore: memcpy */
	}
}
...
retry:
	...
	do {
		...
		if (ret == -1) {
			...
			break;
#ifdef CONFIG_SDIO_HI_CLOCK_RETUNING
		else if (ret == TUNING_ERR_ALL_PHASE_PASS)
			break;
#endif
...
void dw_mci_set_clk_08v(struct dw_mci *host)
{
	if (!IS_ERR(host->volt_hold_sd_clk) &&
		...
		if (clk_prepare_enable(host->volt_hold_sd_clk))
			dev_err(host->dev,
				"volt_hold_sd_clk clk_prepare_enable failed\n");
	}
	...
	if (!IS_ERR(host->volt_hold_sdio_clk) &&
		...
		if (clk_prepare_enable(host->volt_hold_sdio_clk))
			dev_err(host->dev,
				"volt_hold_sdio_clk clk_prepare_enable failed\n");
	}
}
#endif
...
static int dw_mci_hs_resume(struct device *dev)
{
	...
	if (!IS_ERR(host->biu_clk)) {
		if (clk_prepare_enable(host->biu_clk))
			dev_err(host->dev,
				"biu_clk clk_prepare_enable failed\n");
	}
...
void dw_mci_reg_dump_fortrace(struct mmc_host *mmc)
{
	...
	mmc_trace_comm_record(
		mmc, ": CDTHRCTL: 	0x%x\n", mci_readl(host, CDTHRCTL));
	mmc_trace_comm_record(
		mmc, ": UHS_REG_EXT:	0x%x\n", mci_readl(host, UHS_REG_EXT));
	mmc_trace_comm_record(
		mmc, ": ============== STATUS DUMP ================\n");
	mmc_trace_comm_record(
		mmc, ": cmd_status:      0x%x\n", host->cmd_status);
	mmc_trace_comm_record(
		mmc, ": data_status:     0x%x\n", host->data_status);
	mmc_trace_comm_record(
		mmc, ": pending_events:  0x%x\n", host->pending_events);
	mmc_trace_comm_record(
		mmc, ": completed_events:0x%x\n", host->completed_events);
	...
	mmc_trace_comm_record(
		mmc, ": RESP3:	0x%x\n", mci_readl(host, RESP3));
	mmc_trace_comm_record(
		mmc, ": host : cmd_status=0x%x, data_status=0x%x.\n",
		host->cmd_status, host->data_status);
	mmc_trace_comm_record(
		mmc, ": host : pending_events=0x%x, completed_events=0x%x.\n",
		host->pending_events, host->completed_events);
...
bool dw_mci_fifo_reset(struct device *dev, struct dw_mci *host)
{
	...
	do {
		result = dw_mci_wait_reset(
			host->dev, host, SDMMC_CTRL_FIFO_RESET);
...
void dw_mci_set_clk_peri_08v(struct dw_mci *host)
{
	...
	if ((host->hw_mmc_id == DWMMC_SD_ID) &&
		...
		queue_work(host->card_workqueue,
			&host->work_volt_mmc); /* dw_mci_work_volt_mmc_func */
	}
