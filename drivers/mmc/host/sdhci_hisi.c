...
static int sdhci_trylock_hostlock(struct sdhci_host *host, unsigned long *flags)
{
	...
	do {
		locked = spin_trylock_irqsave(
			&host->lock, *flags); /*lint !e730 !e550 !e1072 !e666*/
		if (locked)
			break;
...
void sdhci_retry_req(struct sdhci_host *host, struct mmc_request *mrq)
{
	...
	if (host->mmc->card && mmc_card_mmc(host->mmc->card) &&
		...
		if ((mrq->data && mrq->data->error &&
			...
			ret = host->ops->tuning_move(
				host, TUNING_STROBE, TUNING_FLAG_NOUSE);
		} else {
			ret = host->ops->tuning_move(
		...
		if (vdd == 0) {
			if (mmc->regulator_enabled) {
				if (!regulator_disable(mmc->supply.vmmc))
					...
					pr_err("%s: regulator vmmc disable "
					       "failed !\n", __func__);
			}
		} else {
			if (regulator_set_voltage(
				    mmc->supply.vmmc, 2950000, 2950000)) {
			...
			if (!mmc->regulator_enabled) {
				if (!regulator_enable(mmc->supply.vmmc))
					...
					pr_err("%s: regulator vmmc enable "
					       "failed !\n", __func__);
			}
		}
	} else {
