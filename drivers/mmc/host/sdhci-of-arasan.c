...
void __iomem *iosoc_acpu_pmc_base_addr;
...
static void sdhci_set_strobe_sample(
	struct sdhci_host *host, unsigned char timing, int sam_phase);
...
static void sdhci_of_arasan_hardware_reset(
	struct sdhci_host *host, unsigned char set)
{
...
static void sdhci_of_arasan_PHY_set_dll(
	struct sdhci_host *host, u32 dll_freq_sel)
{
	u32 reg_data = 0;
	...
	if (dll_freq_sel == 0xff) {
		...
		reg_data = reg_data & (~(SDHCI_PHY_CTRL3_DLL_FREQ_SEL_MASK
					      << SDHCI_PHY_CTRL3_DLL_FREQ_SEL));
		reg_data = reg_data | ((dll_freq_sel &
				SDHCI_PHY_CTRL3_DLL_FREQ_SEL_MASK)
				<< SDHCI_PHY_CTRL3_DLL_FREQ_SEL);
...
static void sdhci_of_arasan_PHY_init(struct sdhci_host *host)
{
	...
	reg_data = reg_data & (~(SDHCI_PHY_CTRL1_DLL_TRIM_ICP_MASK
				      << SDHCI_PHY_CTRL1_DLL_TRIM_ICP));
	reg_data = reg_data | ((0x4 & SDHCI_PHY_CTRL1_DLL_TRIM_ICP_MASK)
				      << SDHCI_PHY_CTRL1_DLL_TRIM_ICP);
	sdhci_writel(host, reg_data, SDHCI_PHY_CTRL1);
...
static u32 freq_sel_efuse_enable;
static u32 freq_sel_efuse_value;
...
static void sdhci_of_arasan_update_phy_control(
	struct sdhci_host *host, unsigned char timing)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_arasan_data *sdhci_arasan = pltfm_host->priv;
	u32 reg_data = 0;
	...
	/*
	 * DLL support 50MHz to 200MHz clk config.
	 * 0: 200-170MHz, 1: 170-140MHz, 2:140-110MHz,
	 * 3: 110-80MHz, 4: 80-50MHz, 7: 200-225MHz
	 */
	...
	/*
	 * SDHCI_PHY_CTRL3_OTAPDLYENA:enable manual adjustment of TX clk phase
	 * SDHCI_PHY_CTRL3_OTAPDLYSEL:TX clk phase:0-0xFㄛdefault:0
	 * SDHCI_PHY_CTRL3_ITAPDLYENA:enable manual adjustment of RX clk phase,
	 *                            Use the phase value of automatic tuning
	 *                            when not enabling
	 * The software is configured to not enable when initialization
	 * SDHCI_PHY_CTRL3_ITAPDLYSEL:RX clk phase:0-0x1F, default:0
	 * SDHCI_PHY_CTRL1_DR_TY:impedance value,
	 * support for the following four; default:0x0
	 * 0x0-50次
	 * 0x1-33次 the most powerful
	 * 0x2-66次
	 * 0x3-100次 the weakest driving force
	 * 0x4-40次
	 */
	...
	if (tx_phase == 0xff) { /* disable tx phase */
		...
		reg_data = reg_data | SDHCI_PHY_CTRL3_OTAPDLYENA;
		reg_data = reg_data & (~(SDHCI_PHY_CTRL3_OTAPDLYSEL_MASK
				<< SDHCI_PHY_CTRL3_OTAPDLYSEL));
		reg_data = reg_data | ((tx_phase & SDHCI_PHY_CTRL3_OTAPDLYSEL_MASK)
				<< SDHCI_PHY_CTRL3_OTAPDLYSEL);
	}
	sdhci_writel(host, reg_data, SDHCI_PHY_CTRL3);
	...
	if (rx_phase == 0xff) { /* disable rx phase */
		...
		reg_data = reg_data | SDHCI_PHY_CTRL3_ITAPDLYENA;
		reg_data = reg_data & (~(SDHCI_PHY_CTRL3_ITAPDLYSEL_MASK
				<< SDHCI_PHY_CTRL3_ITAPDLYSEL));
		reg_data = reg_data | ((rx_phase & SDHCI_PHY_CTRL3_ITAPDLYSEL_MASK)
				<< SDHCI_PHY_CTRL3_ITAPDLYSEL);
	}
	sdhci_writel(host, reg_data, SDHCI_PHY_CTRL3);
	...
	reg_data = reg_data | ((drv_strength & SDHCI_PHY_CTRL1_DR_TY_MASK)
			<< SDHCI_PHY_CTRL1_DR_TY);
	sdhci_writel(host, reg_data, SDHCI_PHY_CTRL1);
	...
	struct sdhci_host *host, unsigned char timing, int sam_phase)
{
	...
	reg_data = sdhci_readl(host, SDHCI_PHY_CTRL3);
	reg_data = reg_data & (~(SDHCI_PHY_CTRL3_STRBSEL_MASK
			<< SDHCI_PHY_CTRL3_STRBSEL));
	...
	reg_data = reg_data | ((sam_phase_set & SDHCI_PHY_CTRL3_STRBSEL_MASK)
			<< SDHCI_PHY_CTRL3_STRBSEL);
	sdhci_writel(host, reg_data, SDHCI_PHY_CTRL3);
	...
	struct sdhci_host *host, unsigned char timing, int sam_phase)
{
	...
	if (sam_phase == 0xff) {
		...
		reg_data = reg_data & (~(SDHCI_PHY_CTRL3_ITAPDLYSEL_MASK
						<< SDHCI_PHY_CTRL3_ITAPDLYSEL));
		reg_data = reg_data | ((sam_phase & SDHCI_PHY_CTRL3_ITAPDLYSEL_MASK)
						<< SDHCI_PHY_CTRL3_ITAPDLYSEL);
		sdhci_writel(host, reg_data, SDHCI_PHY_CTRL3);
		udelay(10);
		reg_data = sdhci_readl(host, SDHCI_PHY_CTRL3);
 ...
 */
static int sdhci_of_arasan_tuning_find_condition(
	struct sdhci_host *host, unsigned char timing)
{
	int i, j;
	int ret = -1;
	...
	pr_debug("%s: timing is %d, tuning_current_sample = 0X%x, "
		 "tuning_init_sample=0X%x\n",
		__func__, timing, sdhci_arasan->tuning_current_sample,
		sdhci_arasan->tuning_init_sample);

	if (SDHCI_TUNING_MAX_SAMPLE == SDHCI_TUNING_MIN_SAMPLE) {
		sdhci_arasan->tuning_init_sample =
			(SDHCI_TUNING_MAX_SAMPLE + SDHCI_TUNING_MIN_SAMPLE) / 2;
		sdhci_set_sample(
			host, timing, sdhci_arasan->tuning_init_sample);
		pr_debug("%s: no need tuning: timing is %d, tuning sample = "
			 "%d\n", __func__, timing, sdhci_arasan->tuning_init_sample);
		return 0;
	}

	if (sdhci_arasan->tuning_current_sample == -1) {
	...
	if (sdhci_arasan->tuning_sample_count >= MAX_TUNING_LOOP) {
		...
		sdhci_arasan->tuning_sample_flag |=
			((sdhci_arasan->tuning_sample_flag &
				((1UL << (SDHCI_TUNING_COUNT -
				MAX_TUNING_LOOP)) - 1)) << MAX_TUNING_LOOP);

		/* tuning finish, select the best sam_del */
		/* set sam del to -1, for next tuning */
		sdhci_arasan->tuning_current_sample = -1;
		sdhci_arasan->tuning_init_sample = 0xff;
		for (mask_lenth = (((SDHCI_TUNING_COUNT - 1 - SDHCI_TUNING_MIN_SAMPLE)
				>> 1) << 1) + 1; mask_lenth >= 1; mask_lenth -= 2) {
			mask = ((unsigned long)1 << mask_lenth) - 1;
			for (i = (SDHCI_TUNING_MIN_SAMPLE + SDHCI_TUNING_COUNT -
					1 - mask_lenth + 1) / 2, j = 1;
					(i <= SDHCI_TUNING_COUNT - 1 - mask_lenth + 1) &&
					(i >= SDHCI_TUNING_MIN_SAMPLE);
					i = ((SDHCI_TUNING_MIN_SAMPLE +
					SDHCI_TUNING_COUNT - 1 - mask_lenth + 1) /
					2) + ((j % 2) ? -1 : 1) * (j / 2)) {
				if ((sdhci_arasan->tuning_sample_flag &
					    (mask << i)) == (mask << i)) {
					sdhci_arasan->tuning_init_sample =
						i + mask_lenth / 2;
					break;
			...
			if (sdhci_arasan->tuning_init_sample != 0xff) {
				if (sdhci_arasan->tuning_init_sample >=
					MAX_TUNING_LOOP) {
					sdhci_arasan->tuning_init_sample =
						sdhci_arasan->tuning_init_sample -
						MAX_TUNING_LOOP;
				}
				pr_err("kernel tuning OK: timing is %d, tuning "
					"sample = %d, tuning_flag = 0x%lx\n",
					timing, sdhci_arasan->tuning_init_sample,
					sdhci_arasan->tuning_sample_flag);
				ret = 0;
				break;
			}
		...
		if (ret) {
			pr_err("tuning err: no good sam_del, "
				"timing is %d, tuning_init_sample=%d, "
			    	"tuning_flag = 0x%lx.\n",
				timing, sdhci_arasan->tuning_init_sample,
				sdhci_arasan->tuning_sample_flag);
			ret = -1;
		...
		if (sdhci_arasan->tuning_current_sample >
			SDHCI_TUNING_MAX_SAMPLE) {
			sdhci_arasan->tuning_current_sample =
				SDHCI_TUNING_MIN_SAMPLE;
		}
		sdhci_set_sample(
			host, timing, sdhci_arasan->tuning_current_sample);
		ret = 1;
	}
	return ret;
}

static void sdhci_of_arasan_tuning_set_current_state(
	struct sdhci_host *host, int ok)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_arasan_data *sdhci_arasan = pltfm_host->priv;
...
static int sdhci_of_arasan_tuning_move_strobe(struct sdhci_host *host)
{
	...
	/*
	 * first move tuning, set soft tuning optimum strobe sample. When second
	 * or more move tuning, use the sample near optimum sample
	 */
	for (loop = 0; loop < 2; loop++) {
		sdhci_arasan->tuning_strobe_move_count++;
		sdhci_arasan->tuning_strobe_move_sample =
			sdhci_arasan->tuning_strobe_init_sample +
			((sdhci_arasan->tuning_strobe_move_count % 2) ? 1 : -1) *
			(sdhci_arasan->tuning_strobe_move_count / 2);

		if ((sdhci_arasan->tuning_strobe_move_sample >
			    SDHCI_TUNING_STROBE_MAX_SAMPLE) ||
			(sdhci_arasan->tuning_strobe_move_sample <
				SDHCI_TUNING_STROBE_MIN_SAMPLE)) {
			continue;
		} else {
			break;
		}
	...
	if ((sdhci_arasan->tuning_strobe_move_sample >
		...
		sdhci_set_strobe_sample(host, host->mmc->ios.timing,
			sdhci_arasan->tuning_strobe_move_sample);
...
static int sdhci_of_arasan_tuning_move_clk(struct sdhci_host *host)
{
	...
	if (sdhci_arasan->tuning_init_sample > SDHCI_TUNING_MAX_SAMPLE) {
		pr_err("%s: soft tuning fail, can not move tuning, "
		       "tuning_init_sample=%d.\n",
			__func__, sdhci_arasan->tuning_init_sample);
		return -1;
	}
	/*
	 * first move tuning, set soft tuning optimum sample. When second or
	 * more move tuning, use the sample near optimum sample
	 */
	for (loop = 0; loop < 2; loop++) {
		sdhci_arasan->tuning_move_count++;
		sdhci_arasan->tuning_move_sample =
			sdhci_arasan->tuning_init_sample +
			((sdhci_arasan->tuning_move_count % 2) ? 1 : -1) *
			(sdhci_arasan->tuning_move_count / 2);

		if ((sdhci_arasan->tuning_move_sample >
			    SDHCI_TUNING_MAX_SAMPLE) ||
			(sdhci_arasan->tuning_move_sample <
				SDHCI_TUNING_MIN_SAMPLE)) {
			continue;
	...
	if ((sdhci_arasan->tuning_move_sample > SDHCI_TUNING_MAX_SAMPLE) ||
		(sdhci_arasan->tuning_move_sample < SDHCI_TUNING_MIN_SAMPLE)) {
		pr_err("%s: tuning move fail.\n", __func__);
		sdhci_arasan->tuning_move_sample = 0xff;
		ret = -1;
	} else {
		sdhci_set_sample(host, host->mmc->ios.timing,
			sdhci_arasan->tuning_move_sample);
 ...
 */
static int sdhci_of_arasan_tuning_move(
	struct sdhci_host *host, int is_move_strobe, int flag)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_arasan_data *sdhci_arasan = pltfm_host->priv;

	/*
	 * set tuning_strobe_move_count to 0, next tuning move will begin from
	 * optimum sample
	 */
	if (flag) {
		sdhci_arasan->tuning_move_count = 0;
...
static int sdhci_of_arasan_tuning_soft(
	struct sdhci_host *host, u32 opcode, bool set)
{
	...
	if (set)
		ret = sdhci_of_arasan_tuning_move(
			host, TUNING_CLK, TUNING_FLAG_NOUSE);
...
int sdhci_of_arasan_chk_busy_before_send_cmd(
...
static int sdhci_arasan_select_drive_strength(struct sdhci_host *host,
	struct mmc_card *card, unsigned int max_dtr, int host_drv, int card_drv,
	int *drv_type)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_arasan_data *sdhci_arasan = pltfm_host->priv;
...
static int sdhci_arasan_suspend(struct device *dev)
{
	...
	/*
	 * disable dll then enable dll & wait ready,
	 * finally disable dll again.
	 */
...
static int sdhci_arasan_resume(struct device *dev)
{
	...
	pr_debug("%s: sdhci_arasan->clk_ahb=%lu, sdhci_arasan->clk=%lu.\n",
		__func__, clk_get_rate(sdhci_arasan->clk_ahb),
		clk_get_rate(sdhci_arasan->clk));
	...
	if (host->mmc->ios.timing >= MMC_TIMING_MMC_HS200) {
		pr_info("%s: tuning_move_sample=%d, tuning_init_sample=%d, "
			"tuning_strobe_move_sample=%d, "
			"tuning_strobe_init_sample=%d.\n",
		...
		/*
		 * START for emmc data crc & end bit error when resume first
		 * transfer data.
		 */
		sdhci_set_clock(host, host->mmc->ios.clock);
		if ((host->mmc->ios.clock <= 200000000) &&
			(host->mmc->ios.clock >= 190000000)) {
			if (freq_sel_efuse_enable)
				dll_freq_data = freq_sel_efuse_value;
			else
				dll_freq_data = hs400_dll_freq_data;
		} else if ((host->mmc->ios.clock < 190000000) &&
			   (host->mmc->ios.clock >= 170000000)) {
			dll_freq_data = 0x0;
		} else if ((host->mmc->ios.clock < 170000000) &&
			   (host->mmc->ios.clock >= 140000000)) {
			dll_freq_data = 0x1;
		} else if ((host->mmc->ios.clock < 140000000) &&
			   (host->mmc->ios.clock >= 110000000)) {
			dll_freq_data = 0x2;
		} else if ((host->mmc->ios.clock < 110000000) &&
			   (host->mmc->ios.clock >= 80000000)) {
			dll_freq_data = 0x3;
		} else if ((host->mmc->ios.clock < 80000000) &&
			   (host->mmc->ios.clock >= 50000000)) {
			dll_freq_data = 0x4;
		} else {
			dll_freq_data = 0xff;
		}
		sdhci_of_arasan_PHY_set_dll(host, dll_freq_data);
		/*
		 * END for emmc data crc & end bit error when resume first
		 * transfer data.
		 */
	}
...
static int sdhci_arasan_runtime_resume(struct device *dev)
{
	...
	if (!IS_ERR(sdhci_arasan->clk)) {
		if (clk_prepare_enable(sdhci_arasan->clk))
			pr_warn("%s: clk_prepare_enable sdhci_arasan->clk "
				"failed.\n", __func__);
	}
...
static void sdhci_dsm_work(struct work_struct *work)
{
	...
	struct sdhci_arasan_data *sdhci_arasan =
		container_of(work, struct sdhci_arasan_data, dsm_work);
...
void sdhci_dsm_handle(struct sdhci_host *host, struct mmc_request *mrq)
{
	...
	if (error_bits) {
		...
		if (error_bits) {
			sdhci_arasan->para =
				((error_bits << 16) |
				((mrq->cmd ? mrq->cmd->opcode : 0) &
				0x3f) | 0x8000000000000000ULL); /*lint !e647*/
			queue_work(system_freezable_wq,
				&sdhci_arasan->dsm_work);
		}
	}
}
...
/*lint -save -e593*/
	...
	if (of_device_is_available(np)) {
		...
		if (!of_property_read_u32_array(
			    np, "drv-strength", config_drv_strength, 2)) {
			if (config_drv_strength[0] > 0x4 ||
				config_drv_strength[1] > 0x4) {
				dev_err(&pdev->dev, "drv_strength value error, "
						    "use the default value!\n");
				sdhci_arasan->host_drv_strength =
					HOST_DRV_STRENGTH_40; /*use default 40ohm*/
				sdhci_arasan->dev_drv_strength =
					EXT_CSD_DRVIER_STRENGTH_40; /*use default 40ohm*/
			} else {
				sdhci_arasan->host_drv_strength =
					config_drv_strength[0]; /*mask 0-4bit*/
				sdhci_arasan->dev_drv_strength =
					config_drv_strength[1]; /*mask 0-4bit*/
			}
		} else {
			dev_err(&pdev->dev, "drv_strength-config cannot get "
					    "from dts, use the default value!\n");
			sdhci_arasan->host_drv_strength =
				HOST_DRV_STRENGTH_40; /* use default 40ohm */
			sdhci_arasan->dev_drv_strength =
				EXT_CSD_DRVIER_STRENGTH_40; /* use default 40ohm */
	...
	if (ret)
		dev_err(&pdev->dev,
			"Error setting desired ahb_clk=%u, get clk=%lu.\n",
			ahb_clk, clk_get_rate(sdhci_arasan->clk_ahb));
	ret = clk_prepare_enable(sdhci_arasan->clk_ahb);
	if (ret) {
	...
	pr_info("%s: clk_ahb=%lu, clk_xin=%lu\n", __func__,
		clk_get_rate(sdhci_arasan->clk_ahb),
		clk_get_rate(sdhci_arasan->clk));
...
#define HW_CMDQ_REG_OFF 0x200
	if (host->mmc->caps2 & MMC_CAP2_CMD_QUEUE) {
		host->cq_host =
			cmdq_pltfm_init(pdev, host->ioaddr + HW_CMDQ_REG_OFF);
		if (IS_ERR(host->cq_host)) {
			ret = PTR_ERR(host->cq_host);
			dev_err(&pdev->dev,
				"cmd queue platform init failed (%u)\n", ret);
