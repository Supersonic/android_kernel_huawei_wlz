...
static u32 sdhci_i2c_readl(struct sdhci_host *host, u32 addr)
{
	...
	if (host->i2c_client) {
		...
		ret = i2c_transfer(
			host->i2c_client->adapter, (struct i2c_msg *)msg, 2);
		if (ret < 0)
			pr_err("%s  fail\n", __func__);
	} else {
		pr_err("%s  fail client empty\n", __func__);
...
void sdhci_mshc_set_strobe_move_para(struct sdhci_host *host)
{
	...
	sdhci_mshc->tuning_strobe_phase_max =
		sdhci_mshc->tuning_strobe_phase_init + move_step * 2;
...
void sdhci_combo_phy_delay_measurement(
...
int sdhci_chk_busy_before_send_cmd(
...
static void sdhci_mshc_save_tuning_phase(struct sdhci_host *host)
{
	...
	if (sdhci_mshc->tuning_count < TUNING_LOOP_64)
		...
		sdhci_mshc->tuning_phase_record_high |=
			(u64)0x1 << (sdhci_mshc->tuning_count - TUNING_LOOP_64);
}
...
static int sdhci_mshc_find_best_phase(struct sdhci_host *host)
{
	...
	if ((tuning_phase_record & 0x1) &&
		(tuning_phase_record & ((u64)0x1 << 63)))
	...
	sdhci_mshc->tuning_phase_best =
		max_continuous_start + max_continuous_len / 2;
	if (sdhci_mshc->tuning_phase_best >= TUNING_LOOP_64)
		sdhci_mshc->tuning_phase_best =
			sdhci_mshc->tuning_phase_best - TUNING_LOOP_64;
...
static int sdhci_mshc_tuning_move_strobe(struct sdhci_host *host)
{
	 ...
	 * When second or moreb move tuning,
	...
	if ((sdhci_mshc->tuning_strobe_move_phase >=
		...
		sdhci_mshc_set_tuning_strobe_phase(
			host, sdhci_mshc->tuning_strobe_move_phase);
...
static int sdhci_mshc_tuning_move_clk(struct sdhci_host *host)
{
	...
	for (loop = 0; loop < 2; loop++) {
		...
		sdhci_mshc->tuning_move_phase =
			sdhci_mshc->tuning_phase_best +
			((sdhci_mshc->tuning_move_count % 2) ? move_step
							     : -move_step) *
				(sdhci_mshc->tuning_move_count / 2);

		if ((sdhci_mshc->tuning_move_phase >=
			    sdhci_mshc->tuning_phase_max) ||
			(sdhci_mshc->tuning_move_phase <
				sdhci_mshc->tuning_phase_min)) {
			continue;
		} else {
			break;
		}
	...
	if ((sdhci_mshc->tuning_move_phase >= sdhci_mshc->tuning_loop_max) ||
		...
		sdhci_mshc_set_tuning_phase(
			host, sdhci_mshc->tuning_move_phase);
...
static int sdhci_mshc_tuning_move(
...
int sdhci_mshc_soft_tuning(struct sdhci_host *host, u32 opcode, bool set)
{
	struct sdhci_mshc_data *sdhci_mshc =
		((struct sdhci_pltfm_host *)sdhci_priv(host))->priv;
	...
	hw_tuning_phase = CENTER_PH_CODE_MASK &
				sdhci_readl(host, SDHCI_AT_STAT_R);
	...
	while (tuning_loop_counter--) {
		...
		if (!cmd.error && !data.error && tuning_blk &&
			(memcmp(tuning_blk, tuning_blk_pattern,
				 sizeof(blksz)) == 0)) { //lint !e668
...
static int sdhci_mshc_suspend(struct device *dev)
{
	...
	sdhci_mmc_sys_writel(
		host, CTRL_ISO_EN | PHY_ISO_EN, EMMC_SYS_PHY_ISOEN);
...
static int sdhci_mshc_resume(struct device *dev)
{
	...
	sdhci_mmc_sys_writel(
		host, PHY_MTCMOS_EN | CTRL_MTCMOS_EN, EMMC_SYS_MTCMOS_EN);
	do {
		if (count++ > 1000) {
...
static int sdhci_mshc_get_resource(
...
static void sdhci_mshc_cmdq_init(struct platform_device *pdev)
{
	...
	if (host->mmc->caps2 & MMC_CAP2_CMD_QUEUE) {
		cmdq_reg_offset = 0xFFF &
			sdhci_readw(host, SDHCI_VENDOR2_SPECIFIC_AREA);
...
static void sdhci_dsm_work(struct work_struct *work)
{
	...
	struct sdhci_mshc_data *sdhci_mshc =
		container_of(work, struct sdhci_mshc_data, dsm_work);
...
void sdhci_dsm_handle(struct sdhci_host *host, struct mmc_request *mrq)
{
	...
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_mshc_data *sdhci_mshc = pltfm_host->priv;
	error_bits = sdhci_mshc->cmd_data_status;
