...
kbase_sync_fence_out_trigger(struct kbase_jd_atom *katom, int result)
{
	...
	if (!kbase_fence_out_is_ours(katom)) {
		...
		if(katom->dma_fence.fence)
		{
			dev_warn(katom->kctx->kbdev->dev,
				"kctx is 0x%pK, fence is %pK, not our fence\n", katom->kctx, katom->dma_fence.fence);
		}
	...
	return (result != 0) ? BASE_JD_EVENT_JOB_CANCELLED : BASE_JD_EVENT_DONE; /* [false alarm]: thirdparty code */
}
...
void kbase_sync_fence_in_cancel_wait(struct kbase_jd_atom *katom)
{
	if (!kbase_fence_free_callbacks(katom)) {
		...
		dev_err(katom->kctx->kbdev->dev, "kbase_sync_fence_in_cancel_wait kctx %pK atom %pK wait couldn't be cancelled\n", (void *)katom->kctx, katom);
		return;
	}

	dev_err(katom->kctx->kbdev->dev, "kbase_sync_fence_in_cancel_wait kctx %pK atom %pK wait was cancelled\n", (void *)katom->kctx, katom);
