...
int kbasep_js_devdata_init(struct kbase_device * const kbdev)
{
	...
	memset(jsdd->runpool_irq.ctx_attr_ref_count, 0, /* unsafe_function_ignore: memset */
			sizeof(jsdd->runpool_irq.ctx_attr_ref_count));
	memset(jsdd->runpool_irq.slot_affinities, 0, /* unsafe_function_ignore: memset */
			sizeof(jsdd->runpool_irq.slot_affinities));
	memset(jsdd->runpool_irq.slot_affinity_refcount, 0, /* unsafe_function_ignore: memset */
...
int kbasep_js_kctx_init(struct kbase_context *const kctx)
{
	...
	memset(js_kctx_info->ctx.ctx_attr_ref_count, 0, /* unsafe_function_ignore: memset */
						...
						int js)
{
	...
	for (i = 0; i < KBASE_JS_ATOM_SCHED_PRIO_COUNT; i++) {
		...
		kctx = list_entry(kbdev->js_data.ctx_list_pullable[js][i].next, /* [false alarm]: thirdparty code */
...
void kbasep_js_resume(struct kbase_device *kbdev)
{
	...
	for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
		for (prio = 0; prio < KBASE_JS_ATOM_SCHED_PRIO_COUNT; prio++) {
			...
			list_for_each_entry_safe(kctx, n, /* [false alarm]: thirdparty code */
