					...
					u8 d, bool ctx_is_dying)
{
	...
	while (!list_empty(&katom->dep_head[d])) {
		...
		dep_atom = list_entry(katom->dep_head[d].next, /* [false alarm]: thirdparty code */
		...
		struct kbase_jd_atom *node)
{
	...
	for (i = 0; i < 2; i++) {
		...
		list_for_each(pos, &node->dep_head[i]) {
			struct kbase_jd_atom *dep_atom = list_entry(pos, /* [false alarm]: thirdparty code */
	...
	for (i = 0; i < 2; i++) {
		...
		if (dep_atom->status == KBASE_JD_ATOM_STATE_UNUSED ||
			...
			KBASE_TLSTREAM_TL_NEW_ATOM(
					...
					katom,
					kbase_jd_atom_id(kctx, katom)); //lint !e648
			KBASE_TLSTREAM_TL_RET_ATOM_CTX(kbdev, katom, kctx); //lint !e648
		...
		bool uk6_atom)
{
	...
	for (i = 0; i < nr_atoms; i++) { //lint !e574
