		...
		dma_addr_t handle, size_t size)
{
	 ...
	 */
#if !defined(CONFIG_MALI_PLATFORM_HI3660) || !defined(CONFIG_MALI_GPU_DRM_HI3660)
	...
	struct kbase_va_region *region)
{
	...
	struct kbase_context *kctx = kbase_reg_flags_to_kctx(region);
	...
	kbdev = kctx->kbdev;
	fault_rel_pfn = (faulting_as->pf_data.addr >> PAGE_SHIFT) - region->start_pfn;
	fault_block_pfn = fault_rel_pfn & (~(u64)PAGE_FAULT_MAP_BLOCK_MASK);
	map_block_size = MIN(PAGE_FAULT_MAP_BLOCK,
		 region->cpu_alloc->nents - fault_block_pfn);
	...
	pr_debug("%s kctx %d region %pK, pfn range [%llu, %llu]\n",
		__func__, (int)kctx->tgid, region, region->start_pfn + fault_block_pfn,
	...
	if (!err) {
		/* insert gpu mappings */
		if (region->flags & KBASE_REG_SCRAMBLE_BIT) {
			/* Set scramble bit while doing mmu in GPU kernel. */
			header_page_number = 0;
		} else {
			/* No scramble needed for this buffer, set header_page_number to allocated pages. */
			header_page_number = kbase_reg_current_backed_size(region);
		}
		err = kbase_mmu_insert_pages(kbdev, &kctx->mmu,
			region->start_pfn + fault_block_pfn,
			&region->cpu_alloc->pages[fault_block_pfn], map_block_size,
			region->flags, kctx->as_nr,header_page_number);
	}
 ...
 * @group_id:     The memory group id of the allocating pages.
 *
...
static bool page_fault_try_alloc(struct kbase_context *kctx,
		...
		struct kbase_sub_alloc **prealloc_sas, u8 group_id)
{
	...
	/* Early out without failure if nothing to do. */
	if (!new_pages) {
		*pages_to_grow = 0;
		return true;
	}
	...
	if (new_pages >= (SZ_2M / SZ_4K)) {
		root_pool = &kctx->mem_pools.large[group_id];
		*grow_2mb_pool = true;
	} else {
#endif
		root_pool = &kctx->mem_pools.small[group_id];
		*grow_2mb_pool = false;
#ifdef CONFIG_MALI_2MB_ALLOC
	}
#endif
...
/*lint -e571, -e648, -e574, -e679, -e647, -e514, -e666*/
void page_fault_worker(struct work_struct *data)
{
	...
	bool grow_2mb_pool;
	size_t nr_pages = 0;
	size_t nr_lb_pages = 0;
	bool lb_grown = false;
	int lb_pages_to_grow = 0;
	bool lb_grow_2mb_pool = false;
	struct kbase_sub_alloc *prealloc_sas[4] = { NULL, NULL };
	int i;
	...
	kbdev = container_of(faulting_as, struct kbase_device, as[as_no]); /* [false alarm]: thirdparty code */
...
page_fault_retry:
	...
	/* Alloc the pages routes last-buffer if group id is not zero. */
	if (region->gpu_alloc->group_id != BASE_MEM_GROUP_DEFAULT) {
		if (region->gpu_alloc->lb_pages > region->gpu_alloc->nents)
			nr_lb_pages = min(new_pages, region->gpu_alloc->lb_pages - region->gpu_alloc->nents);

		lb_grown = page_fault_try_alloc(kctx, region, nr_lb_pages, &lb_pages_to_grow,
					&lb_grow_2mb_pool, &prealloc_sas[0], region->gpu_alloc->group_id);
	}

	/* Alloc the pages bypass last-buffer if
	 * (1) No pages needed to allocate from last buffer before normal memory allocate.
	 * (2) The pages that routes last buffer have been allocated successfully.
	 */
	if (!nr_lb_pages || lb_grown) {
		nr_pages = max(new_pages - nr_lb_pages, (size_t)0);
		grown = page_fault_try_alloc(kctx, region, nr_pages, &pages_to_grow,
					&grow_2mb_pool, &prealloc_sas[2], BASE_MEM_GROUP_DEFAULT);
	}
	...
	if (grown) {
		...
		err = kbase_mmu_insert_pages_no_flush(kbdev, &kctx->mmu,
				...
				&kbase_get_gpu_phy_pages(region)[pfn_offset],
				new_pages, region->flags, new_pages);
		if (err) {
			kbase_free_phy_pages_wraper(region->gpu_alloc, new_pages);
			if (region->gpu_alloc != region->cpu_alloc)
				kbase_free_phy_pages_wraper(region->cpu_alloc,
						new_pages);
			kbase_gpu_vm_unlock(kctx);
		 ...
		 */
		if (lb_pages_to_grow > 0) {
#ifdef CONFIG_MALI_2MB_ALLOC
			if (lb_grow_2mb_pool) {
				/* Round page requirement up to nearest 2 MB */
				struct kbase_mem_pool *const lp_mem_pool =
					&kctx->mem_pools.large[region->gpu_alloc->group_id];

				lb_pages_to_grow = (lb_pages_to_grow +
					((1 << lp_mem_pool.order) - 1))
						>> lp_mem_pool.order;

				ret = kbase_mem_pool_grow(lp_mem_pool, lb_pages_to_grow);
			} else {
#endif
				struct kbase_mem_pool *const mem_pool =
					&kctx->mem_pools.small[region->gpu_alloc->group_id];

				ret = kbase_mem_pool_grow(mem_pool, lb_pages_to_grow);
#ifdef CONFIG_MALI_2MB_ALLOC
			}
#endif
		}
					...
					unsigned long flags)
{
	...
	while (remain) {
		...
		for (i = 0; i < count; i++) {
			...
			mmu_mode->entry_set_ate(kctx->kbdev, &pgd_page[ofs],
						phys, flags,
						MIDGARD_MMU_BOTTOMLEVEL);
		}
				    ...
				    unsigned long flags, u64 header_page_number)
{
	...
	while (remain) {
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			mmu_mode->entry_set_ate(kbdev, target, *phys, flags,
						cur_level);
		} else {
			for (i = 0; i < count; i++) {
				...
				/* Allocate/import scramble pages for GPU.
				 * 1. header_page_number == nr means no pages need be scrambled;
				 * 2. header_page_number = 0 means all pages allocated must be scrambled;
				 * 3. 0 < header_page_nmuber < nr, means the first header_page_number pages need no scramble,
				       the reset pages need be scrambled.
				 */
#ifdef CONFIG_GPU_GMC_GENERIC
				if (!kbase_is_entry_decompressed(phys[i])) {
					kbdev->mmu_mode->entry_invalidate(target);
				} else
#endif
				if (header_page_number == nr)
				{
					kbdev->mmu_mode->entry_set_ate(kbdev,
						target, phys[i],
						flags, cur_level);
				}
				else
				{
					...
					{
						kbdev->mmu_mode->entry_set_ate(kbdev, target,
							phys[i], flags, cur_level);
						header_flags++;
					}
					else
					{
						kbdev->mmu_mode->entry_set_ate_scramble_bit(kbdev, target,
							phys[i], flags, cur_level);
					}
				}
			}
...
int kbase_mmu_insert_pages(struct kbase_device *kbdev,
		...
		struct tagged_addr *phys, size_t nr,
		unsigned long flags, int as_nr, u64 header_page_number)
{
	int err;

	err = kbase_mmu_insert_pages_no_flush(kbdev, mmut, vpfn,
			phys, nr, flags, header_page_number);
					...
					unsigned long flags)
{
	...
	while (nr) {
		...
		for (i = 0; i < count; i++)
			mmu_mode->entry_set_ate(kctx->kbdev, &pgd_page[index + i], phys[i],
		...
		int level, u64 *pgd_page_buffer)
{
	 ...
	 * kmap_atomic usage */
	memcpy(pgd_page_buffer, pgd_page, PAGE_SIZE); /* unsafe_function_ignore: memcpy */
...
static size_t kbasep_mmu_dump_level(struct kbase_context *kctx, phys_addr_t pgd, int level, char ** const buffer, size_t *size_left)
{
	...
	if (*size_left >= size) {
		...
		memcpy(*buffer, &m_pgd, sizeof(m_pgd)); /* unsafe_function_ignore: memcpy */
		*buffer += sizeof(m_pgd);

		/* Followed by the page table itself */
		memcpy(*buffer, pgd_page, sizeof(u64) * KBASE_MMU_PAGE_ENTRIES); /* unsafe_function_ignore: memcpy */
...
void *kbase_mmu_dump(struct kbase_context *kctx, int nr_pages)
{
	...
	if (kaddr) {
		...
		if (kctx->api_version >= KBASE_API_VERSION(8, 4)) {
			...
			memcpy(buffer, &config, sizeof(config)); /* unsafe_function_ignore: memcpy */
		...
		/* Add the end marker */
		memcpy(mmu_dump_buffer, &end_marker, sizeof(u64)); /* unsafe_function_ignore: memcpy */
	}
		...
		struct kbase_fault *fault)
{
	...
	if (dsm_report_enable(kbdev->error_num.page_fault, 30, kbdev->runmode_normal)) {
		gpu_dsm_report(kbdev, GPU_PAGE_FAULT_DSM_NO, "Unhandled Page fault!!!!");
	}
#endif
	...
	if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8245)) {
		...
		/* For normal runmode,report dmd every 30 times. */
		if (dsm_report_enable(kbdev->error_num.page_fault, 30, kbdev->runmode_normal)) {
			gpu_dsm_report(kbdev, GPU_PAGE_FAULT_DSM_NO, "Unhandled page fault. For this GPU version we now soft-reset the GPU as part of page fault recovery.");
		}
#endif
		kbdev->error_num.page_fault++;
		kbdev->error_num.ts = hisi_getcurtime();
...
void kbasep_as_do_poke(struct work_struct *work)
{
	...
	kbdev = container_of(as, struct kbase_device, as[as->number]); /* [false alarm]: thirdparty code */
