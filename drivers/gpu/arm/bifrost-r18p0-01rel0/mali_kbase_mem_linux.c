		...
		struct kbase_vmap_struct **out_kern_mapping)
{
	...
	kern_mapping = reg->cpu_alloc->permanent_map; /* [false alarm]: thirdparty code */
...
static unsigned int kbase_mem_get_group_id(struct kbase_context *kctx, u64 *flags)
{
	int i = 0;
	unsigned int group_id = 0;

#ifdef CONFIG_MALI_LAST_BUFFER
	/* Get group_id corresponding to the cache_policy if last buffer enabled. */
	u8 policy = 0;
	struct kbase_device *kbdev = kctx->kbdev;
	struct kbase_policy_manager *manager = kbdev->hisi_dev_data.policy_manager;
	KBASE_DEBUG_ASSERT(manager);

	/* Parse the cache policy from the high 8-bit of flags. */
	policy = (u8)((*flags & BASE_MEM_LB_POLICY_MASK) >> 56);
	for (i = 0; i < BASE_MEM_GROUP_COUNT; ++i) {
		if ((manager->policy[i].cache_policy & 0xF) == ((policy & 0xF0) >> 4)) {
			group_id = manager->policy[i].policy_id;
			break;
		}
	}
#endif

	/* Clear the cache policy setted by the user space. */
	*flags &= ~BASE_MEM_GROUP_ID_MASK;
	*flags &= ~BASE_MEM_LB_POLICY_MASK;
		...
		u64 *gpu_va)
{
	...
	*gpu_va = 0; /* return 0 on failure */
	...
	/* As the user space is not compatiable with the memory group manager, so, we have to
	 * set the memory group id here.
	 */
	group_id = kbase_mem_get_group_id(kctx, flags);
	if (kctx->kbdev->lb_enabled)
		*flags |= base_mem_group_id_set(group_id);
 ...
 */
/*lint -e648, -e574, -e1058, -e58, -e572, -e668, -e578, -e501, -e429, -e1564, -e666, -e570, -e571*/
static
		...
		struct shrink_control *sc)
{
	...
	list_for_each_entry_safe(alloc, tmp, &kctx->evict_list, evict_node) {
		...
		kbase_free_phy_pages_wraper(alloc, alloc->evicted);
		freed += alloc->evicted;
...
bool kbase_mem_evictable_unmake(struct kbase_mem_phy_alloc *gpu_alloc)
{
	...
	if (gpu_alloc->evicted == 0) {
		...
		if (gpu_alloc->reg) {
			...
			err = kbase_alloc_phy_pages_wraper(gpu_alloc,
					gpu_alloc->evicted);
		...
		struct kbase_va_region *reg)
{
	...
	header_page_number = reg->gpu_alloc->header_page_number ? reg->gpu_alloc->header_page_number : kbase_reg_current_backed_size(reg);
		...
		int fd, u64 *va_pages, u64 *flags, u32 padding, u64 header_page_number)
{
	...
	*flags |= BASE_MEM_CACHED_CPU;
	...
	/* For AFBC/scramble memory, we can enlarge the last buffer double. */
	if (reg->flags & KBASE_REG_SCRAMBLE_BIT)
		reg->gpu_alloc->lb_pages *= 2;
		...
		u64 new_pages, u64 old_pages)
{
	...
	ret = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu, reg->start_pfn + old_pages,
			phy_pages + old_pages, delta, reg->flags, kctx->as_nr, delta);
...
int kbase_mem_commit(struct kbase_context *kctx, u64 gpu_addr, u64 new_pages)
{
	...
	if (new_pages > old_pages) {
		...
		if (kbase_alloc_phy_pages_wraper(reg->cpu_alloc, delta) != 0) {
			res = -ENOMEM;
			goto out_unlock;
		}
		if (reg->cpu_alloc != reg->gpu_alloc) {
			if (kbase_alloc_phy_pages_wraper(
					reg->gpu_alloc, delta) != 0) {
				res = -ENOMEM;
				kbase_free_phy_pages_wraper(reg->cpu_alloc,
						delta);
				goto out_unlock;
			}
		}
		...
		kbase_free_phy_pages_wraper(reg->cpu_alloc, delta);
		if (reg->cpu_alloc != reg->gpu_alloc)
			kbase_free_phy_pages_wraper(reg->gpu_alloc, delta);
	}
...
static void kbase_cpu_vm_close(struct vm_area_struct *vma)
{
	...
	if (map->free_on_close) {
		 ...
		 */
		rcu_read_lock();
		task = pid_task(find_vpid(map->kctx->tgid), PIDTYPE_PID);
		if (!(task && (task->flags & PF_EXITING))) {
			rcu_read_unlock();

			/* when task = NULL only when mm_users > 0 need to free region or will let it free in destry context */
			if ((task != NULL) || (!IS_ERR(mm) && (atomic_read(&mm->mm_users) > 0))) {
...
static vm_fault_t kbase_cpu_vm_fault(struct vm_fault *vmf)
{
	...
	kbase_get_compressed_alloc(map->alloc, map_start_pgoff, map->alloc->nents - map_start_pgoff);
	...
	while (i < nents && (addr < vma->vm_end >> PAGE_SHIFT)) {
		// Map a physical page to the CPU.
		struct kbase_device *kbdev = map->kctx->kbdev;
		ret = kbdev->mgm_dev->ops.vm_insert_pfn(kbdev->mgm_dev, vma,
				        addr << PAGE_SHIFT,
		...
		int free_on_close)
{
	...
	if (!kaddr) {
		...
		err = kbdev->mgm_dev->ops.remap_vmalloc_range(kbdev->mgm_dev, vma, kaddr, 0);
		WARN_ON(err);
	}
...
static int kbase_mmu_dump_mmap(struct kbase_context *kctx, struct vm_area_struct *vma, struct kbase_va_region **const reg, void **const kmap_addr)
{
	...
	/* For AFBC/scramble memory, we can enlarge the last buffer double. */
	if (new_reg->flags & KBASE_REG_SCRAMBLE_BIT)
		new_reg->cpu_alloc->lb_pages *= 2;
		...
		struct kbase_vmap_struct *map)
{
	...
	kbase_get_compressed_region(reg, (offset_bytes >> PAGE_SHIFT) + reg->start_pfn, page_count);
	...
	/* There may exists pages that route last buffer, calculate the offset in the pages[] */
	if (page_index < reg->cpu_alloc->lb_pages)
		offset = reg->cpu_alloc->lb_pages - page_index;

	cpu_addr = kbdev->mgm_dev->ops.vmap(kbdev->mgm_dev, pages,
						page_count, offset, VM_MAP, prot);
