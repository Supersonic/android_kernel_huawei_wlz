...
#else
bool corestack_driver_control = false; /* Default value of 0/false */
#endif
					...
					enum kbasep_pm_action action)
{
	...
	if (cores) {
		...
		if (action == ACTION_PWRON)
			...
			state &= ~cores;
		KBASE_TLSTREAM_AUX_PM_STATE(kbdev, core_type, state); //lint !e648
	}
	...
	do {
		...
		default:
			WARN(1, "Invalid state in l2_state: %d",
					backend->l2_state); //lint !e146, !e665
		}
...
static void kbase_pm_shaders_update_state(struct kbase_device *kbdev)
{
	...
	if (!corestack_driver_control) {
		...
		while (kbase_pm_get_state(kbdev, KBASE_PM_CORE_SHADER, ACTION_PWRTRANS)
					&& (timeout < 100000)) {
	...
	do {
		...
		case KBASE_SHADERS_ON_CORESTACK_ON:
			...
			if (!backend->shaders_desired ||
				...
				/* when platform is power down only,
				 * partial power off shader cores, do nothing
				 */
		...
		case KBASE_SHADERS_WAIT_GPU_IDLE:
			WARN_ON(platform_power_down_only);
...
static void kbase_pm_trace_power_state(struct kbase_device *kbdev)
{
	...
	KBASE_TLSTREAM_AUX_PM_STATE(
			...
			KBASE_PM_CORE_L2,
			kbase_pm_get_ready_cores(
				kbdev, KBASE_PM_CORE_L2)); //lint !e648
	KBASE_TLSTREAM_AUX_PM_STATE(
			kbdev,
			KBASE_PM_CORE_SHADER,
			kbase_pm_get_ready_cores(
				kbdev, KBASE_PM_CORE_SHADER)); //lint !e648
	KBASE_TLSTREAM_AUX_PM_STATE(
			kbdev,
			KBASE_PM_CORE_TILER,
			kbase_pm_get_ready_cores(
				kbdev,
				KBASE_PM_CORE_TILER)); //lint !e648
...
void kbase_pm_wait_for_l2_powered(struct kbase_device *kbdev)
{
	...
	err = wait_event_killable(kbdev->pm.backend.gpu_in_desired_state_wait,
			kbase_pm_is_in_desired_state_with_l2_powered(kbdev)); //lint !e666
...
void kbase_pm_wait_for_desired_state(struct kbase_device *kbdev)
{
	...
	/* Wait for cores */
	err = wait_event_killable(kbdev->pm.backend.gpu_in_desired_state_wait,
			kbase_pm_is_in_desired_state(kbdev)); //lint !e666
...
static void kbase_pm_reset_instead(struct kbase_device *kbdev)
{
	...
	while (--max_loop && kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_STATUS)) & KBASE_PWR_ACTIVE_BIT) {
		;
	}
...
static int kbase_pm_do_hard_reset(struct kbase_device *kbdev)
{
	struct kbasep_reset_timeout_data rtdata;

	/* Try hard reset */
	dev_err(kbdev->dev, "Attempting a hard reset\n");
...
static int kbase_pm_do_reset(struct kbase_device *kbdev)
{
	...
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_HI0002))
		...
		KBASE_TLSTREAM_JD_GPU_SOFT_RESET(kbdev, kbdev); //lint !e648
		kbase_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND),
	...
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_HI0002)) {
		...
		/* For normal runmode,report dmd every 5 times. */
		if (dsm_report_enable(kbdev->error_num.hard_reset, 5, kbdev->runmode_normal)) {
			gpu_dsm_report(kbdev, GPU_HARD_RESET_DSM_NO, "Failed to hard-reset the GPU (timed out after %d ms) !!!!", RESET_TIMEOUT);
		}
#endif
		kbdev->error_num.hard_reset++;
		kbdev->error_num.ts = hisi_getcurtime();
...
#ifdef CONFIG_MALI_GPU_DRM_HI3660
...
int kbase_pm_init_hw(struct kbase_device *kbdev, unsigned int flags)
{
	...
	if (kbdev->as_stuck_hard_reset)
		err = kbase_pm_do_hard_reset(kbdev);
	else if (kbdev->protected_mode_support)
	...
	kbdev->as_stuck_hard_reset = false;
