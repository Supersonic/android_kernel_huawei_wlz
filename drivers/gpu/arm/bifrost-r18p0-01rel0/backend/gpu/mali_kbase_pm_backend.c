...
int kbase_hwaccess_pm_early_init(struct kbase_device *kbdev)
{
	...
	mutex_init(&kbdev->pm.suspend_lock);
...
#endif /* CONFIG_HISI_GPU_CORE_HOTPLUG */
...
void kbase_pm_do_poweroff(struct kbase_device *kbdev, bool is_suspend)
{
	...
	if (is_suspend) {
		/* If GPU is powered off in both software and hardware semantics */
...
bool kbase_pm_poweroff_done(struct kbase_device *kbdev)
{
	bool ret;
	unsigned long flags;
...
#ifdef CONFIG_HISI_GPU_CORE_HOTPLUG
			kbdev->pm.thermal_required_core_mask =
#endif
