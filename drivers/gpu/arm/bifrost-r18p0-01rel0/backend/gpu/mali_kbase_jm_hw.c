...
void kbase_jm_wait_for_zero_jobs(struct kbase_context *kctx)
{
	...
	wait_event(kbdev->hwaccess.backend.reset_wait,
			...
			== KBASE_RESET_GPU_NOT_PENDING); //lint !e31
exit:
...
static enum hrtimer_restart kbasep_reset_timer_callback(struct hrtimer *timer)
{
	...
	dev_err(kbdev->dev, "%s reset_gpu = %d",__func__, atomic_read(&kbdev->hwaccess.backend.reset_gpu));
...
static void kbasep_try_reset_gpu_early_locked(struct kbase_device *kbdev)
{
	...
	if (atomic_cmpxchg(&kbdev->hwaccess.backend.reset_gpu,
		...
		dev_err(kbdev->dev, "%s reset_gpu = %d early out",__func__, atomic_read(&kbdev->hwaccess.backend.reset_gpu));
		return;
	}
	dev_err(kbdev->dev, "%s reset_gpu = %d",__func__, atomic_read(&kbdev->hwaccess.backend.reset_gpu));
...
bool kbase_prepare_to_reset_gpu_locked(struct kbase_device *kbdev)
{
	...
	if (atomic_cmpxchg(&kbdev->hwaccess.backend.reset_gpu,
		...
		dev_err(kbdev->dev, "%s reset_gpu = %d earlier return",__func__, atomic_read(&kbdev->hwaccess.backend.reset_gpu));
		return false;
	}
	dev_err(kbdev->dev, "%s reset_gpu = %d",__func__, atomic_read(&kbdev->hwaccess.backend.reset_gpu));
...
void kbase_reset_gpu_queued(struct kbase_device *kbdev)
{
	...
	kbdev->as_stuck_hard_reset = true;
	queue_work(kbdev->hwaccess.backend.reset_workq,
					&kbdev->hwaccess.backend.reset_work);
	spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
}
