...
#include <platform/hisilicon/mali_kbase_hisi_callback.h>
...
#define FENCE_TIMEOUT_TIME        (2000)
#endif
...
struct kbase_pm_device_data {
	...
	/**
	 * The lock to synchronize the pm runtime suspend and device suspend.
	 */
	...
	/**
	 * Bit mask identifying the available shader cores are specified by IPA
	 * via interface called by IPA
	 */
	u64 thermal_required_core_mask;
	/* Flag indicating that cores are controlling by IPA */
	bool thermal_controlling;
#endif
...
#ifdef CONFIG_MALI_BOUND_REPORT
#define BOUND_FIFO_SIZE	10
#define BOUND_DURATION_TIMES	250

struct kbase_report_info{
	struct kfifo bound_fifo;
	bool	report_bound_flag;
	u8	bound_times_in_fifo;
	int	duration_times;
};
#endif
...
struct kbase_device {
...
#ifdef CONFIG_MALI_BOUND_REPORT
	struct kbase_report_info	bound_report_info;
#endif
	...
	/* gpu outstanding setting be store in dvice tree node */
	u32 gpu_outstanding;
...
do { \
	if (kbdev->gpu_dsm_client) { \
		if(!dsm_client_ocuppy(kbdev->gpu_dsm_client)) { \
			dsm_client_record(kbdev->gpu_dsm_client, fmt, ##args); \
			dsm_client_notify(kbdev->gpu_dsm_client, dsm_no); \
