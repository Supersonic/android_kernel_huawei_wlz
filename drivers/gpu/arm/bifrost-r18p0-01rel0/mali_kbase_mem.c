	...
	if (gpu_pfn) {
		...
		if ((reg->flags & KBASE_REG_TILER_ALIGN_TOP)) {
			WARN(align > 1, "%s with align %lx might not be honored for KBASE_REG_TILER_ALIGN_TOP memory",
					__func__,
					(unsigned long)align); //lint !e146, !e665
...
static size_t kbase_get_same_va_bits(struct kbase_context *kctx)
{
	return min(kbase_get_num_cpu_va_bits(kctx),
			(size_t) kctx->kbdev->gpu_props.mmu.va_bits); //lint !e666
}
...
int kbase_gpu_munmap(struct kbase_context *kctx, struct kbase_va_region *reg)
{
	...
	case KBASE_MEM_TYPE_IMPORTED_USER_BUF: {
		...
		if (user_buf->current_mapping_usage_count & PINNED_ON_IMPORT) { //lint !e648
			user_buf->current_mapping_usage_count &=
				~PINNED_ON_IMPORT; //lint !e648
		...
		off_t offset, size_t size, enum kbase_sync_type sync_fn)
{
	...
	if (likely(cpu_pa == gpu_pa)) {
		...
		if (WARN(!gpu_pa, "No GPU PA found for infinite cache op")) //lint !e146, !e665
			return;
		...
		memcpy(dst, src, size); /* unsafe_function_ignore: memcpy */ //lint !e668
		kunmap(gpu_page);
		kunmap(cpu_page);
		if (sync_fn == KBASE_SYNC_TO_DEVICE)
		...
		struct basep_syncset *sset, enum kbase_sync_type sync_fn)
{
	...
	u64 page_off, page_count; //lint !e578
	u64 i;
	u64 offset;
...
int kbase_alloc_phy_pages_wraper(struct kbase_mem_phy_alloc *alloc, size_t nr_pages_requested)
{
	size_t nr_pages = 0;
	...
	if (!nr_pages_requested)
		return 0;

	if (alloc->group_id == BASE_MEM_GROUP_DEFAULT)
		return kbase_alloc_phy_pages_helper(alloc, nr_pages_requested, alloc->group_id);

	/* There may exists pages that route last buffer, so we have to alloc last buffer & normal memory
	 * seperatately. Allocate the last buffer pages first.
	 */
	if (alloc->lb_pages > alloc->nents) {
		int ret = 0;
		nr_lb_pages = min(nr_pages_requested, alloc->lb_pages - alloc->nents);
		ret = kbase_alloc_phy_pages_helper(alloc, nr_lb_pages, alloc->group_id);
		if (ret < 0) {
			dev_err(kctx->kbdev->dev, "Alloc last buffer failed: group_id: %d nr_pages: %zu.",
			        alloc->group_id, nr_lb_pages);
			return ret;
		}
	}

	/* Allocate the normal memory at last. */
	nr_pages = max(nr_pages_requested - nr_lb_pages, (size_t)0);
	return kbase_alloc_phy_pages_helper(alloc, nr_pages, BASE_MEM_GROUP_DEFAULT);
}

/*lint -e571*/
/*lint -e648*/
int kbase_alloc_phy_pages_helper(struct kbase_mem_phy_alloc *alloc,
		size_t nr_pages_requested, u8 group_id)
{
...
alloc_failed:
	...
	if (nr_left != nr_pages_requested) {
		...
		kbase_free_phy_pages_helper(alloc, nr_pages_to_free, group_id);
	}
...
int kbase_free_phy_pages_helper(
	struct kbase_mem_phy_alloc *alloc,
	size_t nr_pages_to_free, u8 group_id)
{
	...
	while (nr_pages_to_free) {
		if (is_huge_head(*start_free)) {
			...
			free_partial(kctx, group_id, *start_free);
			nr_pages_to_free--;
			start_free++;
			freed++;
		} else {
	...
	if (!reclaimed) {
		...
		KBASE_TLSTREAM_AUX_PAGESALLOC(
			kbdev,
			kctx->id,
			(u64)new_page_count); //lint !e571, !e648
	}

	return 0;
}

int kbase_free_phy_pages_wraper(
   struct kbase_mem_phy_alloc *alloc,
   size_t nr_pages_to_free)
{
	...
	if (alloc->group_id == BASE_MEM_GROUP_DEFAULT)
		return kbase_free_phy_pages_helper(alloc, nr_pages_to_free, alloc->group_id);

	/* There may exists pages that route last buffer that we needed to free. So, we have to
	 * free the last buffer & normal memory seperatately. Free the normal memory first.
	 */
	if (alloc->lb_pages < alloc->nents) {
		int ret = 0;
		nr_pages = min(nr_pages_to_free, alloc->nents - alloc->lb_pages);
		ret = kbase_free_phy_pages_helper(alloc, nr_pages, BASE_MEM_GROUP_DEFAULT);
		if (ret < 0) {
			dev_err(kctx->kbdev->dev, "Free normal memory failed: nr_pages: %zu.", nr_pages);
			return ret;
		}
	}

	/* Free the pages that routes last buffer at last. */
	nr_lb_pages = min(nr_pages_to_free - nr_pages, alloc->lb_pages);
	return kbase_free_phy_pages_helper(alloc, nr_lb_pages, alloc->group_id);
}
...
void kbase_mem_kref_free(struct kref *kref)
{
	...
	case KBASE_MEM_TYPE_NATIVE: {
		...
		kbase_free_phy_pages_wraper(alloc, alloc->nents);
		break;
	}
	case KBASE_MEM_TYPE_ALIAS: {
...
int kbase_alloc_phy_pages(struct kbase_va_region *reg, size_t vsize, size_t size)
{
	...
	if (kbase_alloc_phy_pages_wraper(reg->cpu_alloc, size) != 0)
		goto out_term;

	reg->cpu_alloc->reg = reg;
	if (reg->cpu_alloc != reg->gpu_alloc) {
		if (kbase_alloc_phy_pages_wraper(reg->gpu_alloc, size) != 0)
			goto out_rollback;
...
out_rollback:
	kbase_free_phy_pages_wraper(reg->cpu_alloc, size);
out_term:
	return -1;
}
		...
		u64 va_pages, u64 commit_pages, u64 large_extent)
{
	...
	u64 gpu_pc_pages_max = 1ULL << gpu_pc_bits >> PAGE_SHIFT; //lint !e504
...
{
	KBASE_DEBUG_ASSERT(kctx != NULL);
	mutex_lock(&kctx->reg_lock); //lint !e454
} //lint !e454
...
{
	KBASE_DEBUG_ASSERT(kctx != NULL);
	mutex_unlock(&kctx->reg_lock); //lint !e455
}
		...
		u32 bin_id, u32 max_allocations)
{
	...
	KBASE_TLSTREAM_AUX_JIT_STATS(kbdev, kctx->id, bin_id,
		max_allocations, alloc_count, va_pages, ph_pages); //lint !e648
}
...
void kbase_jit_free(struct kbase_context *kctx, struct kbase_va_region *reg)
{
	...
	if (reg->initial_commit < old_pages) {
		...
		u64 new_size = MAX(reg->initial_commit,
			div_u64(old_pages * (100 - kctx->trim_level), 100)); //lint !e666
		...
		if (delta) {
			...
			kbase_free_phy_pages_wraper(reg->cpu_alloc, delta);
			if (reg->cpu_alloc != reg->gpu_alloc)
				kbase_free_phy_pages_wraper(reg->gpu_alloc,
						delta);
		}
	}
		...
		struct kbase_va_region *reg)
{
	...
	err = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu, reg->start_pfn,
			...
			reg->flags & gwt_mask, kctx->as_nr, kbase_reg_current_backed_size(reg));
	if (err == 0)
		return 0;
		...
		struct kbase_mem_phy_alloc *alloc, bool writeable)
{
	...
	pages = alloc->imported.user_buf.pages;
	for (i = 0; i < alloc->imported.user_buf.nr_pages; i++) { //lint !e574
		unsigned long local_size;
