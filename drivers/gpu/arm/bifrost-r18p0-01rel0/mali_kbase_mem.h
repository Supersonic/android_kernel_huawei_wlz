...
struct kbase_mem_phy_alloc {
	...
	/* the maximum pages that route last buffer in a memory region. */
	size_t lb_pages;
...
#define KBASE_MEM_LB_PAGES (896) /* The maximum size of last buffer in a memory. In pages. */
		...
		enum kbase_memory_type type, int group_id)
{
	...
	alloc->lb_pages = KBASE_MEM_LB_PAGES;
#else
	alloc->lb_pages = 0;
#endif
...
/**
 * kbase_alloc_phy_pages_wraper - Allocates physical pages.
 ...
 */
int kbase_alloc_phy_pages_wraper(struct kbase_mem_phy_alloc *alloc,
...
*/
int kbase_free_phy_pages_wraper(struct kbase_mem_phy_alloc *alloc, size_t nr_pages_to_free);
...
* @param[in] alloc allocation object to free pages from
* @param[in] nr_pages_to_free number of physical pages to free
* @param[in] the memory group id of the allocating pages
*
* Return: 0 on success, otherwise a negative error code
*/
int kbase_free_phy_pages_helper(struct kbase_mem_phy_alloc *alloc, size_t nr_pages_to_free, u8 group_id);
