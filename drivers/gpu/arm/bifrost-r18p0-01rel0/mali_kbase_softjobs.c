 ...
 */
/*lint -e648, -e426, -e574, -e514, -e578, -e668, -e527*/
static void kbasep_add_waiting_soft_job(struct kbase_jd_atom *katom)
{
	...
	katom->soft_job_queued = true;
	list_add_tail(&katom->queue, &kctx->waiting_soft_jobs);
	spin_unlock_irqrestore(&kctx->waiting_soft_jobs_lock, lflags);
}

static void validate_list(struct kbase_jd_atom *katom)
{
	if (katom->queue.next == LIST_POISON1)
		dev_err(katom->kctx->kbdev->dev, "kctx %pK atom %pK next pointer corrupted\n", (void *)katom->kctx, katom);

	if (katom->queue.prev == LIST_POISON2)
		dev_err(katom->kctx->kbdev->dev, "kctx %pK atom %pK prev pointer corrupted\n", (void *)katom->kctx, katom);
}
...
void kbasep_remove_waiting_soft_job(struct kbase_jd_atom *katom)
{
	...
	spin_lock_irqsave(&kctx->waiting_soft_jobs_lock, lflags);
	if (!katom->soft_job_queued)
		dev_err(katom->kctx->kbdev->dev, "!!!!!!!kctx %pK atom %pK job haven't be queued \n", (void *)katom->kctx, katom);
	validate_list(katom);
...
static int kbase_dump_cpu_gpu_time(struct kbase_jd_atom *katom)
{
	...
	memcpy(user_result, &data, sizeof(data)); /* unsafe_function_ignore: memcpy */
...
#if defined(CONFIG_SYNC) || defined(CONFIG_SYNC_FILE)
void validate_fence_wait_katom(struct kbase_jd_atom *katom)
{
	struct kbase_context *kctx = katom->kctx;

	if (list_empty(&kctx->waiting_soft_jobs))
		dev_err(kctx->kbdev->dev, "kctx %pK waiting_soft_jobs list unexpectedly empty\n", (void *)kctx);

	if ((katom->core_req & BASE_JD_REQ_SOFT_JOB_TYPE) != BASE_JD_REQ_SOFT_FENCE_WAIT)
		dev_err(kctx->kbdev->dev, "kctx %pK unexpected core_req(%x) for katom\n", (void *)kctx, katom->core_req);

#ifdef CONFIG_SYNC_FILE
	if (atomic_read(&katom->dma_fence.dep_count) != -1)
		dev_err(kctx->kbdev->dev, "kctx %pK unexpected dep_count for katom\n", (void *)kctx);
#endif

	if ((katom->status == KBASE_JD_ATOM_STATE_UNUSED) ||
	    (katom->status == KBASE_JD_ATOM_STATE_COMPLETED))
		dev_err(kctx->kbdev->dev, "kctx %pK unexpected status (%x) for katom\n", (void *)kctx, katom->status);
}
...
void kbase_soft_event_wait_callback(struct kbase_jd_atom *katom)
{
	...
	/* if already context dying, ignore this let
	   context dying handle this */
	if (kbase_ctx_flag(katom->kctx, KCTX_DYING))
		dev_err(kctx->kbdev->dev, "kctx %pK atom %pK fence callback invoked for dying context\n", (void *)kctx, katom);

	validate_fence_wait_katom(katom);
		...
		unsigned int *target_page_nr, size_t offset, size_t *to_copy)
{
	...
	chunk = min(chunk, *to_copy); //lint !e1058

	memcpy(target_page + offset, extres_page, chunk); /* unsafe_function_ignore: memcpy */
	*to_copy -= chunk;
	...
	*target_page_nr += 1;
	...
	chunk = min(offset, *to_copy); //lint !e1058
	memcpy(target_page, extres_page + PAGE_SIZE-offset, chunk); /* unsafe_function_ignore: memcpy */
	*to_copy -= chunk;
					...
					struct base_jit_alloc_info *info)
{
	...
	if (kctx->jit_version == 1) {
		...
		info->bin_id = 0;
		info->flags = 0;
		memset(info->padding, 0, sizeof(info->padding)); /* unsafe_function_ignore: memset */
	} else {
		int j;
...
int kbase_process_soft_job(struct kbase_jd_atom *katom)
{
	...
	case BASE_JD_REQ_SOFT_FENCE_WAIT:
	{
		...
		if (ret == 1) {
			...
			hrtimer_start(&katom->kctx->fence_wait_timer, HR_TIMER_DELAY_MSEC(FENCE_TIMEOUT_TIME), HRTIMER_MODE_REL);
			katom->kctx->timer_started = 1;
#endif
#ifdef CONFIG_MALI_FENCE_DEBUG
			kbasep_add_waiting_with_timeout(katom);
#else
...
#if BASE_DEBUG_FENCE_TIMEOUT
		if (katom->kctx && katom->kctx->timer_started == 1)
		{
			hrtimer_cancel(&katom->kctx->fence_wait_timer);
			katom->kctx->timer_started = 0;
		}
#endif
		break;
#endif /* CONFIG_SYNC || CONFIG_SYNC_FILE */
