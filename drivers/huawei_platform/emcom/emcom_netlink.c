...
* emcom_netlink.c
*
* communication for emcom module
*
...
void emcom_send_msg2daemon(int cmd, const void *data, int len)
{
	...
	NETLINK_CB(pskb_out).dst_group = 0; /* For unicast */
	if (data && (len > 0)) {
...
static void emcom_common_evt_proc(const struct nlmsghdr *nlh, uint8_t *data, uint16_t len)
{
	if (nlh == NULL)
		return;
