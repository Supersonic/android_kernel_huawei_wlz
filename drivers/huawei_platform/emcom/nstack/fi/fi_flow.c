			...
			__be32 saddr, __u16 sport, __be32 daddr, __be16 dport, fi_flow_cb flow_cb, struct sock *sk)
{
	...
	if (head != NULL) {
		...
		if (newnode == NULL) {
			fi_loge("failed to malloc new IPV4 node for uid", uid);
			return NULL;
		}
		...
		list_for_each_entry_safe(node, tmp, &(flow->flow_table[i].list), list) {
			if ((node->flow_ctx.stat_stop & FI_FLOW_PERIODIC_REPORT_NEED) ||
				...
				if (node->flow_ctx.flow_msg.l3proto == ETH_P_IP) {
					fi_logi("Periodic report. SrcIP: "IPV4_FMT", SrcPort: %hu, DstIP: "IPV4_FMT", DstPort: %hu, "
						"Proto: %u, SockState: %u, sk %pK, UpPktNum: %d, DownPktNum: %d",
					...
					fi_logi("Periodic report. SrcIP: "IPV6_FMT", SrcPort: %hu, DstIP: "IPV6_FMT", DstPort: %hu, "
						"Proto: %u, SockState: %u, UpPktNum: %d, DownPktNum: %d",
