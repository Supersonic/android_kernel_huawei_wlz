...
void fi_register_nfhook(void)
{
	...
	char dev_list[][IFNAMSIZ] = {FI_WLAN_IFNAME, FI_LTE_IFNAME};
	...
	for (i = 0; i < FI_DEV_NUM; i++) {
		...
		if (dev == NULL) {
			fi_loge(" can't find dev %s", dev_list[i]);
			continue;
		}
...
{
	int name_len = strlen(update_msg->app_name) < FI_APP_NAME_SIZ ?
					strlen(update_msg->app_name) : FI_APP_NAME_SIZ;

	if ((memcmp(app->msg.app_name, update_msg->app_name, name_len) == 0)
		|| (app->msg.uid == update_msg->uid))
		return true;
...
static void fi_app_status_update(const struct fi_cfg_head *cfg)
{
	...
	for (i = 0; i < FI_LAUNCH_APP_MAX; i++) {
		spin_lock_bh(&(g_fi_app_info.app[i].lock));
		if (memcmp(g_fi_app_info.app[i].msg.app_name, update_msg->app_name,
			strlen(update_msg->app_name) < FI_APP_NAME_SIZ ?
			strlen(update_msg->app_name) : FI_APP_NAME_SIZ) == 0 &&
			g_fi_app_info.app[i].msg.status != FI_INVALID_APP_STATUS) {
			app = &(g_fi_app_info.app[i]);
			break;
		} else if (g_fi_app_info.app[i].msg.uid == update_msg->uid &&
			g_fi_app_info.app[i].msg.status != FI_INVALID_APP_STATUS) {
			app = &(g_fi_app_info.app[i]);
			break;
		}
...
static void fi_ipv6_pkt_parse(struct fi_pkt_parse *pktinfo, struct sk_buff *skb)
{
	...
	for (i = 0; i < FI_END_PARSE; i++) {
		if (pktinfo->flow_cb.flow_identify_cfg->pkt[pktinfo->dir].opt_enable[i] &&
			g_fi_pkt_parse_f[i].opt_val_parse != NULL) {
			fi_logd("parse %d function", __func__, i);
			(void)(g_fi_pkt_parse_f[i].opt_val_parse(skb, pktinfo));
		}
	}
	spin_unlock_bh(&(pktinfo->flow_cb.app_info->lock));
...
void fi_init(struct sock *nlfd)
{
	...
	g_fi_ctx.tm.data = (unsigned long)"fi_callback_timer";
	g_fi_ctx.nlfd = nlfd;
...
void fi_bw_update(struct fi_flow_node *flow)
{
	if (flow->rcv_bw.burst_bytes > FI_MIN_BURST_SIZE) {
		...
		fi_logi("burst: uid %u, sport: %u, burst_bytes: %u, delta_us: %u",
...
void fi_rtt_update(struct fi_flow_node *flow)
{
	...
	fi_logi("sport %u now %lu srtt_time %lu srtt %u now_mstamp %u rcv_rtt_time %u rcv_rtt %u",
			flow->sk->sk_num, now_jiffies, srtt_time, srtt, now_mstamp, rcv_rtt_time, tp->rcv_rtt_est.rtt_us);
	fi_logi("updatetime %u tcp_mstamp %u mdev_max_us %u rttvar_us %u total_retrans %u retrans_stamp %u",
...
int fi_iface_msg_by_index(char *data, int index, uint32_t size)
{
	...
	if (dev == NULL) {
		fi_loge(" can't find dev %s", dev_list[index]);
		return -1;
	}
	read_lock(&dev_base_lock);
	dev_get_stats(dev, &stats);
	...
	fi_logi("Periodic report. Nic: %s, total_byes: %lu", dev_list[index], stats.rx_bytes);
