...
static int cq_test_buff_index;
int test_dbg_cmd_test(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	enum ts_test_item_enum test_item =
	    (enum ts_test_item_enum)in_cmd->cmd_param.prv_params;
	int error = NO_ERR;
	int rec_v, cal_v;
	...
	case TEST_CMDQ_ITEM_3:
		...
		TS_LOG_INFO("index :%d, rec data:%d, cal data:%d, result: %s\n",
			    cq_test_buff_index, rec_v, cal_v,
			    (rec_v == cal_v) ? "pass" : "fail");
		cq_test_buff_index++;
	...
	case TEST_INPUT_ITEM_1:
		...
		memcpy(&out_cmd->cmd_param.pub_params.report_info, &test_pos,
	...
	for (i = 0; i < 100; i++) {
		...
		if (NO_ERR == put_one_cmd(&cmd)) {
			if (wait_for_completion_timeout
				...
				TS_LOG_INFO
				    ("CMD PROCESS ELAPSED TIME: %lldms%lldus\n",
				     delta_ms, delta_us);
			} else
				TS_LOG_ERR
				    ("ts thread process TEST CMD expire 5 sec\n");
		} else
	...
	for (i = 0; i < TS_CMD_QUEUE_SIZE + 5; i++) {
		...
		if (put_one_cmd(&cmd)) {
			...
			if (try_wait_for_completion(&cq_test_sync_2) ||
				...
				TS_LOG_ERR
				    ("ts thread process TEST CMD expire 10 sec\n");
...
static int test_create_debugfs(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_test_items); i++) {
		dent =
		    debugfs_create_u32(g_test_items[i].item_name,
				       S_IRUGO | S_IWUSR, dir,
				       &g_test_items[i].item_switch);
		if (!dent) {
...
repeat:
	...
	for (i = 0, ret = -99; i < ARRAY_SIZE(g_test_items); i++) {
		...
		if (g_test_items[i].test_func)
			ret =
			    g_test_items[i].test_func(g_test_items[i].
						      item_data);
...
static int __init ts_test_init(void)
{
	...
	test_task =
	    kthread_create_on_node(ts_test_thread, NULL, cpu_to_node(2),
				   "ts_test_thread");
	if (IS_ERR(test_task))
