...
static void mxt_regulator_set(struct mxt_data *data)
{
	...
	error =
	    regulator_set_voltage(data->reg_avdd,
				  data->chip_data->regulator_ctr.vci_value,
				  data->chip_data->regulator_ctr.vci_value);
	if (error < 0) {
	...
	error =
	    regulator_set_voltage(data->reg_vddio,
				  data->chip_data->regulator_ctr.vddio_value,
				  data->chip_data->regulator_ctr.vddio_value);
	if (error < 0) {
		TS_LOG_ERR
...
static void mxt_parse_module_dts(struct ts_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, "test_rawdata_normalizing",
			   ...
			   struct ts_device_data *chip_data)
{
	...
	error = of_property_read_u32(device, "atmel_vci_value", &chip_data->regulator_ctr.vci_value);
	if(!error)
	{
		TS_LOG_INFO("get chip vci value = %d\n", chip_data->regulator_ctr.vci_value);
	} else {
		TS_LOG_INFO("Not define vci value in Dts, use fault value\n");
		chip_data->regulator_ctr.vci_value = AVDD_LDO_VALUE;
	}
	error = of_property_read_u32(device, "atmel_vddio_value", &chip_data->regulator_ctr.vddio_value);
	if(!error)
	{
		TS_LOG_INFO("get chip vddio value = %d\n", chip_data->regulator_ctr.vddio_value);
	} else {
		TS_LOG_INFO("Not define vddio value in Dts, use fault value\n");
		chip_data->regulator_ctr.vddio_value = VDDIO_LDO_VALUE;
	}
...
static void atmel_dsm_record_chip_status(struct mxt_data *data)
{
	if (atomic_read(&atmel_mxt_moisture_flag)) {
		if (!dsm_client_ocuppy(tp_dclient)) {
			...
			dsm_client_notify(tp_dclient, DSM_TP_IN_MOISTURE_ERROR_NO);
		}
	...
	if (atomic_read(&atmel_mxt_vnoise_flag)) {
		if (!dsm_client_ocuppy(tp_dclient)) {
			...
			dsm_client_notify(tp_dclient, DSM_TP_IN_VNOISE_ERROR_NO);
		}
			     ...
			     struct ts_cmd_node *out_cmd)
{
 ...
 /**ATTENTION : Should be set to MXT_T6_CMD58_ON in each return branch**/
	...
	if(mxt_g_data->chip_data->test_rawdata_normalizing) {
		info->used_sharp_selcap_touch_delta_size = data->info->matrix_xsize + data->info->matrix_ysize;
		info->used_sharp_selcap_single_ended_delta_size = data->info->matrix_ysize;
		memcpy(&info->buff[2 + offset + offset], data->T37_buf,
			(info->used_sharp_selcap_touch_delta_size +
			info->used_sharp_selcap_single_ended_delta_size)*sizeof(int));
	} else {
