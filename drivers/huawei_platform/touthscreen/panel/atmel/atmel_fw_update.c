 ...
 *
 */
int mxt_diagnostic_command(struct mxt_data *data, u8 cmd, u8 page, u8 index,
...
#define T37_COMMAND_RESEND_INDEX  2
	if ((u8) command_register[0] != cmd
		...
		if (abs((s8) current_page - (s8) page) > (s8) page) {	/* far away with dis page*/
			ret =
			    mxt_t6_command(data, MXT_COMMAND_DIAGNOSTIC, cmd,
					   0);
			if (ret) {
				TS_LOG_ERR
				    ("T6 Command DIAGNOSTIC cmd 0x%x failed %d!\n",
...
int mxt_pds_get_data(struct mxt_data *data, struct pds_code *code)
{
	...
	do {
		ret =
		    mxt_diagnostic_command(data, MXT_T6_DEBUG_PID, 0, 0,
					   MISC_PDS_HEAD_LEN,
					   (char *)&code->head[0], 10, 5);
		if (ret == 0) {
...
void mxt_update_fw_file_name(struct mxt_data *data, char *file_name)
{
	...
	if (update_sd_mode == 0) {
		...
		offset +=
		    snprintf(file_name + offset, PAGE_SIZE - offset,
