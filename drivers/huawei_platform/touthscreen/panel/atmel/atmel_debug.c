			      ...
			      int data_lowlimit)
{
	...
	for (rx_n = 0; rx_n < (data->y_size - data->y_origin) - 1; rx_n++) {
		for (tx_n = 0; tx_n < (data->x_size - data->x_origin); tx_n++) {
			rx_delta_value =
			    data->T37_buf[tx_n *
					  (data->y_size - data->y_origin) +
					  rx_n + 1] -
			    data->T37_buf[tx_n *
					  (data->y_size - data->y_origin) +
					  rx_n];
			if ((rx_delta_value <= data_uplimit)
			    && (rx_delta_value >= data_lowlimit)) {
				result++;
			}
	...
	if (result ==
	    (data->T37_buf_size / sizeof(int) -
	     (data->x_size - data->x_origin))) {
		TS_LOG_INFO("rawdata rx diff is all right, result = %zu\n",
			    result);
		return 1;
	} else
...
int get_refs_tx2tx_delta_test(struct mxt_data *data, int data_uplimit,
			      int data_lowlimit)
{
	int tx_n, rx_n;
		...
		for (rx_n = 0; rx_n < (data->y_size - data->y_origin); rx_n++) {
			tx_delta_value =
			    ...
			    data->T37_buf[tx_n *
					  (data->y_size - data->y_origin) +
					  rx_n];
			if ((tx_delta_value <= data_uplimit)
			    && (tx_delta_value >= data_lowlimit))
				result++;
...
int get_refs_or_deltas_data_test(struct mxt_data *data, int data_uplimit,
				 int data_lowlimit)
{
	int i = 0;
	size_t result = 0;
	...
	for (i = 0; i < data->T37_buf_size / sizeof(int); i++) {
		if ((data->T37_buf[i] <= data_uplimit)
		    && (data->T37_buf[i] >= data_lowlimit))
			result++;
		else
...
int mxt_T37_fetch(struct mxt_data *data, u8 mode)
{
	...
	{
		tem_buf_size = ((data->info->matrix_xsize) + (data->info->matrix_ysize)) *
						sizeof(int) * 2;
		...
		for (i = 0; i < num_pages; i++) {
			...
			if(1 == i) {
				...
				TS_LOG_INFO("Current Page is obuf[1]=%d",obuf[1]);
				switch(i){
					case 0:
						chunk_len = (data->info->matrix_xsize + data->info->matrix_ysize) * 2;
						break;
					case 2:
						chunk_len = (data->info->matrix_ysize) * 2;
						break;
					default:
						break;
				}
		...
		for (i = 0, j = 0; i < (data->info->matrix_xsize*2 + data->info->matrix_ysize*4); i += 2, j++) {
			tem_buf[j] =
		...
		i = 0;
		j = 0;
		while (i < (data->info->matrix_xsize + data->info->matrix_ysize*2)) {
		...
		i = 0;
		j = 0;
		while ((i < (data->info->matrix_xsize) * (data->info->matrix_ysize))
			&& (j < (data->x_size - data->x_origin) * (data->y_size -
			data->y_origin))) {
			data->T37_buf[j] = tem_buf[i];
			i++;
			j++;
			if (0 == j % (data->y_size - data->y_origin)) {
				i += (data->info->matrix_ysize - data->y_size +
					data->y_origin);
			}
		}
		ret = 0;
...
int mxt_t25_selftest(struct mxt_data *data, int cmd)
{
	u32 i;
	data->t25_msg[0] = 0;
	data->t25_msg[5] = 0;
...
static DEVICE_ATTR(debug_v2_enable, 0660 /*S_IWUSR | S_IRUSR */, NULL,
...
int mxt_mem_access_init(struct mxt_data *data)
{
	...
	error =
	    sysfs_create_group(&g_ts_data.client->dev.kobj, &mxt_attr_group);
	if (error) {
