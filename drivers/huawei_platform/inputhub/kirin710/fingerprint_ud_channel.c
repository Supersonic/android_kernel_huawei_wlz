...
static uint8_t g_opt_brt_len = 0;
...
static int fingerprint_recovery_config_para(void *data, int len)
{
	...
	if (len > sizeof(fp_pkt.buf)) {
		hwlog_warn("fingerprint: fingerprint_recovery_config_para len \
			is out of size, len=%d\n", len);
		return -1;
	}
	...
	hwlog_info("fingerprint: fingerprint_recovery_config_para data=%d, \
		len=%d\n", fp_pkt.buf[0], len);
