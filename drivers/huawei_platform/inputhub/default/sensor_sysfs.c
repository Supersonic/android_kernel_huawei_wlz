	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	memset(&ps_test, 0, sizeof(ps_test));
	#endif
	if ((sensor_tof_flag != 1) && (sensor_ps_flag != 1) && (ps_cali_supported != 1)) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch (val) {
	case 1: /* als light calibrate */
		break;
	case 2: /* als dark current calibrate */
		if (als_data.als_phone_type == LAYA_PHONE_TYPE)
			break;
		return count;
	case 7: /* als leakage light calibrate */
