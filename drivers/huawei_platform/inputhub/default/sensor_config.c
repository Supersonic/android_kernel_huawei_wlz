...
s16 min_threshold_als_para;
s16 max_threshold_als_para;
...
static uint8_t msensor_folder_calibrate_data[MAX_MAG_FOLDER_CALIBRATE_DATA_LENGTH];
...
extern int tp_color_from_nv_flag;
...
int write_gsensor1_offset_to_nv(const char *temp, int length)
{
	...
	if (write_calibrate_data_to_nv(ACC1_OFFSET_NV_NUM,
		...
		return -1;
	memcpy((void *)gsensor1_calibrate_data, (void *)temp,
		ACC1_CALIBRATE_DATA_LENGTH);
...
int send_gyro1_calibrate_data_to_mcu(void)
{
	...
	memcpy(gyro1_sensor_offset, user_info.nv_data,
		sizeof(gyro1_sensor_offset));
...
int write_gyro1_sensor_offset_to_nv(const char *temp, int length)
{
	...
	memcpy(gyro1_sensor_calibrate_data, temp,
...
int write_gyro_sensor_offset_to_nv(const char *temp, int length)
{
	...
	if (write_calibrate_data_to_nv(GYRO_CALIDATA_NV_NUM, length,
		"GYRO", temp))
		return -1;
...
int send_mag_calibrate_data_to_mcu(void)
{
	...
	if (g_mag_folder_function == 1) {
		memcpy(msensor_folder_calibrate_data, user_info.nv_data,
			MAG_FOLDER_CALIBRATE_DATA_NV_SIZE);
		...
		if (send_calibrate_data_to_mcu(TAG_MAG, SUB_CMD_SET_OFFSET_REQ,
			...
			MAG_FOLDER_CALIBRATE_DATA_NV_SIZE, false))
			return -1;
	} else if (akm_cal_algo == 1) {
		...
		if (send_calibrate_data_to_mcu(TAG_MAG,
			SUB_CMD_SET_OFFSET_REQ, msensor_calibrate_data,
...
int write_mag1sensor_calibrate_data_to_nv(const char *src)
{
	...
	mag_size = MAG_CALIBRATE_DATA_NV_SIZE;
	memcpy(&msensor1_calibrate_data, src,
...
int write_magsensor_calibrate_data_to_nv(const char *src, int length)
{
	...
	if (g_mag_folder_function == 1) {
		mag_size = MAG_FOLDER_CALIBRATE_DATA_NV_SIZE;
		memcpy(&msensor_folder_calibrate_data, src,
...
int send_tof_calibrate_data_to_mcu(void)
{
	...
	hwlog_info("nve_direct_access read tof sensor offset offset[0]=%d offset[9]=%d offset[19]=%d offset[27]=%d)\n",
		tof_sensor_offset[0], tof_sensor_offset[9],
		tof_sensor_offset[19],
...
int send_laya_als_calibrate_data_to_mcu(void)
{
	...
	memcpy(&als_dark_noise_offset, user_info.nv_data + ALS_CALIDATA_NV_SIZE,
		sizeof(als_dark_noise_offset));
	hwlog_info("send_laya_als_calibrate_data_to_mcu %d %d %d %d %d %d %d\n",
		als_offset[0], als_offset[1], als_offset[2], als_offset[3],
		als_offset[4], als_offset[5], als_dark_noise_offset);
...
int send_als_calibrate_data_to_mcu(void)
{
	...
	if (send_calibrate_data_to_mcu(TAG_ALS, SUB_CMD_SET_OFFSET_REQ,
		als_sensor_calibrate_data, ALS_CALIDATA_NV_SIZE, false))
		return -1;
...
int send_als_under_tp_calibrate_data_to_mcu(void)
{
	...
	if (sizeof(als_under_tp_cal_data) >
		...
		if (read_calibrate_data_from_nv(ALS_TP_CALIDATA_NV2_NUM,
			...
			return -1;
		memcpy((char *)(&als_under_tp_cal_data) + ALS_TP_CALIDATA_NV1_SIZE,
			user_info.nv_data, ALS_TP_CALIDATA_NV2_SIZE);

		if (read_calibrate_data_from_nv(ALS_TP_CALIDATA_NV3_NUM,
			sizeof(als_under_tp_cal_data) - ALS_TP_CALIDATA_NV1_SIZE -
			ALS_TP_CALIDATA_NV2_SIZE, "ALSTP3"))
			return -1;
		memcpy((char *)(&als_under_tp_cal_data) + ALS_TP_CALIDATA_NV1_SIZE +
			ALS_TP_CALIDATA_NV2_SIZE, user_info.nv_data,
			sizeof(als_under_tp_cal_data) - ALS_TP_CALIDATA_NV1_SIZE -
...
#ifdef CONFIG_CONTEXTHUB_SHMEM
		if (shmem_send(TAG_ALS, &als_under_tp_cal_data,
			sizeof(als_under_tp_cal_data)))
...
int send_airpress_calibrate_data_to_mcu(void)
{
	...
	if (send_calibrate_data_to_mcu(TAG_PRESSURE, SUB_CMD_SET_OFFSET_REQ,
		&airpress_data.offset, sizeof(airpress_data.offset), false))
		return -1;
...
int send_cap_prox1_calibrate_data_to_mcu(void)
{
	...
	/* send to mcu */
	memcpy(&g_sar_calibrate_datas, user_info.nv_data,
		sizeof(g_sar_calibrate_datas));
...
void reset_calibrate_data(void)
{
	...
	bu27006muc_flag = sensor_get_als_bu27006muc_flag();
	...
	if (rohm_rgb_flag == 1 || avago_rgb_flag == 1 ||
		...
		vishay_vcnl36658_als_flag == 1 || tsl2591_flag == 1 ||
		...
		if (als_data.als_phone_type == LAYA) {
			...
			als_data.als_phone_type == VOGUE) {
			send_calibrate_data_to_mcu(TAG_ALS, SUB_CMD_SET_ALS_PA,
				(const void *)&als_rgb_pa_to_sh,
				sizeof(als_rgb_pa_to_sh), true);
#ifdef CONFIG_CONTEXTHUB_SHMEM
		if (shmem_send(TAG_ALS, &als_under_tp_cal_data,
			sizeof(als_under_tp_cal_data)))
			hwlog_err("%s shmem_send error\n", __func__);
			...
			send_calibrate_data_to_mcu(TAG_ALS,
				SUB_CMD_SET_OFFSET_REQ,
				als_sensor_calibrate_data,
	...
	if (strlen(sensor_chip_info[AIRPRESS])) {
		...
		send_calibrate_data_to_mcu(TAG_PRESSURE, SUB_CMD_SET_OFFSET_REQ,
			&airpress_data.offset, sizeof(airpress_data.offset), true);
	}
...
int send_para_flag;
static int set_pinhole_para_flag;
static int set_rpr531_para_flag;
static int set_tmd2745_para_flag;
...
void set_als_extend_prameters(
	als_para_normal_table *tp_color_table, int arraysize)
{
	...
	hwlog_info( "%s: phone_type = %d, phone_version = %d, tp = %d\n",
		__func__, als_data.als_phone_type,
		als_data.als_phone_version, tplcd_manufacture);

	for (i = 0; i < arraysize; i++) {
		if (((tp_color_table + i)->phone_type == als_data.als_phone_type) &&
			((tp_color_table + i)->phone_version == als_data.als_phone_version) &&
			(((tp_color_table + i)->tp_manufacture == tplcd_manufacture) ||
			((tp_color_table + i)->tp_manufacture == TS_PANEL_UNKNOWN))) {
			table_index = i;
			break;
		}
	}
	...
	memcpy(als_data.als_extend_data,
		(tp_color_table + table_index)->als_para,
		sizeof(s16) * (tp_color_table + table_index)->len >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(s16) * (tp_color_table + table_index)->len);
	min_threshold_num = (tp_color_table + table_index)->len - 1;
	max_threshold_num = (tp_color_table + table_index)->len - 2;
	min_threshold_als_para =
		(tp_color_table + table_index)->als_para[min_threshold_num];
	max_threshold_als_para =
		(tp_color_table + table_index)->als_para[max_threshold_num];
}

void select_stk3638_als_data(void)
{
	int j = 0;
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(stk3638_als_para); i++) {
		if ((stk3638_als_para[i].phone_type ==
			als_data.als_phone_type) &&
			(stk3638_als_para[i].phone_version ==
			als_data.als_phone_version) &&
			(stk3638_als_para[i].tp_manufacture ==
			tp_manufacture ||
			stk3638_als_para[i].tp_manufacture ==
			DEFAULT_TPLCD) &&
			(stk3638_als_para[i].tp_color == phone_color)) {
			j = i;
			break;
		}
	}
	memcpy(als_data.als_extend_data,
		stk3638_als_para[j].stk3638_para,
		sizeof(stk3638_als_para[j].stk3638_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(stk3638_als_para[j].stk3638_para));
	min_threshold_als_para =
		stk3638_als_para[j].stk3638_para[STK3638_MIN_THRESHOLD_NUM];
	max_threshold_als_para =
		stk3638_als_para[j].stk3638_para[STK3638_MAX_THRESHOLD_NUM];
	hwlog_info("stk3638_als_para_tabel= %d\n", j);
	for (i = 0; i < STK3638_PARA_SIZE; i++)
		hwlog_info("the stk3638 %d als paramater is the %d", i,
			stk3638_als_para[j].stk3638_para[i]);
	hwlog_info("\n");
}

void select_liteon_ltr2594_als_data(void)
{
	int j = 0;
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(ltr2594_als_para); i++) {
		if ((ltr2594_als_para[i].phone_type ==
			als_data.als_phone_type) &&
			(ltr2594_als_para[i].phone_version ==
			als_data.als_phone_version) &&
			(ltr2594_als_para[i].tp_manufacture ==
			tp_manufacture ||
			ltr2594_als_para[i].tp_manufacture ==
			DEFAULT_TPLCD) &&
			(ltr2594_als_para[i].tp_color == phone_color)) {
			j = i;
			break;
		}
	}
	memcpy(als_data.als_extend_data,
		ltr2594_als_para[j].ltr2594_para,
		sizeof(ltr2594_als_para[j].ltr2594_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(ltr2594_als_para[j].ltr2594_para));
	min_threshold_als_para =
		ltr2594_als_para[j].ltr2594_para[LTR2594_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		ltr2594_als_para[j].ltr2594_para[LTR2594_MAX_ThRESHOLD_NUM];

	hwlog_info("ltr2594_als_para_tabel= %d\n", j);
	for (i = 0; i < LTR2594_PARA_SIZE; i++)
		hwlog_info("the ltr2594 %d als paramater is the %d", i,
			ltr2594_als_para[j].ltr2594_para[i]);
	hwlog_info("\n");
}
...
void set_vd6281_als_extend_prameters(void)
{
	...
	memcpy(als_data.als_extend_data,
		vd6281_als_para_diff_tp_color_table[vd6281_als_para_table].vd6281_para,
	...
	min_threshold_als_para =
		vd6281_als_para_diff_tp_color_table[vd6281_als_para_table].vd6281_para[VD6281_MAX_ThRESHOLD_NUM];
	max_threshold_als_para =
		vd6281_als_para_diff_tp_color_table[vd6281_als_para_table].vd6281_para[VD6281_MIN_ThRESHOLD_NUM];

	hwlog_info("vd6281_als_para_tabel= %d\n", vd6281_als_para_table);
	for (i = 0; i < VD6281_PARA_SIZE; i++)
		hwlog_info("the vd6281 %d als paramater is the %d", i,
			vd6281_als_para_diff_tp_color_table[vd6281_als_para_table].vd6281_para[i]);
	hwlog_info("\n");
}
...
void set_tsl2591_als_extend_prameters(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(tsl2591_als_para_diff_tp_color_table); i++) {
		if ((tsl2591_als_para_diff_tp_color_table[i].phone_type == als_data.als_phone_type) &&
			...
			(tsl2591_als_para_diff_tp_color_table[i].tp_color == phone_color ||
			 tsl2591_als_para_diff_tp_color_table[i].tp_color == OTHER)) {
			tsl2591_als_para_table = i;
			break;
		}
	}
	memcpy(als_data.als_extend_data,
		tsl2591_als_para_diff_tp_color_table[tsl2591_als_para_table].tsl2591_para,
	...
	min_threshold_als_para =
		tsl2591_als_para_diff_tp_color_table[tsl2591_als_para_table].tsl2591_para[TSL2591_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		tsl2591_als_para_diff_tp_color_table[tsl2591_als_para_table].tsl2591_para[TSL2591_MAX_ThRESHOLD_NUM];

	hwlog_info("A_als_para_tabel= %d\n", tsl2591_als_para_table);
	for (i = 0; i < TSL2591_PARA_SIZE; i++)
		hwlog_info("the A %d als paramater is the %d", i,
			tsl2591_als_para_diff_tp_color_table[tsl2591_als_para_table].tsl2591_para[i]);
	hwlog_info("\n");
}
...
void set_bh1726_als_extend_prameters(void)
{
	...
	memcpy(als_data.als_extend_data,
		bh1726_als_para_diff_tp_color_table[bh1726_als_para_table].bh1726_para,
	...
	min_threshold_als_para =
		bh1726_als_para_diff_tp_color_table[bh1726_als_para_table].bh1726_para[BH1726_MAX_ThRESHOLD_NUM];
	max_threshold_als_para =
		bh1726_als_para_diff_tp_color_table[bh1726_als_para_table].bh1726_para[BH1726_MIN_ThRESHOLD_NUM];

	hwlog_info("B_als_para_tabel= %d\n", bh1726_als_para_table);
	for (i = 0; i < BH1726_PARA_SIZE; i++)
		hwlog_info("the B %d als paramater is the %d", i,
			bh1726_als_para_diff_tp_color_table[bh1726_als_para_table].bh1726_para[i]);
	hwlog_info("\n");
}
...
void set_rpr531_als_extend_prameters(void)
{
	...
	min_threshold_als_para =
		rpr531_als_para_diff_tp_color_table[rpr531_als_para_table].rpr531_para[RPR531_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		rpr531_als_para_diff_tp_color_table[rpr531_als_para_table].rpr531_para[RPR531_MAX_ThRESHOLD_NUM];

	hwlog_info("rpr531_als_para_tabel= %d\n", rpr531_als_para_table);
	for (i = 0; i < RPR531_PARA_SIZE; i++)
		hwlog_info("the rpr531 %d als paramater is the %d", i,
			rpr531_als_para_diff_tp_color_table[rpr531_als_para_table].rpr531_para[i]);
	hwlog_info("\n");
}
...
static void set_pinhole_als_extend_parameters(void)
{
	...
	memcpy(als_data.als_extend_data,
		pinhole_als_para_diff_tp_color_table[als_para_table].pinhole_para,
		sizeof(pinhole_als_para_diff_tp_color_table[als_para_table].pinhole_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(pinhole_als_para_diff_tp_color_table[als_para_table].pinhole_para));
	min_threshold_als_para =
		pinhole_als_para_diff_tp_color_table[als_para_table].pinhole_para[LTR578_APDS9922_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		pinhole_als_para_diff_tp_color_table[als_para_table].pinhole_para[LTR578_APDS9922_MAX_ThRESHOLD_NUM];
	hwlog_info("als_para_tabel=%d phone_type=%d, sens_name=%d, phone_version=%d\n",
		...
		for (i = 0; i < PINHOLE_PARA_SIZE; i++)
			hwlog_info("the %d als paramater is the %d", i,
				pinhole_als_para_diff_tp_color_table[als_para_table].pinhole_para[i]);
	hwlog_info("\n");
}
...
static void set_tmd2745_als_extend_parameters(void)
{
	...
	min_threshold_als_para =
		tmd2745_als_para_diff_tp_color_table[als_para_table].als_para[TMD2745_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		tmd2745_als_para_diff_tp_color_table[als_para_table].als_para[TMD2745_MAX_ThRESHOLD_NUM];
	...
	for (i = 0; i < TMD2745_PARA_SIZE; i++)
		hwlog_info("the ams %d als paramater is the %d", i,
			tmd2745_als_para_diff_tp_color_table[als_para_table].als_para[i]);
...
static int read_tp_module_notify(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	} else if (set_rpr531_para_flag == 1) {
		set_rpr531_als_extend_prameters();
	} else if (set_tmd2745_para_flag == 1) {
...
static void select_rohm_als_data(void)
{
	...
	memcpy(als_data.als_extend_data, als_para_diff_tp_color_table[als_para_table].bh745_para,
			...
			sizeof(als_para_diff_tp_color_table[als_para_table].bh745_para));
	min_threshold_als_para =
		als_para_diff_tp_color_table[als_para_table].bh745_para[BH1745_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		als_para_diff_tp_color_table[als_para_table].bh745_para[BH1745_MAX_ThRESHOLD_NUM];
...
static void select_rohmbh1749_als_data(void)
{
	...
	memcpy(als_data.als_extend_data,
		g_bh1749_als_para_diff_tp_color_table[als_para_table].bh1749_para,
...
static void select_rohmbu27006muc_als_data(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_bu27006muc_als_para_diff_tp_color_table); i++) {
		if ((g_bu27006muc_als_para_diff_tp_color_table[i].phone_type == als_data.als_phone_type) &&
		    (g_bu27006muc_als_para_diff_tp_color_table[i].phone_version == als_data.als_phone_version) &&
		    (g_bu27006muc_als_para_diff_tp_color_table[i].tp_lcd_manufacture == tplcd_manufacture ||
		     g_bu27006muc_als_para_diff_tp_color_table[i].tp_lcd_manufacture == DEFAULT_TPLCD) &&
		    (g_bu27006muc_als_para_diff_tp_color_table[i].tp_color == phone_color)) {
			als_para_table = i;
			break;
		}
	}
	memcpy(als_data.als_extend_data, g_bu27006muc_als_para_diff_tp_color_table[als_para_table].bu27006muc_para,
		       sizeof(g_bu27006muc_als_para_diff_tp_color_table[als_para_table].bu27006muc_para) >
		       SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE ? SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		       sizeof(g_bu27006muc_als_para_diff_tp_color_table[als_para_table].bu27006muc_para));
	min_threshold_als_para =
	    g_bu27006muc_als_para_diff_tp_color_table[als_para_table].bu27006muc_para[BU27006MUC_MIN_THRESHOLD_NUM];
	max_threshold_als_para =
	    g_bu27006muc_als_para_diff_tp_color_table[als_para_table].bu27006muc_para[BU27006MUC_MAX_THRESHOLD_NUM];

	hwlog_info("als_para_tabel=%d ,bu27006muc phone_color=0x%x  tplcd_manufacture=%d, phone_type=%d,phone_version=%d\n",
...
static void select_avago_als_data(void)
{
	...
	memcpy(als_data.als_extend_data,
		...
		sizeof(apds_als_para_diff_tp_color_table[als_para_table].apds251_para));
	min_threshold_als_para =
		apds_als_para_diff_tp_color_table[als_para_table].apds251_para[APDS9251_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		apds_als_para_diff_tp_color_table[als_para_table].apds251_para[APDS9251_MAX_ThRESHOLD_NUM];
...
static void select_apds9999_als_data(void)
{
	...
	memcpy(als_data.als_extend_data,
		...
		sizeof(apds9999_als_para_diff_tp_color_table[als_para_table].apds9999_para));
	min_threshold_als_para =
		apds9999_als_para_diff_tp_color_table[als_para_table].apds9999_para[APDS9251_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		apds9999_als_para_diff_tp_color_table[als_para_table].apds9999_para[APDS9251_MAX_ThRESHOLD_NUM];
	hwlog_info("als_para_tabel=%d apds9251 phone_color=0x%x phone_type=%d,phone_version=%d\n",
		als_para_table, phone_color, als_data.als_phone_type, als_data.als_phone_version);
}
...
static void select_ams_tmd3725_als_data(void)
{
	...
	memcpy(als_data.als_extend_data,
		...
		sizeof(tmd3725_als_para_diff_tp_color_table[als_para_table].tmd3725_para));
	min_threshold_als_para =
		tmd3725_als_para_diff_tp_color_table[als_para_table].tmd3725_para[TMD3725_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		tmd3725_als_para_diff_tp_color_table[als_para_table].tmd3725_para[TMD3725_MAX_ThRESHOLD_NUM];
...
static void select_ams_tmd3702_als_data(void)
{
	...
	memcpy(als_data.als_extend_data,
		...
		sizeof(tmd3702_als_para_diff_tp_color_table[als_para_table].tmd3702_para));
	min_threshold_als_para =
		tmd3702_als_para_diff_tp_color_table[als_para_table].tmd3702_para[TMD3702_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		tmd3702_als_para_diff_tp_color_table[als_para_table].tmd3702_para[TMD3702_MAX_ThRESHOLD_NUM];
	hwlog_info("als_para_tabel=%d tmd3702 phone_color=0x%x phone_type=%d,phone_version=%d\n",
		     als_para_table, phone_color, als_data.als_phone_type, als_data.als_phone_version);
}

static void select_ams_tcs3707_als_data(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(tcs3707_tp_color_table); i++) {
		if ((tcs3707_tp_color_table[i].phone_type == als_data.als_phone_type) &&
			(tcs3707_tp_color_table[i].phone_version == als_data.als_phone_version) &&
			(tcs3707_tp_color_table[i].tp_lcd_manufacture == tplcd_manufacture ||
			tcs3707_tp_color_table[i].tp_lcd_manufacture == DEFAULT_TPLCD) &&
			(tcs3707_tp_color_table[i].tp_color == phone_color)) {
			als_para_table = i;
			break;
		}
	}
	memcpy(als_data.als_extend_data, tcs3707_tp_color_table[als_para_table].tcs3707_para,
		sizeof(tcs3707_tp_color_table[als_para_table].tcs3707_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(tcs3707_tp_color_table[als_para_table].tcs3707_para));
	min_threshold_als_para =
		tcs3707_tp_color_table[als_para_table].tcs3707_para[TCS3707_MIN_THRESHOLD_NUM];
	max_threshold_als_para =
		tcs3707_tp_color_table[als_para_table].tcs3707_para[TCS3707_MAX_THRESHOLD_NUM];
	hwlog_info("als_para_tabel=%d tcs3707 phone_color=0x%x phone_type=%d,phone_version=%d\n",
		als_para_table, phone_color, als_data.als_phone_type, als_data.als_phone_version);
}
...
static void select_ams_tcs3701_als_data(void)
{
	...
	memcpy(als_data.als_extend_data,
		...
		sizeof(tcs3701_als_para_diff_tp_color_table[als_para_table].tcs3701_para));
	min_threshold_als_para =
		tcs3701_als_para_diff_tp_color_table[als_para_table].tcs3701_para[TCS3701_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		tcs3701_als_para_diff_tp_color_table[als_para_table].tcs3701_para[TCS3701_MAX_ThRESHOLD_NUM];
...
static void select_vishay_vcnl36658_als_data(void)
{
	...
	memcpy(als_data.als_extend_data,
		...
		sizeof(vcnl36658_als_para_diff_tp_color_table[als_para_table].vcnl36658_para));
	min_threshold_als_para =
		vcnl36658_als_para_diff_tp_color_table[als_para_table].vcnl36658_para[VCNL36658_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		vcnl36658_als_para_diff_tp_color_table[als_para_table].vcnl36658_para[VCNL36658_MAX_ThRESHOLD_NUM];
...
static void select_liteon_ltr582_als_data(void)
{
	...
	min_threshold_als_para =
		ltr582_als_para_diff_tp_color_table[als_para_table].ltr582_para[LTR582_MIN_ThRESHOLD_NUM];
	max_threshold_als_para =
		ltr582_als_para_diff_tp_color_table[als_para_table].ltr582_para[LTR582_MAX_ThRESHOLD_NUM];
...
int read_tpcolor_from_nv(void)
{
	...
	else if (bu27006muc_flag == 1)
		select_rohmbu27006muc_als_data();
...
void select_als_para(struct device_node *dn)
{
	...
	if (rohm_rgb_flag == 1) {
		...
		select_ams_tcs3707_als_data();
	} else if (ltr2594_als_flag == 1) {
		select_liteon_ltr2594_als_data();
	} else if (stk3638_als_flag == 1) {
		select_stk3638_als_data();
		...
		set_als_extend_prameters(
			&vcnl36832_als_para_diff_tp_color_table[0],
			ARRAY_SIZE(vcnl36832_als_para_diff_tp_color_table));
	} else if (stk3338_als_flag == 1) {
		set_als_extend_prameters(
			&stk3338_als_para_diff_tp_color_table[0],
			ARRAY_SIZE(stk3338_als_para_diff_tp_color_table));
	} else if (ltr2568_als_flag == 1) {
		set_als_extend_prameters(
			&ltr2568_als_para_diff_tp_color_table[0],
			ARRAY_SIZE(ltr2568_als_para_diff_tp_color_table));
	} else if (veml32185_als_flag == 1) {
		set_als_extend_prameters(
			&veml32185_als_para[0],
			ARRAY_SIZE(veml32185_als_para));
	} else {
...
static int send_current_to_mcu_mag(int current_value_now)
{
	...
	charge_current_data.current_offset_x =
		current_value_now * akm_current_x_fac / MAG_CURRENT_FAC_RAIO;
	charge_current_data.current_offset_y =
		current_value_now * akm_current_y_fac / MAG_CURRENT_FAC_RAIO;
	charge_current_data.current_offset_z =
		current_value_now * akm_current_z_fac / MAG_CURRENT_FAC_RAIO;
...
{
	close_send_current();
	memset(&charge_current_data, 0,
		sizeof(charge_current_data));
	memcpy(charge_current_data.str_charge,
		str_charger_current_out,
		sizeof(str_charger_current_out));
	send_mag_charger_to_mcu();
}
			...
			unsigned long action, void *data)
{
	...
	if (akm_need_charger_current) {
		...
		memset(&charge_current_data, 0,
			sizeof(charge_current_data));
		memcpy(charge_current_data.str_charge,
			str_charger, sizeof(str_charger));
		send_mag_charger_to_mcu();
	}
	return 0;
}
