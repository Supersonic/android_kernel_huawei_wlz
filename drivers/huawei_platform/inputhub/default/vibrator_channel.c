...
#define VIB_IRQ_RESET_ENABLE_MAX    3
#define VIB_IRQ_NO_RESET_ENABLE_MAX 3
...
#define TFA9874_INT_ALARM_DELAY     250 /* datasheet alarm delay min 200ms */
...
static void vibrator_irq_work(struct work_struct *work)
{
	int32_t ret;
	int32_t gpio_value;
	uint32_t out_reg_addr, clear_reg_addr;
	...
	if (vibrator_data.chip_type == VIBRATOR_CHIP_TFA9874) {
		...
		vibrator_reg_read(TFA9874_INT_ENABLE_REG1, &enable_bits);
		alarm_delay = TFA9874_INT_ALARM_DELAY;
...
static void vibrator_irq_enable(void)
{
	if (data && data->irq_requested && !data->irq_enabled) {
		data->irq_enabled = true;
		enable_irq(data->irq_int);
	}
}

static void vibrator_irq_disable(void)
{
	if (data && data->irq_requested && data->irq_enabled) {
...
static void vibrator_enable(struct led_classdev *cdev, enum led_brightness value)
{
	...
	hwlog_info("vibrator_enable, time=%d\n", val);
	if (val > 0) {
		vibrator_shake = 1;
		...
		vib_time = val;
		vibrator_irq_enable();
		hwlog_info("vibrator_enable, time=%d end\n", val);
	}else{
		vibrator_irq_disable();
		vib_time = 0;
		vibrator_shake = 0;
