...
#define GF_G3_G5_CHIP_ID_ADDR_HIGH   0x00
#define GF_G3_G5_CHIP_ID_ADDR_LOW    0x00
#define GOODIX_WRITE_CMD_TX_LEN      3
#define GF_G3_G5_NAME_LEN            15
...
struct sar_sensor_detect g_abov_sar_detect_aux = {
	.cfg = DEF_SENSOR_COM_SETTING,
	.detect_flag = 0,
	.chip_id = 0,
};
...
void read_als_data_from_dts(struct device_node *dn)
{
	...
	if (!strncmp(chip_info, "huawei,rohm_bu27006muc", sizeof("huawei,rohm_bu27006muc"))) {
		g_bu27006muc_flag = 1;
		hwlog_err("%s:rohm_bu27006muc i2c_address suc,%d \n", __func__, temp);
	}
	...
	if (of_property_read_u32(dn, "is_dc_supported", &temp))
		...
		als_data.is_dc_supported = (int)temp;
...
static void read_abov_sar_data_from_dts(struct device_node *dn, struct sensor_detect_manager *sm)
{
	...
	struct sar_platform_data *sar_pdata_abov = (struct sar_platform_data *)sm->spara;

	read_chip_info(dn, sm->sensor_id);
	threshold_to_modem = sar_pdata_abov->sar_datas.abov_data.threshold_to_modem;
	...
	if (of_property_read_u16(dn, "phone_type", &abov_phone_type)) {
		sar_pdata_abov->sar_datas.abov_data.phone_type = 0;
		hwlog_err("%s:read phone_type fail\n", __func__);
	} else {
		sar_pdata_abov->sar_datas.abov_data.phone_type = abov_phone_type;
		hwlog_info("%s:read phone_type:0x%x\n",
			__func__, sar_pdata_abov->sar_datas.abov_data.phone_type);
	}

	if (of_property_read_u16(dn, "abov_project_id", &abov_project_id)) {
		sar_pdata_abov->sar_datas.abov_data.abov_project_id = 0;
		hwlog_err("%s:read project id fail\n", __func__);
	} else {
		sar_pdata_abov->sar_datas.abov_data.abov_project_id = abov_project_id;
		hwlog_info("%s:read project id:0x%x\n", __func__,
			sar_pdata_abov->sar_datas.abov_data.abov_project_id);
	}

	if (of_property_read_u32(dn, "ph", &ph)) {
		sar_pdata_abov->sar_datas.abov_data.ph = 0; // 0x0: enable ph0
		hwlog_err("%s:read ph fail\n", __func__);
	} else {
		sar_pdata_abov->sar_datas.abov_data.ph = (uint8_t)ph;
		hwlog_info("%s:read ph:0x%x\n",
			__func__, sar_pdata_abov->sar_datas.abov_data.ph);
	}
	calibrate_thred = sar_pdata_abov->sar_datas.abov_data.calibrate_thred;
	...
	struct sensor_detect_manager *sm)
{
	...
	if (!strncmp(sensor_chip_info[sm->sensor_id], "huawei,semtech-sx9323",
		...
		read_abov_sar_data_from_dts(dn, sm);
	}
	read_sensorlist_info(dn, sm->sensor_id);
}
...
static void read_sound_data_from_dts(struct device_node *dn)
{
	...
	if (of_property_read_u32(dn, "gpio_irq_soc", &temp))
		...
		ps_data.sound_gpio_irq_soc = (GPIO_NUM_TYPE)temp;

	if (of_property_read_u32(dn, "gpio_irq_sh", &temp))
		hwlog_warn("%s:read sound gpio_irq_sh fail\n", __func__);
	else
		ps_data.sound_gpio_irq_sh = (GPIO_NUM_TYPE)temp;

	hwlog_info("sound_gpio_irq_soc = %d, sound_gpio_irq_sh = %d\n",
		ps_data.sound_gpio_irq_soc, ps_data.sound_gpio_irq_sh);
}
...
static void gf_ud_common_handle(GPIO_NUM_TYPE gpio_cs, const uint32_t size,
	const struct goodix_reg_info *gf_reg_info)
{
	uint32_t tx_len;
	union SPI_CTRL ctrl;
	uint32_t i;
	...
	for (i = 0; i < size; i++) {
		tx[1] = gf_reg_info[i].addr >> 8; /* shift 8 bits */
		tx[2] = gf_reg_info[i].addr;
		tx[5] = gf_reg_info[i].value >> 8; /* shift 8 bits */
		tx[6] = gf_reg_info[i].value;
		mcu_spi_rw(FP_SPI_NUM, ctrl, tx, tx_len, NULL, 0);

		if (gf_reg_info[i].delay)
			msleep(gf_reg_info[i].delay);
	}

	tx[0] = GOODIX_WRITE_CMD;
	tx[1] = GF_G3_G5_CHIP_ID_ADDR_HIGH;
	tx[2] = GF_G3_G5_CHIP_ID_ADDR_LOW;
	tx_len = GOODIX_WRITE_CMD_TX_LEN;
	mcu_spi_rw(FP_SPI_NUM, ctrl, tx, tx_len, NULL, 0);
}

static void set_goodix_udg3_config(GPIO_NUM_TYPE gpio_cs)
{
	...
	const uint32_t size = ARRAY_SIZE(g3_reg_info);

	gf_ud_common_handle(gpio_cs, size, g3_reg_info);
}

static void set_goodix_udg5_config(GPIO_NUM_TYPE gpio_cs)
{
	struct goodix_reg_info g5_reg_info[] = {
		{ 0xE500, 0x0000, 2 },
		{ 0x00C2, 0xBFD1, 0 },
		{ 0xE600, 0x0000, 0 },
		{ 0x00C2, 0xBFD0, 0 },
	...
	const uint32_t size = ARRAY_SIZE(g5_reg_info);

	gf_ud_common_handle(gpio_cs, size, g5_reg_info);
}
	...
	struct sensor_combo_cfg *cfg)
{
	...
	if (!ret) {
		if (!strncmp(sensor_vendor, "goodix,goodixG2",
			...
			GF_G3_G5_NAME_LEN)) {
			set_goodix_udg3_config(gpio_cs);
		} else if (!strncmp(sensor_vendor, "goodix,goodixG5",
			GF_G3_G5_NAME_LEN)) {
			set_goodix_udg5_config(gpio_cs);
		} else if (!strncmp(sensor_vendor, "goodix", GOODIX_NAME_LEN)) {
...
static void read_cap_prox1_info(struct device_node *dn)
{
	...
	if (!strncmp(chip_info, "huawei,semtech-sx9335",
		...
		strlen("huawei,abov-a96t3x6"))) {
		hwlog_info("cap_prox1 sensor from dts is abov-a96t3x6\n");
		g_abov_sar_detect_aux.detect_flag = 1;
		g_abov_sar_detect_aux.cfg.bus_num = (uint8_t)i2c_bus_num;
		g_abov_sar_detect_aux.cfg.i2c_address = (uint8_t)i2c_address;
		g_abov_sar_detect_aux.chip_id = (uint16_t)register_add;
		g_abov_sar_detect_aux.chip_id_value[0] = (uint32_t)wia[0];
		g_abov_sar_detect_aux.chip_id_value[1] = (uint32_t)wia[1];
	}
}
...
int sensor_get_als_bu27006muc_flag(void)
{
	return g_bu27006muc_flag;
}
