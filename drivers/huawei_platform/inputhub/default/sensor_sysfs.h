...
extern struct sar_platform_data sar1_pdata;
...
#define AIRPRESS_RESULT      ((airpress_data.offset >= MIN_AIRPRESS_OFFSET && \
	airpress_data.offset <= MAX_AIRPRESS_OFFSET) ? "pass" : "fail")
...
extern union sar_calibrate_data g_sar_calibrate_datas;
...
ssize_t show_sensor_in_board_status_sysfs(struct device *dev,
	struct device_attribute *attr, char *buf);
read_info_t send_airpress_calibrate_cmd(uint8_t tag,
	unsigned long val, RET_TYPE *rtype);
