...
#define  VIB_GESTURE_NUM 	700
#define  ACC_DATA_LEN 		3
#define  ACC_NUM_FOR_VIB 	3
#ifdef CONFIG_HUAWEI_DSM
...
//vibrator issue
static struct mutex mutex_vibrator;
static int32_t vibrator_acc_data[ACC_NUM_FOR_VIB][ACC_DATA_LEN] = {0};
static int32_t vibrator_ps_data = 1;
static int32_t vibrator_index = 0;
int vibrator_abnormal_sound = 0;
...
extern void enable_kb_when_recovery_iom3(void);
extern void disable_kb_when_sysreboot(void);
extern int send_kb_cmd(unsigned int cmd, unsigned long arg);
...
int ap_hall_report(int value)
{
	...
	if(hall_number == 2){
        ...
        return report_sensor_event(TAG_HALL, &double_hall_value, sizeof(double_hall_value));
}
...
{
	/*all sensors report data with command CMD_PRIVATE*/
	return (head->tag == TAG_KB)
		&& (CMD_KB_REPORT_REQ == head->cmd);
}
...
static void iom7_charging_event_handle(struct work_struct *work)
{
	...
	if (charge_event == VCHRG_THERMAL_POWER_OFF) {
		__pm_wakeup_event(&wlock, jiffies_to_msecs(15 * HZ));
		return;
	}
			      ...
			      uint64_t timestamp)
{
	...
	if (TAG_FLUSH_META != tag) {
		...
		memcpy(&event.value, (char *)&(sensor_batch_data->x),
		       event.length);
		sensor_get_data(&event);
		//get three accel data for vibrator
		if(1 == vibrator_abnormal_sound && tag == TAG_ACCEL)
		{
			memcpy(&vibrator_acc_data[vibrator_index++ % ACC_NUM_FOR_VIB][0], (char *)&(sensor_batch_data->x),
							ACC_DATA_LEN * sizeof(int32_t));
		}
		if(1 == vibrator_abnormal_sound && tag == TAG_PS)
		{
			vibrator_ps_data = sensor_batch_data->x;
		}
	} else {
		ltimestamp = 0;
		event.type = tag_to_hal_sensor_type[tag];
...
static int64_t get_sensor_syscounter_timestamp(pkt_batch_data_req_t* sensor_event)
{
    ...
    if (sensor_event->data_flag & DATA_FLAG_VALID_TIMESTAMP)
    {
...
void inputhub_process_sensor_report(const pkt_header_t* head)
{
        ...
        if (head->tag == TAG_RPC)
        {
            hwlog_info("RPC tag=%d:data length:%d, [data0:%d][data1:%d][data2:%d]",
                       head->tag, head->length, sensor_event->xyz[0].x, sensor_event->xyz[0].y, sensor_event->xyz[0].z);
        }
...
void vibrator_control_sensor(int enable, int tag)
{
	int val = 2;
	int ret = -1;
	char buf[MAX_PKT_LENGTH];
	...
	{
		//send_cmd_to_mcu
		mutex_lock(&mutex_vibrator);
		memset(&buf, 0, sizeof(buf));
		((pkt_header_t *) buf)->tag = tag;
		((pkt_header_t *) buf)->cmd = CMD_CMN_INTERVAL_REQ;
		((pkt_header_t *) buf)->resp =  NO_RESP;
		((pkt_header_t *) buf)->length = sizeof(val);
		memcpy(buf + sizeof(pkt_header_t), &val, sizeof(val));

		ret = inputhub_mcu_send((const char *)buf, (unsigned int)(sizeof(pkt_header_t) + sizeof(val)));
		mutex_unlock(&mutex_vibrator);
		if (ret) {
			hwlog_err("vibrator send sensor %d enable cmd to mcu fail,ret=%d\n", tag, ret);
			return;
		}
		//hwlog_info(" sensor %d enable for vib \n", tag);
	}
	else if(enable == 0)
	{
		mutex_lock(&mutex_vibrator);
		memset(&buf, 0, sizeof(buf));
		((pkt_header_t *) buf)->tag = tag;
		((pkt_header_t *) buf)->cmd = CMD_CMN_CLOSE_REQ;
		((pkt_header_t *) buf)->resp =  NO_RESP;
		((pkt_header_t *) buf)->length = 0;
		memcpy(buf + sizeof(pkt_header_t), &val, ((pkt_header_t *) buf)->length);

		ret = inputhub_mcu_send((const char *)buf, sizeof(pkt_header_t) + ((pkt_header_t *) buf)->length);
		mutex_unlock(&mutex_vibrator);
		if (ret) {
			hwlog_err("send sensor %d close cmd to mcu fail,ret=%d\n", tag, ret);
			return;
		}
		//hwlog_info(" sensor %d close for vib\n", tag);
	}
	else
	{
		mutex_lock(&mutex_vibrator);
		//recovery_sensor_status()
		if (sensor_status.status[tag]) {
			...
			//hwlog_info("recovery sensor %d for vib\n", tag);
		}else
		{
			memset(&buf, 0, sizeof(buf));
			((pkt_header_t *) buf)->tag = tag;
			...
			//hwlog_info(" sensor close %d for vib\n", tag);
		}
		mutex_unlock(&mutex_vibrator);
	}
}
int is_ps_near(void)
{
	...
	vibrator_ps_data = -1;
	//close
	vibrator_control_sensor(0, TAG_PS);
	msleep(50);
	//open
	vibrator_control_sensor(1, TAG_PS);

	msleep(300);
	//recovery(TAG_PS);
	vibrator_control_sensor(2, TAG_PS);
	//judge_the_accel_date and do some work
	hwlog_info("vibor ps %d \n", vibrator_ps_data);
...
{
	//clear_accel_data;
	memset(vibrator_acc_data, 0, sizeof(vibrator_acc_data));
	vibrator_control_sensor(1, TAG_ACCEL);
	msleep(20);
	vibrator_control_sensor(2, TAG_ACCEL);
	//judge_the_accel_date and do some work
	if(vibrator_acc_data[0][2] != 0 || vibrator_acc_data[1][2] != 0 || vibrator_acc_data[2][2] != 0)
	{
		//recognize the phone gesture by three sets of accel z_data
		if(vibrator_acc_data[0][2] < VIB_GESTURE_NUM && vibrator_acc_data[0][2] > -(VIB_GESTURE_NUM)
			&& vibrator_acc_data[1][2] < VIB_GESTURE_NUM && vibrator_acc_data[1][2] > -(VIB_GESTURE_NUM)
			&& vibrator_acc_data[2][2] < VIB_GESTURE_NUM && vibrator_acc_data[2][2] > -(VIB_GESTURE_NUM))
		{
			//hwlog_info("vibor acc gesture 1\n");
			return 1;
		}
		else
			return 0;
