...
static ssize_t fhb_write(struct file *file, const char __user *data, size_t len, loff_t *ppos)
{
	...
	if(NULL == data)
	{
		hwlog_err("fingerprint: fhb_write data buf is NULL\n");
		return -1;
	}

	if (len > sizeof(buf))
	{
		hwlog_err("fingerprint: fhb_write len is out of size, len=%d\n", len);
		return -1;
	}
	...
	if (ret) {
		hwlog_err("zl fhb_write fail,ret=%d\n", ret);
	}
...
void fingerprint_report(void)
{
    ...
    int fingerprint_data = fingerprint_upload.data;
    inputhub_route_write(ROUTE_FHB_PORT, (char*)(&fingerprint_data), sizeof(fingerprint_data));
}
