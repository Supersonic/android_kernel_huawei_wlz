...
#define DIST_STD_MM_TENTH_FULLSTEP         96
...
static void drv8846_param_set_default_val(struct drv_data *di)
{
	...
	di->step_device = NULL;
	if (di->step_gen_manner == GPIO_STEP) {
		di->start_speed = STARTING_SPEED_STD_FULLSTEP * speed_multiplier;
		di->speed_step = STARTING_SPEED_STEP_STD_FULLSTEP * speed_multiplier;
		di->cur_speed = STARTING_SPEED_STD_FULLSTEP * speed_multiplier;
