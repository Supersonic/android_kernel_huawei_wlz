	...
	struct sw_device *rootdev)
{
	...
	if (!scmdev->commonkeys_dev) {
		scmdev->commonkeys_dev = sw_register_new_hiddevice(
			KB_COMMON_KEY_DEV,
	...
	if (!scmdev->customkeys_dev) {
		scmdev->customkeys_dev = sw_register_new_hiddevice(
			KB_CUSTOM_KEY_DEV,
...
{
	struct scmkb_device *scmdev = NULL;
	int ret = SUCCESS;
