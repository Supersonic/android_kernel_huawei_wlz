	...
	struct sw_device *rootdev)
{
	...
	if (!cmrdev->commonkeys_dev) {
		cmrdev->commonkeys_dev = sw_register_new_hiddevice(
			KB_COMMON_KEY_DEV,
	...
	if (!cmrdev->customkeys_dev) {
		cmrdev->customkeys_dev = sw_register_new_hiddevice(
			KB_CUSTOM_KEY_DEV,
...
{
	struct cmrkb_device *cmrdev = NULL;
	int ret = SUCCESS;
