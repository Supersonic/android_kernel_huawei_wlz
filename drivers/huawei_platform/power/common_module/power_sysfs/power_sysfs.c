...
#ifdef CONFIG_HISI_BCI_BATTERY
	if (!strcmp("hw_power", cls_name))
		return hw_power_get_class();
#endif /* CONFIG_HISI_BCI_BATTERY */
...
static struct class *power_sysfs_get_class(const char *cls_name)
{
	int i;
	...
	const char *dev_name)
{
	...
	l_device = device_create(l_class, NULL, 0, NULL, dev_name);
	if (IS_ERR(l_device))
		return NULL;
...
EXPORT_SYMBOL_GPL(power_sysfs_create_link_group);
...
EXPORT_SYMBOL_GPL(power_sysfs_remove_link_group);
...
EXPORT_SYMBOL_GPL(power_sysfs_create_group);
...
EXPORT_SYMBOL_GPL(power_sysfs_remove_group);
