...
static int get_batt_para(void)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"vbat_max", (u32 *)&di->vbat_max, BATTERY_DEFAULT_MAX_VOLTAGE);
	...
	/* vbat_para */
	array_len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"vbat_para", VOLT_PARA_LEVEL, VOLT_PARA_TOTAL);
	if (array_len < 0)
		return -EINVAL;
	...
	/* segment_para */
	array_len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"segment_para", SEGMENT_PARA_LEVEL, SEGMENT_PARA_TOTAL);
	if (array_len < 0)
		return -EINVAL;
...
static struct hisi_coul_ops bq28z610_ops = {
