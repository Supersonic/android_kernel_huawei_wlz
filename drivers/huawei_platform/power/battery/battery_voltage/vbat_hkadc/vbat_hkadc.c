...
static int __init early_parse_vbat_hkadc_cmdline(char *p)
{
	...
	if (token) {
		/* 10: decimal base */
		if (kstrtoul(token, 10, &g_v_offset_a) != 0)
			return -1;
	}
	...
	if (token) {
		/* 10: decimal base */
		if (kstrtol(token, 10, &g_v_offset_b) != 0)
			return -1;
	}
	...
	if (token) {
		/* 10: decimal base */
		if (kstrtol(token, 10, &g_v_offset_c) != 0)
			return -1;
	}
...
static struct vbat_hkadc_sysfs_field_info vbat_hkadc_sysfs_field_tbl[] = {
	...
	VBAT_HKADC_SYSFS_FIELD_RW(v_offset_c, V_OFFSET_C),
