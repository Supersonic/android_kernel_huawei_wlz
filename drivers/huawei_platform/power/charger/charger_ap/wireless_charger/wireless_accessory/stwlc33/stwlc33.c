...
static void stwlc33_sleep_enable(int enable)
{
	struct stwlc33_device_info *di = g_stwlc33_di;

	if (di && !g_irq_abnormal_flag) {
		gpio_set_value(di->gpio_sleep_en, enable);
		hwlog_info("gpio value = %d\n",
			gpio_get_value(di->gpio_sleep_en));
	}
}
...
static int stwlc33_kick_watchdog(void)
{
	int ret;

	ret = stwlc33_write_word(STWLC33_RX_FC_TIMER_ADDR, 0);
	if (ret) {
		hwlog_err("write register[0x%x] failed\n",
			STWLC33_RX_FC_TIMER_ADDR);
		return -1;
	}
	return 0;
}
...
static int stwlc33_tx_chip_init(void)
{
	...
	ret |= stwlc33_set_tx_min_fop(STWLC33_TX_MIN_FOP_VAL);
	ret |= stwlc33_set_tx_max_fop(STWLC33_TX_MAX_FOP_VAL);
	...
	struct device_node *np)
{
	...
	ret = gpio_request(di->gpio_en, "stwlc33_en");
	if (ret) {
		hwlog_err("could not request stwlc33_en\n");
		ret = -ENOMEM;
		goto gpio_init_fail_0;
	}
	...
	ret = gpio_request(di->gpio_sleep_en, "stwlc33_sleep_en");
	if (ret) {
		hwlog_err("could not request stwlc33_sleep_en\n");
		ret = -ENOMEM;
		goto gpio_init_fail_1;
	}
	...
	struct device_node *np)
{
	...
	ret = gpio_request(di->gpio_int, "stwlc33_int");
	if (ret) {
		hwlog_err("%s:could not request stwlc33_int\n", __func__);
		ret = -ENOMEM;
		goto irq_init_fail_0;
	}
	...
	if (di->irq_int < 0) {
		hwlog_err("%s:map stwlc33 gpio_int to irq fail\n", __func__);
		ret = -1;
...
static struct wltx_dev_ops stwlc33_tx_ops = {
	...
	.rx_sleep_enable            = stwlc33_sleep_enable,
	...
	.kick_watchdog              = stwlc33_kick_watchdog,
