...
static const char * const charger_otg_device_id_table[] = {
	...
	[OTG_DEVICE_ID_SCHARGER_V200] = "scharger_v200",
	[OTG_DEVICE_ID_SCHARGER_V300] = "scharger_v300",
	[OTG_DEVICE_ID_SCHARGER_V600] = "scharger_v600",
...
static int charger_otg_get_device_id(const char *str)
{
	...
	for (i = 0; i < ARRAY_SIZE(charger_otg_device_id_table); i++) {
		if (!strncmp(str, charger_otg_device_id_table[i], strlen(str)))
			return i;
	}
...
int charger_otg_ops_register(struct charger_otg_device_ops *ops)
{
	...
	if (!g_charger_otg_dev || !ops || !ops->chip_name) {
		hwlog_err("g_charger_otg_dev or ops or chip_name is null\n");
		return -1;
	}

	dev_id = charger_otg_get_device_id(ops->chip_name);
	if (dev_id < 0) {
