...
#define BQ2429X_SYSFS_FIELD_RW(_name, r, f) \
	BQ2429X_SYSFS_FIELD(_name, r, f, 0640, bq2429x_sysfs_store)

#define BQ2429X_SYSFS_FIELD_RO(_name, r, f) \
	BQ2429X_SYSFS_FIELD(_name, r, f, 0440, NULL)
...
{
	int i;
	int limit = ARRAY_SIZE(bq2429x_sysfs_field_tbl);
...
static ssize_t bq2429x_sysfs_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct bq2429x_sysfs_field_info *info = NULL;
	int ret;
	u8 v;
...
static void bq2429x_sysfs_create_group(struct device *dev)
{
	bq2429x_sysfs_init_attrs();
	power_sysfs_create_link_group("hw_power", "charger", "bq2429x",
		dev, &bq2429x_sysfs_attr_group);
}

static void bq2429x_sysfs_remove_group(struct device *dev)
{
	power_sysfs_remove_link_group("hw_power", "charger", "bq2429x",
		dev, &bq2429x_sysfs_attr_group);
}
#else
static inline void bq2429x_sysfs_create_group(struct device *dev)
{
}

static inline void bq2429x_sysfs_remove_group(struct device *dev)
{
}
#endif /* CONFIG_SYSFS */
...
static int bq2429x_dump_register(char *reg_value, int size)
{
...
static int bq2429x_get_register_head(char *reg_head, int size)
{
	char buff[BUF_LEN] = {0};
	int i;
	...
	const struct i2c_device_id *id)
{
	...
	bq2429x_sysfs_create_group(di->dev);
...
static int bq2429x_remove(struct i2c_client *client)
{
	...
	bq2429x_sysfs_remove_group(di->dev);
