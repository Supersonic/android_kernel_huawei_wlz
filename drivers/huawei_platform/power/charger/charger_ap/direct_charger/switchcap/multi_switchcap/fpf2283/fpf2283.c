	...
	u8 *value, u8 reg, unsigned int num_bytes)
{
	...
	*value = reg;
	...
	msg[0].flags = 0;
	msg[0].buf = value;
	msg[0].len = num_bytes + FPF2283_ADDR_LEN;

	ret = i2c_transfer(di->client->adapter, msg, WR_MSG_LEN);
	...
	u8 *value, u8 reg, unsigned int num_bytes)
{
	...
	ret = i2c_transfer(di->client->adapter, msg, RD_MSG_LEN);
...
static int fpf2283_write_byte(u8 reg, u8 value)
{
	...
	temp_buffer[1] = value;
	return fpf2283_write_block(di, temp_buffer, reg, BYTE_LEN);
}
...
static int fpf2283_read_byte(u8 reg, u8 *value)
{
	...
	return fpf2283_read_block(di, value, reg, BYTE_LEN);
}
...
static int fpf2283_check_i2c(struct fpf2283_device_info *di)
{
	...
	return fpf2283_read_block(di, &val, FPF2283_ID_REG, BYTE_LEN);
}
