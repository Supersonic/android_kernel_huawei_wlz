...
static struct loadswitch_ops multi_switchcap_ops = {
	.ls_init = multi_switchcap_charge_init,
	.ls_exit = multi_switchcap_charge_exit,
	.ls_enable = multi_switchcap_charge_enable,
	.ls_discharge = multi_switchcap_discharge,
	.is_ls_close = multi_switchcap_is_device_close,
	.get_ls_id = multi_switchcap_get_device_id,
	.watchdog_config_ms = multi_switchcap_config_watchdog_ms,
	.ls_status = multi_switchcap_status,
	.ls_enable_prepare = multi_switchcap_enable_prepare,
};
	...
	struct device_node *np)
{
	...
	di->gpio_btst_delay = of_get_named_gpio(np, "gpio_btst_delay", 0);
	hwlog_info("gpio_btst_delay=%d\n", di->gpio_btst_delay);

	if (!gpio_is_valid(di->gpio_btst_delay)) {
		hwlog_err("gpio_btst_delay is not valid\n");
		return  -EINVAL;
	}

	ret = gpio_request(di->gpio_btst_delay, "multi_sc_delay");
	if (ret) {
		hwlog_err("gpio_btst_delay request fail\n");
		return -EINVAL;
	}

	ret = gpio_direction_output(di->gpio_btst_delay, 1);
	if (ret) {
		hwlog_err("gpio_btst_delay set output fail\n");
...
static int multi_switchcap_probe(struct platform_device *pdev)
{
	...
	ret = sc_ops_register(&multi_switchcap_ops);
	if (ret) {
