...
static const struct adapter_protocol_device_data scp_protocol_device_data[] = {
...
static int scp_protocol_get_device_id(const char *str)
{
	...
	for (i = 0; i < ARRAY_SIZE(scp_protocol_device_data); i++) {
		if (!strncmp(str, scp_protocol_device_data[i].name,
			strlen(str)))
			return scp_protocol_device_data[i].id;
	}
...
static void scp_protocol_set_reg80_rw_error_flag(int flag)
{
	struct scp_protocol_dev *l_dev = NULL;
...
static int scp_protocol_reg_read(int reg, int *val, int num)
{
	...
	if ((l_ops->reg_multi_read) && (num == BYTE_TWO) &&
		...
		if (ret) {
			...
			hwlog_err("reg 0x%x multi_read fail\n", reg);
			return -1;
		}
...
static int scp_protocol_reg_write(int reg, const int *val, int num)
{
	int ret;
	struct scp_protocol_ops *l_ops = NULL;
...
static int scp_protocol_check_adp_type(int mode, int value)
{
	if (mode & ADAPTER_SUPPORT_SCP_B_LVC) {
		switch (value) {
		/* check 5v4.5a */
		case SCP_PROTOCOL_ADP_B_TYPE1_25W_IWATT:
		case SCP_PROTOCOL_ADP_B_TYPE1_25W_RICH1:
		case SCP_PROTOCOL_ADP_B_TYPE1_25W_RICH2:
	...
	if ((mode & ADAPTER_SUPPORT_SCP_B_LVC) &&
		...
		switch (value) {
		/* check 10v4a */
		case SCP_PROTOCOL_ADP_B_TYPE1_40W:
		case SCP_PROTOCOL_ADP_B_TYPE1_40W_1:
			return ADAPTER_TYPE_10V4A;
		/* check 20v3.25a-max */
		case SCP_PROTOCOL_ADP_B_TYPE1_65W_MAX:
			return ADAPTER_TYPE_20V3P25A_MAX;
		/* check 10v4a-bank */
		case SCP_PROTOCOL_ADP_B_TYPE1_40W_BANK:
			return ADAPTER_TYPE_10V4A_BANK;
		/* check 10v4a-car */
		case SCP_PROTOCOL_ADP_B_TYPE1_40W_CAR:
	...
	if (mode & ADAPTER_SUPPORT_SCP_B_SC) {
		switch (value) {
		/* check 10v2a */
		case SCP_PROTOCOL_ADP_B_TYPE1_20W:
			return ADAPTER_TYPE_10V2A;
		/* check 20v3.25a */
		case SCP_PROTOCOL_ADP_B_TYPE1_65W:
		case SCP_PROTOCOL_ADP_B_TYPE1_65W_1:
			return ADAPTER_TYPE_20V3P25A;
		/* check 10v2.25a */
		case SCP_PROTOCOL_ADP_B_TYPE1_22P5W:
...
static int scp_protocol_get_adp_type(int *type)
{
	...
	*type = scp_protocol_check_adp_type(mode, value[0]);
...
static int scp_protocol_detect_adapter_support_mode_by_0x80(int *mode)
{
	...
	scp_protocol_set_reg80_rw_error_flag(NO_RW_ERROR_FLAG);

	/* reg: 0x80 */
	if (scp_protocol_reg_read(SCP_PROTOCOL_ADP_TYPE1, value_a, BYTE_ONE)) {
		scp_protocol_set_reg80_rw_error_flag(RW_ERROR_FLAG);
...
static int scp_protocol_get_device_info(struct adapter_info *info)
{
	...
	if (scp_protocol_get_adp_type(&info->adp_type))
		return -1;
...
static int scp_protocol_get_chip_vendor_id(int *id)
{
	...
	if (scp_protocol_get_vendor_id(&vendor_id))
		return -1;

	switch (vendor_id) {
	case SCP_PROTOCOL_VENDOR_ID_RICHTEK:
		hwlog_info("adapter chip is richtek\n");
		*id = ADAPTER_CHIP_RICHTEK;
		break;
	case SCP_PROTOCOL_VENDOR_ID_WELTREND:
		hwlog_info("adapter chip is weltrend\n");
		*id = ADAPTER_CHIP_WELTREND;
		break;
	case SCP_PROTOCOL_VENDOR_ID_IWATT:
...
static int scp_protocol_set_init_data(struct adapter_init_data *data)
{
	...
	if (scp_protocol_set_output_voltage(data->init_voltage))
		return -1;
