...
static const struct adapter_protocol_device_data fcp_protocol_device_data[] = {
	...
	{ DEVICE_ID_BQ2560X, "bq2560x" },
	{ DEVICE_ID_RT9466, "rt9466" },
};
...
static int fcp_protocol_get_device_id(const char *str)
{
	...
	for (i = 0; i < ARRAY_SIZE(fcp_protocol_device_data); i++) {
		if (!strncmp(str, fcp_protocol_device_data[i].name,
			strlen(str)))
			return fcp_protocol_device_data[i].id;
	}
...
static int fcp_protocol_get_device_info(struct adapter_info *info)
{
	if (!info)
		return -1;
...
static int fcp_protocol_set_output_voltage(int volt)
{
	...
	if ((l_dev->dev_id != DEVICE_ID_BQ2560X) &&
		(l_dev->dev_id != DEVICE_ID_RT9466)) {
