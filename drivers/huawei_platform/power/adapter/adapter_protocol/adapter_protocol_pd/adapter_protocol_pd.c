...
static const struct adapter_protocol_device_data pd_protocol_device_data[] = {
	{ DEVICE_ID_SCHARGER_V600, "scharger_v600" },
	{ DEVICE_ID_FUSB3601, "fusb3601" },
	{ DEVICE_ID_RT1711H, "rt1711h" },
	{ DEVICE_ID_FUSB30X, "fusb30x" },
};
...
static int pd_protocol_get_device_id(const char *str)
{
	...
	for (i = 0; i < ARRAY_SIZE(pd_protocol_device_data); i++) {
		if (!strncmp(str, pd_protocol_device_data[i].name,
			strlen(str)))
			return pd_protocol_device_data[i].id;
	}
